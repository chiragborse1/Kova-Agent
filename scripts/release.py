...
VERSION_FILE = REPO_ROOT / "kova_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+kova-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "kova@nousresearch.com": "NousResearch",
    ...
    "kova@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaKova.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (KOVA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (KOVA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: kova-eval community link)
    ...
    "KovaAgent26@gmail.com": "KovaAgent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+KovaAgent26@users.noreply.github.com": "KovaAgent26",
    ...
    "vigo@kova": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local kova runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level KOVA_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live kova
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"kova-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "kova@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/chiragborse1/Kova-Agent",
    ...
    lines.append(f"# Kova Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Kova Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Kova Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Kova Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Kova Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Kova Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Kova Agent v{new_version} ({calver_date})' "
