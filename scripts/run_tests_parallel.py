...
Environment:
    ...
    KOVA_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        nousresearch/kova-agent:test image, via
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/kova-agent/kova-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("KOVA_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $KOVA_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
