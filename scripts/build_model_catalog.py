#!/usr/bin/env python3
"""Build the kova model Catalog — a centralized JSON manifest of curated models.
...
Kova CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a kova release.
...
# Ensure KOVA_HOME is set for imports that touch it at module level.
os.environ.setdefault("KOVA_HOME", os.path.join(os.path.expanduser("~"), ".kova"))

from kova_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            ...
            "docs": "https://kova-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
