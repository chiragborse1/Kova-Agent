...
#   if [ "$KOVA_NODE_AVAILABLE" = true ]; then ...; fi
#
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$KOVA_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $KOVA_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$KOVA_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$KOVA_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $KOVA_NODE_TARGET_MAJOR..."
    proto install node "$KOVA_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $KOVA_NODE_TARGET_MAJOR..."
    nvm install "$KOVA_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$KOVA_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${KOVA_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${KOVA_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${KOVA_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${KOVA_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${KOVA_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $KOVA_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
...
# ---------------------------------------------------------------------------
# Heal a broken kova-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
ensure_node() {
    ...
    _nb_try_fnm   && { KOVA_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { KOVA_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { KOVA_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { KOVA_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { KOVA_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { KOVA_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $KOVA_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
