...
# Python-driven tool session (e.g. kova terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:chiragborse1/Kova-Agent.git"
REPO_URL_HTTPS="https://github.com/chiragborse1/Kova-Agent.git"
KOVA_HOME="${KOVA_HOME:-$HOME/.kova}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${KOVA_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/kova-agent, command at /usr/local/bin/kova,
#   data still at /root/.kova (KOVA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$KOVA_HOME/.no-bundled-skills so future"
            echo "                   'kova update' runs never inject bundled skills either"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, kova installs the code under"
            ...
            echo "                   Installs optional deps + runs kova setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `kova update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (kova-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Kova Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install kova command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `kova` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KOVA_HOME/kova-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KOVA_HOME/kova-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/kova-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KOVA_HOME/kova-agent — then preserve it)
#
# Always no-op when the user set --dir or $KOVA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in KOVA_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KOVA_HOME/kova-agent/.git" ]; then
            INSTALL_DIR="$KOVA_HOME/kova-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/kova-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/kova-agent"
        ...
        # /usr/local/bin/kova wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/kova"
        log_info "  Data:    $KOVA_HOME (unchanged)"
...
# Point a kova-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $KOVA_HOME/node/bin — which is NOT on
...
# kova's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no kova-managed npm, so calling it on
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://kova-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # kova owns its own uv at $KOVA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (KOVA_cli/managed_uv.py) looks in the same
    # place, so install.sh and `kova update` stay in sync.
    local _managed_uv="$KOVA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $KOVA_HOME/bin ..."
    mkdir -p "$KOVA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/kova-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/kova-uv-installer.$$.sh")"
    ...
    # directly into $KOVA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$KOVA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the kova-
...
check_node() {
    ...
    # Repair pre-existing kova-managed installs where `npm install -g` lands
    ...
    if [ -x "$KOVA_HOME/node/bin/node" ] && node_satisfies_build "$("$KOVA_HOME/node/bin/node" --version)"; then
        export PATH="$KOVA_HOME/node/bin:$PATH"
        log_success "Node.js $("$KOVA_HOME/node/bin/node" --version) found (kova-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing kova-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    mkdir -p "$KOVA_HOME"
    mv "$extracted_dir" "$KOVA_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$KOVA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$KOVA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$KOVA_HOME/node/bin/npx"  "$node_link_dir/npx"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. kova install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Kova Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `kova update` path
                # (#4735).
                ...
                local stash_name
                stash_name="kova-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if kova behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up kova command..."

    if [ "$USE_VENV" = true ]; then
        KOVA_BIN="$INSTALL_DIR/venv/bin/kova"
    else
    ...
    if [ ! -x "$KOVA_BIN" ]; then
        log_warn "kova entry point not found at $KOVA_BIN"
    ...
    # Create a user-facing shim for the kova command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $KOVA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$KOVA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/kova"
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "kova not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Kova Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Kova Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Kova Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so kova works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # This MUST match DEFAULT_SOUL_MD in KOVA_cli/default_soul.py — the
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$KOVA_HOME/.no-bundled-skills" 2>/dev/null || true
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Kova Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "kova will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Kova Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (kova --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `kova update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'kova setup' after install."
        return 0
    fi
    ...
    # Run kova setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m KOVA_cli.main setup < /dev/tty
    else
        python -m KOVA_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for kova to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$KOVA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'kova whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'Kova Gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $KOVA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $KOVA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: Kova Gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: Kova Gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
        ...
        log_info "Skipped. Start the gateway later with: Kova Gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $KOVA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $KOVA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $KOVA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}kova${NC}              Start chatting"
    echo -e "   ${GREEN}kova setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}kova config${NC}       View/edit configuration"
    echo -e "   ${GREEN}kova config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}Kova Gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}kova update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'kova' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'kova' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'kova' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$KOVA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$KOVA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$KOVA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$KOVA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$KOVA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up kova for pip install"
    ...
    KOVA_CMD="$(command -v kova 2>/dev/null || echo "")"
    if [ -n "$KOVA_CMD" ]; then
        log_info "Running kova setup..."
        "$KOVA_CMD" setup
    else
        log_warn "kova command not found on PATH"
        log_info "Try: python -m KOVA_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`kova` rename
...
# _purge_electron_build_cache() used by `kova desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the kova-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `kova desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/kova" ]; then
            app="$desktop_dir/release/linux-unpacked/kova"
        elif [ -x "$desktop_dir/release/linux-unpacked/kova" ]; then
            app="$desktop_dir/release/linux-unpacked/kova"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/kova.app" \
            "$desktop_dir/release/mac/kova.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("kova is damaged and can't be opened").
    ...
    # checkout stays clean for the next `kova update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the kova-managed Node
            # provisioned during prerequisites/node-deps (at $KOVA_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $KOVA_HOME.
    # $KOVA_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'kova update' on this host install.
