...
# Bootstrap Open WebUI against Kova Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.kova/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-kova.sh
...
#   KOVA_API_PORT=8642
#   KOVA_API_HOST=127.0.0.1
#   KOVA_API_MODEL_NAME='Kova Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Kova Agent WebUI}"
...
KOVA_ENV_FILE="${KOVA_ENV_FILE:-$HOME/.kova/.env}"
KOVA_API_PORT="${KOVA_API_PORT:-8642}"
KOVA_API_HOST="${KOVA_API_HOST:-127.0.0.1}"
KOVA_API_CONNECT_HOST="${KOVA_API_CONNECT_HOST:-127.0.0.1}"
KOVA_API_MODEL_NAME="${KOVA_API_MODEL_NAME:-Kova Agent}"
KOVA_API_BASE_URL="http://${KOVA_API_CONNECT_HOST}:${KOVA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-kova.sh"
LOG_DIR="$HOME/.kova/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$KOVA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$KOVA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.kova.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.kova</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.kova"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.kova"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-kova.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Kova Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-kova.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.kova/logs/openwebui.log
StandardError=append:%h/.kova/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-kova.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$KOVA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring kova API server is configured...'
  upsert_env API_SERVER_ENABLED true "$KOVA_ENV_FILE"
  upsert_env API_SERVER_HOST "$KOVA_API_HOST" "$KOVA_ENV_FILE"
  upsert_env API_SERVER_PORT "$KOVA_API_PORT" "$KOVA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$KOVA_API_MODEL_NAME" "$KOVA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$KOVA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Kova Gateway so API server settings take effect...'
  Kova Gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${KOVA_API_CONNECT_HOST}:${KOVA_API_PORT}/health" >/dev/null; then
    log 'kova API server did not answer on the first check. Trying to start gateway in the background...'
    nohup Kova Gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${KOVA_API_CONNECT_HOST}:${KOVA_API_PORT}/health" >/dev/null
  ...
  log "kova API endpoint: ${KOVA_API_BASE_URL}"
