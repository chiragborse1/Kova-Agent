# ============================================================================
# Kova Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable kova.exe.
    #
    # Why opt-in:
    #   * kova-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched kova.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live kova.exe
    ...
    #     `kova desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:chiragborse1/Kova-Agent.git"
$RepoUrlHttps = "https://github.com/chiragborse1/Kova-Agent.git"
$PythonVersion = "3.11"
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under kova-Setup.exe
...
function Install-Git {
    ...
    which kova uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\kova\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    kova needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the kova copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\kova\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``KOVA_GIT_BASH_PATH`` (User scope) so kova can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent kova features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  kova needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``KOVA_GIT_BASH_PATH`` (User env scope) so kova can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("KOVA_GIT_BASH_PATH", $candidate, "User")
            $env:KOVA_GIT_BASH_PATH = $candidate
            Write-Info "Set KOVA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- kova may not find Git Bash."
...
function Test-Node {
    ...
    Write-Info "Installing kova-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\kova-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\kova-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `kova
                    # update` path (#4735).
                    ...
                    $stashName = "kova-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if kova behaves unexpectedly."
                        } else {
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/chiragborse1/Kova-Agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/chiragborse1/Kova-Agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/chiragborse1/Kova-Agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\kova-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\kova-agent-extract"
    ...
    # next `kova update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under kova-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running kova process.
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # The launcher CLI (kova.exe) plus its child tree.
            & taskkill /F /T /IM kova.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM kova.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m KOVA_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not kova.exe.
            ...
            # user (see KOVA_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `kova.exe` exists
...
"@ 2>$null
    ...
    # isn't set, leaving venv\ empty and kova.exe broken with
    ...
    # here, not 30 seconds later when the user runs `kova`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `kova dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `kova dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `kova dashboard` should now work."
            } else {
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and kova's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (kova-Setup.exe runs each -Stage NAME
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\kova-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\kova-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\kova-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `kova` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    ...
    # The --dir mode produces an unpacked kova.exe in
    ...
    # are NOT trying to work around). The kova icon + product name are
    # stamped onto kova.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\kova-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\kova.exe",
        "$desktopDir\release\win-arm64-unpacked\kova.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no kova.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The kova icon + identity are stamped onto kova.exe by the
    ...
    #    kova.exe. We deliberately do NOT point them at `kova desktop`: that
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'kova setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m KOVA_cli.main setup
    } else {
        python -m KOVA_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'kova whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: Kova Gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: Kova Gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: Kova Gateway"
    }
}
...
if ($IncludeDesktop) {
    ...
    # (kova-Setup.exe), never via the irm|iex CLI one-liner.
