#!/usr/bin/env python3
"""Drive the kova TUI under KOVA_DEV_PERF and summarize the pipeline.
...
bypasses the kova_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.
...
or produced no perf data (suggests KOVA_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("KOVA_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("KOVA_PERF_LOG", str(get_kova_home() / "perf.log")))
DEFAULT_STATE_DB = get_kova_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — KOVA_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["KOVA_DEV_PERF"] = "1"
    env["KOVA_DEV_PERF_MS"] = str(args.threshold_ms)
    env["KOVA_DEV_PERF_LOG"] = str(log)
    env["KOVA_TUI_RESUME"] = sid
    ...
    node = os.environ.get("KOVA_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="KOVA_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "kova-ink" / "src"
