...
# ``sessions_dir / f"{session_id}.json"`` in kova_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `kova tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `kova tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_kova_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.kova/state.db and are shown by `kova sessions list` and "
