...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{KOVA_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``KOVA_HOME`` is a **durable** store — on kova
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``Kova Gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting KOVA_HOME."""
    base = home if home is not None else get_kova_home()
...
def write_drain_request(
    *, principal: str = "drain-control", home: Optional[Path] = None
    ...
    later survives a machine restart on the durable KOVA_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (KOVA_HOME is a
    durable Fly volume on kova Cloud) and the lifecycle action that triggered
