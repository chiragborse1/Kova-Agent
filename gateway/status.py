...
The PID file lives at ``{KOVA_HOME}/gateway.pid``.  KOVA_HOME defaults to
``~/.kova`` but can be overridden via the environment variable.  This means
separate KOVA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting KOVA_HOME."""
    home = get_kova_home()
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Kova Gateway lifecycle subcommand from a command line.
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `Kova Gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating kova-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Kova Gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a KOVA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.kova`` or ``$KOVA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``KOVA_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit KOVA_HOME= on the argv. KOVA_HOME is usually
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `kova status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``KOVA_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    KOVA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different KOVA_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``Kova Gateway stop`` on Windows would be misclassified as an
