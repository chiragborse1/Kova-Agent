...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_kova_home() / "sessions")
    ...
    # gateway behaves exactly as before — single KOVA_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.kova/config.yaml (primary user-facing config)
    3. ~/.kova/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_kova_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through kova_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``kova config set gateway.streaming.*``
