...
# Audio file extensions kova recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through kova' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. kova-created Telegram private-chat
...
from kova_constants import get_default_kova_root, get_kova_dir, get_kova_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.kova/.env manually."
)
...
# Default location: {KOVA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_kova_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_kova_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_kova_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_kova_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_kova_dir("cache/screenshots", "browser_screenshots")
_KOVA_HOME = get_kova_home()
_KOVA_ROOT = get_default_kova_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "KOVA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "KOVA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "KOVA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "KOVA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _KOVA_HOME / "image_cache",
    _KOVA_HOME / "audio_cache",
    _KOVA_HOME / "video_cache",
    _KOVA_HOME / "document_cache",
    _KOVA_HOME / "browser_screenshots",
    ...
    _KOVA_HOME / "cache" / "images",
    _KOVA_HOME / "cache" / "audio",
    _KOVA_HOME / "cache" / "videos",
    _KOVA_HOME / "cache" / "documents",
    _KOVA_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active kova profile and shared kova root both contain control
    ...
    # KOVA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.kova
    ...
    for KOVA_root in (_KOVA_HOME, _KOVA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(KOVA_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and kova
    secrets (``~/.kova/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``KOVA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    kova-managed cache, under an operator-allowlisted root
    (``KOVA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the kova root (~/.kova/.env,
    ...
    # ``MEDIA:~/.kova/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?kova\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+kova[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``KOVA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # kova commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("KOVA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "KOVA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "KOVA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: kova sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.kova/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from kova_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          KOVA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          KOVA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          KOVA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("KOVA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("KOVA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("KOVA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _KOVA_run_generation is set on
