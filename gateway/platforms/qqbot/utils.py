...
def _get_KOVA_version() -> str:
    """Return the kova-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; kova/<KOVA_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; kova/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    KOVA_version = _get_KOVA_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; kova/{KOVA_version})"
