...
There are two ways to add a platform to the Kova Gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.kova/plugins/` (or under `plugins/platforms/`
...
**zero changes to core kova code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `Kova Gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `KOVA_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the kova core.
...
And add it to the `kova-gateway` composite:

```python
"kova-gateway": {
    "includes": [..., "kova-your-platform"]
}
```
...
## 12. Status Display (`KOVA_cli/status.py`)
...
## 13. Gateway Setup Wizard (`KOVA_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ KOVA_cli/ toolsets.py \
  --include="*.py" -l | sort -u
