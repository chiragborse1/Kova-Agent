...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-kova-Session-Id header; opt-in long-term memory scoping via X-kova-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-kova-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists kova-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible kova sessions
- POST /api/sessions               — create an empty kova session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to kova-agent
...
def _KOVA_version() -> str:
    """Return the kova-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``kova_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_kova_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted KOVA_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # kova_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same kova session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through kova-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-kova-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-kova-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-kova-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-kova-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-kova-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``kova sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the kova-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-kova-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "kova-agent", "version": _KOVA_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return kova-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every kova version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side kova AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-kova-Session-Id",
                "session_key_header": "X-kova-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted kova sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty kova session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "kova.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "kova.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "kova.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "kova.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-kova-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-kova-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "kova.session.chat.completion",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-kova-Session-Key.  This
        # is independent of X-kova-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-kova-Session-Id.
        ...
        provided_session_id = request.headers.get("X-kova-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-kova-Session-Id rejected: "
            ...
            # conversation map to the same kova session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``kova.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-kova-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-kova-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-kova-Completed"] = "false"
            response_headers["X-kova-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + kova-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-kova-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-kova-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-kova-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-kova-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: kova.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-kova-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-kova-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-kova-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-kova-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "kova.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local kova-Relay
            ...
            # the June 2026 kova-0day hardening (an 8-char key dispatching
            ...
            # that is the exact surface the kova-0day campaign abused to write
            # ~/.kova/config.yaml and plant persistence. Sandboxing (Docker /
            ...
            if is_network_accessible(self._host):
                try:
                    from kova_cli.config import load_config as _load_cfg
                    _backend = (
