...
# Inbound media cache lives under the user's kova dir so it survives
# restarts and gateway reloads — same convention the Baileys bridge uses.
_INBOUND_MEDIA_CACHE = Path(get_kova_dir("platforms/whatsapp_cloud/media", "whatsapp_cloud/media"))
...
def check_whatsapp_cloud_requirements() -> bool:
    ...
    for Graph API calls (outbound). Both ship with kova-agent's default
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        a generic audio attachment. kova TTS produces MP3, so we try
    ...
    async def _build_message_event_from_cloud(
        ...
        """Convert a Cloud-API message object into a kova MessageEvent.
