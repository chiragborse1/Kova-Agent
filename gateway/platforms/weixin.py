...
Connects Kova Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(KOVA_HOME: str) -> Path:
    path = Path(KOVA_HOME) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(KOVA_HOME: str, account_id: str) -> Path:
    return _account_dir(KOVA_HOME) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(KOVA_HOME, account_id)
...
def load_weixin_account(KOVA_HOME: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, KOVA_HOME: str):
        self._root = _account_dir(KOVA_HOME)
...
def _sync_buf_path(KOVA_HOME: str, account_id: str) -> Path:
    return _account_dir(KOVA_HOME) / f"{account_id}.sync.json"


def _load_sync_buf(KOVA_HOME: str, account_id: str) -> str:
    path = _sync_buf_path(KOVA_HOME, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(KOVA_HOME: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(KOVA_HOME, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native kova adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._token_store = ContextTokenStore(KOVA_HOME)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(KOVA_HOME, self._account_id)
            if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach kova regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._KOVA_HOME, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._KOVA_HOME, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"kova-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"kova-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_kova_home()))
