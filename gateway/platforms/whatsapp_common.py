...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Kova Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to KOVA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/kova), this function
    mirrors the bridge source to a writable KOVA_HOME location and returns that
    ...
    # Install dir is read-only, mirror to KOVA_HOME if needed
    if KOVA_HOME_bridge.exists():
        return KOVA_HOME_bridge

    # Mirror the bridge source to KOVA_HOME
    try:
