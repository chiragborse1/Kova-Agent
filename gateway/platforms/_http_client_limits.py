...
Override via ``KOVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY`` /
``KOVA_GATEWAY_HTTPX_MAX_KEEPALIVE`` env vars when tuning under load.
"""
...
def platform_httpx_limits() -> "httpx.Limits | None":
    ...
    keepalive_expiry = _env_float(
        "KOVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", _DEFAULT_KEEPALIVE_EXPIRY_S
    )
    max_keepalive = _env_int(
        "KOVA_GATEWAY_HTTPX_MAX_KEEPALIVE", _DEFAULT_MAX_KEEPALIVE
    )
