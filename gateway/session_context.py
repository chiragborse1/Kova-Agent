"""
Session-scoped context variables for the Kova Gateway.

Replaces the previous ``os.environ``-based session state
(``KOVA_SESSION_PLATFORM``, ``KOVA_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["KOVA_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("KOVA_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("KOVA_SESSION_PLATFORM", "")

    # after
    from gateway.session_context import get_session_env
    platform = get_session_env("KOVA_SESSION_PLATFORM", "")
"""
...
_SESSION_PLATFORM: ContextVar = ContextVar("KOVA_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("KOVA_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("KOVA_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("KOVA_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("KOVA_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("KOVA_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("KOVA_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("KOVA_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("KOVA_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("KOVA_SESSION_MESSAGE_ID", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("KOVA_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("KOVA_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("KOVA_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("KOVA_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "KOVA_SESSION_PLATFORM": _SESSION_PLATFORM,
    "KOVA_SESSION_SOURCE": _SESSION_SOURCE,
    "KOVA_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "KOVA_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "KOVA_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "KOVA_SESSION_USER_ID": _SESSION_USER_ID,
    "KOVA_SESSION_USER_NAME": _SESSION_USER_NAME,
    "KOVA_SESSION_KEY": _SESSION_KEY,
    "KOVA_SESSION_ID": _SESSION_ID,
    "KOVA_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "KOVA_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "KOVA_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "KOVA_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``KOVA_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("KOVA_SESSION_ID")`` with an ``os.environ`` fallback,
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``KOVA_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("KOVA_SESSION_*", default)``.
