...
Module-level run.py helpers a handler needs (``_KOVA_HOME``,
...
from kova_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach kova commands.
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from kova_cli.kanban import run_slash
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _KOVA_HOME / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _KOVA_HOME / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from kova_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _KOVA_HOME, _load_gateway_config
        import yaml
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from kova_cli.context_switch_guard import (
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — kova default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_kova_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from kova_cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _KOVA_HOME / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _KOVA_HOME, _platform_config_key
        import yaml
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.kova/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``kova skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _KOVA_HOME
        from kova_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `kova skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.kova/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _KOVA_HOME, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _KOVA_HOME, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _KOVA_HOME, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from kova_cli.session_listing import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``kova update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``kova update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _KOVA_HOME, _resolve_KOVA_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Kova Agent')}"
        ...
        if not KOVA_cmd:
            return t("gateway.update.KOVA_cmd_not_found")
        ...
        # Spawn `kova update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `kova update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # KOVA_cmd is a list of argv parts we can pass directly
                ...
                KOVA_cmd_str = " ".join(shlex.quote(part) for part in KOVA_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {KOVA_cmd_str} update --gateway"
