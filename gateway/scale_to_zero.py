...
Design constraints (decisions.md):
  - Per-instance enable is gated SOLELY by the NAS "Labs" toggle, carried to the
    gateway as the ``KOVA_SCALE_TO_ZERO`` env stamp (D11/Q8=A). NOT a user
...
SCALE_TO_ZERO_ENV = "KOVA_SCALE_TO_ZERO"
...
def scale_to_zero_enabled(environ: Optional[dict] = None) -> bool:
    """Whether the per-instance Labs toggle is on (the KOVA_SCALE_TO_ZERO stamp).
