        ...
        if KOVA_HOME_str:
            ...
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(KOVA_HOME_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded kova-agent but never re-ran ``kova setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../kova-gateway.service"
    ...
    # that's the common case for kova.
    ...
    # constant in kova_cli/gateway.py.
    headroom = 30.0
