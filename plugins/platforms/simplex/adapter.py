"""SimpleX Chat platform adapter (kova plugin).
...
This adapter ships as a kova platform plugin under
``plugins/platforms/simplex/``. The kova plugin loader scans the
...
discoverable and ``kova setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside kova — no
