...
Allows users to interact with kova by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "kova-agent" "version" "{_KOVA_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `kova config show` already read. Without the
    ...
    def _send_email(
        ...
        msg_id = f"<kova-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# kova_cli/gateway.py, and the _send_email dispatch in
