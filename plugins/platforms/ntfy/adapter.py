"""ntfy platform adapter (kova plugin).
...
``plugins/platforms/ntfy/``. The kova plugin loader scans the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a kova dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `Kova Gateway status` without
