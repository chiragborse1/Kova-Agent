"""
IRC Platform Adapter for Kova Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Kova Agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kova-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Kova Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Kova Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: KOVA_, KOVA_1, KOVA_2...
...
def check_requirements() -> bool:
    ...
    # kova setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `Kova Gateway setup` flow for the IRC platform.

    Lazy-imports ``kova_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect kova to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #kova — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.kova/.env")
    print_info("Restart the gateway for changes to take effect: Kova Gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``kova cron`` running as a
    separate process from ``Kova Gateway``).  Without this hook,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kova-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "kova-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Kova Agent (cron)")
