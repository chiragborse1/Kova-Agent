...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer kova-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_kova_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `Kova Gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run kova whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `kova whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `kova whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `kova update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".kova-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # kova-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `kova update`, so without this check it
            ...
            # hardcodes ~/.kova/{image,audio,document}_cache, which diverges
            # under KOVA_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["KOVA_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["KOVA_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["KOVA_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
...
# dict in kova_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in kova_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``kova setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via kova_cli.gateway.get_env_value (not os.getenv) so setup-status
