"""
``kova photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other kova
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``kova photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.kova/.env,
    ...
    print("  Start the gateway:  Kova Gateway start")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.kova/.env.
...
#
# `Kova Gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``kova photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `Kova Gateway setup` wizard."""
