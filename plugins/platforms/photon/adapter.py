"""
Photon Spectrum (iMessage) platform adapter for Kova Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a kova dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `kova photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `kova setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use kova defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-kova-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any kova checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `kova photon setup`)"
            )
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-kova-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the kova plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `kova photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: kova photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `Kova Gateway setup` alongside every other
    ...
    # Register CLI subcommands — `kova photon ...`
