...
Credential storage mirrors every other kova channel:

    * runtime SDK creds  -> ``~/.kova/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.kova/auth.json`` under
...
# registers kova as its own client_id.
...
# Default name of the project kova provisions for the operator.
DEFAULT_PROJECT_NAME = "Kova Agent"
...
def _auth_json_path() -> Path:
    """Resolve ``~/.kova/auth.json`` honouring the active kova profile."""
    try:
        from kova_constants import get_kova_home
        return Path(get_kova_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.kova")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.kova/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.kova/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.kova/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `kova photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `kova photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `kova photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `kova photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `kova photon setup`)"
