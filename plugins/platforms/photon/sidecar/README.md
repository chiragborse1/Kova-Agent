...
Small Node helper that bridges Kova Agent to Photon's Spectrum SDK
(`spectrum-ts`).  kova is Python; Photon has no public HTTP
...
The kova plugin's `kova photon setup` command runs `npm install`
here automatically.
