...
This plugin connects Kova Agent to iMessage (and other Spectrum
...
TypeScript-only, kova runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other kova channel,
...
Run `kova photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.kova/.env` (the same place every other
...
Management metadata lives in `~/.kova/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `kova photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `kova photon status`, a DM and a group roundtrip,
