...
and injects content-free wake hints into kova' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # kova only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active kova turn."""
        if not self._message_handler:
            return
