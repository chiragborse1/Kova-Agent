...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "KOVA_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "KOVA_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. kova-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on kova' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        kova must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `kova update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other kova or OpenClaw instance is running "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the kova process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("KOVA_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("KOVA_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("KOVA_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("KOVA_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("KOVA_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            try:
                ...
                from kova_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  kova defaults to 60 to
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``kova update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``kova model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from kova_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from kova_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.kova/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".kova" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@KOVA_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several kova bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the kova inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the kova inbox (#52363).
        if self._is_own_message(message):
            return False
...
# kova_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in kova_cli/gateway.py.
    """
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kova-agent[telegram]'",
