...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.kova/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Kova Gateway
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your kova session")
        ...
        @tree.command(name="status", description="Show kova session status")
        ...
        @tree.command(name="stop", description="Stop the running Kova Agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.kova/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Kova Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Kova Gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a kova session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to kova in the thread",
        ...
        # kova_cli/commands.py automatically appear as Discord slash
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from kova_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new kova session in the thread
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by kova: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
...
def _component_check_auth(
    ...
    the pairing store (``kova pairing approve``).
    ...
    # so users approved via ``kova pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``kova update`` prompts.
...
# is not in this process (e.g. ``kova cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``kova_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kova-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # kova_cli/setup.py::_setup_discord function.  Same shape as Teams.
