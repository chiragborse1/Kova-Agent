...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_kova_home() / "slack_tokens.json"
        ...
        try:
            ...
            # kova does not currently need to route them into the agent loop.
            ...
            # /model work directly without /kova prefix). A single regex
            ...
            # manifest (see `kova slack manifest`). In Socket Mode, Slack
            ...
            from kova_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/kova$")

            @self._app.command(_slash_pattern)
            async def handle_KOVA_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: kova handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own kova session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@kova`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "KOVA_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "KOVA_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "KOVA_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "KOVA_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "KOVA_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "KOVA_confirm_once": "once",
            "KOVA_confirm_always": "always",
            "KOVA_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "KOVA_approve_once": "once",
            "KOVA_approve_session": "session",
            "KOVA_approve_always": "always",
            "KOVA_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/kova <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/kova
        ...
        if slash_name in {"kova", ""}:
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/kova   ``).
        ...
        # "Running /cmd…" ack shown by handle_KOVA_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/kova <question>" must produce public replies so
...
# ``kova_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``kova_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="kova",
                bot_description="Your Kova Agent on Slack",
            )
            ...
            print_info(
                "   Re-run `kova slack manifest --write` anytime to refresh after "
                "kova adds new commands."
            )
        except Exception as e:
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``kova_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kova-agent[slack]'",
        # Interactive setup wizard — replaces kova_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in kova_cli/gateway.py.
