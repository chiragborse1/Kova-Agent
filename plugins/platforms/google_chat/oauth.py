...
- Per-user tokens (keyed by sender email):
    ``${KOVA_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${KOVA_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${KOVA_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${KOVA_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${KOVA_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's KOVA_HOME helper so the token follows the user's
# profile (e.g. tests can override via KOVA_HOME=/tmp/...).
try:
    from kova_constants import display_kova_home, get_kova_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where kova_constants isn't importable
...
def _KOVA_HOME() -> Path:
    """Resolve KOVA_HOME at call time (NOT module import).

    Tests and ``KOVA_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.kova/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _KOVA_HOME() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _KOVA_HOME() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _KOVA_HOME() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _KOVA_HOME() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _KOVA_HOME() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install kova-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'kova-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into KOVA_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_kova_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_kova_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for kova (native attachment delivery)"
    )
