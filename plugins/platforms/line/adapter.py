"""
LINE Messaging API platform adapter for Kova Agent.
...
to Kova Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``KOVA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            KOVA_HOME = Path(get_kova_home()).resolve()
        except Exception:
            KOVA_HOME = Path.home().joinpath(".kova").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``kova status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``kova status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kova setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.kova/.env`` via ``kova_cli.config``.
    """
    print()
    ...
    try:
        from kova_cli.config import get_env_var, set_env_var
    except ImportError:
        print("kova_cli.config not available; set LINE_* vars manually in ~/.kova/.env")
        return
