...
def _xai_user_agent() -> str:
    try:
        ...
        return kova_xai_user_agent()
    except Exception:
        return "kova-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``kova_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `kova auth add xai-oauth` "
