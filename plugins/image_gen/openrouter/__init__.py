"""OpenRouter-compatible image generation backend.

Speaks the same OpenAI-style ``/chat/completions`` image-generation protocol:
send ``modalities: ["image", "text"]`` with an image-output model (e.g.
...
("nano-banana") does, so the provider advertises image-to-image support.
"""
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``kova tools``) → the quality-first default chain.
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `kova tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
