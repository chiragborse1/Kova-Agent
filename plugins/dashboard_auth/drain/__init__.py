...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    KOVA_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``KOVA_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``KOVA_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("KOVA_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
            "KOVA_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent "
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"KOVA_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
    ...
    try:
        from kova_cli.dashboard_auth.token_auth import register_token_route
