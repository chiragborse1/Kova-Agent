...
A standards-compliant OpenID Connect Relying Party for the ``kova dashboard``
...
:class:`~kova_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``kova_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~kova_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    KOVA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — kova does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``KOVA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "KOVA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "KOVA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("KOVA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(KOVA_DASHBOARD_OIDC_ISSUER + KOVA_DASHBOARD_OIDC_CLIENT_ID) "
