...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      KOVA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      KOVA_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      KOVA_DASHBOARD_BASIC_AUTH_SECRET
      KOVA_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "KOVA_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or KOVA_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "KOVA_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "KOVA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "KOVA_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "KOVA_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "KOVA_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # KOVA_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "KOVA_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
