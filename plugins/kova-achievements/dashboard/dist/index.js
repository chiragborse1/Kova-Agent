(function () {
  "use strict";
  // kova-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/kova-achievements (MIT).
  // Bundled into kova-agent. Upstream repo remains the staging ground for new
  ...
  if (!SDK || !window.__KOVA_PLUGINS__) return;
  ...
  function api(path, options) {
    ...
    // BOTH dashboard modes: loopback (X-Kova-Session-Token header) and
    // gated OAuth (KOVA_session_at cookie via credentials:'include').
    // Hand-rolling fetch + reading window.__KOVA_SESSION_TOKEN__ directly
    ...
    const url = "/api/plugins/kova-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("Kova Agent  ·  kova-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "kova-achievement-" + (achievement.id || "badge") + ".png";
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in Kova Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@NousResearch · https://kova-agent.nousresearch.com";
    }
  ...
  window.__KOVA_PLUGINS__.register("kova-achievements", AchievementsPage);
})();
