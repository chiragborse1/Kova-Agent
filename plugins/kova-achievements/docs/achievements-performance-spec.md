...
Owner: kova-achievements plugin
...
### `GET /api/plugins/kova-achievements/achievements`
...
### `POST /api/plugins/kova-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/kova-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/kova-achievements/overview`
...
- Persist plugin data under:
  - `~/.kova/plugins/kova-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.kova/plugins/kova-achievements/scan_checkpoint.json`
...
- `~/.kova/plugins/kova-achievements/`
