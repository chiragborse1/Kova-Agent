"""disk_cleanup — ephemeral file cleanup for Kova Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under KOVA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KOVA_HOME and /tmp/kova-*
Never touches: ~/.kova/logs/ or any system directory.
"""
...
try:
    ...
    def get_kova_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KOVA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".kova").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KOVA_HOME/logs/``."""
    return get_kova_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KOVA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KOVA_HOME or ``/tmp/kova-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    KOVA_HOME = get_kova_home()
    try:
        path.resolve().relative_to(KOVA_HOME)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/kova-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("kova-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "kova-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $KOVA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so KOVA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        KOVA_HOME = get_kova_home()
        for parent in ("cron", "cronjobs"):
            base = KOVA_HOME / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KOVA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KOVA_HOME, but never recurse into known
    # durable state trees.  Some installs place the kova checkout, venv,
    # and desktop build under KOVA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    KOVA_HOME = get_kova_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in KOVA_HOME.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    KOVA_HOME = get_kova_home()
    try:
        rel = path.resolve().relative_to(KOVA_HOME)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "kova-agent",
        }:
            return None
        ...
        # Path isn't under KOVA_HOME (e.g. /tmp/kova-*) — fall through.
        pass
