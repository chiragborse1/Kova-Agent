...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``KOVA_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$KOVA_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to KOVA_HOME and /tmp/kova-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside KOVA_HOME): "
            f"{path_arg}"
        )
