...
Auto-tracks and cleans up ephemeral files created during Kova Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$KOVA_HOME` and `/tmp/kova-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KOVA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `KOVA_HOME` or `/tmp/kova-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$KOVA_HOME/disk-cleanup/` is itself excluded
- `$KOVA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
