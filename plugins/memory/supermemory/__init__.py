...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=kova"
...
def _load_supermemory_config(KOVA_HOME: str) -> dict:
    config = _default_config()
    config_path = Path(KOVA_HOME) / "supermemory.json"
...
def _save_supermemory_config(values: dict, KOVA_HOME: str) -> None:
    config_path = Path(KOVA_HOME) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "kova"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "kova", **(metadata or {})}
...
def _resolve_container_tag_for_setup(KOVA_HOME: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(KOVA_HOME)
...
def _probe_supermemory_connection(api_key: str, KOVA_HOME: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(KOVA_HOME)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(KOVA_HOME, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `kova memory setup`.
        # All other options are documented for $KOVA_HOME/supermemory.json
    ...
    def save_config(self, values, KOVA_HOME):
        ...
        _save_supermemory_config(sanitized, KOVA_HOME)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        api_key = os.environ.get("SUPERMEMORY_API_KEY", "")
        status = _probe_supermemory_connection(api_key, KOVA_HOME)
    ...
    def post_setup(self, KOVA_HOME: str, config: dict) -> None:
        ...
        from kova_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(KOVA_HOME) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from kova_constants import get_kova_home
        self._KOVA_HOME = kwargs.get("KOVA_HOME") or str(get_kova_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._KOVA_HOME)
