...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip kova slash-skill scaffolding before sending content to OpenViking.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this kova run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new kova session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in kova only", "write values only to kova .env"),
    ...
    _print_openviking_ready("Connection saved to kova .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this kova run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of kova' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert kova canonical messages into OpenViking batch payloads."""
