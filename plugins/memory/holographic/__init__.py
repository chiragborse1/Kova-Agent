"""kova-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $KOVA_HOME/config.yaml (profile-scoped):
  plugins:
    kova-memory-store:
      db_path: $KOVA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_kova_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $KOVA_HOME in user-supplied paths so config values like
        # "$KOVA_HOME/memory_store.db" or "~/.kova/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$KOVA_HOME", _KOVA_HOME)
            db_path = db_path.replace("${KOVA_HOME}", _KOVA_HOME)
