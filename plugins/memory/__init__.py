...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with kova-agent)
2. User-installed providers: ``$KOVA_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_KOVA_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_KOVA_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_KOVA_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$KOVA_HOME/plugins/`` or None if unavailable."""
    try:
        from kova_constants import get_kova_home
        d = get_kova_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$KOVA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _KOVA_user_memory.<name>.cli, usually
