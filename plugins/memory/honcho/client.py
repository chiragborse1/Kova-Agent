...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a kova profile."""
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. KOVA_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``kova.<profile>``
      3. Fallback: ``"kova"`` (default profile)
    """
    explicit = os.environ.get("KOVA_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into kova system prompt
    ...
    def from_global_config(
        ...
        # A hosts.kova block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — kova session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. kova session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run kova honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `kova honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                KOVA_cfg = load_config()
                honcho_cfg = KOVA_cfg.get("honcho", {})
