...
Handles: kova honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all kova profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'kova honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'kova honcho setup' first.\n")
        return
...
def _local_config_path() -> Path:
    ...
    Always returns $KOVA_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives kova persistent cross-session memory.")
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'kova honcho setup' again once you have a key.\n")
                return
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Kova Gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: kova config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    kova honcho status     -- show full config")
    print("    kova honcho mode       -- change recall/observation mode")
    print("    kova honcho tokens     -- tune context and dialectic budgets")
    print("    kova honcho peer       -- update peer names")
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'kova honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'kova honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: kova honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    kova' identity in Honcho. Seed with 'kova honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: kova honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: kova honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. kova asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'kova honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'kova honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    kova honcho identity --show        — show both peer representations")
        print("    kova honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  kova honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'kova honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'kova honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  kova honcho identity <path/to/file>")
    ...
    print()
    print("  In kova, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    kova honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  kova honcho identity <path/to/SOUL.md>")
    ...
    print("    kova:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    kova:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    kova:   Honcho observes every message and updates representations")
    ...
    print("    kova:   per-session by default — each run gets its own session")
    print("              Map a custom name:  kova honcho map <session-name>")
    ...
    if not has_key:
        ...
        print("  5. kova honcho mode               — view or change memory mode")
    print()
...
def register_cli(subparser) -> None:
    """Build the ``kova honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``kova honcho``.
    """
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to kova Honcho",
    )
