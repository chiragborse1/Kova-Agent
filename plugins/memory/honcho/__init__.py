...
Config: Uses the existing Honcho config chain:
  1. $KOVA_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, KOVA_HOME):
        """Write config to $KOVA_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `kova honcho setup`
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Kova CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "kova-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every kova run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_kova_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "kova-default",
