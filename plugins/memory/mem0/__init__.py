...
Configuration
-------------
Secret (lives in $KOVA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $KOVA_HOME/mem0.json, set via `kova memory
setup`):
  ...
  agent_id           — Agent identifier (default: kova)
...
def _load_config() -> dict:
    """Load config from env vars, with $KOVA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "kova"),
        "oss": {},
    }
    ...
    config_path = get_kova_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, KOVA_HOME):
        """Write config to $KOVA_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $KOVA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "kova")
