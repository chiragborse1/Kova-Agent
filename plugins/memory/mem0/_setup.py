...
def _setup_platform(KOVA_HOME: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "kova-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "kova"},
...
def _setup_oss(KOVA_HOME: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "kova-user")
    ...
    if env_writes:
        _write_env(Path(KOVA_HOME) / ".env", env_writes)
    _save_mem0_json(KOVA_HOME, {"mode": "oss", "user_id": user_id, "agent_id": "kova", "oss": oss_config})
...
_PGVECTOR_CONTAINER = "kova-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "kova"
...
def _setup_oss_interactive(KOVA_HOME: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], KOVA_HOME)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], KOVA_HOME)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'kova-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "kova-user")

    agent_id = input("  Agent ID [kova]: ").strip()
    agent_id = agent_id or "kova"
    ...
    if env_writes:
        _write_env(Path(KOVA_HOME) / ".env", env_writes)
    _save_mem0_json(KOVA_HOME, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(KOVA_HOME: str, config: dict) -> None:
    """Entry point called by kova memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(KOVA_HOME, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(KOVA_HOME, config, flags)
        return
