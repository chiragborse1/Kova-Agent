...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: kova)
...
Or via $KOVA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so kova can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $KOVA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_kova_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "kova": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "kova"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this kova config."""
    profile = config.get("profile", "kova")
    return str(profile or "kova")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active kova profile name (from agent_identity)
      {workspace} — kova workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``kova-{user}`` with no user becomes ``kova``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Kova Agent and the User"
    ...
    def save_config(self, values, KOVA_HOME):
        """Write config to $KOVA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(KOVA_HOME) / "hindsight"
    ...
    def post_setup(self, KOVA_HOME: str, config: dict) -> None:
        ...
        from kova_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "kova")
        ...
        self.save_config(provider_config, KOVA_HOME)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(KOVA_HOME) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(KOVA_HOME) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "kova"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: kova-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Kova Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "kova"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "kova"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "kova", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "kova")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Kova Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run kova as a non-root user, or switch "
                    "to cloud / local_external mode via 'kova memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # kova get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When kova created/used that client on the shared async loop,
