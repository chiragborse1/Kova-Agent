"""Thin Spotify Web API helper used by kova native tools."""
...
def _friendly_spotify_error_message(
    *,
    ...
    if status_code == 401:
        return "Spotify authentication failed or expired. Run `kova auth spotify` again."
    ...
    if status_code == 403:
        ...
        if "scope" in normalized_detail or "permission" in normalized_detail:
            return "Spotify rejected the request because the current auth scope is insufficient. Re-run `kova auth spotify` to refresh permissions."
