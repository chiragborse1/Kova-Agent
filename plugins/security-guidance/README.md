...
```bash
kova plugins enable security-guidance
# or edit ~/.kova/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  on every agent turn that touched files. On kova that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.kova/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  NousResearch, MIT-licensed alongside the rest of kova-agent.
