...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with kova-agent)
...
_USER_NAMESPACE = "_KOVA_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_KOVA_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_KOVA_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$KOVA_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
