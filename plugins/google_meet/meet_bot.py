...
``$KOVA_HOME/workspace/meetings/<meeting-id>/``. The main kova process
...
Run standalone for debugging::
    ...
    KOVA_MEET_URL=https://meet.google.com/abc-defg-hij \\
    KOVA_MEET_OUT_DIR=/tmp/meet-debug \\
    KOVA_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``KOVA_MEET_OUT_DIR``.
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "KOVA_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("KOVA_MEET_URL", "").strip()
    out_dir_env = os.environ.get("KOVA_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("KOVA_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("KOVA_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("KOVA_MEET_GUEST_NAME", "Kova Agent")
    duration_s = _parse_duration(os.environ.get("KOVA_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when KOVA_MEET_MODE=realtime.
    mode = os.environ.get("KOVA_MEET_MODE", "transcribe").strip().lower()
    ...
    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — KOVA_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: KOVA_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in KOVA_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("KOVA_MEET_LOBBY_TIMEOUT", "300")
            )
