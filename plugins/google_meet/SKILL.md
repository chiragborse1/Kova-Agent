...
kova meet install                 # pip deps + Chromium (transcribe only)
kova meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
kova meet auth                    # optional; skips guest-lobby wait
kova meet setup                   # preflight checks
```

`kova meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or KOVA_MEET_REALTIME_KEY in ~/.kova/.env
```
...
pip install playwright websockets && python -m playwright install chromium
kova plugins enable google_meet
kova meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
kova meet node approve my-mac ws://<mac-ip>:18789 <token>
kova meet node ping my-mac                   # confirm reachable
```

Run `kova meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Kova Agent bot is in this call taking notes."
...
Local:
```
$KOVA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$KOVA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `kova meet node approve`.
