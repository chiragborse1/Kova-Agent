...
gateway (which kova uses to bill Browser Use sessions to a Nous
...
Auth env vars (one of)::
    ...
    # OR a managed Nous gateway entry (configured via 'kova setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when kova only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"KOVA_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
