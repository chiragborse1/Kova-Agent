...
``{title, url, description, position}`` rows every other kova web
provider produces.
...
prefers kova-managed xAI Grok OAuth (via ``kova auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.kova/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``kova tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `kova auth` to sign in with "
