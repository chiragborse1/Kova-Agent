...
``kova_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__KOVA_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``kova_cli/web_server.py``.

This means ``kova dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from kova_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``kova_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from kova_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``kova_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from kova_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from kova_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``KOVA_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from kova_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``kova kanban
...
# spawn failures, stuck-blocked). See kova_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``kova kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``kova kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``kova kanban specify <task_id>``.
    ...
    # KOVA_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from kova_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``kova kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.kova/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current kova profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.kova/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from kova_constants import get_kova_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_kova_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``kova profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from kova_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``kova kanban decompose <task_id>``.
    ...
    # KOVA_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from kova_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.kova/config.yaml.
