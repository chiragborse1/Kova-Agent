"""langfuse — kova plugin for Langfuse observability.

Traces kova conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the kova plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``kova plugins enable
observability/langfuse`` or ``kova tools → Langfuse Observability``). At
...
Optional env vars:
  KOVA_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  KOVA_LANGFUSE_RELEASE     - release/version tag
  KOVA_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  KOVA_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  KOVA_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("KOVA_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the kova plugin system —
    ...
    public_key = _env("KOVA_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("KOVA_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("KOVA_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("KOVA_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset KOVA_LANGFUSE_PUBLIC_KEY / KOVA_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("KOVA_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("KOVA_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("KOVA_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("KOVA_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid KOVA_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _trace_key(
    ...
    *,
    ...
    Older kova paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("KOVA_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older kova branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current kova also has a turn-scoped
    ...
    # messages is a list only for legacy kova branches that fired
    # pre_llm_call with API messages directly. Current kova fires
...
def register(ctx) -> None:
    ...
    # kova versions.  pre_api_request / post_api_request fire per API
