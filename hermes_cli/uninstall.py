...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • kova config/data: {hermes_home}")
        ...
        print(f"  • Keep kova config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/chiragborse1/Kova-Agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/chiragborse1/Kova-Agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
