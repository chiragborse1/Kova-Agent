...
Pure logic: walks a kova config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `kova doctor` and a future `hermes migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a kova config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a kova config and return retirement issues.
