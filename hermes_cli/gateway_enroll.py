...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC).
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "could not be verified — try `kova auth add` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    #    gapped / self-hosted-IdP); otherwise the access token.
    try:
