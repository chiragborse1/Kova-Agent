"""Safe Kova Console command engine.
...
class HermesConsoleEngine:
    """Curated line-command executor for Kova Console."""
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Kova Console."""
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Kova Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Kova Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in Kova Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Kova Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Kova Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Kova Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Kova Console.",
            ("skills", "publish"): "`skills publish` is not available in Kova Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Kova Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Kova Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Kova Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Kova Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Kova Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Kova Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Kova Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Kova Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Kova Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Kova Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Kova Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Kova Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Kova Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Kova Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Kova Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Kova Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Kova Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Kova Console.")
