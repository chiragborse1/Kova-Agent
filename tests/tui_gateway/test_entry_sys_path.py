...
kova's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``kova_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call kova_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call kova_bootstrap.harden_import_path()"
    )
...
def test_entry_does_not_reimplement_guard_inline():
    ...
    assert '{"", "."}' not in source and "{'', '.'}" not in source, (
        "entry.py should delegate to kova_bootstrap.harden_import_path, "
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the kova root.  harden_import_path must relocate the
    kova root to the front so ``from utils import ...`` resolves to kova."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/kova", "/usr/lib"]
        kova_bootstrap.harden_import_path(src_root="/opt/kova")
        assert sys.path[0] == "/opt/kova"
        assert sys.path.index("/opt/kova") < sys.path.index(
