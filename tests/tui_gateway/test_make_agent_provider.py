...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "KOVA_TUI_MAX_TURNS": "7",
                "KOVA_TUI_CHECKPOINTS": "1",
                "KOVA_TUI_PASS_SESSION_ID": "1",
                "KOVA_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.kova/config.yaml parse as
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("kova_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("kova_cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("kova_cli.model_switch.switch_model", return_value=_FakeResult()),
