...
def server():
    with patch.dict("sys.modules", {
        "kova_constants": MagicMock(get_kova_home=MagicMock(return_value="/tmp/KOVA_test")),
        "kova_cli.env_loader": MagicMock(),
        "kova_cli.banner": MagicMock(),
        "kova_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `KOVA_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("KOVA_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("KOVA_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/kova-agent-dev": {"name": "kova-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "kova-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "kova_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "kova-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "kova-agent-dev"
