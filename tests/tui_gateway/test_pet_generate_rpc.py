...
def test_pet_hatch_then_adopt_activates(monkeypatch, tmp_path):
    ...
    activated = {}
    monkeypatch.setattr("kova_cli.pets._set_active", lambda slug: activated.setdefault("slug", slug))
...
def test_pet_sprite_payload_includes_concrete_row_counts():
    ...
    # Current Codex rows can have more/fewer frames than kova' generic
...
def test_pet_info_meta_avoids_full_payload(monkeypatch):
    import kova_cli.config as cli_config
    from agent.pet import constants, store
