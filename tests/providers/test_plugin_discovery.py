...
Verifies that:
 ...
 2. User plugins at $KOVA_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_KOVA_user_provider")
        ):
            del sys.modules[mod]
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from kova_cli import plugins as plugin_mod
    ...
    user_plugin = KOVA_HOME / "plugins" / "test-model-provider"
