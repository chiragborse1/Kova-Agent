"""Tests for kova_constants module."""
...
from kova_constants import (
    ...
    find_kova_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_kova_root,
    get_kova_dir,
    get_kova_home,
    heal_kova_managed_node,
    kova_managed_node_tree_present,
    iter_kova_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultkovaRoot:
    ...
    def test_no_KOVA_HOME_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        ...
        assert get_default_kova_root() == local_appdata / "kova"
    ...
    def test_no_KOVA_HOME_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        ...
        assert get_default_kova_root() == home / "AppData" / "Local" / "kova"
...
class TestGetkovaHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(kova_constants.sys, "platform", "win32")
        monkeypatch.setattr(kova_constants, "_profile_fallback_warned", False)

        assert get_kova_home() == local_appdata / "kova"


class TestKovaManagedNode:
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert kova_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_KOVA_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "kova"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken kova-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(kova_constants, "heal_kova_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_kova_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(kova_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(kova_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".kova"
...
class TestGetKovaDir:
    """Tests for ``get_kova_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_kova_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_kova_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_kova_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_kova_dir("cache/audio", "audio_cache")
