...
($INSTALL_DIR), not into the shared $KOVA_HOME.

Background (shared-$KOVA_HOME bug)
------------------------------------
$KOVA_HOME is a data directory users frequently bind-mount into a Docker
...
$KOVA_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``kova update``).
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$KOVA_HOME/\.install_method"', text), (
        "install.sh must not stamp $KOVA_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `kova update`"
    )
