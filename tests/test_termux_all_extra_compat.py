...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"kova-agent[termux]"' in text
    assert '"kova-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"kova-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
