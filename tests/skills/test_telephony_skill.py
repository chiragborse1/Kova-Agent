...
def test_save_twilio_writes_env_and_state(tmp_path: Path, monkeypatch):
    ...
    env_text = (tmp_path / ".kova" / ".env").read_text(encoding="utf-8")
    state = json.loads((tmp_path / ".kova" / "telephony_state.json").read_text(encoding="utf-8"))
...
def test_diagnose_includes_decision_tree_and_saved_state(tmp_path: Path, monkeypatch):
    ...
    mod._save_state(
        ...
        KOVA_HOME / "telephony_state.json",
    )
    (KOVA_HOME / ".env").parent.mkdir(parents=True, exist_ok=True)
