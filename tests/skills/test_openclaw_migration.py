...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `kova cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "KOVA_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``kova``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "kova prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told kova to use dark mode"
    ...
    # real filesystem path ``~/.kova`` (kova home) when rebranding
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "kova remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "kova prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "kova was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "kova said to ask kova about kova settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.kova`` (the real kova home), not the broken
    ``.kova``.
