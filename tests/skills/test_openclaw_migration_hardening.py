"""Tests for the OpenClaw→kova migration hardening features.
...
def _load():
    spec = importlib.util.spec_from_file_location("openclaw_to_KOVA_hard", SCRIPT_PATH)
...
def test_redact_preserves_non_secret_keys_and_values():
    mod = _load()
    input_data = {"name": "kova", "count": 42, "tags": ["a", "b"]}
