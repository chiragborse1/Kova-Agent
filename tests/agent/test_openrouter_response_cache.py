...
class TestBuildOrHeaders:
    ...
    def test_base_attribution_always_present(self):
        ...
        assert headers["HTTP-Referer"] == "https://kova-agent.nousresearch.com"
    ...
    def test_none_config_falls_back_to_load_config(self):
        ...
        with patch("kova_cli.config.load_config", return_value=fake_cfg):
...
class TestEnvVarOverrides:
    ...
    def test_env_enables_cache(self, monkeypatch):
        """KOVA_OPENROUTER_CACHE=true enables cache even when config disables it."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", "true")
        headers = build_or_headers(or_config={"response_cache": False})
        assert headers["X-OpenRouter-Cache"] == "true"

    def test_env_disables_cache(self, monkeypatch):
        """KOVA_OPENROUTER_CACHE=false disables cache even when config enables it."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", "false")
        headers = build_or_headers(or_config={"response_cache": True})
        assert "X-OpenRouter-Cache" not in headers
    ...
    def test_truthy_values(self, monkeypatch, value):
        ...
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", value)
        headers = build_or_headers(or_config={})
        assert headers["X-OpenRouter-Cache"] == "true"
    ...
    def test_env_ttl_overrides_config(self, monkeypatch):
        """KOVA_OPENROUTER_CACHE_TTL overrides config TTL."""
        from agent.auxiliary_client import build_or_headers

        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", "true")
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE_TTL", "1800")
    ...
    def test_invalid_env_ttl_dropped(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", "1")
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE_TTL", ttl)
        headers = build_or_headers(or_config={})
        assert headers["X-OpenRouter-Cache"] == "true"
        assert "X-OpenRouter-Cache-TTL" not in headers
    ...
    def test_valid_env_ttl_boundaries(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE", "yes")
        monkeypatch.setenv("KOVA_OPENROUTER_CACHE_TTL", ttl)
    ...
    def test_no_env_vars_falls_through_to_config(self, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_OPENROUTER_CACHE", raising=False)
        monkeypatch.delenv("KOVA_OPENROUTER_CACHE_TTL", raising=False)
