...
History:
  - PR #1775 first fixed this on ``run_KOVA_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_KOVA_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_KOVA_oauth_login_pure()`` run
    ...
    monkeypatch.setattr(
        "kova_cli.auth._can_open_graphical_browser", lambda: True
    )
...
def test_authorization_url_state_is_not_pkce_verifier(monkeypatch, tmp_path):
    ...
    from agent.anthropic_adapter import run_KOVA_oauth_login_pure

    result = run_KOVA_oauth_login_pure()
