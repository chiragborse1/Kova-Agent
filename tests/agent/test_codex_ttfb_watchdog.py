...
def test_ttfb_default_tolerates_slow_first_event(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("KOVA_CODEX_TTFB_TIMEOUT_SECONDS", raising=False)
    monkeypatch.delenv("KOVA_CODEX_TTFB_MAX_SECONDS", raising=False)
...
def test_ttfb_high_env_is_capped_for_openai_codex(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("KOVA_CODEX_TTFB_TIMEOUT_SECONDS", "90")
    monkeypatch.setenv("KOVA_CODEX_TTFB_MAX_SECONDS", "1")
...
def test_event_idle_kills_after_first_event_then_silence(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("KOVA_CODEX_TTFB_TIMEOUT_SECONDS", "10")
    monkeypatch.setenv("KOVA_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "1")
...
def test_ttfb_disabled_via_env_zero(tmp_path, monkeypatch):
    """Setting KOVA_CODEX_TTFB_TIMEOUT_SECONDS=0 disables the TTFB watchdog;
    a no-event stall then falls through to the (here, 60s) stale timeout, so a
    short hang is NOT killed by TTFB."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("KOVA_CODEX_TTFB_TIMEOUT_SECONDS", "0")
