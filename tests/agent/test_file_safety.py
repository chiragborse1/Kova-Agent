...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = KOVA_HOME / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._KOVA_HOME_path", return_value=KOVA_HOME):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        KOVA_HOME = tmp_path / ".kova"
        hub = KOVA_HOME / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        KOVA_HOME = tmp_path / ".kova"
        cache = KOVA_HOME / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
