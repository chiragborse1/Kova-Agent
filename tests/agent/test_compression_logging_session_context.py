...
which moves ``KOVA_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``kova_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    kova_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(kova_logging._session_context, "session_id", None)
