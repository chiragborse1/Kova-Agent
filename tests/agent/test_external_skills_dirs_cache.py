...
@pytest.fixture
def KOVA_HOME_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.kova/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".kova"
    home.mkdir()
...
def test_returns_configured_external_dir(KOVA_HOME_with_config):
    _home, external, _cfg = KOVA_HOME_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(KOVA_HOME_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = KOVA_HOME_with_config
...
def test_cache_invalidates_on_mtime_change(KOVA_HOME_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = KOVA_HOME_with_config
...
def test_returned_list_is_a_copy(KOVA_HOME_with_config):
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different KOVA_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".kova"
    ...
    monkeypatch.setenv("KOVA_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("KOVA_HOME", str(home_b))
