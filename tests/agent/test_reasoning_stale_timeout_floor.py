...
thinking phase.  kova's default cloud-stream stale detector
(``KOVA_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``KOVA_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_reasoning_floor_applies_to_nemotron_3_ultra(monkeypatch, tmp_path):
    ...
    import importlib
    from kova_cli import config as cfg_mod, timeouts as to_mod
...
def test_reasoning_floor_loses_to_env_var_when_no_floor_match(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    (tmp_path / ".env").write_text("", encoding="utf-8")
    monkeypatch.setenv("KOVA_API_CALL_STALE_TIMEOUT", "300")
    _write_config(tmp_path, "")
