...
class TestBuildLearnPrompt:
    ...
    def test_teaches_the_full_hardline_standards(self):
        ...
        # author is always the literal kova, never the host/OS identity (#52368).
        assert "author: always the literal value `kova`" in std
        assert "never fill it from the host" in std
        # #2 kova-tool framing names the wrapped tools, not shell utilities.
