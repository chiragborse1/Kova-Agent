...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. kova passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("KOVA_HOME", "/users/alice/.kova/profiles/backend-worker")
        monkeypatch.setenv("KOVA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "KOVA_KANBAN_DB",
            "/users/alice/.kova/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.kova/kanban/boards/smoke"]'
            in cmd
        )
