...
CA bundle env vars (KOVA_CA_BUNDLE, REQUESTS_CA_BUNDLE, SSL_CERT_FILE)
...
_CA_ENV_VARS = ("KOVA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE")
...
class TestResolveRequestsVerify:
    ...
    def test_KOVA_ca_bundle_returns_path(self, clean_env, bundle_file):
        clean_env.setenv("KOVA_CA_BUNDLE", bundle_file)
    ...
    def test_priority_KOVA_over_requests(self, clean_env, tmp_path, bundle_file):
    ...
    def test_nonexistent_path_falls_through(self, clean_env, tmp_path, bundle_file):
        ...
        clean_env.setenv("KOVA_CA_BUNDLE", str(missing))
        clean_env.setenv("REQUESTS_CA_BUNDLE", bundle_file)
        assert _resolve_requests_verify() == bundle_file
    ...
    def test_all_nonexistent_returns_true(self, clean_env, tmp_path):
        ...
        clean_env.setenv("KOVA_CA_BUNDLE", str(missing1))
    ...
    def test_empty_string_env_var_ignored(self, clean_env, bundle_file):
        clean_env.setenv("KOVA_CA_BUNDLE", "")
        clean_env.setenv("REQUESTS_CA_BUNDLE", bundle_file)
        assert _resolve_requests_verify() == bundle_file
