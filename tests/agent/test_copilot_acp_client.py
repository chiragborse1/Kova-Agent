...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_read_text_file_blocks_internal_KOVA_hub_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            home = Path(tmpdir) / "home"
            blocked = home / ".kova" / "skills" / ".hub" / "index-cache" / "entry.json"
            blocked.parent.mkdir(parents=True, exist_ok=True)
            blocked.write_text('{"token":"sk-test-secret-1234567890"}')

            with patch.dict(
                os.environ,
                {"HOME": str(home), "KOVA_HOME": str(home / ".kova")},
                clear=False,
            ):
                response = self._dispatch(
                    {
    ...
    def test_read_text_file_redacts_sensitive_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # agent.redact snapshots KOVA_REDACT_SECRETS at import time into
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"KOVA_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    (KOVA_HOME / "home").mkdir(parents=True)
    ...
    assert captured["kwargs"]["env"]["KOVA_REAL_HOME"] == str(real_home)
