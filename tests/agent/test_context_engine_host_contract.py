...
engine plugins (e.g. kova-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``kova plugins`` picker."""
    from kova_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "kova_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "kova_cli.plugins.get_plugin_context_engine",
