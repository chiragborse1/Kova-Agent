...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "kova Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_KOVA_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("KOVA_HOME", str(tmp_path / ".kova"))

    KOVA_env = tmp_path / ".kova" / ".env"
    KOVA_env.parent.mkdir(parents=True)
    KOVA_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.kova/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
