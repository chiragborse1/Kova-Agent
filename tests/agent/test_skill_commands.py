...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("KOVA_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"KOVA_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"KOVA_PLATFORM": "discord"}):
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        """``KOVA_SESSION_PLATFORM`` from the gateway session context must
        also trigger a rescan, not just ``KOVA_PLATFORM`` (#14536).

        Exercises the real ContextVar path: the gateway sets the active
        adapter via ``set_session_vars(platform=...)`` and the resolver
        reads it via ``get_session_env``. Setting ``KOVA_SESSION_PLATFORM``
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(self, tmp_path, monkeypatch):
        ...
        def _disabled_skills():
            if os.getenv("KOVA_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
    ...
    def test_get_skill_commands_does_not_rescan_when_platform_unchanged(self, tmp_path):
        ...
        with (
            ...
            patch.object(sc_mod, "_skill_commands", {}),
            patch.object(sc_mod, "_skill_commands_platform", None),
            patch.dict(os.environ, {"KOVA_PLATFORM": "telegram"}),
        ):
            _make_skill(tmp_path, "shared")
            # Prime the cache.
            get_skill_commands()
...
class TestTemplateVarSubstitution:
    """``${KOVA_SKILL_DIR}`` and ``${KOVA_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${KOVA_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${KOVA_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${KOVA_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${KOVA_SKILL_DIR}/scripts/foo.js" in msg
