...
Companion to ``tests/kova_cli/test_xai_oauth_writethrough.py``. That file
...
from kova_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.kova/auth.json``; a tmp_path
