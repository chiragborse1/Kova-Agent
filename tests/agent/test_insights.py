...
class TestEdgeCases:
    ...
    def test_custom_model_shows_zero_cost(self, db):
        ...
        db.create_session(session_id="s1", source="cli", model="FP16_KOVA_4.5")
        ...
        report = engine.generate(days=30)
        assert report["overview"]["estimated_cost"] == 0.0
        assert "FP16_KOVA_4.5" in report["overview"]["models_without_pricing"]

        models = report["models"]
        custom = next(m for m in models if m["model"] == "FP16_KOVA_4.5")
