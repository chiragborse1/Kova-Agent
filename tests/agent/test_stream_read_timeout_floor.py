...
def _resolve_read_timeout(base_url, stale_timeout, base_timeout=1800.0):
    ...
    read_timeout = float(os.getenv("KOVA_STREAM_READ_TIMEOUT", 120.0))
...
class TestCloudReadTimeoutFloor:
    ...
    def _clear_env(self):
        with pytest.MonkeyPatch.context() as mp:
            mp.delenv("KOVA_STREAM_READ_TIMEOUT", raising=False)
            yield
    ...
    def test_user_override_is_respected(self):
        """An explicit KOVA_STREAM_READ_TIMEOUT is never overridden by the floor."""
        with pytest.MonkeyPatch.context() as mp:
            mp.setenv("KOVA_STREAM_READ_TIMEOUT", "90")
