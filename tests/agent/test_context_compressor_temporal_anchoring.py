...
def test_first_compaction_prompt_contains_dated_anchoring_rule():
    ...
    with patch.object(kova_time, "now", _fixed_now), patch(
...
def test_clock_failure_omits_rule_but_compaction_still_runs():
    ...
    with patch.object(kova_time, "now", _boom), patch(
        "agent.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
...
def test_anchoring_rule_uses_date_from_KOVA_time_now():
    """The date is taken from kova_time.now(), which respects the user's TZ."""
    compressor = _compressor()
    fixed = datetime(2025, 12, 31, 23, 30, tzinfo=timezone.utc)
    with patch.object(kova_time, "now", lambda: fixed), patch(
        "agent.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
        compressor._generate_summary(_turns())
