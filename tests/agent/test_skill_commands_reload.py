...
def KOVA_HOME(monkeypatch):
    """Isolate KOVA_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``KOVA_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="kova-reload-skills-")
    monkeypatch.setenv("KOVA_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "KOVA_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_returns_expected_keys(self, KOVA_HOME):
    ...
    def test_detects_newly_added_skill_with_description(self, KOVA_HOME):
        from agent.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(KOVA_HOME / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_detects_removed_skill_carries_description(self, KOVA_HOME):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(KOVA_HOME / "skills", "demo", "soon to be gone")
    ...
    def test_description_passes_through_verbatim(self, KOVA_HOME):
        ...
        long_desc = "x" * 200
        _write_skill(KOVA_HOME / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, KOVA_HOME):
        from agent.skill_commands import reload_skills, get_skill_commands

        _write_skill(KOVA_HOME / "skills", "alpha")
        # Prime cache
        get_skill_commands()
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, KOVA_HOME):
