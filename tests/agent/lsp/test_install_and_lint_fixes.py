...
2. ``kova lsp status`` surfaces a ``Backend warnings`` section when
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.KOVA_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
def test_existing_binary_finds_windows_wrapper_in_staging(tmp_path, monkeypatch):
    ...
    wrapper = install_mod.KOVA_lsp_bin_dir() / "pyright-langserver.cmd"
...
def test_install_pip_finds_windows_scripts_launcher(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        scripts_dir = install_mod.KOVA_lsp_bin_dir().parent / "python-packages" / "Scripts"
    ...
    assert (install_mod.KOVA_lsp_bin_dir() / "fake-language-server.exe").exists()


# ---------------------------------------------------------------------------
# Fix 2: ``kova lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------
