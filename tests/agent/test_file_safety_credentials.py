...
Regression for https://github.com/chiragborse1/Kova-Agent/issues/17656 —
...
for the credential stores while leaving arbitrary ``KOVA_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_KOVA_HOME_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "KOVA_HOME"
    home.mkdir()
    monkeypatch.setattr(fs, "_KOVA_HOME_path", lambda: home)
    return home
...
def test_arbitrary_KOVA_HOME_file_not_blocked(fake_home):
    """Non-credential files inside KOVA_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal kova cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into KOVA_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "KOVA_HOME" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at KOVA_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == KOVA_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at KOVA_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in KOVA_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_KOVA_files_outside_home_not_blocked(
    ...
    """kova-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside KOVA_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside KOVA_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside KOVA_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, KOVA_HOME = <root>/profiles/<name>, but
