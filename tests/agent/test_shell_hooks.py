...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical kova block shape so that
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "kova keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("KOVA_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``kova hooks doctor``'s executability check and silently
