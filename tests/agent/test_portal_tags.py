"""Tests for agent.portal_tags — Portal request tag contract."""

from __future__ import annotations


def test_kova_client_tag_includes_current_version():
    """The client tag must reflect kova_cli.__version__ verbatim."""
    from kova_cli import __version__
    from agent.portal_tags import kova_client_tag

    assert kova_client_tag() == f"client=kova-client-v{__version__}"


def test_kova_client_tag_format():
    """The client tag has the exact shape expected."""
    from agent.portal_tags import kova_client_tag

    tag = kova_client_tag()
    assert tag.startswith("client=kova-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    """Every Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import kova_client_tag, nous_portal_tags
