...
``…/sandboxes/<backend>/<task>/home/.kova/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.kova/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.kova/… must not be flagged."""
        from agent.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.kova/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.kova/profiles/group1/SOUL.md",
            mirror_prefix="/root/.kova",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.kova")
    ...
    def test_non_KOVA_path_not_flagged(self):
        """/root/workspace/… is not .kova state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.kova/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.kova") is not None
