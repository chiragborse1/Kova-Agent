...
class TestLocalStreamReadTimeout:
    ...
    def test_local_endpoint_bumps_read_timeout(self, base_url):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_STREAM_READ_TIMEOUT", None)
            _base_timeout = float(os.getenv("KOVA_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("KOVA_STREAM_READ_TIMEOUT", 120.0))
    ...
    def test_user_override_respected_for_local(self):
        """User sets KOVA_STREAM_READ_TIMEOUT -> keep their value even for local."""
        with patch.dict(os.environ, {"KOVA_STREAM_READ_TIMEOUT": "300"}, clear=False):
            _base_timeout = float(os.getenv("KOVA_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("KOVA_STREAM_READ_TIMEOUT", 120.0))
