...
kova state stays profile-scoped through KOVA_HOME. Host subprocesses should
...
See: https://github.com/chiragborse1/Kova-Agent/issues/25114
See: https://github.com/chiragborse1/Kova-Agent/issues/36144
See: https://github.com/chiragborse1/Kova-Agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for kova_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(kova_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("KOVA_REAL_HOME", raising=False)
    ...
    def test_returns_none_when_KOVA_HOME_unset(self, monkeypatch):
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        KOVA_HOME = real_home / ".kova" / "profiles" / "coder"
        profile_home = KOVA_HOME / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_REAL_HOME", str(real_home))

        from kova_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from kova_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".kova" / "profiles"
        ...
        monkeypatch.setenv("KOVA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("KOVA_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["KOVA_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(KOVA_HOME / "home")
        assert result["KOVA_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_KOVA_HOME_unset(self, monkeypatch):
        monkeypatch.delenv("KOVA_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["KOVA_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from kova_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from kova_cli.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(KOVA_HOME / "home"), original_home)
