...
Build the real image and verify the actual runtime behavior:
  ...
  1. Root-owned top-level state files (auth.json, state.db, gateway.lock,
     gateway_state.json) are chowned to kova on boot
...
def test_root_owned_state_files_repaired_on_boot(
    ...
    """Root-owned top-level state files must be chowned to kova on boot."""
    ...
    # Verify files are now kova-owned
    r = docker_exec_sh(
        container_name,
        " ".join(f'stat -c %U /opt/data/{f}' for f in ALLOWLISTED_FILES),
        timeout=5,
    ...
    for line in r.stdout.split():
        assert line == "kova", (
            f"expected kova-owned after restart, got: {line}"
        )
...
def test_non_allowlisted_host_file_not_touched(
    ...
    $KOVA_HOME may contain host-owned files kova does not manage."""
    start_container(built_image, container_name)
...
def test_symlinked_allowlisted_file_not_chowned(
    ...
    try:
        ...
        # Clean up root/kova-owned files left by stage2 chown
