...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$KOVA_HOME" = "/opt/data" && '
        'test "$KOVA_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_KOVA_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/kova/agent/message_sanitization.py")
        try:
