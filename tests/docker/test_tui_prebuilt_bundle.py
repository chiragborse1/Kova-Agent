...
package-lock.json. Without KOVA_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV KOVA_TUI_DIR=/opt/kova/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the kova user, return stdout."""
    inner = (
        "source /opt/kova/.venv/bin/activate && "
        "cd /opt/kova && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the kova user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "kova", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_KOVA_TUI_dir_env_is_set(built_image: str) -> None:
    """KOVA_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$KOVA_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/kova/ui-tui", (
        f"KOVA_TUI_DIR={r.stdout.strip()!r} (expected /opt/kova/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from kova_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/kova/ui-tui')\n"
    ...
    # With KOVA_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
