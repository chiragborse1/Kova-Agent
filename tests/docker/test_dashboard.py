"""Harness: dashboard opt-in via KOVA_DASHBOARD.

Today (tini): dashboard starts once when KOVA_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    assert r.returncode != 0, (
        "Dashboard should not be running without KOVA_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without KOVA_DASHBOARD, s6-svstat should report the dashboard
    ...
    KOVA_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without KOVA_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with KOVA_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # KOVA_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "KOVA_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With KOVA_DASHBOARD=1, a dashboard process should be visible."""
    ...
    assert ok, "Dashboard should be running with KOVA_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """KOVA_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    # Kill the dashboard. The dashboard process runs as kova, so the
    # kova user can kill it (same UID).
...
# whenever `KOVA_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``KOVA_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    assert "nous" in provider_names, (
        ...
        f"KOVA_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and KOVA_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``KOVA_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
