...
Build the real image and verify at runtime:
  ...
  1. /opt/kova is not writable by the kova user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and KOVA_DISABLE_LAZY_INSTALLS are set
  3. /opt/kova/.install_method contains "docker" (code-scoped stamp)
  4. $KOVA_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $KOVA_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_kova(
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/kova as the kova user
        "touch /opt/kova/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"kova user can write to /opt/kova (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/kova/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"kova user can write to /opt/kova/.venv: {r.stdout}"
    )


def test_KOVA_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    KOVA_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/kova/.install_method (code-scoped), NOT in $KOVA_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/kova/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/kova/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $KOVA_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$KOVA_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $KOVA_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $KOVA_HOME was not healed on boot: "
        f"{r.stdout}"
    )
