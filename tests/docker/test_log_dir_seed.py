"""Runtime smoke test for Docker $KOVA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the kova user after container boot.
...
registered later runs its log service as the dropped kova user and
...
def test_logs_gateways_seeded_and_KOVA_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by kova after boot."""
    ...
    assert "logs=kova" in r.stdout, (
        f"logs/ not owned by kova: {r.stdout}"
    )
