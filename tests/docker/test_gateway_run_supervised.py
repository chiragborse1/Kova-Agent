...
Before the s6 migration, ``docker run nousresearch/kova-agent gateway
...
crash and a supervised dashboard alongside (when ``KOVA_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``KOVA_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``KOVA_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``kova`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'kova.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'kova.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → Kova Gateway run) can take
    ...
    # top), NOT the static main-kova service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-kova
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``Kova Gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "KOVA_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The KOVA_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``Kova Gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``Kova Gateway
    ...
    # (main-wrapper.sh → python → Kova Gateway run) can take well
    ...
    # started either — so we'd see the CMD's `Kova Gateway run`
    ...
    # `Kova Gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*kova.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `Kova Gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-kova`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``KOVA_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${KOVA_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Kova Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Kova Gateway
    ...
    assert "⚕" in combined or "Kova Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Kova Gateway Starting" in file_contents, (
