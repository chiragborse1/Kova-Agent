...
def test_stage2_browser_path_accessible_to_KOVA_user(
    built_image: str, container_name: str,
) -> None:
    """The discovered browser binary must be accessible to the
    unprivileged kova user (UID 10000), since that's who runs
    ...
    assert r.returncode == 0, (
        f"browser binary not readable+executable by kova user: {r.stderr}"
    )
