...
def test_no_args_starts_kova(built_image: str) -> None:
    """``docker run <image>`` should start kova cleanly.
...
def test_bare_executable_passthrough(built_image: str) -> None:
    ...
    kova wrapper. Useful for long-lived sandbox mode and for testing.
    """
    r = subprocess.run(
