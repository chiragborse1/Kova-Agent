...
binary download are exercised manually by `kova secrets bitwarden
...
def test_install_bws_rejects_malicious_member(KOVA_HOME, monkeypatch):
...
def test_install_bws_happy_path(KOVA_HOME, monkeypatch):
...
def test_install_bws_checksum_mismatch(KOVA_HOME, monkeypatch):
...
def test_install_bws_missing_checksum_entry(KOVA_HOME, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under KOVA_HOME/cache/."""
    home = tmp_path / ".kova"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
