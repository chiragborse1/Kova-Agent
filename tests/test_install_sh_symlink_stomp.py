...
Older versions of ``install.sh`` created ``$command_link_dir/kova`` as a
symlink to the pip-generated entry point at ``$KOVA_BIN`` (i.e.
``venv/bin/kova``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/kova" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$KOVA_BIN" "$@"`` then self-recursed and ``kova`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/kova``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/kova\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/kova"')
    cat_idx = block.find('cat > "$command_link_dir/kova" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/kova before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/kova        <- pip entry point (the one we must preserve)
          local_bin/kova       <- symlink → ../venv/bin/kova  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``KOVA_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/kova`` still contains its original pip-script body
      * ``local_bin/kova`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "kova"
    ...
    shim_path = command_link_dir / "kova"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/kova was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/kova must be replaced with a regular file, not "
