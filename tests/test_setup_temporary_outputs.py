...
def test_setup_uses_temporary_outputs_when_source_tree_is_read_only(
    ...
    """WebUI installs from read-only /opt/kova must not write build metadata."""
    ...
    assert Path(build_cmd.build_base).name.startswith("kova-agent-build")
    ...
    assert Path(source_relative_build.build_base).name.startswith("kova-agent-build")
    ...
    assert Path(egg_info_cmd.egg_base).name.startswith("kova-agent-egg-info")
