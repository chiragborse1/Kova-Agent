        ...
        json.dumps(
            {
                ...
                "distribution": {
                    "uvx": {
                        "package": f"kova-agent[acp]=={version}",
                        "args": ["kova-acp"],
                    }
                },
            },
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    assert manifest["distribution"]["uvx"]["package"] == "kova-agent[acp]==0.14.0"
    # args stay untouched so we don't accidentally rewrite them.
    assert manifest["distribution"]["uvx"]["args"] == ["kova-acp"]
