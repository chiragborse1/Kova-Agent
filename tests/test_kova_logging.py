"""Tests for kova_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import kova_logging
# Use whatever RotatingFileHandler class kova_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. kova_logging
...
from kova_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    kova_logging._logging_initialized = False
    ...
    kova_logging._install_session_record_factory()
    yield
    ...
    kova_logging._logging_initialized = False
    kova_logging.clear_session_context()


@pytest.fixture
def KOVA_HOME(tmp_path, monkeypatch):
    """Provide an isolated KOVA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_KOVA_HOME from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, KOVA_HOME):
        log_dir = kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        assert log_dir == KOVA_HOME / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, force=True)
    ...
    def test_custom_log_level(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, KOVA_HOME):
        kova_logging.setup_logging(
            KOVA_HOME=KOVA_HOME, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
    ...
    def test_writes_to_agent_log(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)

        test_logger = logging.getLogger("test_KOVA_logging.write_test")
        ...
        agent_log = KOVA_HOME / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)

        test_logger = logging.getLogger("test_KOVA_logging.warning_test")
        ...
        agent_log = KOVA_HOME / "logs" / "agent.log"
        errors_log = KOVA_HOME / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)

        test_logger = logging.getLogger("test_KOVA_logging.info_test")
    ...
    def test_explicit_params_override_config(self, KOVA_HOME):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (KOVA_HOME / "config.yaml").write_text(yaml.dump(config))

        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, log_level="WARNING")
    ...
    def test_record_factory_installed(self, KOVA_HOME):
        """The custom record factory injects session_tag on all records."""
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_KOVA_session_injector", False), (
            "Record factory should have _KOVA_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, KOVA_HOME):
        ...
        gw_log = KOVA_HOME / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, KOVA_HOME):
    ...
    def test_gateway_log_receives_gateway_records(self, KOVA_HOME):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, KOVA_HOME):
    ...
    def test_agent_log_still_receives_all(self, KOVA_HOME):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="cli")
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="gui")

        logging.getLogger("kova_cli.web_server").info("dashboard online")
        ...
        gui_log = KOVA_HOME / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, KOVA_HOME):
        """When session context is set, log lines include [session_id]."""
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        kova_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, KOVA_HOME):
    ...
    def test_clear_session_context(self, KOVA_HOME):
        """After clearing, session tag disappears."""
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        kova_logging.set_session_context("xyz789")
        kova_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, KOVA_HOME):
        ...
        def thread_a():
            kova_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            kova_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        kova_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        kova_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = kova_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = kova_logging._ComponentFilter(
            kova_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = kova_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in kova_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the KOVA_plugins
        ...
        # break this test.
        gateway_prefixes = kova_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "KOVA_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = kova_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == kova_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = kova_logging.COMPONENT_PREFIXES["cli"]
        assert "kova_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == kova_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = kova_logging.COMPONENT_PREFIXES["gui"]
        assert "kova_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        kova_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_KOVA_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, KOVA_HOME):
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME)
        kova_logging.setup_verbose_logging()
        kova_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        kova_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = kova_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        kova_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("kova_cli.config.is_managed", return_value=True):
                kova_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, KOVA_HOME):
        level, max_size, backup = kova_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, KOVA_HOME):
        import yaml
    ...
    def test_handles_missing_logging_section(self, KOVA_HOME):
        import yaml
        config = {"model": "test"}
        (KOVA_HOME / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> kova_logging._ManagedRotatingFileHandler:
        handler = kova_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that kova_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        kova_logging.setup_logging(KOVA_HOME=KOVA_HOME, mode="gateway")
        gw_path = KOVA_HOME / "logs" / "gateway.log"
        rotated = KOVA_HOME / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = kova_logging._safe_stderr()
