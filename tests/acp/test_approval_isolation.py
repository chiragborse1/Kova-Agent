...
- 96vc: ACP didn't set KOVA_EXEC_ASK, so `check_all_command_guards`
...
1. Setting KOVA_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds KOVA_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set KOVA_INTERACTIVE so
    ...
    (KOVA_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When KOVA_INTERACTIVE is set and an approval callback is
        ...
        # Without KOVA_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without KOVA_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With KOVA_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with KOVA_INTERACTIVE the approval path should consult the "
