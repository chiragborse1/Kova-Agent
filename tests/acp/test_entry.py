...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting kova-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "kova acp check OK"
...
def test_main_setup_runs_model_configuration(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.main.main", fake_KOVA_main)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`kova-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("kova_cli.dep_ensure.ensure_dependency", fake_ensure)
