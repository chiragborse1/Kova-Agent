...
def test_root_session_no_compression(db):
    ...
    assert prov["currentKovaSessionId"] == "root1"
    assert prov["rootKovaSessionId"] == "root1"
    assert prov["parentKovaSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_KOVA_SESSION_ID="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentKovaSessionId"] == "old"
    assert prov["rootKovaSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousKovaSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootKovaSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootKovaSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_KOVA_SESSION_ID="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousKovaSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"kova"}
    assert "sessionProvenance" in meta["kova"]
    assert meta["kova"]["sessionProvenance"]["currentKovaSessionId"] == "root1"
