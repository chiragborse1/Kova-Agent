...
from acp_adapter.server import KovaACPAgent, KOVA_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "kova-agent"
        assert resp.agent_info.version == KOVA_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open kova' interactive model/provider setup in a terminal. "
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        with patch(
            "kova_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "KovaACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("KovaACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_KOVA_SESSION_ID_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``KOVA_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("KOVA_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "KOVA_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("KOVA_SESSION_ID") is None, (
            "KOVA_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_KOVA_SESSION_ID(self, agent, monkeypatch):
        """If the env already had KOVA_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("KOVA_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("KOVA_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("KOVA_SESSION_ID") == "outer-sess"
