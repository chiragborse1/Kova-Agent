...
def test_mention_patterns_list_matches():
    adapter = _make_adapter(mention_patterns=["hey kova", "kova,"])
    assert adapter._slack_message_matches_mention_patterns("hey kova, you there?") is True
    assert adapter._slack_message_matches_mention_patterns("just chatting") is False


def test_mention_patterns_case_insensitive():
    adapter = _make_adapter(mention_patterns=["hey kova"])
    assert adapter._slack_message_matches_mention_patterns("HEY kova!") is True


def test_mention_patterns_single_string():
    adapter = _make_adapter(mention_patterns="^kova")
    assert adapter._slack_message_matches_mention_patterns("kova do this") is True
    assert adapter._slack_message_matches_mention_patterns("ok kova") is False


def test_mention_patterns_invalid_regex_skipped_without_crash():
    # An invalid pattern is dropped; valid siblings still work.
    adapter = _make_adapter(mention_patterns=["(unclosed", "hey kova"])
    assert adapter._slack_message_matches_mention_patterns("hey kova") is True


def test_mention_patterns_env_var_fallback(monkeypatch):
    monkeypatch.setenv("SLACK_MENTION_PATTERNS", '["hey kova", "kova,"]')
    adapter = _make_adapter()  # no config value -> falls back to env
    assert adapter._slack_message_matches_mention_patterns("hey kova") is True
...
def test_mention_patterns_env_var_csv_fallback_splits_patterns(monkeypatch):
    ...
    assert [pattern.pattern for pattern in patterns] == ["hey kova", "kova"]
    assert adapter._slack_message_matches_mention_patterns("hey kova") is True
...
def test_mention_patterns_trigger_in_channel_without_literal_mention():
    ...
    adapter = _make_adapter(require_mention=True, mention_patterns=["hey kova"])
    assert _would_process(adapter, text="hey kova what's the status") is True
