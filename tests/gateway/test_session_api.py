...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("KOVA_SESSION_ID")
            observed["context_platform"] = get_session_env("KOVA_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("KOVA_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("KOVA_SESSION_ID")
...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert deleted == {"object": "kova.session.deleted", "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == "kova.session"
    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", "X-kova-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-kova-Session-Id"] == session_id
    assert resp.headers["X-kova-Session-Key"] == "client-42"
    assert payload["object"] == "kova.session.chat.completion"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-kova-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-kova-Session-Key requires API key" in data["error"]["message"]
