...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    KOVA_HOME = tmp_path / ".kova"
    KOVA_HOME.mkdir()
    (KOVA_HOME / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (KOVA_HOME / ".env").write_text("KOVA_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["KOVA_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("KOVA_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["KOVA_MAX_ITERATIONS"] = "200"
