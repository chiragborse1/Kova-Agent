...
Precedence verified here:
    KOVA_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    ...
    _saved = {
        ...
        if k.startswith(("kova_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (KOVA_HOME / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("kova_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """KOVA_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("KOVA_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("kova_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("kova_cli.runtime_provider")
