...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @kova:example.org help")
    ...
    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@kova:example.org">kova</a> help'
        assert self.adapter._is_bot_mentioned("kova help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "KovaBot" should not match word-boundary check for "kova"
        assert not self.adapter._is_bot_mentioned("KovaBot is here")
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@kova:example.org help me")
    ...
    def test_localpart_in_path_preserved(self):
        ...
        result = self.adapter._strip_mention("read /home/kova/config.yaml")
        assert result == "read /home/kova/config.yaml"
    ...
    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@kova:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@kova:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@kova:example.org">kova</a> help'
    event = _make_event("kova help", formatted_body=formatted)
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@kova:example.org")
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@kova:example.org help me"
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@kova:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@kova:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@kova:example.org help", event_id="$dm1")
