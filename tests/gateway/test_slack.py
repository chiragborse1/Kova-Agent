...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.kova."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /kova, /btw, /stop,
        ...
        for expected in ("/kova", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@kova`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@kova"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@kova")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /kova subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /kova btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_KOVA_prefix_still_works(self, adapter):
        """Backward compat: /kova btw foo must still route to /btw foo.

        Old workspace manifests only declared /kova as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_KOVA_freeform_question(self, adapter):
        """/kova <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/kova",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_KOVA_slash_stashes_context(self, adapter):
        """Legacy /kova <subcommand> also stashes context."""
        command = {
            "command": "/kova",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_KOVA_question_does_not_stash_context(self, adapter):
        """Free-form /kova <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/kova",
