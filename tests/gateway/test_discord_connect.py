...
class FakeBot:
    def __init__(self, *, intents, proxy=None, allowed_mentions=None, **_):
        ...
        self.user = SimpleNamespace(id=999, name="kova")
        self._events = {}
        self.tree = FakeTree()
...
async def test_safe_sync_slash_commands_only_mutates_diffs():
    ...
    desired_same = {
        "name": "status",
        "description": "Show kova session status",
        "type": 1,
        "options": [],
        "nsfw": False,
        "dm_permission": True,
...
async def test_post_connect_initialization_reraises_non_rate_limit_exceptions(tmp_path, monkeypatch):
    ...
    class _DesiredCommand:
        def to_dict(self, tree):
            return {"name": "status", "description": "Show kova status", "type": 1, "options": []}
