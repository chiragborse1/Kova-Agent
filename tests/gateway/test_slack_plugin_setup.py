...
``kova_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``kova_cli.config`` (get_env_value / save_env_value) and
``kova_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to kova_cli.slack_cli + filesystem; stub it.
    import kova_cli.slack_cli as slack_cli_mod
