...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade kova-agent" in result
    ...
    @pytest.mark.asyncio
    async def test_no_KOVA_binary(self, tmp_path):
        """Returns error when kova is not on PATH and kova_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m kova_cli.main when kova not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m kova_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "kova_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_KOVA_bin_prefers_which(self, tmp_path):
        """_resolve_KOVA_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_KOVA_bin

        with patch("shutil.which", return_value="/custom/path/kova"):
            result = _resolve_KOVA_bin()

        assert result == ["/custom/path/kova"]

    @pytest.mark.asyncio
    async def test_resolve_KOVA_bin_fallback(self):
        """_resolve_KOVA_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_KOVA_bin
    ...
    @pytest.mark.asyncio
    async def test_resolve_KOVA_bin_returns_none_when_both_fail(self):
        """_resolve_KOVA_bin returns None when both strategies fail."""
        from gateway.run import _resolve_KOVA_bin
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((KOVA_HOME / ".update_pending.json").read_text())
...
class TestSendUpdateNotification:
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (KOVA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (KOVA_HOME / ".update_output.txt").write_text("x" * 5000)
        (KOVA_HOME / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (KOVA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (KOVA_HOME / ".update_output.txt").write_text("Traceback: boom")
