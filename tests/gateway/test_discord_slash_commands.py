...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="KovaBot"),
    )
...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "kova_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("kova_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello kova", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello kova",
    )
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_KOVA_when_only_mentions(adapter):
    """If a message contains only mention syntax, the stripped content is
    empty — fall back to the 'kova' default rather than ''."""
    thread = SimpleNamespace(id=999, name="kova")
    message = SimpleNamespace(
...
async def test_auto_create_thread_falls_back_to_seed_message(adapter):
    ...
    result = await adapter._auto_create_thread(message)
    assert result is thread
    message.channel.send.assert_awaited_once_with("🧵 Thread created by kova: **Hello**")
    seed_message.create_thread.assert_awaited_once_with(
        name="Hello",
        auto_archive_duration=1440,
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    KOVA_dir.mkdir()
    config_path = KOVA_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("KOVA_HOME", str(KOVA_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "kova_cli.commands.discord_skill_commands_by_category",
