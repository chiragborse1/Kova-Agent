...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_KOVA_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("kova, summarize this")
        assert adapter._message_matches_mention_patterns("@Kova Agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antikova should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("kova what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("kova, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Kova Agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask kova about this") == "please ask kova about this"
