...
- KOVA_KANBAN_DISPATCH_IN_GATEWAY env var disables without loading config.
...
def test_notifier_watcher_skips_when_dispatch_disabled():
    ...
    runner = _make_runner()
    with patch("kova_cli.config.load_config", return_value=_fake_config(False)):
        with patch("kova_cli.kanban_db.connect") as mock_connect:
            asyncio.run(runner._kanban_notifier_watcher())
    mock_connect.assert_not_called()


def test_notifier_watcher_env_override_disables(monkeypatch):
    """KOVA_KANBAN_DISPATCH_IN_GATEWAY=false skips config load entirely."""
    runner = _make_runner()
    monkeypatch.setenv("KOVA_KANBAN_DISPATCH_IN_GATEWAY", "false")
    with patch("kova_cli.config.load_config") as mock_load_config:
        with patch("kova_cli.kanban_db.connect") as mock_connect:
            asyncio.run(runner._kanban_notifier_watcher())
...
def test_notifier_watcher_runs_when_dispatch_enabled():
    ...
    with patch("kova_cli.config.load_config", return_value=_fake_config(True)):
        with patch.object(
            _kb, "list_boards",
