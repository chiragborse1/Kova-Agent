...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("KOVA_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("KOVA_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("KOVA_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("KOVA_MEDIA_TRUST_RECENT_SECONDS", "60")
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("KOVA_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("KOVA_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_KOVA_credentials(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        KOVA_dir = fake_home / ".kova"
        KOVA_dir.mkdir(parents=True)
        env_file = KOVA_dir / ".env"
    ...
    def test_denylist_blocks_KOVA_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        KOVA_dir = fake_home / ".kova"
        KOVA_dir.mkdir(parents=True)
        config_file = KOVA_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_KOVA_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared kova root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".kova" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        KOVA_root = fake_home / ".kova"
        config_file = KOVA_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._KOVA_ROOT",
            KOVA_root,
        )
    ...
    def test_KOVA_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        KOVA_dir = fake_home / ".kova"
        cache_dir = KOVA_dir / "cache" / "documents"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting KOVA_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``KOVA_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("KOVA_MEDIA_DELIVERY_STRICT", raw)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"KOVA_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"KOVA_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "KOVA_HUMAN_DELAY_MODE": "natural",
            "KOVA_HUMAN_DELAY_MIN_MS": "oops",
            "KOVA_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "KOVA_HUMAN_DELAY_MODE": "custom",
            "KOVA_HUMAN_DELAY_MIN_MS": "100",
            "KOVA_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"KOVA_MEDIA_DELIVERY_STRICT": "1",
                                     "KOVA_MEDIA_TRUST_RECENT_FILES": "0"}), \
