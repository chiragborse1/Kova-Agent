...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`kova_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + KOVA_HOME so the real user config is untouched."""
    home = tmp_path / ".kova"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `kova skills config` writes).
    monkeypatch.setattr(
    ...
    assert "kova skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "kova_constants.get_optional_skills_dir",
