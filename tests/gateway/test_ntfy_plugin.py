...
class TestNtfyRequirements:
    ...
    def test_returns_false_when_httpx_unavailable(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-test")
...
class TestNtfyAdapterInit:
    ...
    def test_default_server_url(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "kova-in"})
    ...
    def test_publish_topic_falls_back_to_topic(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "kova-in"})
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "kova-in"

    def test_publish_topic_uses_extra_value(self):
        config = PlatformConfig(
            enabled=True,
            extra={"topic": "kova-in", "publish_topic": "kova-out"},
        )
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "kova-out"
...
class TestConnect:
    ...
    def test_connect_starts_stream_task(self, monkeypatch):
        monkeypatch.setattr(_ntfy, "HTTPX_AVAILABLE", True)
        config = PlatformConfig(enabled=True, extra={"topic": "kova-test"})
        adapter = NtfyAdapter(config)
...
class TestSend:
    ...
    def _make_adapter(self, topic="kova-in", publish_topic="", token="", markdown=False):
    ...
    def test_send_fails_without_http_client(self):
        adapter = self._make_adapter()
        result = _run(adapter.send("kova-in", "hello"))
        assert result.success is False
        assert "not initialized" in result.error.lower()

    def test_send_posts_to_publish_topic(self):
        adapter = self._make_adapter(topic="kova-in", publish_topic="kova-out")
        ...
        result = _run(adapter.send("kova-in", "Hello ntfy!"))
        assert result.success is True
        assert result.message_id == "abc123"

        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/kova-out")

    def test_send_falls_back_to_subscribe_topic(self):
        adapter = self._make_adapter(topic="kova-in")
        ...
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/kova-in")
    ...
    def test_send_uses_metadata_publish_topic(self):
        ...
        result = _run(adapter.send(
            "kova-in", "Hi!", metadata={"publish_topic": "override-out"}
        ))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
    ...
    def test_get_chat_info_returns_dict(self):
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "t"}))
        info = _run(adapter.get_chat_info("kova-in"))
        assert info["name"] == "kova-in"
        assert info["type"] == "dm"

    def test_send_includes_bearer_auth_header(self):
        adapter = self._make_adapter(topic="kova-in", token="mytoken")
        ...
        _run(adapter.send("kova-in", "secure message"))
    ...
    def test_send_emits_markdown_header_when_enabled(self):
        adapter = self._make_adapter(topic="kova-in", markdown=True)
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {}
        mock_client = AsyncMock()
    ...
    def test_send_omits_markdown_header_when_disabled(self):
        ...
        _run(adapter.send("kova-in", "plain"))
        call_headers = mock_client.post.call_args[1]["headers"]
    ...
    def test_send_emits_echo_tag_header(self):
        ...
        _run(adapter.send("kova-in", "Hello!"))
        call_headers = mock_client.post.call_args[1]["headers"]
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        return NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "kova-in"}))
    ...
    def test_duplicate_message_skipped(self):
        ...
        adapter.set_message_handler(handler)
        event = {"id": "dup-1", "event": "message", "topic": "kova-in", "message": "hi", "time": None}
        _run(adapter._on_message(event))
        _run(adapter._on_message(event))
        assert len(calls) == 1
    ...
    def test_title_not_used_as_user_id(self):
        ...
        assert captured[0].source.user_id == "kova-in"
        assert captured[0].source.user_name == "kova-in"
    ...
    def test_source_chat_id_is_topic(self):
        ...
        assert captured[0].source.chat_id == "kova-in"
...
class TestEnvEnablement:
    ...
    def test_seeds_topic_and_server(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-in")
        monkeypatch.delenv("NTFY_SERVER_URL", raising=False)
        seed = _env_enablement()
        assert seed is not None
        assert seed["topic"] == "kova-in"
    ...
    def test_publish_topic_seeded(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-in")
        monkeypatch.setenv("NTFY_PUBLISH_TOPIC", "kova-out")
        seed = _env_enablement()
        assert seed["publish_topic"] == "kova-out"
    ...
    def test_home_channel_defaults_to_topic(self, monkeypatch):
        ...
        seed = _env_enablement()
        assert seed["home_channel"]["chat_id"] == "kova-in"
        assert seed["home_channel"]["name"] == "kova-in"
...
class TestStandaloneSend:
    ...
    def test_posts_to_server(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-in")
        pconfig = MagicMock()
        pconfig.extra = {"server": "https://ntfy.example.com", "topic": "kova-in"}
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            ...
            result = _run(_standalone_send(pconfig, "kova-in", "hello"))

        assert result.get("success") is True
        assert result["platform"] == "ntfy"
        assert result["message_id"] == "id-42"
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url == "https://ntfy.example.com/kova-in"
    ...
    def test_emits_echo_tag_header(self, monkeypatch):
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            mock_httpx.AsyncClient.return_value = mock_client
            _run(_standalone_send(pconfig, "kova-in", "hi"))
    ...
    def test_emits_bearer_token_when_configured(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "kova-in", "token": "tk_xyz"}
    ...
    def test_basic_auth_when_token_has_colon(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "kova-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "kova-in", "token": "user:pass"}
    ...
    def test_returns_error_on_http_failure(self, monkeypatch):
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            mock_httpx.AsyncClient.return_value = mock_client
            result = _run(_standalone_send(pconfig, "kova-in", "hi"))
