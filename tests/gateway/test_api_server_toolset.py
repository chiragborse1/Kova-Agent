"""Tests for kova-api-server toolset and API server tool availability."""
...
class TestKovaApiServerToolset:
    """Tests for the kova-api-server toolset definition."""

    def test_toolset_exists(self):
        ts = get_toolset("kova-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("kova-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("kova-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        ...
        assert PLATFORMS["api_server"]["default_toolset"] == "kova-api-server"
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            # No platform_toolsets override — should fall back to kova-api-server default
