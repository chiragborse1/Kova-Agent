...
Supergroup forum topics route with ``message_thread_id``. kova-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """kova-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_KOVA_dm_topics():
    """kova-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in kova-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
