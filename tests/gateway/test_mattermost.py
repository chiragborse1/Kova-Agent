...
class TestMattermostWebSocketParsing:
    def setup_method(self):
        ...
        self.adapter._bot_username = "kova-bot"
    ...
    async def test_mention_stripped_from_text(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            await self.adapter._handle_ws_event(
                self._make_event("@kova-bot what is 2+2")
            )
            assert self.adapter.handle_message.called
            msg = self.adapter.handle_message.call_args[0][0]
            assert "@kova-bot" not in msg.text
...
async def test_mattermost_top_level_channel_post_is_thread_root():
    ...
    adapter._bot_username = "kova-bot"
    adapter.handle_message = AsyncMock()
    post_data = {
        "id": "top_post_123",
        "user_id": "user_123",
        "channel_id": "chan_456",
        "message": "@kova-bot start work",
        "root_id": "",
    }
    event = {
        "event": "posted",
