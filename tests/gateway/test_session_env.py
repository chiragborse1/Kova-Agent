...
def test_set_session_env_sets_contextvars(monkeypatch):
    ...
    monkeypatch.delenv("KOVA_SESSION_CHAT_NAME", raising=False)
    monkeypatch.delenv("KOVA_SESSION_USER_ID", raising=False)
    monkeypatch.delenv("KOVA_SESSION_USER_NAME", raising=False)
    ...
    assert get_session_env("KOVA_SESSION_PLATFORM") == "telegram"
    assert get_session_env("KOVA_SESSION_SOURCE") == ""
    assert get_session_env("KOVA_SESSION_CHAT_ID") == "-1001"
    assert get_session_env("KOVA_SESSION_CHAT_NAME") == "Group"
    assert get_session_env("KOVA_SESSION_USER_ID") == "123456"
    assert get_session_env("KOVA_SESSION_USER_NAME") == "alice"
    assert get_session_env("KOVA_SESSION_THREAD_ID") == "17585"

    # os.environ should NOT be touched
    assert os.getenv("KOVA_SESSION_PLATFORM") is None
    assert os.getenv("KOVA_SESSION_SOURCE") is None
    assert os.getenv("KOVA_SESSION_THREAD_ID") is None
...
def test_session_source_uses_contextvars(monkeypatch):
    ...
    assert get_session_env("KOVA_SESSION_SOURCE") == "tool"
...
def test_clear_session_env_restores_previous_state(monkeypatch):
    ...
    assert get_session_env("KOVA_SESSION_PLATFORM") == ""
    assert get_session_env("KOVA_SESSION_CHAT_ID") == ""
    assert get_session_env("KOVA_SESSION_CHAT_NAME") == ""
    assert get_session_env("KOVA_SESSION_USER_ID") == ""
    assert get_session_env("KOVA_SESSION_USER_NAME") == ""
    assert get_session_env("KOVA_SESSION_THREAD_ID") == ""
...
def test_get_session_env_falls_back_to_os_environ(monkeypatch):
    ...
    assert get_session_env("KOVA_SESSION_PLATFORM") == "discord"
...
def test_get_session_env_default_when_nothing_set(monkeypatch):
    ...
    assert get_session_env("KOVA_SESSION_PLATFORM") == ""
    assert get_session_env("KOVA_SESSION_PLATFORM", "fallback") == "fallback"
...
def test_session_key_set_via_contextvars(monkeypatch):
    """set_session_vars should set KOVA_SESSION_KEY via contextvars."""
    ...
    assert get_session_env("KOVA_SESSION_KEY") == "tg:-1001:17585"

    clear_session_vars(tokens)
    assert get_session_env("KOVA_SESSION_KEY") == ""


def test_session_key_falls_back_to_os_environ(monkeypatch):
    """get_session_env for SESSION_KEY should fall back to os.environ."""
    monkeypatch.setenv("KOVA_SESSION_KEY", "env-session-123")

    # No contextvar set — should read from os.environ
    assert get_session_env("KOVA_SESSION_KEY") == "env-session-123"

    # Set contextvar — should prefer it
    tokens = set_session_vars(session_key="ctx-session-456")
    assert get_session_env("KOVA_SESSION_KEY") == "ctx-session-456"
...
def test_session_id_set_via_contextvars(monkeypatch):
    """set_session_vars should set KOVA_SESSION_ID via contextvars."""
    monkeypatch.setenv("KOVA_SESSION_ID", "stale-env-session")

    tokens = set_session_vars(session_id="ctx-session-456")
    assert get_session_env("KOVA_SESSION_ID") == "ctx-session-456"

    clear_session_vars(tokens)
    assert get_session_env("KOVA_SESSION_ID") == ""
...
def test_session_key_no_race_condition_with_contextvars(monkeypatch):
    ...
    async def handler(key: str, delay: float):
        ...
        try:
            ...
            read_back = get_session_env("KOVA_SESSION_KEY")
    ...
    try:
        result = await runner._run_in_executor_with_context(
            lambda: {
                ...
                "user_id": get_session_env("KOVA_SESSION_USER_ID"),
