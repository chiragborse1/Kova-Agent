...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="nas-token"):
    ...
    monkeypatch.setattr("kova_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_provisions_on_nas_host_that_is_NOT_is_managed(monkeypatch):
    """Regression: a NAS-hosted Fly agent sets neither KOVA_MANAGED nor a
    ...
    monkeypatch.setattr("kova_cli.config.is_managed", lambda: False)
    _arm(monkeypatch)
    captured: dict = {}
...
def test_no_nas_token_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.resolve_nous_access_token", _boom)
