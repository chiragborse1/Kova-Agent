...
Tests the new --gateway mode for kova update, including:
...
def _make_runner(KOVA_HOME=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (KOVA_HOME / ".update_response").write_text("y")

        thread = threading.Thread(target=write_response)
        thread.start()

        with patch.dict(os.environ, {"KOVA_HOME": str(KOVA_HOME)}):
            from kova_cli.main import _gateway_prompt
            result = _gateway_prompt("Restore? [Y/n]", "y", timeout=5.0)

        thread.join()
        assert result == "y"
        # Both files should be cleaned up
        assert not (KOVA_HOME / ".update_prompt.json").exists()
        assert not (KOVA_HOME / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = KOVA_HOME / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (KOVA_HOME / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
        (KOVA_HOME / ".update_response").write_text("")
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from kova_cli.main import _restore_stashed_changes
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns kova update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        # Write output
        (KOVA_HOME / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (KOVA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (KOVA_HOME / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (KOVA_HOME / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.5)
            (KOVA_HOME / ".update_response").write_text("y")
            (KOVA_HOME / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (KOVA_HOME / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_forwarding_preserves_thread_metadata(self, tmp_path):
        ...
        (KOVA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (KOVA_HOME / ".update_output.txt").write_text("")
        (KOVA_HOME / ".update_prompt.json").write_text(json.dumps({
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (KOVA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (KOVA_HOME / ".update_output.txt").write_text("error occurred\n")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._KOVA_HOME", KOVA_HOME):
            ...
            assert (KOVA_HOME / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        (KOVA_HOME / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = KOVA_HOME / ".update_response"
