...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test KOVA_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(KOVA_HOME):
    ...
    with patch("kova_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None)):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(KOVA_HOME):
    ...
    with patch("kova_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(KOVA_HOME):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from kova_cli.goals import GoalManager, save_goal
    ...
    with patch("kova_cli.goals.judge_goal", return_value=("continue", "keep going", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(KOVA_HOME):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(KOVA_HOME):
    ...
    with patch("kova_cli.goals.judge_goal", return_value=("done", "ok", False, None)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
