...
def _clear_env(monkeypatch):
    monkeypatch.delenv("KOVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("KOVA_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_keepalive_expiry(monkeypatch):
    monkeypatch.setenv("KOVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "7.5")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_max_keepalive(monkeypatch):
    monkeypatch.setenv("KOVA_GATEWAY_HTTPX_MAX_KEEPALIVE", "25")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("KOVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("KOVA_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
