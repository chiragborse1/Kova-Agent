...
class FakeTextChannel:
    def __init__(self, channel_id: int = 1, name: str = "general", guild_name: str = "kova Server"):
        self.id = channel_id
        self.name = name
...
class FakeForumChannel:
    def __init__(self, channel_id: int = 1, name: str = "support-forum", guild_name: str = "kova Server"):
        self.id = channel_id
        self.name = name
        self.guild = SimpleNamespace(name=guild_name)
        self.type = 15
...
class FakeThread:
    def __init__(self, channel_id: int = 1, name: str = "thread", parent=None, guild_name: str = "kova Server"):
        self.id = channel_id
        self.name = name
        self.parent = parent
...
async def test_discord_forum_threads_are_handled_as_threads(adapter, monkeypatch):
    ...
    thread = FakeThread(channel_id=456, name="Can kova reply here?", parent=forum)
    ...
    assert event.source.thread_id == "456"
    assert event.source.chat_type == "thread"
    assert event.source.chat_name == "kova Server / support-forum / Can kova reply here?"
...
async def test_fetch_channel_context_skips_self_improvement_boundary_message(adapter, monkeypatch):
    ...
    channel = FakeHistoryChannel(
        [
            ...
            make_history_message(
                author=adapter._client.user,
                content="💾 Self-improvement review: Skill 'kova-gateway-display-config' patched",
                msg_id=4,
            ),
