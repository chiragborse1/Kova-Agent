...
``kova update``, serving pre-update bridge.js behavior forever (e.g.
...
class TestDepRefreshStamp:
    ...
    async def test_reinstalls_when_package_json_changed(self, tmp_path):
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        # Simulate `kova update` bumping the Baileys pin
        ...
        from plugins.platforms.whatsapp.adapter import _file_content_hash
        stamp = (bridge_dir / "node_modules" / ".kova-pkg-hash").read_text().strip()
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["KOVA_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["KOVA_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["KOVA_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
