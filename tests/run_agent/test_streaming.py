...
class TestStreamingFallback:
    ...
    def test_exhausted_transient_stream_error_propagates(self, mock_close, mock_create):
        ...
        # Should have retried 3 times (default KOVA_STREAM_RETRIES=2 → 3 attempts)
    ...
    def test_sse_connection_lost_retried_as_transient(self, mock_close, mock_create):
        ...
        # Should retry 3 times (default KOVA_STREAM_RETRIES=2 → 3 attempts)
        assert mock_client.chat.completions.create.call_count == 3
...
class TestPartialToolCallWarning:
    ...
    def test_partial_tool_call_surfaces_warning(self, mock_close, mock_create):
        ...
        import os as _os
        _prev = _os.environ.get("KOVA_STREAM_RETRIES")
        _os.environ["KOVA_STREAM_RETRIES"] = "0"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
                _os.environ.pop("KOVA_STREAM_RETRIES", None)
            else:
                _os.environ["KOVA_STREAM_RETRIES"] = _prev
...
class TestSilentRetryMidToolCall:
    ...
    def test_silent_retry_recovers_tool_call(
        ...
        import os as _os
        _prev = _os.environ.get("KOVA_STREAM_RETRIES")
        _os.environ["KOVA_STREAM_RETRIES"] = "2"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
    ...
    def test_silent_retry_exhausted_falls_back_to_stub(
        ...
        import os as _os
        _prev = _os.environ.get("KOVA_STREAM_RETRIES")
        _os.environ["KOVA_STREAM_RETRIES"] = "1"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
