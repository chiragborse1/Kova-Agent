...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    kova_state = ModuleType("kova_state")
    kova_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "kova_state", kova_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "kova"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "kova"
