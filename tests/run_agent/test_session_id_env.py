...
def test_session_id_env_set_on_init():
    """AIAgent.__init__ sets KOVA_SESSION_ID in the environment."""
    agent = AIAgent(
        api_key="test-key",
    ...
    assert os.environ.get("KOVA_SESSION_ID") == agent.session_id
...
def test_session_id_contextvar_set():
    ...
    assert get_session_env("KOVA_SESSION_ID") == custom_id
