...
def test_moa_slots_routed_through_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_moa_slot_runtime_falls_back_on_resolution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.runtime_provider.resolve_runtime_provider", boom
    )
...
def test_reference_messages_strips_system_and_tool_history():
    ...
    messages = [
        {"role": "system", "content": "huge kova system prompt"},
