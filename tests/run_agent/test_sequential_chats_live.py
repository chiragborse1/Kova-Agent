...
AlexKucera reported on Discord (2026-04-16) that after ``kova update`` pulled
...
Opt-in — not part of default CI:
    KOVA_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.kova/.env).
"""
...
LIVE = os.environ.get("KOVA_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set KOVA_LIVE_TESTS=1"),
