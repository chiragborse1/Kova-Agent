"""Tests for `kova photon setup`'s access auto-configuration.

`_autoconfigure_access` allowlists the operator and points the cron home
channel at their DM, writing to the per-test ~/.kova/.env (the hermetic
KOVA_HOME fixture isolates this). It must fill only unset keys so a re-run
...
def test_setup_hint_uses_gateway_service_command(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    ...
    assert "Start the gateway:  Kova Gateway start" in out
