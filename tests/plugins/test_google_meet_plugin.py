...
def test_start_spawns_subprocess_and_writes_active_pointer(tmp_path):
    ...
    assert captured_env["KOVA_MEET_URL"] == "https://meet.google.com/abc-defg-hij"
    assert captured_env["KOVA_MEET_GUEST_NAME"] == "Test Bot"
    assert captured_env["KOVA_MEET_DURATION"] == "15m"
...
def test_transcript_reads_last_n_lines(tmp_path):
    ...
    meeting_dir = Path(os.environ["KOVA_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
def test_enqueue_say_rejects_transcribe_mode(tmp_path):
    ...
    out_dir = Path(os.environ["KOVA_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
def test_start_realtime_env_vars_threaded_through():
    ...
    assert captured_env["KOVA_MEET_MODE"] == "realtime"
    assert captured_env["KOVA_MEET_REALTIME_MODEL"] == "gpt-realtime"
    assert captured_env["KOVA_MEET_REALTIME_VOICE"] == "alloy"
    assert captured_env["KOVA_MEET_REALTIME_INSTRUCTIONS"] == "Be brief."
    assert captured_env["KOVA_MEET_REALTIME_KEY"] == "sk-test"
...
def test_cli_register_includes_node_subcommand():
    """`kova meet` argparse tree includes the node subtree."""
    import argparse
...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Kova Agent", "Kova Agent"):
        assert not _looks_like_human_speaker(s, "Kova Agent"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Kova Agent"), f"{s!r} SHOULD be human"
