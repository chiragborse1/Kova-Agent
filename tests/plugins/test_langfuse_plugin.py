...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing KOVA_ prefixed keys.
        assert "KOVA_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "KOVA_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "KOVA_LANGFUSE_PUBLIC_KEY", "KOVA_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("KOVA_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_KOVA_config(self, monkeypatch):
        ...
        # Drop any cached import of kova_cli.config.
        sys.modules.pop("kova_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "kova_cli.config" not in sys.modules, (
            "langfuse plugin imported kova_cli.config — regression toward "
...
# KOVA_LANGFUSE_PUBLIC_KEY / SECRET_KEY at a template value like
...
# kova logs.  The fix in `_get_langfuse()` validates the documented
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "KOVA_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "KOVA_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "KOVA_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "KOVA_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("KOVA_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("KOVA_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("KOVA_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "KOVA_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("KOVA_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("KOVA_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "KOVA_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("KOVA_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("KOVA_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
    ...
    def test_common_placeholders_detected(self, monkeypatch, caplog, placeholder):
        ...
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        assert "KOVA_LANGFUSE_PUBLIC_KEY" in caplog.text

    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical KOVA_-prefixed env var and
        ...
        # KOVA_-prefixed one — operators set the KOVA_-prefixed one).
        assert "KOVA_LANGFUSE_PUBLIC_KEY" in caplog.text
