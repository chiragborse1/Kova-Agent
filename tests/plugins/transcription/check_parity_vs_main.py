...
the legacy ``KOVA_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "kova-agent-main"
...
# Isolated KOVA_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'kova-agent-main' checkout pinned to "
