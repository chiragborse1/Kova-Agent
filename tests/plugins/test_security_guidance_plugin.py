...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "KOVA_plugins.security_guidance",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "KOVA_plugins.security_guidance"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["KOVA_plugins.security_guidance"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestPluginDiscovery:
    def test_loads_via_plugin_manager(self, _isolate_env, monkeypatch):
        ...
        for k in list(sys.modules):
            if k.startswith(("KOVA_plugins", "kova_cli.plugins")):
                del sys.modules[k]
