...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "KOVA_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``kova kanban boards switch`` last pinned.
    """
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "kova_cli.web_server", stub)
    monkeypatch.setattr(kova_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.kanban._check_dispatcher_presence",
...
# tests/kova_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see kova_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "kova-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "kova-kanban-card--failed" in js
    assert "kova-kanban-card--failed" in css
