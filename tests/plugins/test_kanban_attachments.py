...
Covers three layers:
  * ``kova_cli.kanban_db`` accessors (add/list/get/delete + path helpers)
...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "KOVA_dashboard_plugin_kanban_attach_test", plugin_file,
    )
...
def test_attachments_root_is_per_board(kanban_home, monkeypatch):
    ...
    monkeypatch.delenv("KOVA_KANBAN_ATTACHMENTS_ROOT", raising=False)
...
def test_attachments_root_env_override(kanban_home, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KOVA_KANBAN_ATTACHMENTS_ROOT", str(override))
