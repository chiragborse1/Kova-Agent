...
def test_post_setup_create_remote_user_can_keep_KOVA_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "kova peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this kova run."
    ]
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (Kova-Agent#28296)
# ---------------------------------------------------------------------------
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for Kova-Agent#28296 review (M3): once a session is marked
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for Kova-Agent#28296 review (H1): on_session_switch must
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See Kova-Agent#28296
