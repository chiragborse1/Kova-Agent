...
class TestPlatformBackend:
    ...
    def test_add_forwards_kwargs(self):
        ...
        result = backend.add(msgs, user_id="u1", agent_id="kova", infer=False)
        call = client.calls[0]
    ...
    def test_add_omits_empty_metadata(self):
        backend, client = self._make()
        msgs = [{"role": "user", "content": "hi"}]
        backend.add(msgs, user_id="u1", agent_id="kova", infer=False, metadata={})
...
class TestOSSBackend:
    ...
    def test_add_forwards_kwargs(self):
        backend, memory = self._make()
        msgs = [{"role": "user", "content": "hi"}]
        backend.add(msgs, user_id="u1", agent_id="kova", infer=False)
