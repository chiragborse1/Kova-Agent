...
def _inject_fake_KOVA_cli(monkeypatch):
    """Inject fake kova_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("kova_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("kova_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_KOVA_cli = types.ModuleType("kova_cli")
    fake_KOVA_cli.config = fake_config_mod
    fake_KOVA_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "kova_cli", fake_KOVA_cli)
    monkeypatch.setitem(sys.modules, "kova_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "kova_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["kova", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_kova_home", lambda: tmp_path)
        _inject_fake_KOVA_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_oss_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "kova", "--mode", "oss", "--oss-llm-key", "sk-oai",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_kova_home", lambda: tmp_path)
        _inject_fake_KOVA_cli(monkeypatch)
...
class TestDryRun:
    ...
    def test_dry_run_platform_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["kova", "--mode", "platform", "--api-key", "sk-test", "--dry-run"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_kova_home", lambda: tmp_path)
        _inject_fake_KOVA_cli(monkeypatch)
        config = {"memory": {}}
        post_setup(str(tmp_path), config)
    ...
    def test_dry_run_oss_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "kova", "--mode", "oss", "--oss-llm-key", "sk-oai", "--dry-run",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_kova_home", lambda: tmp_path)
        _inject_fake_KOVA_cli(monkeypatch)
        monkeypatch.setattr("plugins.memory.mem0._setup._install_provider_deps", lambda l, e, v: None)
