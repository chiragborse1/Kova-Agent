...
from kova_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_kova_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", KOVA_HOME=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", KOVA_HOME=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:kova-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:kova-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:kova-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:kova-agent"]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Kova Agent and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_kova_home",
        ...
        assert cfg["banks"]["kova"]["bankId"] == "env-bank"
        assert cfg["banks"]["kova"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_kova_home", lambda: KOVA_HOME)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("kova_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("kova_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(KOVA_HOME), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (KOVA_HOME / ".env").exists()
        assert not (KOVA_HOME / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "kova.env").exists()
    ...
    def test_local_embedded_setup_cancel_at_llm_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("kova_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", which)
        monkeypatch.setattr("subprocess.run", run)
        monkeypatch.setattr("builtins.input", MagicMock(side_effect=AssertionError("prompt should not run")))
        monkeypatch.setattr("getpass.getpass", MagicMock(side_effect=AssertionError("prompt should not run")))
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr("kova_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(KOVA_HOME), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (KOVA_HOME / ".env").read_text()
        assert "HINDSIGHT_LLM_API_KEY=sk-local-test\n" in env_text
        assert "HINDSIGHT_TIMEOUT=120\n" in env_text
        assert "HINDSIGHT_IDLE_TIMEOUT=300\n" in env_text

        profile_env = user_home / ".hindsight" / "profiles" / "kova.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(KOVA_HOME))
        provider.post_setup(str(KOVA_HOME), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        KOVA_env = user_home / ".hindsight" / "profiles" / "kova.env"
        assert coder_env.exists()
        assert not KOVA_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(KOVA_HOME))
        ...
        monkeypatch.setattr("kova_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((KOVA_HOME / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Kova Agent and the User"
        ...
        assert item["metadata"]["source"] == "kova"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Kova Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_kova_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", KOVA_HOME=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", KOVA_HOME=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("kova") == "kova"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "kova-{user}" becomes "kova-" -> trimmed to "kova"
        result = _resolve_bank_id_template(
            "kova-{user}", fallback="default",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        ...
        p.initialize(
            ...
            agent_workspace="kova",
        )
        assert p._bank_id == "kova-coder"
        assert p._bank_id_template == "kova-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "kova-" which collapses to "kova"
        p.initialize(session_id="s1", KOVA_HOME=str(tmp_path), platform="cli")
        assert p._bank_id == "kova"
