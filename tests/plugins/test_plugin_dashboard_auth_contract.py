...
plugin SDK (``window.__KOVA_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-kova-Session-Token`` header) and gated OAuth
(``KOVA_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__KOVA_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__KOVA_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + kova-achievements are bundled today; assert at least one is
