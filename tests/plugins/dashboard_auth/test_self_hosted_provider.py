...
class TestStartLogin:
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://kova.example/auth/callback")
        b = provider.start_login(redirect_uri="https://kova.example/auth/callback")
        assert (
            a.cookie_payload["KOVA_session_pkce"]
            != b.cookie_payload["KOVA_session_pkce"]
        )
...
class TestPluginRegister:
    ...
    def test_empty_env_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_DASHBOARD_OIDC_ISSUER", "")
        monkeypatch.setenv("KOVA_DASHBOARD_OIDC_CLIENT_ID", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_non_https_issuer_skips_with_reason(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv(
            "KOVA_DASHBOARD_OIDC_ISSUER", "http://insecure.example"
        )
        monkeypatch.setenv("KOVA_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
