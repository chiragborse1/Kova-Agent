...
from kova_cli.dashboard_auth import TokenPrincipal, assert_protocol_compliance
from kova_cli.dashboard_auth import token_auth
...
def _clean_env_and_routes(monkeypatch):
    monkeypatch.delenv("KOVA_DASHBOARD_DRAIN_SECRET", raising=False)
...
class TestRegister:
    def test_skips_when_no_secret(self, drain, monkeypatch):
        ...
        assert "KOVA_DASHBOARD_DRAIN_SECRET" in drain.LAST_SKIP_REASON
    ...
    def test_skips_and_fails_closed_on_weak_secret(self, drain, monkeypatch):
        monkeypatch.setenv("KOVA_DASHBOARD_DRAIN_SECRET", "tooweak")
        monkeypatch.setattr(drain, "_load_config_drain_auth_section", lambda: {})
        ctx = MagicMock()
        drain.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
    ...
    def test_registers_with_strong_env_secret(self, drain, monkeypatch):
        s = _strong_secret()
        monkeypatch.setenv("KOVA_DASHBOARD_DRAIN_SECRET", s)
        monkeypatch.setattr(drain, "_load_config_drain_auth_section", lambda: {})
        ctx = MagicMock()
        drain.register(ctx)
