...
""",
    ...
    monkeypatch.setenv("KOVA_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof"))
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))

    base = {
        "session_id": "s1",
        "task_id": "t1",
        "turn_id": "turn-1",
        "telemetry_schema_version": "kova.observer.v1",
    }
    ...
    assert (tmp_path / "atif" / "kova-atif-s1.json").exists()
...
def test_nemo_relay_plugin_emits_approval_marks(monkeypatch):
    ...
    assert "kova.approval.request" in mark_names
    assert "kova.approval.response" in mark_names
...
def test_nemo_relay_plugin_emits_unmatched_fallback_marks(monkeypatch):
    ...
    mark_names = [event[1] for event in fake.events if event[0] == "scope.event"]
    assert "kova.api.response.unmatched" in mark_names
    assert "kova.api.error" in mark_names
    assert "kova.tool.response.unmatched" in mark_names
...
def test_nemo_relay_plugin_metadata_promotes_trajectory_and_subagent_ids(monkeypatch):
    ...
    turn_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "kova.turn.start")
    ...
    start_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "kova.subagent.start")
    ...
    stop_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "kova.subagent.stop")
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "kova-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "kova-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "kova-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "kova-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE", "all")
    ...
    assert (tmp_path / "atif" / "kova-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "kova-atif-child-session.json").exists()
...
""",
    ...
    assert "kova-session-s2" in scope_push_names
...
""",
    ...
    monkeypatch.setenv("KOVA_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atif"))
    ...
    assert not (tmp_path / "direct-atif" / "kova-atif-s1.json").exists()
...
""",
    ...
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "kova-atif-s1.json").exists()
...
""",
    ...
    monkeypatch.setenv("KOVA_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("KOVA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atof"))
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(monkeypatch):
    ...
    # ``kova_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from kova_cli import middleware
