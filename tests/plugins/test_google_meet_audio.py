...
def test_setup_linux_loads_null_sink_and_virtual_source():
    ...
    assert any(a.startswith("sink_name=KOVA_meet_sink") for a in calls[0])
    assert calls[1][0] == "pactl" and calls[1][1] == "load-module"
    assert "module-virtual-source" in calls[1]
    assert any(a.startswith("source_name=KOVA_meet_src") for a in calls[1])
    assert any("master=KOVA_meet_sink.monitor" in a for a in calls[1])

    # Dict shape.
    assert info["platform"] == "linux"
    assert info["device_name"] == "KOVA_meet_src"
    assert info["write_target"] == "KOVA_meet_sink"
    assert info["sample_rate"] == 48000
    assert info["channels"] == 2
    assert info["module_ids"] == [42, 43]

    # Properties.
    assert br.device_name == "KOVA_meet_src"
    assert br.write_target == "KOVA_meet_sink"
...
def test_chrome_fake_audio_flags_linux():
    ...
    with patch("plugins.google_meet.audio_bridge.platform.system",
        ...
        flags = chrome_fake_audio_flags(
            {"platform": "linux", "device_name": "KOVA_meet_src"}
        )
