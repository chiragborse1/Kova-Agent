...
Run from the PR worktree:
    ...
    cd ~/.kova/kova-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.kova/kova-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.kova/kova-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a kova-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a kova-agent checkout"
)
...
# Isolated KOVA_HOME for the config write.
