...
(SQLITE_PROTOCOL — typical on NFS/SMB), kova must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``KOVA_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    kova_state._set_last_init_error(None)
    yield
    kova_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    kova_state._wal_fallback_warned_paths.clear()
    yield
    kova_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see kova_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        kova_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("kova_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        kova_state._set_last_init_error("OperationalError: generic SQLite error")
