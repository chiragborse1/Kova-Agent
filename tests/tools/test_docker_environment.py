...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across kova processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/kova-snap-test123.sh"
    env._cwd_file = "/tmp/kova-cwd-test123.txt"
    env._cwd_marker = "__KOVA_CWD_test123__"
...
def test_init_env_args_uses_KOVA_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _KOVA_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_KOVA_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_KOVA_dotenv(monkeypatch):
...
def test_init_env_args_uses_KOVA_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.kova/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_KOVA_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_KOVA_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled kova image, or ``gosu``/``su``
...
def test_run_command_tags_KOVA_agent_label(monkeypatch):
    """Every container kova-agent starts must carry the kova-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=kova-agent=1`` call. Regression test
    ...
    assert "kova-agent=1" in labels, (
        f"kova-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root kova home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "kova-task-id=kanban-42" in labels, (
        f"kova-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "kova-profile=research-bot" in labels, (
        f"kova-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "kova-task-id=task_with_weird_chars" in labels, (
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every kova process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new kova process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("kova-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    kova startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd kova processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a kova process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=kova-profile=<profile>`` to
    ...
    assert "label=kova-agent=1" in flat, (
        f"kova-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling kova process and must NEVER be reaped"
    )
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.kova/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.kova/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.kova/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "kova-agent:latest") is True
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="kova-agent:latest")
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: kova-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: kova-x", "returncode": 1},
