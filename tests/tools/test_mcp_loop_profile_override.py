"""Regression tests for KOVA_HOME override propagation onto the MCP loop.
...
get_kova_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: KovaTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(KovaTokenStorage("probe-srv")._tokens_path())
