...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.kova/credentials/a.json"),
            (str(src_b), "/root/.kova/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.kova/credentials/a.json" in names
            assert "root/.kova/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.kova/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.kova/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.kova/credentials/f.txt"),
            (str(src), "/root/.kova/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.kova/credentials" in cmd
        assert "/root/.kova/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.kova/cache/file_{i}.txt"))
    ...
    def test_bulk_upload_wired_in_filesyncmanager(self, monkeypatch):
        ...
        env._sync_manager = modal_env.FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.kova"),
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.kova/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.kova/large.bin" in names
