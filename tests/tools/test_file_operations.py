...
class TestIsWriteDenied:
    ...
    def test_KOVA_control_files_requested_writable(self, path):
        from kova_constants import get_kova_home

        assert _is_write_denied(str(get_kova_home() / path)) is False
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.kova/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__KOVA_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__KOVA_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "KOVA_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__KOVA_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__KOVA_FENCE_a9f7b3__\n"
        )
