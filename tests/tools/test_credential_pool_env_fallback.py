...
@pytest.fixture
def isolated_KOVA_HOME(tmp_path, monkeypatch):
    """Point KOVA_HOME at a temp dir and clear known API key env vars.
...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    load_KOVA_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_KOVA_HOME):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_KOVA_HOME, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
...
class TestAuthResolvesFromDotEnv:
    ...
    def test_key_from_dotenv_only(self, isolated_KOVA_HOME):
        ...
        _write_env_file(isolated_KOVA_HOME, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from kova_cli.auth import _resolve_api_key_provider_secret
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_KOVA_HOME):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_KOVA_HOME):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_KOVA_HOME, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_KOVA_HOME):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_KOVA_HOME, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
