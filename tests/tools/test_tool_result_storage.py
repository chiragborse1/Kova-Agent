...
class TestHeredocMarker:
    ...
    def test_uuid_marker_on_collision(self):
        ...
        assert marker.startswith("KOVA_PERSIST_")
...
class TestWriteToSandbox:
    def test_success(self):
        ...
        result = _write_to_sandbox("hello world", "/tmp/kova-results/abc.txt", env)
    ...
    def test_failure_returns_false(self):
        ...
        result = _write_to_sandbox("content", "/tmp/kova-results/abc.txt", env)
    ...
    def test_large_content_via_stdin(self):
        ...
        env.execute.return_value = {"output": "", "returncode": 0}
        big = "x" * 200_000
        _write_to_sandbox(big, "/tmp/kova-results/big.txt", env)
        cmd = env.execute.call_args[0][0]
    ...
    def test_timeout_passed(self):
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        _write_to_sandbox("content", "/tmp/kova-results/abc.txt", env)
        assert env.execute.call_args[1]["timeout"] == 30

    def test_uses_parent_dir_of_remote_path(self):
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        remote_path = "/data/data/com.termux/files/usr/tmp/kova-results/abc.txt"
        _write_to_sandbox("content", remote_path, env)
        cmd = env.execute.call_args[0][0]
        assert "mkdir -p /data/data/com.termux/files/usr/tmp/kova-results" in cmd

    def test_path_with_spaces_is_quoted(self):
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        remote_path = "/tmp/kova results/abc file.txt"
        _write_to_sandbox("content", remote_path, env)
        cmd = env.execute.call_args[0][0]
        assert "'/tmp/kova results'" in cmd
        assert "'/tmp/kova results/abc file.txt'" in cmd

    def test_shell_metacharacters_neutralized(self):
        """Paths with shell metacharacters must be quoted to prevent injection."""
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        malicious_path = "/tmp/kova-results/$(whoami).txt"
        _write_to_sandbox("content", malicious_path, env)
        cmd = env.execute.call_args[0][0]
        # The $() must not appear unquoted — shlex.quote wraps it
        assert "'/tmp/kova-results/$(whoami).txt'" in cmd
...
class TestResolveStorageDir:
    ...
    def test_uses_env_temp_dir_when_available(self):
        ...
        assert _resolve_storage_dir(env) == "/data/data/com.termux/files/usr/tmp/kova-results"
...
class TestBuildPersistedMessage:
    def test_structure(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/kova-results/test123.txt",
        )
        ...
        assert "/tmp/kova-results/test123.txt" in msg
    ...
    def test_no_ellipsis_when_complete(self):
        msg = _build_persisted_message(
            ...
            has_more=False,
            original_size=16,
            file_path="/tmp/kova-results/x.txt",
        )
    ...
    def test_large_size_shows_mb(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/kova-results/big.txt",
        )
        assert "MB" in msg
...
class TestMaybePersistToolResult:
    ...
    def test_env_temp_dir_changes_persisted_path(self):
        ...
        assert "/data/data/com.termux/files/usr/tmp/kova-results/tc_termux.txt" in result
        cmd = env.execute.call_args[0][0]
        assert "mkdir -p /data/data/com.termux/files/usr/tmp/kova-results" in cmd
