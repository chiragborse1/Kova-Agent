...
class TestCronApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "deny"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "off"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "allow"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "yes"}}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": "maybe"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_config_load_failure_defaults_to_deny(self):
        """If config loading fails entirely, default to deny (safe)."""
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", side_effect=RuntimeError("config broken")):
            assert _get_cron_approval_mode() == "deny"

    def test_yaml_boolean_false_maps_to_deny(self):
        """YAML 1.1 parses bare 'off' as False. Ensure it maps to deny."""
        from unittest.mock import patch as mock_patch
        with mock_patch("kova_cli.config.load_config", return_value={"approvals": {"cron_mode": False}}):
...
class TestCronDenyMode:
    """When KOVA_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""
...
class TestCronApproveMode:
    """When KOVA_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        # prompt injection from runtime-setting KOVA_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds KOVA_SESSION_PLATFORM via contextvars for
