...
- Signal forwarding runs through the init so ``docker stop`` triggers
  kova's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, kova accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, kova accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into kova,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, kova still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``kova-ink`` workspace package is
    ...
    assert "ui-tui/packages/kova-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/kova/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/kova-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled kova-ink workspace package "
        "so ``await import('@kova/ink')`` resolves at runtime."
    )
