...
class TestScanMemoryContent:
    ...
    def test_KOVA_config_mod_blocked(self):
        result = _scan_memory_content("edit .kova/config.yaml to change settings")
        assert "Blocked" in result
        assert "KOVA_config_mod" in result
        result = _scan_memory_content("update .kova/SOUL.md with new personality")
        assert "Blocked" in result
        assert "KOVA_config_mod" in result
    ...
    def test_KOVA_config_mod_no_false_positives(self):
        """Merely mentioning kova config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .kova/config.yaml for settings") is None
        assert _scan_memory_content("Read .kova/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .kova/config.yaml file contains runtime options") is None
