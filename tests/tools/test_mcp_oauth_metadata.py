...
Covers:
- :class:`KovaTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.KovaMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _KOVA_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# KovaTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = KovaTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (KovaMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: KovaTokenStorage, **context_attrs):
    ...
    """
    if _KOVA_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _KOVA_PROVIDER_CLS.__new__(_KOVA_PROVIDER_CLS)
    provider._KOVA_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("mgr-srv")
        ...
        with patch.object(
            _KOVA_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("noop-srv")
        ...
        with patch.object(
            KovaTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        storage = KovaTokenStorage("flow-srv")
        ...
        with patch.object(
            _KOVA_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
