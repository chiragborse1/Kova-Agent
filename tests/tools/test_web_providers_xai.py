...
class TestXAIProviderIsAvailable:
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.kova/auth.json
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``kova auth`` sign-in) rather than an env-var key.
    Patches at the ``kova_cli.runtime_provider.resolve_runtime_provider``
