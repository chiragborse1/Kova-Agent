...
from tools.environments.local import _KOVA_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `kova update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_KOVA_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_KOVA_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/KOVA_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/KOVA_bg_" not in bg_command
        assert "cat /tmp/KOVA_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/KOVA_bg.log",
                "/path with spaces/KOVA_bg.pid",
                "/path with spaces/KOVA_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/KOVA_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/KOVA_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/KOVA_bg.exit' 2>/dev/null"
