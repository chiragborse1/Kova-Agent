...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the kova_KANBAN_TASK env var means it's a focused
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["kova_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KOVA_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to kova_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``kova_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``kova-system`` and
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from kova_PROFILE in the fixture, not the
        # caller-supplied "kova-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("kova_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no kova_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KOVA_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from kova_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no kova_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (kova_KANBAN_TASK set) MUST have the full
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("kova_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("kova_KANBAN_RUN_ID", str(run2.id))
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via KOVA_KANBAN_BOARD env var,
...
# restarting kova. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated kova home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    kova_KANBAN_TASK is pinned (orchestrator context) — workers test
    ...
    monkeypatch.delenv("kova_KANBAN_TASK", raising=False)
    monkeypatch.setenv("kova_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning kova_KANBAN_TASK
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("kova_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("kova_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("KOVA_SESSION_KEY", raising=False)
    monkeypatch.delenv("KOVA_SESSION_ID", raising=False)
