...
def test_save_config_set_supports_critical_bridged_keys():
    """``kova config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`kova config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to TERMINAL_CONFIG_ENV_MAP "
        f"in kova_cli/config.py (set_config_value bridges through it)."
    )
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``kova config
    ...
    producing the "kova partially reads the Docker config" symptom.  Guard
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on kova exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether kova sweeps stale
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``kova config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``kova config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
