...
def isolated_session(monkeypatch, tmp_path):
    ...
    KOVA_HOME so the real user's command_allowlist doesn't leak in."""
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: KOVA_INTERACTIVE is set, the
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("kova_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("kova_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: KOVA_GATEWAY_SESSION is set and a
