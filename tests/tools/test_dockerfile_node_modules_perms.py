...
Older images made /opt/kova/ui-tui and /opt/kova/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/kova/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R kova:kova /opt/kova/ui-tui" not in text
    assert "chown -R kova:kova /opt/kova/node_modules" not in text
