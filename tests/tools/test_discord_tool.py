...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in kova-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_KOVA_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["kova-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["kova-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _KOVA_CORE_TOOLS
        assert "discord" not in _KOVA_CORE_TOOLS
        assert "discord_admin" not in _KOVA_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"kova-discord", "kova-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["kova-discord"], quiet_mode=True)
