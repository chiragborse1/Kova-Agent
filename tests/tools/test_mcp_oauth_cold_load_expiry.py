...
1. ``KovaTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``KovaMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# KovaTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        from tools.mcp_oauth import KovaTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# KovaMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import KovaTokenStorage
    from tools.mcp_oauth_manager import _KOVA_PROVIDER_CLS, reset_manager_for_tests
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Kova Agent",
    )
    provider = _KOVA_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = KovaTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Kova Agent",
    )
