...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_kova):
        from tools.file_tools import write_file_tool
        target = fake_kova["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_kova):
        ...
        assert "kova-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_kova):
    ...
    def test_non_KOVA_path_unaffected(self, fake_kova, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_kova):
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_kova):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_multiple_profiles(self, fake_kova, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_kova["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_kova["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_kova):
        """When active profile is 'kova-security', the prompt warns
