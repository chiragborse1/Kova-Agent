...
``kova_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``KOVA_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        "kova_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``kova_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``kova_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        stub = generate_KOVA_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_KOVA_tools_module
        import tempfile, ast
        stub = generate_KOVA_tools_module(
...
# let the child import kova_tools, a user script that printed non-ASCII
