...
Verifies that kova-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed kova secrets.

See: https://github.com/chiragborse1/Kova-Agent/issues/1002
See: https://github.com/chiragborse1/Kova-Agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _KOVA_PROVIDER_ENV_BLOCKLIST,
    _KOVA_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.kova/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a kova inference secret
        ...
        models`` run inside a kova terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _KOVA_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        kova-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "KOVA_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _KOVA_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_KOVA_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_KOVA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_KOVA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_KOVA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_KOVA_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their kova-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _KOVA_PROVIDER_ENV_BLOCKLIST
...
class TestkovaBinDirOnPath:
    """The kova install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``kova`` via the terminal tool must work
    even when the gateway was launched without the kova install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_KOVA_bin_dir / _prepend_KOVA_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._KOVA_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/kova/bin/kova" if name == "kova" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/kova/bin")
        assert local_mod._resolve_KOVA_bin_dir() == "/opt/kova/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "kova").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_KOVA_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_KOVA_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._KOVA_BIN_DIR = "/opt/kova/bin"
        out = local_mod._prepend_KOVA_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/kova/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._KOVA_BIN_DIR = "/opt/kova/bin"
        once = local_mod._prepend_KOVA_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_KOVA_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/kova/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._KOVA_BIN_DIR = None
        assert local_mod._prepend_KOVA_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_KOVA_bin_dir(self, monkeypatch):
        """A gateway env missing the kova dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._KOVA_BIN_DIR = "/opt/kova/bin"
