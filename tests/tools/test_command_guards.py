...
def _clean_state():
    ...
    saved = {}
    for k in ("KOVA_INTERACTIVE", "KOVA_GATEWAY_SESSION", "KOVA_EXEC_ASK", "KOVA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["KOVA_INTERACTIVE"] = "1"
        session_key = os.getenv("KOVA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No KOVA_INTERACTIVE or KOVA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
