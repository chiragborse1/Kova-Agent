...
class TestFileDedup(unittest.TestCase):
    ...
    def test_write_allows_large_file_that_quotes_status_text(self, mock_ops):
        ...
        kova' own docs / SKILL.md files may legitimately mention the dedup
...
class TestConfigOverride(unittest.TestCase):
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("kova_cli.config.load_config", return_value={"file_read_max_chars": 50})
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("kova_cli.config.load_config", return_value={"file_read_max_chars": 500_000})
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    Regression test for https://github.com/chiragborse1/Kova-Agent/issues/13144
    """
