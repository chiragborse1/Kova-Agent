...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_config_not_a_dict(self):
        ...
        with patch("kova_cli.config.load_config", return_value="not a dict"):
            limits = tol.get_tool_output_limits()
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("kova_cli.config.load_config", side_effect=_boom):
            limits = tol.get_tool_output_limits()
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in kova_cli.config must expose tool_output so
    that ``kova setup`` and default installs stay in sync with the
    helpers here."""
