...
class TestCamofoxIdentity:
    ...
    def test_identity_differs_by_profile(self, tmp_path):
        state = _load_module()
        with patch.object(state, "get_kova_home", return_value=tmp_path / "profile-a"):
            a = state.get_camofox_identity("task-1")
        with patch.object(state, "get_kova_home", return_value=tmp_path / "profile-b"):
    ...
    def test_default_task_id(self, tmp_path):
        ...
        with patch.object(state, "get_kova_home", return_value=tmp_path):
            ...
            assert identity["user_id"].startswith("KOVA_")
