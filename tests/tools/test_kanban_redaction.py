...
def worker_env(monkeypatch, tmp_path):
    """Isolated KOVA_HOME with a running task; returns the task id."""
    home = tmp_path / ".kova"
    home.mkdir()
...
# ---------------------------------------------------------------------------
# Negative test — force=True bypasses KOVA_REDACT_SECRETS=false
# ---------------------------------------------------------------------------

def test_scrub_respects_force_flag_regardless_of_config(worker_env, monkeypatch):
    """force=True must fire even when KOVA_REDACT_SECRETS=false is set."""
