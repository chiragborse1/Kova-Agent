"""Tests for --yolo (KOVA_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When KOVA_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With KOVA_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """KOVA_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("KOVA_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for KOVA_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("KOVA_YOLO_MODE", "")
        monkeypatch.setenv("KOVA_INTERACTIVE", "1")
        monkeypatch.setenv("KOVA_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("KOVA_YOLO_MODE") returns ""
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("KOVA_YOLO_MODE", value)
        monkeypatch.setenv("KOVA_INTERACTIVE", "1")
        monkeypatch.setenv("KOVA_SESSION_KEY", "test-session")
