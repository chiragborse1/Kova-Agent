...
class TestSensitivePathCheck:
    ...
    def test_KOVA_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved_loaded", True)
    ...
    def test_KOVA_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved_loaded", True)
    ...
    def test_KOVA_config_blocked_for_patch(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved", "/home/user/.kova/config.yaml")
        monkeypatch.setattr("tools.file_tools._KOVA_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
