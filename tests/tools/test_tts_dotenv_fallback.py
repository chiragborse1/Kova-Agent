...
already addressed for ``agent/credential_pool`` and ``kova_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    ...
    ``kova_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while kova_cli.config.get_env_value is patched must
