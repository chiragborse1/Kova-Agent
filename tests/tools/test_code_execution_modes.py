...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("kova_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The kova-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_KOVA_tools(self):
        """Regression: kova_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from kova_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
    ...
    def test_strict_mode_can_still_import_KOVA_tools(self):
