...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.kova/)``
    ...
    def test_refuses_KOVA_HOME_itself(self, tmp_path):
        """``~/.kova/`` itself is what the #48200 wipe destroyed."""
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `kova profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.kova) rely on so bundled
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.KOVA_HOME", KOVA_HOME):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`kova skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.KOVA_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.KOVA_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
