...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad KOVA_ prefix dropped,
...
def test_env_scrub_KOVA_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "KOVA_KANBAN_DB": "postgres://u:p@h/db",
...
def test_env_scrub_logs_dropped_KOVA_vars(caplog):
    """Dropping a non-allowlisted, non-secret KOVA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset KOVA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "KOVA_HOME": "/h",          # allowlisted → kept, not logged
        "KOVA_BASE_URL": "https://x",   # dropped → logged
        "KOVA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "KOVA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "KOVA_HOME" in out and "PATH" in out
    assert "KOVA_BASE_URL" not in out and "KOVA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "KOVA_BASE_URL" in msgs and "KOVA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "KOVA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped KOVA_* vars."""
    import logging
