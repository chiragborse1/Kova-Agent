...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        with patch("kova_cli.config.read_raw_config", side_effect=Exception("no config")):
            assert _global_allow_private_urls() is False

    def test_env_var_true(self, monkeypatch):
        """KOVA_ALLOW_PRIVATE_URLS=true enables the toggle."""
        monkeypatch.setenv("KOVA_ALLOW_PRIVATE_URLS", "true")
        assert _global_allow_private_urls() is True

    def test_env_var_1(self, monkeypatch):
        """KOVA_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("KOVA_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """KOVA_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("KOVA_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """KOVA_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("KOVA_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
