...
def _make_get_files(tmp_files, remote_base="/root/.kova"):
...
def _make_manager(tmp_files, remote_base="/root/.kova", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.kova/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
