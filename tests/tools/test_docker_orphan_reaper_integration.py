...
def test_maybe_reap_respects_disable_flag(monkeypatch):
    ...
    hatch for operators running multiple kova processes in the same
    profile."""
    _reset_reaper_gate()
    call_count = {"reap": 0}
...
def test_maybe_reap_doubles_lifetime_for_max_age(monkeypatch):
    ...
    floor). Generous default — gives sibling kova processes ample grace
...
def test_maybe_reap_passes_current_profile_as_filter(monkeypatch):
    """The reaper must be scoped to the current kova profile — a research
