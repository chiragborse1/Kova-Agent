...
``~/.kova/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, KOVA_HOME):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under KOVA_HOME/logs/
        assert p.parent == KOVA_HOME / "logs"
    ...
    def test_truncates_very_long_goal(self, KOVA_HOME):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        huge_goal = "x" * 5000
    ...
    def test_missing_worker_thread_is_handled(self, KOVA_HOME):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        path = _dump_subagent_timeout_diagnostic(
            child=child,
    ...
    def test_exited_worker_thread_is_handled(self, KOVA_HOME):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point KOVA_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".kova"
        monkeypatch.setenv("KOVA_HOME", str(bogus))
        child = _StubChild()
...
class TestRunSingleChildTimeoutDump:
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, KOVA_HOME, monkeypatch):
        ...
        assert dump_path.parent == KOVA_HOME / "logs"
    ...
    def test_nonzero_api_calls_skips_dump_and_uses_old_message(self, KOVA_HOME, monkeypatch):
