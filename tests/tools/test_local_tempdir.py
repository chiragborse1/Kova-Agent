...
class TestLocalTempDir:
    def test_uses_os_tmpdir_for_session_artifacts(self, monkeypatch):
        ...
        assert env._snapshot_path == f"/data/data/com.termux/files/usr/tmp/kova-snap-{env._session_id}.sh"
        assert env._cwd_file == f"/data/data/com.termux/files/usr/tmp/kova-cwd-{env._session_id}.txt"
    ...
    def test_prefers_backend_env_tmpdir_override(self, monkeypatch):
        ...
        with patch.object(LocalEnvironment, "init_session", autospec=True, return_value=None):
            env = LocalEnvironment(
                cwd=".",
                timeout=10,
                env={"TMPDIR": "/data/data/com.termux/files/home/.cache/kova-tmp/"},
            )

        assert env.get_temp_dir() == "/data/data/com.termux/files/home/.cache/kova-tmp"
        assert env._snapshot_path == (
            f"/data/data/com.termux/files/home/.cache/kova-tmp/kova-snap-{env._session_id}.sh"
        )
        assert env._cwd_file == (
            f"/data/data/com.termux/files/home/.cache/kova-tmp/kova-cwd-{env._session_id}.txt"
        )
    ...
    def test_falls_back_to_tempfile_when_tmp_missing(self, monkeypatch):
        ...
        with patch("tools.environments.local.os.path.isdir", return_value=False), \
            ...
            assert env._snapshot_path == f"/cache/tmp/kova-snap-{env._session_id}.sh"
            assert env._cwd_file == f"/cache/tmp/kova-cwd-{env._session_id}.txt"
