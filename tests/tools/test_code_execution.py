...
UDS socket lifecycle, kova_tools generation, timeout enforcement,
...
class TestKovaToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_KOVA_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_KOVA_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_KOVA_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_KOVA_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_KOVA_tools_module(["terminal"])
        self.assertIn("kova_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_KOVA_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"kova_rpc\")", src)
        self.assertNotIn('os.environ.get("kova_RPC_DIR", "/tmp/kova_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_KOVA_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/kova_exec_", mkdir_cmd)
        self.assertIn("kova_RPC_DIR=/data/data/com.termux/files/usr/tmp/kova_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/kova_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/kova_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """KOVA_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"KOVA_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import kova_constants; print(kova_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("kova_constants.py", result["output"])
...
"""
        ...
        # terminal won't be in kova_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from kova_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from kova_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from kova_tools import shell_quote
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `kova tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `kova tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_KOVA_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("kova_RPC_SOCKET", child_env)
    ...
    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["KOVA_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("KOVA_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from kova_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from kova_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("kova_cli.config.read_raw_config", return_value={}):
            result = _load_config()
