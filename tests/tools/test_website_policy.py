...
def test_check_website_access_uses_dynamic_KOVA_HOME(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new KOVA_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old KOVA_HOME set by the autouse _isolate_KOVA_HOME fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing KOVA_HOME to tmp_path
