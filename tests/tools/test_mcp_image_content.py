...
kova' existing ``cache_image_from_bytes`` so a ``MEDIA:<path>`` tag
...
def _png_bytes():
    ...
    kova' ``cache_image_from_bytes`` has a format-sniff guard that rejects
...
class TestCacheMcpImageBlock:
    def test_returns_media_tag_for_valid_image_block(self, tmp_path, monkeypatch):
        ...
        # The cached file should be in kova' image cache dir
        from gateway.platforms.base import get_image_cache_dir
        cache_dir = str(get_image_cache_dir().resolve())
        assert tag.startswith(f"MEDIA:{cache_dir}"), (
            f"cached file not under KOVA_HOME image cache dir. "
