...
class TestSaveClipboardImage:
    def test_dispatches_to_macos_on_darwin(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("kova_cli.clipboard._macos_save", return_value=False) as m:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.sys") as mock_sys:
            ...
            with patch("kova_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_dispatches_to_linux_on_linux(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.sys") as mock_sys:
            ...
            with patch("kova_cli.clipboard._linux_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("kova_cli.clipboard._linux_save", return_value=False):
                save_clipboard_image(dest)
...
class TestMacosPngpaste:
    ...
    def test_not_installed(self, tmp_path):
        with patch("kova_cli.clipboard.subprocess.run", side_effect=FileNotFoundError):
    ...
    def test_timeout_returns_false(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("kova_cli.clipboard.subprocess.run",
...
class TestMacosOsascript:
    ...
    def test_clipboard_info_fails(self, tmp_path):
        with patch("kova_cli.clipboard.subprocess.run", side_effect=Exception("fail")):
            assert _macos_osascript(tmp_path / "out.png") is False
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is kova_constants.is_wsl; reset the function's own module
        # globals so this stays stable even if kova_constants was imported
        # through a different module object earlier in a large xdist run.
        import kova_constants
        kova_constants._wsl_detected = None
    ...
    def test_regular_linux(self):
        ...
        # supposed to intercept kova_constants.is_wsl's `open` call,
...
class TestWaylandSave:
    ...
    def test_bmp_extraction_with_pillow_convert(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("kova_cli.clipboard._convert_to_png", side_effect=fake_convert):
                assert _wayland_save(dest) is True
    ...
    def test_jpeg_extraction_converts_to_real_png(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("kova_cli.clipboard._convert_to_png", side_effect=fake_convert) as mock_convert:
                assert _wayland_save(dest) is True
    ...
    def test_non_png_conversion_failure_cleans_up(self, tmp_path):
        ...
        with patch("kova_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("kova_cli.clipboard._convert_to_png", return_value=True):
...
class TestLinuxSave:
    ...
    def test_wsl_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("kova_cli.clipboard._is_wsl", return_value=True):
            with patch("kova_cli.clipboard._wsl_save", return_value=False):
    ...
    def test_wayland_tried_when_display_set(self, tmp_path):
        ...
        with patch("kova_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("kova_cli.clipboard._wayland_save", return_value=True) as m:
                    assert _linux_save(dest) is True
                    m.assert_called_once_with(dest)
...
class TestWindowsHasImage:
    ...
    def test_no_powershell_available(self):
        with patch("kova_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("kova_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch("kova_cli.clipboard._windows_has_image", return_value=True) as m:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("kova_cli.clipboard.subprocess.run", side_effect=fake_run_fail):
    ...
    def test_imagemagick_timeout_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("kova_cli.clipboard.subprocess.run", side_effect=subprocess.TimeoutExpired("convert", 5)):
                _convert_to_png(dest)
...
class TestHasClipboardImage:
    ...
    def test_linux_wsl_dispatch(self):
        with patch("kova_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("kova_cli.clipboard._is_wsl", return_value=True):
                with patch("kova_cli.clipboard._wsl_has_image", return_value=True) as m:
                    assert has_clipboard_image() is True
                    m.assert_called_once()
    ...
    def test_linux_wayland_dispatch(self):
        with patch("kova_cli.clipboard.sys") as mock_sys:
            ...
            with patch("kova_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch("kova_cli.clipboard._wayland_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
    ...
    def test_linux_x11_dispatch(self):
        with patch("kova_cli.clipboard.sys") as mock_sys:
            ...
            with patch("kova_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch("kova_cli.clipboard._xclip_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
...
class TestTryAttachClipboardImage:
    ...
    def test_no_image_doesnt_attach(self, cli):
        with patch("kova_cli.clipboard.save_clipboard_image", return_value=False):
            result = cli._try_attach_clipboard_image()
    ...
    def test_mixed_success_and_failure(self, cli):
        ...
        with patch("kova_cli.clipboard.save_clipboard_image", side_effect=results):
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
