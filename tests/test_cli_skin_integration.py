...
from cli import KovaCLI, _build_compact_banner, _rich_text_from_ansi
from kova_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("kova_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_KOVA_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Kova Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "NOUS kova" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_kova(self):
        ...
        assert "NOUS kova" not in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Kova Agent v1.0 (test) · upstream abc12345"}):
            banner = _build_compact_banner()
