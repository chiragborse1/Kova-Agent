"""Regression tests for the kova-managed Node's npm global prefix.

When the installer falls back to a bundled Node under ``$KOVA_HOME/node``,
npm's default global prefix is that Node dir, so ``npm install -g <pkg>``
drops the package binary in ``$KOVA_HOME/node/bin`` — which is NOT on PATH
...
def test_install_sh_redirects_bundled_npm_global_prefix_to_link_dir() -> None:
    ...
    assert 'printf \'prefix=%s\\n\' "$(dirname "$link_dir")" > "$KOVA_HOME/node/etc/npmrc"' in text
...
def test_install_sh_repairs_existing_managed_node_on_rerun() -> None:
    ...
    assert '[ -x "$KOVA_HOME/node/bin/npm" ] || return 0' in text
...
def test_node_bootstrap_redirects_bundled_npm_global_prefix_to_link_dir() -> None:
    ...
    assert 'printf \'prefix=%s\\n\' "$(dirname "$_link_dir")" > "$KOVA_HOME/node/etc/npmrc"' in text
