...
def test_sessiondb_auto_heals_on_open(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(kova_state, "_repair_attempted_paths", set())
...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    calls = {"n": 0}
    real_repair = kova_state.repair_state_db_schema
    ...
    monkeypatch.setattr(kova_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(kova_state, "repair_state_db_schema", real_repair)
...
def test_strategy_b_rebuild_when_dedup_insufficient(tmp_path, monkeypatch):
    ...
    real_check = kova_state._db_opens_cleanly
    calls = {"n": 0}
    ...
    monkeypatch.setattr(kova_state, "_db_opens_cleanly", flaky_check)
    report = repair_state_db_schema(db_path)
    monkeypatch.undo()
...
def test_non_malformed_error_is_not_auto_repaired(tmp_path, monkeypatch):
    ...
    called = {"n": 0}
    orig = kova_state.repair_state_db_schema

    def spy(*a, **kw):
        called["n"] += 1
        return orig(*a, **kw)

    monkeypatch.setattr(kova_state, "repair_state_db_schema", spy)
    with pytest.raises(sqlite3.DatabaseError):
        SessionDB(db_path=db_path)
...
def test_fts_write_corruption_detected_by_write_probe(tmp_path):
    ...
    from kova_state import _db_opens_cleanly
