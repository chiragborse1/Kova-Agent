...
from kova_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(KOVA_HOME: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <KOVA_HOME>/plugins/<name> and opt it in."""
    plugin_dir = KOVA_HOME / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path / "KOVA_empty"))
