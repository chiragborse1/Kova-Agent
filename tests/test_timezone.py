"""
Tests for timezone support (kova_time module + integration points).
...
def _reset_KOVA_time_cache():
    """Reset the kova_time module cache (replacement for removed reset_cache)."""
    kova_time._cached_tz = None
    kova_time._cached_tz_name = None
    kova_time._cache_resolved = False


# =========================================================================
# kova_time.now() — core helper
# =========================================================================

class TestKovaTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_KOVA_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["KOVA_TIMEZONE"] = "Asia/Kolkata"
        result = kova_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["KOVA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="kova_time"):
            result = kova_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["KOVA_TIMEZONE"] = "Europe/London"
        tz = kova_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["KOVA_TIMEZONE"] = "Not/A/Timezone"
        tz = kova_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When KOVA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) KOVA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("KOVA_TIMEZONE=" + os.environ.get("KOVA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "KOVA_TIMEZONE=NOT_SET" in result["output"], (
            "KOVA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When KOVA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("KOVA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=KOVA_tz) which shifted
        absolute time when system-local tz != kova tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_KOVA_tz(self):
        """Already-aware datetimes should be normalized to kova tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in kova tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of kova tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), kova is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when kova tz
        ...
        # Use a kova timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _KOVA_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["KOVA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_KOVA_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of kova timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["KOVA_TIMEZONE"] = "US/Eastern"
        _reset_KOVA_time_cache()
