...
The ``kova update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m kova_cli.main gateway run`` — image name
    ``pythonw``, not ``kova.exe`` — so the ``taskkill /IM kova.exe`` guard
    ...
    # The kova.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM kova.exe' in text
