    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"kova": {"saveMessages": True, "recallMode": "hybrid"}},
            }
...
def fake_as(monkeypatch):
    ...
    monkeypatch.setenv("HONCHO_OAUTH_CLIENT_ID", "kova-desktop")
    try:
        yield base
    finally:
        server.shutdown()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "kova", now=saved["hosts"]["kova"]["oauth"]["expiresAt"] + 10
    )
...
def test_state_mismatch_is_rejected(fake_as, tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown or expired"):
        oauth_flow.complete_authorization(
            ...
            config_path=tmp_path / "honcho.json", host="kova",
        )


def test_source_tags_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, source="kova-cli")
    assert "source=kova-cli" in url
    untagged, _ = oauth_flow.begin_authorization(endpoints)
    assert "source=" not in untagged


def test_client_id_defaults_to_KOVA_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "kova-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=kova-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["kova"]["oauth"]["clientId"] == "kova-agent"


def test_config_path_rides_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, config_path="~/.kova/honcho.json")
    q = parse_qs(urlparse(url).query)
    assert q["config_path"][0] == "~/.kova/honcho.json"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".kova" / "profiles" / "work" / "honcho.json"
    assert oauth_flow._display_config_path(under_home) == "~/.kova/profiles/work/honcho.json"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"kova": {"saveMessages": False}}}))
...
def test_launcher_runs_flow_in_background_and_reports_connected(monkeypatch, reset_flow):
    ...
    st = oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="kova")
    assert st["state"] == "pending"  # returns immediately, before the flow finishes
    assert _wait_until(lambda: seen.get("source") == "kova-desktop")  # default source tag
    assert seen["host"] == "kova"
    gate.set()
...
def test_launcher_reports_error_on_flow_failure(monkeypatch, reset_flow):
    ...
    oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="kova")
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="kova")
    assert _wait_until(lambda: len(calls) == 1)  # first flow is running
    s2 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="kova")
    block.set()
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "kova")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"kova": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"kova": {"apiKey": "hch-v3-static"}}}))
    ...
    cfgfile.write_text(json.dumps({"hosts": {"kova": {
        ...
        "oauth": {"refreshToken": "hch-rt-x", "expiresAt": 9_999_999_999,
                  "clientId": "kova-desktop", "tokenEndpoint": "http://x/oauth/token"},
    }}}))
    s = oauth_flow.get_flow_status()
...
def test_memory_oauth_router_dispatches_by_provider_convention():
    ...
    from kova_cli.memory_oauth import _resolve_flow
