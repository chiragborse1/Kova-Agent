...
class TestCredentialModel:
    def test_roundtrip(self):
        ...
        assert block["clientId"] == "kova-desktop"
...
class TestEnsureFreshToken:
    def test_no_oauth_credential_is_noop(self, tmp_path):
        ...
        _write(path, {"hosts": {"kova": {"apiKey": "hch-v3-static"}}})
        token, refreshed = oauth.ensure_fresh_token(path, "kova", now=0)
        assert token is None and refreshed is False

    def test_fresh_token_skips_refresh(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"kova": _host_block(expires_at=10_000)}})
        monkeypatch.setattr(
    ...
    def test_fresh_token_served_from_cache_without_disk(self, tmp_path, monkeypatch):
        ...
        oauth.ensure_fresh_token(path, "kova", now=0)
        ...
        token, refreshed = oauth.ensure_fresh_token(path, "kova", now=100)
        assert token == "hch-at-old" and refreshed is False

    def test_expired_token_refreshes_and_persists_rotation(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"kova": _host_block(expires_at=100)}})

        def fake_post(url, data, timeout):
            assert data["grant_type"] == "refresh_token"
            assert data["refresh_token"] == "hch-rt-old"
            assert data["client_id"] == "kova-desktop"
            return {
        ...
        token, refreshed = oauth.ensure_fresh_token(path, "kova", now=1000)
    ...
    def test_double_check_uses_disk_when_already_rotated(self, tmp_path, monkeypatch):
        ...
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"kova": _host_block(refresh="hch-rt-fresh", expires_at=10_000)}})
        stale_raw = {"hosts": {"kova": _host_block(refresh="hch-rt-old", expires_at=100)}}
        stale_raw["hosts"]["kova"]["apiKey"] = "hch-at-stale"
        monkeypatch.setattr(
            oauth, "_http_post_form",
            lambda *a, **k: pytest.fail("must not refresh; disk token is fresh"),
        )
        token, refreshed = oauth.ensure_fresh_token(path, "kova", stale_raw, now=1000)
...
class TestInstallGrant:
    def test_deep_merges_config_and_preserves_other_hosts(self, tmp_path):
        ...
        _write(path, {
            ...
            "hosts": {
                ...
                "kova": {"workspace": "kova", "saveMessages": False},
            },
        })
        grant = {
        ...
        assert h["workspace"] == "kova"  # pre-existing key preserved
    ...
    def test_rejects_grant_without_tokens(self, tmp_path):
        ...
        with pytest.raises(ValueError):
            oauth.install_grant(
                path, "kova", {"access_token": "hch-at-x"},  # no refresh_token
