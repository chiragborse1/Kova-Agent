...
class TestFromGlobalConfig:
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"kova": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kova-agent"
        ):
            result = config.resolve_session_name("/home/user/kova-agent/subdir")
...
class TestResolveConfigPath:
    def test_prefers_KOVA_HOME_when_exists(self, tmp_path):
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        ...
        assert _get_default_KOVA_HOME() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_KOVA_HOME_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".kova".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".kova"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "KOVA_coder"
        assert profile_host_key("default") == "kova"

    def test_default_returns_kova(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("KOVA_HONCHO_HOST", None)
            os.environ.pop("KOVA_HOME", None)
            assert resolve_active_host() == "kova"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"KOVA_HONCHO_HOST": "kova.coder"}):
            assert resolve_active_host() == "kova.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_HONCHO_HOST", None)
            with patch("kova_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "KOVA_coder"

    def test_default_profile_returns_kova(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_HONCHO_HOST", None)
            with patch("kova_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "kova"

    def test_custom_profile_returns_kova(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_HONCHO_HOST", None)
            with patch("kova_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "kova"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_HONCHO_HOST", None)
            # Temporarily remove kova_cli.profiles to simulate import failure
            saved = sys.modules.get("kova_cli.profiles")
            sys.modules["kova_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kova"
            finally:
                if saved is not None:
                    sys.modules["kova_cli.profiles"] = saved
                else:
                    sys.modules.pop("kova_cli.profiles", None)
...
class TestProfileScopedConfig:
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "kova": {"aiPeer": "kova", "peerName": "alice"},
                "KOVA_coder": {
                    "aiPeer": "KOVA_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="KOVA_coder", config_path=config_file,
        )
        assert config.host == "KOVA_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "KOVA_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="KOVA_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kova.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="KOVA_dreamer",
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kova": {"enabled": True, "aiPeer": "kova"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kova": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_KOVA_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="kova",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kova_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kova_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_KOVA_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="kova",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kova_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
