...
CronScheduler plan, .kova/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - kova_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from kova_cli.web_server import _start_desktop_cron_ticker
...
def test_cron_status_reports_alive_but_failing(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("kova_cli.gateway.find_gateway_pids", lambda: [4321])
