...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("kova_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._KOVA_HOME", tmp_path), \
             ...
             patch("kova_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``kova tools`` platform config for ``cron``
        ...
        # The cron default is _KOVA_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``kova tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``kova tools`` configured to enable web+file
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_KOVA_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_KOVA_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_KOVA_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_KOVA_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_KOVA_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved KOVA_MODEL.
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._KOVA_HOME", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_KOVA_HOME", return_value=tmp_path), \
             patch("dotenv.load_dotenv"), \
             patch("kova_state.SessionDB", return_value=fake_db), \
             patch(
                 "kova_cli.runtime_provider.resolve_runtime_provider",
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``kova cron create --deliver=<platform>`` silently
