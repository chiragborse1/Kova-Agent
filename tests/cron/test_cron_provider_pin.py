...
resolve_runtime_provider against a temp KOVA_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("kova_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_kova_home", lambda: tmp_path, raising=True
        )
        with patch(
            "kova_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
