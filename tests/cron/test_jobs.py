...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED kova
        ...
        due-check compares against ``_KOVA_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("cron.jobs._KOVA_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured kova timezone differs from the
    ...
    def test_recent_past_oneshot_is_due_under_diverging_tz(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._KOVA_now", lambda: now)
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _KOVA_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _KOVA_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _KOVA_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _KOVA_now(), "next_run_at should remain in the future"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_KOVA_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_KOVA_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_stale_repeat_limited_job_consumes_one_run_on_catchup(self, tmp_cron_dir, monkeypatch):
        ...
        from cron.jobs import _KOVA_now
        job = create_job(prompt="Limited", schedule="every 5m", repeat=3)
        jobs = load_jobs()
        jobs[0]["next_run_at"] = (_KOVA_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["last_run_at"] = (_KOVA_now() - timedelta(minutes=11)).isoformat()
        save_jobs(jobs)
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while kova/system local time was UTC+10.
...
class TestCronOutputRetention:
    ...
    def test_save_job_output_prunes_old_runs(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._KOVA_now", lambda: next(seq))
        for _ in range(8):
    ...
    def test_cron_output_keep_reads_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.config.load_config", lambda: {"cron": {"output_retention": 7}}
        )
    ...
    def test_cron_output_keep_defaults_on_bad_config(self, monkeypatch):
        import cron.jobs as jobs
        monkeypatch.setattr(
            "kova_cli.config.load_config", lambda: {"cron": {"output_retention": "oops"}}
        )
