...
- Path resolution (absolute, relative to KOVA_HOME/scripts/)
"""
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit kova provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_KOVA_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.kova/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("KOVA_SESSION_PLATFORM") is None
        assert os.environ.get("KOVA_SESSION_CHAT_ID") is None
        assert os.environ.get("KOVA_SESSION_CHAT_NAME") is None
