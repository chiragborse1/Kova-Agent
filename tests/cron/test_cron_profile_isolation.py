...
profile's KOVA_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.kova/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active KOVA_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever KOVA_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    KOVA_HOME is a profile under it (``<root>/profiles/<name>``)."""
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile KOVA_HOME (<root>/profiles/<name>), the cron store
...
def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (KOVA_HOME == root), the store is the root's cron dir
