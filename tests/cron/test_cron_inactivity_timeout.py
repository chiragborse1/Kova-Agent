...
- Unlimited timeout (KOVA_CRON_TIMEOUT=0)
- Backward compat: KOVA_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """KOVA_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """KOVA_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("KOVA_CRON_TIMEOUT", "1200")
        raw = os.getenv("KOVA_CRON_TIMEOUT", "").strip()
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """KOVA_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """KOVA_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("KOVA_CRON_TIMEOUT", "abc")
        raw = os.getenv("KOVA_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """KOVA_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("KOVA_CRON_TIMEOUT", "")
        raw = os.getenv("KOVA_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_KOVA_time_importable(self):
        """kova_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _KOVA_now
        assert callable(_KOVA_now)

    def test_KOVA_constants_importable(self):
        """kova_constants should be importable from cron context."""
        from kova_constants import get_kova_home
        assert callable(get_kova_home)
