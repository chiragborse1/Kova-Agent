...
def test_create_job_no_agent_requires_script(KOVA_env):
...
def test_create_job_no_agent_stores_field(KOVA_env):
    from cron.jobs import create_job

    script_path = KOVA_env / "scripts" / "watchdog.sh"
...
def test_create_job_default_is_not_no_agent(KOVA_env):
    from cron.jobs import create_job
...
def test_update_job_roundtrips_no_agent_flag(KOVA_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = KOVA_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(KOVA_env):
...
def test_cronjob_tool_create_no_agent_with_script_succeeds(KOVA_env):
    from tools.cronjob_tools import cronjob

    script_path = KOVA_env / "scripts" / "alert.sh"
    script_path.write_text("#!/bin/bash\necho alert\n")
...
def test_cronjob_tool_update_toggles_no_agent(KOVA_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_no_agent_without_script_errors(KOVA_env):
...
def test_cronjob_tool_create_does_not_require_prompt_when_no_agent(KOVA_env):
...
def test_run_job_no_agent_success_returns_script_stdout(KOVA_env):
...
def test_run_job_no_agent_wake_gate_is_silent(KOVA_env):
    """wakeAgent=false gate in stdout triggers a silent run."""
    from cron.jobs import create_job
    from cron.scheduler import run_job, SILENT_MARKER

    script_path = KOVA_env / "scripts" / "gated.sh"
...
def test_run_job_no_agent_script_failure_delivers_error(KOVA_env):
    """Non-zero exit → success=False, error alert is the delivered message."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = KOVA_env / "scripts" / "broken.sh"
    script_path.write_text("#!/bin/bash\necho oops >&2\nexit 3\n")
...
def test_run_job_no_agent_never_invokes_aiagent(KOVA_env):
...
def test_run_job_script_shell_script_runs_via_bash(KOVA_env):
    """.sh files should execute under /bin/bash even without a shebang line."""
    from cron.scheduler import _run_job_script

    script_path = KOVA_env / "scripts" / "shelly.sh"
...
def test_run_job_script_bash_extension_also_runs_via_bash(KOVA_env):
    from cron.scheduler import _run_job_script

    script_path = KOVA_env / "scripts" / "thing.bash"
...
def test_run_job_script_python_still_runs_via_python(KOVA_env):
    """Regression: .py files must keep running via sys.executable."""
    from cron.scheduler import _run_job_script

    script_path = KOVA_env / "scripts" / "py.py"
...
def test_run_job_script_path_traversal_still_blocked(KOVA_env):
