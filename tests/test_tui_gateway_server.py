...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["KOVA_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("KOVA_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("KOVA_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("KOVA_VOICE", "1")
    monkeypatch.delenv("KOVA_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "kova_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("KOVA_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "kova_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import kova_cli.config as config_mod
    ...
    # _get_platform_tools (a non-configurable platform toolset in kova-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "kova_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("KOVA_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "kova_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown KOVA_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("kova_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("KOVA_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("KOVA_MODEL", raising=False)
    monkeypatch.delenv("KOVA_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/kova-test "}}
    )

    assert server._resolve_model() == "nous/kova-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set KOVA_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("KOVA_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("KOVA_MODEL", raising=False)
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("KOVA_MODEL", "nous/kova-test")
    monkeypatch.setenv("KOVA_TUI_PROVIDER", "nous")
    monkeypatch.delenv("KOVA_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/kova-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("KOVA_MODEL", "nous/kova-test")
    monkeypatch.delenv("KOVA_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("KOVA_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "kova_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/kova-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("KOVA_MODEL", "sonnet")
    monkeypatch.delenv("KOVA_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("KOVA_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "kova_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with KovaCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "kova_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "kova_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_KOVA_HOME", tmp_path)
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("KOVA_YOLO_MODE", "false")
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["KOVA_GATEWAY_SESSION"] == "1"
    assert server.os.environ["KOVA_EXEC_ASK"] == "1"
    assert server.os.environ["KOVA_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("kova_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("kova_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "kova_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "kova_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing KOVA_INFERENCE_PROVIDER on a switch leaked the new
    ...
    try:
        ...
        assert os.environ["KOVA_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "KOVA_TUI_PROVIDER" not in os.environ
        assert "KOVA_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write KOVA_MODEL / KOVA_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("KOVA_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("KOVA_MODEL", raising=False)
    monkeypatch.delenv("KOVA_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("kova_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["KOVA_TUI_PROVIDER"] == "openai-codex"
        assert "KOVA_MODEL" not in os.environ
        assert "KOVA_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors KovaCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".kova" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.kova/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".kova" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.kova/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".kova" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.kova/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("kova_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "kova_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("kova_state")
    ...
    monkeypatch.setitem(sys.modules, "kova_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_KOVA_HOME
    # fixture pins KOVA_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `kova model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `kova model` and the classic CLI /model picker.
    ...
    with patch(
        "kova_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("kova_cli.models.provider_model_ids") as live_fetch:
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``kova --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"kova_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "kova_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "kova_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "kova_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_KOVA_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.kova/.env
    ...
    with patch.dict(sys.modules, {"kova_cli.config": fake}):
...
def test_session_save_writes_under_KOVA_HOME_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the kova profile
    ...
    Regression: the gateway handler wrote ``KOVA_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are kova.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("KOVA_conversation_*.json"))
    ...
    assert payload["model"] == "kova-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are kova."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._KOVA_HOME, which
    # is captured at import time — patch it directly (set_kova_home_override
    # does NOT affect this snapshot).
    monkeypatch.setattr(cli, "_KOVA_HOME", tmp_path)
