...
def test_KOVA_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in KovaCLI.__init__")

    monkeypatch.setattr("kova_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("kova_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.KovaCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("kova_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.KovaCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "kova_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.KovaCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "kova_cli.nous_subscription.managed_nous_tools_enabled",
    ...
    monkeypatch.setattr(
        "kova_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("kova_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("kova_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("kova_cli.auth._update_config_for_provider", lambda provider, url: None)

    KOVA_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "kova"
    config_home.mkdir()
    monkeypatch.setenv("KOVA_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "kova_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("kova_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("kova_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "kova_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "kova_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "kova_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    KOVA_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "kova_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("kova_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "kova_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: "anthropic/claude-sonnet-4.6",
    )
    monkeypatch.setattr("kova_cli.auth.deactivate_provider", lambda: None)

    KOVA_main._model_flow_openrouter({}, current_model="glm-5.2")
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "kova_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "kova_cli.auth._prompt_model_selection",
    ...
    KOVA_main._model_flow_anthropic({}, current_model="glm-5.2")
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kova_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.KovaCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.KovaCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kova_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("kova_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kova_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("kova_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(KOVA_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    KOVA_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("kova_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("kova_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("kova_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("kova_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "kova_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("kova_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    KOVA_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("kova_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "kova_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("kova_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    KOVA_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(KOVA_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "kova_cli.config.load_config",
    ...
    monkeypatch.setattr("kova_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kova_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("kova_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("kova_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(KOVA_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("kova_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from kova_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from kova_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "kova_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("kova_cli.config.save_config", _save)
