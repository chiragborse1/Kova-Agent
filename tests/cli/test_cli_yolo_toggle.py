...
Pre-fix bug (issue #33925): ``cli.KovaCLI._toggle_yolo`` mutated only
``os.environ["KOVA_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``kova --yolo`` (process-start env), ``KOVA_YOLO_MODE=1``,
and ``kova config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``KovaCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``KovaCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``KovaCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``KovaCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            KovaCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            KovaCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False

    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``KOVA_YOLO_MODE`` — that path is
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert KovaCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            KovaCLI._toggle_yolo(stand_in)

        assert KovaCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``kova --yolo`` sets ``KOVA_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                KovaCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``KovaCLI.__new__(KovaCLI)`` to skip ``__init__``). It must
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert KovaCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            KovaCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            KovaCLI._transfer_session_yolo(stand_in, "same-id", "same-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        KovaCLI._transfer_session_yolo(stand_in, "", "new")
        KovaCLI._transfer_session_yolo(stand_in, "old", "")
