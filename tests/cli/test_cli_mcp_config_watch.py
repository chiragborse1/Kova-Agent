...
def _make_cli(tmp_path, mcp_servers=None):
    """Create a minimal KovaCLI instance with mocked config."""
...
class TestMCPConfigWatch:
    ...
    def test_no_change_does_not_reload(self, tmp_path):
        ...
        with patch("kova_cli.config.get_config_path", return_value=cfg_file):
    ...
    def test_missing_config_file_does_not_crash(self, tmp_path):
        ...
        with patch("kova_cli.config.get_config_path", return_value=missing):
