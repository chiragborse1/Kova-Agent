"""Tests for cli.KovaCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full KovaCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        KovaCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        ...
        result = _bound(KovaCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(KovaCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(KovaCLI._get_slash_confirm_display_fragments, self_)()
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert KovaCLI._split_destructive_skip("/reset now") == ("", True)
    assert KovaCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert KovaCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_strips_command_word():
    ...
    assert KovaCLI._split_destructive_skip("/new My title") == ("My title", False)
    assert KovaCLI._split_destructive_skip("/new --yes My title") == ("My title", True)
...
def test_split_destructive_skip_case_insensitive():
    ...
    assert KovaCLI._split_destructive_skip("/new NOW") == ("", True)
    # Substring match must NOT trigger — "Now-Title" is a literal title token.
    assert KovaCLI._split_destructive_skip("/new Now-Title") == ("Now-Title", False)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert KovaCLI._split_destructive_skip(None) == ("", False)
    assert KovaCLI._split_destructive_skip("") == ("", False)
    assert KovaCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    self_._split_destructive_skip = KovaCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_yes_flag_skips_modal():
    ...
    self_._split_destructive_skip = KovaCLI._split_destructive_skip
