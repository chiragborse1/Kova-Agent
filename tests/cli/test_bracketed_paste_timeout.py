...
def _reset_and_apply_production_patch():
    """Reload prompt_toolkit's parser and apply kova' production patch."""
    ...
    # does not redefine, so clear kova' sentinel before re-applying.
    if hasattr(vt100_mod, "_KOVA_bp_timeout_patched"):
        delattr(vt100_mod, "_KOVA_bp_timeout_patched")
    _load_production_patch_helper()()
    assert getattr(vt100_mod, "_KOVA_bp_timeout_patched", False)
    return vt100_mod
...
class TestBracketedPasteTimeout:
    ...
    def test_incomplete_paste_times_out(self):
        ...
        parser._KOVA_bp_start = time.monotonic() - 3.0
    ...
    def test_subsequent_data_after_incomplete_paste(self):
        ...
        parser._KOVA_bp_start = time.monotonic() - 5.0
        parser.feed("x")
    ...
    def test_no_timeout_under_threshold(self):
        ...
        parser._KOVA_bp_start = time.monotonic() - 0.5
