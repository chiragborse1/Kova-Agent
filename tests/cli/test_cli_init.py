"""Tests for KovaCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a KovaCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "KOVA_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), \
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
            ...
            return _cli_mod.KovaCLI(**kwargs)
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"KOVA_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"KOVA_MAX_ITERATIONS": "not-a-number"})
...
class TestHistoryDisplay:
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running Kova Agent",
                "preview": "check running gateways for Kova Agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running Kova Agent" in output
    ...
    def test_resume_updates_KOVA_SESSION_ID_env_and_context(self, tmp_path):
        ...
        os.environ["KOVA_SESSION_ID"] = "current_session"
        _VAR_MAP["KOVA_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["KOVA_SESSION_ID"] == "target_session"
            assert get_session_env("KOVA_SESSION_ID") == "target_session"
        finally:
            cli._session_db.close()
            os.environ.pop("KOVA_SESSION_ID", None)
            _VAR_MAP["KOVA_SESSION_ID"].set(_UNSET)
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running Kova Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running Kova Agent"
        )
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_KOVA_HOME", KOVA_HOME)
