...
Verifies that ``KovaCLI._handle_update_command`` correctly:
...
Also verifies that ``kova_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``KovaCLI`` construction.
...
def _call(self_):
    ...
    return KovaCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("kova_cli.config.is_managed", return_value=True),
        patch(
            "kova_cli.config.format_managed_message",
            return_value="Use `brew upgrade kova-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade kova-agent" in out
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("kova_cli.config.is_managed", return_value=False):
        result = _call(self_)
