...
class TestCliApprovalUi:
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is KovaCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is KovaCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = KovaCLI._paint_now.__get__(cli, KovaCLI)
    cli._invalidate = KovaCLI._invalidate.__get__(cli, KovaCLI)
