...
class TestLightModeDetection:
    def test_KOVA_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("KOVA_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_KOVA_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("KOVA_LIGHT", "0")
        # Also blank out other signals so nothing else flips it light.
        monkeypatch.delenv("KOVA_TUI_LIGHT", raising=False)
        monkeypatch.delenv("KOVA_TUI_THEME", raising=False)
        monkeypatch.delenv("KOVA_TUI_BACKGROUND", raising=False)
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False

    def test_theme_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("KOVA_LIGHT", raising=False)
        monkeypatch.delenv("KOVA_TUI_LIGHT", raising=False)
        monkeypatch.setenv("KOVA_TUI_THEME", "light")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_light(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_TUI_LIGHT", raising=False)
        monkeypatch.delenv("KOVA_TUI_THEME", raising=False)
        monkeypatch.setenv("KOVA_TUI_BACKGROUND", "#FFFFFF")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_dark(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_TUI_LIGHT", raising=False)
        monkeypatch.delenv("KOVA_TUI_THEME", raising=False)
        monkeypatch.setenv("KOVA_TUI_BACKGROUND", "#1a1a2e")
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        from kova_cli.skin_engine import SkinConfig

        assert getattr(SkinConfig, "_KOVA_light_mode_hook_installed", False) is True
