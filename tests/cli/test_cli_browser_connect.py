...
class TestChromeDebugLaunch:
    ...
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == r"C:\Chrome\chrome.exe"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert KovaCLI._try_launch_chrome_debug(9333, "Windows") is True
    ...
    def test_windows_launch_falls_back_to_common_install_dirs(self, monkeypatch):
        ...
        with patch("kova_cli.browser_connect.shutil.which", return_value=None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == installed), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert KovaCLI._try_launch_chrome_debug(9222, "Windows") is True

        _assert_chrome_debug_cmd(captured["cmd"], installed, 9222)

    def test_manual_command_uses_detected_linux_browser(self):
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: "/usr/bin/chromium" if name == "chromium" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == "/usr/bin/chromium"):
    ...
    def test_linux_candidates_prefer_chrome_before_brave_when_both_exist(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=fake_which), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
    ...
    def test_linux_candidates_prefer_chrome_install_path_before_brave_on_path(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave-browser" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_windows_candidates_prefer_chrome_install_path_before_brave_on_path(self, monkeypatch):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave.exe" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
    ...
    def test_linux_candidates_include_arch_brave_install_path(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", return_value=None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_brave_binary_name(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_official_brave_and_edge_stable_paths(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", return_value=None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {brave, edge}):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_launch_tries_next_browser_when_first_candidate_fails(self):
        ...
        with patch("kova_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert KovaCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_manual_command_uses_wsl_windows_chrome_when_available(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", return_value=None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_manual_command_uses_windows_quoting_on_windows(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: chrome if name == "chrome.exe" else None), \
             patch("kova_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
    ...
    def test_manual_command_returns_none_when_linux_browser_missing(self):
        with patch("kova_cli.browser_connect.shutil.which", return_value=None), \
             patch("kova_cli.browser_connect.os.path.isfile", return_value=False):
    ...
    def test_connect_context_note_allows_expected_browser_use(self, monkeypatch):
        ...
        with patch("kova_cli.cli_commands_mixin.is_browser_debug_ready", return_value=True), \
