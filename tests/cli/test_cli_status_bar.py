...
class TestCLIStatusBar:
    ...
    def test_scrollback_box_width_returns_viewport_width(self):
        ...
        assert KovaCLI._scrollback_box_width(20) == 32
        assert KovaCLI._scrollback_box_width(32) == 32
        # Above the floor, return the actual viewport width — no cap.
        assert KovaCLI._scrollback_box_width(48) == 48
        assert KovaCLI._scrollback_box_width(80) == 80
        assert KovaCLI._scrollback_box_width(120) == 120
        assert KovaCLI._scrollback_box_width(200) == 200
...
class TestIdleSinceLastTurn:
    ...
    def test_hidden_before_first_turn(self):
        assert KovaCLI._format_idle_since(None, turn_live=False) == ""

    def test_hidden_while_turn_is_live(self):
        assert KovaCLI._format_idle_since(time.time() - 30, turn_live=True) == ""

    def test_shows_compact_idle_time_after_turn(self):
        label = KovaCLI._format_idle_since(time.time() - 42, turn_live=False)
        assert label.startswith("✓ ")
        assert label == "✓ 42s"

    def test_scales_to_minutes(self):
        label = KovaCLI._format_idle_since(time.time() - 3 * 60, turn_live=False)
        assert label == "✓ 3m"
