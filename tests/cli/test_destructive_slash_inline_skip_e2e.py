...
Drives ``KovaCLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
def _make_cli_stub():
    """Build a minimal KovaCLI-shaped object that can run ``process_command``
    ...
    self_._confirm_destructive_slash = KovaCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    )
    self_.process_command = KovaCLI.process_command.__get__(self_, type(self_))
...
def test_new_without_skip_token_still_consults_modal():
    ...
    self_._split_destructive_skip = KovaCLI._split_destructive_skip
    self_._normalize_slash_confirm_choice = KovaCLI._normalize_slash_confirm_choice.__get__(
        self_, type(self_)
    )
    self_._confirm_destructive_slash = KovaCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
