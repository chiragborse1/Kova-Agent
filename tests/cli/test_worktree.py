...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("kova/kova-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned kova/* and pr-* branches."""

    def test_prunes_orphaned_KOVA_branch(self, git_repo):
        """kova/kova-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "kova/kova-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "kova/kova-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "kova/kova-deadbeef" in result.stdout
        ...
        assert "kova/kova-deadbeef" in orphaned
        ...
        assert "kova/kova-deadbeef" not in result.stdout
