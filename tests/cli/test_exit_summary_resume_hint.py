...
class TestExitSummaryResumeHint:
    ...
    profile boundaries — sessions live under `~/.kova-profiles/<profile>/`,
    ...
    def test_resume_hint_no_profile_flag_on_default(self, capsys):
        ...
        assert "kova --resume 20260524_000001_abc123" in out
        assert " -p " not in out
    ...
    def test_resume_hint_no_profile_flag_on_custom(self, capsys):
        ...
        with patch("kova_cli.profiles.get_active_profile_name", return_value="custom"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
        # "custom" is the standard KOVA_HOME indicator — no -p needed.
        assert "kova --resume 20260524_000001_abc123" in out
        assert " -p " not in out

    def test_resume_hint_includes_profile_flag_for_named_profile(self, capsys):
        cli_obj = _make_cli()
        with patch("kova_cli.profiles.get_active_profile_name", return_value="dev"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
    ...
    def test_resume_hint_includes_profile_flag_on_title_hint_too(self, capsys, tmp_path):
        """When a session title is available, the `kova -c "title"` hint
    ...
    def test_resume_hint_falls_back_when_profile_lookup_fails(self, capsys):
        ...
        missing during ``kova update`` mid-flight), fall back to no
