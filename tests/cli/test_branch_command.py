...
def cli_instance(tmp_path, session_db):
    """Create a minimal KovaCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        KovaCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        KovaCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_KOVA_SESSION_ID_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import KovaCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["KOVA_SESSION_ID"] = old_session_id
        _VAR_MAP["KOVA_SESSION_ID"].set(old_session_id)

        try:
            KovaCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["KOVA_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("KOVA_SESSION_ID") == cli_instance.session_id
        finally:
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from kova_cli.commands import COMMAND_REGISTRY
