"""Tests for kova_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every kova entry point
(kova, kova-agent, kova-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every kova entry point imports kova_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported kova_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("kova_bootstrap", None)
    import kova_bootstrap  # noqa: WPS433
    return kova_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Kova CLI
...
class TestEntryPointsImportBootstrap:
    """Every kova entry point must import kova_bootstrap as its
    ...
    # Entry points that invoke kova as a process.  Each one must
    # import kova_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "kova_cli/main.py",   # Kova CLI (console_script)
        "run_agent.py",          # kova-agent (console_script)
        "acp_adapter/entry.py",  # kova-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import kova_bootstrap' and that
        ...
        half-finished ``kova update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``kova_bootstrap`` as a top-level module) leaves kova
        ...
        """
        # Resolve relative to the kova-agent repo root.  Tests live
        # at tests/test_KOVA_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # kova start even when kova_bootstrap hasn't been
        ...
        assert first_import_name == "kova_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'kova_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import kova_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing kova's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("KOVA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["KOVA_PYTHON_SRC_ROOT"] = env
            elif "KOVA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["KOVA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/kova")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KOVA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["KOVA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/kova", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # kova root.  The guard must relocate kova to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/kova"])
        assert result[0] == "/opt/kova"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the kova root.
        assert result.index("/opt/kova") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/kova", "/opt/kova", ""])
        assert result.count("/opt/kova") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["KOVA_PYTHON_SRC_ROOT"] = "/env/kova"
            hb.harden_import_path()
            assert sys.path[0] == "/env/kova"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KOVA_PYTHON_SRC_ROOT", None)
