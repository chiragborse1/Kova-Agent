...
The root ``package.json`` is installed by ``kova update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``kova tools`` → Browser Automation → Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``kova update`` for
...
guidance in the ``kova-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in kova_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`kova tools` → Browser Automation → Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `kova setup` / `kova update` without an explicit "
        "post_setup step."
    )
