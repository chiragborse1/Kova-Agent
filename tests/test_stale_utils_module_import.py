...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.kova/kova-agent/utils.py)
...
2. ``kova update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``kova_cli/main.py`` (the ``kova update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``kova_constants`` after the
