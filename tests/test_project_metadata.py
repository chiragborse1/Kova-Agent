...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install kova-agent[matrix]`"
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `kova update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid kova update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `kova update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `kova update` downgrades the "
