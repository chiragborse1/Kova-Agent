...
def test_installed_wheel_renders_i18n_strings(tmp_path):
    ...
    #    (no PYTHONPATH leaking the repo, no KOVA_BUNDLED_LOCALES).
    probe = (
    ...
    env = {k: v for k, v in os.environ.items() if k not in ("PYTHONPATH", "KOVA_BUNDLED_LOCALES")}
...
def test_built_sdist_ships_locale_catalogs(tmp_path):
    ...
    with tarfile.open(tarballs[0]) as tf:
        ...
        # KOVA_agent-0.15.1/locales/en.yaml — match on the suffix.
