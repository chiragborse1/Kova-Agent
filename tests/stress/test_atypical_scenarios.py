...
normal tests assume away:
  ...
  - Filesystem: KOVA_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own KOVA_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"KOVA_atyp_{name}_")
            os.environ["KOVA_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("kova_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from kova_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "KOVA_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from kova_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["KOVA_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE KOVA_HOME: {resolved}")
...
@scenario("KOVA_HOME_with_spaces")
def _(home, kb):
    """KOVA_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="kova with spaces ")
    os.environ["KOVA_HOME"] = weird
    ...
    try:
        ...
        print(f"  KOVA_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("KOVA_HOME_with_unicode")
def _(home, kb):
    """KOVA_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/KOVA_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  KOVA_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("KOVA_HOME_via_symlink")
def _(home, kb):
    """KOVA_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="KOVA_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["KOVA_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same KOVA_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from kova_cli import web_server as ws  # noqa
    except Exception:
        pass
