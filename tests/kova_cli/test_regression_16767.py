...
from kova_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_chat_provider_argparse_acceptance(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.main.cmd_chat", mock_cmd_chat)
    monkeypatch.setattr(sys, "argv", ["kova", "chat", "--provider", "my-custom-key"])
...
def test_resolve_named_custom_runtime_honors_explicit_base_url(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.runtime_provider.has_usable_secret", lambda x: x == "test-api-key")
