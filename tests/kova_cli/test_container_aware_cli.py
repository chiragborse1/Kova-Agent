...
def container_env(tmp_path, monkeypatch):
    """Set up a fake KOVA_HOME with .container-mode file."""
    KOVA_HOME = tmp_path / ".kova"
    KOVA_HOME.mkdir()
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))
    monkeypatch.delenv("KOVA_DEV", raising=False)

    container_mode = KOVA_HOME / ".container-mode"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("kova_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_KOVA_dev(container_env, monkeypatch):
    """Returns None when KOVA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("KOVA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_KOVA_dev_zero(container_env, monkeypatch):
    """KOVA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("KOVA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        KOVA_HOME = Path(tmpdir) / ".kova"
        KOVA_HOME.mkdir()
        (KOVA_HOME / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("kova_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_kova_home": lambda: KOVA_HOME}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOVA_DEV", None)
            info = get_container_exec_info()
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from kova_cli.main import _exec_in_container
