"""Regression tests for TOCTOU-safe credential file writers in ``kova_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.kova/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks KOVA_HOME via
    # get_default_kova_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("KOVA_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from kova_cli import auth as auth_mod
