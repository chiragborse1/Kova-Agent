"""Tests for IRC gateway configuration via `kova setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh kova install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new kova install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#kova")
            monkeypatch.setenv("IRC_NICKNAME", "kova-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `kova setup gateway` experience with IRC present."""
