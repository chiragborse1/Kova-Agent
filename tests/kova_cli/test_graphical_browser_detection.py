"""Tests for `_can_open_graphical_browser()` in kova_cli.auth.

Guards the fix for the May 2026 report where `kova auth add xai-oauth`
...
from kova_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("kova_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.sys.platform", "darwin")
