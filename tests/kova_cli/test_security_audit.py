"""Unit tests for kova_cli.security_audit — parsers + OSV plumbing.
...
from kova_cli import security_audit as sa
...
class TestRunAudit:
    def test_no_components_returns_empty(self, tmp_path: Path):
        findings = sa.run_audit(
            skip_venv=True, skip_plugins=True, skip_mcp=True, KOVA_HOME=tmp_path
        )
        assert findings == []
    ...
    def test_findings_sorted_by_severity_desc(self, tmp_path: Path):
        ...
        with patch.object(sa, "_osv_query_batch", side_effect=fake_batch), \
            ...
            findings = sa.run_audit(
                skip_venv=True, skip_plugins=False, skip_mcp=True, KOVA_HOME=tmp_path
            )
...
class TestExitCodes:
    ...
    def test_clean_audit_exits_zero(self, tmp_path: Path, monkeypatch, capsys):
        monkeypatch.setattr(sa, "get_kova_home", lambda: str(tmp_path))
