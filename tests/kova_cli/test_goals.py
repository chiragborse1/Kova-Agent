"""Tests for kova_cli/goals.py — persistent cross-turn goals."""
...
def KOVA_HOME(tmp_path, monkeypatch):
    """Isolated KOVA_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves KOVA_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from kova_cli.goals import _parse_judge_response
...
class TestGoalManager:
    def test_no_goal_initial(self, KOVA_HOME):
    ...
    def test_set_then_status(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, KOVA_HOME):
    ...
    def test_evaluate_after_turn_done(self, KOVA_HOME):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, KOVA_HOME):
    ...
    def test_evaluate_after_turn_inactive(self, KOVA_HOME):
    ...
    def test_continuation_prompt_shape(self, KOVA_HOME):
        ...
        mgr.set("port goal command to kova")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to kova" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from kova_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, KOVA_HOME):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from kova_cli import goals
        from kova_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, KOVA_HOME):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, KOVA_HOME):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from kova_cli import goals
        from kova_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from kova_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, KOVA_HOME):
        from kova_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_no_goal_to_migrate_returns_false(self, KOVA_HOME):
        from kova_cli.goals import migrate_goal_to_session, load_goal
        assert migrate_goal_to_session("empty-parent", "child2") is False
        assert load_goal("child2") is None

    def test_does_not_clobber_existing_child_goal(self, KOVA_HOME):
        from kova_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_same_id_is_noop(self, KOVA_HOME):
        from kova_cli.goals import save_goal, migrate_goal_to_session, GoalState
        save_goal("same", GoalState(goal="g"))
        assert migrate_goal_to_session("same", "same") is False

    def test_cleared_goal_not_migrated(self, KOVA_HOME):
        from kova_cli.goals import save_goal, clear_goal, migrate_goal_to_session, load_goal, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, KOVA_HOME):
        import pytest
        from kova_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, KOVA_HOME):
        import pytest
        from kova_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, KOVA_HOME):
        import pytest
        from kova_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, KOVA_HOME):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, KOVA_HOME):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_wait_on_requires_active_goal(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
        mgr = GoalManager(session_id="wb-noactive")
        with pytest.raises(RuntimeError):
            mgr.wait_on(12345)

    def test_wait_on_rejects_bad_pid(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_barrier_auto_clears_when_process_exits_and_loop_resumes(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_dead_pid_never_parks(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_pause_and_resume_clear_barrier(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_barrier_persists_and_reloads(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_old_state_row_loads_without_barrier_fields(self, KOVA_HOME):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_judge_wait_seconds_parks_loop(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, KOVA_HOME):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_is_session_waiting_running_unmatched(self, KOVA_HOME):
        s, reg = self._inject("proc_t1", watch_patterns=["READY"])
        assert reg.is_session_waiting("proc_t1") is True

    def test_registry_releases_on_watch_match_while_alive(self, KOVA_HOME):
    ...
    def test_registry_releases_on_exit_plain_session(self, KOVA_HOME):
    ...
    def test_registry_unknown_session_never_waits(self, KOVA_HOME):
        from tools.process_registry import process_registry
        assert process_registry.is_session_waiting("proc_does_not_exist") is False

    def test_goal_parks_on_session_and_releases_on_trigger(self, KOVA_HOME):
        from kova_cli import goals
        from kova_cli.goals import GoalManager
    ...
    def test_wait_on_session_validation(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_session_directive_parsed_from_judge(self, KOVA_HOME):
        from kova_cli.goals import _parse_judge_response
    ...
    def test_old_state_loads_without_session_field(self, KOVA_HOME):
        from kova_cli.goals import GoalState
...
class TestParseContract:
    def test_plain_goal_no_contract(self):
        from kova_cli.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from kova_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from kova_cli.goals import GoalContract
...
class TestGoalManagerContract:
    def test_set_with_contract(self, KOVA_HOME):
        from kova_cli.goals import GoalManager, GoalContract
    ...
    def test_set_without_contract_no_marker(self, KOVA_HOME):
        from kova_cli.goals import GoalManager
    ...
    def test_continuation_prompt_includes_contract(self, KOVA_HOME):
        from kova_cli.goals import GoalManager, GoalContract
    ...
    def test_set_contract_after_the_fact(self, KOVA_HOME):
        from kova_cli.goals import GoalManager, GoalContract
        ...
        assert mgr.has_contract()
        # Survives reload.
        from kova_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, KOVA_HOME):
        from kova_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
        from kova_cli.goals import GoalContract
    ...
    def test_contract_plus_subgoals_combine(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
        from kova_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
    ...
    def test_draft_returns_none_on_bad_json(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
        from kova_cli.goals import GoalContract
    ...
    def test_contract_goal_can_still_complete_on_evidence(self, KOVA_HOME):
        from unittest.mock import patch
        from kova_cli import goals
        from kova_cli.goals import GoalContract
