...
def isolated_kanban_home(monkeypatch):
    ...
    # Force-reimport so the fresh KOVA_HOME is picked up.
    ...
    # gives each test its own monkeypatched KOVA_HOME so no cross-test
    # contamination.
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `kova kanban dispatch --dry-run` see the routing decision before
    committing."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
...
def test_dispatch_result_has_auto_assigned_default_field():
    ...
    from kova_cli.kanban_db import DispatchResult
    r = DispatchResult()
