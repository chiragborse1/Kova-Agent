...
which broke ``kova --tui`` because ``ui-tui/`` had vanished.
...
from kova_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.kova-update-staging").exists()
    assert not (dst.parent / "ui-tui.kova-update-old").exists()
...
def test_atomic_replace_clears_stale_staging_leftovers(tmp_path: Path) -> None:
    ...
    stale_staging = dst.parent / "ui-tui.kova-update-staging"
    stale_backup = dst.parent / "ui-tui.kova-update-old"
    stale_staging.mkdir()
    stale_backup.mkdir()
