...
prefix (e.g. ``mission-control.tilos.com/kova/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /kova`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__KOVA_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/kova/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/kova/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/kova`` when behind a prefix so they
...
from kova_cli.dashboard_auth import clear_providers, register_provider
from tests.kova_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/kova"},
        ...
        assert r.headers["location"].startswith("/kova/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/kova/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/kova/auth/callback`` — the
        ...
        assert parsed.path == "/kova/auth/callback", (
...
# ---------------------------------------------------------------------------
# KOVA_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``KOVA_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``kova_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "kova_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("KOVA_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "KOVA_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/kova``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "KOVA_DASHBOARD_PUBLIC_URL", "https://example.com/kova",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/kova/auth/callback"
    ...
    def test_public_url_ignores_x_forwarded_prefix(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "KOVA_DASHBOARD_PUBLIC_URL", "https://example.com/already-prefixed",
        )
    ...
    def test_public_url_strips_trailing_slash(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "KOVA_DASHBOARD_PUBLIC_URL", "https://example.com/",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("KOVA_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        entry."""
        monkeypatch.setenv("KOVA_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        assert any(
            "KOVA_DASHBOARD_PUBLIC_URL" in m
            and "kova.domain.com" in m
            and "scheme" in m
            for m in warnings
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "kova.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "KOVA_DASHBOARD_PUBLIC_URL", "https://kova.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://kova.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "KOVA_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /kova here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/kova" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-KOVA_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-KOVA_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from kova_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("KOVA_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/kova, so the next request under
        ...
        (``/kova/auth/login``, ``/kova/auth/callback``). A cookie
        set with ``Path=/kova`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/kova/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "KOVA_session_pkce" in c
        )
        # Parse "__Secure-KOVA_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-KOVA_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/kova cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/kova",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-KOVA_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/kova" in at_cookies[0]
