"""Regression test: ``kova mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``kova_cli/_parser.py``).  The dispatcher in ``kova_cli/main.py``
...
result, ``kova mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`kova mcp add foo --url ...` must keep args.command == "mcp".
