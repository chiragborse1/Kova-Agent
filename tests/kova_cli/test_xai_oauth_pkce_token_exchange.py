...
Issue [#26990] reported that ``kova auth add xai-oauth`` succeeds at the
...
The fix in ``kova_cli/auth.py`` extracts the token POST into
...
def post_recorder(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.post", recorder)
    return recorder
...
def test_transport_error_wraps_as_auth_error(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.post", _boom)
    with pytest.raises(AuthError) as exc_info:
        _xai_oauth_exchange_code_for_tokens(
            token_endpoint="https://auth.x.ai/oauth2/token",
            code="AUTHCODE",
...
def test_wire_format_is_form_urlencoded_with_all_pkce_fields(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.post", _post)
