...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from kova_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # kova model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(doctor_mod, "KOVA_HOME", home)
        ...
        try:
            from kova_cli import auth as _auth_mod
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("kova_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
