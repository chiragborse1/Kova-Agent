"""Unit tests for kova_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/kova_cli/test_pty_bridge.py.  Drives
...
``kova_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from kova_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from kova_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo kova-ok"])
        try:
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from kova_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from kova_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('KOVA_PTY_TEST=' + os.environ.get('KOVA_PTY_TEST',''))",
            ],
