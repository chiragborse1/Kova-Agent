...
- Bare `kova setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `kova setup --quick` runs the narrower "fill in missing items" flow.
- `kova setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ...
        ("kova_cli.setup.save_config", {}),
        ("kova_cli.setup.get_env_value", {"return_value": None}),
        ("kova_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("kova_cli.setup._print_setup_summary", {}),
        ("kova_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("kova_cli.setup.load_config", {"return_value": {}}),
        ("kova_cli.setup.save_config", {}),
        ("kova_cli.auth.get_active_provider", {"return_value": None}),
        ("kova_cli.setup.get_env_value", {"return_value": None}),
        ("kova_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `kova setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="kova_cli.setup.prompt_choice",
                quick="kova_cli.setup._run_quick_setup",
                model="kova_cli.setup.setup_model_provider",
                terminal="kova_cli.setup.setup_terminal_backend",
                agent="kova_cli.setup.setup_agent_settings",
                gateway="kova_cli.setup.setup_gateway",
                tools="kova_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`kova setup --reconfigure` behaves the same as bare `kova setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("kova_cli.setup.prompt_choice", {"return_value": 0}),
                first="kova_cli.setup._run_first_time_quick_setup",
            )
            from kova_cli.setup import run_setup_wizard
            run_setup_wizard(args)
