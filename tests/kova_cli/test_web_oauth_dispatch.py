"""Regression tests for the OAuth dispatcher in kova_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "kova_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "kova_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "kova_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    from kova_cli import web_server as ws
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_kova_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "kova auth add fake-oauth",
...
def test_nous_dashboard_device_flow_does_not_retry_legacy_scope_on_invoke_refusal(monkeypatch):
    ...
    monkeypatch.delenv("KOVA_AGENT_USE_LEGACY_SESSION_KEYS", raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_nous_poller", lambda sid: None)
...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_kova_home()),
    )
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "kova_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "kova_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "kova_cli.web_server._start_anthropic_pkce",
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `kova model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `kova model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by kova."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "kova auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to kova_cli.auth.get_auth_status (the
    ...
    with patch("kova_cli.auth.get_auth_status", return_value=fake_status):
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    ...
    with patch(
        "kova_cli.auth.get_nous_auth_status",
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("kova_cli.auth.get_auth_status", return_value={"logged_in": False}):
