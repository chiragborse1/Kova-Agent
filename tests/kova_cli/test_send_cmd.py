"""Tests for the ``kova send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``kova_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
def test_load_KOVA_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``kova send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``kova config set``) lives
    ...
    KOVA_HOME.mkdir()
    (KOVA_HOME / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_kova_home() to re-resolve under the patched env.
    from importlib import reload

    import kova_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_KOVA_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_KOVA_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    KOVA_HOME = tmp_path / ".kova"
    KOVA_HOME.mkdir()
    (KOVA_HOME / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_KOVA_env_handles_missing_files(tmp_path, monkeypatch):
