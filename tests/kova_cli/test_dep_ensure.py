...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("kova_cli.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("kova_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("kova_cli.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from kova_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("kova_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "kova_cli", repo_root=tmp_path)
...
def test_find_install_script_from_wheel(tmp_path):
    ...
    from kova_cli.dep_ensure import _find_install_script
    bundled = tmp_path / "kova_cli" / "scripts"
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    scripts_dir = tmp_path / "kova_cli" / "scripts"
    scripts_dir.mkdir(parents=True)
    (scripts_dir / "install.ps1").write_text("# fake")
    (scripts_dir / "install.sh").write_text("# fake")
    from kova_cli.dep_ensure import _find_install_script
    with patch("kova_cli.dep_ensure._IS_WINDOWS", True):
        path, shell = _find_install_script(package_dir=tmp_path / "kova_cli")
...
def test_find_install_script_falls_back_to_repo_root(tmp_path):
    ...
    from kova_cli.dep_ensure import _find_install_script
    with patch("kova_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "kova_cli", repo_root=repo_root)
...
def test_has_system_browser_checks_windows_names():
    from kova_cli.dep_ensure import _has_system_browser
    with patch("kova_cli.dep_ensure._IS_WINDOWS", True), \
         patch("kova_cli.dep_ensure.shutil") as mock_shutil:
...
def test_has_KOVA_agent_browser_windows_path(tmp_path):
    node_dir = tmp_path / "node"
    node_dir.mkdir(parents=True)
    (node_dir / "agent-browser.cmd").write_text("@echo off")
    from kova_cli.dep_ensure import _has_KOVA_agent_browser
    with patch("kova_cli.dep_ensure._IS_WINDOWS", True), \
         patch("kova_constants.get_kova_home", return_value=tmp_path):
        assert _has_KOVA_agent_browser() is True


def test_has_KOVA_agent_browser_posix_path(tmp_path):
...
def test_has_KOVA_agent_browser_legacy_node_modules_path(tmp_path):
    """Legacy git-clone installs put agent-browser in $KOVA_HOME/node_modules/.bin/."""
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    (scripts_dir / "install.ps1").write_text("# fake")
    with patch("kova_cli.dep_ensure._IS_WINDOWS", True), \
         patch("kova_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("kova_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("kova_cli.dep_ensure.shutil") as mock_shutil, \
         patch("kova_constants.get_kova_home", return_value=tmp_path / "fakehome"), \
