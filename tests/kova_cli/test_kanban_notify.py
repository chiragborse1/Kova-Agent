...
def kanban_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_MEDIA_ALLOW_DIRS", str(tmp_path))
    kb.init_db()
    return home
...
async def test_notifier_does_not_call_init_db(kanban_home):
    ...
    with patch("gateway.run.asyncio.sleep", side_effect=_fast_sleep), \
         patch("kova_cli.kanban_db.init_db", side_effect=_spy_init_db):
        await asyncio.wait_for(
            runner._kanban_notifier_watcher(interval=1),
            timeout=10.0,
        )
...
async def test_notifier_uploads_artifacts_on_completion(kanban_home, tmp_path, monkeypatch):
    ...
    # via ``KOVA_MEDIA_ALLOW_DIRS``. Test fixtures live under ``tmp_path``,
    ...
    import os
    os.environ["KOVA_KANBAN_TASK"] = tid
    try:
        ...
        os.environ.pop("KOVA_KANBAN_TASK", None)
