...
def test_user_config_overrides_default(KOVA_HOME, monkeypatch):
    from kova_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(KOVA_HOME, monkeypatch):
    from kova_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(KOVA_HOME):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_KOVA_dotenv(KOVA_HOME=str(home))
