...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from kova_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "kova" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_KOVA_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:KOVA_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:KOVA_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """kova auth add qwen-oauth must set active_provider in auth.json.
    ...
    monkeypatch.setattr(
        "kova_cli.auth.resolve_qwen_runtime_credentials",
    ...
    # pool entry from pool.add_entry() still present for kova auth list
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `kova auth add nous` must also populate providers.nous so the
    ...
    # raises "kova is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``kova auth add openai-codex`` runs for different ChatGPT
    ...
    monkeypatch.setattr("kova_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """kova auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "kova_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from kova_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from kova_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`kova logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (KOVA_HOME / "config.yaml").read_text()
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from kova_cli.auth import _reset_config_provider
    ...
    with patch("kova_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from kova_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "kova_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kova_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("kova_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kova_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "kova_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    ...
    # Pre-existing suppression (simulating a prior `kova auth remove`)
    ...
    payload = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (KOVA_HOME / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (KOVA_HOME / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `kova auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from kova_cli.auth import is_source_suppressed
    from kova_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `kova auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source kova reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import kova_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_KOVA_pkce_suppression(tmp_path, monkeypatch):
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["KOVA_pkce"]},
    }))

    # Stub the readers so only KOVA_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_KOVA_oauth_credentials", lambda: {
    ...
    # KOVA_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "KOVA_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "kova_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "kova_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `kova auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `kova auth add openai-codex`)
