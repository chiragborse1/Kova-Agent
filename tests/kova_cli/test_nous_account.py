...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    ...
    assert info.product_id == "nous-kova-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("kova_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("kova_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("kova_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `kova model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `kova model`" in message
