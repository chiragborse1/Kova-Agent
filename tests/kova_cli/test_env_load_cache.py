...
The cache exists to keep `kova tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from kova_cli.config import invalidate_env_cache, load_env
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from kova_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_KOVA_HOME", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "kova-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("kova_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
