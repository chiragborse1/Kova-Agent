...
def _clear_jwt_cache():
    ...
    import kova_cli.copilot_auth as mod
...
class TestExchangeCopilotToken:
    ...
    def test_exchanges_token_successfully(self, mock_urlopen):
        from kova_cli.copilot_auth import exchange_copilot_token
    ...
    def test_refreshes_expired_cache(self, mock_urlopen):
        from kova_cli.copilot_auth import exchange_copilot_token, _jwt_cache, _token_fingerprint
...
class TestGetCopilotApiToken:
    ...
    @patch("kova_cli.copilot_auth.exchange_copilot_token")
    def test_returns_exchanged_token(self, mock_exchange):
        from kova_cli.copilot_auth import get_copilot_api_token

        mock_exchange.return_value = ("exchanged_jwt", time.time() + 1800)
        assert get_copilot_api_token("gho_raw") == "exchanged_jwt"

    @patch("kova_cli.copilot_auth.exchange_copilot_token", side_effect=ValueError("fail"))
...
class TestTokenFingerprint:
    ...
    def test_consistent(self):
        from kova_cli.copilot_auth import _token_fingerprint
...
class TestCallerIntegration:
    ...
    @patch("kova_cli.copilot_auth.resolve_copilot_token", return_value=("gho_raw", "GH_TOKEN"))
    @patch("kova_cli.copilot_auth.get_copilot_api_token", return_value="exchanged_jwt")
