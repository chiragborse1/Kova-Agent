...
from kova_cli.profiles import export_profile, _DEFAULT_EXPORT_EXCLUDE_ROOT
...
class TestCredentialExclusion:
    ...
    def test_named_profile_export_excludes_auth(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.profiles._get_profiles_root", lambda: profiles_root)
        monkeypatch.setattr("kova_cli.profiles.get_profile_dir", lambda n: profile_dir)
        monkeypatch.setattr("kova_cli.profiles.validate_profile_name", lambda n: None)
