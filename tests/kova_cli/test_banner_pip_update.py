...
def testcheck_via_pypi_detects_update():
    ...
    with patch("kova_cli.banner.VERSION", "0.12.0"):
        with patch("kova_cli.banner._fetch_pypi_latest", return_value="0.13.0"):
...
def testcheck_via_pypi_up_to_date():
    ...
    from kova_cli.banner import check_via_pypi
    with patch("kova_cli.banner.VERSION", "0.13.0"):
        with patch("kova_cli.banner._fetch_pypi_latest", return_value="0.13.0"):
            result = check_via_pypi()
            assert result == 0
...
def testcheck_via_pypi_network_failure():
    ...
    from kova_cli.banner import check_via_pypi
    with patch("kova_cli.banner._fetch_pypi_latest", return_value=None):
        result = check_via_pypi()
...
def test_version_tuple_comparison():
    ...
    from kova_cli.banner import _version_tuple
