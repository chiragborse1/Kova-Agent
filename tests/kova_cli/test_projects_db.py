"""Tests for the per-profile Projects store (kova_cli/projects_db)."""
...
def test_create_get_list(conn):
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "kova-agent").id == pid
...
def test_slug_collision_disambiguates(conn):
    ...
    assert slugs == ["kova-agent", "kova-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' KOVA_HOME.
...
def test_db_path_under_KOVA_HOME():
    # Resolves under KOVA_HOME (set by the autouse isolation fixture).
