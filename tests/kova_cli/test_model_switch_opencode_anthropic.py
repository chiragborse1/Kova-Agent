...
``kova_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``kova_cli.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("kova_cli.model_switch.resolve_alias", return_value=None),
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new KOVA_HOME so config cache is fresh.
        import kova_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import kova_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import kova_cli.model_switch as _ms_mod
