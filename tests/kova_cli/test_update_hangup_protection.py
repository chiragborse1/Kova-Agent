"""Tests for SIGHUP protection and stdout mirroring in ``kova update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``kova_cli/main.py``.  These exist so
that ``kova update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_kova_home if present
        import kova_cli.config as _cfg
        if hasattr(_cfg, "_KOVA_HOME_CACHE"):
            _cfg._KOVA_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "kova update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_kova_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "kova_cli.config.get_kova_home", _boom, raising=True
        )
