"""Tests for the update check mechanism in kova_cli.banner."""
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import kova_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "kova_cli" / "banner.py"
    ...
    with patch("kova_cli.banner.subprocess.run") as mock_run, \
         patch("kova_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    with patch("kova_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:chiragborse1/Kova-Agent.git\n")
    ...
    with patch("kova_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/chiragborse1/Kova-Agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    with patch("kova_cli.banner.subprocess.run") as mock_run:
        with patch("kova_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when KOVA_HOME has no git repo."""
    import kova_cli.banner as banner
    ...
    # Point KOVA_HOME at a temp dir with no kova-agent/.git
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    with patch("kova_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    container, and `kova update` correctly refusing to run there. The guard
    ...
    with patch("kova_cli.config.detect_install_method", return_value="docker"), \
         patch("kova_cli.banner.subprocess.run") as mock_run, \
         patch("kova_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.config.detect_install_method", return_value="pip"), \
         patch("kova_cli.banner.subprocess.run") as mock_run, \
         patch("kova_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    # Build a fake ~/.kova with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"KOVA_HOME": str(default_home)}):
