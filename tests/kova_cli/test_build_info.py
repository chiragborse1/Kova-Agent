"""Tests for KOVA_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``KOVA_GIT_SHA`` build-arg
into ``<project_root>/.KOVA_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from kova_cli import build_info

    missing = tmp_path / ".KOVA_build_sha"  # never created
...
def test_get_build_sha_reads_baked_file(tmp_path):
    ...
    sha_file = tmp_path / ".KOVA_build_sha"
