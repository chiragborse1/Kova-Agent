...
import pytest
from kova_cli.model_switch import list_authenticated_providers, switch_model
...
def test_list_authenticated_providers_includes_full_models_list_from_user_providers(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.providers.KOVA_OVERLAYS", {})
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical kova write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though kova's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("kova_cli.providers.KOVA_OVERLAYS", {})
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical kova shape
    ...
    ``default_model``, so new-shape entries written by kova's own writer
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "kova_cli.models.validate_requested_model",
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("kova_cli.model_switch.resolve_alias", return_value=None), \
         patch("kova_cli.model_switch.list_provider_models", return_value=[]), \
         patch("kova_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("kova_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("kova_cli.models.detect_provider_for_model", return_value=None), \
         patch("kova_cli.model_switch.get_model_info", return_value=None), \
         patch("kova_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("kova_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.models.fetch_api_models", _fake_fetch)
...
def test_section3_skips_probe_when_no_key_but_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.models.fetch_api_models", _fail_fetch)
...
def test_current_custom_model_is_surfaced_in_builtin_provider_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.models.cached_provider_model_ids",
