"""Tests for the `kova memory reset` CLI command.
...
- Profile-scoped reset (uses KOVA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KOVA_HOME with memory files."""
    KOVA_HOME = tmp_path / ".kova"
    memories = KOVA_HOME / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nKova repo is at ~/.kova/kova-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `kova memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        KOVA_HOME = tmp_path / ".kova"
        (KOVA_HOME / "memories").mkdir(parents=True)
        monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KOVA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_kova_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
