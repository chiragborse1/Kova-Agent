...
from kova_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_get_codex_model_ids_prioritizes_default_and_cache(tmp_path, monkeypatch):
    ...
    # backend still accepts it via the OAuth-backed CLI/kova route.
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in kova_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from kova_cli.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "kova_cli.codex_models._fetch_models_from_api",
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from kova_cli import codex_models
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from kova_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "kova_cli.auth.get_codex_auth_status",
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "kova_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "kova-auth-store"},
    )
    monkeypatch.setattr(
        "kova_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("kova_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
        "kova_cli.codex_models.get_codex_model_ids",
...
def test_model_command_uses_existing_codex_session_without_relogin(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._login_openai_codex",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a KovaCLI with minimal mocking."""
    ...
    with (
        ...
        cli = KovaCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "KOVA_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import KovaCLI
            cli = KovaCLI()
