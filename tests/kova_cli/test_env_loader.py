...
def test_user_env_overrides_stale_shell_values(tmp_path, monkeypatch):
    ...
    loaded = load_KOVA_dotenv(KOVA_HOME=home)
...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_KOVA_dotenv(KOVA_HOME=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nKOVA_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("kova_cli.main", None)
    importlib.import_module("kova_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("KOVA_INFERENCE_PROVIDER") == "custom"
