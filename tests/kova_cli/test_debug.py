"""Tests for ``kova debug`` CLI command and debug utilities."""
...
def KOVA_HOME(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO kova_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from kova_cli.debug import _upload_paste_rs
        ...
        with patch("kova_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "kova_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from kova_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from kova_cli.debug import upload_to_pastebin

        with patch("kova_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("kova_cli.debug._upload_paste_rs",
             ...
             patch("kova_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, KOVA_HOME):
    ...
    def test_empty_primary_reports_file_empty(self, KOVA_HOME):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (KOVA_HOME / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, KOVA_HOME, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = KOVA_HOME / "logs" / "agent.log"
        from kova_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, KOVA_HOME):
        ...
        (KOVA_HOME / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, KOVA_HOME):
    ...
    def test_unknown_log_returns_none(self, KOVA_HOME):
        from kova_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, KOVA_HOME):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from kova_cli.debug import _capture_log_snapshot

        logs_dir = KOVA_HOME / "logs"
    ...
    def test_prefers_primary_over_rotated(self, KOVA_HOME):
    ...
    def test_falls_back_when_primary_empty(self, KOVA_HOME):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of KOVA_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_default_redacts_tail_and_full_text(self, KOVA_HOME_with_secret):
        from kova_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, KOVA_HOME_with_secret):
        from kova_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, KOVA_HOME_with_secret, monkeypatch
    ):
        ...
        input unchanged when KOVA_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("KOVA_REDACT_SECRETS", "false")

        from kova_cli.debug import _capture_log_snapshot

        assert os.environ.get("KOVA_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, KOVA_HOME_with_secret
    ):
        from kova_cli.debug import _capture_log_snapshot

        log_path = KOVA_HOME_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, KOVA_HOME_with_secret):
        from kova_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from kova_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report

        with patch("kova_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- kova dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- kova dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report

        with patch("kova_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, KOVA_HOME):
        from kova_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, KOVA_HOME, capsys):
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("kova_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, KOVA_HOME, capsys):
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch(
                 "kova_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, KOVA_HOME, capsys):
    ...
    def test_share_uploads_five_pastes(self, KOVA_HOME, capsys):
        ...
        with patch("kova_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- kova dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- kova dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- kova dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- kova dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- kova dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, KOVA_HOME, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from kova_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("kova_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, KOVA_HOME, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, KOVA_HOME, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, KOVA_HOME_with_secret, capsys
    ):
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("kova_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "kova debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, KOVA_HOME):
        from kova_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from kova_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from kova_cli.debug import delete_paste
        ...
        with patch("kova_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, KOVA_HOME):
        ...
        import ast
        import inspect
        from kova_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, KOVA_HOME):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from kova_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, KOVA_HOME):
    ...
    def test_merges_with_existing_pending(self, KOVA_HOME):
        """Subsequent calls merge into existing pending.json."""
        from kova_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, KOVA_HOME):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, KOVA_HOME):
    ...
    def test_sweep_deletes_expired_entries(self, KOVA_HOME):
        from kova_cli.debug import (
        ...
        with patch("kova_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, KOVA_HOME):
        from kova_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("kova_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, KOVA_HOME):
    ...
    def test_sweep_drops_entries_past_grace_window(self, KOVA_HOME):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, KOVA_HOME):
        from kova_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("kova_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, KOVA_HOME, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: kova debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from kova_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("kova_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("kova_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, KOVA_HOME, capsys):
        from kova_cli.debug import run_debug_share
        ...
        with patch("kova_cli.dump.run_dump"), \
             ...
             patch("kova_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, KOVA_HOME, capsys):
        from kova_cli.debug import run_debug_share
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("kova_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, KOVA_HOME, capsys):
        from kova_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``kova debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, KOVA_HOME):
        from kova_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("kova_cli.dump.run_dump"), patch(
            "kova_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("kova_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, KOVA_HOME):
        from kova_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (KOVA_HOME / "logs" / "desktop.log").unlink()

        with patch("kova_cli.dump.run_dump"), patch(
            "kova_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, KOVA_HOME):
        from kova_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, KOVA_HOME):
        from kova_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, KOVA_HOME):
        from kova_cli.debug import build_debug_share
