...
def test_status_preserves_existing_fields(loopback_client):
    ...
    body = r.json()
    expected_keys = {
        "version", "release_date", "KOVA_HOME", "config_path", "env_path",
...
_HOST_DETAIL_FIELDS = frozenset({
    "KOVA_HOME", "config_path", "env_path", "gateway_pid",
    "gateway_health_url",
})
...
def test_status_includes_host_detail_in_loopback_mode(loopback_client):
    ...
    the StatusPage / ``kova status`` experience for local operators."""
    r = loopback_client.get("/api/status")
    assert r.status_code == 200
    body = r.json()
