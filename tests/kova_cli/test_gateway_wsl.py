...
class TestIsWsl:
    ...
    def test_detects_wsl2(self):
        ...
        with patch("builtins.open", mock_open(read_data=fake_content)):
            assert kova_constants.is_wsl() is True
    ...
    def test_native_linux(self):
        ...
        with patch("builtins.open", mock_open(read_data=fake_content)):
            assert kova_constants.is_wsl() is False
    ...
    def test_result_is_cached(self):
        ...
        kova_constants._wsl_detected = True
...
class TestGatewayCommandWSLMessages:
    ...
    def test_install_wsl_no_systemd(self, monkeypatch, capsys):
        """Kova Gateway install on WSL without systemd shows guidance."""
        monkeypatch.setattr(gateway, "is_linux", lambda: True)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
        ...
        # The user then sees a broken KOVA_Gateway.cmd flash a cmd.exe
    ...
    def test_start_wsl_no_systemd(self, monkeypatch, capsys):
        """Kova Gateway start on WSL without systemd shows guidance."""
        monkeypatch.setattr(gateway, "is_linux", lambda: True)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    ...
    def test_status_wsl_running_manual(self, monkeypatch, capsys):
        """Kova Gateway status on WSL with manual process shows WSL note."""
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
    ...
    def test_status_wsl_not_running(self, monkeypatch, capsys):
        """Kova Gateway status on WSL with no process shows WSL start advice."""
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
