"""Tests for `kova fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate KOVA_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".kova" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from kova_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "kova-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from kova_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("kova_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("kova_cli.main._require_tty"):
            from kova_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from kova_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / kova-4")
        with patch("kova_cli.setup._curses_prompt_choice", return_value=1):
            from kova_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from kova_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `kova fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "kova_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
