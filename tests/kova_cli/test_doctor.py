...
import kova_cli.doctor as doctor
import kova_cli.gateway as gateway_cli
from kova_cli import doctor as doctor_mod
from kova_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `kova doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "KOVA_HOME", KOVA_HOME)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_KOVA_HOME(self, tmp_path, provider=""):
        """Create a minimal KOVA_HOME with config.yaml."""
        home = tmp_path / ".kova"
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from kova_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("kova_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "kova_cli.doctor", raising=False)
        from kova_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`kova doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken kova install.
