...
def test_login_start_has_redirect_and_state():
    ls = LoginStart(
        ...
        cookie_payload={"KOVA_session_pkce": "verifier=abc;state=xyz"},
    )
    assert ls.redirect_url.startswith("https://")
    assert "KOVA_session_pkce" in ls.cookie_payload
...
from kova_cli.dashboard_auth import (  # noqa: E402  (after-imports for clarity)
    register_provider,
    get_provider,
    list_providers,
    clear_providers,
