"""Tests for kova_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate KOVA_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".kova"
    home.mkdir()
    ...
    import importlib
    from kova_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from kova_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    FALLBACK = (
        "https://raw.githubusercontent.com/chiragborse1/Kova-Agent"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in kova_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from kova_cli import model_catalog
        from kova_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from kova_cli import model_catalog
        from kova_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``kova model`` CLI — not the live
        ...
        # ``KOVA_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` KOVA_HOME directly instead.
        import importlib
        from kova_cli import model_catalog
        from kova_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from kova_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["KOVA_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("kova_cli.models.check_nous_free_tier", return_value=False), patch(
                "kova_cli.models.union_with_portal_free_recommendations",
