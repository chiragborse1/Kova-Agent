"""Tests for the unified provider catalog (kova_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``kova model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from kova_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_KOVA_model_provider():
    ...
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `kova model` but missing from provider_catalog()"
        )
