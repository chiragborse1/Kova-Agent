"""Tests for the Phase 4 s6 dispatch helper in kova_cli.gateway.
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "kova_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`kova -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from kova_cli import gateway as gw
    from kova_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager",
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("kova_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("kova_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "kova_cli.gateway._block_until_terminated",
    ...
    assert "KOVA_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("kova_cli.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "kova_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("kova_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    KOVA_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
...
def test_redirect_respects_no_supervise_env(
    ...
    """`KOVA_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("KOVA_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("KOVA_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of KOVA_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `KOVA_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("KOVA_GATEWAY_NO_SUPERVISE", falsy)
