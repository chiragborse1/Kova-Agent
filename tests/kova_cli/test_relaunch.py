"""Tests for kova_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from kova_cli import relaunch as relaunch_mod


class TestresolveKovaBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/kova"
        ...
        assert relaunch_mod.resolve_KOVA_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'kova' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_KOVA_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/kova" if name == "kova" else None
        )
        assert relaunch_mod.resolve_KOVA_bin() == "/usr/bin/kova"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_KOVA_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_KOVA_bin", lambda: "/usr/bin/kova")
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_KOVA_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
...
class TestRelaunch:
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        kova).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_KOVA_bin", lambda: r"C:\Users\test\kova.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\kova.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_KOVA_bin", lambda: r"C:\kova.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_KOVA_bin", lambda: r"C:\missing.exe")
...
class TestresolveKovaBinWindowsPyGuard:
    """On Windows, resolve_KOVA_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a kova.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\kova.exe" if name == "kova" else None,
        )

        bin_path = relaunch_mod.resolve_KOVA_bin()
        # Must NOT be the .py — must be the kova.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\kova.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "kova"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_KOVA_bin() == str(script)

    def test_windows_py_argv0_with_no_KOVA_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND kova.exe
        isn't on PATH, return None so the caller falls back to
        python -m kova_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
