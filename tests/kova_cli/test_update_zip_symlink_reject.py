...
def _build_normal_zip(zip_path: str) -> None:
    ...
    with zipfile.ZipFile(zip_path, "w") as zf:
        zf.writestr("kova-agent-main/README.md", "ok\n")
...
def test_update_via_zip_rejects_symlink_member(tmp_path, monkeypatch):
    ...
    _build_zip_with_symlink_member(
        str(zip_path),
        link_name="kova-agent-main/evil-link",
        target="/etc/passwd",
    )
    ...
    if tmp_dir:
        evil_path = os.path.join(tmp_dir, "kova-agent-main", "evil-link")
...
def test_update_via_zip_accepts_normal_member(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(KOVA_main, "PROJECT_ROOT", fake_root)
    ...
    with patch("urllib.request.urlretrieve", side_effect=fake_urlretrieve), \
        ...
        try:
            KOVA_main._update_via_zip(args)
        except SystemExit:
            pass
