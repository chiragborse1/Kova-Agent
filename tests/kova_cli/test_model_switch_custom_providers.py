...
The terminal `kova model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import kova_cli.providers as providers_mod
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "KOVA_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("kova_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("kova_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: kova's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.models.fetch_lmstudio_models", _fake_fetch)
