"""Unit tests for kova_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_get_managed_dir_empty_override_falls_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_MANAGED_DIR", "   ")  # whitespace = unset
    # Under pytest the /etc/kova default is ignored, so this is None; the
...
def test_get_managed_dir_default_ignored_under_pytest(monkeypatch):
    ...
    monkeypatch.delenv("KOVA_MANAGED_DIR", raising=False)
    assert managed_scope.get_managed_dir() is None
...
def test_managed_dir_env_scrubbed_by_default():
    """conftest must scrub KOVA_MANAGED_DIR so a dev-shell value can't leak in."""
    import os

    assert "KOVA_MANAGED_DIR" not in os.environ
