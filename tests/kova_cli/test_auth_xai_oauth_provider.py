...
def _setup_KOVA_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the kova auth store at the given root."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_KOVA_agent():
    """Attribution: xAI's OAuth server can identify kova-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "kova-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "kova auth add xai-oauth" in body
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "kova-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``KOVA_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    KOVA_HOME: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    KOVA_HOME.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("kova_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    monkeypatch.setattr("kova_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``kova model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``kova model``
    ...
    assert "kova model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from kova_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    kova that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `kova model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `kova auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`kova auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `kova auth remove`.
    from kova_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``kova auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `kova auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((KOVA_HOME / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every kova invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from kova_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another kova process refreshed the singleton
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps(raw))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``kova model`` -> xAI Grok OAuth (or another process
    ...
    # Simulate the user re-running `kova model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``kova auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`kova auth add xai-oauth` entries (source='manual:xai_pkce') are
