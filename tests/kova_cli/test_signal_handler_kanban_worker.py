...
for interactive ``kova chat -q`` invocations, but kanban workers spawned by
...
The fix: when the process is a dispatcher-spawned worker (``KOVA_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full kova_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors kova_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With KOVA_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"KOVA_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with KOVA_KANBAN_TASK set "
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without KOVA_KANBAN_TASK, the original KeyboardInterrupt path runs.

    This is the contrast case proving the fix is gated on the env var: in
    interactive ``kova chat -q`` (no env var), behavior is unchanged. The
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when KOVA_KANBAN_TASK is set.
