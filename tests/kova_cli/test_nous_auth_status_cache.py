...
`kova tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test KOVA_HOME.
...
def test_get_nous_auth_status_caches_failure_path(tmp_path, monkeypatch):
    ...
    Teknium's case: ~31 cache misses per `kova tools` "All Platforms"
