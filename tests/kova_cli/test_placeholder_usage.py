...
from kova_cli.config import config_command, show_config
from kova_cli.setup import _print_setup_summary
...
def test_config_set_usage_marks_placeholders(capsys):
    ...
    assert "Usage: kova config set <key> <value>" in out
...
def test_config_unknown_command_help_marks_placeholders(capsys):
    ...
    assert exc.value.code == 1
    out = capsys.readouterr().out
    assert "kova config set <key> <value>   Set a config value" in out
...
def test_show_config_marks_placeholders(tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert "kova config set <key> <value>" in out
