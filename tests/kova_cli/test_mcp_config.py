"""
Tests for kova_cli.mcp_config — ``kova mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.config.get_kova_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "kova_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "kova_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from kova_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from kova_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_kova_home in the mcp_config module namespace
        monkeypatch.setattr(
            "kova_cli.mcp_config.get_kova_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    def test_add_no_transport(self, capsys):
        ...
        from kova_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_connection_failure_save_disabled(
        ...
        monkeypatch.setattr(
            "kova_cli.mcp_config._probe_single_server", mock_probe_fail
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    def test_test_not_found(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from kova_cli.mcp_config import cmd_mcp_test
...
class TestProbeEnvResolution:
    ...
    where `kova mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from kova_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import kova_cli.mcp_config as mc
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from kova_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from kova_cli.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_remove_mcp_server(self, tmp_path):
        from kova_cli.mcp_config import (
            _save_mcp_server,
            _remove_mcp_server,
            _get_mcp_servers,
        )
    ...
    def test_env_key_for_server(self):
        from kova_cli.mcp_config import _env_key_for_server
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from kova_cli.mcp_config import cmd_mcp_login
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.mcp_config._probe_single_server",
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "kova_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from kova_cli.mcp_config import cmd_mcp_reauth
