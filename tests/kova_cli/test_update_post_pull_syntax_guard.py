"""Tests for the post-pull syntax guard in ``kova update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``kova_cli/config.py``), the CLI
becomes unbootable — every ``kova`` invocation imports those files at
...
markers in ``kova_cli/config.py``; users who ran ``kova update`` in
the 7-minute window before #28458 landed could not run any ``kova``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    assert KOVA_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert KOVA_main._capture_head_sha(["git"], tmp_path) is None
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in KOVA_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_ok_when_all_files_parse(tmp_path):
    ...
    ok, failing_path, error = KOVA_main._validate_critical_files_syntax(tmp_path)
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="kova_cli/config.py")

    ok, failing_path, error = KOVA_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("kova_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="kova_cli/main.py")

    ok, failing_path, _ = KOVA_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("kova_cli/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except kova_constants.py
    for relpath in KOVA_main._UPDATE_CRITICAL_FILES:
        if relpath == "kova_constants.py":
            continue
...
# release; if a future ``kova update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = KOVA_main._validate_critical_files_syntax(repo_root)

    assert ok is True, (
        f"Critical-path file {failing_path} fails to parse on current main; "
        f"kova update would brick users. Error: {error}"
    )
