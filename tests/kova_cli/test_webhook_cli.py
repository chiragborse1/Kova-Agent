"""Tests for kova_cli/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("kova_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "Kova Gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "kova_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "kova_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import kova_cli.webhook as wh_mod
