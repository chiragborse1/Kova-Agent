...
from kova_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from kova_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    assert "KOVA_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "KOVA_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "KOVA_SPOTIFY_CLIENT_ID" not in env_path.read_text()
