...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<KOVA_HOME>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<KOVA_HOME>/plugins/``; we derive
    ``<KOVA_HOME>`` from it by walking one level up.
    """
...
class TestPluginDiscovery:
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".kova" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        ...
        assert "KOVA_plugins.ns_plugin" in sys.modules
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `kova plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("kova_cli.plugins._plugin_manager", mgr):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("kova_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("kova_cli.plugins.PluginContext.dispatch_tool.__module__", "kova_cli.plugins"):
...
class TestPluginDebugLogging:
    """KOVA_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With KOVA_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("KOVA_PLUGINS_DEBUG", "1")
        from kova_cli import plugins as plugins_mod
