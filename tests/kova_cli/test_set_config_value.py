...
from kova_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_KOVA_HOME(tmp_path):
    """Point KOVA_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_KOVA_HOME):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_KOVA_HOME)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_KOVA_HOME)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_KOVA_HOME):
        set_config_value(key, "secret-456")
        env_content = _read_env(_isolated_KOVA_HOME)
        assert f"{key}=secret-456" in env_content
        assert key not in _read_config(_isolated_KOVA_HOME)

    def test_case_insensitive(self, _isolated_KOVA_HOME):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_KOVA_HOME):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_KOVA_HOME):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_KOVA_HOME)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_KOVA_HOME)

    def test_nested_key(self, _isolated_KOVA_HOME):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_KOVA_HOME)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_KOVA_HOME)

    def test_terminal_image_goes_to_config(self, _isolated_KOVA_HOME):
    ...
    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_KOVA_HOME):
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_KOVA_HOME):
    ...
    def test_empty_string_routes_to_config(self, _isolated_KOVA_HOME):
    ...
    def test_zero_routes_to_config(self, _isolated_KOVA_HOME):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_KOVA_HOME):
...
class TestListNavigation:
    """kova config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_KOVA_HOME):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_KOVA_HOME, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_KOVA_HOME))
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_KOVA_HOME):
    ...
    def test_deeper_nesting_through_list(self, _isolated_KOVA_HOME):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from kova_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_KOVA_HOME, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_KOVA_HOME, capsys):
