"""Tests for `kova update --yes / -y` — assume yes for interactive prompts.
...
from kova_cli.main import cmd_update
...
class TestUpdateYesConfigMigration:
    ...
    @patch("kova_cli.config.migrate_config")
    @patch("kova_cli.config.check_config_version", return_value=(1, 2))
    @patch("kova_cli.config.get_missing_config_fields", return_value=[])
    @patch("kova_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``kova_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``kova_cli.main`` first could leave
