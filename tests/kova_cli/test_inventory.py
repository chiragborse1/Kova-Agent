"""Behavior tests for kova_cli.inventory.
...
from kova_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    """
    rows = [{"slug": "nous", "name": "Nous", "models": ["kova-4-405b"],
             "total_models": 1, "is_current": False, "is_user_defined": False,
             "source": "built-in"}]
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("kova_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "kova_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("kova_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        assert (
            ...
            or row["warning"].startswith("run `kova model`")
        )
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("kova_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from kova_cli import model_switch

    with patch("kova_cli.models.clear_provider_models_cache") as clear:
