"""Tests for kova_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("KOVA_")
        assert name.endswith("_bot")
        assert len(name) == len("KOVA_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("KOVA_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "KOVA_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="kova & Friends",
        )
        assert "kova+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "KOVA_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_abcdefghijklmnop_bot?name=kova+Agent",
            "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_abcdefghijklmnop_bot?name=kova+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "kova_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="KOVA_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/KovaSetupBot/KOVA_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/KovaSetupBot/KOVA_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "KOVA_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "kova_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("kova_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("kova_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("kova_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("kova_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("kova_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "kova_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("kova_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("kova_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from kova_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from kova_cli import setup

        seen = {}
        profile_home = tmp_path / ".kova" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_kova_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "kova_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from kova_cli.setup import _profile_name_from_KOVA_HOME

        assert (
            _profile_name_from_KOVA_HOME(
                PureWindowsPath(r"C:\Users\test\AppData\Local\kova\profiles\oracle")
            )
            == "oracle"
        )
