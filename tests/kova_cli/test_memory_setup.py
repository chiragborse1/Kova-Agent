...
import kova_cli.memory_setup as memory_setup
from kova_cli.memory_setup import _CANCELLED, _curses_select
...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.config.load_config", load_config)
...
def test_cmd_setup_builtin_selection_still_saves_builtin(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.config.load_config", lambda: config)
    monkeypatch.setattr("kova_cli.config.save_config", save_config)
...
def test_cmd_setup_clears_interactive_picker_before_provider_post_setup(monkeypatch):
    ...
    class PostSetupProvider:
        def post_setup(self, KOVA_HOME, config):
    ...
    monkeypatch.setattr(memory_setup, "get_kova_home", lambda: "/tmp/kova-test")
    monkeypatch.setattr("kova_cli.config.load_config", lambda: {"memory": {}})
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_kova_home", lambda: tmp_path)
    monkeypatch.setattr("kova_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("kova_cli.config.save_config", save_config)
