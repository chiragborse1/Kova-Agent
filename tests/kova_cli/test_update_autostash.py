...
def _patch_managed_uv(request):
    ...
    with patch("kova_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("kova_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("kova_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(KOVA_main.subprocess, "run", fake_run)

    stash_ref = KOVA_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M kova_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert KOVA_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = KOVA_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = KOVA_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    KOVA_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes kova unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="kova_cli/main.py\n", stderr="", returncode=0)
    ...
    result = KOVA_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    assert result is False
    out = capsys.readouterr().out
    assert "Conflicted files:" in out
    assert "kova_cli/main.py" in out
...
def test_restore_stashed_changes_auto_resets_non_interactive(monkeypatch, tmp_path, capsys):
    ...
    result = KOVA_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        KOVA_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def test_discard_lockfile_churn_skips_lock_when_package_json_dirty(tmp_path):
    ...
    KOVA_main._discard_lockfile_churn(["git"], tmp_path)
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(KOVA_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(KOVA_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(KOVA_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(KOVA_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(KOVA_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(KOVA_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(KOVA_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(KOVA_main, "_refresh_active_lazy_features", lambda: None)
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(KOVA_main, "_is_termux_env", lambda env=None: False)
    monkeypatch.setattr(KOVA_main, "_load_installable_optional_extras", lambda group="all": ["matrix", "mcp"])
    ...
    KOVA_main.cmd_update(SimpleNamespace())
...
def test_install_with_optional_fallback_honors_custom_group(monkeypatch):
    ...
    monkeypatch.setattr(KOVA_main, "_run_install_with_heartbeat", fake_run_with_heartbeat)

    KOVA_main._install_python_dependencies_with_optional_fallback(
        ["/usr/bin/uv", "pip"],
        group="termux-all",
    )
...
def test_install_heartbeat_prints_when_dependency_install_is_silent(monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        KOVA_main._time.sleep(1.2)
        return SimpleNamespace(returncode=0)

    monkeypatch.setattr(KOVA_main.subprocess, "run", fake_run)

    KOVA_main._run_install_with_heartbeat(
        ["uv", "pip", "install", "-e", "."],
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(KOVA_main.subprocess, "run", side_effect)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        KOVA_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        KOVA_main, "_restore_stashed_changes",
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        KOVA_main, "_discard_stashed_changes",
...
def test_non_interactive_discard_throws_changes_away(monkeypatch, tmp_path):
    ...
    KOVA_main.cmd_update(SimpleNamespace(gateway=True))
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(KOVA_main.sys.stdin, "isatty", lambda: True)
    monkeypatch.setattr(KOVA_main.sys.stdout, "isatty", lambda: True)

    KOVA_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_non_interactive_defaults_to_stash_when_setting_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(KOVA_config, "load_config", lambda *a, **kw: {"model": {}})
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    repo_gitignore = Path(KOVA_main.__file__).resolve().parents[1] / ".gitignore"
