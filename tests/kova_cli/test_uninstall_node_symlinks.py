"""Tests for kova_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $KOVA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the kova-managed node dir.
"""
...
def _make_KOVA_node(KOVA_HOME: Path) -> Path:
    """Create a fake $KOVA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = KOVA_HOME / "node" / "bin"
...
def test_removes_symlinks_pointing_into_KOVA_node(fake_home):
    KOVA_HOME = fake_home / ".kova"
    node_bin = _make_KOVA_node(KOVA_HOME)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(KOVA_HOME)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(KOVA_HOME) == []


def test_removes_dangling_symlink_into_KOVA_node(fake_home):
    """A link into the kova node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the kova links that exist; ignores the ones that don't."""
    KOVA_HOME = fake_home / ".kova"
    node_bin = _make_KOVA_node(KOVA_HOME)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are kova-managed; node is a real user binary.
