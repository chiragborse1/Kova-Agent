...
from kova_cli.web_server import _save_anthropic_oauth_creds
...
def oauth_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr('agent.anthropic_adapter._KOVA_OAUTH_FILE', target)
...
def test_dashboard_oauth_write_uses_atomic_replace_and_cleans_temp_files(oauth_file, monkeypatch):
    ...
    monkeypatch.setattr('kova_cli.web_server.os.replace', flaky_replace)
