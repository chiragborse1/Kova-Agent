...
import kova_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "kova_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("kova_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nous Portal" in out
        assert "kova setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        # (idempotency key) and once for KOVA_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "KOVA_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "kova_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("kova_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "kova_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "kova_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["KOVA_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://kova.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://kova.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["KOVA_DASHBOARD_PORTAL_URL"]
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the KOVA_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["KOVA_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / KOVA_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        KOVA_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "KOVA_DASHBOARD_PORTAL_URL":
        ...
        with patch(
            ...
            # The ambient process env may carry KOVA_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("KOVA_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
    ...
    def test_explicit_custom_url_persisted_when_var_absent(self, capsys):
        ...
        assert saved["KOVA_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["KOVA_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        assert (
            saved["KOVA_DASHBOARD_PORTAL_URL"] == "https://portal.nousresearch.com"
        )
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists KOVA_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://kova.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    KOVA_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as KOVA_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        KOVA_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "KOVA_DASHBOARD_PUBLIC_URL":
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["KOVA_DASHBOARD_PUBLIC_URL"] == "https://kova.example.com"
        # The full callback path must NOT be persisted verbatim (would double
        # the path at serve time).
        assert "/auth/callback" not in saved["KOVA_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://kova.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["KOVA_DASHBOARD_PUBLIC_URL"] == "https://kova.example.com:8443"
    ...
    def test_public_url_updates_existing_in_place(self, capsys):
        ...
        assert saved["KOVA_DASHBOARD_PUBLIC_URL"] == "https://new.example.com"

    def test_public_url_equal_to_existing_is_noop(self, capsys):
        # Derived origin already matches what's stored → no redundant write.
        saved = self._run(
            args=_ns(redirect_uri="https://kova.example.com/auth/callback"),
            existing_public="https://kova.example.com",
        )
        assert "KOVA_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://kova.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "kova_cli.config.get_env_value", return_value=None
        ), patch(
            "kova_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://kova.example.com/auth/callback",
                )
            )
        assert saved["KOVA_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["KOVA_DASHBOARD_PUBLIC_URL"] == "https://kova.example.com"
