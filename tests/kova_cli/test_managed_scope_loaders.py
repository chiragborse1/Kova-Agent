...
kova_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_KOVA_HOME", home, raising=False)
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_KOVA_HOME", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_kova_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = kova_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # kova_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("KOVA_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import kova_time

    assert kova_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (KOVA_TIMEZONE, KOVA_REDACT_SECRETS, KOVA_MAX_ITERATIONS,
    ...
    # writing KOVA_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
