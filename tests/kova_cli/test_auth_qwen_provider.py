"""Tests for Qwen OAuth provider authentication (kova_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_refresh_qwen_cli_tokens_success(qwen_env):
    ...
    with patch("kova_cli.auth.httpx") as mock_httpx:
...
def test_resolve_qwen_runtime_credentials_triggers_refresh(qwen_env):
    ...
    with patch(
        "kova_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
...
def test_resolve_qwen_runtime_credentials_base_url_env_override(qwen_env, monkeypatch):
    ...
    _write_qwen_creds(qwen_env, tokens)
    monkeypatch.setenv("KOVA_QWEN_BASE_URL", "https://custom.qwen.ai/v1")
...
def test_get_qwen_auth_status_expired_unrefreshable_token_is_not_logged_in(qwen_env):
    ...
    with patch(
        "kova_cli.auth._refresh_qwen_cli_tokens",
...
def test_model_flow_qwen_oauth_stale_token_shows_reauth_guidance(qwen_env, monkeypatch, capsys):
    from kova_cli.main import _model_flow_qwen_oauth
    ...
    monkeypatch.setattr(
        "kova_cli.auth._update_config_for_provider",
