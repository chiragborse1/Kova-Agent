"""Tests for kova_cli.mcp_catalog and kova_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("KOVA_OPTIONAL_MCPS", str(cat))
    return cat
...
def _isolate_KOVA_HOME(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "kova_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_kova_home() lazily through kova_constants
    monkeypatch.setattr(
        "kova_constants.get_kova_home", lambda: hh
    )
    return hh
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from kova_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from kova_cli.config import load_config
        from kova_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from kova_cli.mcp_catalog import install_entry
        from kova_cli.config import get_env_value, load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from kova_cli import mcp_catalog
        from kova_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from kova_cli.mcp_catalog import install_entry, uninstall_entry
        from kova_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from kova_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from kova_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `kova setup` know to update."""
        body = _basic_manifest()
        ...
        assert "requires a newer kova" in out
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from kova_cli import mcp_catalog
        from kova_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from kova_cli import mcp_catalog
        from kova_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "kova_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no KOVA_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("KOVA_OPTIONAL_MCPS", raising=False)
        from kova_cli.mcp_catalog import _catalog_root, _parse_manifest
