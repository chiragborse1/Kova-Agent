"""Tests for kova_cli.dump._get_git_commit — git SHA resolution for ``kova dump``.

``kova dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``KOVA_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("kova_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("kova_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_falls_back_to_build_sha_when_live_git_fails(tmp_path):
    ...
    with patch("kova_cli.dump.subprocess.run", return_value=failed), \
         patch("kova_cli.build_info.get_build_sha", return_value="cafef00d"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_returns_empty_stdout(tmp_path):
    ...
    with patch("kova_cli.dump.subprocess.run", return_value=empty), \
         patch("kova_cli.build_info.get_build_sha", return_value="abcdef12"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_raises(tmp_path):
    ...
    with patch("kova_cli.dump.subprocess.run", side_effect=FileNotFoundError("git")), \
         patch("kova_cli.build_info.get_build_sha", return_value="feedface"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_returns_unknown_when_neither_source_available(tmp_path):
    ...
    failed = MagicMock(returncode=128, stdout="")
    with patch("kova_cli.dump.subprocess.run", return_value=failed), \
         patch("kova_cli.build_info.get_build_sha", return_value=None):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``kova dump`` doesn't have to special-case
    ...
    with patch("kova_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("kova_cli.dump.subprocess.run", return_value=failed), \
         patch("kova_cli.build_info.get_build_sha", return_value="b2f477a3"):
