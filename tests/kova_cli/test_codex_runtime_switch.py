...
class TestApply:
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate kova mcp_servers
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("kova_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "kova-tools"]
        ...
        # User MCP servers are reported (excluding internal kova-tools)
        ...
        # kova tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("kova_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
