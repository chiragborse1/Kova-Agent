...
def test_format_managed_message_homebrew(monkeypatch):
    ...
    assert "brew upgrade kova-agent" in message
...
def test_recommended_update_command_defaults_to_KOVA_update(monkeypatch):
    ...
    with patch("kova_cli.config.get_managed_update_command", return_value=None), \
         patch("kova_cli.config.detect_install_method", return_value="git"):
        assert recommended_update_command() == "kova update"
...
def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    ...
    assert "brew upgrade kova-agent" in captured.err
