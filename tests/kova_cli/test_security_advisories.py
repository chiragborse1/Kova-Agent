"""Tests for kova_cli.security_advisories.
...
import kova_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect KOVA_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".kova"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full kova_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "kova_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "kova_cli.config.save_config",
