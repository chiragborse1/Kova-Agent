...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("kova_cli.config.get_managed_system", return_value=None), \
         patch("kova_cli.config.get_kova_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When KOVA_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("kova_cli.config.get_managed_system", return_value="NixOS"), \
         patch("kova_cli.config.get_kova_home", return_value=tmp_path):
        from kova_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("kova_cli.config.get_managed_system", return_value=None), \
         patch("kova_cli.config.get_kova_home", return_value=home):
        from kova_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    with patch("kova_cli.config.get_managed_system", return_value=None), \
         patch("kova_cli.config.get_kova_home", return_value=home), \
         patch("kova_cli.config._running_in_container", return_value=False):
        from kova_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("kova_cli.config.get_managed_system", return_value=None), \
         patch("kova_cli.config.get_kova_home", return_value=home), \
         patch("kova_cli.config._running_in_container", return_value=True):
        from kova_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $KOVA_HOME keeps
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``kova update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("kova_cli.config.get_managed_system", return_value=None), \
         patch("kova_cli.config.get_kova_home", return_value=tmp_path), \
         patch("kova_constants.is_container", return_value=True):
        from kova_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("kova_cli.config.get_kova_home", return_value=hh), \
         patch("kova_constants.get_kova_home", return_value=hh):
        buf = io.StringIO()
