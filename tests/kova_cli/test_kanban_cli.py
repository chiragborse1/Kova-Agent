"""Tests for the kanban CLI surface (kova_cli.kanban)."""
...
from kova_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`kova kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="kova", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from kova_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("KOVA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("KOVA_KANBAN_BOARD") == "beta"
