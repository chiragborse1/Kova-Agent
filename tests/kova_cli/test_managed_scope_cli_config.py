...
kova_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `kova config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_KOVA_HOME = get_kova_home()`` at import time (module
    singleton), so monkeypatching KOVA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap KOVA_HOME.)
    """
    import cli
