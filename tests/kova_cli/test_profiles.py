"""Comprehensive tests for kova_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and KOVA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.kova/profiles)
    * KOVA_HOME  -> tmp_path/.kova  (so get_kova_home() agrees)
    * Creates the bare-minimum ~/.kova directory.
    """
...
class TestValidateProfileName:
    ...
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the kova install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_KOVA_HOME(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".kova" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".kova" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.kova must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".kova"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes kova-agent, .worktrees,
        ...
        (default_home / "kova-agent" / ".git").mkdir(parents=True)
        (default_home / "kova-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "kova-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "kova-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `kova profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `kova update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `kova update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".kova" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".kova" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("kova_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("kova_cli.profiles._cleanup_gateway_service"), \
             patch("kova_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".kova" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_KOVA_HOME_returns_default(self, profile_env):
        # KOVA_HOME points to tmp_path/.kova which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".kova" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom KOVA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("KOVA_HOME", str(custom))
        # With Docker-aware roots, a custom KOVA_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("kova")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("kova_cli.profiles.shutil.which", lambda name: "/opt/kova/bin/kova")
        ...
        assert "exec /opt/kova/bin/kova -p mybot" in content
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from kova_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from kova_cli.profiles import remove_wrapper_script
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from kova_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from kova_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from kova_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "kova.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["KOVA_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["KOVA_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "kova.ssi_health": {"workspace": "kova", "enabled": True}
            }
        }))
        ...
        assert "kova.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["KOVA_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["KOVA_heimdall"]["workspace"] == "kova"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "kova.ssi_health": {"aiPeer": "ssi_health"},
                "KOVA_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["kova.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["KOVA_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("kova-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".KOVA_history",
        ...
        excluded_prefixes = [
            "default/kova-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.KOVA_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultkovaHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_KOVA_HOME()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".kova" / "profiles"

    def test_default_KOVA_HOME(self, profile_env):
        tmp_path = profile_env
        home = _get_default_KOVA_HOME()
        assert home == tmp_path / ".kova"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (KOVA_HOME outside ~/.kova), profiles go under KOVA_HOME."""
    ...
    def test_default_KOVA_HOME_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_KOVA_HOME() returns KOVA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KOVA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (KOVA_HOME under ~/.kova), profiles root is still ~/.kova/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under KOVA_HOME."""
        from kova_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KOVA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under KOVA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KOVA_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".kova" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from kova_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".kova"
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``kova -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".kova" / "profiles" / "coder"
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_KOVA_HOME()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with KOVA_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("KOVA_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_KOVA_HOME()
