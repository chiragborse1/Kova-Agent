"""Tests for the Phase 4 s6 hooks in kova_cli.profiles.
...
tests/kova_cli/test_profiles.py; here we only exercise the new
...
def test_register_calls_through_on_s6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_swallows_arbitrary_error(
    ...
    down `kova profile create` — print and continue."""
    _patch_detect_s6(monkeypatch)
    mgr = _S6Manager()
...
def test_register_swallows_no_backend_runtime_error(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.get_service_manager", _no_backend,
    )
    # Should NOT raise
...
def test_register_silent_when_detect_throws(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager", _broken_detect,
    )
