"""Unit tests for kova_cli/dingtalk_auth.py (QR device-flow registration)."""
...
class TestApiPost:
    ...
    def test_raises_on_network_error(self):
        import requests
        from kova_cli.dingtalk_auth import _api_post, RegistrationError

        with patch("kova_cli.dingtalk_auth.requests.post",
                   side_effect=requests.ConnectionError("nope")):
            with pytest.raises(RegistrationError, match="Network error"):
                _api_post("/app/registration/init", {"source": "kova"})
    ...
    def test_raises_on_nonzero_errcode(self):
        ...
        with patch("kova_cli.dingtalk_auth.requests.post", return_value=mock_resp):
            with pytest.raises(RegistrationError, match=r"boom \(errcode=42\)"):
                _api_post("/app/registration/init", {"source": "kova"})

    def test_returns_data_on_success(self):
        from kova_cli.dingtalk_auth import _api_post
        ...
        with patch("kova_cli.dingtalk_auth.requests.post", return_value=mock_resp):
            result = _api_post("/app/registration/init", {"source": "kova"})
...
class TestBeginRegistration:
    ...
    def test_chains_init_then_begin(self):
        from kova_cli.dingtalk_auth import begin_registration
        ...
        with patch("kova_cli.dingtalk_auth._api_post", side_effect=responses):
    ...
    def test_missing_nonce_raises(self):
        from kova_cli.dingtalk_auth import begin_registration, RegistrationError

        with patch("kova_cli.dingtalk_auth._api_post",
...
class TestWaitForSuccess:
    ...
    def test_returns_credentials_on_success(self):
        from kova_cli.dingtalk_auth import wait_for_registration_success
        ...
        with patch("kova_cli.dingtalk_auth.poll_registration", side_effect=responses), \
             patch("kova_cli.dingtalk_auth.time.sleep"):
    ...
    def test_success_without_credentials_raises(self):
        from kova_cli.dingtalk_auth import wait_for_registration_success, RegistrationError

        with patch("kova_cli.dingtalk_auth.poll_registration",
...
class TestRenderQR:
    ...
    def test_returns_false_when_qrcode_missing(self, monkeypatch):
        from kova_cli import dingtalk_auth
    ...
    def test_prints_when_qrcode_available(self, capsys):
        ...
        from kova_cli.dingtalk_auth import render_qr_to_terminal
...
class TestConfigOverrides:
    ...
    def test_base_url_default(self, monkeypatch):
        ...
        import importlib
        import kova_cli.dingtalk_auth as mod
