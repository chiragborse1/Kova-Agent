"""Tests for ``kova dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``kova update``
...
from kova_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No kova dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 kova dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("kova_cli.main._find_stale_dashboard_pids",
             ...
             patch("kova_cli.main._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("kova_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("kova_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``kova dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("kova_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"kova_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
...
class TestArgparseWiring:
    ...
    ``kova dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from kova_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("kova_cli.main")
