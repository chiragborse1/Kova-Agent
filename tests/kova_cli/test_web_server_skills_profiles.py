...
profile's KOVA_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``kova -p <profile>``
        ...
        monkeypatch.setattr(web_server, "_spawn_KOVA_action", _fake_spawn)
        resp = client.post(
