...
from kova_cli import main as cli_main
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "kova.app" / "Contents" / "MacOS" / "kova"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "kova.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "kova"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("kova_cli.main._desktop_build_needed", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("kova_cli.main._desktop_build_needed", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value=None), \
         patch("kova_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("kova_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("kova_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("kova_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("kova_cli.main._desktop_build_needed", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main._desktop_build_needed", return_value=False), \
         patch("kova_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("kova_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("kova_cli.main._desktop_build_needed", return_value=False), \
         patch("kova_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"kova"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("kova_cli.main._electron_dist_ok", return_value=False), \
         patch("kova_cli.main._redownload_electron_dist", return_value=True), \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("kova_cli.main._electron_dist_ok", return_value=False), \
         patch("kova_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("kova_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("kova_cli.main._electron_dist_ok", return_value=False), \
         patch("kova_cli.main._redownload_electron_dist", return_value=False), \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("kova_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main._electron_dist_ok", return_value=False), \
         patch("kova_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("kova_cli.main._electron_dist_ok", return_value=True), \
         patch("kova_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("kova_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("kova_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("kova_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("kova_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("kova_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("kova_cli.main.subprocess.run",
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "kova.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "kova.exe"
