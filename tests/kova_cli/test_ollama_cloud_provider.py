...
from kova_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from kova_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from kova_cli.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=None), \
             patch("agent.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        ...
        assert "ollama-cloud" in KOVA_OVERLAYS
        overlay = KOVA_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from kova_cli.providers import normalize_provider as np
        assert np("ollama") == "custom"  # bare "ollama" = local
        assert np("ollama-cloud") == "ollama-cloud"

    def test_label_override(self):
        from kova_cli.providers import _LABEL_OVERRIDES
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from kova_cli.models import _strip_ollama_cloud_suffix
