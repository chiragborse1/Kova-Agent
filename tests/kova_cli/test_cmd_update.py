...
from kova_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/kova-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "kova-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/kova-launcher-venv"
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $KOVA_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("kova_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind chiragborse1/Kova-Agent silently misses updates.
        """
        from kova_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/kova-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "kova_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "kova_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("kova_cli.config.check_config_version", return_value=(1, 2)), patch(
            "kova_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("kova_cli.main.sys") as mock_sys:
            ...
            from kova_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "kova_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "kova_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "kova_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "kova_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "kova_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "kova_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "kova_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "kova_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("kova_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.kova"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.kova/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.kova/profiles/work"))
        ...
        with (
            patch("kova_cli.profiles.list_profiles", return_value=all_profiles),
            patch("kova_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("kova_cli.profiles.list_profiles", return_value=[default_p]),
            patch("kova_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``kova update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``kova update --check --branch <name>`` honors the branch override.
    ...
    @patch("kova_cli.config.detect_install_method", return_value="pip")
    @patch("kova_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``kova update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from kova_cli.main import _update_via_zip
