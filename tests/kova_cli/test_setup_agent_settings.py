...
from kova_cli.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `KOVA_MAX_ITERATIONS` fallback was removed because it was
    ...
    monkeypatch.setattr("kova_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("kova_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("kova_cli.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("kova_cli.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("kova_cli.setup.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    Regression guard for the bug where `.env KOVA_MAX_ITERATIONS=60`
    from an old `kova setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        "kova_cli.setup.get_env_value",
        lambda key: "60" if key == "KOVA_MAX_ITERATIONS" else "",
    )
    monkeypatch.setattr("kova_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("kova_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("kova_cli.setup.save_env_value", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr(
        "kova_cli.setup.remove_env_value",
    ...
    assert "KOVA_MAX_ITERATIONS" in removed_keys
