...
def temp_pyproject(tmp_path, monkeypatch):
    """Point kova_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("kova_cli.main._resolve_install_target_python", return_value=py), \
             patch("kova_cli.main.subprocess.run") as mock_run, \
             patch("kova_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("kova_cli.main._resolve_install_target_python", return_value=py), \
             patch("kova_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("kova_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("kova_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("kova_cli.main._run_install_with_heartbeat") as mock_install:
            from kova_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("kova_cli.main._is_windows", return_value=True):
            from kova_cli.main import _resolve_install_target_python
