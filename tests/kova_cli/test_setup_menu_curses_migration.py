...
handling was unreliable in `kova setup` (the provider->model sub-menu).
"""
...
def test_prompt_model_selection_uses_curses_radiolist():
    ...
    with patch("kova_cli.curses_ui.curses_radiolist", side_effect=_fake), \
...
def test_prompt_model_selection_esc_cancels():
    ...
    with patch("kova_cli.curses_ui.curses_radiolist", return_value=-1), \
         patch("builtins.print"):
        result = _prompt_model_selection(["model-a", "model-b"])
...
def test_reasoning_effort_uses_curses_radiolist():
    ...
    with patch("kova_cli.curses_ui.curses_radiolist", return_value=2), \
         patch("builtins.print"):
