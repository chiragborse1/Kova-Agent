...
Execution via the ``KOVA_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
2. ``kova_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — KOVA_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            ...
            monkeypatch.setenv("KOVA_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"KOVA_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``KOVA_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "KOVA_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "KOVA_dashboard_plugin_evil" not in sys.modules
