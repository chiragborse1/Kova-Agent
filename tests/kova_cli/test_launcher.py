"""Tests for the top-level `./kova` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kova` should use `kova_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kova"
    called = []

    fake_main_module = types.ModuleType("kova_cli.main")

    def fake_main():
        called.append("kova_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kova_cli.main", fake_main_module)
