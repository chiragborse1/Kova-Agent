"""Tests for kova_cli.plugins_cmd — the ``kova plugins`` CLI subcommand."""
...
class TestResolveGitExecutable:
    ...
    def test_prefers_shutil_which(self):
        import kova_cli.plugins_cmd as pc
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kova_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from kova_cli.plugins_cmd import cmd_install

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("kova_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("kova_cli.plugins_cmd._display_after_install")
    @patch("kova_cli.plugins_cmd.shutil.move")
    @patch("kova_cli.plugins_cmd.shutil.rmtree")
    @patch("kova_cli.plugins_cmd._plugins_dir")
    @patch("kova_cli.plugins_cmd._read_manifest")
    @patch("kova_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("kova_cli.plugins_cmd._sanitize_plugin_name")
    @patch("kova_cli.plugins_cmd._plugins_dir")
    @patch("kova_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from kova_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from kova_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from kova_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "kova_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_skips_when_no_requires_env(self):
        from kova_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock
    ...
    def test_skips_already_set_vars(self, monkeypatch):
        ...
        console = MagicMock()
        with patch("kova_cli.config.get_env_value", return_value="already-set"):
    ...
    def test_prompts_for_missing_var_simple_format(self):
        ...
        with patch("kova_cli.config.get_env_value", return_value=None), \
             ...
             patch("kova_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_masked_prompt(self):
        ...
        with patch("kova_cli.config.get_env_value", return_value=None), \
             patch("kova_cli.plugins_cmd.masked_secret_prompt", return_value="s3cret") as mock_prompt, \
             patch("kova_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
...
class TestProviderDiscovery:
    ...
    def test_get_current_memory_provider_default(self, tmp_path, monkeypatch):
        ...
        from kova_cli.plugins_cmd import _get_current_memory_provider
    ...
    def test_get_current_context_engine_default(self, tmp_path, monkeypatch):
        ...
        from kova_cli.plugins_cmd import _get_current_context_engine
    ...
    def test_save_memory_provider(self, tmp_path, monkeypatch):
        ...
        config_file = tmp_path / "config.yaml"
        config_file.write_text("memory:\n  provider: ''\n")
        from kova_cli.plugins_cmd import _save_memory_provider
    ...
    def test_save_context_engine(self, tmp_path, monkeypatch):
        ...
        config_file = tmp_path / "config.yaml"
        config_file.write_text("context:\n  engine: compressor\n")
        from kova_cli.plugins_cmd import _save_context_engine
    ...
    def test_discover_memory_providers_empty(self):
        ...
        with patch("plugins.memory.discover_memory_providers",
            ...
            from kova_cli.plugins_cmd import _discover_memory_providers
    ...
    def test_discover_context_engines_empty(self):
        ...
        with patch("plugins.context_engine.discover_context_engines",
                    side_effect=ImportError("no module")):
            from kova_cli.plugins_cmd import _discover_context_engines
