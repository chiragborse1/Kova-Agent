"""Tests for the decomposer module + `kova kanban decompose` CLI surface.
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("kova_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("kova_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("kova_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "kova_cli.kanban_decompose._load_config",
