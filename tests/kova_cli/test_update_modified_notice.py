...
code paths in ``kova_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``kova skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"kova skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`kova skills list-modified` within the following lines — the "
