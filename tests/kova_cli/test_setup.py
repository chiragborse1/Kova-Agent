...
from kova_cli import setup as setup_mod
from kova_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("kova_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import kova_cli.gateway as _gw_mod
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("kova_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("kova_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "kova_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "kova_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("kova_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("kova_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kova_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("kova_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "kova_cli.setup.get_nous_subscription_features",
    ...
    from kova_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kova_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("kova_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "kova_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """KOVA_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("KOVA_NONINTERACTIVE", "1")
