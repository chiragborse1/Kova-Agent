...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh KOVA_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("KOVA_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("kova_cli") or mod.startswith("kova_state") or mod == "kova_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: kova kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "kova_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.config.load_config", lambda: fake_config)
