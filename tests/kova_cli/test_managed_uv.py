"""Tests for kova_cli.managed_uv — one path, no guessing."""
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv.platform.system", return_value="Linux"):
            from kova_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv.platform.system", return_value="Windows"):
            from kova_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_failure_returns_falsy(self, tmp_path):
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from kova_cli.managed_uv import ensure_uv
            path = ensure_uv()
...
class TestEnsureUvUpdateBoundary:
    ...
    ``kova update`` runs the call site from the old, already-imported
    ``kova_cli.main`` against the freshly pulled ``managed_uv``. A release
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from kova_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\.kova\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv.platform.system", return_value="Windows"):
            from kova_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path):
            from kova_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("kova_cli.managed_uv.get_kova_home", return_value=tmp_path), \
             patch("kova_cli.managed_uv.subprocess.run") as mock_run:
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("kova_cli.managed_uv._install_uv_posix") as mock_posix:
            from kova_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("kova_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("kova_cli.managed_uv._install_uv_windows") as mock_windows:
            from kova_cli.managed_uv import _install_uv
            _install_uv(target)
