...
``uv tool install kova-agent`` lives outside any venv, so the previous
...
that also has ``uv tool install kova-agent`` does not get misclassified.
"""
...
class TestIsUvToolInstall:
    def test_returns_true_when_sys_prefix_matches_uv_tool_layout(self):
        from kova_cli import config

        with patch.object(config.sys, "prefix", "/home/user/.local/share/uv/tools/kova-agent"):
    ...
    def test_returns_true_when_sys_executable_matches_uv_tool_layout(self):
        ...
        with patch.object(config.sys, "prefix", "/some/unrelated/venv"), \
             patch.object(
                 config.sys,
                 "executable",
                 "/home/user/.local/share/uv/tools/kova-agent/bin/python",
             ):
            assert config.is_uv_tool_install() is True
    ...
    def test_does_not_consult_uv_tool_list(self):
        ...
        ``uv tool install kova-agent`` somewhere on disk. Copilot review on
    ...
    def test_case_insensitive_match(self):
        ...
        (e.g. ``...AppData\\Local\\UV\\Tools\\kova-agent``) and a case-sensitive
        ...
        with patch.object(
            config.sys, "prefix", "/HOME/USER/.local/share/UV/Tools/kova-agent"
        ):
            assert config.is_uv_tool_install() is True
...
class TestRecommendedUpdateCommandForUvTool:
    def test_uv_tool_install_recommends_uv_tool_upgrade(self):
        ...
        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
            ...
            assert cmd == "uv tool upgrade kova-agent"
    ...
    def test_uv_pip_install_keeps_legacy_recommendation(self):
        """Existing behavior: uv is on PATH but kova is a regular pip install."""
        from kova_cli import config

        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
             patch.object(config, "is_uv_tool_install", return_value=False):
            cmd = config.recommended_update_command_for_method("pip")
            assert cmd == "uv pip install --upgrade kova-agent"
    ...
    def test_no_uv_falls_back_to_plain_pip(self):
        ...
        with patch("shutil.which", return_value=None), \
             patch.object(config, "is_uv_tool_install", return_value=False):
            cmd = config.recommended_update_command_for_method("pip")
            assert cmd == "pip install --upgrade kova-agent"
...
class TestCmdUpdatePipUsesUvTool:
    ...
    def test_runs_uv_tool_upgrade_when_uv_tool_install(self, mock_run):
        ...
        from kova_cli.main import _cmd_update_pip

        mock_run.return_value = subprocess.CompletedProcess(["uv"], 0, stdout="", stderr="")
        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
             patch("kova_cli.config.is_uv_tool_install", return_value=True):
            _cmd_update_pip(SimpleNamespace())

        assert mock_run.call_args[0][0] == ["/usr/local/bin/uv", "tool", "upgrade", "kova-agent"]

    @patch("subprocess.run")
    def test_runs_uv_pip_install_when_not_uv_tool(self, mock_run):
        """Existing behavior preserved when uv is present but kova isn't a tool install."""
        from kova_cli.main import _cmd_update_pip

        mock_run.return_value = subprocess.CompletedProcess(["uv"], 0, stdout="", stderr="")
        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
             patch("kova_cli.config.is_uv_tool_install", return_value=False):
            _cmd_update_pip(SimpleNamespace())
    ...
    def test_falls_back_to_pip_when_no_uv(self, mock_run):
        ...
        assert cmd[1:] == ["-m", "pip", "install", "--upgrade", "kova-agent"]
...
class TestCmdUpdatePipInstallLayouts:
    ...
    def test_pipx_managed_uses_pipx_upgrade(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.local/pipx/venvs/kova-agent")
        ...
        assert mock_run.call_args[0][0] == ["/usr/bin/pipx", "upgrade", "kova-agent"]
    ...
    def test_pipx_layout_without_pipx_binary_treated_as_venv(
        ...
        assert mock_run.call_args[0][0] == [
            "/usr/bin/uv", "pip", "install", "--upgrade", "kova-agent",
        ]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"].endswith("kova-agent")
    ...
    def test_bare_pip_outside_venv_adds_system(self, mock_run, monkeypatch):
        ...
        assert mock_run.call_args[0][0] == [
            "/usr/bin/uv", "pip", "install", "--system", "--upgrade", "kova-agent",
        ]
        assert "env" not in mock_run.call_args.kwargs
    ...
    def test_venv_exports_virtualenv_and_omits_system(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.kova/kova-agent/venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")
        ...
        assert cmd == ["/usr/bin/uv", "pip", "install", "--upgrade", "kova-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/home/u/.kova/kova-agent/venv"
