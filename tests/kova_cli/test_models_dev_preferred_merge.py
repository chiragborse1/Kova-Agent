...
appear in ``/model`` without a kova release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see kova_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from kova_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("kova_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("kova_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("kova_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "kova_cli.models._merge_with_models_dev",
