"""Tests for kova_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import kova_cli.gui_uninstall as gu


def _make_agent(KOVA_HOME: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = KOVA_HOME / "kova-agent"
    (agent_root / "kova_cli").mkdir(parents=True)
    (agent_root / "kova_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(KOVA_HOME: Path) -> None:
    """Create the source-built GUI artifacts a `kova desktop` run produces."""
    desktop = KOVA_HOME / "kova-agent" / "apps" / "desktop"
    ...
    (KOVA_HOME / "kova-agent" / "node_modules").mkdir(parents=True)
    (KOVA_HOME / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(KOVA_HOME: Path) -> None:
    (KOVA_HOME / "config.yaml").write_text("x: 1\n")
    (KOVA_HOME / ".env").write_text("KEY=secret\n")
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(KOVA_HOME)
    ...
    assert summary["KOVA_HOME"] == str(KOVA_HOME)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    KOVA_HOME with all the destructive externals stubbed out.
    """
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole KOVA_HOME non-interactively."""
    import kova_cli.uninstall as uninstall
