"""Regression test: `kova dashboard --tui` must not hard-crash.

Older kova desktop app shells (<= 0.15.x) spawn the backend as::

    kova dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "kova couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real kova_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "kova_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
