"""Tests for kova_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $KOVA_HOME under tmp_path; no real
...
from kova_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running kova containers, those
    ...
    """
    monkeypatch.setattr(
        "kova_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under KOVA_HOME/profiles/<name>/."""
    p = KOVA_HOME / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    KOVA_HOME root (the implicit default profile)."""
    if state is not None:
        (KOVA_HOME / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (KOVA_HOME / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (KOVA_HOME / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        KOVA_HOME=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from kova_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        KOVA_HOME=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $KOVA_HOME/profiles doesn't exist (fresh install), the
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare KOVA_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $KOVA_HOME/profiles/default/ instead of
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/kova/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/kova/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("KOVA_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the KOVA_HOME root (left
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/kova/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/kova/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ...
        ("/init", "/opt/kova/docker/main-wrapper.sh", "kova", "dashboard"),
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from kova_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/kova/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/kova/docker/main-wrapper.sh", "kova", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/kova/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/kova/docker/main-wrapper.sh", "gateway", "run"),
    )
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    """The legacy KOVA_SKIP_PROFILE_RECONCILE flag is gone: setting it on a
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("KOVA_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/kova/docker/main-wrapper.sh", "gateway", "run"),
