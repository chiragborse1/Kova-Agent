"""Verify `kova -c` picks the session the user most recently used."""

from __future__ import annotations

from kova_cli.main import _resolve_last_session
...
def test_resolve_last_session_prefers_last_active_over_started_at(monkeypatch):
    ...
    monkeypatch.setattr("kova_state.SessionDB", lambda: fake_db)
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = kova_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_returns_none_when_empty(monkeypatch):
    monkeypatch.setattr("kova_state.SessionDB", lambda: _FakeDB([]))
...
def test_resolve_last_session_closes_db_on_search_error(monkeypatch):
    ...
    db = _FailingDB()
    monkeypatch.setattr("kova_state.SessionDB", lambda: db)
...
def test_resolve_last_session_falls_back_to_started_at(monkeypatch):
    ...
    monkeypatch.setattr("kova_state.SessionDB", lambda: _FakeDB(rows))
...
def test_resolve_last_session_not_limited_to_newest_started_20(tmp_path, monkeypatch):
    ...
    real_session_db = kova_state.SessionDB
    ...
    monkeypatch.setattr("kova_state.SessionDB", lambda: real_session_db(db_path=state_db))
