...
from kova_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    ...
    assert any("KOVA_session_at" in c for c in set_cookies)
    assert any("KOVA_session_rt" in c for c in set_cookies)
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``KOVA_session_at`` / ``KOVA_session_rt``
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("KOVA_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("KOVA_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
...
class _UnreachableProvider(StubAuthProvider):
    ...
    def verify_session(self, *, access_token: str):
        from kova_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["KOVA_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
