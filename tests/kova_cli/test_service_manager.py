"""Tests for kova_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `Kova Gateway restart` spawned a
    foreground gateway that fought the supervised one. Detection must key off
    s6 being PID 1 (`_s6_running`) alone."""
    monkeypatch.setattr(
        "kova_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from kova_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "kova_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "kova_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "kova_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `kova_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import kova_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("kova_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "kova_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from kova_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``Kova Gateway run``, an agent action, the Open WebUI helper) holds
    the per-KOVA_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root KOVA_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME / "profiles" / "coder"))
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from kova_cli.service_manager import S6CommandError
...
#
# `Kova Gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    parent_chown = 'chown kova:kova "$KOVA_HOME/logs/gateways"'
    ...
    assert 'chown -R kova:kova "$KOVA_HOME/logs/gateways"' not in log_text
