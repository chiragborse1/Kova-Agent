"""Tests for Codex auth — tokens stored in kova auth store (~/.kova/auth.json)."""
...
def _setup_KOVA_auth(KOVA_HOME: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the kova auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_KOVA_auth(KOVA_HOME, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))
    ...
    monkeypatch.setattr("kova_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    KOVA_HOME = tmp_path / "kova"
    _setup_KOVA_auth(KOVA_HOME, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((KOVA_HOME / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``kova auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``kova auth add openai-codex``.
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `kova auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``kova auth add openai-codex`` for accounts B and C
    ...
    (KOVA_HOME / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``kova auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``kova auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to kova auth store, not ~/.codex/."""
    KOVA_HOME = tmp_path / "kova"
    codex_home = tmp_path / "codex-cli"
    KOVA_HOME.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "kova-at", "refresh_token": "kova-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches kova store
    assert not (codex_home / "auth.json").exists()

    # kova auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "kova-at"


def test_resolve_returns_KOVA_auth_store_source(tmp_path, monkeypatch):
    KOVA_HOME = tmp_path / "kova"
    _setup_KOVA_auth(KOVA_HOME)
    monkeypatch.setenv("KOVA_HOME", str(KOVA_HOME))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run kova auth".
    """
    from kova_cli.auth import (
    ...
    assert "kova auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from kova_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from kova_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "kova_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("kova_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("kova_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch kova_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("kova_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
