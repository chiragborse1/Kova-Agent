"""Tests for ``kova update`` / ``--check`` inside the Docker container.
...
fix, ``kova update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side kova, not an update to the running container, so the message
...
from kova_cli.main import _cmd_update_check, cmd_update
...
@patch("kova_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``kova update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/kova-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``kova update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/kova-agent:latest" in msg
    ...
    assert "KOVA_HOME" in msg or "/opt/data" in msg, (
