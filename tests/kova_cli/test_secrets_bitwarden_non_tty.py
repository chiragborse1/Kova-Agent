"""Regression tests for kova secrets bitwarden setup non-TTY guard.
...
class TestCmdSetupNonTtyGuard:
    ...
    def test_missing_all_flags_returns_1(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "kova_cli.secrets_cli.bw.find_bws", lambda install_if_missing=False: "/usr/bin/bws"
        )
        monkeypatch.setattr(
            "kova_cli.secrets_cli._bws_version", lambda _: "2.0.0"
        )

        from kova_cli.secrets_cli import cmd_setup
    ...
    def test_missing_server_url_with_env_var_passes(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.secrets_cli.load_config", lambda: {})
        monkeypatch.setattr("kova_cli.secrets_cli.save_env_value", lambda *a: None)
        monkeypatch.setattr("kova_cli.secrets_cli.get_env_path", lambda: "/tmp/.env")
        monkeypatch.setattr(
            "kova_cli.secrets_cli.bw.fetch_bitwarden_secrets",
            lambda **kw: ({"KEY": "val"}, []),
        )
    ...
    def test_tty_does_not_trigger_guard(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.secrets_cli.masked_secret_prompt", lambda prompt: "0.valid-token"
        )
        monkeypatch.setattr("kova_cli.secrets_cli.load_config", lambda: {})
        monkeypatch.setattr("kova_cli.secrets_cli.save_env_value", lambda *a: None)
        monkeypatch.setattr("kova_cli.secrets_cli.get_env_path", lambda: "/tmp/.env")
        monkeypatch.setattr(
            "kova_cli.secrets_cli._resolve_server_url",
