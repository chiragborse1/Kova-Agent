...
``KOVA_KANBAN_WORKSPACE``, but did NOT set ``TERMINAL_CWD``. Because
...
def _capture_spawn_env(kb, monkeypatch, workspace: str) -> dict:
    monkeypatch.setattr(kb, "_resolve_KOVA_argv", lambda: ["kova"])
...
def test_terminal_cwd_pinned_to_workspace(monkeypatch, tmp_path):
    ...
    assert captured["env"]["KOVA_KANBAN_WORKSPACE"] == str(workspace)
