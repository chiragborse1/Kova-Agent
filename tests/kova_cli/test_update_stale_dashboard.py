"""Tests for the stale-dashboard handling run at the end of ``kova update``.

``kova update`` detects ``kova dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``kova_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``kova_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from kova_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"kova_cli.main.X\")``
    ...
    live = sys.modules.get("kova_cli.main")
    if live is None:
        live = importlib.import_module("kova_cli.main")
...
class TestFindStaleDashboardPids:
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m kova_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
    ...
    def test_multiple_matches(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(12345, "python3 -m kova_cli.main dashboard --port 9119"),
                    _ps_line(12346, "kova dashboard --port 9120 --no-open"),
                    _ps_line(12347, "python /home/x/kova_cli/main.py dashboard"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m kova_cli.main dashboard"),
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    _ps_line(12345, "python3 -m kova_cli.main dashboard --port 9119"),
                    _ps_line(22222, "python3 -m kova_cli.main chat -q 'rewrite my dashboard'"),
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("kova_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("kova_cli.main._find_stale_dashboard_pids",
...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m kova_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `kova update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
