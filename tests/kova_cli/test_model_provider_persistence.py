"""Tests that provider selection via `kova model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated KOVA_HOME with a minimal string-format config."""
    home = tmp_path / "kova"
    home.mkdir()
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from kova_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from kova_cli.auth import _update_config_for_provider
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from kova_cli.main import _model_flow_api_key_provider
        ...
        with patch("kova_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("kova_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from kova_cli.main import _model_flow_copilot
        from kova_cli.config import load_config
        ...
        with patch(
            ...
            "kova_cli.models.fetch_github_model_catalog",
            return_value=[
                {
                    "id": "gpt-4.1",
            ...
            "kova_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "kova_cli.auth.deactivate_provider",
        ):
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        from kova_cli.main import _model_flow_named_custom
        ...
        with patch("kova_cli.auth._save_model_choice"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("kova_cli.models.fetch_api_models", return_value=["gpt-5.4"]), \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from kova_cli.main import _model_flow_copilot_acp
        from kova_cli.config import load_config

        with patch(
            "kova_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "kova_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("kova_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("kova_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("kova_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.kova/.env"):
...
class TestZaiEndpointPicker:
    ...
    def test_select_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.main._prompt_provider_choice", return_value=0), \
             patch("kova_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_select_coding_plan_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.main._prompt_provider_choice", return_value=2), \
             patch("kova_cli.auth._prompt_model_selection", return_value="glm-5.2"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_select_china_endpoint(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.main._prompt_provider_choice", return_value=1), \
             patch("kova_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_select_custom_proxy_url(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.main._prompt_provider_choice", return_value=custom_idx), \
             patch("kova_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("kova_cli.auth.deactivate_provider"), \
    ...
    def test_cancel_keeps_existing_base_url(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.main._prompt_provider_choice", return_value=None), \
             patch("kova_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("kova_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from kova_cli.auth import ZAI_ENDPOINTS
        from kova_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("kova_cli.main._prompt_provider_choice", side_effect=fake_choice):
