...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``KOVA_session_rt`` cookie (contract V1: no RT to persist).
...
from kova_cli.dashboard_auth.cookies import (
    SESSION_AT_COOKIE,
    SESSION_RT_COOKIE,
...
class TestApi401Envelope:
    ...
    def test_login_url_drops_next_for_deep_api_path(self, gated_app):
        ...
        from ``next=`` entirely; the SPA's own ``kova.lastLocation``
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``KOVA_session_at`` the instant the token lapses,
    while ``KOVA_session_rt`` lives for 30 days. From that moment the
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.kova_cli.conftest_dashboard_auth import _sign
...
class TestNextSameOriginValidation:
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from kova_cli.dashboard_auth.middleware import _safe_next_target
...
class TestValidatePostLoginTarget:
    ...
    def test_accepts_same_origin_paths(self):
        from kova_cli.dashboard_auth.routes import _validate_post_login_target
...
class TestRenderLoginHtmlNext:
    ...
    def test_no_next_emits_plain_button(self):
        from kova_cli.dashboard_auth.login_page import render_login_html
