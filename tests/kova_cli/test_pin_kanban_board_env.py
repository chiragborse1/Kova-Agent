...
board into `KOVA_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`kova kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `kova kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `KOVA_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("KOVA_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["KOVA_KANBAN_BOARD"] = prev
...
def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    ...
    assert main_mod.os.environ.get("KOVA_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("KOVA_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("kova_cli.main")
