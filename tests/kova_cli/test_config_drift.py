...
def test_delegation_default_toolsets_removed_from_cli_config():
    ...
    the user's ~/.kova/config.yaml over the defaults (cli.py:359-366).
    A contributor who still has the legacy key set in their own config
    would cause a false failure, and KOVA_HOME patching via conftest
    doesn't help because cli._KOVA_HOME is frozen at module import time
