"""Tests for kova_cli.auth._default_verify platform-aware fallback.
...
from kova_cli.auth import _default_verify, _resolve_verify
...
class TestResolveVerifyIntegration:
    ...
    def test_no_ca_uses_default_verify_on_darwin(self, monkeypatch):
        monkeypatch.setattr(sys, "platform", "darwin")
        for var in ("KOVA_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE"):
    ...
    def test_requests_ca_bundle_respected(self, monkeypatch, real_bundle_file):
        for var in ("KOVA_CA_BUNDLE", "SSL_CERT_FILE"):
            monkeypatch.delenv(var, raising=False)
    ...
    def test_missing_ca_path_falls_back_to_default_verify(self, monkeypatch, tmp_path):
        monkeypatch.setattr(sys, "platform", "linux")
        monkeypatch.setenv("KOVA_CA_BUNDLE", str(tmp_path / "missing.pem"))
    ...
    def test_insecure_wins_over_everything(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("KOVA_CA_BUNDLE", str(bundle))
