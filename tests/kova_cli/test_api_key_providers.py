...
from kova_cli.copilot_auth import _try_gh_cli_token
...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "KOVA_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "KOVA_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("kova_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("KOVA_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("kova_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("kova_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("kova_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("kova_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "kova_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "kova_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("kova_cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("KOVA_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("kova_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("kova_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "kova_cli.runtime_provider._get_model_config",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        ...
        KOVA_HOME.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: KOVA_HOME / ".env")
        monkeypatch.setattr(config_module, "get_kova_home", lambda: KOVA_HOME)
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when kova is unconfigured."""
        from kova_cli import config as config_module
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: KOVA_HOME / ".env")
        monkeypatch.setattr(config_module, "get_kova_home", lambda: KOVA_HOME)
        monkeypatch.setattr("kova_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("kova_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
        config_file = KOVA_HOME / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_KOVA_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when kova has been explicitly configured."""
        import yaml
        from kova_cli import config as config_module
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from kova_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from kova_cli.main import _PROVIDER_MODELS as main_models
        from kova_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from kova_cli import models as models_mod
...
class TestMinimaxOAuthProvider:
    ...
    def test_minimax_oauth_listed_in_canonical_providers(self):
        from kova_cli.models import CANONICAL_PROVIDERS
