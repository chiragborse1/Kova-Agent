"""Tests for the startup security posture audit (kova_cli.security_audit_startup)."""
...
import kova_cli.security_audit_startup as audit
...
def test_container_no_mount_flags(monkeypatch, tmp_path):
    ...
    msg = audit._container_no_volume_mount(tmp_path / ".kova")
...
def test_container_with_mount_silent(monkeypatch, tmp_path):
    ...
    assert audit._container_no_volume_mount(tmp_path / ".kova") is None
...
def test_run_security_audit_aggregates(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(audit, "_in_container", lambda: False)
    findings = audit.run_security_audit(KOVA_HOME=tmp_path, config={})
...
def test_run_security_audit_clean_posture(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(audit, "_in_container", lambda: False)
    assert audit.run_security_audit(KOVA_HOME=tmp_path, config={}) == []
...
def test_log_startup_security_warnings_emits_and_is_idempotent(monkeypatch, tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="kova.security_audit"):
        first = audit.log_startup_security_warnings(KOVA_HOME=tmp_path, config={})
    assert len(first) == 1
    assert any("ROOT" in r.message for r in caplog.records)

    # Second call is a no-op (idempotent within a process) unless forced.
    second = audit.log_startup_security_warnings(KOVA_HOME=tmp_path, config={})
    assert second == []
    forced = audit.log_startup_security_warnings(KOVA_HOME=tmp_path, config={}, force=True)
    assert len(forced) == 1
