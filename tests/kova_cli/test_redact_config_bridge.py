...
`KOVA_REDACT_SECRETS` at module-import time. `kova_cli/main.py` at
...
in .env), the toggle was silently ignored in both `kova chat` and
`Kova Gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `KOVA_REDACT_SECRETS`
env var in `kova_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports kova_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("KOVA_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import kova_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('KOVA_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env KOVA_REDACT_SECRETS takes precedence over config.yaml."""
    KOVA_HOME = tmp_path / ".kova"
    KOVA_HOME.mkdir()
    (KOVA_HOME / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (KOVA_HOME / ".env").write_text("KOVA_REDACT_SECRETS=true\n")
