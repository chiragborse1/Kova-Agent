"""Tests for kova_cli/completion.py — shell completion script generation."""
...
from kova_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real kova structure."""
    p = argparse.ArgumentParser(prog="kova")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_KOVA_completion()" in out
        assert "complete -F _KOVA_completion kova" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _kova kova' in out
        assert '_Kova "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_KOVA_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[kova]}} == _Kova ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c kova -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from kova_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_KOVA_profiles()" in out
        assert 'profiles_dir="$HOME/.kova/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'kova profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _KOVA_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_KOVA_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _KOVA_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_KOVA_profiles()" in out
        assert "$HOME/.kova/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__KOVA_profiles" in out
        count = out.count("(__KOVA_profiles)")
