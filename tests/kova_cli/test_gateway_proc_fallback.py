...
See: chiragborse1/Kova-Agent#7622
"""
...
_GATEWAY_CMD = "python -m kova_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m kova_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            patch("kova_cli.gateway.is_windows", return_value=False),
            patch("os.path.isdir", side_effect=_isdir),
            patch("os.listdir", side_effect=_listdir),
            patch("builtins.open", side_effect=_open),
            patch("kova_cli.gateway._get_ancestor_pids", return_value=set()),
    ...
    def test_detects_no_supervisor_restart_process_only_when_enabled(self):
        entries = {
            12345: "python -m kova_cli.main gateway restart",
            99999: _OTHER_CMD,
        }
        _isdir, _listdir, _open = _fake_proc_dir(entries)
