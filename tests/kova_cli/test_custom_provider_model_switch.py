"""Tests that `kova model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated KOVA_HOME with a minimal config."""
    home = tmp_path / "kova"
    home.mkdir()
...
class TestCustomProviderModelSwitch:
    ...
    def test_saved_model_still_probes_endpoint(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["model-A", "model-B"]) as mock_fetch, \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_can_switch_to_different_model(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["model-A", "model-B"]), \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_probe_failure_falls_back_to_saved(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=[]), \
    ...
    def test_no_saved_model_still_works(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["model-X"]), \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_api_mode_set_from_provider_info(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["claude-3"]) as mock_fetch, \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_api_mode_cleared_when_not_specified(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["llama-3"]), \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_env_template_api_key_is_preserved_in_model_config(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]) as mock_fetch, \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_key_env_custom_provider_persists_reference_not_secret(self, config_home, monkeypatch):
        ...
        with patch("kova_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]), \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_env_ref_base_url_preserves_api_key_ref_through_picker(
        ...
        with patch("kova_cli.main._prompt_provider_choice",
                   side_effect=_pick_neuralwatt), \
             patch("kova_cli.models.fetch_api_models",
    ...
    def test_bare_custom_current_provider_matches_env_base_url_before_first_fallback(
        ...
        """`kova model` must mark the custom provider matching model.base_url
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: KOVA_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("KOVA_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "KOVA_CRS_HENKEE_KEY"
        ...
        assert "${KOVA_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        provider_info = {
            ...
            "api_mode": "anthropic_messages",
            "provider_key": "crs-henkee",
            "api_key_ref": "${KOVA_CRS_HENKEE_KEY}",  # raw template preserved
        }
        ...
        assert entry["api_key"] == "${KOVA_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text


class TestCustomProviderDiscoverModels:
    """#18726: honor ``discover_models: false`` in the terminal ``kova model``
    ...
    def test_discover_false_uses_configured_list_and_skips_probe(self, config_home):
        ...
        with patch("kova_cli.models.fetch_api_models") as mock_fetch, \
             patch("kova_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
