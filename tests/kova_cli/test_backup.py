"""Tests for kova backup and import commands."""
...
def _make_KOVA_tree(root: Path) -> None:
    ...
    (root / "kova_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    def test_excludes_KOVA_agent(self):
    ...
    def test_includes_nested_KOVA_agent_in_skills(self):
...
class TestBackup:
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, KOVA_HOME)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_KOVA_agent(self, tmp_path, monkeypatch):
    ...
    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        ...
        venv_pkg = KOVA_HOME / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = KOVA_HOME / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_KOVA_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        nested = KOVA_HOME / "skills" / "autonomous-ai-agents" / "kova-agent"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/kova-backup-*.zip."""
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
        ...
        zips = list(tmp_path.glob("kova-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid kova backup."""
        from kova_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only kova_state.db (old wrong name) is rejected."""
        from kova_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["kova_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (KOVA_HOME / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (KOVA_HOME / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (KOVA_HOME / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_KOVA_prefix(self, tmp_path, monkeypatch):
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".kova/config.yaml": "model: test\n",
            ".kova/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (KOVA_HOME / "config.yaml").read_text() == "model: test\n"
        assert (KOVA_HOME / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_KOVA_zip(self, tmp_path, monkeypatch):
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside kova home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (KOVA_HOME / "gateway_state.json").write_text(live_state)
        ...
        assert (KOVA_HOME / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "config.yaml").exists()
        assert not (KOVA_HOME / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        (KOVA_HOME / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (KOVA_HOME / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            KOVA_HOME / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (KOVA_HOME / "gateway.pid").write_text("4242")
        (KOVA_HOME / "processes.json").write_text('{"live": true}')
        ...
        assert (KOVA_HOME / "gateway.pid").read_text() == "4242"
        assert (KOVA_HOME / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (KOVA_HOME / "cron.pid").exists()
        assert not (KOVA_HOME / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (KOVA_HOME / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from kova_cli.backup import run_backup, run_import
...
class TestFormatSize:
    def test_bytes(self):
        from kova_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without kova markers fails validation."""
        import io
        from kova_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_kova(self):
        """Detects .kova/ prefix wrapping all entries."""
        import io
        from kova_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".kova/config.yaml", "test")
            zf.writestr(".kova/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".kova/"
...
class TestBackupEdgeCases:
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("kova-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_KOVA_HOME(self, tmp_path, monkeypatch):
        """Backup handles empty kova home (no files to back up)."""
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = KOVA_HOME / "ancient.txt"
    ...
    def test_skips_output_zip_inside_kova(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside kova root."""
        KOVA_HOME = tmp_path / ".kova"
        KOVA_HOME.mkdir()
        (KOVA_HOME / "config.yaml").write_text("model: test\n")
...
class TestImportEdgeCases:
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "config.yaml").exists()
        assert (KOVA_HOME / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (KOVA_HOME / "profiles" / "coder" / "config.yaml").exists()
        assert (KOVA_HOME / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "kova -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from kova_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        assert snap_id is not None
        snap_dir = KOVA_HOME / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", KOVA_HOME=KOVA_HOME)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        db_copy = KOVA_HOME / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        assert (KOVA_HOME / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        copied = KOVA_HOME / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        with open(KOVA_HOME / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(KOVA_HOME=empty) is None

    def test_list_snapshots(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", KOVA_HOME=KOVA_HOME)
        id2 = create_quick_snapshot(label="second", KOVA_HOME=KOVA_HOME)

        snaps = list_quick_snapshots(KOVA_HOME=KOVA_HOME)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", KOVA_HOME=KOVA_HOME)
        snaps = list_quick_snapshots(limit=3, KOVA_HOME=KOVA_HOME)
        assert len(snaps) == 3

    def test_restore_config(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)

        (KOVA_HOME / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (KOVA_HOME / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, KOVA_HOME=KOVA_HOME)
        assert result is True
        assert "openrouter" in (KOVA_HOME / "config.yaml").read_text()

    def test_restore_state_db(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)

        conn = sqlite3.connect(str(KOVA_HOME / "state.db"))
    ...
    def test_restore_nonexistent(self, KOVA_HOME):
        from kova_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", KOVA_HOME=KOVA_HOME) is False

    def test_auto_prune(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", KOVA_HOME=KOVA_HOME)
        snaps = list_quick_snapshots(limit=100, KOVA_HOME=KOVA_HOME)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", KOVA_HOME=KOVA_HOME)
        deleted = prune_quick_snapshots(keep=3, KOVA_HOME=KOVA_HOME)
        assert deleted == 7
        assert len(list_quick_snapshots(KOVA_HOME=KOVA_HOME)) == 3

    def test_snapshot_includes_pairing_directories(self, KOVA_HOME):
        ...
        (KOVA_HOME / "platforms" / "pairing").mkdir(parents=True)
        (KOVA_HOME / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (KOVA_HOME / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (KOVA_HOME / "pairing").mkdir()
        (KOVA_HOME / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (KOVA_HOME / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, KOVA_HOME):
        ...
        assert restore_quick_snapshot(snap_id, KOVA_HOME=KOVA_HOME) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, KOVA_HOME):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, KOVA_HOME):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, KOVA_HOME):
        ...
        escape_dst = KOVA_HOME.parent.parent / "outside.txt"
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from kova_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        copy = KOVA_HOME / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        copy = KOVA_HOME / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, KOVA_HOME):
        from kova_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(KOVA_HOME / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, KOVA_HOME):
        ...
        board_dir = KOVA_HOME / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        copy = (
            KOVA_HOME / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, KOVA_HOME):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(KOVA_HOME / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, KOVA_HOME):
        ...
        board = KOVA_HOME / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(KOVA_HOME=KOVA_HOME)
        snap = KOVA_HOME / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, KOVA_HOME, monkeypatch):
        ...
        copy = KOVA_HOME / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def test_creates_backup_under_backups_dir(self, KOVA_HOME):
        from kova_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(KOVA_HOME=KOVA_HOME)
        assert out is not None
        assert out.exists()
        assert out.parent == KOVA_HOME / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, KOVA_HOME):
        """Pre-update backup should include the same user data that
        ``kova backup`` would, and should exclude the same directories."""
        from kova_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(KOVA_HOME=KOVA_HOME)
        assert out is not None
    ...
    def test_rotation_keeps_only_n(self, KOVA_HOME):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (KOVA_HOME / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, KOVA_HOME):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from kova_cli.backup import create_pre_update_backup

        (KOVA_HOME / "backups").mkdir(exist_ok=True)
        manual = KOVA_HOME / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from kova_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(KOVA_HOME=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, KOVA_HOME):
        ...
        """
        from kova_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, KOVA_HOME):
    ...
    def test_keep_zero_still_prunes_older_backups(self, KOVA_HOME):
        ...
        first = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def KOVA_HOME(self, tmp_path, monkeypatch):
        root = tmp_path / ".kova"
        root.mkdir()
        _make_KOVA_tree(root)
    ...
    def test_backup_flag_creates_backup(self, KOVA_HOME, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from kova_cli.main import _run_pre_update_backup
        ...
        backups = list((KOVA_HOME / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, KOVA_HOME, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((KOVA_HOME / "backups").glob("pre-update-*.zip")) \
            if (KOVA_HOME / "backups").exists() else True

    def test_no_backup_flag_skips(self, KOVA_HOME, capsys):
        from kova_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (KOVA_HOME / "backups").exists() or not list(
            (KOVA_HOME / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, KOVA_HOME, capsys):
        ...
        import yaml
        (KOVA_HOME / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("kova_cli.config"):
    ...
    def test_config_disabled_is_silent(self, KOVA_HOME, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, KOVA_HOME, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (kova claw migrate safety net)
# ---------------------------------------------------------------------------
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, KOVA_HOME):
        ...
        out = create_pre_migration_backup(KOVA_HOME=KOVA_HOME)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `kova
        # import` and the update-backup listing both pick them up.
        assert out.parent == KOVA_HOME / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, KOVA_HOME):
        """Pre-migration backup reuses the same exclusion rules as
        ``kova backup`` / ``create_pre_update_backup`` — no drift."""
        from kova_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(KOVA_HOME=KOVA_HOME)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_KOVA_import(self, KOVA_HOME, tmp_path):
        """The zip produced by pre-migration backup must be a valid kova
    ...
    def test_does_not_recurse_into_prior_backups(self, KOVA_HOME):
        from kova_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(KOVA_HOME=KOVA_HOME)
        assert out1 is not None
        out2 = create_pre_migration_backup(KOVA_HOME=KOVA_HOME)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, KOVA_HOME):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(KOVA_HOME=KOVA_HOME, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((KOVA_HOME / "backups").glob("pre-migration-*.zip"))
    ...
    def test_does_not_touch_pre_update_backups(self, KOVA_HOME):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from kova_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(KOVA_HOME=KOVA_HOME, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(KOVA_HOME=KOVA_HOME, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def _make_snapshot(self, KOVA_HOME: Path, label="pre-update"):
        from kova_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, KOVA_HOME=KOVA_HOME, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from kova_cli.backup import restore_cron_jobs_if_emptied
        KOVA_HOME = tmp_path / ".kova"
        jobs_path = KOVA_HOME / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(KOVA_HOME)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, KOVA_HOME=KOVA_HOME)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = KOVA_HOME / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, KOVA_HOME=KOVA_HOME) is None
        assert restore_cron_jobs_if_emptied("", KOVA_HOME=KOVA_HOME) is None
...
class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, KOVA_HOME: Path) -> None:
        KOVA_HOME.mkdir(parents=True, exist_ok=True)
        (KOVA_HOME / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (KOVA_HOME / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (KOVA_HOME / "state.db").write_bytes(b"x")
