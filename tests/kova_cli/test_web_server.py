"""Tests for kova_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_KOVA_HOME):
    """Drop the example-dashboard fixture into the per-test KOVA_HOME
    ...
    ``KOVA_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, kova-achievements, model providers
    ...
    #      and start serving requests against a torn-down KOVA_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known kova vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-kova env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints KOVA_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("KOVA_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_KOVA_HOME):
        """Create a TestClient and isolate the state DB under the test KOVA_HOME."""
        try:
    ...
    def test_get_status(self):
        ...
        assert "KOVA_HOME" in data
        assert "active_sessions" in data
        assert data["can_update_kova"] is True
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_kova"] is False

    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        import kova_constants
        import kova_cli.web_server as web_server

        monkeypatch.setattr(kova_constants, "is_container", lambda: True)
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in kova-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "kova"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from kova_constants import get_kova_home
        from kova_cli.config import load_config, load_env
        ...
        config_path = get_kova_home() / "hindsight" / "config.json"
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from kova_cli.web_server import app
    ...
    def test_update_KOVA_returns_docker_guidance_without_spawning(self, monkeypatch):
        import kova_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn kova update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_KOVA_action", fail_spawn)
        web_server._ACTION_PROCS.pop("kova-update", None)
        web_server._ACTION_RESULTS.pop("kova-update", None)

        resp = self.client.post("/api/kova/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "kova-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/kova-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/kova-update/status")
    ...
    def test_update_KOVA_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import kova_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn kova update")
    ...
    def test_update_KOVA_spawns_on_non_docker_install(self, monkeypatch):
        import kova_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_KOVA_action", fake_spawn)
        web_server._ACTION_PROCS.pop("kova-update", None)
        web_server._ACTION_RESULTS.pop("kova-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "kova-update"}
        assert calls == [(["update"], "kova-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("kova-update", None)
        web_server._ACTION_RESULTS.pop("kova-update", None)
        web_server._ACTION_PROCS["kova-update"] = proc

        resp = self.client.get("/api/actions/kova-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "kova-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["kova-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from kova_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `kova model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from kova_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a kova provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_KOVA_action", fake_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from kova_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from kova_cli.config import save_env_value
        from kova_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://kova-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from kova_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://kova-agent.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://kova-agent.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Kova Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("KovaDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "KOVA_pair123_bot",
                "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair123_bot",
                "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "KOVA_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "KOVA_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_KOVA_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "KOVA_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "KOVA_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_KOVA_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``Kova Gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "KOVA_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "KOVA_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "KOVA_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "KOVA_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/KovaSetupBot/KOVA_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/KovaSetupBot/KOVA_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/kova_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/kova"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import kova_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "kova-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from kova_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from kova_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``kova model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "kova-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `kova model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from kova_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from kova_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from kova_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_KOVA_HOME):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("kova://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from kova_constants import get_kova_home
        import kova_cli.profiles as profiles_mod
        ...
        named = KOVA_HOME / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_kova_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_KOVA_for_default_profile(self):
        from kova_constants import get_kova_home
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/kova/bin/kova")
        ...
        if is_windows:
            assert lines == ["@echo off", "kova -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/kova/bin/kova -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_kova_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_kova_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_kova_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_kova_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_kova_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_kova_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_kova_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_kova_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_kova_home() / "profiles" / "builder"
        token = set_kova_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_kova_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import kova_cli.skills_config as skills_config
        import kova_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import kova_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from kova_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from kova_cli.web_server import _denormalize_config_from_web
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from kova_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.kova/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``kova_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in kova_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_KOVA_HOME):
        ...
        # Pin the SessionDB to the isolated KOVA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_KOVA_HOME, _install_example_plugin):
        ...
        installs it into the per-test ``KOVA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into KOVA_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, kova-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (kova-achievements is loaded by default).
        resp = self.client.get("/api/plugins/kova-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``kova --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["KOVA_TUI_DASHBOARD"] == "1"
        assert env["KOVA_TUI_INLINE"] == "1"
        assert env["KOVA_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_KOVA_HOME
    ):
        import kova_cli.main as main_mod

        config_path = Path(os.environ["KOVA_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/kova-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/kova-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"kova-ws-ok" in buf:
                    break
            assert b"kova-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from kova_cli.pty_bridge import PtyUnavailableError
        ...
        import kova_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        KOVA_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from kova_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import kova_cli.main as cli_main
    import kova_cli.web_server as ws
    ...
    gateway_url = env.get("KOVA_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_KOVA_HOME, _install_example_plugin):
        ...
        the per-test ``KOVA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_KOVA_HOME):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under KOVA_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_KOVA_HOME):
        import threading
        import cron.scheduler as sched
