"""Tests for kova_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("kova_cli.copilot_auth._try_gh_cli_token", return_value="gho_from_cli"):
            token, source = resolve_copilot_token()
    ...
    def test_gh_cli_classic_pat_raises(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("kova_cli.copilot_auth._try_gh_cli_token", return_value="ghp_classic"):
    ...
    def test_no_token_returns_empty(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("kova_cli.copilot_auth._try_gh_cli_token", return_value=None):
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
