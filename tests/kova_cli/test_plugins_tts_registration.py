...
``$KOVA_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/kova_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        _enable(KOVA_HOME, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(KOVA_HOME, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(KOVA_HOME, "shadow-tts-plugin")
