...
from kova_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $KOVA_HOME and ~ to a tmp dir for the duration of the test."""
    home = tmp_path / ".kova"
    home.mkdir()
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point KOVA_HOME at a file (not a dir) so mkdir/open will fail.
    broken = tmp_path / "not-a-dir"
    broken.write_text("blocking file")
    monkeypatch.setenv("KOVA_HOME", str(broken))
    # Should NOT raise.
