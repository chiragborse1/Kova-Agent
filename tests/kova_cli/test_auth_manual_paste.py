...
Regression coverage for [#26923](https://github.com/chiragborse1/Kova-Agent/issues/26923):
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # kova generates state internally; we won't know it ahead of
        # time, so capture the state kova baked into the authorize
...
def test_xai_wait_for_callback_accepts_ready_stdin_code(monkeypatch):
    """Users can paste the Grok Build code while kova is still waiting."""
    class _StubServer:
