...
def test_xai_model_flow_reauth_uses_standard_radio_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth.get_xai_oauth_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "kova_cli.setup._curses_prompt_choice",
    ...
    monkeypatch.setattr("kova_cli.auth._login_xai_oauth", _fake_login)
    monkeypatch.setattr(
...
def test_xai_model_flow_cancel_skips_reauth(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.auth._login_xai_oauth",
