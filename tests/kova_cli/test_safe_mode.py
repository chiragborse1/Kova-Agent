"""Tests for `kova chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("KOVA_SAFE_MODE", "KOVA_IGNORE_USER_CONFIG", "KOVA_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in kova_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("KOVA_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under KOVA_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("KOVA_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("KOVA_SAFE_MODE", raising=False)
        from kova_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under KOVA_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `kova chat`."""
