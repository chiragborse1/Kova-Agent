...
_KOVA_BEHAVIORAL_VARS = frozenset({
    ...
    "KOVA_MAX_ITERATIONS",
    # Session-context vars — both the new lowercase prefix (used by code
    # after the rename) and the legacy uppercase prefix (used by the
    # conftest and backward-compat code paths).
    "KOVA_SESSION_PLATFORM", "kova_SESSION_PLATFORM",
    "KOVA_SESSION_CHAT_ID", "kova_SESSION_CHAT_ID",
    "KOVA_SESSION_CHAT_NAME", "kova_SESSION_CHAT_NAME",
    "KOVA_SESSION_THREAD_ID", "kova_SESSION_THREAD_ID",
    "KOVA_SESSION_SOURCE", "kova_SESSION_SOURCE",
    "KOVA_SESSION_KEY", "kova_SESSION_KEY",
    "KOVA_GATEWAY_SESSION", "kova_GATEWAY_SESSION",
    "KOVA_CRON_SESSION", "kova_CRON_SESSION",
    "_KOVA_GATEWAY",
    "KOVA_PLATFORM",
    "KOVA_MODEL",
    ...
    "KOVA_TUI_PROVIDER",
    # Managed env — both uppercase (legacy) and lowercase (current code).
    "KOVA_MANAGED", "kova_MANAGED",
    "KOVA_MANAGED_DIR", "kova_MANAGED_DIR",
    "KOVA_DEV",
    "KOVA_CONTAINER",
...
# environment and finds the developer's live ``kova-gateway`` process
...
# correlated with a ``tests/kova_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> kova-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart kova-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting kova/python patterns are also blocked.
    """
    ...
    def _matches_KOVA_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _KOVA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_KOVA_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch kova-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m kova_cli.main".
                if (
                    "kova" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live kova-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting kova/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `kova update` (or the
        # equivalent `python -m kova_cli.main update`).  These commands
        ...
        if "update" in low and (
            # kova update / kova update --gateway / setsid bash -c ... kova update
            ("kova" in low and "update" in low.split())
            or
            # python -m kova_cli.main update --gateway
            ("kova_cli" in low and "update" in low.split())
            or
            # venv/bin/kova update  (absolute path variant used in tests)
            (".venv/bin/kova" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`kova update` against the real checkout, fetching "
