"""Tests for MiniMax OAuth provider (kova_cli/auth.py).
...
def test_poll_token_timeout_raises():
    ...
    import kova_cli.auth as auth_module
...
def test_refresh_updates_access_token():
    ...
    with patch("httpx.Client") as mock_client_class:
        ...
        with patch("kova_cli.auth._minimax_save_auth_state"):
            result = _refresh_minimax_oauth_state(state)
...
def test_resolve_credentials_requires_login():
    ...
    with patch("kova_cli.auth.get_provider_auth_state", return_value=None):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure():
    ...
    with patch("kova_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("kova_cli.auth._refresh_minimax_oauth_state", side_effect=_terminal_refresh), \
         patch("kova_cli.auth._minimax_save_auth_state", side_effect=_capture_save):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure():
    ...
    with patch("kova_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("kova_cli.auth._refresh_minimax_oauth_state", side_effect=_transient_refresh), \
         patch("kova_cli.auth._minimax_save_auth_state", side_effect=lambda s: saved_states.append(dict(s))):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_get_minimax_oauth_auth_status_logged_in():
    ...
    with patch("kova_cli.auth.get_provider_auth_state", return_value=state):
        status = get_minimax_oauth_auth_status()
...
def test_token_provider_refreshes_when_near_expiry():
    ...
    with patch("kova_cli.auth.get_provider_auth_state", return_value=state), \
         patch("httpx.Client") as mock_client_class, \
         patch("kova_cli.auth._minimax_save_auth_state"):
...
def test_token_provider_rereads_state_each_call():
    ...
    persisted by another kova process are immediately visible."""
    from kova_cli.auth import build_minimax_oauth_token_provider
    ...
    provider = build_minimax_oauth_token_provider()
    with patch("kova_cli.auth.get_provider_auth_state", side_effect=states):
        first = provider()
        second = provider()
