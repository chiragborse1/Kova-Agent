...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kova", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def test_sessions_delete_reports_not_found_when_prefix_is_unknown(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kova", "sessions", "delete", "missing-prefix", "--yes"],
    )
...
def test_sessions_delete_handles_eoferror_on_confirm(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["kova", "sessions", "delete", "20260315_092437_c9a6"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None):
    """Run `kova sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(hermes_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["kova", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `kova sessions prune` keeps the implicit 90-day cutoff."""
