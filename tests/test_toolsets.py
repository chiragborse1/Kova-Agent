...
class TestToolsetConsistency:
    ...
    def test_KOVA_platforms_share_core_tools(self):
        """All kova-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on kova-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["kova-cli", "kova-telegram", "kova-discord", "kova-whatsapp", "kova-slack", "kova-signal", "kova-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_KOVA_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kova-whatsapp")

    def test_KOVA_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kova-api-server")
