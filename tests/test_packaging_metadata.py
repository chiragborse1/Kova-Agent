...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``kova_cli``) for the package
    itself and ``kova_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``kova_cli`` without ``kova_cli.*``,
    so the wheel shipped ``kova_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'kova_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    kova_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (kova_cli/plugins.py) registers each bundled plugin by
