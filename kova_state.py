#!/usr/bin/env python3
"""
SQLite State Store for Kova Agent.
...
DEFAULT_DB_PATH = get_kova_home() / "state.db"
...
# kova_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``kova_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see kova_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_KOVA_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple kova processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``kova_state._set_last_init_error(None)`` explicitly.
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._KOVA_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._KOVA_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another kova surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``kova sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade kova, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one kova session.

        A kova session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a kova session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``KOVA_DISABLE_FTS_TRIGRAM`` or not yet created), so
