"""Shared constants for Kova Agent.
...
_UNSET = object()
_KOVA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_KOVA_HOME_OVERRIDE", default=_UNSET
)


def set_kova_home_override(path: str | Path | None) -> Token:
    """Set a context-local Kova home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _KOVA_HOME_OVERRIDE.set(value)


def reset_kova_home_override(token: Token) -> None:
    """Restore the previous context-local Kova home override."""
    _KOVA_HOME_OVERRIDE.reset(token)


def get_kova_home_override() -> str | None:
    """Return the active context-local Kova home override, if any."""
    override = _KOVA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_kova_home() -> Path:
    """Return the platform-native default Kova home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "kova"
    return Path.home() / ".kova"


def get_kova_home() -> Path:
    """Return the Kova home directory (default: platform-native path).

    Reads KOVA_HOME env var, falls back to the platform-native default.
    ...
    ``kova_cli/kanban_db.py``).  See https://github.com/chiragborse1/Kova-Agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_kova_home()
...
def get_default_kova_root() -> Path:
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when kova is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``KOVA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("KOVA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kova_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``KOVA_OPTIONAL_MCPS``.
    """
    override = os.getenv("KOVA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kova_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<KOVA_HOME>/skills`` last-resort
    """
...
def get_kova_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Kova subdirectory with backward compatibility.
...
def iter_kova_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Kova-managed Node.js directories in preferred lookup order.
    ...
    # NOTE: keep this ordering in sync with kovaManagedNodePathEntries() in
...
_KOVA_NODE_TARGET_MAJOR = int(os.environ.get("KOVA_NODE_TARGET_MAJOR", "22"))
...
def kova_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Kova-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_kova_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    ...
    home = get_kova_home()
    index_url = f"https://nodejs.org/dist/latest-v{_KOVA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_KOVA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_kova_managed_node() -> bool:
    """Redownload Kova-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not kova_managed_node_tree_present():
        return False
...
def find_kova_node_executable(command: str) -> str | None:
    """Return a Kova-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_kova_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_kova_managed_node():
        for directory in iter_kova_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for kova-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Kova-managed installs.

    This is for Kova-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_kova_node_executable(command)
    if managed:
        return managed
    if kova_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_kova_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Kova-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_kova_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``kova update`` — leaving a **dangling symlink** that ``which`` still
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/chiragborse1/Kova-Agent/issues/25821.
    """
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("KOVA_REAL_HOME") or os.getenv("KOVA_REAL_HOME", "")).strip()
    if explicit:
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Kova's subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["KOVA_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: chiragborse1/Kova-Agent#47111
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_kova_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._kova_ipv4_patched = True  # type: ignore[attr-defined]
