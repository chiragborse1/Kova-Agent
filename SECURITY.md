# Kova Agent Security Policy

This document describes Kova Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/chiragborse1/Kova-Agent/security/advisories/new)
...
- Environment details (`kova version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Kova Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Kova Agent's documentation
...
Kova Agent supports two OS-level isolation postures. They address
...
Kova Agent supports this in two ways:

- **Kova Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Kova Agent's in-process heuristics
...
Kova Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Kova Agent
itself. Bugs in Kova Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Kova Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Kova Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Kova Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  kova-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Kova Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Kova Agent. Bugs in kova
...
- Kova Agent includes supply-chain guards for MCP server
