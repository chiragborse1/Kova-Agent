...
# Do not write .pyc files at runtime: /opt/kova is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/kova/.playwright
...
# kova process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main kova process, the dashboard,
...
# for the same reason: it lazy-installs into /opt/kova/.venv at first
...
# Wire the exec shim and install-method stamp.  Files under /opt/kova are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the hermes user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/kova/bin && \
    cp /opt/kova/docker/hermes-exec-shim.sh /opt/kova/bin/hermes && \
    chmod 0755 /opt/kova/bin/hermes && \
...
# /opt/kova/.hermes_build_sha at build time, and have
...
RUN if [ -n "${HERMES_GIT_SHA}" ]; then \
        printf '%s\n' "${HERMES_GIT_SHA}" > /opt/kova/.hermes_build_sha; \
    fi
...
# fast path (`node --expose-gc /opt/kova/ui-tui/dist/entry.js`) and skip the
...
# The published image seals /opt/kova (root-owned, read-only) so a runtime
...
# The shim lives at /opt/kova/bin/hermes, sits earliest on PATH, and
...
# absolute path (/opt/kova/.venv/bin/hermes). See the shim source for
...
#
# /opt/kova/bin is prepended ahead of the venv so the privilege-drop
