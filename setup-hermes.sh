...
# 5. Symlinks the 'kova' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Kova Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/kova-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink kova into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up kova command..."

KOVA_BIN="$SCRIPT_DIR/venv/bin/kova"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KOVA_BIN" "$COMMAND_LINK_DIR/kova"
echo -e "${GREEN}✓${NC} Symlinked kova → $COMMAND_LINK_DISPLAY_DIR/kova"
...
# ============================================================================
# Seed bundled skills into ~/.kova/skills/
# ============================================================================

KOVA_SKILLS_DIR="${KOVA_HOME:-$HOME/.kova}/skills"
mkdir -p "$KOVA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.kova/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KOVA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  kova status        # Check configuration"
if is_termux; then
    echo "  Kova Gateway       # Run gateway in foreground"
else
    echo "  Kova Gateway install # Install gateway service (messaging + cron)"
fi
echo "  kova cron list     # View scheduled jobs"
echo "  kova doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m KOVA_cli.main setup
fi
