"""ACP auth helpers — detect and advertise kova authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "kova-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active kova runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if kova can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for kova.
    ...
    not have kova provider credentials configured yet, so kova always
