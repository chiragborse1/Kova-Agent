...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so kova running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; kova cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a kova/OpenAI-compatible user content payload."""
...
class KovaACPAgent(acp.Agent):
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so kova maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from kova_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from kova_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  kova estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_KOVA_SESSION_ID: Optional[str] = None,
        previous_KOVA_SESSION_ID: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after kova changes it.

        When the internal kova head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_KOVA_SESSION_ID`` so the
        attached ``_meta.kova.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # kova_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_KOVA_SESSION_ID or session_id,
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["kova-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kova-agent", version=KOVA_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # kova' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches kova setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes kova
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # ACP thought panes should not receive kova' local kawaii waiting/status
        ...
        # not the executor's. Also set KOVA_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue KOVA_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind KOVA_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("KOVA_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("KOVA_SESSION_ID")
            ...
            try:
                ...
                # Restore KOVA_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("KOVA_INTERACTIVE", None)
                else:
                    os.environ["KOVA_INTERACTIVE"] = previous_interactive
                # Restore KOVA_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("KOVA_SESSION_ID", None)
                else:
                    os.environ["KOVA_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal kova DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (KOVA_SESSION_KEY in
        ...
        # _meta.kova.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_KOVA_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_KOVA_id
            and pre_turn_KOVA_id
            and post_turn_KOVA_id != pre_turn_KOVA_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_KOVA_SESSION_ID=post_turn_KOVA_id,
        ...
        # kova' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Kova Agent v{KOVA_VERSION}"
