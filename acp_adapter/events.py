...
def _json_loads_maybe_prefix(value: str) -> Any:
    """Parse a JSON object even when kova appended a human hint after it."""
    text = value.strip()
    try:
...
def _build_plan_update_from_todo_result(result: Any) -> AgentPlanUpdate | None:
    """Translate kova' todo tool result into ACP's native plan update.

    Zed renders ``sessionUpdate: plan`` as its first-class task/todo panel. The
    Kova Agent already maintains task state through the ``todo`` tool, so the
