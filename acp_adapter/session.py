...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when kova itself is running in WSL.

    Windows ACP clients can launch ``kova acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch kova from a Windows workspace while the ACP process runs
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current kova runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.kova/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``KOVA_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the kova daemon's process cwd.
