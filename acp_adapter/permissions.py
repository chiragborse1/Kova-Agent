"""ACP permission bridging for kova dangerous-command approvals."""
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match kova approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping kova semantics in the option id.
            kind="allow_always",
...
def make_approval_callback(
    ...
    """
    Return a kova-compatible approval callback that bridges to ACP.
