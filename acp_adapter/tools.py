...
# ---------------------------------------------------------------------------
# Map kova tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some kova tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    ...
    # kova core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # kova read_file output is line-numbered with `|`. If we send it as raw
