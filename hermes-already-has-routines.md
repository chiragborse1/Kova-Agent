# Kova Agent Has Had "Routines" Since March
...
kova equivalent — works today:
```bash
kova webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working kova implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Kova Agent |
|---|---|---|
...
## Things kova Does That Routines Can't
...
kova has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
```bash
Kova Gateway setup    # enable webhooks
kova webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [kova-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://kova-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [kova-agent.nousresearch.com](https://kova-agent.nousresearch.com)

GitHub: [github.com/chiragborse1/Kova-Agent](https://github.com/chiragborse1/Kova-Agent)
