  ...
  it('honors explicit env override', () => {
    expect(
      shouldEmitClipboardSequence({
        KOVA_TUI_CLIPBOARD_OSC52: '1',
        TMUX: '/tmp/tmux-1/default,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
    expect(
      shouldEmitClipboardSequence({ KOVA_TUI_COPY_OSC52: '0', TERM: 'xterm-256color' } as NodeJS.ProcessEnv)
    ).toBe(false)
  })

  it('KOVA_TUI_FORCE_OSC52 takes precedence over TMUX suppression', () => {
    ...
    expect(
      shouldEmitClipboardSequence({
        KOVA_TUI_FORCE_OSC52: '1',
        TMUX: '/tmp/t,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
  })

  it('KOVA_TUI_FORCE_OSC52=0 suppresses OSC 52 even for remote or plain terminals', () => {
    expect(
      shouldEmitClipboardSequence({
        KOVA_TUI_FORCE_OSC52: '0',
        SSH_CONNECTION: '1'
      } as NodeJS.ProcessEnv)
    ).toBe(false)
  })
...
describe('shouldUseNativeClipboard', () => {
  ...
  it('returns true when OSC 52 emission is disabled via KOVA_TUI_FORCE_OSC52=0', () => {
    ...
    expect(shouldUseNativeClipboard({ KOVA_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'ghostty')).toBe(true)
    expect(shouldUseNativeClipboard({ KOVA_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'kitty')).toBe(true)
    expect(shouldUseNativeClipboard({ KOVA_TUI_CLIPBOARD_OSC52: '0' } as NodeJS.ProcessEnv, 'WezTerm')).toBe(true)
    expect(shouldUseNativeClipboard({ KOVA_TUI_COPY_OSC52: 'no' } as NodeJS.ProcessEnv, 'vscode')).toBe(true)
  })

  it('returns true under TMUX even with KOVA_TUI_FORCE_OSC52=1 on an allowlisted terminal (tmux load-buffer path)', () => {
    ...
    expect(
      shouldUseNativeClipboard({ KOVA_TUI_FORCE_OSC52: '1', TMUX: '/tmp/t,1,0' } as NodeJS.ProcessEnv, 'ghostty')
