...
describe('shouldUseAnsiDim', () => {
  ...
  it('honors explicit env override', () => {
    expect(shouldUseAnsiDim({ KOVA_TUI_DIM: '1', VTE_VERSION: '7603' } as NodeJS.ProcessEnv)).toBe(true)
    expect(shouldUseAnsiDim({ KOVA_TUI_DIM: '1', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)).toBe(true)
    expect(shouldUseAnsiDim({ KOVA_TUI_DIM: '0' } as NodeJS.ProcessEnv)).toBe(false)
  })
})
...
describe('dimColorFallback', () => {
  ...
  it('does not apply when dim is explicitly configured', () => {
    expect(
      dimColorFallback({ KOVA_TUI_DIM: '1', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)
    ).toBeUndefined()
    expect(
      dimColorFallback({ KOVA_TUI_DIM: '0', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)
    ).toBeUndefined()
  })
})
