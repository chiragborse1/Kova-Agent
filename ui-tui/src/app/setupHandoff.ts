import type { RunExternalProcess } from '@kova/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`kova ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching kova: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`kova ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
