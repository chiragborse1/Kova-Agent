import { forceRedraw, type MouseTrackingMode } from '@kova/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Kova Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `kova update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try KOVA_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `kova #${i + 1}`
