...
export function ModelPicker({ allowPersistGlobal = true, gw, onCancel, onSelect, sessionId, t }: ModelPickerProps) {
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `kova model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run kova model)
          return
        }
    ...
    // filter. With Ctrl held, @kova/ink reports `ch` as the key name ('g'),
