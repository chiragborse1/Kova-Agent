...
// Mirrors @kova/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @kova/ink is bundled separately from packages/.
...
// Terminal. Explicit KOVA_TUI_THEME / COLORFGBG signals above still win,
...
// `KOVA_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `KOVA_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `KOVA_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `KOVA_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default kova palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.KOVA_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.KOVA_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.KOVA_TUI_BACKGROUND ?? '')
