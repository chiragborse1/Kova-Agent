import type { MouseTrackingMode } from '@kova/ink'
...
export const STARTUP_RESUME_ID = (process.env.KOVA_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.KOVA_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.KOVA_TUI_IMAGE ?? '').trim()
...
//
// - KOVA_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - KOVA_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.KOVA_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.KOVA_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.KOVA_TUI_NO_CONFIRM)
...
export const DASHBOARD_TUI_MODE = truthy(process.env.KOVA_TUI_DASHBOARD)

// KOVA_DEV_CREDITS — dev-only live-spend readout (Δ status segment + "(dev credits)"
// banner). Throwaway dev scaffolding; the whole readout gates on this one flag.
export const DEV_CREDITS_MODE = truthy(process.env.KOVA_DEV_CREDITS)

const inlineOverride = parseToggle(process.env.KOVA_TUI_INLINE)
...
// less fragile. Override explicitly with KOVA_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.KOVA_TUI_FPS)
