...
import { scrollFastPathStats } from '@kova/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.KOVA_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.KOVA_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.KOVA_DEV_PERF_LOG?.trim() || join(homedir(), '.kova', 'perf.log')
