...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because KOVA_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.KOVA_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.kova', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `kova-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.KOVA_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.KOVA_HEAPDUMP_MAX_BYTES?.trim()
