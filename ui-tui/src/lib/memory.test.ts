...
const ENV_KEYS = ['KOVA_AUTO_HEAPDUMP', 'KOVA_HEAPDUMP_DIR', 'KOVA_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'kova-heapdump-test-'))
    process.env.KOVA_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without KOVA_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without KOVA_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when KOVA_AUTO_HEAPDUMP=1', async () => {
    process.env.KOVA_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.KOVA_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of KOVA_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.KOVA_HEAPDUMP_DIR
    savedMax = process.env.KOVA_HEAPDUMP_MAX_BYTES
    delete process.env.KOVA_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'kova-heapdump-prune-'))
    process.env.KOVA_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.KOVA_HEAPDUMP_DIR
    } else {
      process.env.KOVA_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.KOVA_HEAPDUMP_MAX_BYTES
    } else {
      process.env.KOVA_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.KOVA_HEAPDUMP_MAX_BYTES = String(2 * 1024)
