...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.KOVA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep kova on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via KOVA_TUI_TRUECOLOR=1.
