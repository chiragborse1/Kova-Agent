 ...
 * kova-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``KOVA_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but kova-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS kova-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on kova-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is kova-ink's Alt signal and accepting it would
