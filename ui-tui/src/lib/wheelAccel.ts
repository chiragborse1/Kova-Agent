...
import { isXtermJs } from '@kova/ink'
...
/** KOVA_TUI_SCROLL_SPEED (or CLAUDE_CODE_SCROLL_SPEED for portability).
 *  Default 1, clamped (0, 20]. */
export function readScrollSpeedBase(): number {
  const n = parseFloat(process.env.KOVA_TUI_SCROLL_SPEED ?? process.env.CLAUDE_CODE_SCROLL_SPEED ?? '')
