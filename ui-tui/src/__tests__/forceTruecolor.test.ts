...
const ENV_KEYS = ['COLORTERM', 'FORCE_COLOR', 'KOVA_TUI_TRUECOLOR', 'NO_COLOR', 'TERM', 'TERM_PROGRAM'] as const
let importId = 0
...
describe('forceTruecolor', () => {
  ...
  it('sets COLORTERM=truecolor and FORCE_COLOR=3 when explicitly enabled', async () => {
    await withCleanEnv(
      () => {
        process.env.KOVA_TUI_TRUECOLOR = '1'
      },
      async () => {
  ...
  it('respects KOVA_TUI_TRUECOLOR=0 opt-out', async () => {
    await withCleanEnv(
      () => {
        process.env.KOVA_TUI_TRUECOLOR = '0'
        process.env.TERM_PROGRAM = 'Apple_Terminal'
      },
      async () => {
  ...
  it('respects existing FORCE_COLOR unless kova truecolor is explicit', async () => {
    await withCleanEnv(
      () => {
  ...
  it('lets explicit kova truecolor override existing FORCE_COLOR', async () => {
    await withCleanEnv(
      () => {
