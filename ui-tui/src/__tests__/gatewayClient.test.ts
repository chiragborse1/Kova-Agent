...
describe('GatewayClient websocket attach mode', () => {
  ...
  beforeEach(() => {
    originalGatewayUrl = process.env.KOVA_TUI_GATEWAY_URL
    originalSidecarUrl = process.env.KOVA_TUI_SIDECAR_URL
    FakeWebSocket.reset()
  ...
  afterEach(() => {
    if (originalGatewayUrl === undefined) {
      delete process.env.KOVA_TUI_GATEWAY_URL
    } else {
      process.env.KOVA_TUI_GATEWAY_URL = originalGatewayUrl
    }

    if (originalSidecarUrl === undefined) {
      delete process.env.KOVA_TUI_SIDECAR_URL
    } else {
      process.env.KOVA_TUI_SIDECAR_URL = originalSidecarUrl
    }
  ...
  it('waits for websocket open and resolves RPC requests', async () => {
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
  ...
  it('mirrors event frames to sidecar websocket when configured', async () => {
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.KOVA_TUI_SIDECAR_URL = 'ws://gateway.test/api/pub?token=abc&channel=demo'
  ...
  it('reattaches when KOVA_TUI_GATEWAY_URL rotates between requests', async () => {
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway-old.test/api/ws?token=abc'
    const gw = new GatewayClient()
    ...
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway-new.test/api/ws?token=xyz'
  ...
  it('uses the undici WebSocket fallback when global WebSocket is unavailable', () => {
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=hunter2&channel=secret'
    delete (globalThis as { WebSocket?: unknown }).WebSocket
  ...
  it('redacts attach URL secrets when the WebSocket constructor throws', () => {
    ...
    process.env.KOVA_TUI_GATEWAY_URL = secretUrl
  ...
  it('redacts sidecar URL secrets when the WebSocket constructor throws', async () => {
    ...
    process.env.KOVA_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.KOVA_TUI_SIDECAR_URL = sidecarUrl
  ...
  it('redacts user-info credentials even on URLs the WHATWG parser rejects', () => {
    ...
    process.env.KOVA_TUI_GATEWAY_URL = fixture
    ;(globalThis as { WebSocket?: unknown }).WebSocket = class ThrowingWebSocket extends FakeWebSocket {
      constructor(url: string) {
        throw new TypeError(`Invalid URL: ${url}`)
      }
