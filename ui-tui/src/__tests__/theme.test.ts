...
// KOVA_TUI_THEME=light (or KOVA_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Kova Agent')
...
describe('detectLightMode', () => {
  ...
  it('honors KOVA_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ KOVA_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ KOVA_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ KOVA_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ KOVA_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ KOVA_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets KOVA_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', KOVA_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors KOVA_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ KOVA_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ KOVA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', KOVA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', KOVA_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses KOVA_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ KOVA_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // KOVA_TUI_THEME=light would flip the base palette and make these
