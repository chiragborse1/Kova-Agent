...
describe('isTermuxTuiMode', () => {
  ...
  it('allows explicit opt-out override', () => {
    expect(isTermuxTuiMode({ TERMUX_VERSION: '0.118.0', KOVA_TUI_TERMUX_MODE: '0' } as NodeJS.ProcessEnv)).toBe(false)
  })

  it('stays false outside Termux even if override is set', () => {
    expect(isTermuxTuiMode({ KOVA_TUI_TERMUX_MODE: '1', PREFIX: '/usr' } as NodeJS.ProcessEnv)).toBe(false)
  })
})
