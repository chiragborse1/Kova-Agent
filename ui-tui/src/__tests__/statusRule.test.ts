...
describe('statusRuleWidths', () => {
  it('keeps the status rule within the terminal width', () => {
    for (const cols of [8, 12, 20, 40, 100]) {
      const widths = statusRuleWidths(cols, '~/src/kova-agent/main (some-long-branch-name)')
  ...
  it('truncates the cwd segment before it can wrap in skinny terminals', () => {
    const widths = statusRuleWidths(24, '~/src/kova-agent/main (bb/some-extremely-long-branch)')

    expect(widths.rightWidth).toBeLessThan('~/src/kova-agent/main (bb/some-extremely-long-branch)'.length)
  ...
  it('reserves the high-priority left content so the cwd/branch yields first', () => {
    const cwd = '~/src/kova-agent/apps/desktop (bb/tui-statusbar-responsive)'
  ...
  it('keeps the default (no reservation) behaviour identical for legacy callers', () => {
    const cwd = '~/src/kova-agent/main (some-long-branch-name)'
