...
describe('createSlashHandler', () => {
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'kova-agent-dev' })
            }
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: kova-agent-dev')
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/KOVA_conversation_test.json')
    })
  })
