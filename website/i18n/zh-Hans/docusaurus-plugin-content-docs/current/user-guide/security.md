...
Kova Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
在执行任何命令之前，kova 会将其与一份精心维护的危险模式列表进行比对。若匹配，用户必须明确批准。
...
某些命令极具破坏性——不可逆的文件系统清除、fork 炸弹、直接写入块设备——无论以下任何情况，kova 都**拒绝**执行：
...
:::tip
使用 `kova config edit` 查看或删除永久允许列表中的模式。
:::

## 用户授权（Gateway）

运行消息 gateway 时，kova 通过分层授权系统控制谁可以与机器人交互。
...
为实现更灵活的授权，kova 提供了基于验证码的配对系统。无需预先提供用户 ID，未知用户会收到一次性配对码，由机器人所有者通过 CLI 批准。
...
使用 `docker` 终端后端时，kova 对每个容器应用严格的安全加固。
...
某些技能需要在沙箱中访问**文件**（而非仅环境变量）——例如，Google Workspace 将 OAuth token 存储为活跃 profile 的 `KOVA_HOME` 下的 `google_token.json`。技能在 frontmatter 中声明这些文件：
...
加载后，kova 会检查这些文件是否存在于活跃 profile 的 `KOVA_HOME` 中，并将其注册为挂载：
...
| **terminal**（本地） | 阻止明确的 kova 基础设施变量（提供商密钥、gateway token、工具 API 密钥） | ✅ 透传变量绕过黑名单 |
...
- kova 基础设施密钥（提供商 API 密钥、gateway token）不应添加到 `env_passthrough`——它们有专用机制
...
kova 集成了 [tirith](https://github.com/sheeki03/tirith) 用于在执行前进行内容级命令扫描。Tirith 能检测单纯模式匹配所遗漏的威胁：
...
Tirith 为 Linux（x86_64 / aarch64）和 macOS（x86_64 / arm64）提供预构建二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过——模式匹配防护仍然运行，CLI 不会显示"不可用"横幅。若要在 Windows 上使用 tirith，请在 WSL 下运行 Kova。
...
- **CLI 启动横幅。** 若有任何公告匹配，会打印一行警告，并指向 `kova doctor` 获取完整修复方案。
- **`kova doctor`。** 显示所有活跃公告的版本详情和 2-4 步修复说明。
...
3. 若安装失败或用户已禁用懒加载安装，调用会抛出 `FeatureUnavailable`，附带实际的 pip stderr 和指向 `kova tools` 的提示。
...
禁用后，需要可选依赖的后端会提示用户手动运行安装（`pip install …`）或通过 `kova tools` 选择其他后端。