...
**Profile 分发**将一个完整的 Kova Agent——个性、技能、cron 任务、MCP 连接、配置——打包为一个 git 仓库。任何有权访问该仓库的人都可以用一条命令安装整个 agent，就地更新，并保持自己的记忆、会话和 API 密钥不受影响。

如果说 [profile](./profiles.md) 是本地 agent，那么分发就是让该 agent 可共享的形式。

## 这意味着什么

在分发功能出现之前，共享一个 Kova Agent 意味着要发送：
...
……他们就拥有了完整的 agent。填入自己的 API 密钥（`.env.EXAMPLE` → `.env`），即可运行 `my-research-agent chat`，或通过 Telegram / Discord / Slack / 任何 gateway 平台与其交互。当你推送新版本时，他们运行 `kova profile update my-research-agent` 即可拉取你的更改——他们的记忆和会话保持不变。
...
权衡之处：接收方需要安装 git。在 2026 年运行 kova 的任何机器上，这已是既成事实。
...
创建 `~/.kova/profiles/research-bot/distribution.yaml`：
...
运行 `kova profile update research-bot` 的接收方将拉取最新版本。
...
4. 请求确认。传入 `-y` / `--yes` 可跳过。
5. 将分发所有的文件复制到 `~/.kova/profiles/research-bot/`（或 manifest 中 `name` 解析到的位置）。
...
`kova profile list` 还会显示 `Distribution` 列，让你一眼看出哪些 profile 来自仓库，哪些是手动构建的：
...
在笔记本上的任何迭代（`git commit && push`）都可以通过 `kova profile update research-bot` 同步到工作站。记忆按机器独立保存——笔记本记住自己的对话，工作站记住自己的，互不干扰。
...
当负责人发布 v1.1（更好的 SOUL、新技能）时，工程师运行 `kova profile update pr-reviewer`，所有人在几分钟内就能用上新版本。
...
你在 kova 之上构建了产品——也许是合规监控框架、客服技术栈、特定领域的研究平台。你想以产品形式分发它。
...
:::note
Git ref 固定（`#v1.2.0`）已在规划中，但不在初始版本中——目前安装时跟踪默认分支。通过 `kova profile info <name>` 查看已安装版本，在准备好之前暂缓更新。
:::
...
# 在 ~/.kova/profiles/forked-research-bot/ 中本地迭代
...
来自分发的 cron 任务**不会自动调度**——安装器会打印 `kova -p <name> cron list`，你需要显式启用它们。SOUL.md 和技能在你开始与 profile 对话后立即生效，因此如果你从不熟悉的来源安装，请在第一次运行前阅读它们。
...
- `install`、`update`、`info` 位于 `kova profile` 下——不是独立的命令树。
...
- 保留的 profile 名称（`kova`、`test`、`tmp`、`root`、`sudo`）在安装时会被拒绝，以避免与常见二进制文件冲突。
