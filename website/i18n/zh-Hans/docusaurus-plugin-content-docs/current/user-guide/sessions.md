...
Kova Agent 自动将每次对话保存为一个 session。Session 支持对话恢复、跨 session 搜索以及完整的对话历史管理。

## Session 的工作原理

每次对话——无论来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Teams 还是其他任何消息平台——都会以完整消息历史的形式存储为一个 session。Session 记录在：

1. **SQLite 数据库**（`~/.kova/state.db`）——包含 FTS5 全文搜索的结构化 session 元数据，以及完整消息历史
...
kova 存储 session 历史以便恢复对话，但不会在每次对话时重新发送所有历史字节。每轮对话中，模型看到的是：所选系统 prompt、当前对话窗口，以及 kova 为该轮显式注入的内容。
...
例如，如果用户发送一张图片并要求 kova 制作表情包，kova 可能会用视觉能力检查该图片一次并运行图像处理脚本。后续轮次不会自动将原始 JPEG 带入上下文，只携带写入对话的内容，例如用户的请求、简短的图片描述、本地缓存路径或最终的助手回复。

上下文增长最常见的原因不是媒体文件本身，而是冗长的文本：粘贴的转录、完整日志、大型工具输出、长 diff、重复的状态报告以及详细的证明转储。优先使用摘要、文件路径、重点摘录和工具支持的查找，而不是将大型内容复制到聊天中。

:::tip
当 session 变长时使用 `/compress`，用 `/new` 开启新线程，仅在需要从存储中删除旧的已结束 session 时才使用 `kova sessions prune`。压缩会减少活跃上下文，而不是隐私删除。向 `/new` 传入名称（例如 `/new payments-refactor`）可以预先设置新 session 的初始标题——便于之后通过 `/resume <name>` 或 `/sessions` 选择器找到它。
:::
...
# 会自动恢复最新的一个
kova -c "my project"   # → 恢复 "my project #3"
```
...
Session ID 在退出 CLI session 时显示，也可通过 `kova sessions list` 查找。

### 恢复时的对话摘要

恢复 session 时，kova 会在输入提示符前以样式化面板显示之前对话的紧凑摘要：

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="恢复 kova session 时显示的「上次对话」摘要面板的样式化预览。" />
...
要禁用摘要并保留最简单的单行行为，在 `~/.kova/config.yaml` 中设置：
...
**恢复到 CLI：** 当你想回到桌面时，只需运行 `/resume <title>`（或在 shell 中运行 `kova -r "<title>"`），从平台停止的地方继续。
...
kova 在第一次交换后自动为每个 session 生成简短的描述性标题（3–7 个词）。这在后台线程中使用快速辅助模型运行，不增加延迟。浏览 `kova sessions list` 或 `kova sessions browse` 时可以看到自动生成的标题。
...
当 session 的上下文被压缩（通过 `/compress` 手动或自动触发）时，kova 会创建一个新的续接 session。如果原 session 有标题，新 session 会自动获得带编号的标题：

```
"my project" → "my project #2" → "my project #3"
```

按名称恢复时（`kova -c "my project"`），会自动选取谱系中最新的 session。
...
kova 通过 `kova sessions` 提供完整的 session 管理命令集：
...
当 kova 无法获取共享聊天的参与者标识符时，回退为该房间共享一个 session。

### 共享与隔离的群组 Session

默认情况下，kova 在 `config.yaml` 中使用 `group_sessions_per_user: true`。这意味着：

- Alice 和 Bob 可以在同一个 Discord 频道中与 kova 对话，而不共享对话历史
...
| 内容 | 路径 | 描述 |
|------|------|-------------|
| SQLite 数据库 | `~/.kova/state.db` | 所有 session 元数据 + 带 FTS5 的消息 |
| Gateway 消息 | `~/.kova/state.db` | SQLite——所有 session 消息的权威存储 |
| Gateway 路由索引 | `~/.kova/sessions/sessions.json` | 将 session 键映射到活跃 session ID（来源元数据、过期标志） |

SQLite 数据库使用 WAL 模式支持并发读取和单写入，非常适合 gateway 的多平台架构。

:::note 遗留 JSONL 对话记录
在 state.db 成为权威存储之前创建的 session 可能在 `~/.kova/sessions/` 中留有
`*.jsonl` 文件。kova 不再写入或读取这些文件。在确认对应 session 存在于
state.db 后可安全删除。
:::
...
- 清理最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；上次运行时间戳记录在 `state.db` 内部，因此在同一 `KOVA_HOME` 下的所有 kova 进程间共享

默认为**关闭**——session 历史对 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.kova/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个 session 约 10–15 MB），session 历史为跨历史对话的 `session_search` 召回提供支持，因此自动清理默认关闭。如果你运行繁重的 gateway/cron 工作负载且 `state.db` 明显影响性能（已观察到的故障模式：约 1000 个 session 的 384 MB state.db 导致 FTS5 插入和 `/resume` 列表变慢），则启用它。使用 `kova sessions prune` 进行一次性清理，无需开启自动清理。
:::