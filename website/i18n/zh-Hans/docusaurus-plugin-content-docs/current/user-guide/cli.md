...
sidebar_position: 1
title: "CLI 界面"
description: "掌握 Kova Agent 终端界面——命令、快捷键、人格设定等"
---

# CLI 界面

Kova Agent 的 CLI 是一个完整的终端用户界面（TUI），而非 Web UI。它支持多行编辑、斜杠命令自动补全、对话历史、中断并重定向，以及流式工具输出。专为常驻终端的用户而生。

:::tip
kova 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `kova --tui` 启动——参见 [TUI](tui.md) 指南。
:::
...
# 使用指定提供商
kova chat --provider nous        # 使用 Nous Portal
kova chat --provider openrouter  # 强制使用 OpenRouter
...
# 恢复之前的会话
kova --continue             # 恢复最近的 CLI 会话（-c）
kova --resume <session_id>  # 通过 ID 恢复指定会话（-r）

# 详细模式（调试输出）
kova chat --verbose

# 隔离的 git worktree（用于并行运行多个 agent）
kova -w                         # 在 worktree 中以交互模式运行
kova -w -z "Fix issue #123"     # 在 worktree 中以单次查询模式运行
```

## 界面布局

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Kova CLI 布局的风格化预览，展示了横幅、对话区域和固定输入提示符。" />
<p className="docs-figure-caption">Kova CLI 横幅、对话流和固定输入提示符，以稳定的文档图示形式呈现，而非脆弱的文字艺术。</p>
...
恢复之前的会话时（`kova -c` 或 `kova --resume <id>`），横幅与输入提示符之间会出现一个"Previous Conversation"面板，显示对话历史的简洁摘要。详情及配置说明参见[会话——恢复时的对话摘要](sessions.md#conversation-recap-on-resume)。
...
| `Ctrl+D` | 退出 |
| `Ctrl+Z` | 将 kova 挂起到后台（仅 Unix）。在 shell 中运行 `fg` 恢复。 |
...
输入 `/` 查看自动补全下拉菜单。kova 支持大量 CLI 斜杠命令、动态 skill 命令和用户自定义快捷命令。
...
kova 会在第一轮对话前将每个指定的 skill 加载到会话 prompt 中。该标志在交互模式和单次查询模式下均有效。
...
大多数终端默认对 `Enter` 和 `Shift+Enter` 发送相同的字节序列，因此应用程序无法区分它们。kova 仅在终端通过 [Kitty 键盘协议](https://sw.kovidgoyal.net/kitty/keyboard-protocol/)或 xterm 的 `modifyOtherKeys` 模式发送不同序列时才能识别 `Shift+Enter`。
...
当终端无法区分时，`Alt+Enter` 和 `Ctrl+J` 在所有终端中均可正常使用。**特别是在 Windows Terminal 中，`Alt+Enter` 被终端捕获（切换全屏），永远不会传递给 Kova——请直接使用 `Ctrl+Enter`（传递为 `Ctrl+J`）或 `Ctrl+J` 来换行。**
...
:::tip 首次提示
第一次在 kova 工作时按下 Enter，kova 会打印一行提示，说明 `/busy` 选项（`"(tip) Your message interrupted the current run…"`）。每次安装只触发一次——`config.yaml` 中 `onboarding.seen.busy_input_prompt` 下的标志会锁定它。删除该键可再次看到提示。
:::

### 挂起到后台

在 Unix 系统上，按 **`Ctrl+Z`** 将 kova 挂起到后台——与任何终端进程一样。shell 会打印确认信息：
...
```bash
kova --continue                          # 恢复最近的 CLI 会话
kova -c                                  # 简写形式
kova -c "my project"                     # 恢复命名会话（谱系中最新的）
kova --resume 20260225_143052_a1b2c3     # 通过 ID 恢复指定会话
kova --resume "refactoring auth"         # 通过标题恢复
kova -r 20260225_143052_a1b2c3           # 简写形式
```

恢复会从 SQLite 中还原完整的对话历史。agent 能看到所有之前的消息、工具调用和响应——就像从未离开一样。

在聊天中使用 `/title My Session Name` 为当前会话命名，或从命令行使用 `kova sessions rename <id> <title>`。使用 `kova sessions list` 浏览历史会话。
