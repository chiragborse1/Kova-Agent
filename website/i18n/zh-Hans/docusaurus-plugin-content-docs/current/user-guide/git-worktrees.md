...
title: "Git Worktrees"
description: "使用 git worktrees 和隔离检出在同一仓库中安全运行多个 Kova Agent"
---

# Git Worktrees

Kova Agent 常用于大型、长期维护的仓库。当你需要：
...
本页介绍如何将 worktrees 与 kova 结合使用，使每个会话拥有干净、隔离的工作目录。

## 为什么在 kova 中使用 Worktrees？

kova 将**当前工作目录**视为项目根目录：

- CLI：运行 `kova` 或 `kova chat` 时所在的目录
...
- `~/.kova/checkpoints/` 下的 kova checkpoint 数据在移除 worktree 时不会自动清理，但通常体积很小。

## 最佳实践

- **每个 kova 实验对应一个 worktree**
...
- **使用 worktrees 时避免从裸仓库根目录运行 kova**
  - 优先使用 worktree 目录，使每个 agent 拥有明确的作用范围。

## 使用 `kova -w`（自动 Worktree 模式）

kova 内置 `-w` 标志，可**自动创建一个一次性 git worktree** 及其独立分支。无需手动配置 worktree——只需 `cd` 进入仓库并运行：
...
如需并行运行多个 agent，在多个终端中分别运行 `kova -w`——每次调用都会自动获得独立的 worktree 和分支。

## 综合运用

- 使用 **git worktrees** 为每个 kova 会话提供独立的干净检出。
- 使用**分支**记录实验的高层级历史。
