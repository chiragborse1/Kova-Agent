...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/finance/3-statement-model` 安装 |
...
如果模板明确需要从 SEC 申报文件（10-K、10-Q）中提取数据，请参阅 [references/sec-filings.md](https://github.com/NousResearch/Kova-agent/blob/main/optional-skills/finance/3-statement-model/references/sec-filings.md) 获取详细提取指引。仅在使用上市公司监管申报文件数据填写模板时才需要此参考文档。
...
所有公式详情见 [references/formulas.md](https://github.com/NousResearch/Kova-agent/blob/main/optional-skills/finance/3-statement-model/references/formulas.md)。
...
以下许多段落提到"使用 S&P Kensho MCP / Daloopa MCP / FactSet MCP"。这些是原 Cowork 插件上下文中的商业金融数据 MCP。在 kova 中：

- **如果已配置任何结构化金融数据 MCP**（kova 支持 MCP — 参见 `native-mcp` skill），优先使用它获取时点可比数据、前例交易和申报文件。
- **否则**，回退至：
