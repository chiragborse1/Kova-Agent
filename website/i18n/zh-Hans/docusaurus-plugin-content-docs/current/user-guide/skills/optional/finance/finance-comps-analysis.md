...
| | |
|---|---|
| 来源 | 可选——通过 `kova skills install official/finance/comps-analysis` 安装 |
...
以下许多段落提到"使用 S&P Kensho MCP / Daloopa MCP / FactSet MCP"。这些是原始 Cowork 插件背景下的商业金融数据 MCP。在 kova 中：

- **如果你配置了任何结构化金融数据 MCP**（kova 支持 MCP——参见 `native-mcp` skill），优先使用它获取时点可比数据、先例交易和文件。
- **否则**，回退到：
