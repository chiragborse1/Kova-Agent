...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/blockchain/hyperliquid` 安装 |
| 路径 | `optional-skills/blockchain/hyperliquid` |
| 版本 | `0.1.0` |
| 作者 | Hugo Sequier (Hugo-SEQUIER), Kova Agent |
| 许可证 | MIT |
...
辅助脚本：`~/.kova/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
对于 `state`、`spot-balances`、`fills`、`orders` 和 `review`，当 `~/.kova/.env` 中设置了 `HYPERLIQUID_USER_ADDRESS` 时，地址参数为可选。
