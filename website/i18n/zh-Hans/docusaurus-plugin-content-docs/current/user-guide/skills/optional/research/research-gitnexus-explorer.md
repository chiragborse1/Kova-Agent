...
| | |
|---|---|
| 来源 | 可选 — 使用 `kova skills install official/research/gitnexus-explorer` 安装 |
...
- **Web UI 不会创建 `.claude/` 或 `CLAUDE.md`。** 这些文件由 `npx gitnexus analyze` 创建。使用 `--skip-agents-md` 可抑制 markdown 文件的生成，再用 `rm -rf .claude/` 清除其余内容。这些是 Claude Code 集成产物，Kova Agent 用户无需使用。
