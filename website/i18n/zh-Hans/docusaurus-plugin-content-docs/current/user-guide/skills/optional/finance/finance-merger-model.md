...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/finance/merger-model` 安装 |
...
以下部分内容提及"使用 S&P Kensho MCP / Daloopa MCP / FactSet MCP"。这些是原 Cowork 插件场景中的商业金融数据 MCP。在 kova 中：

- **如已配置任何结构化金融数据 MCP**（kova 支持 MCP — 参见 `native-mcp` skill），优先用于时点可比数据、前例交易及文件。
- **否则**，回退至：
