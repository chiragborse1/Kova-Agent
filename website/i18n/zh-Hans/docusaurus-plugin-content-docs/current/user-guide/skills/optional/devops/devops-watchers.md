...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/devops/watchers` 安装 |
...
安装 skill 后，三个脚本均位于 `$KOVA_HOME/skills/devops/watchers/scripts/`。每个脚本读取 `WATCHER_STATE_DIR`（默认为 `$KOVA_HOME/watcher-state/`）作为状态文件目录，以 `--name` 参数作为键名。
...
监控 GitHub 仓库（在 `~/.kova/.env` 中设置 `GITHUB_TOKEN` 以避免匿名请求限制 60 次/小时）：
...
每个 watcher 将状态写入 `$KOVA_HOME/watcher-state/<name>.json`。查看状态：
...
4. **状态目录位于 agent 沙箱无法写入的位置。** `$KOVA_HOME/watcher-state/` 始终可写。Docker/Modal 后端可能无法访问任意宿主机路径。