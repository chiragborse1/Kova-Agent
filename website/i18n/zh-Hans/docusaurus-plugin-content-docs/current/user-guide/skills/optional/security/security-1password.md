...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/security/1password` 安装 |
| 路径 | `optional-skills/security/1password` |
| 版本 | `1.0.0` |
| 作者 | arceus77-7，由 Kova Agent 增强 |
| 许可证 | MIT |
...
- `tmux` 可用，用于在 kova 终端调用期间保持稳定的已认证会话（仅限桌面应用流程）
...
在 `~/.kova/.env` 中设置 `OP_SERVICE_ACCOUNT_TOKEN`（skill 首次加载时会提示输入）。
...
kova 终端命令默认为非交互式，且在多次调用之间可能丢失认证上下文。
