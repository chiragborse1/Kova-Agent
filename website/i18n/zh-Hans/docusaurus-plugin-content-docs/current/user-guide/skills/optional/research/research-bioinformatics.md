...
| | |
|---|---|
| 来源 | 可选 — 使用 `kova skills install official/research/bioinformatics` 安装 |
...
:::info
以下是 kova 在触发该技能时加载的完整技能定义。这是 Agent 在技能激活时所看到的指令内容。
:::
...
3. 读取具体技能：
   ...
   ```
4. 将获取的技能作为参考资料使用。这些**不是** kova 格式的技能——请将其视为专家领域指南。它们包含正确的参数、合适的工具标志和经过验证的流程。
...
- 获取的技能**不是** kova SKILL.md 格式。它们使用各自的结构（bioSkills：代码模式手册；ClawBio：README + Python 脚本）。请将其作为专家参考资料阅读。
