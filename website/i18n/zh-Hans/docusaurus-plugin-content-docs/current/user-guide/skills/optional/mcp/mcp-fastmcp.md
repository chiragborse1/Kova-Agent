...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/mcp/fastmcp` 安装 |
...
- 除工具外还需暴露资源或 prompt
- 在接入 kova 或其他客户端之前，使用 FastMCP CLI 对服务器进行冒烟测试
- 将服务器安装到 Claude Code、Claude Desktop、Cursor 或类似的 MCP 客户端
- 为 HTTP 部署准备 FastMCP 服务器仓库

若服务器已存在且只需连接到 Kova，请使用 `native-mcp`。若目标是对现有 MCP 服务器进行临时 CLI 访问而非构建新服务器，请使用 `mcporter`。
