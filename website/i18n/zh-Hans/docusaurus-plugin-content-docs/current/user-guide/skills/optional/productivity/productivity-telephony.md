---
title: "电话功能 — 无需修改核心工具即可赋予 kova 电话能力"
sidebar_label: "Telephony"
description: "无需修改核心工具即可赋予 kova 电话能力"
---
...
无需修改核心工具即可赋予 kova 电话能力。配置并持久化 Twilio 号码，收发 SMS/MMS，直接拨打电话，以及通过 Bland.ai 或 Vapi 发起 AI 驱动的外呼。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 使用 `kova skills install official/productivity/telephony` 安装 |
...
此可选 skill 为 kova 提供实用的电话能力，同时将电话功能保留在核心工具列表之外。
...
它**不会**将 kova 变成实时入站电话网关（gateway）。入站 SMS 通过轮询 Twilio REST API 处理。这对许多工作流已经足够，包括通知和部分一次性验证码获取，无需添加核心 webhook 基础设施。
...
原因：
- 播放自定义 MP3 的最简方式
- 与 kova `text_to_speech` 加公开文件托管或隧道配合良好
...
这是复用 kova 现有 `text_to_speech` 支持的主要路径。

适用场景：
- 希望通话使用 kova 配置的 TTS 语音，而非 Twilio `<Say>`
...
推荐的 kova TTS -> Twilio Play 工作流：

1. 使用 kova `text_to_speech` 生成音频。
2. 使生成的 MP3 可公开访问。
3. 使用 `--audio-url` 拨打 Twilio 电话进行传递。

示例 agent 流程：
- 让 kova 使用 `text_to_speech` 创建消息音频
...
- 此处单独使用 kova STT/TTS 并非作为全双工电话对话引擎；那将需要比此 skill 所要引入的更重量级的流式/webhook 集成
...
5. 使用正确的命令。
6. 如有需要，轮询结果。
7. 总结结果，不要将第三方电话号码持久化到 kova 记忆中。
