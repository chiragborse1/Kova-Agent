---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 Kova Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 Kova Agent"
---
...
将用户的 OpenClaw 自定义配置迁移到 Kova Agent。从 `~/.openclaw` 导入 kova 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/migration/openclaw-migration` 安装 |
| 路径 | `optional-skills/migration/openclaw-migration` |
| 版本 | `1.0.0` |
| 作者 | Kova Agent (Nous Research) |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `Migration`, `OpenClaw`, `kova`, `Memory`, `Persona`, `Import` |
| 相关 skill | [`kova-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kova-agent) |

## 参考：完整 SKILL.md

:::info
以下是 kova 在触发此 skill 时加载的完整 skill 定义。这是 skill 激活时 agent 所看到的指令内容。
:::

# OpenClaw -> kova 迁移

当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 Kova Agent 时，使用此 skill。
...
**首次设置：** `kova setup` 向导会自动检测 `~/.openclaw`，并在配置开始前提供迁移选项。
...
- 将 OpenClaw 命令审批模式合并到 kova `command_allowlist`
- 迁移 kova 兼容的消息设置，例如 `TELEGRAM_ALLOWED_USERS` 和 `MESSAGING_CWD`
- 将 OpenClaw skill 复制到 `~/.kova/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作区指令文件复制到所选 kova 工作区
- 将兼容的工作区资产（如 `workspace/tts/`）镜像到 `~/.kova/tts/`
- 归档没有直接 kova 目标的非机密文档
...
1. 优先使用 `~/.kova/skills/migration/openclaw-migration/` 下的已安装路径。
...
使用 `--migrate-secrets` 时，还将导入一小组 kova 兼容的白名单 secret，目前包括：
...
Kova CLI 支持 `clarify` 工具进行交互式提示，但有以下限制：
...
13. 如果重命名的已导入 skill 的 `details.backup` 为空，不得暗示现有 kova skill 已被重命名或备份。仅说明已导入的副本被放置在新目标位置，并将 `details.renamed_from` 作为保持原位的已有文件夹引用。
...
2. 默认不迁移 secret。Token、认证 blob、设备凭据和原始 gateway 配置应保留在 kova 之外，除非用户明确要求迁移 secret。
3. 除非用户明确要求，否则不得静默覆盖非空的 kova 目标。辅助脚本在启用覆盖时会保留备份。
4. 始终向用户提供跳过项报告。该报告是迁移的一部分，而非可选附加内容。
5. 优先使用主 OpenClaw 工作区（`~/.openclaw/workspace/`）而非 `workspace.default/`。仅在主文件缺失时才使用默认工作区作为备用。
6. 即使在 secret 迁移模式下，也只迁移具有干净 kova 目标的 secret。不支持的认证 blob 仍须报告为已跳过。
...
- 已导入的 kova persona 状态
- 已填充转换后 OpenClaw 知识的 kova 记忆文件
- 在 `~/.kova/skills/openclaw-imports/` 下可用的 OpenClaw skill
