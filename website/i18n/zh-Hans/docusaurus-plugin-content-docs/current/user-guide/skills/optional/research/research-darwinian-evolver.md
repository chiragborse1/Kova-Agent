...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/research/darwinian-evolver` 安装 |
| 路径 | `optional-skills/research/darwinian-evolver` |
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), Kova Agent |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 kova 本身。
...
```bash
mkdir -p ~/.kova/cache/darwinian-evolver && cd ~/.kova/cache/darwinian-evolver
...
在浏览器中打开 `~/.kova/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.kova/skills/research/darwinian-evolver
DE_DIR=~/.kova/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 kova 核心中使用 `from darwinian_evolver import ...`。`~/.kova/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
