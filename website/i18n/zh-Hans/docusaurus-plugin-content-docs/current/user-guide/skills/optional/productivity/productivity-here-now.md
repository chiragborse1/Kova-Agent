...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/productivity/here-now` 安装 |
...
- Skill 辅助脚本路径：
  - `${KOVA_SKILL_DIR}/scripts/publish.sh` 用于发布站点
  - `${KOVA_SKILL_DIR}/scripts/drive.sh` 用于私有 Drive 存储

## 创建站点

```bash
PUBLISH="${KOVA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client kova
```
...
```bash
PUBLISH="${KOVA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client kova
```
...
```bash
DRIVE="${KOVA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}` | 用于归因的 agent 名称（如 `kova`） |
