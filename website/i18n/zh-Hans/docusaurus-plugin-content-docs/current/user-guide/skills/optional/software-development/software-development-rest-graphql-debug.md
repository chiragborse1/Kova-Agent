...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/software-development/rest-graphql-debug` 安装 |
...
通过 kova 工具驱动 REST 和 GraphQL 诊断 —— `terminal` 用于 `curl`，`execute_code` 用于 Python `requests`，`web_extract` 用于查阅厂商文档。在猜测修复方案之前，先隔离出故障层。
...
- 不要在脚本中硬编码 token。从环境变量（`os.environ["API_TOKEN"]`）或 `~/.kova/.env` 读取。
