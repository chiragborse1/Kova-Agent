...
| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/research/qmd` 安装 |
...
qmd 提供 MCP 服务器，可通过原生 MCP 客户端直接向 Kova Agent 提供搜索工具。这是推荐的集成方式 — 配置完成后，agent 无需每次加载此 skill 即可自动获得 qmd 工具。
