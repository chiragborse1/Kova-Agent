---
title: "Blender Mcp — 通过 socket 连接 blender-mcp 插件，直接从 kova 控制 Blender"
sidebar_label: "Blender Mcp"
description: "通过 socket 连接 blender-mcp 插件，直接从 kova 控制 Blender"
---
...
通过 socket 连接 blender-mcp 插件，直接从 kova 控制 Blender。可创建 3D 对象、材质、动画，并运行任意 Blender Python（bpy）代码。当用户需要在 Blender 中创建或修改任何内容时使用。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/creative/blender-mcp` 安装 |
...
通过 TCP 端口 9876 上的 socket，从 kova 控制正在运行的 Blender 实例。
