...
description: "配置并使用 Honcho 记忆功能与 kova -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。"
---
...
配置并使用 Honcho 记忆功能与 kova -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。适用于设置 Honcho、排查记忆问题、通过 Honcho peers 管理配置文件，或调整观察、召回和辩证设置。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `kova skills install official/autonomous-ai-agents/honcho` 安装 |
...
Honcho 提供 AI 原生的跨会话用户建模。它在多次对话中学习用户特征，并为每个 kova 配置文件提供独立的 peer 身份，同时共享统一的用户视图。
...
3. **AI peer 卡片** -- 此 kova 配置文件的 AI peer 身份卡片
...
Honcho 将对话建模为 **peers** 之间的交互。kova 每个会话创建两个 peers：

- **用户 peer**（`peerName`）：代表人类用户。Honcho 从观察到的消息中构建用户表示。
- **AI peer**（`aiPeer`）：代表此 kova 实例。每个配置文件拥有自己的 AI peer，使 agents 形成独立视角。
...
| `per-session` | 每次 kova 运行创建新的 Honcho 会话 |
| `global` | 跨所有目录使用单一会话 |

手动覆盖：`kova honcho map my-project-name`
...
每个 kova 配置文件拥有自己的 Honcho AI peer，同时共享同一工作区（用户上下文）。这意味着：
...
配置文件：`$KOVA_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
### 配置文件未获得自己的 peer
创建时使用 `--clone`：`kova profile create <name> --clone`。对于现有配置文件：`kova honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `kova honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `kova honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `kova honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `kova honcho disable` | 为当前配置文件禁用 Honcho |
| `kova honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `kova honcho peers` | 显示所有配置文件的 peer 身份 |
| `kova honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `kova honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `kova honcho sessions` | 列出已知的目录到会话名称映射 |
| `kova honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `kova honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `kova honcho sync` | 为所有尚未拥有 host 块的 kova 配置文件创建 host 块 |
| `kova honcho migrate` | 从 OpenClaw 原生记忆迁移到 kova + Honcho 的分步指南 |
| `kova memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `kova memory status` | 显示当前活跃的记忆提供商及配置 |
| `kova memory off` | 禁用外部记忆提供商 |