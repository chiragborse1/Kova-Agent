...
| | |
|---|---|
| 来源 | 可选 — 使用 `kova skills install official/devops/pinggy-tunnel` 安装 |
| 路径 | `optional-skills/devops/pinggy-tunnel` |
| 版本 | `0.1.0` |
| 作者 | Teknium (teknium1), Kova Agent |
| 许可证 | MIT |
...
远程客户端使用 `Authorization: Bearer $TOKEN` 连接到 `$URL`。kova 原生 MCP 客户端配置：`{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`。
