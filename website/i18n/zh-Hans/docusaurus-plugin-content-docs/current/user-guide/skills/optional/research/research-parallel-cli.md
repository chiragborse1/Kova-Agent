...
| | |
|---|---|
| 来源 | 可选 — 使用 `kova skills install official/research/parallel-cli` 安装 |
...
:::info
以下是 kova 在触发此技能时加载的完整技能定义。这是 agent 在技能激活时所看到的指令内容。
:::
...
这是一个可选的第三方工作流，不是 kova 的核心能力。

重要说明：
- Parallel 是付费服务，提供免费套餐，并非完全免费的本地工具。
- 它与 kova 原生的 `web_search` / `web_extract` 存在功能重叠，因此不要在普通查询中优先使用它。
...
在未明确要求 Parallel 的情况下进行快速单次查询时，优先使用 kova 原生的 `web_search` / `web_extract`。
...
7. 除非用户明确要求 Parallel 或需要 Parallel 专属工作流，否则优先使用 kova 原生工具。
...
- 当 kova 原生工具已经足够时，不要静默地选择 Parallel。
