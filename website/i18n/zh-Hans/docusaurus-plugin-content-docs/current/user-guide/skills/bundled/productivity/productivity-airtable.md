...
4. 将令牌存储在 `~/.kova/.env` 中（或通过 `kova setup` 配置）：
   ```
...
`-s` 会抑制 curl 的进度条——每次调用都保持此设置，以确保工具输出对 kova 保持整洁。通过 `python3 -m json.tool`（始终可用）或 `jq`（若已安装）管道输出以获得可读的 JSON。
...
- **`AIRTABLE_API_KEY` 会在此 skill 加载时自动从 `~/.kova/.env` 注入到子进程环境中**——每次 `curl` 调用前无需重新导出。
