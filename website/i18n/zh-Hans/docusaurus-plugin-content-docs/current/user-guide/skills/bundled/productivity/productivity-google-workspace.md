...
Gmail、Calendar、Drive、Contacts、Sheets 和 Docs —— 通过 kova 管理的 OAuth（开放授权）和轻量 CLI 封装器实现。若已安装 `gws`，该 skill 将以其作为执行后端以获得更广泛的 Google Workspace 覆盖；否则回退到内置的 Python 客户端实现。
...
- `scripts/google_api.py` —— 兼容性封装 CLI。在可用时优先使用 `gws` 执行操作，同时保留 kova 现有的 JSON 输出契约。
...
>
> Kova CLI 重要提示：若文件路径以 `/` 开头，请勿在 CLI 中单独发送该裸路径，因为它可能被误识别为斜杠命令。请将其放在句子中发送，例如：
...
若用户粘贴的是原始客户端 ID / 客户端密钥值而非文件路径，请自行为其编写一个有效的桌面 OAuth JSON 文件，保存到明确的位置（例如 `~/Downloads/kova-google-client-secret.json`），然后对该文件运行 `--client-secret`。
...
此命令返回包含 `auth_url` 字段的 JSON，并将该 URL 保存至 `~/.kova/google_oauth_last_url.txt`。
...
- Token 存储于 `~/.kova/google_token.json`，自动刷新。
- 待处理的 OAuth 会话状态/验证器临时存储于 `~/.kova/google_oauth_pending.json`，直至交换完成。
- 若已安装 `gws`，`google_api.py` 会将其指向同一个 `~/.kova/google_token.json` 凭据文件。用户无需单独运行 `gws auth login` 流程。
- 撤销授权：`$GSETUP --revoke`
