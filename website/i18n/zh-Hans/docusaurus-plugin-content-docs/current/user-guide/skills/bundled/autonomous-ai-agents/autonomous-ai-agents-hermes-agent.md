---
title: "Kova Agent — 配置、扩展或贡献 Kova Agent"
sidebar_label: "Kova Agent"
description: "配置、扩展或贡献 Kova Agent"
---
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/kova-agent` |
| 版本 | `2.1.0` |
| 作者 | Kova Agent + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `kova`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 kova 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Kova Agent

Kova Agent 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。kova 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

kova 的差异化特性：

- **通过 skill 自我提升** — kova 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 kova 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 kova 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。
...
kova auth                 交互式凭据管理器
kova auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
kova auth list            列出已存储的凭据
kova auth remove PROVIDER 移除已存储的凭据
...
使用 `kova config edit` 或 `kova config set section.key value` 编辑。
...
支持 20+ 个提供商。通过 `kova model` 或 `kova setup` 设置。
...
通过 `kova tools`（交互式）或 `kova tools enable/disable NAME` 启用/禁用。
...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_KOVA_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
常见的"为什么 kova 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `kova` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 kova 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
kova config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export KOVA_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `kova config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
kova config set privacy.redact_pii true    # 启用
kova config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: manual`），kova 在运行被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）之前会提示用户。模式如下：
...
```bash
kova config set approvals.mode smart       # 推荐的折中方案
kova config set approvals.mode off         # 绕过一切（不推荐）
```
...
要完全阻止模型访问网络或媒体工具，打开 `kova tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 kova 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `kova` 进程 |
|-|-----------------|--------------------------|
...
kova 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
- **fire-and-forget 使用 `kova chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`kova cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `kova kanban <verb>` 驱动；调度器生成的 worker 看到由 `KOVA_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
kova 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`kova config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 kova 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。kova 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pytest-xdist + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 kova 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `kova tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `kova doctor` — 检查配置和依赖
2. `kova auth` — 重新认证 OAuth 提供商（或 `kova auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `kova model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `kova skills list` — 验证已安装
2. `kova skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `kova -s name`
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed kova-gateway`
...
**2. 添加到 `toolsets.py`** → `_KOVA_CORE_TOOLS` 列表。
...
1. 在 `KOVA_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/KOVA_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
...
- 所有路径使用 `KOVA_constants` 中的 `get_KOVA_HOME()`（profile 安全）
