...
description: "通过 Kova CLI 操作 Teams 会议摘要流水线 — 总结会议、检查流水线状态、重放任务、管理 Microsoft Graph 订阅"
---
...
通过 Kova CLI 操作 Teams 会议摘要流水线 — 总结会议、检查流水线状态、重放任务、管理 Microsoft Graph 订阅。
...
所有面向运维人员的操作均通过终端工具执行 `kova teams-pipeline` 子命令完成。此流水线没有新的模型工具 — CLI 是唯一操作界面。
...
```bash
kova teams-pipeline validate              # 配置快照 — 每次变更后首先运行
kova teams-pipeline token-health          # Graph token 状态
kova teams-pipeline token-health --force-refresh   # 强制重新获取 token
kova teams-pipeline list                  # 近期会议任务
kova teams-pipeline list --status failed  # 仅显示失败任务
kova teams-pipeline show <job-id>         # 查看某个任务的完整详情
kova teams-pipeline subscriptions         # 当前 Graph webhook 订阅
```

### 重新运行 / 调试

```bash
kova teams-pipeline run <job-id>          # 重放已存储任务（重新生成摘要并重新投递）
kova teams-pipeline fetch --meeting-id <id>   # 试运行：解析会议及转录文本，不持久化
kova teams-pipeline fetch --join-web-url "<url>"   # 通过加入链接进行试运行
```
...
kova teams-pipeline maintain-subscriptions            # 续期即将到期的订阅
kova teams-pipeline maintain-subscriptions --dry-run  # 显示将被续期的内容
```
...
1. 执行 `kova teams-pipeline subscriptions` — 如果结果为空，或所有条目的 `expirationDateTime` 均已过期，即为原因所在。
2. 按上方示例使用 `subscribe` 重新创建订阅。
3. **立即设置自动续期**，可通过 `kova cron add`、systemd timer 或普通 crontab 实现。运维手册 `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` 提供了三种方案的完整说明。12 小时间隔是安全的（相对 72 小时上限有 6 倍余量）。
