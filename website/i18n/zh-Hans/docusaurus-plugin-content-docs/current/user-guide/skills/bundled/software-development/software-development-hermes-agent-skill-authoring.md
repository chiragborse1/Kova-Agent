---
title: "Kova Agent Skill 编写——在仓库中编写 SKILL"
sidebar_label: "Kova Agent Skill 编写"
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/software-development/kova-agent-skill-authoring` |
| 版本 | `1.0.0` |
| 作者 | Kova Agent |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `skills`, `authoring`, `kova-agent`, `conventions`, `skill-md` |
...
# 编写 kova-agent Skills（仓库内）

## 概述

SKILL.md 可以存放在两个位置：

1. **用户本地：** `~/.kova/skills/<maybe-category>/<name>/SKILL.md` — 个人使用，不共享。通过 `skill_manage(action='create')` 创建。
2. **仓库内（本 skill 讨论此情况）：** `/home/bb/kova-agent/skills/<category>/<name>/SKILL.md` — 已提交，随包一起发布。使用 `write_file` + `git add`。`skill_manage(action='create')` **不**针对此目录树。

## 使用时机

- 用户要求你"在此分支 / 仓库 / 提交中"添加一个 skill
- 你正在提交一个应随 kova-agent 一起发布的可复用工作流
- 你正在编辑 `/home/bb/kova-agent/skills/` 下的现有 skill（小改动用 `patch`，重写用 `write_file`；`skill_manage` 对仓库内 skill 的 `patch` 仍有效，但 `create` 无效）
...
`metadata.kova.related_skills` 在加载时会合并两个目录树（仓库内 `skills/` 和 `~/.kova/skills/`）。你**可以**从仓库内 skill 引用用户本地 skill，但对于全新克隆仓库的其他用户，该引用无法解析。仓库内 skill 优先只引用仓库内 skill。如果某个频繁被引用的 skill 仅存在于 `~/.kova/skills/`，请考虑将其提升到仓库中。
...
1. **对仓库内 skill 使用 `skill_manage(action='create')`。** 它会写入 `~/.kova/skills/`，而非仓库目录树。仓库内创建请使用 `write_file`。
...
- [ ] 文件位于 `skills/<category>/<name>/SKILL.md`（不在 `~/.kova/skills/` 中）
- [ ] Frontmatter 从字节 0 以 `---` 开头，以 `\n---\n` 结束
- [ ] `name`、`description`、`version`、`author`、`license`、`metadata.kova.{tags, related_skills}` 均已填写
- [ ] Name ≤ 64 个字符，小写加连字符
