...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.kova/.env ] && grep -q "^GITHUB_TOKEN=" ~/.kova/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.kova/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Kova Agent\",
    \"comments\": [
...
```bash
source "${KOVA_HOME:-$HOME/.kova}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Kova Agent. Code looks clean — good test coverage, no security concerns."
