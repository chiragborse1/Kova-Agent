...
使用 [OpenCode](https://opencode.ai) 作为由 kova 终端/进程工具编排的自主编码工作器。OpenCode 是一个支持多 provider、开源的 AI 编码 agent，具备 TUI（终端用户界面）和 CLI。
...
Shell 环境可能会解析到不同的 OpenCode 二进制文件。如果你的终端与 kova 的行为不一致，请检查：
