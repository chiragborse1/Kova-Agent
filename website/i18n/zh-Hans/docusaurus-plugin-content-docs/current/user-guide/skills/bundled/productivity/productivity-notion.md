...
将上述 export 添加到你的 shell 配置文件（或 `~/.kova/.env`），使每个会话都能继承这些变量。
...
- Notion 现已推出 MCP 服务器（`Notion MCP`，在数据库操作上比上一版本的 token 效率提升约 91%）——如需在会话中进行流式 Notion 访问，可通过 kova 的 MCP 支持接入，但上述路径已足以应对大多数一次性任务。