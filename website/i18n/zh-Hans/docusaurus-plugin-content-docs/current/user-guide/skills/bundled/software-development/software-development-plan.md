...
Plan 模式：将 Markdown 计划写入 .kova/plans/，不执行任何操作。
...
- 你的交付物是保存在活跃工作区 `.kova/plans/` 目录下的 Markdown 计划文件。
...
将该路径视为相对于活跃工作目录 / 后端工作区的路径。kova 文件工具具备后端感知能力，使用此相对路径可确保计划文件在 local、docker、ssh、modal 和 daytona 后端上均与工作区保持一致。

如果运行时提供了具体的目标路径，则使用该精确路径。
如果没有，则自行在 `.kova/plans/` 下创建一个合理的带时间戳的文件名。
