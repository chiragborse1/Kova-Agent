...
sidebar_position: 7
title: "电子邮件"
description: "通过 IMAP/SMTP 将 Kova Agent 设置为电子邮件助手"
---

# 电子邮件设置

kova 可以使用标准 IMAP 和 SMTP 协议接收并回复电子邮件。向 Agent 的邮箱地址发送邮件，它会在同一线程中回复——无需特殊客户端或 bot API。支持 Gmail、Outlook、Yahoo、Fastmail，以及任何支持 IMAP/SMTP 的邮件服务商。
...
- **为 Kova Agent 准备一个专用邮箱账户**（不要使用个人邮箱）
- **在该邮箱账户上启用 IMAP**
...
sudo Kova Gateway install --system   # 仅 Linux：开机自启的系统服务
```
...
| **重复回复** | 确保只有一个 gateway 实例在运行。检查 `Kova Gateway status`。 |
...
- 密码存储在 `~/.kova/.env` 中——请保护此文件（`chmod 600`）
