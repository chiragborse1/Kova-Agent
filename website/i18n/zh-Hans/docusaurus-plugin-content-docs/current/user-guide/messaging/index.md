...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 kova 对话 — 架构与配置概览"
---
...
```bash
Kova Gateway setup        # 交互式配置所有消息平台
```
...
```bash
Kova Gateway              # 在前台运行
Kova Gateway setup        # 交互式配置消息平台
Kova Gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo Kova Gateway install --system   # 仅 Linux：安装开机启动的系统服务
Kova Gateway start        # 启动默认服务
Kova Gateway stop         # 停止默认服务
Kova Gateway status       # 检查默认服务状态
Kova Gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.kova/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
kova pairing list          # 查看待审核和已批准的用户
kova pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，kova 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.kova/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.kova/config.yaml` 中的 `display.background_process_notifications` 控制：
...
:::info 多个安装
如果你在同一台机器上运行多个 kova 安装（使用不同的 `KOVA_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.kova` 使用 `kova-gateway`；其他安装使用 `kova-gateway-<hash>`。`Kova Gateway` 命令会自动针对当前 `KOVA_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
Kova Gateway install               # 安装为 launchd agent
Kova Gateway start                 # 启动服务
Kova Gateway stop                  # 停止服务
Kova Gateway status                # 检查状态
tail -f ~/.kova/logs/gateway.log   # 查看日志
```
...
- **KOVA_HOME** — 将网关限定到你的 kova 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `Kova Gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `KOVA_HOME` 目录都有自己的 launchd 标签。默认的 `~/.kova` 使用 `ai.kova.gateway`；其他安装使用 `ai.kova.gateway-<suffix>`。
:::
...
1. **网关日志**（`~/.kova/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
