...
sidebar_position: 5
title: "WhatsApp"
description: "通过内置 Baileys 桥接将 Kova Agent 设置为 WhatsApp 机器人"
---

# WhatsApp 配置

kova 通过基于 **Baileys** 的内置桥接连接到 WhatsApp。其工作原理是模拟 WhatsApp Web 会话——**而非**通过官方 WhatsApp Business API。无需 Meta 开发者账号或 Business 认证。
...
发生这种情况时，kova 会更新桥接依赖。如果机器人在 WhatsApp 更新后停止工作，
请拉取最新版 kova 并重新配对。
:::
...
2. 用新号码注册 WhatsApp
3. 运行 `kova whatsapp` 并从该 WhatsApp 账号扫描二维码
...
Baileys 桥接将会话保存在 `~/.kova/platforms/whatsapp/session` 目录下。这意味着：
...
- Agent 响应默认以"⚕ **Kova Agent**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
| **二维码过期** | 二维码约每 20 秒刷新一次。如果超时，重新运行 `kova whatsapp`。 |
| **会话未持久化** | 检查 `~/.kova/platforms/whatsapp/session` 是否存在且可写。如在容器中运行，请将其挂载为持久卷。 |
| **意外退出登录** | WhatsApp 会在长时间不活跃后取消关联设备。保持手机开机并连接网络，如有需要使用 `kova whatsapp` 重新配对。 |
| **桥接崩溃或重连循环** | 重启 gateway，更新 Kova，如会话因 WhatsApp 协议变更而失效则重新配对。 |
| **WhatsApp 更新后机器人停止工作** | 更新 kova 以获取最新桥接版本，然后重新配对。 |
| **macOS："Node.js not installed"但终端中 node 可用** | launchd 服务不继承你的 shell PATH。运行 `Kova Gateway install` 将当前 PATH 重新快照到 plist 中，然后运行 `Kova Gateway start`。详见 [Gateway 服务文档](./index.md#macos-launchd)。 |
...
| **机器人向陌生人回复配对码** | 如需对未授权私信静默处理，在 `~/.kova/config.yaml` 中设置 `whatsapp.unauthorized_dm_behavior: ignore`。 |
...
- `~/.kova/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.kova/platforms/whatsapp/session`
