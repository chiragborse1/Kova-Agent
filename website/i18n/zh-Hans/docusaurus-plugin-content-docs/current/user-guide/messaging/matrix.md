...
sidebar_position: 9
title: "Matrix"
description: "将 Kova Agent 设置为 Matrix 机器人"
---

# Matrix 设置

Kova Agent 与 Matrix 集成，Matrix 是一种开放的联邦消息协议。Matrix 允许你运行自己的 homeserver，也可以使用 matrix.org 等公共 homeserver——无论哪种方式，你都保持对通信的控制权。机器人通过 `mautrix` Python SDK 连接，通过 Kova Agent 管道（包括工具调用、记忆和推理）处理消息，并实时响应。它支持文本、文件附件、图片、音频、视频，以及可选的端对端加密（E2EE）。

kova 兼容任何 Matrix homeserver——Synapse、Conduit、Dendrite 或 matrix.org。

在开始设置之前，先了解大多数人最想知道的：kova 连接后的行为方式。

## kova 的行为方式

| 场景 | 行为 |
|---------|----------|
| **私聊（DM）** | kova 响应每条消息，无需 `@提及`。每个 DM 有独立的会话。设置 `MATRIX_DM_MENTION_THREADS=true` 可在 DM 中被 `@提及` 时创建线程。 |
| **房间** | 默认情况下，kova 需要 `@提及` 才会响应。设置 `MATRIX_REQUIRE_MENTION=false` 或将房间 ID 添加到 `MATRIX_FREE_RESPONSE_ROOMS` 可开启自由响应模式。房间邀请会被自动接受。 |
| **线程** | kova 支持 Matrix 线程（MSC3440）。在线程中回复时，kova 会将线程上下文与主房间时间线隔离。机器人已参与的线程无需提及即可响应。 |
| **自动线程** | 默认情况下，kova 会为其在房间中响应的每条消息自动创建线程，以保持对话隔离。设置 `MATRIX_AUTO_THREAD=false` 可禁用此功能。设置 `MATRIX_DM_AUTO_THREAD=true`（默认 false）可同时为私聊消息自动创建线程——这与 `MATRIX_DM_MENTION_THREADS` 不同，后者仅在私聊中 @提及 Bot 时才创建线程。 |
| **多用户共享房间** | 默认情况下，kova 在房间内按用户隔离会话历史。同一房间中的两个人不会共享同一对话记录，除非你明确禁用该功能。 |
...
你也可以以自己的用户身份运行 Kova。这意味着机器人以你的名义发帖——适合个人助手场景。

## 第二步：获取访问令牌

kova 需要访问令牌（access token）来向 homeserver 进行身份验证。有两种方式：
...
Kova Agent 使用你的 Matrix 用户 ID 来控制谁可以与机器人交互。Matrix 用户 ID 的格式为 `@username:server`。
...
kova 支持 Matrix 端对端加密，你可以在加密房间中与机器人聊天。
...
- 将加密密钥存储在 `~/.kova/platforms/matrix/store/`（旧版安装：`~/.kova/matrix/store/`）
- 在首次连接时上传设备密钥
- 自动解密传入消息并加密传出消息
- 被邀请时自动加入加密房间
...
每次启动时，如果设置了 `MATRIX_RECOVERY_KEY`，kova 会从 homeserver 的安全密钥存储中导入交叉签名密钥并对当前设备进行签名。此操作是幂等的，可以永久启用。

:::warning[删除加密存储]
如果你删除了 `~/.kova/platforms/matrix/store/crypto.db`，机器人将失去其加密身份。仅使用相同的设备 ID 重启**不能**完全恢复——homeserver 仍持有使用旧身份密钥签名的一次性密钥，对等方无法建立新的 Olm 会话。

kova 在启动时会检测到此情况并拒绝启用 E2EE，日志显示：`device XXXX has stale one-time keys on the server signed with a previous identity key`。
...
**原因**：主机系统时钟超前于实际时间。Matrix 适配器应用了 5 秒启动宽限过滤器（`event_ts < startup_ts - 5`）以忽略初始同步中重放的事件。当系统时钟超前时，每个传入事件看起来都"早于启动时间"，在到达消息处理器之前就被丢弃——机器人看起来已连接但从不回复。参见 [#12614](https://github.com/NousResearch/Kova-agent/issues/12614)。
...
如果你之前使用 `MATRIX_ENCRYPTION=true` 运行 Kova，并正在升级到使用新的基于 SQLite 的加密存储的版本，机器人的加密身份已发生变化。你的 Matrix 客户端（Element）可能缓存了旧的设备密钥，并拒绝与机器人共享加密会话。
...
1. **生成新的访问令牌**以获得全新的设备 ID。最简单的方式：
   ...
   复制新的 `access_token` 并更新 `~/.kova/.env` 中的 `MATRIX_ACCESS_TOKEN`。
...
3. **设置恢复密钥**（如果你使用交叉签名——大多数 Element 用户都使用）。在 `~/.kova/.env` 中添加：
...
:::tip
**新安装不受影响。** 此迁移仅在你之前使用旧版 kova 配置了可用的 E2EE 并正在升级时才需要。
...
| 设置 | 说明 |
|---------|-------------|
| `GATEWAY_PROXY_URL` | 远程 kova API 服务器的 URL（例如 `http://192.168.1.100:8642`） |
...
**解决方法**：同步循环在出错时每 5 秒自动重试。检查 kova 日志中与同步相关的警告。如果机器人持续落后，请确保你的 homeserver 有足够的资源。

### 机器人离线

**原因**：Kova Gateway 未运行，或连接失败。

**解决方法**：检查 `Kova Gateway` 是否正在运行。查看终端输出中的错误消息。常见问题：homeserver URL 错误、访问令牌过期、homeserver 不可达。

### "用户不被允许"/机器人忽略你

**原因**：你的用户 ID 不在 `MATRIX_ALLOWED_USERS` 中。

**解决方法**：将你的用户 ID 添加到 `~/.kova/.env` 中的 `MATRIX_ALLOWED_USERS` 并重启 gateway。使用完整的 `@user:server` 格式。
...
- **媒体支持**：kova 可以发送和接收图片、音频、视频和文件附件。媒体通过 Matrix 内容仓库 API 上传到你的 homeserver。
