...
通过 [BlueBubbles](https://bluebubbles.app/) 将 kova 连接至 Apple iMessage——这是一款免费、开源的 macOS 服务端，可将 iMessage 桥接至任意设备。
...
- kova 与 BlueBubbles 服务端之间的网络连通性
...
**DM 配对（推荐）：**
当有人向你的 iMessage 发送消息时，kova 会自动向其发送配对码。使用以下命令批准：
```bash
kova pairing approve bluebubbles <CODE>
```
使用 `kova pairing list` 查看待处理的配对码和已授权用户。

**预授权特定用户**（在 `~/.kova/.env` 中）：
```bash
...
kova 将连接至你的 BlueBubbles 服务端，注册 webhook，并开始监听 iMessage 消息。
...
- **出站：** kova 通过 BlueBubbles REST API 发送消息。
...
自动将消息标记为已读由 `~/.kova/config.yaml` 中 `platforms.bluebubbles.extra` 下的 `send_read_receipts` 键控制（默认值：`true`）。该选项没有对应的环境变量。
...
### 聊天寻址
你可以通过邮箱或手机号寻址聊天——kova 会自动将其解析为 BlueBubbles 聊天 GUID，无需使用原始 GUID 格式。
...
- 查看 `kova logs gateway` 中的 webhook 错误（或使用 `kova logs -f` 实时跟踪）
