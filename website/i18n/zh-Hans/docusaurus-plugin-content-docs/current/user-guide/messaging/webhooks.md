...
sidebar_position: 13
title: "Webhooks"
description: "接收来自 GitHub、GitLab 等服务的事件以触发 Kova Agent 运行"
---

# Webhooks

接收来自外部服务（GitHub、GitLab、JIRA、Stripe 等）的事件，并自动触发 Kova Agent 运行。Webhook 适配器运行一个 HTTP 服务器，接受 POST 请求、验证 HMAC 签名、将 payload（载荷）转换为 agent prompt（提示词），并将响应路由回来源或其他已配置的平台。
...
1. 通过 `Kova Gateway setup` 或环境变量启用
2. 在 `config.yaml` 中定义路由，**或**使用 `kova webhook subscribe` 动态创建
...
按照上方示例，将 `github-pr` 路由添加到 `~/.kova/config.yaml`。
...
在仓库中打开一个 pull request。webhook 触发后，kova 处理事件并在 PR 上发布审查评论。
...
除了 `config.yaml` 中的静态路由，还可以使用 `kova webhook` CLI 命令动态创建 webhook 订阅。当 agent 本身需要设置事件驱动触发器时，这尤为有用。
...
- 订阅存储在 `~/.kova/webhook_subscriptions.json`
...
agent 可通过 terminal 工具在 `webhook-subscriptions` skill 的引导下创建订阅。向 agent 请求"为 GitHub issues 设置 webhook"，它将运行相应的 `kova webhook subscribe` 命令。
...
- 在前台运行 gateway 以查看日志：`Kova Gateway run`
- 检查 prompt 模板是否正确渲染
- 验证投递目标已配置并连接
