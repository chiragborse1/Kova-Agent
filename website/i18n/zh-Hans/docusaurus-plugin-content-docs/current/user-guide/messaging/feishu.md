...
sidebar_position: 11
title: "飞书 / Lark"
description: "将 Kova Agent 配置为飞书或 Lark 机器人"
---

# 飞书 / Lark 配置

Kova Agent 可作为全功能机器人与飞书和 Lark 集成。连接后，你可以在私信或群聊中与 Agent 对话，在 home chat 中接收 cron job 结果，并通过标准 gateway 流程发送文本、图片、音频和文件附件。

该集成支持两种连接模式：

- `websocket` — 推荐；kova 主动建立出站连接，无需公开 webhook 端点
- `webhook` — 适用于已将 kova 部署在可访问 HTTP 端点后的场景
...
选择 **飞书 / Lark**，用飞书或 Lark 手机端扫描二维码。kova 将自动创建具有正确权限的机器人应用并保存凭据。
...
4. 为应用开启 **机器人** 能力。
5. 运行 `Kova Gateway setup`，选择 **飞书 / Lark**，并在提示时输入凭据。
...
当 kova 运行在你的笔记本、工作站或私有服务器上时，使用 WebSocket 模式。无需公开 URL。官方 Lark SDK 会建立并维护一个持久的出站 WebSocket 连接，并支持自动重连。
...
仅当 kova 已部署在可访问的 HTTP 端点后时，才使用 webhook 模式。

```bash
FEISHU_CONNECTION_MODE=webhook
```

在 webhook 模式下，kova 启动一个 HTTP 服务器（通过 `aiohttp`），并在以下路径提供飞书端点：
...
`FEISHU_GROUP_POLICY` 环境变量控制 kova 是否以及如何在群聊中响应：
...
| 值 | 行为 |
|-------|----------|
| `open` | kova 响应任意群中任意用户的 @提及。 |
| `allowlist` | kova 仅响应 `FEISHU_ALLOWED_USERS` 中列出的用户的 @提及。 |
| `disabled` | kova 完全忽略所有群消息。 |

在所有模式下，消息处理前机器人必须被明确 @提及（或 @all）。私信始终绕过此限制。

设置 `FEISHU_REQUIRE_MENTION=false` 可让 kova 读取所有群消息而无需 @提及：
...
kova 在启动时自动检测机器人的 `open_id` 和显示名称。仅当自动检测无法访问飞书 API，或你的应用使用租户范围用户 ID 时，才需要手动设置：
...
默认情况下，kova 忽略其他机器人发送的消息。当你希望 kova 参与 A2A 编排或接收同一群中其他机器人的通知时，可启用机器人间消息传递。
...
Gateway 驱动的更新提示使用原生飞书 `Yes` / `No` 卡片，而非回退到纯文本回复。当 `kova update --gateway` 需要确认时，适配器将所选答案记录到 kova 的 `.update_response` 文件中，并将卡片内联替换为已解决状态。
...
除聊天外，适配器还可以回复**飞书/Lark 文档**中的 `@` 提及。当用户在文档中评论（局部文本选区或全文评论）并 @提及机器人时，kova 读取文档内容及周围的评论线程，并在线程中内联发布 LLM 回复。
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 静默期 | `KOVA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| 每批最大消息数 | `KOVA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| 每批最大字符数 | `KOVA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 静默期 | `KOVA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 缓存大小 | `KOVA_FEISHU_DEDUP_CACHE_SIZE` | 2048 条 |
...
| `KOVA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | 最大去重消息 ID 追踪数量 |
| `KOVA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | 文本突发防抖静默期 |
| `KOVA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | 每批文本合并的最大消息数 |
| `KOVA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | 每批文本合并的最大字符数 |
| `KOVA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | 媒体突发防抖静默期 |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | 设置两个环境变量，或通过 `Kova Gateway setup` 配置 |
| `Another local Kova Gateway is already using this Feishu app_id` | 同一时间只能有一个 kova 实例使用相同的 app_id。请先停止另一个 gateway。 |
...
| 启用 `FEISHU_ALLOW_BOTS` 后对端机器人消息仍被忽略 | kova 尚无法识别自身——请设置 `FEISHU_BOT_OPEN_ID`（若应用使用 `sender_id_type=user_id` 则同时设置 `FEISHU_BOT_USER_ID`）。 |
...
飞书 / Lark 使用 `kova-feishu` 平台预设，包含与 Telegram 及其他基于 gateway 的消息平台相同的核心工具。