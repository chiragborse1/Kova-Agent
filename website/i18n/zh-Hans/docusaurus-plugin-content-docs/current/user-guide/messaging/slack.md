...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 Kova Agent 设置为 Slack 机器人"
---

# Slack 设置

使用 Socket Mode 将 Kova Agent 作为机器人连接到 Slack。Socket Mode 使用 WebSocket 而非公开 HTTP 端点，因此你的 kova 实例无需公开访问——它可以在防火墙后、笔记本电脑上或私有服务器上正常运行。

:::warning 经典 Slack 应用已弃用
使用 RTM API 的经典 Slack 应用已于 **2025 年 3 月完全弃用**。kova 使用带有 Socket Mode 的现代 Bolt SDK。如果你有旧的经典应用，必须按照以下步骤创建新应用。
:::
...
最快的方式是粘贴 kova 为你生成的 manifest（清单文件）。它会一次性声明所有内置斜杠命令（`/btw`、`/stop`、`/model`……）、所有必需的 OAuth 权限范围、所有事件订阅，并启用 Socket Mode。

### 方式 A：使用 kova 生成的 manifest（推荐）

1. 生成 manifest：
   ```bash
   kova slack manifest --write
   ```
   此命令会将 `~/.kova/slack-manifest.json` 写入磁盘并打印粘贴说明。
...
2. 点击 **Create New App**
3. 选择 **From scratch**
4. 输入应用名称（例如 "Kova Agent"）并选择你的工作区
5. 点击 **Create App**
...
:::caution 缺少权限范围 = 功能缺失
没有 `channels:history` 和 `groups:history`，机器人**将无法接收频道消息**——它只能在私信中工作。没有 `files:read`，kova 可以聊天，但**无法可靠读取用户上传的附件**。这是最常被遗漏的权限范围。
:::
...
即使拥有所有正确的权限范围和事件订阅，除非启用 Messages Tab，否则 Slack 不允许用户向机器人发送私信。这是 Slack 平台的要求，而非 kova 的配置问题。
:::
...
kova 使用 Slack **Member ID**（而非用户名或显示名称）作为白名单。
...
```bash
Kova Gateway setup    # 提示时选择 Slack
```
...
```bash
Kova Gateway              # 前台运行
Kova Gateway install      # 安装为用户服务
sudo Kova Gateway install --system   # 仅 Linux：开机启动系统服务
```
...
底层实现：kova 附带一个生成的 Slack 应用 manifest（见第一步，方式 A），它将 [`COMMAND_REGISTRY`](https://github.com/NousResearch/Kova-agent/blob/main/KOVA_cli/commands.py) 中的每个命令声明为斜杠命令。在 Socket Mode 下，无论 manifest 的 `url` 字段如何，Slack 都会通过 WebSocket 路由命令事件。

### 更新后刷新斜杠命令

当 kova 添加新命令时（例如执行 `kova update` 后），重新生成 manifest 并更新你的 Slack 应用：
...
3. 粘贴 `~/.kova/slack-manifest.json` 的新内容
...
Slack 本身会阻止在话题回复中使用原生斜杠命令——在话题中尝试 `/queue`，Slack 会回复 *"/queue is not supported in threads. Sorry!"*。没有任何应用端设置可以重新启用它们；Slack 从不将它们传递给 Kova。

作为解决方案，kova 识别前导 `!` 作为在话题（以及任何其他地方）中有效的替代命令前缀。在话题回复中输入 `!queue`、`!stop`、`!model gpt-5.4` 等普通回复——kova 会以与斜杠形式完全相同的方式处理，并在同一话题中回复。
...
|---------|----------|
| **私信** | 机器人响应每条消息——无需 @ 提及 |
| **频道** | 机器人**仅在被 @ 提及时响应**（例如 `@Kova Agent what time is it?`）。在频道中，kova 在该消息附带的话题中回复。 |
| **话题** | 如果你在现有话题中 @ 提及 Kova，它会在同一话题中回复。一旦机器人在话题中有活跃会话，**该话题中的后续回复无需 @ 提及**——机器人会自然跟进对话。 |
...
除了第八步中的必需环境变量外，你还可以通过 `~/.kova/config.yaml` 自定义 Slack 机器人行为。
...
为 `true`（默认值）时，共享频道中的每个用户都有自己独立的对话会话。在 `#general` 中与 kova 对话的两个人将有各自独立的历史记录和上下文。
...
将 `SLACK_HOME_CHANNEL` 设置为频道 ID，kova 将在此频道发送计划消息、定时任务结果和其他主动通知。查找频道 ID 的方法：
...
确保机器人已被**邀请到该频道**（`/invite @Kova Agent`）。

---

## 多工作区支持

kova 可以使用单个 gateway 实例**同时连接多个 Slack 工作区**。每个工作区使用其自己的机器人用户 ID 独立认证。
...
除了环境变量或配置中的 token 外，kova 还会从以下位置的 **OAuth token 文件**加载 token：
...
| 机器人在私信中正常但在频道中不响应 | **最常见问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并用 `/invite @Kova Agent` 邀请机器人加入频道 |
...
| 机器人响应但无法在频道中发帖 | 用 `/invite @Kova Agent` 邀请机器人加入频道 |
| 机器人可以聊天但无法读取上传的图片/文件 | 添加 `files:read`，然后**重新安装**应用。当 Slack 返回权限范围/认证/权限失败时，kova 现在会在聊天中显示附件访问诊断信息。 |
...
7. ✅ 已**邀请**机器人加入频道（`/invite @Kova Agent`）
...
- Token 应存储在 `~/.kova/.env` 中（文件权限 `600`）
