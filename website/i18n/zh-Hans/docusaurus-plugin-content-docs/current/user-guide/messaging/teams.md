...
description: "将 Kova Agent 设置为 Microsoft Teams 机器人"
---

# Microsoft Teams 设置

将 Kova Agent 作为机器人接入 Microsoft Teams。与 Slack 的 Socket Mode 不同，Teams 通过调用**公开 HTTPS webhook**（钩子）来投递消息，因此你的实例需要一个可公开访问的端点——本地开发时使用开发隧道，生产环境使用真实域名。
...
Teams 将 @提及作为普通消息投递，其中包含 `<at>BotName</at>` 标签，kova 在处理前会自动去除这些标签。
...
| `No inference provider configured` | 检查 `~/.kova/.env` 中是否设置了 `ANTHROPIC_API_KEY`（或其他提供商密钥） |
...
- 将凭据存储在权限为 `600` 的 `~/.kova/.env` 中（`chmod 600 ~/.kova/.env`）
