...
sidebar_position: 12
title: "Google Chat"
description: "使用 Cloud Pub/Sub 将 Kova Agent 设置为 Google Chat 机器人"
---

# Google Chat 设置

将 Kova Agent 作为机器人接入 Google Chat。该集成使用 Cloud Pub/Sub 拉取订阅接收入站事件，使用 Chat REST API 发送出站消息。与 Slack Socket Mode 或 Telegram 长轮询的使用体验相当：kova 进程无需公网 URL、隧道或 TLS 证书。它直接连接、认证并监听订阅——就像 Telegram 机器人通过 token 监听一样。
...
创建完成后，打开该 SA，进入 **Keys → Add Key → Create new key → JSON**，下载文件。将其保存到只有 kova 可读的位置（例如 `~/.kova/google-chat-sa.json`，`chmod 600`）。
...
在浏览器中打开 Google Chat。在 **+ New Chat** 菜单中搜索应用名称，向其发起私信。第一次发消息时，Google 会发送一个 `ADDED_TO_SPACE` 事件，kova 用它来缓存机器人自身的 `users/{id}`，以便过滤自发消息。

---

## 第九步：配置 kova

在 `~/.kova/.env` 中添加 Google Chat 配置段：

```bash
# 必填
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/kova-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.kova/google-chat-sa.json
...
安装 Google Chat 适配器所需的依赖（目前没有发布 kova extra，请直接安装）：
...
在测试私信中发送"hola"。机器人会先发送一条"kova is thinking…"占位消息，然后原地编辑该消息为真实回复——不会留下"消息已删除"的墓碑。
...
Thread（线程）支持：当用户在 thread 中回复时，kova 会检测 `thread.name` 并在同一 thread 中发送回复，每个 thread 对应独立的 kova 会话。
...
3. 下载 JSON 文件，移动到运行 kova 的宿主机上。
4. 在宿主机上，向 kova 注册该客户端：
...
该命令会写入 `~/.kova/google_chat_user_client_secret.json`。这是共享基础设施——它标识 OAuth *应用*，而非某个具体用户。无论后续有多少用户授权，每台宿主机只需一个文件。
...
token 保存在 `~/.kova/google_chat_user_tokens/<sanitized_email>.json`。该用户私信中后续的文件请求将使用*其*token，机器人以其身份上传，消息投递到其 space。
...
当请求者尚无每用户 token 时，机器人会回退到 `~/.kova/google_chat_user_token.json` 中的旧版单用户 token（如果存在于多用户支持之前的安装中）。两者均不可用时，机器人会发送清晰的文字提示，告知请求者运行 `/setup-files`。
...
1. 在控制台检查 Pub/Sub 订阅是否有未投递消息。如果有，说明 kova 未通过认证——验证 `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`，并确认 SA 在订阅上具有 `Pub/Sub Subscriber` 角色。
...
3. 检查 `Kova Gateway` 日志中是否有 `[GoogleChat] Connected`。如果看到 `[GoogleChat] Config validation failed`，错误信息会告诉你需要修复哪个环境变量。
...
一次性宿主机设置未完成。在运行 kova 的宿主机终端中执行：
...
- **附件下载保护**：kova 只会将 SA bearer token 附加到主机名匹配 Google 自有域名短名单的 URL（`googleapis.com`、`drive.google.com`、`lh[3-6].googleusercontent.com` 等）。其他主机在发起 HTTP 请求前即被拒绝，以防范 SSRF 场景——即精心构造的事件将 bearer token 重定向到 GCE 元数据服务。
...
- **用户 OAuth scope**：每用户附件流程*仅*请求 `chat.messages.create`——覆盖 `media.upload` 及后续 `messages.create` 所需的最小权限。token 以明文 JSON 形式持久化在 `~/.kova/google_chat_user_tokens/<sanitized_email>.json`（文件系统权限是保护手段——与 SA 密钥文件采用相同模型）。每个 token 归属于唯一一位用户；撤销操作仅限于该用户。