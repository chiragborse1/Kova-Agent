...
sidebar_position: 10
title: "DingTalk"
description: "将 Kova Agent 设置为钉钉聊天机器人"
---

# 钉钉设置

Kova Agent 可作为聊天机器人集成到钉钉（DingTalk），让你通过单聊或群聊与 AI 助手对话。机器人通过钉钉的 Stream Mode（流模式）连接——一种长连接 WebSocket，无需公网 URL 或 webhook 服务器——并通过钉钉的 session webhook API 以 markdown 格式回复消息。

在开始设置之前，先了解大多数人最关心的内容：kova 进入你的钉钉工作空间后的行为方式。

## kova 的行为方式

| 场景 | 行为 |
|---------|----------|
| **单聊（1:1 对话）** | kova 响应每条消息，无需 `@提及`，每个单聊有独立会话。 |
| **群聊** | kova 仅在被 `@提及` 时响应，未被提及则忽略消息。 |
| **多用户共享群聊** | 默认情况下，kova 在群内按用户隔离会话历史。同一群中的两个用户不共享同一对话记录，除非你明确禁用该功能。 |
...
Kova Agent 使用你的钉钉用户 ID 来控制谁可以与机器人交互。钉钉用户 ID 是由组织管理员设置的字母数字字符串。
...
- **二维码设备流（推荐）。** 用钉钉手机 App 扫描终端中打印的二维码——Client ID 和 Client Secret 将自动返回并写入 `~/.kova/.env`，无需前往开发者控制台。
- **手动粘贴。** 如果你已有凭证（或扫码不方便），在提示时粘贴你的 Client ID、Client Secret 和允许的用户 ID。

:::note openClaw 品牌披露
由于钉钉的 `verification_uri_complete` 在 API 层硬编码为 openClaw 身份，在 Alibaba / DingTalk-Real-AI 在服务端注册 kova 专属模板之前，二维码目前以 `openClaw` 来源字符串进行授权。这仅是钉钉呈现授权界面的方式——你创建的机器人完全属于你，且对你的租户私有。
:::

### 方式 B：手动配置

在 `~/.kova/.env` 文件中添加以下内容：
...
:::tip
你可以在后台运行 `Kova Gateway`，或将其配置为 systemd 服务以持续运行。详见部署文档。
:::
...
kova 可以使用钉钉 AI 卡片代替纯 markdown 消息进行回复。卡片提供更丰富、更结构化的展示，并支持在 agent 生成响应时进行流式更新。
...
kova 会自动在你的消息上添加 emoji 反应以显示处理状态：
...
**解决方法**：确认 `DINGTALK_CLIENT_ID` 和 `DINGTALK_CLIENT_SECRET` 已在 `~/.kova/.env` 中正确设置。Client ID 是你的 AppKey，Client Secret 是钉钉开发者控制台中的 AppSecret。
...
**解决方法**：检查 `Kova Gateway` 是否正在运行。查看终端输出中的错误信息。常见问题：凭证错误、应用被停用、`dingtalk-stream` 或 `httpx` 未安装。
