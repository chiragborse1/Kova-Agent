...
sidebar_position: 17
title: "LINE"
description: "将 Kova Agent 设置为 LINE Messaging API 机器人"
---

# LINE 配置

通过官方 LINE Messaging API 将 Kova Agent 作为 [LINE](https://line.me/) 机器人运行。适配器以捆绑平台插件的形式存放于 `plugins/platforms/line/` — 无需修改核心代码，像其他平台一样启用即可。
...
**机器人在群组中收不到消息。** 检查 `LINE_ALLOWED_GROUPS` 是否包含对应的 `C...` 群组 ID。如需查找群组 ID，发送一条测试消息后在 `~/.kova/logs/gateway.log` 中搜索 `LINE: rejecting unauthorized source` — 被拒绝的 source 字典中包含相关 ID。
...
**"already in use by another profile"。** 同一个频道访问 token 已被另一个运行中的 kova profile 占用。请停止另一个 gateway，或使用独立的频道。
