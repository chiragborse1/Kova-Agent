...
description: 通过 Home Assistant 集成，使用 Kova Agent 控制您的智能家居。
...
Kova Agent 通过以下两种方式与 [Home Assistant](https://www.home-assistant.io/) 集成：
...
在 `~/.kova/config.yaml` 中，于 Home Assistant 平台的 `extra` 部分配置 agent 接收的事件：
...
Agent 发出的消息将以 **Home Assistant 持久通知**的形式推送（通过 `persistent_notification.create`），标题为"Kova Agent"，显示在 HA 通知面板中。
