...
在同一台机器上运行多个独立的 Kova Agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。

## 什么是 profile？

profile 是一个独立的 kova 主目录。每个 profile 拥有自己的目录，其中包含各自的 `config.yaml`、`.env`、`SOUL.md`、记忆、会话、技能、cron 任务和状态数据库。profile 让你可以为不同用途运行独立的 agent——编程助手、个人机器人、研究 agent——而不会混淆 kova 状态。
...
```bash
kova profile create coder       # 创建 profile + "coder" 命令别名
coder setup                       # 配置 API 密钥和模型
coder chat                        # 开始对话
```

就这些。`coder` 现在是拥有独立配置、记忆和状态的 kova profile。
...
你也可以稍后通过 `kova profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
将当前 profile 的 `config.yaml`、`.env`、`SOUL.md` 和 skills 复制到新 profile。API 密钥、模型和能力相同，但会话和记忆是全新的。编辑 `~/.kova/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.kova/profiles/work/SOUL.md` 可设置不同的人格。
...
复制**所有内容**——配置、API 密钥、人格、记忆、技能、cron 任务、插件。会排除每个 profile 自己的历史数据（会话历史、`state.db`、`backups/`、`state-snapshots/`、`checkpoints/`），这些数据属于源 profile 且可能达到数十 GB。若要包含历史的完整备份，请使用 `kova profile export` 或 `kova backup`。
...
别名支持所有 kova 子命令——底层实际上是 `kova -p <name>`。
...
kova chat -p coder -q "hello"    # 可在任意位置使用
```

### 粘性默认值（`kova profile use`）

```bash
kova profile use coder
kova chat                   # 现在指向 coder
kova tools                  # 配置 coder 的工具
kova profile use default    # 切换回默认
```

设置默认值后，普通 `kova` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`kova profile`**：显示当前 profile 名称、路径、模型、gateway 状态

## Profile vs 工作区 vs 沙箱

profile 常与工作区或沙箱混淆，但它们是不同的概念：

- **profile** 为 kova 提供独立的状态目录：`config.yaml`、`.env`、`SOUL.md`、会话、记忆、日志、cron 任务和 gateway 状态。
...
在 local 后端使用 `cwd: "."` 表示"kova 启动时所在的目录"，而非"profile 目录"。
...
:::note 在官方 Docker 镜像中
各 profile 的 gateway 由 [s6-overlay](https://github.com/just-containers/s6-overlay)（容器中的 PID 1）监管，因此 `kova profile create <name>` 会自动在 `/run/service/gateway-<name>/` 注册 s6 服务槽。`kova -p <name> gateway start/stop/restart` 会调度到 `s6-svc` 而非直接启动裸进程——崩溃后自动重启，`docker restart` 会保留之前运行的 gateway 集合。详见 [各 profile gateway 监管](/user-guide/docker#per-profile-gateway-supervision)。
:::
...
`kova update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
kova profile list           # 显示所有 profile 及其状态
kova profile show coder     # 显示某个 profile 的详细信息
kova profile rename coder dev-bot   # 重命名（同步更新别名和服务）
kova profile export coder   # 导出为 coder.tar.gz
kova profile import coder.tar.gz   # 从归档文件导入
```
...
:::note
你无法删除默认 profile（`~/.kova`）。如需删除所有内容，请使用 `kova uninstall`。
:::
...
profile 使用 `KOVA_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 kova 前将 `KOVA_HOME` 设置为 `~/.kova/profiles/coder`。由于代码库中 119+ 个文件通过 `get_KOVA_HOME()` 解析路径，kova 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。

这与终端工作目录是分开的。工具执行从 `terminal.cwd` 开始（或在 local 后端使用 `cwd: "."` 时从启动目录开始），而非自动从 `KOVA_HOME` 开始。

默认 profile 就是 `~/.kova` 本身。无需迁移——现有安装的工作方式完全不变。
