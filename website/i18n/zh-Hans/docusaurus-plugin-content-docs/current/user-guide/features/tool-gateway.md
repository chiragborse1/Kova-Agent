...
运行 `kova model` 并选择 Nous Portal 作为提供商时，kova 会主动询问是否启用 Tool Gateway：
...
在 `~/.kova/config.yaml` 中直接设置 `use_gateway`：
...
网关使用 Nous Portal 凭据认证（在 `kova model` 完成后写入 `~/.kova/auth.json`）。
...
在 `kova tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.kova/.env` 中用环境变量覆盖端点：
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.nousresearch.com/manage-subscription) 或通过 `kova tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `kova setup terminal` 或在 `config.yaml` 中配置。
