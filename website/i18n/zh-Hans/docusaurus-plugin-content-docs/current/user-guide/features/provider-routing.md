...
使用 [OpenRouter](https://openrouter.ai) 作为 LLM provider 时，Kova Agent 支持 **provider routing**（提供商路由）——对哪些底层 AI provider 处理你的请求以及如何排列优先级进行精细控制。
...
在 `~/.kova/config.yaml` 中添加 `provider_routing` 部分：
...
- **CLI 模式** — 在 `~/.kova/config.yaml` 中配置，启动时加载
