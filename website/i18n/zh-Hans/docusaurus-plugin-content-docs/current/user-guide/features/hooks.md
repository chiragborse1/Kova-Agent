...
kova 有三套 hook 系统，可在关键生命周期节点运行自定义代码：
...
kova 早期版本将此作为内置 hook 发布，每次 gateway 启动时都会静默生成一个使用裸默认值的 agent。这让使用自定义端点的用户感到意外，也让不知道它在运行的用户无从察觉。将其作为文档化模式保留——由你在 hooks 目录中构建——意味着你能清楚地看到它的行为，并通过编写文件来选择启用。
...
**上下文注入位置：** 始终注入到**用户消息**，而非系统 prompt。这保留了 prompt 缓存——系统 prompt 在各轮次间保持不变，已缓存的 token 得以复用。系统 prompt 是 kova 的领域（模型指导、工具执行、个性、技能）。插件在用户输入旁边贡献上下文。
...
参见 **[构建插件指南](/guides/build-a-kova-plugin)**，获取包含工具 schema、处理器和高级 hook 模式的完整演练。
...
在 `cli-config.yaml` 中声明 shell 脚本 hook，kova 会在对应的插件 hook 事件触发时将其作为子进程运行——在 CLI 和 gateway 会话中均适用。无需编写 Python 插件。
...
每次事件触发时，kova 为每个匹配的 hook（在 matcher 允许的情况下）生成一个子进程，将 JSON 载荷通过 **stdin** 传入，并从 **stdout** 读取 JSON 响应。
...
Claude Code 的 `UserPromptSubmit` 事件在 kova 中没有对应的独立事件——`pre_llm_call` 在相同位置触发，且已支持上下文注入。在此使用即可。
...
**脚本编辑被静默信任。** 允许列表以精确的命令字符串为键，而非脚本的哈希值，因此编辑磁盘上的脚本不会使授权失效。`kova hooks doctor` 会标记 mtime 漂移，以便你发现编辑并决定是否重新审批。

### `kova hooks` CLI

| 命令 | 功能 |
|------|------|
| `kova hooks list` | 列出已配置的 hook，包含 matcher、超时和授权状态 |
| `kova hooks test <event> [--for-tool X] [--payload-file F]` | 对合成载荷触发所有匹配的 hook 并打印解析后的响应 |
| `kova hooks revoke <command>` | 删除所有匹配 `<command>` 的允许列表条目（下次重启后生效） |
| `kova hooks doctor` | 对每个已配置的 hook 检查：执行位、允许列表状态、mtime 漂移、JSON 输出有效性和大致执行时间 |
