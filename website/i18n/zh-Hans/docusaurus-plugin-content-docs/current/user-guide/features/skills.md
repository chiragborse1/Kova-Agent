...
所有 skills 存放在 **`~/.kova/skills/`** 中——这是主目录和唯一可信来源。全新安装时，捆绑的 skills 会从仓库复制过来。通过 Hub 安装和 agent 创建的 skills 也存放在此处。agent 可以修改或删除任何 skill。

你也可以让 kova 指向**外部 skill 目录**——与本地目录一起扫描的额外文件夹。参见下方的[外部 Skill 目录](#external-skill-directories)。
...
当遇到缺失的值时，kova 仅在本地 CLI 中实际加载 skill 时才会安全地请求输入。你可以跳过设置并继续使用该 skill。消息平台不会在聊天中请求密钥——它们会告诉你改用本地的 `kova setup` 或 `~/.kova/.env`。
...
设置存储在 config.yaml 的 `skills.config` 下。`kova config migrate` 会提示配置未设置的项，`kova config show` 会显示它们。当 skill 加载时，其解析后的配置值会注入到上下文中，agent 会自动知晓已配置的值。
...
如果你在 kova 之外维护 skills——例如，供多个 AI 工具使用的共享 `~/.agents/skills/` 目录——你可以告诉 kova 也扫描这些目录。

在 `~/.kova/config.yaml` 的 `skills` 部分下添加 `external_dirs`：
...
- **本地创建，就地更新**：新的 agent 创建的 skills 写入 `~/.kova/skills/`。现有 skills 在找到的位置被修改，包括 `external_dirs` 下的 skills，当 agent 使用 `skill_manage` 操作（如 `patch`、`edit`、`write_file`、`remove_file` 或 `delete`）时。
- **外部目录不是写保护边界**：如果外部 skill 目录对 kova 进程可写，agent 管理的 skill 更新可以修改该目录中的文件。如果共享的外部 skills 必须保持只读，请使用文件系统权限或单独的 profile/toolset 设置。
...
- **不存在的路径会被静默跳过**：如果配置的目录不存在，kova 会忽略它而不报错。适用于可能不在每台机器上都存在的可选共享目录。
...
捆绑包存放在 **`~/.kova/skill-bundles/<slug>.yaml`** 中，格式如下：
...
- `description`（可选）——在 `/bundles` 和 `kova bundles list` 中显示的简短文本。
...
- 你想通过将捆绑包 YAML 提交到共享 dotfiles 仓库并符号链接到 `~/.kova/skill-bundles/` 来发布团队范围的"任务配置文件"。

捆绑包只是一个 YAML 别名——它不会为你安装 skills。Skills 本身必须已经存在（在 `~/.kova/skills/` 或外部 skill 目录中）。否则捆绑包调用只会跳过缺失的 skills。
...
kova skills install https://sharethis.chat/SKILL.md              # Direct URL (single-file SKILL.md)
...
| 来源 | 示例 | 说明 |
|--------|---------|-------|
| `official` | `official/security/1password` | kova 随附的可选 skills。 |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `kova skills search <query> --source skills-sh` 搜索。当 skills.sh slug 与仓库文件夹不同时，kova 会解析别名式 skills。 |
...
kova 目前与以下 skills 生态系统和发现来源集成：

#### 1. 官方可选 skills（`official`）

这些 skills 在 kova 仓库中维护，以内置信任级别安装。
...
这是 Vercel 的公共 skills 目录。kova 可以直接搜索它、查看 skill 详情页、解析别名式 slug，并从底层源仓库安装。
...
kova 可以直接从 GitHub 仓库和基于 GitHub 的 tap 安装。当你已知仓库/路径或想添加自己的自定义源仓库时非常有用。
...
kova 支持发布 Claude 兼容插件/市场清单的市场仓库。

已知集成来源包括：
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

kova 来源 id：`claude-marketplace`

#### 7. LobeHub（`lobehub`）

kova 可以从 LobeHub 的公共目录中搜索并将 agent 条目转换为可安装的 kova skills。
...
kova 与 [browse.sh](https://browse.sh) 集成，这是 Browserbase 的目录，包含 200+ 个针对特定站点的浏览器自动化 SKILL.md 文件（Airbnb、Amazon、arXiv、12306.cn、Etsy、Xero 等）。每个 skill 描述如何端到端驱动一个网站，适合与 kova 的浏览器工具以及你已安装的任何浏览器自动化 skills 配合使用。

- 站点：[browse.sh](https://browse.sh/)
- 目录 API：`https://browse.sh/api/skills`
- kova 来源 id：`browse-sh`
- 信任级别：`community`
...
直接从任何 HTTP(S) URL 安装单文件 `SKILL.md`——当作者在自己的站点上托管 skill 时非常有用（无 hub 列表，无需输入 GitHub 路径）。kova 获取 URL，解析 YAML frontmatter，进行安全扫描并安装。
...
信任级别始终为 `community`——与所有其他来源一样运行相同的安全扫描。URL 作为安装标识符存储，因此当你想刷新时，`kova skills update` 会自动从同一 URL 重新获取。

### 安全扫描与 `--force`

所有通过 hub 安装的 skills 都经过**安全扫描器**检查，检测数据泄露、prompt 注入、破坏性命令、供应链信号及其他威胁。

`kova skills inspect ...` 现在还会在可用时显示上游元数据：
- 仓库 URL
- skills.sh 详情页 URL
- 安装命令
- 每周安装量
...
如果你想分享一组精选的 skills——为你的团队、组织或公开分享——你可以将它们发布为 **tap**：其他 kova 用户通过 `kova skills tap add <owner/repo>` 添加的 GitHub 仓库。无需服务器，无需注册表注册，无需发布流水线。只需一个包含 `SKILL.md` 文件的目录。
...
kova 通过列出 tap 路径的每个子目录并探测每个目录中的 `SKILL.md` 来发现 skills。
...
如果你的 skills 不在 `skills/` 下（当你向现有项目添加 `skills/` 子树时很常见），请编辑 `~/.kova/.hub/taps.json` 中的 tap 条目：
...
`kova skills tap add` CLI 默认将新 tap 的 `path` 设为 `"skills/"`；如果需要不同路径，请直接编辑该文件。`kova skills tap list` 显示每个 tap 的有效路径。
...
新 tap 默认分配 `community` 信任级别。从中安装的 skills 经过标准安全扫描，首次安装时显示第三方警告面板。如果你的组织或广泛受信任的来源应获得更高信任，请将其仓库添加到 `tools/skills_hub.py` 中的 `TRUSTED_REPOS`（需要 kova 核心 PR）。
...
Tap 存储在 `~/.kova/.hub/taps.json` 中（按需创建）。

## 捆绑 skill 更新（`kova skills reset`）

kova 在仓库的 `skills/` 中附带一组捆绑 skills。在安装时以及每次 `kova update` 时，同步过程会将这些 skills 复制到 `~/.kova/skills/` 中，并在 `~/.kova/skills/.bundled_manifest` 记录一个清单，将每个 skill 名称映射到同步时的内容哈希（**origin hash**）。
...
这种保护机制很好，但有一个棘手的边缘情况。如果你编辑了一个捆绑 skill，后来想通过从 `~/.kova/kova-agent/skills/` 复制粘贴来放弃更改并回到捆绑版本，清单仍然保存着上次成功同步时的*旧* origin hash。你新复制粘贴的内容（当前捆绑哈希）与那个过时的 origin hash 不匹配，因此同步继续将其标记为用户修改。
...
:::note Profiles
每个 profile 在其自己的 `KOVA_HOME` 下有自己的 `.bundled_manifest`，因此 `kova -p coder skills reset <name>` 只影响该 profile。
:::
