...
sidebar_position: 10
title: "皮肤与主题"
description: "使用内置和用户自定义皮肤定制 Kova CLI 的外观"
---

# 皮肤与主题

皮肤控制 Kova CLI 的**视觉呈现**：横幅颜色、spinner（加载动画）面孔与动词、响应框标签、品牌文本以及工具活动前缀。
...
| 皮肤 | 描述 | Agent 品牌 | 视觉特征 |
|------|------|-----------|---------|
| `default` | 经典 kova — 金色与 kawaii 风格 | `Kova Agent` | 暖金色边框，cornsilk 文字，spinner 中的 kawaii 面孔。熟悉的双蛇杖横幅。简洁亲切。 |
| `ares` | 战神主题 — 深红与青铜 | `Ares Agent` | 深红色边框配青铜点缀。激进的 spinner 动词（"forging"、"marching"、"tempering steel"）。自定义剑盾 ASCII 艺术横幅。 |
| `mono` | 单色 — 简洁灰度 | `Kova Agent` | 全灰色，无彩色。边框为 `#555555`，文字为 `#c9d1d9`。适合极简终端或录屏场景。 |
| `slate` | 冷蓝色 — 面向开发者 | `Kova Agent` | 皇家蓝边框（`#4169e1`），柔和蓝色文字。沉稳专业。无自定义 spinner，使用默认面孔。 |
| `daylight` | 适用于亮色终端的浅色主题，深色文字配冷蓝点缀 | `Kova Agent` | 专为白色或亮色终端设计。深石板色文字配蓝色边框，浅色状态面板，补全菜单在亮色终端配置下保持清晰可读。 |
| `warm-lightmode` | 适用于浅色终端背景的暖棕/金色文字 | `Kova Agent` | 适合浅色终端的暖羊皮纸色调。深棕色文字配马鞍棕点缀，奶油色状态面板。比 daylight 主题更温暖的大地色系选择。 |
...
| 键 | 描述 | 默认值 |
|----|------|--------|
| `agent_name` | 横幅标题和状态显示中的名称 | `Kova Agent` |
| `welcome` | CLI 启动时显示的欢迎消息 | `Welcome to Kova Agent! Type your message or /help for commands.` |
| `goodbye` | 退出时显示的消息 | `Goodbye! ⚕` |
| `response_label` | 响应框标题上的标签 | ` ⚕ kova ` |
...
在 `~/.kova/skins/` 下创建 YAML 文件。用户皮肤会从内置 `default` 皮肤继承缺失的值，因此只需指定要更改的键。
...
6. 点击 **Save** 将皮肤 YAML 写入 `~/.kova/skins/`。
7. 点击 **Activate** 将其设为当前皮肤（更新 `config.yaml` 中的 `display.skin`）。

kova Mod 遵循 `KOVA_HOME` 环境变量，因此也适用于[配置文件](/user-guide/profiles)。
