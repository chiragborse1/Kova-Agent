...
| 凭据 | 来源 | 配置方式 |
|------|------|---------|
| **SuperGrok / X Premium+ OAuth**（推荐） | 在 `accounts.x.ai` 浏览器登录，自动刷新 | `kova auth add xai-oauth` — 参见 [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** | 付费 xAI API 密钥 | 在 `~/.kova/.env` 中设置 |
...
当 xAI 凭据（OAuth token 或 `XAI_API_KEY`）存在时自动启用。如不需要，可通过 `kova tools` → Search → x_search 显式禁用。
...
当两种认证路径均失败时，工具会显示此错误。请在 `~/.kova/.env` 中设置 `XAI_API_KEY`，或运行 `kova auth add xai-oauth` 并完成浏览器登录。然后重启会话，让 agent 重新加载工具注册表。
...
1. **工具集未启用。** 运行 `kova tools`，确认 `🐦 X (Twitter) Search` 已勾选。
2. **无 xAI 凭据。** `check_fn` 返回 False，schema 保持隐藏。运行 `kova auth status` 确认 xai-oauth 登录状态，并检查 `XAI_API_KEY` 是否已设置（如使用 API 密钥路径）。
