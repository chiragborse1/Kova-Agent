...
[Honcho](https://github.com/plastic-labs/honcho) 是一个 AI 原生记忆后端，在 kova 内置记忆系统之上增加了辩证推理（dialectic reasoning）和深度用户建模能力。它不是简单的键值存储，而是通过对对话事后推理，持续维护一个关于用户的动态模型——涵盖其偏好、沟通风格、目标与行为模式。
...
**多 agent 画像**：当多个 kova 实例与同一用户交互时（例如编程助手和个人助手），Honcho 为每个 peer 维护独立画像。每个 peer 只能看到自己的观察和结论，防止上下文交叉污染。

## 设置

```bash
kova memory setup    # 从 provider 列表中选择 "honcho"
```
...
Honcho 在 `~/.honcho/config.json`（全局）或 `$KOVA_HOME/honcho.json`（profile 本地）中配置。设置向导会自动处理。
...
- `per-session` — 每次 `kova` 运行获得一个新会话。干净启动，通过 tools 访问记忆。推荐新用户使用。
...
通过 [Honcho 控制台](https://app.honcho.dev) 设置的服务端开关优先于本地默认值——kova 在会话初始化时同步回本地。
...
`kova honcho` 子命令**仅在 Honcho 为当前活跃 memory provider 时注册**（`config.yaml` 中 `memory.provider: honcho`）。先运行 `kova memory setup` 并选择 Honcho，子命令将在下次调用时出现。

```bash
kova honcho status          # 连接状态、配置及关键设置
kova honcho setup           # 重定向到 `kova memory setup`
kova honcho strategy        # 查看或设置会话策略（per-session/per-directory/per-repo/global）
kova honcho peer            # 查看或更新 peer 名称及辩证推理级别
kova honcho mode            # 查看或设置 recall 模式（hybrid/context/tools）
kova honcho tokens          # 查看或设置上下文和辩证的 token 预算
kova honcho identity        # 初始化或查看 AI peer 的 Honcho 身份
kova honcho sync            # 将 Honcho 配置同步到所有现有 profile
kova honcho peers           # 查看所有 profile 中的 peer 身份
kova honcho sessions        # 列出已知的 Honcho 会话映射
kova honcho map             # 将当前目录映射到 Honcho 会话名称
kova honcho enable          # 为当前 profile 启用 Honcho
kova honcho disable         # 为当前 profile 禁用 Honcho
kova honcho migrate         # 从 openclaw-honcho 迁移的分步指南
```

## 从 `kova honcho` 迁移

如果你之前使用了独立的 `kova honcho setup`：

1. 你的现有配置（`honcho.json` 或 `~/.honcho/config.json`）已保留
2. 你的服务端数据（记忆、结论、用户画像）完好无损
3. 在 config.yaml 中设置 `memory.provider: honcho` 即可重新激活

无需重新登录或重新设置。运行 `kova memory setup` 并选择"honcho"——向导会自动检测你的现有配置。
