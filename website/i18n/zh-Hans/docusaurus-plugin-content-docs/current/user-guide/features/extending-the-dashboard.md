...
sidebar_position: 17
title: "扩展 Dashboard"
description: "为 kova Web Dashboard 构建主题和插件——调色板、字体排版、布局、自定义标签页、shell 插槽、页面级插槽以及后端 API 路由"
---

# 扩展 Dashboard

kova Web Dashboard（`kova dashboard`）在设计上支持换肤和扩展，无需 fork 代码库。对外暴露三个层次：

1. **主题（Themes）** — YAML 文件，用于重绘 dashboard 的调色板、字体排版、布局以及各组件的外观。将文件放入 `~/.kova/dashboard-themes/`，即可在主题切换器中看到它。
...
主题是存储在 `~/.kova/dashboard-themes/` 中的 YAML 文件。文件名无关紧要（系统使用主题的 `name:` 字段），但惯例是 `<name>.yaml`。所有字段均为可选——缺失的键会回退到内置的 `default` 主题，因此一个主题可以只包含一个颜色。

### 快速上手——你的第一个主题

```bash
mkdir -p ~/.kova/dashboard-themes
```

```yaml
# ~/.kova/dashboard-themes/neon.yaml
name: neon
label: Neon
...
CSS 在主题应用时以单个带作用域的 `<style data-kova-theme-css>` 标签注入，主题切换时清除。**每个主题上限为 32 KiB。**
...
| 主题 | 调色板 | 字体排版 | 布局 |
|-------|---------|------------|--------|
| **kova Teal**（`default`） | 深青色 + 奶油色 | 系统字体栈，15px | 0.5rem 圆角，comfortable |
| **kova Teal (Large)**（`default-large`） | 同 default | 系统字体栈，18px，行高 1.65 | 0.5rem 圆角，spacious |
...
引用 Google Fonts 的主题（除 kova Teal 外均如此）会按需加载样式表——首次切换时会向 `<head>` 注入一个 `<link>` 标签。
...
```yaml
# ~/.kova/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
Dashboard 插件是一个包含 `manifest.json`、预构建 JS bundle，以及可选的 CSS 文件和带 FastAPI 路由的 Python 文件的目录。插件与其他 kova 插件一起存放在 `~/.kova/plugins/<name>/`——dashboard 扩展是该插件目录内的 `dashboard/` 子文件夹，因此一个插件可以从单次安装中同时扩展 CLI/gateway 和 dashboard。

插件不打包 React 或 UI 组件，而是使用暴露在 `window.__KOVA_PLUGIN_SDK__` 上的 **Plugin SDK**。这使插件 bundle 保持极小体积（通常只有几 KB），并避免版本冲突。
...
```bash
mkdir -p ~/.kova/plugins/my-plugin/dashboard/dist
```

编写 manifest：

```json
// ~/.kova/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.kova/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
插件所需的一切均在 `window.__KOVA_PLUGIN_SDK__` 上。插件不应直接导入 React。
...
```json
// ~/.kova/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.kova/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__KOVA_PLUGIN_SDK__;
  const { React } = SDK;
...
参考插件（[`kova-example-plugins`](https://github.com/NousResearch/kova-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
```python
# ~/.kova/plugins/my-plugin/dashboard/plugin_api.py
...
后端路由在 dashboard 进程内运行，因此可以直接从 kova-agent 代码库导入：
...
| 优先级 | 目录 | 来源标签 |
|----------|-----------|--------------|
| 1（冲突时优先） | `~/.kova/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.kova/plugins/<name>/dashboard/` | `project`——仅在设置 `KOVA_ENABLE_PROJECT_PLUGINS` 时生效 |
...
1. Dashboard 加载。`main.tsx` 在 `window.__KOVA_PLUGIN_SDK__` 上暴露 SDK，在 `window.__KOVA_PLUGINS__` 上暴露注册表。
2. `App.tsx` 调用 `usePlugins()` → 获取 `GET /api/dashboard/plugins`。
3. 对于每个 manifest：注入 CSS `<link>`（如已声明），然后通过 `<script>` 标签加载 JS bundle。
4. 插件的 IIFE 运行并调用 `window.__KOVA_PLUGINS__.register(name, Component)`——以及可选的 `.registerSlot(name, slot, Component)` 用于每个插槽。
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/kova-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `kova-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
...
cp kova-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.kova/dashboard-themes/

# Plugin
cp -r kova-example-plugins/strike-freedom-cockpit ~/.kova/plugins/
```

打开 dashboard，从主题切换器中选择 **Strike Freedom**。驾驶舱侧边栏出现，徽标显示在顶栏，标语替换底栏。切换回 **kova Teal**，插件仍然安装但不可见（`sidebar` 插槽仅在 `cockpit` 布局变体下渲染）。
...
| 全局变量 | 类型 | 提供方 |
|--------|------|----------|
| `window.__KOVA_PLUGIN_SDK__` | object | `registry.ts` — React、hooks、UI 组件、API 客户端、工具函数。 |
| `window.__KOVA_PLUGINS__.register(name, Component)` | function | 注册插件的主组件。 |
| `window.__KOVA_PLUGINS__.registerSlot(name, slot, Component)` | function | 注册到命名 shell 插槽。 |
...
**我的主题没有出现在选择器中。**
检查文件是否在 `~/.kova/dashboard-themes/` 中且以 `.yaml` 或 `.yml` 结尾。刷新页面。运行 `curl http://127.0.0.1:9119/api/dashboard/themes`——你的主题应出现在响应中。如果 YAML 有解析错误，dashboard 会记录到 `~/.kova/logs/` 下的 `errors.log`。

**我的插件标签页没有显示。**
1. 检查 manifest 是否在 `~/.kova/plugins/<name>/dashboard/manifest.json`（注意 `dashboard/` 子目录）。
...
4. 打开浏览器开发工具 → Console——查找 IIFE 执行期间的错误或 `window.__KOVA_PLUGINS__ is undefined`（表示 SDK 未初始化，通常是更早的 React 渲染崩溃导致）。
5. 验证你的 bundle 以与 `manifest.json:name` **相同的名称**调用 `window.__KOVA_PLUGINS__.register(...)`。
...
2. 重启 `kova dashboard`——插件 API 路由在启动时挂载一次，**不会**在重新扫描时挂载。
3. 检查 `plugin_api.py` 是否导出了模块级的 `router = APIRouter()`。其他导出名称不会被识别。
4. 查看 `~/.kova/logs/errors.log` 中的 `Failed to load plugin <name> API routes`——导入错误会记录在那里。
...
**我想在 PyPI 上发布插件。**
Dashboard 插件通过目录结构安装，而非 pip 入口点。目前最简洁的分发方式是用户克隆到 `~/.kova/plugins/` 的 git 仓库。基于 pip 的 dashboard 插件安装器目前尚未实现。