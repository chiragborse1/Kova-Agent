...
凭证池允许你为同一提供商注册多个 API 密钥或 OAuth 令牌。当某个密钥触达速率限制或计费配额时，kova 会自动轮换到下一个健康密钥——在不切换提供商的情况下保持会话持续运行。
...
如果你已在 `.env` 中设置了 API 密钥，kova 会自动将其识别为单密钥池。要充分利用池化功能，请添加更多密钥：
...
|---------|-------------|
| `kova auth` | 交互式池管理向导 |
| `kova auth list` | 显示所有池和凭证 |
| `kova auth list <provider>` | 显示指定提供商的池 |
| `kova auth add <provider>` | 添加凭证（提示选择类型和密钥） |
| `kova auth add <provider> --type api-key --api-key <key>` | 非交互式添加 API 密钥 |
| `kova auth add <provider> --type oauth` | 通过浏览器登录添加 OAuth 凭证 |
| `kova auth remove <provider> <index>` | 按从 1 开始的索引删除凭证 |
| `kova auth reset <provider>` | 清除所有冷却时间/耗尽状态 |

## 轮换策略

通过 `kova auth` → "Set rotation strategy" 配置，或在 `config.yaml` 中设置：
...
通过 `kova model` 设置自定义端点时，会自动生成类似 "Together.ai" 或 "Local (localhost:8080)" 的名称，该名称即成为池的键。
...
| kova PKCE OAuth | `~/.kova/auth.json` | 是（Anthropic） |
| 自定义端点配置 | `config.yaml` 中的 `model.api_key` | 是（自定义端点） |
| 手动条目 | 通过 `kova auth add` 添加 | 持久化至 auth.json |

自动初始化的条目在每次池加载时更新——如果你删除了某个环境变量，其池条目会自动清除。通过 `kova auth add` 添加的手动条目永远不会被自动清除。
...
2. **`KOVA_cli/auth_commands.py`** — CLI 命令和交互式向导
3. **`KOVA_cli/runtime_provider.py`** — 感知池的凭证解析
4. **`run_agent.py`** — 错误恢复：429/402/401 → 池轮换 → 备用

## 存储

池状态存储在 `~/.kova/auth.json` 的 `credential_pool` 键下：
