...
Kova Agent 具备三层弹性机制，在提供商出现问题时保持会话正常运行：
...
当主 LLM 提供商遇到错误——速率限制、服务器过载、认证失败、连接中断——kova 可以在会话中途自动切换到备用提供商:模型对，且不会丢失对话内容。
...
`fallback_model`（单数）是旧版单备用键——kova 仍支持以保持向后兼容。`fallback_providers`（复数，列表）支持按顺序尝试多个备用；`kova fallback` 写入此键。当两者同时设置时，kova 会合并它们，`fallback_providers` 优先。
:::
...
| xAI Grok OAuth（SuperGrok） | `xai-oauth`（别名 `grok-oauth`） | `kova model` → xAI Grok OAuth（浏览器登录；需 SuperGrok 订阅） |
| AWS Bedrock | `bedrock` | 标准 boto3 认证（`AWS_REGION` + `AWS_PROFILE` 或 `AWS_ACCESS_KEY_ID`） |
| Qwen Portal（OAuth） | `qwen-oauth` | `kova model`（Qwen Portal OAuth；可选：`KOVA_QWEN_BASE_URL`） |
| MiniMax（OAuth） | `minimax-oauth` | `kova model`（MiniMax 门户 OAuth） |
...
:::info 按轮次，而非按会话
备用机制的**作用域为单次轮次**：每条新用户消息都从主模型重新开始。若主模型在某轮次中途失败，备用仅对该轮次生效。下一条消息时，kova 会再次尝试主模型。在单次轮次内，备用最多激活一次——若备用也失败，则进入常规错误处理流程（重试，然后返回错误消息）。这既防止了单轮次内的级联故障转移循环，又让主模型在每轮次都有重新尝试的机会。
:::
...
:::tip
没有针对主备用链的环境变量——只能通过 `config.yaml` 或 `kova fallback` 进行配置。这是有意为之：备用配置是一个经过深思熟虑的选择，不应被过期的 shell 导出变量覆盖。
:::
...
kova 为附属任务使用独立的轻量级模型。每个任务都有自己的提供商解析链，充当内置的备用系统。
...
| Triage Specifier | `kova kanban specify` / 看板（kanban）✨ 按钮——将单行 triage 任务扩展为完整规格 | `auxiliary.triage_specifier` |

### 自动检测链

当任务的提供商设置为 `"auto"`（默认值）时，kova 按顺序尝试各提供商，直到找到可用的：
...
若解析到的提供商在调用时失败，kova 还有内部重试机制：若该提供商不是 OpenRouter 且未设置显式 `base_url`，则尝试以 OpenRouter 作为最后备用。
...
| `"nous"` | 强制使用 Nous Portal | `kova auth` |
| `"codex"` | 强制使用 Codex OAuth | `kova model` → Codex |
...
`base_url` 优先于 `provider`。kova 使用配置的 `api_key` 进行认证，若未设置则回退到 `OPENAI_API_KEY`。对于自定义端点，**不会**复用 `OPENROUTER_API_KEY`。

---

## 辅助任务容量错误备用

当你设置了显式的辅助提供商（例如 `auxiliary.vision.provider: glm`）时，kova 将其视为首选——但若该提供商因**容量错误**（HTTP 402 付款要求、HTTP 429 每日配额耗尽、连接失败）而无法处理请求，kova 会通过分层链进行备用，而不是静默失败：

1. **主辅助提供商** — 你配置的那个（始终优先尝试）
2. **`auxiliary.<task>.fallback_chain`** — 你的每任务覆盖列表（若已配置）
3. **主 Agent 提供商 + 模型** — 最后的安全网（始终尝试，即使未配置链）
4. **警告 + 重新抛出** — 若所有层均失败，kova 以 WARNING 级别记录 `Auxiliary <task>: ... all fallbacks exhausted` 并重新抛出原始错误
...
kova 将以下情况识别为等同于 402 额度耗尽的容量错误（而非瞬时速率限制）：
...
若你的提供商对每日配额耗尽返回不同的错误信息，而 kova 未触发备用，这是一个 bug——请附上确切的错误字符串提交 issue。
...
若压缩没有可用的提供商，kova 会直接丢弃中间对话轮次而不生成摘要，而不是让会话失败。
