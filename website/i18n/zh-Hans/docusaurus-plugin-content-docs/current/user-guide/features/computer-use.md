...
Kova Agent 可以在**后台**驱动你的 Mac 桌面——点击、输入、滚动、拖拽。你的光标不会移动，键盘焦点不会改变，macOS 也不会切换 Spaces。你和 Agent 可以在同一台机器上协同工作。
...
使用 `kova computer-use status` 验证安装结果。

**方式二：通过交互式界面启用工具集。**

1. 运行 `kova tools`，选择 `🖱️ Computer Use (macOS)` → `cua-driver (background)`。
2. 安装程序将运行上游安装脚本（与方式一相同）。

安装完成后，无论采用哪种方式，继续执行以下步骤：

3. 在提示时授予 macOS 权限：
   - **系统设置 → 隐私与安全性 → 辅助功能** → 允许终端（或 kova 应用）。
   - **系统设置 → 隐私与安全性 → 屏幕录制** → 允许同一应用。
4. 启动启用了该工具集的会话：
   ```
   kova -t computer_use chat
   ```
   或在 `~/.kova/config.yaml` 中将 `computer_use` 添加到已启用的工具集列表。

## 保持 cua-driver 最新

cua-driver 项目会定期发布修复（例如 v0.1.6 修复了 UTM 工作流中的 Safari 窗口焦点问题）。kova 在两处刷新二进制文件，避免你停留在过时版本：

- **`kova update`** — 更新 kova 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
- **`kova computer-use install --upgrade`** — 手动强制刷新。无论 cua-driver 是否已安装，都会重新运行上游安装程序。在不等待下次 Agent 更新的情况下获取最新修复时使用此命令。

`kova computer-use status` 会在二进制路径旁显示已安装的版本号。
...
如需对每个操作进行确认，可在 `~/.kova/config.yaml` 中配置 `approvals.mode: manual`。

## Token 效率

截图开销较大。kova 应用四层优化措施：
...
- **私有 SPI 风险。** Apple 可能在任何 OS 更新中更改 SkyLight 的符号接口。kova 始终安装最新版 cua-driver，并在已安装的二进制文件低于其测试基线版本（按操作系统分别设定）时发出警告。没有版本固定开关——如需可复现的版本，请将 `KOVA_CUA_DRIVER_CMD` 指向特定的二进制文件。
...
```
KOVA_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
```
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `kova computer-use install` 获取 cua-driver 二进制文件，或运行 `kova tools` 并启用 Computer Use 工具集。
...
- [通用技能：`macos-computer-use`](https://github.com/NousResearch/Kova-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
