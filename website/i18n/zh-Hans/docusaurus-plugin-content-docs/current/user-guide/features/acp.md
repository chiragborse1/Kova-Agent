...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Kova Agent"
---

# ACP 编辑器集成

Kova Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 kova 通信并渲染：
...
当你希望 kova 表现得像编辑器原生的编码 agent，而非独立 CLI 或消息机器人时，ACP 是合适的选择。
...
对于 Zed registry 安装，Zed 通过官方 ACP Registry 条目启动 Kova。该条目使用 `uvx` 发行版运行：
...
kova 将日志输出到 stderr，以保留 stdout 用于 ACP JSON-RPC 流量。
...
```bash
kova acp --setup-browser           # 交互式（下载约 400 MB 前会提示确认）
kova acp --setup-browser --yes     # 非交互式接受下载
```

这是独立命令。Zed registry 的终端认证流程（`kova acp --setup`）在模型选择后也会将浏览器引导作为后续问题提供，因此大多数用户无需直接运行 `--setup-browser`。
...
2. 从内置 agent 列表中选择 **Kova Agent**。
3. 连接并开始聊天。

如需手动定义 Kova，通过 VS Code 设置在 `acp.agents` 下添加：
...
3. 搜索 **Kova Agent**。
4. 安装后启动新的 kova 外部 agent 线程。

前提条件：

- 先通过 `kova model` 配置 kova provider 凭据，或在 `~/.kova/.env` / `~/.kova/config.yaml` 中设置。
- 安装 `uv`，以便 registry 启动器可以运行 `uvx --from 'kova-agent[acp]==<version>' kova-acp`。
...
kova 官方 ACP Registry 元数据的源文件位于：
...
上游 registry PR 将这些文件复制到 `agentclientprotocol/registry` 中的顶层 `kova-agent/` 目录。

Registry 条目使用直接指向 `kova-agent` PyPI 发行版的 `uvx` 发行版：
...
Provider 解析使用 kova 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。kova 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 kova 的交互式模型/provider 设置。
...
底层 `AIAgent` 仍使用 kova 的正常持久化/日志路径，但 ACP 的 `list/load/resume/fork` 仅限于当前运行的 ACP 服务器进程。

## 工作目录行为

ACP 会话将编辑器的 cwd 绑定到 kova 任务 ID，使文件和终端工具相对于编辑器工作区运行，而非服务器进程的 cwd。
...
| `allow_always` | 始终允许 | 所有未来会话 | 是（写入 kova 永久允许列表） |
| `deny` | 拒绝 | 本次工具调用 | 否 |
...
ACP 桥接将这些选项映射到 kova 的内部审批语义——`allow_always` 与 CLI 相同地写入永久允许列表条目，而 `allow_session` 仅影响当前 ACP 会话的进程内审批缓存。
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **Kova Agent**。
...
ACP 模式使用 kova 现有的 provider 设置。通过以下方式配置凭据：

```bash
kova model
```

或编辑 `~/.kova/.env`。Registry 客户端也可以触发 kova 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Kova Agent 线程。
