...
sidebar_label: "内置插件"
title: "内置插件"
description: "随 Kova Agent 附带并通过生命周期 hook 自动运行的插件——disk-cleanup 等"
---
...
请参阅 [插件](/user-guide/features/plugins) 页面了解通用插件系统，以及 [构建 kova 插件](/guides/build-a-kova-plugin) 了解如何编写自己的插件。
...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `kova memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `kova plugins list` 和交互式 `kova plugins` UI 中），但在你明确启用之前不会加载：
...
这与用户安装的插件使用的机制相同。内置插件永远不会自动启用——无论是全新安装，还是现有用户升级到更新版本的 Kova，都需要你明确选择启用。
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `kova plugins enable <name>` 启用。
...
| `kova-achievements` | 仪表盘标签页 | Steam 风格的可收集徽章，根据你真实的 kova 会话历史生成 |
| `kanban/dashboard` | 仪表盘标签页 | 多智能体调度器的看板（Kanban）UI——任务、评论、扇出、切换看板。参见 [Kanban 多智能体](./kanban.md)。 |

内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `kova memory` 和 `kova plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
| Hook | 行为 |
|---|---|
| `post_tool_call` | 当 `write_file` / `terminal` / `patch` 在 `KOVA_HOME` 或 `/tmp/kova-*` 内创建匹配 `test_*`、`tmp_*` 或 `*.test.*` 的文件时，静默追踪为 `test` / `temp` / `cron-output`。 |
...
**状态** — 所有内容存储在 `$KOVA_HOME/disk-cleanup/`：
...
**安全性** — 清理操作仅涉及 `KOVA_HOME` 或 `/tmp/kova-*` 下的路径。Windows 挂载点（`/mnt/c/...`）会被拒绝。已知的顶级状态目录（`logs/`、`memories/`、`sessions/`、`cron/`、`cache/`、`skills/`、`plugins/`、`disk-cleanup/` 本身）即使为空也不会被删除——全新安装不会在第一次会话结束时被清空。

**启用：** `kova plugins enable disk-cleanup`（或在 `kova plugins` 中勾选复选框）。

**再次禁用：** `kova plugins disable disk-cleanup`。

### observability/langfuse

将 kova 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 kova 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `kova logs` 中显示的一致。
...
KOVA_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # 或你的自托管 URL
```
...
| Hook | 行为 |
|---|---|
| `pre_api_request` / `pre_llm_call` | 打开（或复用）每轮的根 span "kova turn"。为本次 API 调用启动一个 `generation` 子 observation，将最近的消息序列化为输入。 |
...
| `post_tool_call` | 关闭 tool observation，附加经过清理的 `result`。`read_file` 的内容会被摘要化（头部 + 尾部 + 省略行数），以使大文件读取保持在 `KOVA_LANGFUSE_MAX_CHARS` 以内。 |

会话分组基于 kova 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `kova chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
kova plugins list                 # observability/langfuse 应显示 "enabled"
kova chat -q "hello"              # 在 Langfuse UI 中检查是否有 "kova turn" trace
```
...
| `KOVA_LANGFUSE_RELEASE` | — | 发布/版本标签 |
| `KOVA_LANGFUSE_SAMPLE_RATE` | `1.0` | 传递给 SDK 的采样率（0.0–1.0） |
| `KOVA_LANGFUSE_MAX_CHARS` | `12000` | 消息内容 / 工具参数 / 工具结果的单字段截断长度 |
| `KOVA_LANGFUSE_DEBUG` | `false` | 向 `agent.log` 输出详细插件日志 |

kova 前缀的环境变量和标准 SDK 环境变量（`LANGFUSE_PUBLIC_KEY`、`LANGFUSE_SECRET_KEY`、`LANGFUSE_BASE_URL`）均被接受——两者同时设置时，kova 前缀的优先。

**性能：** Langfuse 客户端在第一次 hook 调用后被缓存。如果凭据或 SDK 缺失，该决定也会被缓存——后续 hook 会快速返回，不再重新检查环境变量或重新加载配置。

**禁用：** `kova plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
在仪表盘中添加一个 **Steam 风格的成就标签页**——60 多个可收集的分级徽章，根据你真实的 kova 会话历史生成。工具链成就、调试模式、vibe-coding 连击、技能/内存使用、模型/提供者多样性、生活方式特征（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件编写；已并入仓库，以便与 kova 功能变更保持同步。
...
- 解锁状态持久化到 `$KOVA_HOME/plugins/kova-achievements/state.json`
...
| 已解锁 | 至少达到一个等级 |
| 已发现 | 已知成就，进度可见，尚未获得 |
| 隐藏 | 在 kova 检测到你历史中的第一个相关信号之前保持隐藏 |

**API** — 路由挂载在 `/api/plugins/kova-achievements/` 下：
...
**状态文件** — 位于 `$KOVA_HOME/plugins/kova-achievements/`：

| 文件 | 内容 |
|---|---|
| `state.json` | 解锁历史：你获得了哪些徽章以及获得时间。在 kova 更新间保持稳定。 |
...
**退出：** 删除或重命名 `plugins/kova-achievements/dashboard/manifest.json`，或在 `~/.kova/plugins/kova-achievements/` 中用同名用户插件覆盖它（该插件不包含仪表盘）。`$KOVA_HOME/plugins/kova-achievements/` 下的插件状态文件会保留——重新安装后你的解锁历史依然存在。

## 添加内置插件

内置插件的编写方式与其他 kova 插件完全相同——参见 [构建 kova 插件](/guides/build-a-kova-plugin)。唯一的区别是：
