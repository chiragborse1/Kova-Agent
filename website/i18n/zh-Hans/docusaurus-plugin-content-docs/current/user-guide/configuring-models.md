...
[Nous Portal](/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `kova setup --portal` 即可登录并一键将 Nous 设为提供商。使用 `kova portal info` 查看当前配置。
:::
...
- **右列** — 所选提供商的精选模型列表。这些是 kova 针对该提供商推荐的 agentic 模型，而非原始的 `/models` 接口返回结果（OpenRouter 的原始列表包含 400+ 个模型，涵盖 TTS、图像生成器和重排序器）。
...
每个辅助任务默认为 `auto`，即 kova 对该任务也使用主模型。当某个边缘任务需要更便宜或更快的模型时，可单独覆盖该槽位。
...
| **Skills Hub（技能中心）** | `kova skills search` 使用此槽位。通常保持 `auto` 即可。 |
...
`provider: auto` 加 `model: ''` 表示 kova 对该任务使用主模型。

## 何时生效？

- **CLI**（`kova chat`）：下次执行 `kova chat` 时生效。
- **Gateway**（Telegram、Discord、Slack 等）：下一个*新*会话生效。现有会话保持原有模型。如需强制所有会话使用新配置，重启 gateway（`Kova Gateway restart`）。
...
kova 仅列出具有有效凭据的提供商。检查侧边栏中的 **Keys** — 应存在以下之一：API key、成功的 OAuth 或自定义端点 URL。若所需提供商不在列表中，运行 `kova setup` 进行配置，或前往 **Keys** 添加环境变量。
