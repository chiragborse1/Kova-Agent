...
sidebar_position: 14
title: "AWS Bedrock"
description: "将 Kova Agent 与 Amazon Bedrock 配合使用——原生 Converse API、IAM 身份验证、Guardrails 及跨区域推理"
---

# AWS Bedrock

Kova Agent 通过 **Converse API** 原生支持 Amazon Bedrock——而非 OpenAI 兼容端点。这让你可以完整访问 Bedrock 生态系统：IAM 身份验证、Guardrails、跨区域推理配置文件以及所有基础模型。
...
- **boto3** — 通过 `cd ~/.kova/kova-agent && uv pip install -e ".[bedrock]"` 安装
- **IAM 权限** — 至少需要：
...
:::tip EC2 / ECS / Lambda
在 AWS 计算环境中，为实例附加带有 `AmazonBedrockFullAccess` 的 IAM 角色即可。无需 API 密钥，无需 `.env` 配置——kova 会自动检测实例角色。
:::
...
运行 `kova model` 后，你的 `~/.kova/config.yaml` 将包含以下内容：
...
kova 通过 Bedrock 控制平面自动发现可用模型。你可以自定义发现行为：
...
Bedrock 模型使用**推理配置文件 ID** 进行按需调用。`kova model` 选择器会自动显示这些 ID，并将推荐模型置于顶部：
...
Bedrock 可与所有 Kova Gateway 平台配合使用（Telegram、Discord、Slack、飞书等）。将 Bedrock 配置为提供商后，正常启动 gateway 即可：
...
你已触及 Bedrock 单模型速率限制。kova 会自动进行退避重试。如需提高限额，请在 [AWS Service Quotas 控制台](https://console.aws.amazon.com/servicequotas/)申请配额提升。

## 一键 AWS 部署

如需在 EC2 上通过 CloudFormation 进行全自动部署：

**[sample-kova-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-kova-agent-on-aws-with-bedrock)** — 自动创建 VPC、IAM 角色、EC2 实例并配置 Bedrock。一键即可在任意区域完成部署。