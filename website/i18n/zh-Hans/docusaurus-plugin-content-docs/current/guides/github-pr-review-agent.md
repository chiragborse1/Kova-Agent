...
:::tip 想要实时审查？
如果你有可用的公开端点，请查看[使用 Webhook 自动化 GitHub PR 评论](./webhook-github-pr-review.md)——GitHub 会在 PR 被打开或更新时立即向 kova 推送事件。
:::
...
- **已安装 Kova Agent** — 参见[安装指南](/getting-started/installation)
...
验证是否已加载——启动 `kova`，你应该能在启动时的 skill 列表中看到 `code-review`。

---

## 第四步：教会它你的团队规范

这才是让审查器真正有用的关键。启动一个会话，向 kova 传授你的团队标准：
...
2. 通过 memory（记忆）向 kova 传授你的团队规范（第四步）
...
- **[构建 Plugin](/guides/build-a-kova-plugin)** — 将审查逻辑封装为可共享的 plugin
