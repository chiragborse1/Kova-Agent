...
sidebar_position: 12
title: "使用 Skills"
description: "查找、安装、使用和创建 skills——按需加载的知识文档，用于教会 kova 新的工作流程"
---

# 使用 Skills

Skills（技能）是按需加载的知识文档，用于教会 kova 如何处理特定任务——从生成 ASCII 艺术到管理 GitHub PR。本指南介绍日常使用方法。
...
每个 kova 安装都内置了捆绑的 skills。查看可用列表：
...
你也可以通过自然对话触发 skills——告诉 kova 使用某个特定 skill，它会通过 `skill_view` 工具加载。
...
官方可选 skills 随 kova 一起发布，但默认未激活，需显式安装：
...
关于如何在自己的插件中捆绑 skills，请参见 [构建 kova 插件 → 捆绑 skills](/guides/build-a-kova-plugin#bundle-skills)。
...
当带有配置的 skill 首次加载时，kova 会提示你输入相应值，并将其存储在 `config.yaml` 的 `skills.config.*` 下。
...
Skill 会自动出现——无需注册。放入 `~/.kova/skills/` 即可立即生效。

:::info
Agent 也可以使用 `skill_manage` 自行创建和更新 skills。解决复杂问题后，kova 可能会主动提议将该方法保存为 skill，以便下次使用。
:::
...
**让 agent 创建 skills。** 完成复杂的多步骤任务后，kova 通常会主动提议将该方法保存为 skill。接受它——这些由 agent 编写的 skills 会捕捉到完整的工作流程，包括过程中发现的各种坑。

**使用分类目录。** 将 skills 整理到子目录中（`~/.kova/skills/devops/`、`~/.kova/skills/research/` 等），保持列表整洁，并帮助 agent 更快找到相关 skills。

**及时更新过时的 skills。** 如果使用某个 skill 时遇到它未覆盖的问题，告诉 kova 用你学到的内容更新该 skill。不维护的 skills 会成为负担。
