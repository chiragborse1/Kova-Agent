...
两者均暴露兼容 OpenAI 的 `/v1/chat/completions` 端点。kova 支持任意一个——只需将其指向 `http://localhost:8080` 或 `http://localhost:8000`。
...
kova 会自动检测本地端点（localhost、局域网 IP）并放宽其流式传输超时限制。大多数情况下无需额外配置。
...
|---------|---------|----------------------|------------------|
| 流式读取（socket 级别） | 120s | 提升至 1800s | `KOVA_STREAM_READ_TIMEOUT` |
| 停滞流检测 | 180s | 完全禁用 | `KOVA_STREAM_STALE_TIMEOUT` |
| API 调用（非流式） | 1800s | 无需调整 | `KOVA_API_TIMEOUT` |
