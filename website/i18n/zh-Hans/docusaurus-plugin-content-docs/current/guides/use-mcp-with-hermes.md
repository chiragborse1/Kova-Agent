---
sidebar_position: 6
title: "在 kova 中使用 MCP"
description: "将 MCP 服务器连接到 Kova Agent、过滤其工具并在实际工作流中安全使用的实践指南"
---

# 在 kova 中使用 MCP

本指南介绍如何在日常工作流中实际使用 Kova Agent 的 MCP 功能。
...
在以下情况下使用 MCP：
- 工具已以 MCP 形式存在，且你不想构建原生 kova 工具
- 你希望 kova 通过干净的 RPC 层操作本地或远程系统
- 你需要细粒度的按服务器暴露控制
- 你希望将 kova 连接到内部 API、数据库或公司系统，而无需修改 kova 核心

在以下情况下不要使用 MCP：
- 内置 kova 工具已能很好地完成该工作
...
如果你使用标准安装脚本安装了 Kova，MCP 支持已包含在内（安装程序会运行 `uv pip install -e ".[all]"`）。
...
在此配置中，kova **不**直接连接到 Chrome，而是：

- kova 在 WSL 中运行
- kova 启动一个本地 stdio MCP 服务器
...
- kova 保持在其支持的 Unix 环境（WSL2）中
- 浏览器控制以 MCP 工具的形式暴露，而不依赖 kova 核心浏览器传输
...
加载后，kova 可以直接使用带 MCP 前缀的浏览器工具。例如：
...
如果 kova 在 WSL 中运行而 Chrome 在 Windows 上运行，即使 Chrome 已打开且可调试，`/browser connect` 也可能失败。
...
- 通过 MCP 使用 Windows stdio 可执行文件时，从 `/mnt/c/Users/<you>` 或 `/mnt/c/workspace/...` 等 Windows 挂载路径启动 Kova。
- 如果从 `/root` 或 `/home/...` 启动 Kova，Windows 可能在 MCP 服务器启动前发出 `UNC` 当前目录警告。
...
因此，如果服务器不支持 resources/prompts，kova 不会假装它支持。

## 常见模式

### 模式 1：本地项目助手

当你希望 kova 在有界工作区内推理时，使用 MCP 连接仓库本地的文件系统或 git 服务器。
...
这就是 MCP 的强大之处：无需修改 kova 核心即可实现多系统工作流。
...
因为 kova 现在遵守你的按服务器策略和能力感知注册。这是预期行为，通常也是期望的结果。
