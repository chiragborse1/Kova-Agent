---
sidebar_position: 9
title: "使用 Ollama 在本地运行 kova — 零 API 费用"
description: "使用 Ollama 和 Gemma 4 等开放权重模型在本机完整运行 Kova Agent 的分步指南，无需云端 API 密钥或付费订阅"
---

# 使用 Ollama 在本地运行 kova — 零 API 费用
...
你将在自己的硬件上完整运行 Kova Agent，使用 [Ollama](https://ollama.com) 作为模型后端。无需 API 密钥，无需订阅，数据不会离开你的机器。配置完成后，kova 的使用体验与 OpenRouter 或 Anthropic 完全一致——终端命令、文件编辑、网页浏览、任务委派——只是模型在本地运行。

完成后，你将拥有：

- Ollama 提供一个或多个开放权重模型的服务
- kova 通过自定义端点连接到 Ollama
...
```bash
# ~/.kova/.env
KOVA_API_TIMEOUT=1800   # 30 分钟 — 为慢速本地模型留出充裕时间
```
:::
...
:::warning 工具调用至关重要
kova 是一个**agentic（智能体）**助手——它通过工具调用来编辑文件、执行命令和浏览网页。不支持工具调用的模型只能进行对话，无法执行操作。要体验完整的 kova 功能，请使用支持工具的模型（如 `gemma4:31b`）。
:::
...
:::info 多个模型
你可以拉取多个模型，并在 kova 中使用 `/model` 切换。Ollama 按需将活跃模型加载到内存，并自动卸载空闲模型。
:::
...
kova 将使用终端工具、文件操作和你的本地模型——无需任何云端调用。
...
然后将 kova 配置中的模型名称更新为 `gemma4-16k`。
...
一旦 kova 在 CLI 中本地运行正常，你可以将其作为 Telegram 或 Discord 机器人对外提供服务——仍完全运行在你的硬件上。

### Telegram

1. 通过 [@BotFather](https://t.me/BotFather) 创建机器人并获取 token
2. 添加到 `~/.kova/config.yaml`：
...
- **kova 具备自动修复功能** —— 它能检测格式错误的工具调用并自动尝试修复。
- **设置回退方案** —— 如果本地模型连续失败 3 次，kova 将回退到云端提供商。
