...
kova 将此称为**无 agent 模式**。这是去掉 LLM 的 cron 系统。
...
- **Bash 或 Python。** `.sh` / `.bash` 文件在 `/bin/bash` 下运行；其他扩展名在当前 Python 解释器下运行。`~/.kova/scripts/` 中的任何文件均可接受。
...
无 agent 模式的真正优势在于：agent 本身可以为你设置看门狗——无需编辑器、无需 shell、无需记忆 CLI 参数。你描述需求，kova 编写脚本、安排计划，并告知你何时触发。

### 示例对话

> **你：** 每 5 分钟检查一次，如果内存超过 85% 就在 telegram 通知我
>
> **Kova：** *（写入 `~/.kova/scripts/memory-watchdog.sh`，然后以 `no_agent=true` 调用 `cronjob(...)`）*
>
...
当你的请求类似"当 X 时告警我"或"每 N 分钟检查 Y，如果 Z 则通知我"，kova 的 `cronjob` 工具描述会指示它在消息内容完全由脚本决定时使用 `no_agent=True`。当请求需要推理时（*"总结新 issue"*、*"挑选最有趣的头条"*、*"起草一条友好提醒"*），则回退到普通的 LLM 驱动路径。
...
> **你：** 今晚先停掉内存看门狗
>
> **Kova：** *（调用 `cronjob(action='pause', job_id='abc123')`）*
>
> 已暂停。说"重新开启"或执行 `kova cron resume abc123` 可恢复。

> **你：** 改成每 15 分钟一次
>
> **Kova：** *（调用 `cronjob(action='update', job_id='abc123', schedule='every 15m')`）*
...
脚本必须位于 `~/.kova/scripts/`。这在任务创建时和运行时均会强制检查——绝对路径、`~/` 展开以及路径穿越模式（`../`）均会被拒绝。该目录与 LLM 任务使用的预检脚本门控共享。
...
```bash
kova cron create "every 5m"        # 间隔
kova cron create "every 2h"
kova cron create "0 9 * * *"       # 标准 cron：每天上午 9 点
kova cron create "30m"             # 单次：30 分钟后运行一次
```
...
--deliver local                          # 仅保存到 ~/.kova/cron/output/
```

对于使用 bot token 的平台（Telegram、Discord、Slack、Signal、SMS、WhatsApp），脚本运行时无需运行中的 gateway——工具直接使用 `~/.kova/.env` / `~/.kova/config.yaml` 中已有的凭据调用各平台的 REST 端点。

## 编辑与生命周期

```bash
kova cron list                                    # 查看所有任务
kova cron pause <job_id>                          # 停止触发，保留定义
kova cron resume <job_id>
kova cron edit <job_id> --schedule "every 10m"    # 调整频率
kova cron edit <job_id> --agent                   # 切换为 LLM 模式
kova cron edit <job_id> --no-agent --script …     # 切换回无 agent 模式
kova cron remove <job_id>                         # 删除任务
```
...
| 方式 | 运行内容 | 适用场景 |
|----------|-----------|-------------|
| `cronjob --no-agent`（本页） | 你的脚本，由 kova 调度 | 不需要推理的周期性看门狗 / 告警 / 指标 |
| `cronjob`（默认，LLM） | 带可选预检脚本的 agent | 消息内容需要对数据进行推理时 |
| OS cron + `curl` 到 [webhook 订阅](/user-guide/messaging/webhooks) | 你的脚本，由 OS 调度 | 当 kova 本身可能不健康时（即被监控对象） |

对于必须在 **gateway 宕机时也能触发**的关键系统健康看门狗，请使用 OS 级 cron 配合 `curl` 调用 kova webhook 订阅（或任何外部告警端点）——这些作为独立 OS 进程运行，不依赖 kova 是否在线。当被监控对象是外部系统时，in-gateway 调度器才是正确选择。
