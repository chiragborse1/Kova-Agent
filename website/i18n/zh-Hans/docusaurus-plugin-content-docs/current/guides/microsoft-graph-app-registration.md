...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.kova/.env`。
...
4. 填写以下内容：
   - **Name：**`kova Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the kova app
New-CsApplicationAccessPolicy `
  -Identity "kova-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict kova meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "kova-Meeting-Pipeline-Policy" `
...
kova 内置了 Graph 身份验证冒烟测试。在 kova 安装目录下执行：
...
2. 用新值更新 `~/.kova/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
3. 重启 gateway 以使新密钥生效：`Kova Gateway restart`。
4. 使用上述冒烟测试进行验证。
5. 在 Azure 门户中删除旧密钥。
