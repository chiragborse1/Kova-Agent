...
sidebar_position: 15
title: "MiniMax OAuth"
description: "通过浏览器 OAuth 登录 MiniMax，在 Kova Agent 中使用 MiniMax-M2.7 模型——无需 API 密钥"
---

# MiniMax OAuth

Kova Agent 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，kova 即可自动刷新您的会话。
...
首次登录后，凭据将存储在 `~/.kova/auth.json` 下，并在每次会话前自动刷新。
...
```bash
kova auth add minimax-cn --type oauth   # 如果您的中国账户支持 OAuth
# 或更简单的方式：
...
kova 将打印验证 URL 和用户码——在任意设备上打开该 URL，并在提示时输入用户码。

## OAuth 流程

kova 针对 MiniMax OAuth 端点实现了 PKCE 设备码流程：

1. kova 生成 PKCE verifier/challenge 对和一个随机 state 值。
...
4. kova 轮询 `{base_url}/oauth/token`，直到令牌到达（或超过截止时间）。
5. 令牌（`access_token`、`refresh_token`、过期时间）以 `minimax-oauth` 为键保存到 `~/.kova/auth.json`。
...
```bash
kova config set model MiniMax-M2.7
kova config set provider minimax-oauth
```

## 配置参考

登录后，`~/.kova/config.yaml` 将包含类似如下的条目：
...
```bash
kova --provider minimax-oauth    # 规范名称
kova --provider minimax-portal   # 别名
kova --provider minimax-global   # 别名
kova --provider minimax_oauth    # 别名（下划线形式）
```
...
要将 `minimax-oauth` 设为活跃 provider，请在 `config.yaml` 中设置 `model.provider: minimax-oauth`（使用 `kova setup` 进行引导式配置），或在单次调用时传入 `--provider minimax-oauth`：
...
kova 在每次会话启动时，若 access token 距过期不足 60 秒则刷新令牌。如果 access token 已经过期（例如长时间离线后），刷新将在下一次请求时自动触发。如果刷新失败并返回 `refresh_token_reused` 或 `invalid_grant`，kova 会将会话标记为需要重新登录。

当刷新失败为终态（HTTP 4xx、`invalid_grant`、授权已撤销等）时，kova 将 refresh token 标记为失效并在本地隔离，避免持续重放注定失败的交换。Agent 会显示一条"需要重新认证"的消息，并在您再次登录之前保持等待。

**解决方法：** 再次运行 `kova auth add minimax-oauth` 以开始全新登录。下一次成功交换后隔离状态将自动清除。

### 授权超时

设备码流程有有限的过期窗口。如果您未在规定时间内批准登录，kova 将抛出超时错误。

**解决方法：** 重新运行 `kova auth add minimax-oauth`（或 `kova model`）。流程将重新开始。

### State 不匹配（可能的 CSRF）

kova 检测到授权服务器返回的 `state` 值与其发送的值不匹配。

**解决方法：** 重新运行登录。如果问题持续，请检查是否有代理或重定向正在修改 OAuth 响应。

### 从远程服务器登录

如果 `kova` 无法打开浏览器窗口，请使用 `--no-browser`：

```bash
kova auth add minimax-oauth --no-browser
```

kova 将打印 URL 和用户码。在任意设备上打开该 URL 并在那里完成流程。

### 运行时出现"未登录 MiniMax OAuth"错误

auth 存储中没有 `minimax-oauth` 的凭据。您尚未登录，或凭据文件已被删除。

**解决方法：** 运行 `kova model` 并选择 MiniMax (OAuth)，或运行 `kova auth add minimax-oauth`。
