---
sidebar_position: 8
title: "在 kova 中使用语音模式"
description: "在 CLI、Telegram、Discord 及 Discord 语音频道中设置和使用 kova 语音模式的实用指南"
---
...
如果你在设置向导中选择了 NeuTTS，kova 会检查 `neutts` 是否已安装。如果缺失，向导会告知你 NeuTTS 需要 Python 包 `neutts` 和系统包 `espeak-ng`，并提供自动安装，使用平台包管理器安装 `espeak-ng`，然后运行：
...
如果打字不方便，语音模式是保持完整 kova 工作流的最快方式之一。

## 调整 CLI 行为

### 静音阈值

如果 kova 开始/停止过于激进，调整：
...
kova 加入 Discord 语音频道（VC），监听用户语音，转录后运行正常的 agent 流水线，并将回复朗读回频道。
