...
description: "将 kova 连接到 GitHub，使其自动获取 PR diff、审查代码变更并发布评论——由 webhook 触发，无需手动提示"
---

# 使用 Webhook 自动发布 GitHub PR 评论

本指南介绍如何将 Kova Agent 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。

当 PR 被打开或更新时，GitHub 会向你的 kova 实例发送一个 webhook POST 请求。kova 使用一个 prompt 运行 agent，该 prompt 指示其通过 `gh` CLI 获取 diff，并将响应发布回 PR 线程。
...
- Kova Agent 已安装并运行（`Kova Gateway`）
- [`gh` CLI](https://cli.github.com/) 已安装并在 gateway 主机上完成认证（`gh auth login`）
- 你的 kova 实例有一个可公网访问的 URL（如果在本地运行，请参阅[使用 ngrok 进行本地测试](#local-testing-with-ngrok)）
...
创建一个分支，推送一个变更，并打开一个 PR。在 30–90 秒内（取决于 PR 大小和模型），kova 应该会发布一条审查评论。
...
如果 kova 在你的笔记本上运行，使用 [ngrok](https://ngrok.com/) 将其暴露到公网：
...
:::note
`kova webhook test <name>` 仅适用于通过 `kova webhook subscribe` 创建的**动态订阅**。它不读取 `config.yaml` 中的路由。
:::
...
加载一个 [kova skill](/user-guide/features/skills) 以赋予 agent 一致的审查风格。在 `config.yaml` 的 `platforms.webhook.extra.routes` 中，向你的路由添加 `skills`：
...
> **注意：** 列表中只有第一个找到的 skill 会被加载。kova 不会叠加多个 skill——后续条目会被忽略。
...
同一适配器也适用于 GitLab。GitLab 使用 `X-Gitlab-Token` 进行认证（纯字符串匹配，非 HMAC）——kova 会自动处理两者。
...
GitLab 的 payload 字段与 GitHub 不同——例如，MR 标题使用 `{object_attributes.title}`，MR 编号使用 `{object_attributes.iid}`。发现完整 payload 结构最简单的方式是使用 GitLab webhook 设置中的 **Test** 按钮，结合 **Recent Deliveries** 日志。或者，在路由配置中省略 `prompt`——kova 将把完整 payload 作为格式化 JSON 直接传递给 agent，agent 的响应（在 gateway 日志中通过 `deliver: log` 可见）将描述其结构。
...
- **[构建 Plugin](/guides/build-a-kova-plugin)** —— 将审查逻辑打包为可共享的 plugin
