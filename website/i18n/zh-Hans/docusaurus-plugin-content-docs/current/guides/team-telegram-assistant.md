...
本教程将引导你搭建一个由 Kova Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，你的团队将拥有一个共享 AI 助手，可以向它发消息寻求代码、研究、系统管理等方面的帮助——并通过按用户授权保障安全。
...
- **已在服务器或 VPS 上安装 Kova Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/getting-started/installation)。
- **一个 Telegram 账号**（机器人所有者）
- **已配置 LLM 提供商**——至少在 `~/.kova/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥

:::tip
一台 $5/月的 VPS 足以运行 gateway（网关）。kova 本身很轻量——花钱的是 LLM API 调用，而那些调用发生在远端。
:::
...
sudo Kova Gateway install --system   # 仅 Linux：开机启动的系统服务
```
...
:::tip macOS PATH
launchd plist 在安装时捕获你的 Shell PATH，以便 gateway 子进程能找到 Node.js 和 ffmpeg 等工具。如果之后安装了新工具，请重新运行 `Kova Gateway install` 以更新 plist。
:::
...
控制机器人在使用工具时显示的详细程度。在 `~/.kova/config.yaml` 中：
...
```bash
# 通过 CLI
kova cron list          # 查看所有定时任务
kova cron status        # 检查调度器是否运行
...
| 内容 | 位置 |
|------|----------|
| Gateway 日志 | `journalctl --user -u kova-gateway`（Linux）或 `~/.kova/logs/gateway.log`（macOS） |
| Cron 任务输出 | `~/.kova/cron/output/{job_id}/{timestamp}.md` |
| Cron 任务定义 | `~/.kova/cron/jobs.json` |
| 配对数据 | `~/.kova/pairing/` |
| 会话历史 | `~/.kova/sessions/` |
