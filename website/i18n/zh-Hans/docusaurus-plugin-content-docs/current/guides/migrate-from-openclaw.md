...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 Kova Agent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 OpenClaw 迁移

`kova claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 Kova。本指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
| `--no-backup` | 跳过迁移前对 `~/.kova/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.kova/backups/pre-migration-*.zip`；可通过 `kova import` 还原）。 |
...
| 内容 | OpenClaw 来源 | kova 目标 | 备注 |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.kova/SOUL.md` | 直接复制 |
| 工作区指令 | `workspace/AGENTS.md` | `--workspace-target` 中的 `AGENTS.md` | 需要 `--workspace-target` 标志 |
| 长期记忆 | `workspace/MEMORY.md` | `~/.kova/memories/MEMORY.md` | 解析为条目，与现有内容合并并去重，使用 `§` 分隔符 |
| 用户档案 | `workspace/USER.md` | `~/.kova/memories/USER.md` | 与记忆相同的条目合并逻辑 |
| 每日记忆文件 | `workspace/memory/*.md` | `~/.kova/memories/MEMORY.md` | 所有每日文件合并至主记忆 |
...
| 来源 | OpenClaw 位置 | kova 目标 |
|--------|------------------|-------------------|
| 工作区 skills | `workspace/skills/` | `~/.kova/skills/openclaw-imports/` |
| 托管/共享 skills | `~/.openclaw/skills/` | `~/.kova/skills/openclaw-imports/` |
| 个人跨项目 skills | `~/.agents/skills/` | `~/.kova/skills/openclaw-imports/` |
| 项目级共享 skills | `workspace/.agents/skills/` | `~/.kova/skills/openclaw-imports/` |

Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 kova skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | kova 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| Provider API 密钥 | `models.providers.*.apiKey` | `~/.kova/.env` | 需要 `--migrate-secrets`。参见下方 [API 密钥解析](#api-key-resolution) |

### Agent 行为

| 内容 | OpenClaw 配置路径 | kova 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
| TTS 资源文件 | `~/.kova/tts/`（文件复制） |

### 消息平台

| 平台 | OpenClaw 配置路径 | kova `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
以下内容保存至 `~/.kova/migration/openclaw/<timestamp>/archive/` 供人工审查：

| 内容 | 归档文件 | 在 kova 中的重建方式 |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | 合并至 `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | kova 内置工具说明 |
| `HEARTBEAT.md` | `archive/workspace/HEARTBEAT.md` | 使用 cron 作业执行周期性任务 |
| `BOOTSTRAP.md` | `archive/workspace/BOOTSTRAP.md` | 使用上下文文件或 skills |
| Cron 作业 | `archive/cron-config.json` | 通过 `kova cron create` 重建 |
| 插件 | `archive/plugins-config.json` | 参见 [插件指南](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | 使用 `kova webhook` 或 gateway hooks |
| 记忆后端 | `archive/memory-backend-config.json` | 通过 `kova honcho` 配置 |
| Skills 注册表 | `archive/skills-registry-config.json` | 使用 `kova skills config` |
| UI/身份 | `archive/ui-identity-config.json` | 使用 `/skin` 命令 |
| 日志 | `archive/logging-diagnostics-config.json` | 在 `config.yaml` 日志部分设置 |
| 多 Agent 列表 | `archive/agents-list.json` | 使用 kova profiles |
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，会从 `~/.openclaw/.env` 和 `openclaw.json` 的 env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会对此发出警告，相关值需通过 `kova config set` 手动添加至 Kova。

## 迁移后

1. **检查迁移报告** — 完成后打印，包含已迁移、已跳过和冲突项的计数。

2. **审查归档文件** — `~/.kova/migration/openclaw/<timestamp>/archive/` 中的所有内容需要人工处理。
...
7. **重新配对 WhatsApp** — WhatsApp 使用二维码配对（Baileys），不支持 token 迁移。运行 `kova whatsapp` 进行配对。

8. **清理归档** — 确认一切正常后，运行 `kova claw cleanup` 将残留的 OpenClaw 目录重命名为 `.pre-migration/`（防止状态混淆）。
...
根据 OpenClaw 版本不同，密钥可能存储在多个位置：`openclaw.json` 中 `models.providers.*.apiKey` 内联、`~/.openclaw/.env`、`openclaw.json` 的 `"env"` 子对象，或 `agents/main/agent/auth-profiles.json`。迁移会检查所有四个位置。若密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `kova config set` 手动添加。

### 迁移后 skills 未出现

导入的 skills 位于 `~/.kova/skills/openclaw-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。

### TTS 语音未迁移

OpenClaw 在两处存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会检查两处。若你的 voice ID 是通过 OpenClaw UI 设置的（存储路径不同），可能需要手动设置：`kova config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。