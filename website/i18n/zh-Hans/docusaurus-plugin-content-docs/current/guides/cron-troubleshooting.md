...
sidebar_position: 12
title: "Cron 故障排查"
description: "诊断并修复常见的 kova cron 问题——任务未触发、投递失败、skill 加载错误及性能问题"
---
...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `Kova Gateway`，安装为服务用 `Kova Gateway start`）。如需单次调试，可手动触发一次 tick：`kova cron tick`。
...
```bash
date
kova cron list   # 将 next_run 时间与本地时间对比
```
...
| 目标 | 所需配置 |
|--------|----------|
| `telegram` | `~/.kova/.env` 中的 `TELEGRAM_BOT_TOKEN` |
| `discord` | `~/.kova/.env` 中的 `DISCORD_BOT_TOKEN` |
| `slack` | `~/.kova/.env` 中的 `SLACK_BOT_TOKEN` |
| `whatsapp` | 已配置 WhatsApp gateway |
| `signal` | 已配置 Signal gateway |
| `matrix` | 已配置 Matrix homeserver |
| `email` | `config.yaml` 中已配置 SMTP |
| `sms` | 已配置 SMS 提供商 |
| `local` | 对 `~/.kova/cron/output/` 有写权限 |
...
若投递失败，任务仍会执行——只是不会发送到任何地方。检查 `kova cron list` 中的 `last_error` 字段（如有）。
...
Skill 必须先安装才能附加到 cron 任务。若 skill 缺失，先用 `kova skills install <skill-name>` 安装，或在 CLI 中通过 `/skills` 安装。

### 检查 2：检查 skill 名称与 skill 文件夹名称

Skill 名称区分大小写，必须与已安装 skill 的文件夹名称完全匹配。若任务指定的是 `ai-funding-daily-report`，但 skill 文件夹也是 `ai-funding-daily-report`，请从 `kova skills list` 确认确切名称。
...
1. 任务投递的聊天会话（若投递成功）
2. `~/.kova/logs/agent.log`（调度器消息）或 `errors.log`（警告信息）
3. 通过 `kova cron list` 查看任务的 `last_run` 元数据

### 检查 2：常见错误模式

**脚本报 "No such file or directory"**
`script` 路径必须为绝对路径（或相对于 kova 配置目录的路径）。验证：
```bash
ls ~/.kova/scripts/your-script.py   # 必须存在
kova cron edit <job_id> --script ~/.kova/scripts/your-script.py
```

**任务执行时报 "Skill not found"**
Skill 必须安装在运行调度器的机器上。若你在不同机器间切换，skill 不会自动同步——请用 `kova skills install <skill-name>` 重新安装。

**任务运行但没有投递任何内容**
可能是投递目标问题（见上方"投递失败"部分）或响应被静默抑制（`[SILENT]`）。

**任务挂起或超时**
调度器使用基于不活跃时间的超时机制（默认 600 秒，可通过 `KOVA_CRON_TIMEOUT` 环境变量配置，`0` 表示无限制）。只要 agent 持续调用工具，就可以一直运行——计时器仅在持续不活跃后触发。长时间运行的任务应使用脚本处理数据采集，仅将结果投递出去。
...
任务存储在 `~/.kova/cron/jobs.json`。若该文件对当前用户不可读写，调度器将静默失败：

```bash
ls -la ~/.kova/cron/jobs.json
chmod 600 ~/.kova/cron/jobs.json   # 应由你的用户拥有
```
...
```bash
kova cron list                    # 显示所有任务、状态、next_run 时间
kova cron run <job_id>            # 安排在下次 tick 执行（用于测试）
kova cron edit <job_id>           # 修复配置问题
kova logs                         # 查看近期 kova 日志
kova skills list                  # 确认已安装的 skill
```
...
1. 使用 `kova cron run <job_id>` 运行任务（在下次 gateway tick 时触发），观察聊天输出中的错误
2. 查看 `~/.kova/logs/agent.log` 中的调度器消息和 `~/.kova/logs/errors.log` 中的警告
3. 在 [github.com/NousResearch/Kova-agent](https://github.com/NousResearch/Kova-agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
