...
常见自动化模式的复制粘贴蓝图。每个蓝图使用 kova 内置的 [cron 调度器](/user-guide/features/cron) 实现基于时间的触发，使用 [webhook 平台](/user-guide/messaging/webhooks) 实现事件驱动触发。
...
| **GitHub 事件** | PR 开启、推送、issue、CI 结果时触发 | Webhook 平台（`kova webhook subscribe`） |
| **API 调用** | 外部服务向你的端点 POST JSON | Webhook 平台（config.yaml 路由或 `kova webhook subscribe`） |
