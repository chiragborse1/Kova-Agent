...
本页涵盖所有与 [kova profiles](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
管理 profile 的顶级命令。不带子命令运行 `kova profile` 将显示帮助信息。
...
将 `<name>` 设为活跃 profile。此后所有 `kova` 命令（不带 `-p`）都将使用该 profile。
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `kova profile describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空** profile，不启用任何内置 skill。会在 profile 目录中写入 `.no-bundled-skills` 标记，使后续 `kova update` 不再重新植入内置 skill 集，且拒绝与 `--clone`、`--clone-from` 或 `--clone-all` 组合使用（因为这些选项会复制 skill）。适用于不应继承完整 skill 目录的窄化编排器 profile 或沙箱 profile。 |
...
此处显示的是 profile 的 kova 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 启动（或在本地后端 `cwd: "."` 时从启动目录启动）。
...
重新生成位于 `~/.local/bin/<name>` 的 shell alias 脚本。适用于 alias 被意外删除，或移动 kova 安装目录后需要更新的情况。
...
:::info
`kova profile export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::
...
| `--alias` | 同时创建 shell wrapper（例如 `telemetry` → `kova -p telemetry`）。 |
...
打印 profile 的发行版 manifest — 名称、版本、所需 kova 版本、作者、环境变量要求、来源 URL/路径，以及发行版最后一次 `install` 或 `update` 时记录的 `Installed:` 时间戳。适用于安装前检查共享 profile 的需求，以及发现"该 profile 已安装 6 个月未更新"等情况。

`kova profile list` 也会在 `Distribution` 列中显示发行版名称和版本，`kova profile show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
`KOVA_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 kova 版本不满足规格，安装将失败并给出明确错误。
...
2. 初始化 git 仓库（或使用已有仓库），推送到 GitHub / GitLab / 任何 kova 可克隆的托管平台。
3. 告知接收方运行 `kova profile install <your-repo-url>`。
...
全局标志，用于在不更改默认 profile 的情况下，在指定 profile 下运行任意 kova 命令。仅在该命令执行期间覆盖活跃 profile。
...
安装后，Tab 补全适用于：
- `kova profile <TAB>` — 子命令（list、use、create 等）
- `kova profile use <TAB>` — profile 名称
- `kova -p <TAB>` — profile 名称
