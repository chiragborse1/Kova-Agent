...
sidebar_position: 3
title: "常见问题与故障排查"
description: "Kova Agent 常见问题解答及常见问题解决方案"
---
...
Kova Agent 可与任何兼容 OpenAI 的 API 配合使用。支持的提供商包括：
...
- **Anthropic** — Claude 模型（直接 API、通过 `kova auth add anthropic` 进行 OAuth、OpenRouter 或任何兼容代理）
...
使用 `kova model` 设置提供商，或直接编辑 `~/.kova/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。

### 支持 Windows 吗？

**原生不支持。** Kova Agent 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 Kova。标准安装命令在 WSL2 中可完美运行：

```bash
curl -fsSL https://kova-agent.nousresearch.com/install.sh | bash
```

### 我在 WSL2 中运行 Kova，如何控制 Windows 上的普通 Chrome？
...
- 让 kova 使用生成的 MCP 浏览器工具

这比强制 kova 核心浏览器传输直接跨越 WSL2/Windows 边界进行附加更为可靠。
...
支持 — kova 现已为 Android 手机提供经过测试的 Termux 安装路径。
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。Kova Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.kova/` 目录中。

### 可以离线使用 / 使用本地模型吗？

可以。运行 `kova model`，选择**自定义端点**，然后输入您服务器的 URL：
...
kova 会将端点、提供商和 base URL 持久化到 `config.yaml`，重启后仍然有效。如果您的本地服务器只加载了一个模型，`/model custom` 会自动检测到它。您也可以在 config.yaml 中设置 `provider: custom` — 这是一个一等提供商，不是其他任何东西的别名。

此方式适用于 Ollama、vLLM、llama.cpp server、SGLang、LocalAI 等。详情请参阅[配置指南](../user-guide/configuration.md)。

:::tip Ollama 用户
如果您在 Ollama 中设置了自定义 `num_ctx`（例如 `ollama run --num_ctx 16384`），请确保在 kova 中设置匹配的上下文长度 — Ollama 的 `/api/show` 报告的是模型的*最大*上下文，而非您配置的实际 `num_ctx`。
:::
...
Kova Agent 本身**免费且开源**（MIT 许可证）。您只需为所选提供商的 LLM API 用量付费。本地模型完全免费运行。

### 多人可以使用同一个实例吗？

可以。[消息网关](../user-guide/messaging/index.md)允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 Kova Agent 实例交互。访问权限通过白名单（特定用户 ID）和私信配对（第一个发消息的用户获得访问权）来控制。
...
**原因：** kova 在启动时通过运行一次 `bash -l` 构建每个会话的环境快照。bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`** — 因此在 `~/.bashrc` 中安装自身的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义 `PATH` 导出）对快照不可见。当 kova 在 systemd 下运行或在未预加载交互式 shell 配置的最小 shell 中运行时，此问题最为常见。

**解决方案：** kova 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.kova/config.yaml` 中列出需要额外 source 的文件：
...
**解决方案：** 退出当前会话，在终端中使用 `kova model` 添加新提供商：

```bash
# 先退出 kova 聊天会话（Ctrl+C 或 /quit）
...
通过 `kova model` 添加新提供商后，启动新的聊天会话 — `/model` 将显示所有已配置的提供商。

:::tip 快速参考
| 目标 | 使用方式 |
|-----------|-----|
| 添加新提供商 | `kova model`（从终端） |
| 输入/更改 API key | `kova model`（从终端） |
...
:::warning
请确保 key 与提供商匹配。OpenAI 的 key 无法用于 OpenRouter，反之亦然。检查 `~/.kova/.env` 中是否有冲突条目。
:::
...
- 升级您的提供商套餐
- 切换到其他模型或提供商
- 使用 `kova chat --provider <alternative>` 路由到其他后端

#### 上下文长度超限

**原因：** 对话内容超出模型的上下文窗口，或 kova 检测到的模型上下文长度有误。
...
如果在第一次长对话时就出现此问题，kova 可能检测到了错误的模型上下文长度。检查检测结果：
...
**原因：** kova 检测到潜在的破坏性命令（例如 `rm -rf`、`DROP TABLE`）。这是一项安全功能。
...
:::tip
这是预期行为 — kova 绝不会静默执行破坏性命令。审批提示会向您显示将要执行的确切内容。
:::
...
**解决方案：**
- 使用 `Kova Gateway setup` 验证您的 bot token 是否有效
- 检查网关日志：`cat ~/.kova/logs/gateway.log | tail -50`
...
在 `~/.kova/config.yaml` 中您的网关设置下进行配置。请参阅[消息文档](../user-guide/messaging/index.md)。
...
```bash
# 安装核心消息网关依赖项
cd ~/.kova/kova-agent && uv pip install -e ".[messaging]"  # Telegram、Discord、Slack 及共享网关依赖
...
#### WSL：网关持续断开连接或 `Kova Gateway start` 失败
...
1. 创建一个任务，运行 `wsl -d Ubuntu -- bash -lc 'Kova Gateway run'`
2. 设置在用户登录时触发
:::
...
**解决方案：** 网关在您运行 `Kova Gateway install` 时会捕获您的 shell PATH。如果您在设置网关后安装了新工具，请重新运行 install 以捕获更新后的 PATH：

```bash
Kova Gateway install    # 重新快照当前 PATH
Kova Gateway start      # 检测到更新的 plist 并重新加载
```
...
**解决方案：**
- 尝试更快/更小的模型：`kova chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- 减少激活的工具集：`kova chat -t "terminal"`
- 检查到提供商的网络延迟
...
验证您的 `~/.kova/config.yaml` 中的 MCP 配置：
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
:::warning
如果 MCP 服务器在请求中途崩溃，kova 会报告超时。请检查服务器自身的日志（而非仅 kova 日志）以诊断根本原因。
:::
...
### Profiles 与直接设置 KOVA_HOME 有何不同？

Profiles 是构建在 `KOVA_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `KOVA_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`kova-work`）、在 `~/.kova/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `kova profile create newname --clone-all` 从当前 profile 复制所有内容，或添加 `--clone-from <profile>` 从指定源 profile 复制。

### 运行 `kova update` 时会发生什么？

`kova update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次 `kova update` — 它覆盖机器上的每个 profile。

### 可以运行多少个 profiles？

没有硬性限制。每个 profile 只是 `~/.kova/profiles/` 下的一个目录。实际限制取决于您的磁盘空间以及系统能处理多少个并发网关（每个网关是一个轻量级 Python 进程）。运行数十个 profiles 完全没问题；每个空闲的 profile 不占用任何资源。
...
**解决方案：委托配置。** kova 可以自动将子智能体路由到不同的模型。在 `~/.kova/config.yaml` 中设置：
...
现在当您告诉 kova "帮我写一个关于 X 的 Twitter 帖子"并生成 `delegate_task` 子智能体时，该子智能体将在 Gemini 上运行，而非您的主模型。您的主对话仍在 GPT-5.4 上进行。
...
**场景：** 在 OpenClaw 中，您可以将多个独立智能体绑定到特定的 WhatsApp 聊天 — 一个用于家庭购物清单群组，另一个用于您的私聊。kova 能做到吗？

**当前限制：** kova 的每个 profile 都需要自己的 WhatsApp 号码/会话。您无法将多个 profiles 绑定到同一个 WhatsApp 号码上的不同聊天 — WhatsApp bridge（Baileys）每个号码使用一个已认证的会话。
...
**场景：** 您在 Telegram 中看到了网关执行日志、kova 推理过程和工具调用详情，而不是最终输出。
...
**场景：** Telegram 有 100 个 slash 命令的限制，您的技能数量已超过此限制。您想禁用 Telegram 上不需要的技能，但 `kova skills config` 设置似乎没有生效。

**解决方案：** 使用 `kova skills config` 按平台禁用技能。这会写入 `config.yaml`：
...
更改后，**重启网关**（`Kova Gateway restart` 或终止并重新启动）。Telegram bot 命令菜单在启动时重建。
...
**当前行为：** kova 在大多数平台上按用户 ID 创建会话，因此每个人都有自己的对话上下文。这是出于隐私和上下文隔离的设计考量。
...
2. 在**源机器**上创建完整备份：
   ...
   kova backup
   ```
   这会将您整个 `~/.kova/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/kova-backup-<timestamp>.zip`。
...
### `kova backup` 与 `kova profile export` 的对比

| 功能 | `kova backup` | `kova profile export` |
| :--- | :--- | :--- |
| **使用场景** | **整机迁移** | **移植/共享特定 profile** |
| **范围** | 全局（整个 `~/.kova` 目录） | 局部（单个 profile 目录） |
...
:::tip
`kova backup` 即使在 kova 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::

### 安装后重新加载 shell 时出现权限拒绝

**场景：** 运行 kova 安装程序后，`source ~/.zshrc` 提示权限拒绝错误。

**原因：** 这通常发生在 `~/.zshrc`（或 `~/.bashrc`）文件权限不正确，或安装程序无法干净写入时。这不是 kova 特有的问题 — 而是 shell 配置权限问题。
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/NousResearch/Kova-agent/issues)
