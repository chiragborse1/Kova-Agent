...
description: "通过 Termux 在 Android 手机上直接运行 Kova Agent"
---

# 在 Android 上通过 Termux 运行 kova

这是在 Android 手机上通过 [Termux](https://termux.dev/) 直接运行 Kova Agent 的已验证路径。
...
这并不妨碍 kova 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。
...
- 将 `kova` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### `kova doctor` 提示缺少 ripgrep 或 Node
