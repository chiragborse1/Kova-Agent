...
description: "使用 Nix 安装和部署 Kova Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Kova Agent 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`kova setup`、`Kova Gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 kova 的方式与管理其他 NixOS 服务相同。
:::
...
就这些。`nixos-rebuild switch` 会创建 `kova` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `kova` CLI 添加到系统 PATH，**并**在系统范围内设置 `KOVA_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `kova` 会创建独立的 `~/.kova/` 目录。
:::

### 容器感知 CLI

:::info
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**所有** `kova` 命令都会自动路由到托管容器中执行。这意味着你的交互式 CLI 会话在与 gateway 服务相同的环境中运行——可以访问所有容器内安装的包和工具。
...
`hostUsers` 中列出的用户会自动加入 `kova` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo kova chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，kova 默认使用 OpenRouter。
:::
...
这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$KOVA_HOME/config.yaml`。
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$KOVA_HOME/.env` 中。kova 在每次启动时读取此文件，因此更改在 `systemctl restart kova-agent` 后生效——无需重建容器。
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。kova 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：kova 从 `$KOVA_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.kova-agent.stateDir}/.kova/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$KOVA_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。kova 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$KOVA_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，kova 将授权 URL 打印到 stdout/日志，而不是打开浏览器。

**方案 A：交互式引导** — 通过 `docker exec`（容器）或 `sudo -u kova`（原生）运行一次流程：
...
当 kova 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `kova setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `kova config edit` | 配置由 `settings` 生成 |
| `kova config set <key> <value>` | 配置由 `settings` 生成 |
| `Kova Gateway install` | systemd 服务由 NixOS 管理 |
| `Kova Gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`KOVA_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `KOVA_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it kova-agent kova config set ...` 也会被屏蔽）
...
`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 kova 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `kova plugins install`。
...
插件在激活时以符号链接方式安装到 `$KOVA_HOME/plugins/`。kova 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
该包的 `site-packages` 会添加到 kova wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。
...
:::note
构建时冲突检查可防止插件包覆盖核心 kova 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
| `stateDir` | `str` | `"/var/lib/kova"` | 状态目录（`KOVA_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `kova` CLI 添加到系统 PATH 并在系统范围内设置 `KOVA_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$KOVA_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `Kova Gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 kova 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$KOVA_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
