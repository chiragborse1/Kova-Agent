...
kova 工具是自注册函数，按 toolset（工具集）分组，并通过中央注册表/调度系统执行。
...
2. **插件工具** — `KOVA_cli.plugins.discover_plugins()` 加载用户/项目/pip 插件，这些插件可能注册额外的工具。
...
Toolset 是工具的命名集合。kova 通过以下方式解析它们：

- 显式启用/禁用的 toolset 列表
- 平台预设（`kova-cli`、`kova-telegram` 等）
- 动态 MCP toolset
- 精选的特殊用途集合，如 `kova-acp`
