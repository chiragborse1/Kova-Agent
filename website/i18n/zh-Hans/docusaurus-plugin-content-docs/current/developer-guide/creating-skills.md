...
sidebar_position: 3
title: "创建 Skill"
description: "如何为 Kova Agent 创建 skill——SKILL.md 格式、规范与发布"
---

# 创建 Skill

Skill 是为 Kova Agent 添加新能力的首选方式。与 tool 相比，skill 更易于创建，无需修改 agent 代码，且可与社区共享。
...
当 skill 需要 API key 或 token 时，使用 `required_environment_variables`。缺少值**不会**将 skill 从发现列表中隐藏。kova 会在本地 CLI 加载 skill 时安全地提示用户输入。
...
用户可以跳过配置并继续加载 skill。kova 不会将原始密钥值暴露给模型。Gateway 和消息会话会显示本地配置指引，而不是在带内收集密钥。
...
- `prompt`（可选）——`kova config migrate` 时显示的提示文本；若未设置则回退到 `description`
...
2. **发现：** `kova config migrate` 扫描所有已启用的 skill，找出未配置的项并提示用户。配置项也会在 `kova config show` 的"Skill Settings"部分显示。
...
优先使用标准库 Python、curl 以及现有 kova tool（`web_extract`、`terminal`、`read_file`）。若确实需要依赖项，请在 skill 中记录安装步骤。
...
内置 skill（位于 `skills/`）随每次 kova 安装一起发布，应对**大多数用户广泛有用**：

- 文档处理、网页研究、常见开发工作流、系统管理
- 被广泛人群定期使用

如果你的 skill 是官方的且有用，但并非所有人都需要（例如付费服务集成、重量级依赖），请放入 **`optional-skills/`**——它随仓库一起发布，可通过 `kova skills browse` 发现（标记为"official"），并以内置信任级别安装。

如果你的 skill 是专业化的、社区贡献的或小众的，更适合放在 **Skills Hub**——将其上传到注册表并通过 `kova skills install` 分享。
...
kova 现在可以通过多种外部发现模型使用第三方 skill：
