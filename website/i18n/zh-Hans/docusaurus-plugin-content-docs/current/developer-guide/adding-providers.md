...
1. `KOVA_cli/auth.py` 决定如何查找凭据。
2. `KOVA_cli/runtime_provider.py` 将其转换为运行时数据：
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `KOVA_cli/models.py` 和 `KOVA_cli/main.py` 使 provider 在 CLI 中可见。（`KOVA_cli/setup.py` 自动委托给 `main.py`——无需在此处做任何修改。）
...
:::tip
`KOVA_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `kova setup` 中。
:::
...
2. 就这些。Provider 插件在任何代码首次调用 `get_provider_profile()` 或 `list_providers()` 时自动加载——捆绑插件（本仓库）和位于 `$KOVA_HOME/plugins/model-providers/` 的用户插件都会被加载。
...
位于 `$KOVA_HOME/plugins/model-providers/<name>/` 的用户插件会覆盖同名的捆绑插件（`register_provider()` 中后写者获胜）——因此第三方可以在不编辑本仓库的情况下对任何内置 profile 进行 monkey-patch 或替换。
...
- `KOVA_cli/auth.py` 中的 `PROVIDER_REGISTRY`
- `KOVA_cli/models.py` 中的 `_PROVIDER_LABELS`
- `KOVA_cli/auth.py` 和 `KOVA_cli/models.py` 中的 `_PROVIDER_ALIASES`
- `KOVA_cli/main.py` 中的 CLI `--provider` 选项
- setup / 模型选择分支
- 辅助模型默认值
- 测试
...
## 第 2 步：在 `KOVA_cli/auth.py` 中添加认证元数据
...
## 第 3 步：在 `KOVA_cli/models.py` 中添加模型目录和别名
...
## 第 4 步：在 `KOVA_cli/runtime_provider.py` 中解析运行时数据
...
## 第 5 步：在 `KOVA_cli/main.py` 中接线 CLI

在交互式 `kova model` 流程中出现之前，provider 是不可发现的。

在 `KOVA_cli/main.py` 中更新以下内容：
...
:::tip
`KOVA_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `kova model` 和 `kova setup` 中。
:::
...
- [ ] 在 `KOVA_cli/auth.py` 中添加 `ProviderConfig`
- [ ] 在 `KOVA_cli/auth.py` 和 `KOVA_cli/models.py` 中添加别名
- [ ] 在 `KOVA_cli/models.py` 中添加模型目录
- [ ] 在 `KOVA_cli/runtime_provider.py` 中添加运行时分支
- [ ] 在 `KOVA_cli/main.py` 中添加 CLI 接线（setup.py 自动继承）
