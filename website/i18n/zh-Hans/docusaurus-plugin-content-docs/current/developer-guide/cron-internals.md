...
sidebar_position: 11
title: "Cron 内部机制"
description: "kova 如何存储、调度、编辑、暂停、加载技能以及投递 cron 任务"
---
...
| `KOVA_cli/cron.py` | CLI `kova cron` 子命令 |
...
任务存储在 `~/.kova/cron/jobs.json` 中，采用原子写入语义（先写入临时文件，再重命名）。每条任务记录包含：
...
在 CLI 模式下，cron 任务仅在运行 `kova cron` 命令或活跃 CLI 会话期间触发。
...
2. **环境变量** — `KOVA_CRON_SCRIPT_TIMEOUT`
...
| 本地文件 | `local` | 保存到 `~/.kova/cron/output/` |
...
调度器使用跨进程文件锁（Unix 上的 `fcntl.flock`，Windows 上的 `msvcrt.locking`）防止重叠的 tick 对同一批到期任务执行两次——即使在 gateway 的进程内 ticker 与独立的 `kova cron` / 手动 `tick()` 调用之间也如此。若无法获取锁，`tick()` 立即返回 0。

## CLI 接口

`kova cron` CLI 提供直接的任务管理功能：

```bash
kova cron list                    # 显示所有任务
kova cron create                  # 交互式创建任务（别名：add）
kova cron edit <job_id>           # 编辑任务配置
kova cron pause <job_id>          # 暂停运行中的任务
kova cron resume <job_id>         # 恢复已暂停的任务
kova cron run <job_id>            # 触发立即执行
