...
description: "如何为 Kova Agent 构建 memory provider 插件"
---

# 构建 Memory Provider 插件

Memory provider 插件为 Kova Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。本指南介绍如何构建一个 memory provider 插件。

:::tip
Memory provider 是两种 **provider 插件**类型之一。另一种是 [Context Engine 插件](/developer-guide/context-engine-plugin)，用于替换内置的上下文压缩器。两者遵循相同的模式：单选、配置驱动、通过 `kova plugins` 管理。
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs 始终包含：
          KOVA_HOME (str): 当前活跃的 KOVA_HOME 路径。用于存储数据。
        """
...
| 方法 | 用途 | 是否必须实现？ |
|--------|---------|-----------------|
| `get_config_schema()` | 为 `kova memory setup` 声明配置字段 | **是** |
| `save_config(values, KOVA_HOME)` | 将非敏感配置写入原生位置 | **是**（除非仅使用环境变量） |
...
`get_config_schema()` 返回一个字段描述符列表，供 `kova memory setup` 使用：
...
`get_config_schema()` 中的每个字段都会在 `kova memory setup` 期间提示用户输入。选项较多的 provider 应保持 schema 精简——只包含用户**必须**配置的字段（API key、必要凭证）。可选配置请在配置文件参考文档中说明（例如 `$KOVA_HOME/myprovider.json`），而不是在 setup 向导中逐一提示。这样既能保持 setup 流程简洁，又支持高级配置。可参考 Supermemory provider 的实现——它只提示输入 API key，其余选项均位于 `supermemory.json` 中。
:::
...
Memory provider 插件可以注册自己的 CLI 子命令树（例如 `kova my-provider status`、`kova my-provider config`）。这套系统基于约定发现，无需修改核心文件。
...
4. 你的命令以 `kova <provider-name> <subcommand>` 的形式出现

**仅对活跃 provider 开放：** 你的 CLI 命令只在你的 provider 是配置中活跃的 `memory.provider` 时才会出现。如果用户尚未配置你的 provider，你的命令不会显示在 `kova --help` 中。
...
def register_cli(subparser) -> None:
    """构建 kova my-provider 的 argparse 树。
