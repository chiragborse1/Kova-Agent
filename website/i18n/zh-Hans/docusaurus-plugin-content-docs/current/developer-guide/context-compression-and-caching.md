---
title: 上下文压缩与缓存
description: Kova Agent 如何通过双重压缩系统和 Anthropic prompt 缓存高效管理上下文窗口。
---

# 上下文压缩与缓存

Kova Agent 使用双重压缩系统和 Anthropic prompt（提示词）缓存，在长对话中高效管理上下文窗口用量。
...
通过 `kova plugins` → Provider Plugins → Context Engine 进行配置，或直接编辑 `config.yaml`。
...
Anthropic 每次请求最多允许 4 个 `cache_control` 断点。kova 使用"system_and_3"策略：
