...
ACP 适配器将 kova 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'kova-agent[acp]==<version>' kova-acp` 启动同一适配器，指向 `kova-agent` PyPI 发布包。
...
`acp_adapter/tools.py` 将 kova 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
因此 ACP 通告并使用当前配置的 kova provider/凭据。它还始终通告一个终端 setup 认证方法（`kova-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 kova 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.kova/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `kova-acp` toolset 定义
- `KOVA_cli/main.py` — `kova acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `kova-acp` 脚本