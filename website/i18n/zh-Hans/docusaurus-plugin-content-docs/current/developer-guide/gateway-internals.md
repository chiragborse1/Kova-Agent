...
消息 gateway 是一个长期运行的进程，通过统一架构将 kova 连接到 20 余个外部消息平台。
...
1. `KOVA_cli/commands.py` 中的 `resolve_command()` 将输入映射为规范名称（处理别名、前缀匹配）
...
| 来源 | 提供内容 |
|--------|-----------------|
| `~/.kova/.env` | API 密钥、bot token、平台凭据 |
| `~/.kova/config.yaml` | 模型设置、工具配置、显示选项 |
| 环境变量 | 覆盖上述任意配置 |
...
- **显式目标投递** — `send_message` 工具指定 `telegram:-1001234567890`，或通过 [`kova send` CLI](/guides/pipe-script-output) 封装同一工具供 shell 脚本使用
- **跨平台投递** — 投递至与原始消息不同的平台
...
Hook 从 `gateway/builtin_hooks/`（扩展点 — 当前发行版中为空；`_register_builtin_hooks()` 是一个空操作存根）和 `~/.kova/hooks/`（用户安装）中发现。每个 hook 是一个包含 `HOOK.yaml` 清单和 `handler.py` 的目录。
...
- `Kova Gateway start` / `Kova Gateway stop` — 手动控制
- `systemctl`（Linux）或 `launchctl`（macOS）— 服务管理
- PID 文件位于 `~/.kova/gateway.pid` — 面向 profile 的进程追踪

**Profile 范围 vs 全局**：`start_gateway()` 使用 profile 范围的 PID 文件。`Kova Gateway stop` 仅停止当前 profile 的 gateway。`Kova Gateway stop --all` 使用全局 `ps aux` 扫描来终止所有 gateway 进程（用于更新时）。
