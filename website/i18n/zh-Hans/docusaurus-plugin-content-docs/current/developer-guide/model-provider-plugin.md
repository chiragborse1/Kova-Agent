...
sidebar_position: 10
title: "模型提供商插件"
description: "如何为 Kova Agent 构建模型提供商（推理后端）插件"
---

# 构建模型提供商插件

模型提供商插件声明一个推理后端——兼容 OpenAI 的端点、Anthropic Messages 服务器、Codex 风格的 Responses API，或 Bedrock 原生接口——kova 可通过这些后端路由 `AIAgent` 调用。每个内置提供商（OpenRouter、Anthropic、GMI、DeepSeek、Nvidia……）都以此类插件形式提供。第三方可通过在 `$KOVA_HOME/plugins/model-providers/` 下放置一个目录来添加自己的提供商，无需对仓库做任何修改。
...
1. **内置插件** — `<repo>/plugins/model-providers/<name>/` — 随 kova 一同发布
2. **用户插件** — `$KOVA_HOME/plugins/model-providers/<name>/` — 放入任意目录；后续会话无需重启即可生效
3. **旧版单文件** — `<repo>/providers/<name>.py` — 为树外可编辑安装提供向后兼容

**同名用户插件会覆盖内置插件**，因为 `register_provider()` 采用后写者优先策略。放入 `$KOVA_HOME/plugins/model-providers/gmi/` 目录即可替换内置 GMI profile，无需修改仓库。
...
唯一必需的文件是 `__init__.py`。`plugin.yaml` 供 `kova plugins` 用于自省，以及供通用 PluginManager 将插件路由到正确的加载器；若缺少该文件，通用加载器会回退到源码文本启发式检测。
...
| 集成点 | 位置 | 获得的能力 |
|---|---|---|
| 凭据解析 | `KOVA_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` 从 profile 填充 |
| `--provider` CLI 标志 | `KOVA_cli/main.py` | 接受 `acme-inference` |
| `kova model` 选择器 | `KOVA_cli/models.py` | 出现在 `CANONICAL_PROVIDERS` 中，从 `{base_url}/models` 获取模型列表 |
| `kova doctor` | `KOVA_cli/doctor.py` | 对 `ACME_API_KEY` 及 `{base_url}/models` 进行健康检查 |
| `kova setup` | `KOVA_cli/config.py` | `ACME_API_KEY` 出现在 `OPTIONAL_ENV_VARS` 和设置向导中 |
| URL 反向映射 | `agent/model_metadata.py` | 主机名 → 提供商名称，用于自动检测 |
| 辅助模型 | `agent/auxiliary_client.py` | 使用 `default_aux_model` 进行压缩/摘要 |
| 运行时解析 | `KOVA_cli/runtime_provider.py` | 返回正确的 `base_url`、`api_key`、`api_mode` |
...
| `display_name` | str | 在 `kova model` 选择器中显示的人类可读标签 |
...
`auth_type` 控制哪些代码路径将你的提供商视为"简单 api-key 提供商"——若不是 `api_key`，PluginManager 仍会记录 manifest，但 Kova CLI 层面的自动化（doctor 检查、`--provider` 标志、设置向导委托）可能会跳过它。
...
通用 `PluginManager`（即 `kova plugins` 操作的对象）**能看到**模型提供商插件，但不会导入它们——`providers/__init__.py` 负责管理其生命周期。Manager 记录 manifest 用于自省，并按 `kind: model-provider` 分类。当你将一个未标记的用户插件放入 `$KOVA_HOME/plugins/`，而该插件恰好调用了带 `ProviderProfile` 的 `register_provider`，Manager 会通过源码文本启发式检测自动将其归类为 `kind: model-provider`——因此即使没有 `plugin.yaml`，插件仍能正确路由。

## 通过 pip 分发

与所有 kova 插件一样，模型提供商可以作为 pip 包发布。在你的 `pyproject.toml` 中添加入口点：
...
……其中 `acme_KOVA_plugin:register` 是一个调用 `register_provider(profile)` 的函数。通用 PluginManager 在 `discover_and_load()` 期间会拾取入口点插件。对于 `kind: model-provider` 的 pip 插件，你仍需在 manifest 中声明 kind（或依赖源码文本启发式检测）。

完整的入口点设置请参阅 [构建 kova 插件](/guides/build-a-kova-plugin#distribute-via-pip)。
...
- [构建 kova 插件](/guides/build-a-kova-plugin) — 通用插件编写指南