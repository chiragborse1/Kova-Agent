...
sidebar_position: 5
title: "Prompt 组装"
description: "kova 如何构建系统 prompt、保持缓存稳定性并注入临时层"
---
...
1. agent 身份 — 优先使用 `KOVA_HOME` 中的 `SOUL.md`，否则回退到 `prompt_builder.py` 中的 `DEFAULT_AGENT_IDENTITY`
2. 工具感知行为指导
3. Honcho 静态块（激活时）
4. 可选系统消息
5. 冻结的 MEMORY 快照
...
`SOUL.md` 位于 `~/.kova/SOUL.md`，作为 agent 的身份标识——系统 prompt 的第一个部分。`prompt_builder.py` 中的加载逻辑如下：
...
| 优先级 | 文件 | 搜索范围 | 说明 |
|--------|------|----------|------|
| 1 | `.kova.md`、`kova.md` | 从 CWD 向上至 git 根目录 | kova 原生项目配置 |
...
- **剥离 YAML frontmatter** — `.kova.md` 的 frontmatter 会被移除（保留供未来配置覆盖使用）
...
1. `.kova.md` / `kova.md`（向上遍历至 git 根目录）
...
大多数用户应将 `agent/prompt_builder.py` 视为实现代码，而非配置入口。推荐的自定义路径是修改 kova 已加载的 prompt 输入，而非直接编辑 Python 模板。

### 优先使用这些入口

- `~/.kova/SOUL.md` — 用自定义 agent 角色和固定行为替换内置默认身份块。
- `~/.kova/MEMORY.md` 和 `~/.kova/USER.md` — 提供应在新会话中快照的持久跨会话事实和用户配置文件数据。
- 项目上下文文件，如 `.kova.md`、`kova.md`、`AGENTS.md`、`CLAUDE.md` 或 `.cursorrules` — 注入仓库特定的工作规则。
- Skills — 打包可复用的工作流和参考资料，无需编辑核心 prompt 代码。
- 可选系统 prompt 配置 / API 覆盖 — 添加部署特定的指令文本，无需 fork Kova。
- 临时覆盖层，如 `KOVA_EPHEMERAL_SYSTEM_PROMPT` 或 prefill 消息 — 添加不应成为已缓存 prompt 前缀一部分的轮次级指导。
...
- 若想改变 kova 为所有人组装 prompt 的方式，修改 Python 代码并将其视为代码贡献
