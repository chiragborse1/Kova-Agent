...
description: "如何为 Kova Agent 构建视频生成后端插件"
---
...
Provider 的 `capabilities()` 声明上述哪些参数会被实际处理。Agent 在工具描述中看到的是当前活跃后端的能力信息，当用户通过 `kova tools` 切换后端时会动态重建。
...
用户在 `kova tools` 中只需选择一次 `veo3.1`。Agent 无需关心端点——它只负责传入（或不传入）`image_url`。
...
如果你的后端返回 base64 数据，使用 `save_b64_video()` 将其写入 `$KOVA_HOME/cache/videos/`。对于通过后续 HTTP 请求获取的原始字节，使用 `save_bytes_video()`。否则直接返回上游 URL——gateway 在交付时会解析远程 URL。
