...
sidebar_position: 8
title: "程序化集成"
description: "从外部程序驱动 kova-agent 的三种协议：ACP、TUI gateway JSON-RPC 以及兼容 OpenAI 的 HTTP API"
---

# 程序化集成

kova 提供三种协议，供外部程序驱动 agent——IDE 插件、自定义 UI、CI 流水线、嵌入式子 agent。根据你的传输方式和消费端选择合适的协议。
...
`kova acp` 启动一个基于 stdio 的 JSON-RPC 服务器，使用 ACP 协议。已在 VS Code（Zed Industries 的 ACP 扩展）、Zed 以及所有安装了 ACP 插件的 JetBrains IDE 中投入生产使用。
...
```bash
kova acp                  # 在 stdio 上提供 ACP 服务
kova acp --bootstrap      # 打印适用于支持 ACP 的 IDE 的安装代码片段
```
...
`tui_gateway/server.py` 是 Ink TUI（`kova --tui`）和嵌入式仪表板 PTY 桥接所使用的协议。任何外部宿主均可通过 stdio（或经由 `tui_gateway/ws.py` 的 WebSocket）使用相同协议。
...
Pi-mono RPC 规范（[issue #360](https://github.com/NousResearch/Kova-agent/issues/360)）中的每条命令均有对应的 TUI gateway 等价项：
...
`gateway/platforms/api_server.py` 通过 HTTP 暴露 Kova，供任何已支持 OpenAI 格式的客户端使用。适用于需要 Web 前端、curl 驱动的 CI 运行器或非 Python 消费端的场景。
...
GET  /v1/models                  列出 kova-agent
GET  /health, /health/detailed
```

配置、请求头（`X-kova-Session-Id`、`X-kova-Session-Key`）及前端接入：[API Server](../user-guide/features/api-server)。
...
- **正在编写自定义桌面 / Web / TUI 宿主，且需要 kova 的全部功能**（slash 命令、审批、clarify、多 agent、会话分支）→ 选 TUI gateway JSON-RPC。
...
- **API server：** 在请求体中包含 `model` 字段，或设置 `X-kova-Model`

内置 provider 感知解析（相同的模型名称会根据当前 provider 自动选择正确格式）。参见 `KOVA_cli/model_switch.py`。

---

## 关于 `--mode rpc` 的说明

kova 没有 `--mode rpc` 标志。上述三种协议已覆盖所有使用场景——ACP 用于 IDE 协议客户端，TUI gateway 用于 stdio JSON-RPC 宿主，API server 用于 HTTP。如果你发现上述协议均无法满足的真实需求，请提交 issue 并说明你正在构建的具体消费端。