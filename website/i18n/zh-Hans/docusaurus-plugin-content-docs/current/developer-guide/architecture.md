...
├── KOVA_constants.py       # KOVA_HOME、感知 profile 的路径
...
→ [插件指南](/guides/build-a-kova-plugin)，[记忆提供者插件](./memory-provider-plugin.md)
...
| **Profile 隔离** | 每个 profile（`kova -p <name>`）拥有独立的 KOVA_HOME、配置、记忆、会话和 gateway PID。多个 profile 可并发运行。 |
