...
description: "如何为 Kova Agent 构建图像生成后端插件"
---
...
:::tip
图像生成是 kova 支持的多种**后端插件**之一。其他插件（各有更专用的 ABC）包括：[Memory Provider 插件](/developer-guide/memory-provider-plugin)、[Context Engine 插件](/developer-guide/context-engine-plugin) 和 [Model Provider 插件](/developer-guide/model-provider-plugin)。通用工具/hook/CLI 插件请参阅 [构建 kova 插件](/guides/build-a-kova-plugin)。
:::
...
`image_generate` 工具包装器向注册表请求活跃 provider 并分发调用。若未注册任何 provider，工具会显示一条有用的错误信息，指引用户使用 `kova tools`。
...
`kind: backend` 决定插件被路由到图像生成注册路径。`requires_env` 在 `kova plugins install` 期间会提示用户输入。
...
| `display_name` | — | `name.title()` | 在 `kova tools` 中显示的标签 |
| `is_available()` | — | `True` | 缺少凭据/依赖时的拦截门控 |
| `list_models()` | — | `[]` | `kova tools` 模型选择器的目录 |
...
部分后端返回图像 URL（fal、Replicate）；其他后端返回 base64 载荷（OpenAI gpt-image-2）。对于 base64 情况，使用 `save_b64_image()` — 它将文件写入 `$KOVA_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` 并返回绝对 `Path`。将该路径（转为 `str`）作为 `image=` 传入 `success_response()`。Gateway 投递（Telegram 图片气泡、Discord 附件）同时识别 URL 和绝对路径。
...
或交互式操作：`kova tools` → "Image Generation" → 选择 `my-backend` → 根据提示输入 API key。
...
`my_backend_imggen_package` 必须暴露一个顶层 `register` 函数。完整配置请参阅通用插件指南中的 [通过 pip 分发](/guides/build-a-kova-plugin#distribute-via-pip)。

## 相关页面

- [图像生成](/user-guide/features/image-generation) — 面向用户的功能文档
- [插件概览](/user-guide/features/plugins) — 所有插件类型一览
- [构建 kova 插件](/guides/build-a-kova-plugin) — 通用工具/hook/斜杠命令指南