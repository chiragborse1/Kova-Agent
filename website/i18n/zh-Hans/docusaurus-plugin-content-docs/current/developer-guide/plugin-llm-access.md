...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——kova 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
第三个完整示例（包含图像输入）位于
[`kova-example-plugins`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-example)
仓库（参考 plugin 的配套仓库——不随 kova-agent 本体打包）。关于异步接口（`acomplete()` / `acomplete_structured()` 与 `asyncio.gather()` 配合使用），请参见同一仓库中的
[`plugin-llm-async-example`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-async-example)。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——kova provider id（如 "openrouter"）
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 kova provider
        # （必须是 kova 已知的 provider——与
        # `kova model` 和 config.yaml model.provider 中的名称相同）
...
* **认证。** 从 `~/.kova/auth.json` / 环境变量中提取 API 密钥、OAuth token 或刷新 token，包括配置了凭据池时的处理。Plugin 永远看不到这些内容。
* **视觉路由。** 当提供图像输入而用户当前激活的文本模型仅支持文本时，宿主自动回退到已配置的视觉模型。
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 kova 常规的聚合器感知回退流程。
...
* 实现：[`agent/plugin_llm.py`](https://github.com/NousResearch/Kova-agent/blob/main/agent/plugin_llm.py)
* 测试：[`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/Kova-agent/blob/main/tests/agent/test_plugin_llm.py)
* 参考 plugin（配套仓库）：
  * [`plugin-llm-example`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-example) — 带图像输入的同步结构化提取
  * [`plugin-llm-async-example`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-async-example) — 使用 `asyncio.gather()` 的异步示例
* 辅助客户端（底层引擎）：参见
