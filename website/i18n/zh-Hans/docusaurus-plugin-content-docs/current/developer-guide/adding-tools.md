...
sidebar_position: 2
title: "添加工具"
description: "如何向 Kova Agent 添加新工具——schema、handler、注册与 toolset"
---
...
:::warning 仅限内置核心工具
本页面用于向仓库本身添加 **kova 内置工具**。
如果你想要个人专用、项目本地或其他自定义工具，而不修改 kova 核心，请使用插件方式：

- [插件](/user-guide/features/plugins)
- [构建 kova 插件](/guides/build-a-kova-plugin)
...
2. **`toolsets.py`** — 将工具名称添加到 `_KOVA_CORE_TOOLS`（或特定 toolset）
...
如果你的工具需要 API 密钥，将其添加到 `KOVA_cli/config.py`：
...
- [ ] 可选：已将 API 密钥添加到 `KOVA_cli/config.py` 的 `OPTIONAL_ENV_VARS`
- [ ] 可选：已添加到 `toolset_distributions.py` 以支持批量处理
- [ ] 已通过 `kova chat -q "Use the weather tool for London"` 测试