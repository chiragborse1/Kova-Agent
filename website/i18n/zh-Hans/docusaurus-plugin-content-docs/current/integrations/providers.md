...
本页介绍如何为 Kova Agent 配置推理提供商——从 OpenRouter、Anthropic 等云端 API，到 Ollama、vLLM 等自托管端点，再到高级路由与故障转移配置。使用 kova 至少需要配置一个提供商。

## 推理提供商

你需要至少一种方式连接到 LLM。使用 `kova model` 交互式切换提供商和模型，或直接配置：

| 提供商 | 配置方式 |
|----------|-------|
| **Nous Portal** | `kova model`（OAuth，订阅制） |
| **OpenAI Codex** | `kova model`（ChatGPT OAuth，使用 Codex 模型） |
...
```bash
kova setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
kova model              # 已有安装——从列表中选择"Nous Portal"
kova portal info        # 随时查看登录状态和路由信息
```
...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），kova 将这些任务路由到你的**主聊天模型**——即你在 `kova model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`kova setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `kova model` 或 `kova tools` 按工具启用。随时使用 `kova portal info` 查看路由状态。
:::
...
| 命令 | 运行位置 | 功能 |
|---------|-------------|--------------|
| **`kova model`** | 终端（任何会话之外） | 完整配置向导——添加提供商、运行 OAuth、输入 API key、配置端点 |
| **`/model`** | kova 聊天会话内部 | 在**已配置的**提供商和模型之间快速切换 |

如果你想切换到尚未配置的提供商（例如你只配置了 OpenRouter，想使用 Anthropic），需要使用 `kova model`，而不是 `/model`。先退出会话（`Ctrl+C` 或 `/quit`），运行 `kova model`，完成提供商配置，然后开启新会话。
...
通过 `kova model` → Anthropic OAuth（或 `kova auth add anthropic --type oauth`）认证时，kova 以 Claude Code 身份路由到你的 Anthropic 账户。**仅当你订阅了 Claude Max 计划且购买了额外用量积分时才有效。** Claude Max 基础计划的配额（Claude Code 默认包含的用量）不会被 kova 消耗——只有你额外购买的超额积分才会被使用。Claude Pro 订阅者无法使用此路径。
...
# 推荐：通过 `kova model` 认证
# 如果已使用 Claude Code，kova 会直接使用其凭据存储
kova model
...
kova 以一等提供商身份支持 GitHub Copilot，提供两种模式：
...
如果未找到 token，`kova model` 会提供 **OAuth 设备码登录**——与 Copilot CLI 和 opencode 使用的流程相同。
...
| 类型 | 前缀 | 获取方式 |
|------|--------|------------|
| OAuth token | `gho_` | `kova model` → GitHub Copilot → 使用 GitHub 登录 |
| 细粒度 PAT | `github_pat_` | GitHub 设置 → 开发者设置 → 细粒度 token（需要 **Copilot Requests** 权限） |
| GitHub App token | `ghu_` | 通过 GitHub App 安装获取 |

如果你的 `gh auth token` 返回 `ghp_*` token，请使用 `kova model` 通过 OAuth 认证。
:::

:::info kova 中的 Copilot 认证行为
kova 将支持的 GitHub token（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送到 `api.githubcopilot.com`，并附带 Copilot 专用请求头（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

收到 HTTP 401 时，kova 在回退前会执行一次性凭据恢复：

1. 通过正常优先级链重新解析 token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新后的请求头重建共享 OpenAI 客户端
3. 重试请求一次

部分旧版社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点对某些账户类型可能不可用（返回 404）。因此 kova 以直接 token 认证为主路径，依靠运行时凭据刷新 + 重试保证健壮性。
:::
...
:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，kova 会自动探测多个端点（全球版、中国版、编程版）以找到接受你 API key 的端点。无需手动设置 `GLM_BASE_URL`——可用端点会被自动检测并缓存。
:::
...
使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），kova 会在每次 API 请求中自动发送 `x-grok-conv-id` 请求头以启用 prompt（提示词）缓存。这会将同一会话的请求路由到同一服务器，使 xAI 基础设施能够复用已缓存的系统 prompt 和对话历史。

无需任何配置——检测到 xAI 端点且存在会话 ID 时，缓存自动激活。这可降低多轮对话的延迟和成本。

xAI 还提供专属 TTS 端点（`/v1/tts`）。在 `kova tools` → 语音与 TTS 中选择 **xAI TTS**，或参见[语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面了解配置。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源模型目录，无需 GPU。在 `kova model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API key，kova 会自动发现可用模型。
...
阿里巴巴 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `kova model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，kova 会持久化刷新 token。
...
如果你订阅了阿里巴巴的 **Coding Plan**（独立于标准 DashScope API 访问的计费 SKU），kova 将其作为独立的一等提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。与常规 `alibaba` 提供商一样兼容 OpenAI，但基础 URL 和计费面不同。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7——无需 API key。在 `kova model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，kova 会持久化访问 token 和刷新 token。底层使用 Anthropic Messages 兼容端点（`/anthropic`）。
...
kova 会在每次向 `build.nvidia.com` 发送请求时自动附加 NIM 计费来源请求头——无需任何配置。这会在 NVIDIA 计费仪表板中将消耗路由到正确的来源。
...
Kova Agent 可与**任何 OpenAI 兼容 API 端点**配合使用。只要服务器实现了 `/v1/chat/completions`，就可以将 kova 指向它。这意味着你可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。kova 的任何部分都不再读取这两个变量——`config.yaml` 是模型和端点配置的唯一来源。如果你的 `.env` 中有过时条目，下次运行 `kova setup` 或配置迁移时会自动清除。请使用 `kova model` 或直接编辑 `config.yaml`。
:::
...
**添加新提供商：** 退出会话（`Ctrl+C` 或 `/quit`），运行 `kova model`，配置新提供商，然后开启新会话。
:::
...
切换提供商时，kova 会将基础 URL 和提供商持久化到配置中，使更改在重启后保留。从自定义端点切换到内置提供商时，过时的基础 URL 会自动清除。
...
**无法通过 OpenAI 兼容 API**（`/v1/chat/completions`）设置上下文长度。必须在服务端或通过 Modelfile 配置。这是将 Ollama 与 kova 等工具集成时最常见的困惑来源。
:::
...
kova 会自动以 64K 上下文长度加载 LM Studio 模型。
...
**工具调用：** 自 LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Kova）会被自动检测并显示工具徽章。其他模型使用通用回退，可靠性可能较低。
...
如果收到列出模型的 JSON 响应，说明配置正确。在 kova 配置中使用相同的 URL 作为 `base_url`。

---

### 本地模型故障排查

以下问题影响与 kova 配合使用的**所有**本地推理服务器。

#### 从 WSL2 连接 Windows 托管模型服务器时"连接被拒绝"

如果你在 WSL2 内运行 kova 而模型服务器在 Windows 主机上，在 WSL2 默认 NAT 网络模式下 `http://localhost:<port>` 无法工作。参见上方的 [WSL2 网络](#wsl2-networking-windows-users) 了解解决方案。
...
| **vLLM** | 添加 `--enable-auto-tool-choice --tool-call-parser kova` |
...
**原因：** 上下文窗口太小。当对话超过上下文限制时，大多数服务器会静默丢弃较早的消息。kova 的系统 prompt 加工具 schema 单独就可能占用 4k–8k tokens。
...
kova 从服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本不报告），kova 使用模型声明的限制，该值可能不正确。
...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 kova 中启用[上下文压缩](/user-guide/configuration#context-compression)。
...
然后通过 `kova model` → 自定义端点 → `http://localhost:4000/v1` 配置 Kova。
...
然后通过 `kova model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 配置 Kova。
...
通过 `kova model` → 自定义端点，或在 `config.yaml` 中配置任意上述服务：
...
:::note 两个设置，容易混淆
**`context_length`** 是**总上下文窗口**——输入和输出 token 的合计预算（例如 Claude Opus 4.6 为 200,000）。kova 用它来决定何时压缩历史记录以及验证 API 请求。
...
`kova model` 在配置自定义端点时会提示输入上下文长度。留空则自动检测。
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key 省略——kova 对无 key 的本地服务器使用"no-key-required"
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # 由 `kova model` → 自定义端点向导显式设置；自动检测仍作为回退
...
某些 OpenAI 兼容端点需要特定于提供商的请求体字段。在对应的自定义提供商中添加 `extra_body` 映射，kova 会将其合并到该端点的每个 chat-completions 请求中：
...
`kova model` → 自定义端点向导现在会显式提示 `api_mode` 并将你的答案持久化到 `config.yaml`。当字段留空时，基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）仍作为回退。
...
也可以从交互式 `kova model` 菜单中选择命名自定义提供商。
...
Together 的 `/v1/models` 端点可用，因此 `kova model` 可以自动发现可用模型。
...
:::tip 故障排查
- `kova doctor` 对于上述任何名称都不应打印 `Unknown provider` 警告（在 #15083 的 CLI 验证器修复之后）。
- 如果某个提供商的 `/v1/models` 端点不可达（Perplexity 是常见情况），`kova model` 会在警告后持久化模型而不是硬性拒绝——参见 #15136。
...
:::tip
可以随时使用 `kova model` 切换提供商——无需重启。无论使用哪个提供商，你的对话历史、记忆和技能都会保留。
:::
...
默认情况下，kova 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你希望在本地运行 Firecrawl，可以将 kova 指向自托管实例。完整配置说明参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
配置一个备用提供商链，当主模型失败时（速率限制、服务器错误、认证失败）kova 按顺序尝试。规范格式是顶级 `fallback_providers:` 列表：
...
:::tip
故障转移仅通过 `config.yaml` 配置——或通过 `kova fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/user-guide/features/fallback-providers)。
:::
