...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Kova Agent — no API key required"
---

# MiniMax OAuth

Kova Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and kova automatically refreshes your session.
...
# → kova opens your browser to the MiniMax authorization page
...
```bash
kova auth add minimax-oauth --no-browser
```

kova will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

kova implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. kova generates a PKCE verifier / challenge pair and a random state value.
...
```bash
kova config set model.default MiniMax-M2.7
kova config set model.provider minimax-oauth
```
...
```bash
kova --provider minimax-oauth    # canonical
kova --provider minimax-portal   # alias
kova --provider minimax-global   # alias
kova --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `kova setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
kova --provider minimax-oauth
```
...
kova refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, kova marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), kova marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `kova auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, kova raises a timeout error.

**Fix:** re-run `kova auth add minimax-oauth` (or `kova model`). The flow starts fresh.

### State mismatch (possible CSRF)

kova detected that the `state` value returned by the authorization server does not match what it sent.
...
If `kova` cannot open a browser window, use `--no-browser`:

```bash
kova auth add minimax-oauth --no-browser
```

kova prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `kova model` and select MiniMax (OAuth), or run `kova auth add minimax-oauth`.
...
```bash
kova auth logout minimax-oauth
```
