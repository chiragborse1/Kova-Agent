---
sidebar_position: 8
title: "Use Voice Mode with kova"
description: "A practical guide to setting up and using kova voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with kova
...
- you want kova sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in kova.
...
## Step 1: make sure normal kova works first
...
kova supports both local and cloud speech stacks.
...
### If you use `kova setup`

If you choose NeuTTS in the setup wizard, kova checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. kova transcribes and responds
...
- asking kova to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full kova loop.

## Tuning CLI behavior

### Silence threshold

If kova starts/stops too aggressively, tune:
...
kova stays a normal chat bot, but can speak replies.
...
- you want kova to function like a portable research or ops assistant
...
kova joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- kova detects speech boundaries
- transcripts are posted in the associated text channel
- kova responds in text and audio
...
1. get text kova working
2. install `kova-agent[voice]`
