...
description: "Find, install, use, and create skills — on-demand knowledge that teaches kova new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach kova how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every kova installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask kova to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with kova but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
kova skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.kova/skills/`
...
For how to ship skills in your own plugin, see [Build a kova Plugin → Bundle skills](/guides/build-a-kova-plugin#bundle-skills).
...
When a skill with config is first loaded, kova prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
kova skills config gif-search

# View all skill config
kova config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.kova/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.kova/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
kova chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.kova/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, kova may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, kova will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.kova/skills/devops/`, `~/.kova/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell kova to update the skill with what you learned. Skills that aren't maintained become liabilities.
