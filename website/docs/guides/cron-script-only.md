...
kova calls this **no-agent mode**. It's the cron system minus the LLM.
...
- **Bash or Python.** `.sh` / `.bash` files run under `/bin/bash`; any other extension runs under the current Python interpreter. Anything in `~/.kova/scripts/` is accepted.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, kova writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **kova:** *(writes `~/.kova/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.kova/scripts/memory-watchdog.sh",
...
>
> **kova:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `kova cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **kova:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.kova/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.kova/scripts/memory-watchdog.sh
...
# 3. Verify
kova cron list
kova cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.kova/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
kova cron create "every 5m"        # interval
kova cron create "every 2h"
kova cron create "0 9 * * *"       # standard cron: 9am daily
kova cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.kova/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.kova/.env` / `~/.kova/config.yaml`.

## Editing and Lifecycle

```bash
kova cron list                                    # see all jobs
kova cron pause <job_id>                          # stop firing, keep definition
kova cron resume <job_id>
kova cron edit <job_id> --schedule "every 10m"    # adjust cadence
kova cron edit <job_id> --agent                   # flip to LLM mode
kova cron edit <job_id> --no-agent --script …     # flip back
kova cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.kova/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.kova/scripts/disk-alert.sh
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on kova' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When kova might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a kova webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on kova being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
