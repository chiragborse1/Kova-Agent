...
title: "Build a kova Plugin"
description: "Step-by-step guide to building a complete kova plugin with tools, hooks, data files, and skills"
---

# Build a kova Plugin

This guide walks through building a complete kova plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
kova has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
This tells kova: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** kova may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `kova my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but kova continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `KOVA_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
KOVA_PLUGINS_DEBUG=1 kova plugins list
```
...
When users run `kova plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing kova into installing arbitrary packages — only specs kova itself ships are eligible |
...
- **The system prompt is kova's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `kova <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for kova my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: kova my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for kova my-plugin."""
...
After registration, users can run `kova my-plugin status`, `kova my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `kova name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `kova chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `KOVA_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `kova <subcommand>` (e.g. `kova kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "kova kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive kova from a hook.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `Kova Gateway status` without SDK instantiation.
...
kova also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.
...
kova connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).
...
kova skills search my-workflow --source myorg/skills-repo
kova skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install kova-plugin-calculator
# Plugin auto-discovered on next kova startup
```
...
```python
# Wrong — will break if kova passes extra context
def handler(args):
    ...
