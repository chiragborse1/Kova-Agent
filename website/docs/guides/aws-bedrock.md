...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use Kova Agent with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

Kova Agent supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — kova detects the instance role automatically.
:::
...
kova auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `kova model` picker shows these automatically, with recommended models at the top:
...
Bedrock works with all Kova Gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
kova checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. kova automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).

## One-Click AWS Deployment

For a fully automated deployment on EC2 with CloudFormation:

**[sample-kova-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-kova-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
