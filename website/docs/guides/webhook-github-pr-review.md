...
description: "Connect kova to GitHub so it automatically fetches PR diffs, reviews code changes, and posts comments — triggered by webhooks with no manual prompting"
---

# Automated GitHub PR Comments with Webhooks

This guide walks you through connecting Kova Agent to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.

When a PR is opened or updated, GitHub sends a webhook POST to your kova instance. kova runs the agent with a prompt that instructs it to retrieve the diff via the `gh` CLI, and the response is posted back to the PR thread.
...
- Kova Agent installed and running (`Kova Gateway`)
- [`gh` CLI](https://cli.github.com/) installed and authenticated on the gateway host (`gh auth login`)
- A publicly reachable URL for your kova instance (see [Local testing with ngrok](#local-testing-with-ngrok) if running locally)
...
Add the following to your `~/.kova/config.yaml`:
...
The GitHub webhook payload includes PR metadata (title, description, branch names, URLs) but **not the diff**. The prompt above instructs the agent to run `gh pr diff` to fetch the actual changes. The `terminal` tool is included in the default `kova-webhook` toolset, so no extra configuration is needed.
:::
...
Create a branch, push a change, and open a PR. Within 30–90 seconds (depending on PR size and model), kova should post a review comment.

To follow the agent's progress in real time:

```bash
tail -f "${KOVA_HOME:-$HOME/.kova}/logs/gateway.log"
```
...
If kova is running on your laptop, use [ngrok](https://ngrok.com/) to expose it:
...
:::note
`kova webhook test <name>` only works for **dynamic subscriptions** created with `kova webhook subscribe`. It does not read routes from `config.yaml`.
:::
...
Load a [kova skill](/user-guide/features/skills) to give the agent a consistent review persona. Add `skills` to your route inside `platforms.webhook.extra.routes` in `config.yaml`:
...
> **Note:** Only the first skill in the list that is found is loaded. kova does not stack multiple skills — subsequent entries are ignored.
...
The same adapter works with GitLab. GitLab uses `X-Gitlab-Token` for authentication (plain string match, not HMAC) — kova handles both automatically.
...
GitLab payload fields differ from GitHub's — e.g. `{object_attributes.title}` for the MR title and `{object_attributes.iid}` for the MR number. The easiest way to discover the full payload structure is GitLab's **Test** button in your webhook settings, combined with the **Recent Deliveries** log. Alternatively, omit `prompt` from your route config — kova will then pass the full payload as formatted JSON directly to the agent, and the agent's response (visible in the gateway log with `deliver: log`) will describe its structure.
...
- **[Build a Plugin](/guides/build-a-kova-plugin)** — package review logic into a shareable plugin
