...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `kova send`."
---

# Pipe Script Output to Messaging Platforms

`kova send` is a small, scriptable CLI that pushes a message to any
messaging platform kova is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | kova send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `kova
...
Any platform kova ships adapters for works as a target:
...
`kova send` resolves the message body in this order:

1. **Positional argument** — `kova send --to telegram "hi"`
2. **`--file PATH`** — `kova send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | kova send --to telegram`

When stdin is a TTY (no pipe), kova does **not** wait for input — you'll
...
Because `kova send` reuses your kova config, the same script works on
any host where kova is installed — no need to export bot tokens into
...
`kova send`. If the Python interpreter can't load because the box is
...
## Does `kova send` Need the Gateway Running?
...
pointing at the gateway; start it with `Kova Gateway start` and retry.
...
| Approach | Multi-platform | Reuses kova creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `kova send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`kova send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `kova send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `kova send` shares with cron delivery.
