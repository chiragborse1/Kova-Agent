...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `kova setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **kova spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.kova/cron/output/` and you can read them anytime.
:::
...
kova will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Kova Agent
```
...
Before creating cron jobs, ensure kova has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell kova what you want:
...
kova will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell kova to delegate each topic to a sub-agent:
...
kova will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
