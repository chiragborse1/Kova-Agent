...
description: "Diagnose and fix common kova cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`Kova Gateway` for foreground, or `Kova Gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `kova cron tick`.
...
```bash
date
kova cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.kova/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.kova/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.kova/.env` |
...
| `local` | Write access to `~/.kova/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `kova cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `kova skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `kova skills list`.
...
2. `~/.kova/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `kova cron list`
...
```bash
ls ~/.kova/scripts/your-script.py   # Must exist
kova cron edit <job_id> --script ~/.kova/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `kova skills install <skill-name>`.
...
Jobs are stored in `~/.kova/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.kova/cron/jobs.json
chmod 600 ~/.kova/cron/jobs.json   # Your user should own it
```
...
```bash
kova cron list                    # Show all jobs, states, next_run times
kova cron run <job_id>            # Schedule for next tick (for testing)
kova cron edit <job_id>           # Fix configuration issues
kova logs                         # View recent kova logs
kova skills list                  # Verify installed skills
```
...
3. Open an issue at [github.com/NousResearch/Kova-agent](https://github.com/NousResearch/Kova-agent) with:
