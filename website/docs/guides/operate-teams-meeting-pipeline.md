...
kova teams-pipeline token-health --force-refresh
```
...
kova teams-pipeline maintain-subscriptions --dry-run
```
...
```ini
[Unit]
Description=kova Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run kova Teams pipeline subscription maintenance every 12 hours
...
```bash
kova teams-pipeline subscriptions   # should show expirationDateTime advanced
kova teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
kova teams-pipeline list --status failed
kova teams-pipeline show <job-id>
```

### Replay a stored job

```bash
kova teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
kova teams-pipeline fetch --meeting-id <meeting-id>
kova teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `kova teams-pipeline maintain-subscriptions --dry-run`
- inspect `kova teams-pipeline list --status failed`
...
- run `kova teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `kova teams-pipeline run`
...
- [ ] `kova teams-pipeline validate` returns an OK snapshot
- [ ] `kova teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (kova cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
