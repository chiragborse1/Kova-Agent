...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Kova Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to kova instantly when PRs are opened or updated.
:::
...
- **Kova Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  Kova Gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.kova/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure kova can access GitHub. Start a chat:
...
Still in the chat, ask kova to review a real PR:
...
A skill gives kova consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.kova/skills/code-review
```

Create `~/.kova/skills/code-review/SKILL.md`:
...
Verify it loaded — start `kova` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach kova your team's standards:
...
2. Teach kova your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
Kova Gateway status    # Is the gateway running?
kova cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-kova-plugin)** — wrap the review logic into a shareable plugin
