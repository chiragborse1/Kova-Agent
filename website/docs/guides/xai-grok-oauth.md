...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Kova Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Kova Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and kova automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in kova — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → kova opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.kova/auth.json` and refreshed automatically before they expire.
...
On servers, containers, or SSH sessions where no browser is available, kova detects the remote environment and prints the authorization URL instead of opening a browser.
...
kova auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running kova inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — kova skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
kova auth add xai-oauth --manual-paste
# Or via the model picker:
kova model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/NousResearch/Kova-agent/issues/26923).

If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — kova accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.

## How the Login Works

1. kova opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to kova and the tokens are saved to `~/.kova/auth.json`.
4. From then on, kova refreshes the access token in the background — you stay signed in until you `kova auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
kova config set model.default grok-build-0.1
kova config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.kova/config.yaml` will contain:
...
```bash
kova --provider xai-oauth        # canonical
kova --provider grok-oauth       # alias
kova --provider x-ai-oauth       # alias
kova --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `kova tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `kova tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `kova setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

kova refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), kova surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), kova marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `kova auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, kova raises a timeout error.

**Fix:** re-run `kova auth add xai-oauth` (or `kova model`). The flow starts fresh.

### State mismatch (possible CSRF)

kova detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions kova prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `kova model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/Kova-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
kova config set model.provider xai
```
...
**Fix:** run `kova model` and pick the xAI Grok OAuth provider, or run `kova auth add xai-oauth`.
...
```bash
kova auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `kova auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `kova auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if kova is on a different machine than your browser
