...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. kova works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
On an 8 GB Mac, use `q4_0` KV cache and choose a smaller model that can still fit kova' 64K minimum context. On 16 GB, you can comfortably do 128K context. On 32 GB+, you can run larger models or multiple parallel slots.

If you're still running out of memory, reduce context only while staying at or above kova' 64K minimum; otherwise switch to a smaller model or smaller quantization (Q3_K_M instead of Q4_K_M).
...
kova automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
KOVA_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `KOVA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `KOVA_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `KOVA_API_TIMEOUT` |
