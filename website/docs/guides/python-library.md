---
sidebar_position: 5
title: "Using kova as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using kova as a Python Library

kova isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install kova directly from the repository:

```bash
pip install git+https://github.com/NousResearch/Kova-agent.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+https://github.com/NousResearch/Kova-agent.git
```

You can also pin it in your `requirements.txt`:

```text
kova-agent @ git+https://github.com/NousResearch/Kova-agent.git
```

:::tip
The same environment variables used by the CLI are required when using kova as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use kova is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding kova in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, kova includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!kova "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your kova config at runtime) |
