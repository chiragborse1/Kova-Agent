...
This tutorial walks you through setting up a Telegram bot powered by Kova Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Kova Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.kova/.env`

:::tip
A $5/month VPS is plenty for running the gateway. kova itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Kova Agent. DM me for help with code, research, debugging, and more.
   ```
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `Kova Gateway install` to update the plist.
:::
...
```bash
Kova Gateway stop && Kova Gateway start
```
...
kova pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.kova/.env`:
...
Control how much detail the bot shows when using tools. In `~/.kova/config.yaml`:
...
Customize how the bot communicates by editing `~/.kova/SOUL.md`:
...
```bash
# From the CLI
kova cron list          # View all scheduled jobs
kova cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u kova-gateway` (Linux) or `~/.kova/logs/gateway.log` (macOS) |
| Cron job output | `~/.kova/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.kova/cron/jobs.json` |
| Pairing data | `~/.kova/pairing/` |
| Session history | `~/.kova/sessions/` |
