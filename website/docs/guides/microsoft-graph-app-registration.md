...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.kova/.env` at the end.
...
4. Fill in:
   - **Name:** `kova Teams Meeting Pipeline` (or any name you'll recognize).
...
Put the three values you collected into `~/.kova/.env`:
...
kova ships a Graph auth smoke-test. From your kova install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.kova/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `Kova Gateway restart`.
