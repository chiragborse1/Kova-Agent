---
sidebar_position: 9
title: "Run kova Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Kova Agent entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run kova Locally with Ollama — Zero API Cost
...
You'll set up Kova Agent running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, kova works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- kova connected to Ollama as a custom endpoint
...
kova is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full kova experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside kova with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
kova will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. kova requires at least 64,000 tokens for agentic work with tools:
...
Then update your kova config to use `gemma4-64k` as the model name.
...
Once kova works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.
...
- **Terminal commands** — kova wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below kova' 64K minimum unless you configure them
