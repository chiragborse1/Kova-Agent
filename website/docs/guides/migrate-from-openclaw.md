...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Kova Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`kova claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into kova. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `kova setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
kova claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.kova/` (by default a single restore-point archive is written before apply, under `~/.kova/backups/pre-migration-*.zip`; restorable with `kova import`). |
...
| What | OpenClaw source | kova destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.kova/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.kova/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.kova/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.kova/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | kova destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.kova/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.kova/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.kova/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.kova/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing kova skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | kova destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.kova/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | kova config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | kova config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | kova field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.kova/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | kova `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | kova path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `KOVA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct kova equivalent)

These are saved to `~/.kova/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in kova |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | kova has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `kova cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `kova webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `kova honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `kova skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use kova profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to kova manually via `kova config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.kova/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `kova status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart kova-gateway`

6. **Check session policies** — run `kova config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `kova whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `kova claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `kova config set`.

### Skills not appearing after migration

Imported skills land in `~/.kova/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `kova config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
