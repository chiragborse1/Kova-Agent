...
This page covers all commands related to [kova profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `kova profile` without a subcommand shows help.
...
```bash
kova profile use <name>
```

Sets `<name>` as the active profile. All subsequent `kova` commands (without `-p`) will use this profile.
...
kova profile use default
```

## `kova profile create`

```bash
kova profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `kova profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `kova update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.kova`), use `kova skills opt-out` / `kova skills opt-in`. |
...
kova profile create mybot

# Clone config only from current profile
kova profile create work --clone

# Clone everything from current profile
kova profile create backup --clone-all

# Clone config from a specific profile
kova profile create work2 --clone-from work

# Clone everything from a specific profile
kova profile create work2-backup --clone-from work --clone-all
```

## `kova profile describe`

```bash
kova profile describe [<name>] [options]
```
...
kova profile describe researcher

# Set it explicitly
kova profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
kova profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
kova profile describe --all --auto
```

## `kova profile delete`

```bash
kova profile delete <name> [options]
```
...
```bash
kova profile delete mybot
kova profile delete mybot --yes
```
...
```bash
kova profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's kova home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
kova profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your kova installation.
...
```bash
kova profile alias work
# Creates/updates ~/.local/bin/work

kova profile alias work --name mywork
# Creates ~/.local/bin/mywork

kova profile alias work --remove
...
```bash
kova profile rename <old-name> <new-name>
```
...
```bash
kova profile export <name> [options]
```
...
```bash
kova profile export work
# Creates work.tar.gz in the current directory

kova profile export work -o ./work-2026-03-29.tar.gz
```

## `kova profile import`

```bash
kova profile import <archive> [options]
```
...
```bash
kova profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

kova profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`kova profile export` / `import` are still the right commands for
...
### `kova profile install`

```bash
kova profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `kova -p telemetry`). |
...
kova profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
kova profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
kova profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
kova profile install ./telemetry/
```

### `kova profile update`

```bash
kova profile update <name> [--force-config] [--yes]
```
...
```bash
kova profile info <name>
```

Prints the profile's distribution manifest — name, version, required
kova version, author, env var requirements, the source URL/path, and
...
`kova profile list` also shows the distribution name and version in a
`Distribution` column, and `kova profile show <name>` / `delete <name>`
...
kova profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
kova profile install https://github.com/your-org/internal-assistant.git
```
...
`KOVA_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
kova version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host kova can clone from.
3. Tell recipients to run `kova profile install <your-repo-url>`.
...
```bash
kova -p <name> <command> [options]
kova --profile <name> <command> [options]
```

Global flag to run any kova command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
kova -p work chat -q "Check the server status"
kova --profile dev gateway start
kova -p personal skills list
kova -p work config edit
```

## `kova completion`

```bash
kova completion <shell>
```
...
- `kova profile <TAB>` — subcommands (list, use, create, etc.)
- `kova profile use <TAB>` — profile names
- `kova -p <TAB>` — profile names
