...
kova has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `KOVA_cli/commands.py`:
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of kova config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
| `/goal <text>` | Set a standing goal kova works toward across turns — our take on the Ralph loop. After each turn an auxiliary judge model decides whether the goal is done; if not, kova auto-continues. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Budget defaults to 20 turns (`goals.max_turns`); any real user message preempts the continuation loop, and state survives `/resume`. See [Persistent Goals](/user-guide/features/goals) for the full walkthrough. |
...
| `/busy [queue\|steer\|interrupt\|status]` | CLI-only: control what pressing Enter does while kova is working — queue the new message, steer mid-turn, or interrupt immediately. |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.kova/config.yaml`. See [Skill Bundles](/user-guide/features/skills#skill-bundles). |
...
| `/version` | Show Kova Agent version, build, and environment info. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.kova/config.yaml`:
...
**Full form** — pin an exact model, provider, and optionally a base URL. Put this in `~/.kova/config.yaml`:
...
| `/skills [pending\|approve\|reject\|diff\|approval]` | Review pending **skill** writes staged by the write-approval gate (`skills.write_approval`). Shows a one-line gist per staged write; `/skills diff <id>` is truncated for chat — read the full diff on the CLI or in `~/.kova/pending/skills/<id>.json`. Only appears when the gate is on (or staged writes remain); search/install stay CLI-only. |
...
| `/update` | Update Kova Agent to the latest version. |
...
- In the TUI, `/sessions` shows live sessions in the current TUI process. Use `/resume [name]` or `kova --tui --resume <id-or-title>` for saved or closed transcripts.
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/NousResearch/Kova-agent/issues/30768) for native Windows PowerShell) or when scripting against the CLI.

Set `approvals.destructive_slash_confirm: false` in `~/.kova/config.yaml` to disable the prompts globally; set it back to `true` to re-enable. See [Security — Destructive slash command confirmation](../user-guide/security.md#dangerous-command-approval) for context.
