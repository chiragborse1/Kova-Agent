...
description: "How to build a memory provider plugin for Kova Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Kova Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `kova plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          KOVA_HOME (str): Active KOVA_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `kova memory setup` | **Yes** |
| `save_config(values, KOVA_HOME)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `kova memory setup`:
...
Every field in `get_config_schema()` is prompted during `kova memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$KOVA_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, KOVA_HOME: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(KOVA_HOME) / "my-provider.json"
...
the `messages` parameter; kova will continue calling them with the legacy
signature.
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `kova my-provider status`, `kova my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `kova <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `kova --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: kova my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the kova my-provider argparse tree.
