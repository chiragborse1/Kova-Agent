...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept kova already has applies. The plugin never sees a
...
kova-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — kova provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different kova provider
        # (must be one kova already knows about — same names as
        # `kova model` and config.yaml model.provider).
        ...
        # plugin sends — kova does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through kova' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing kova subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/Kova-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/Kova-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/kova-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
