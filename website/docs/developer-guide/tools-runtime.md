...
kova tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `KOVA_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
...
Toolsets are named bundles of tools. kova resolves them through:

- explicit enabled/disabled toolset lists
- platform presets (`kova-cli`, `kova-telegram`, etc.)
- dynamic MCP toolsets
- curated special-purpose sets like `kova-acp`
