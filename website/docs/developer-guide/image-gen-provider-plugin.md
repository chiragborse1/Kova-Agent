...
description: "How to build an image-generation backend plugin for Kova Agent"
---
...
The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `kova tools`.
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `kova tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `kova tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `kova tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $KOVA_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `kova plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `kova tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `kova tools` model picker |
...
```bash
export KOVA_HOME=/tmp/kova-imggen-test
mkdir -p $KOVA_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
kova plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $KOVA_HOME/config.yaml
echo "  provider: my-backend" >> $KOVA_HOME/config.yaml

# Exercise it
kova -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `kova tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-kova-plugin#distribute-via-pip) in the general plugin guide for the full setup.
