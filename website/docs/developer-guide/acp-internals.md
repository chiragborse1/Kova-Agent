...
The Zed ACP Registry path launches the same adapter through `uvx --from 'kova-agent[acp]==<version>' kova-acp`, pointed at the `kova-agent` PyPI release.
...
So ACP advertises and uses the currently configured kova provider/credentials. It also always advertises a terminal setup auth method (`kova-setup`, args `--setup`) so first-run registry clients can open kova' interactive model/provider configuration before starting a normal ACP session.
