...
This guide covers adding a new messaging platform to the Kova Gateway. A platform adapter connects kova to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.kova/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core kova code. Your plugin is a directory with two files:

```
~/.kova/plugins/my-platform/
...
| `kova config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `kova status` | Shows plugin platforms with `(plugin)` tag |
| `Kova Gateway setup` | Plugin platforms appear in setup menu |
| `kova tools` / `kova skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.kova/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `Kova Gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `kova cron run` separate from `Kova Gateway`), register a `standalone_sender_fn`:
...
version: 1.0.0
description: >
  My Platform gateway adapter for Kova Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the kova core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
