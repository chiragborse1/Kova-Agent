...
kova supports three API execution modes, resolved from provider selection, explicit args, and base URL heuristics:
...
After each turn:
- Messages are saved to the session store (SQLite via `KOVA_state.py`)
- Memory changes are flushed to `MEMORY.md` / `USER.md`
- The session can be resumed later via `/resume` or `kova chat --resume`
