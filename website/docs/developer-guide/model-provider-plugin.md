...
description: "How to build a model provider (inference backend) plugin for Kova Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that kova can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$KOVA_HOME/plugins/model-providers/` with zero changes to the repo.
...
**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$KOVA_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `kova plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `KOVA_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `KOVA_cli/main.py` | Accepts `acme-inference` |
| `kova model` picker | `KOVA_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `kova doctor` | `KOVA_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `kova setup` | `KOVA_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `KOVA_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `kova model` picker |
...
Four values are recognized. kova picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but kova' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `KOVA_HOME` at a temp directory so you don't pollute your real config:

```bash
export KOVA_HOME=/tmp/kova-plugin-test
mkdir -p $KOVA_HOME/plugins/model-providers/my-provider
cat > $KOVA_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
kova -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `kova plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$KOVA_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any kova plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."KOVA_agent.plugins"]
acme-inference = "acme_KOVA_plugin:register"
```

…where `acme_KOVA_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a kova Plugin](/guides/build-a-kova-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a kova Plugin](/guides/build-a-kova-plugin) — general plugin authoring
