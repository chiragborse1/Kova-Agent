...
sidebar_position: 1
title: "Architecture"
description: "Kova Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Kova Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── KOVA_constants.py       # KOVA_HOME, profile-aware paths
...
│
├── KOVA_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `kova` subcommands (large file)
...
│   ├── skills_config.py      # kova skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # kova tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # Kova Gateway start/stop
│
...
→ [Plugin Guide](/guides/build-a-kova-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes kova as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`kova -p <name>`) gets its own KOVA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
