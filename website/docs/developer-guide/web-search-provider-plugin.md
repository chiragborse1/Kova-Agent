...
description: "How to build a web-search/extract/crawl backend plugin for Kova Agent"
---
...
:::tip
Web search is one of several **backend plugins** kova supports. The others (with their own ABCs) are [Image Generation Provider Plugins](/developer-guide/image-gen-provider-plugin), [Video Generation Provider Plugins](/developer-guide/video-gen-provider-plugin), [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a kova Plugin](/guides/build-a-kova-plugin).
:::
...
When neither key is set, kova auto-detects the backend from whichever API key/URL is present in the environment. `kova tools` walks users through selection.
...
class MyBackendWebSearchProvider(WebSearchProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `kova tools`. Defaults to `name`.
        return "My Backend"

    def is_available(self) -> bool:
        # Cheap check — env var present, optional dep importable, etc.
        # MUST NOT make network calls (runs on every `kova tools` paint).
...
| `provides_web_providers` | List of provider `name`s this plugin registers — used by the loader to advertise the plugin in `kova tools` even before `register()` runs |
| `requires_env` | Interactive credential prompt during `kova plugins install` (see [Build a kova Plugin](/guides/build-a-kova-plugin#gate-on-environment-variables) for the rich format) |
...
| `display_name` | — | `name` | Label shown in `kova tools` |
...
When `web.search_backend` or `web.extract_backend` aren't set, both fall through to `web.backend`. When that's also unset, kova picks the first available provider that supports the requested capability based on env-var presence.
...
## How kova wires it into the tools
...
Errors surface as the tool result; the LLM decides how to explain them. If no provider is registered (or every available one fails the capability gate), the tool returns a helpful error pointing at `kova tools`.

## Lazy-installing optional dependencies

If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — kova will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a kova Plugin → Lazy-install](/guides/build-a-kova-plugin#lazy-install-optional-python-dependencies) for the security model.
...
`my_backend_web_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-kova-plugin#distribute-via-pip) in the general plugin guide for the full setup.
...
- [Build a kova Plugin](/guides/build-a-kova-plugin) — general tools/hooks/slash commands guide
