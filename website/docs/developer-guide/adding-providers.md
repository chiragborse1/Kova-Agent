...
description: "How to add a new inference provider to Kova Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

kova can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `kova model` menu entries
...
1. `KOVA_cli/auth.py` decides how credentials are found.
2. `KOVA_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `KOVA_cli/models.py` and `KOVA_cli/main.py` make the provider show up in the CLI. (`KOVA_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `KOVA_cli/auth.py`
2. `KOVA_cli/models.py`
3. `KOVA_cli/runtime_provider.py`
4. `KOVA_cli/main.py`
...
:::tip
`KOVA_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `kova setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$KOVA_HOME/plugins/model-providers/` both get picked up.
...
7. `kova model` menu includes the provider
8. `kova setup` wizard delegates to `main.py` automatically
...
User plugins at `$KOVA_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `kova model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `KOVA_cli/auth.py`
- `_PROVIDER_LABELS` in `KOVA_cli/models.py`
- `_PROVIDER_ALIASES` in both `KOVA_cli/auth.py` and `KOVA_cli/models.py`
- CLI `--provider` choices in `KOVA_cli/main.py`
...
## Step 2: Add auth metadata in `KOVA_cli/auth.py`
...
- What env vars should kova check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `KOVA_cli/models.py`
...
## Step 4: Resolve runtime data in `KOVA_cli/runtime_provider.py`
...
Be careful with API-key precedence. kova already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `KOVA_cli/main.py`

A provider is not discoverable until it shows up in the interactive `kova model` flow.

Update these in `KOVA_cli/main.py`:
...
:::tip
`KOVA_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `kova model` and `kova setup` automatically.
:::
...
When you add a native provider, double-check that kova is only sending fields that provider actually understands.
...
- `tests/KOVA_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/KOVA_cli/test_model_switch_custom_providers.py` (and adjacent `tests/KOVA_cli/test_model_switch_*.py`)
- `tests/KOVA_cli/test_setup_model_provider.py`
...
python -m pytest tests/KOVA_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/KOVA_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m KOVA_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `KOVA_cli/auth.py`
- [ ] aliases added in `KOVA_cli/auth.py` and `KOVA_cli/models.py`
- [ ] model catalog added in `KOVA_cli/models.py`
- [ ] runtime branch added in `KOVA_cli/runtime_provider.py`
- [ ] CLI wiring added in `KOVA_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `kova model` but not `kova setup`
