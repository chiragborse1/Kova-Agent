...
description: "How kova detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in kova' browser tooling:
...
(`kova-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per kova `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `kova-acp`, `kova-api-server`, and core toolsets (gated on CDP reachability)
- `KOVA_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
