...
description: "How kova builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

kova deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.kova.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.kova/SOUL.md)
You are kova, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance kova
...
`SOUL.md` lives at `~/.kova/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_KOVA_md(cwd_path)       # 1. .kova.md / kova.md (walks to git root)
    ...
    # SOUL.md from KOVA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.kova.md`, `kova.md` | CWD up to git root | kova-native project config |
...
- **YAML frontmatter stripped** — `.kova.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, kova concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.kova.md` / `kova.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs kova already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.kova/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.kova/MEMORY.md` and `~/.kova/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.kova.md`, `kova.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking kova.
- Ephemeral overlays such as `KOVA_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how kova assembles prompts for everyone, change Python and treat it as a code contribution
