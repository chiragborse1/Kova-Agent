...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Kova Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in kova tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying kova core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a kova Plugin](/guides/build-a-kova-plugin)
...
2. **`toolsets.py`** — add tool name to `_KOVA_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `KOVA_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `KOVA_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `kova chat -q "Use the weather tool for London"`
