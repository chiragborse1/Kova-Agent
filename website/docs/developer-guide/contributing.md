...
- Building a custom/local tool without modifying kova core? Start with [Build a kova Plugin](../guides/build-a-kova-plugin.md)
...
`$KOVA_HOME/kova-agent` (usually `~/.kova/kova-agent`). That keeps your
...
```bash
# The standard installer already put `kova` on PATH.
kova doctor
kova chat -q "Hello"
```
...
- **Profile-safe paths**: Never hardcode `~/.kova` — use `get_KOVA_HOME()` from `KOVA_constants` for code paths and `display_KOVA_HOME()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/Kova-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
- Use [GitHub Issues](https://github.com/NousResearch/Kova-agent/issues)
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/Kova-agent/blob/main/LICENSE).
