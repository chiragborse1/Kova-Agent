...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Kova Agent"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `kova chat` immediately after.
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Kova Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `kova model` between sessions — the Portal becomes one of your available providers, not your only one.
...
If you use [kova profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
kova portal            # log in to Nous Portal + set it up (one-shot onboarding)
kova portal info       # login status, subscription info, model + gateway routing
kova portal status     # alias for `portal info`
kova portal tools      # detailed Tool Gateway catalog with per-tool routing
kova portal open       # open the subscription management page in your browser
```

`kova portal` (with no subcommand) is the human-readable alias for `kova auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `kova setup --portal`, and the same Nous flow as the first-time quick setup).

`kova portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `kova tools` to pick backends per tool:
...
- **CLI shortcut:** `kova portal open` (opens the same page in your default browser)
...
### `kova portal info` shows "not logged in"
...
or use `kova model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `kova auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/Kova-agent/issues) — we surface the Portal's catalog to kova and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `kova portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `kova model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-kova tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple kova configurations sharing one Portal login
