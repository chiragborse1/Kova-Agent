...
This page covers setting up inference providers for Kova Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use kova.

## Inference Providers

You need at least one way to connect to an LLM. Use `kova model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `kova model` (OAuth, subscription-based) |
| **OpenAI Codex** | `kova model` (ChatGPT OAuth, uses Codex models) |
...
kova model              # existing install — pick "Nous Portal" from the list
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), kova marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `kova auth add codex-oauth` (or `kova model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), kova routes these tasks to your **main chat model** — the same model you picked in `kova model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `kova setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `kova model` or per-tool from `kova tools`. Inspect routing at any time with `kova portal info`.
:::

### Two Commands for Model Management

kova has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`kova model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a kova chat session | Quick switch between **already-configured** providers and models |
...
When you authenticate via `kova model` → Anthropic OAuth (or via `kova auth add anthropic --type oauth`), kova routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by kova — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
kova chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `kova model`
# kova will use Claude Code's credential store directly when available
kova model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
kova chat --provider anthropic
...
kova supports GitHub Copilot as a first-class provider with two modes:
...
```bash
kova chat --provider copilot --model gpt-5.4
```
...
If no token is found, `kova model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `kova model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `kova model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in kova
kova sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, kova now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). kova therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
kova chat --provider copilot-acp --model copilot-acp
...
| `KOVA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `KOVA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
When using the Z.AI / GLM provider, kova automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::
...
When using xAI as a provider (any base URL containing `x.ai`), kova automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `kova tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `kova doctor` and `kova chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `kova migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `kova model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and kova auto-discovers the available models.
...
kova chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 kova chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `kova model`, sign in through the browser, and kova persists the refresh token.
...
Set `KOVA_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), kova exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
kova chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `kova model`, sign in through the browser, and kova persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
kova automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
Kova Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point kova at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `kova model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `kova setup` or config migration.
:::
...
:::warning kova model vs /model
**`kova model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active kova chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `kova model`, set up the new provider, then start a new session.
:::
...
When switching providers, kova persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Kova Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like kova.
:::
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for kova. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below kova' minimum per active session.

Then configure kova to point at it:
...
kova will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, kova) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Kova Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your kova config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your kova config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with kova.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running kova inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser kova` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. kova's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what kova thinks the context is
...
kova auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), kova uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure kova with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in kova.
...
Then configure kova with `kova model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure kova with `kova model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `kova model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). kova uses this to decide when to compress history and to validate API requests.
...
`kova model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
You can also select named custom providers from the interactive `kova model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.kova/config.yaml` and the matching API key goes in `~/.kova/.env`.
...
Together's `/v1/models` endpoint works, so `kova model` can auto-discover available models.
...
:::tip
You can switch between providers at any time with `kova model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point kova at your instance (no API key needed):
   ```bash
   kova config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
Configure a chain of backup providers kova tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
