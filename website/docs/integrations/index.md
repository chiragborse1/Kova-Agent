...
Kova Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what kova can do and where it can run.
...
kova supports multiple AI inference providers out of the box. Use `kova model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. kova auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect kova to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native kova tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `kova tools`:
...
kova includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use Kova Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. kova runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose kova as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use kova as a backend with its full toolset.
...
kova runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
- **[Plugin System](/user-guide/features/plugins)** — Extend kova with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.kova/plugins/`, project-local `.kova/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/guides/build-a-kova-plugin)** — Step-by-step guide for creating kova plugins with tools, hooks, and CLI commands.
