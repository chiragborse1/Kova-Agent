...
A **profile distribution** packages a complete Kova Agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Kova Agent meant sending someone:
...
```bash
kova profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `kova profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running kova in 2026, that's already true.
...
:::caution
**kova does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `kova profile install` or `kova profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
kova profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.kova/profiles/research-bot/SOUL.md
...
Create `~/.kova/profiles/research-bot/distribution.yaml`:
...
Create `~/.kova/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `kova profile update research-bot` will pull the latest.
...
```bash
kova profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.kova/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
kova profile install github.com/you/research-bot

# Full HTTPS
kova profile install https://github.com/you/research-bot.git

# SSH
kova profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
kova profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
kova profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
kova profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
kova profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
kova profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this kova distribution.
...
```bash
cp ~/.kova/profiles/research-bot/.env.EXAMPLE ~/.kova/profiles/research-bot/.env
...
```bash
kova profile info research-bot
```
...
`kova profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
kova profile update research-bot
```
...
```bash
kova profile delete research-bot
```
...
Path:    ~/.kova/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `kova profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
kova profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `kova profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.kova/profiles/polymarket-trader
...
You built kova-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
kova profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
kova profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `kova profile info <name>` and hold off on updates until you're ready.
:::
...
kova profile info research-bot | grep Version
...
```yaml
# ~/.kova/profiles/research-bot/local/my-overrides.yaml
...
kova profile delete research-bot --yes
kova profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
kova profile update research-bot --force-config --yes
```
...
kova profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.kova/profiles/forked-research-bot/
...
kova profile install ~/.kova/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
kova profile delete research-bot-test --yes
kova profile install ~/.kova/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `kova -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `kova profile` — not a parallel command tree.
...
- Reserved profile names (`kova`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
