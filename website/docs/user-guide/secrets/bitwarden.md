...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.kova/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. kova stores that single token in `~/.kova/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `kova` (or the gateway, or a cron job) starts, after `~/.kova/.env` has loaded, kova calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
The `bws` binary is auto-downloaded into `~/.kova/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "kova keys").
...
4. **Machine accounts → New machine account → My kova machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.kova/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.kova/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
kova secrets bitwarden status
```

From now on, every `kova` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `kova secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `kova secrets bitwarden status` | Show config + binary version + token presence |
| `kova secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `kova secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `kova secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `kova secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |

## Configuration

Defaults in `~/.kova/config.yaml`:
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `kova` invocations start fresh. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.kova/bin/` on first use. |

## Failure modes

Bitwarden never blocks kova startup. If anything goes wrong, you'll see a one-line warning in stderr and kova continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `kova secrets bitwarden setup` |
...
- kova will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — kova does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.kova/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple kova installations.
