...
sidebar_position: 3
title: "Desktop App"
description: "The native kova desktop app — a polished experience for chatting with kova, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The kova desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Kova Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `kova` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
kova has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`kova`) and **[TUI](./tui.md)** (`kova --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`kova dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for kova desktop](../getting-started/installation.md).
...
- **The same conversation history** as every other kova surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `kova desktop --cwd <path>` (or the `KOVA_DESKTOP_CWD` environment variable).

### Voice

Talk to kova and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `kova model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader kova management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#kova-cron).
- **Profiles** — switch between [kova profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Kova Agent, your config, and your chats stay. (Same as `kova uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `kova uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `kova uninstall --full`.)
...
You can do the same from the terminal — `kova uninstall --gui` for the GUI alone, or `kova uninstall` / `kova uninstall --full` for the agent too.

:::note
Running `kova uninstall --gui` from a **source checkout** (a `kova desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `kova desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `kova desktop`

To launch via the CLI, simply run `kova desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `kova update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `KOVA_DESKTOP_CWD`)           |
| `--kova-root PATH` | Override the kova source root the app uses (sets `KOVA_DESKTOP_KOVA_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `kova` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Kova Agent runtime into `KOVA_HOME` (`~/.kova`, or `%LOCALAPPDATA%\kova` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `KOVA_DESKTOP_KOVA_ROOT`, then a completed managed install, then a probed `kova` on `PATH` (unless `--ignore-existing` / `KOVA_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `KOVA_DESKTOP_Kova` command override for packagers such as Nix. The React renderer talks to a `kova dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `kova --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a kova backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `kova dashboard` process
"Remote backend" means a **`kova dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `kova dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.kova/.env` (the secrets file, mode 0600):
...
Keep that `kova dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.kova/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/kova` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `KOVA_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `KOVA_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting kova desktop to a remote backend](./features/web-dashboard.md#connecting-kova-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & kova desktop](../reference/environment-variables.md#web-dashboard--kova-desktop).
...
- [TUI](./tui.md) — the modern terminal UI used by `kova --tui` and the dashboard chat tab
