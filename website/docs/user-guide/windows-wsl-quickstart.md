...
description: "Run Kova Agent on Windows via WSL2 — setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
Kova Agent now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
- You're doing POSIX-heavy development work and want your kova sessions to share the same filesystem / paths as your dev tools.
- You already have a WSL2 environment and don't want to maintain a second install.

**When native is fine (or better):**
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most kova features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect kova: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.
...
The native Windows install runs in Windows directly: your Windows terminal (PowerShell, Windows Terminal, etc.), Windows filesystem paths (`C:\Users\…`), and Windows processes.  kova uses Git Bash to run shell commands, which is how Claude Code and other agents handle Windows today — it sidesteps the POSIX-vs-Windows gap without a full rewrite.

WSL2 runs a real Linux kernel in a lightweight VM, so kova inside it is essentially identical to running on Ubuntu.  That's valuable when you want a real POSIX environment: `fork`, `/tmp`, UNIX sockets, signal semantics, PTY-backed terminals, shells like `bash`/`zsh`, and tools like `rg`, `git`, `ffmpeg` that behave the way they do on Linux.

Practical consequences of WSL2:

- The Kova CLI, gateway, sessions, memory, skills, and tool runtimes all live inside the Linux VM.
- Windows programs (browsers, native apps, Chrome with your logged-in profile) live outside it.
- Every time you want the two to talk — share files, open URLs, control Chrome, hit a local model server, expose the Kova Gateway to your phone — you cross a boundary. Those boundaries are what this guide is about.
...
kova does not work reliably on WSL1 — WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.
...
### Where to put kova and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your kova install (`~/.kova/`) — Linux side. The installer already does this.
...
### Case 1 — kova in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and kova (inside WSL) needs to hit it.
...
### Case 2 — Something on Windows (or your LAN) talks to kova in WSL

This is the reverse direction and is less documented elsewhere, but it's what you need for:

- Using the kova **web dashboard** from a Windows browser.
- Using the **OpenAI-compatible API server** (exposed by `Kova Gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a kova service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows → WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   New-NetFirewallRule -DisplayName "kova WSL 8080" `
...
## Running kova services long-term on Windows

The kova [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening kova quickly

If you just want a double-click launcher for an interactive kova shell, create
...
home directory, and launches kova. If `kova` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run kova` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the kova favicon from the repo.
...
If you enabled systemd per the setup section above, `Kova Gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside kova's test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
**Massive slowness on `git status` / `kova chat` in a repo.**
...
kova's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start kova from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
**`kova` not found after running the installer.**
