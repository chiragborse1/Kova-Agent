...
kova uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `kova setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `kova portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `kova setup` or `kova model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `kova model` (or click **Change** in the dashboard) and kova will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models kova recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).
...
Every auxiliary task defaults to `auto` — meaning kova tries your main model for that job too. If that route is unavailable or hits a capacity-style failure, `auto` follows any task-specific `auxiliary.<task>.fallback_chain`, then the main `fallback_providers` / `fallback_model` chain, then kova' built-in auxiliary discovery chain. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `kova skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`kova kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`kova profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
`provider: auto` with `model: ''` tells kova to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
...
- **CLI** (`kova chat`): next `kova chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`Kova Gateway restart`) if you want to force all sessions to pick up the change.
...
kova lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `kova setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but kova switched providers on me
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `kova config set` only writes scalar values, but it can't carry a custom `base_url`:

```bash
kova config set model.aliases.fav anthropic/claude-opus-4.6
kova config set model.aliases.grok x-ai/grok-4
```

Both paths feed the same loader (`KOVA_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `kova model` subcommand

```bash
kova model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`kova model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.kova/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `kova config show | grep '^model\.'` and `kova status`.
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__KOVA_SESSION_TOKEN__`) if you're scripting against a running dashboard.
