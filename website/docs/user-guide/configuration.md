...
sidebar_position: 2
title: "Configuration"
description: "Configure Kova Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.kova/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `kova setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
kova config              # View current configuration
kova config edit         # Open config.yaml in your editor
kova config set KEY VAL  # Set a specific value
kova config check        # Check for missing options (after updates)
kova config migrate      # Interactively add missing options

# Examples:
kova config set model anthropic/claude-opus-4
kova config set terminal.backend docker
kova config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `kova config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `kova chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.kova/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.kova/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `KOVA_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `KOVA_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`KOVA_API_TIMEOUT=1800`s, `KOVA_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/Kova-agent/blob/main/cli-config.yaml.example).

## Update Behavior

`kova update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: false       # Create a full KOVA_HOME zip before every update
  backup_keep: 5                 # Keep this many pre-update backup zips
  non_interactive_local_changes: stash  # stash | discard
```

For git installs, kova auto-stashes dirty tracked files and untracked files before checking out the update branch or pulling. Interactive terminal updates prompt before restoring that stash. Non-interactive updates (desktop/chat app, gateway, or `--yes`) use `updates.non_interactive_local_changes`: `stash` restores local source edits after a successful pull, while `discard` drops the update-created stash after a successful pull. Use `discard` only on managed installs where local source edits are never meant to persist.

Before that stash step, kova also restores tracked `package-lock.json` diffs left by npm install/build churn. Commit or manually stash intentional lockfile edits before updating.

## Terminal Backend Configuration

kova supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nous-managed gateway), a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means kova will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. kova
state is still profile-scoped through `KOVA_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

kova does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that kova launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{KOVA_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{KOVA_HOME}/home` when it exists | Use `{KOVA_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{KOVA_HOME}/home` as `HOME`. kova also
sets `KOVA_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{KOVA_HOME}/home` in `auto` mode
because that directory lives on the persistent kova data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `KOVA_HOME` for kova data and `KOVA_REAL_HOME` for the account home:
...
real_home = Path(os.environ.get("KOVA_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `kova tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across kova processes.** kova starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one kova process to the next. When you close a TUI session, run `/quit`, or start a new `kova` invocation, the container keeps running and the next kova process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  docker_persist_across_processes: true   # Reuse container across kova restarts
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.kova/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that kova doesn't surface as first-class keys — `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override kova' defaults if needed. Use sparingly — flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**Requirements:** Docker Desktop or Docker Engine installed and running. kova probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `KOVA_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every kova-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `kova-agent=1` — marks it as kova-managed
- `kova-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `kova-profile=<sanitized profile name>` — scopes reuse and reaping to the active kova profile

On startup, kova runs `docker ps --filter label=kova-task-id=<id> --filter label=kova-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.

When a kova process exits — `/quit`, closing a TUI session, gateway shutdown, even SIGKILL — the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next kova process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** kova-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `kova-profile=work` is invisible to a kova process running under `kova-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start kova again under their original profile.

Parallel subagents spawned via `delegate_task(tasks=[...])` share this one container — concurrent `cd`, env mutations, and writes to the same path will collide. If a subagent needs an isolated sandbox, it must register a per-task image override via `register_task_env_overrides()`, which RL and benchmark environments (TerminalBench2, KovaSweEnv, etc.) do automatically for their per-task Docker images.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.kova/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `KOVA_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.kova/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.kova/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `kova-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/kova-agent` (HPC convention) → `~/.kova/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `kova config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. kova logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `kova doctor` to check.
...
For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running kova), kova tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.kova/cache/remote-syncs/<session-id>/`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.kova/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.kova/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.kova/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.kova/cache/documents/report.txt`
...
kova resolves each listed variable from your current shell first, then falls back to `~/.kova/.env` if it was saved with `kova config set`.
...
When enabled, kova appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` — use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. kova does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch kova from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched kova from
...
```bash
kova config set terminal.persistent_shell false
```
...
- `kova config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `kova config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
kova config set skills.config.myplugin.path ~/myplugin-data
```
...
When the agent uses `skill_manage` to create, edit, patch, or delete a skill, kova can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** — real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Controls how much content kova loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.kova.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Three related caps control how much raw output a tool can return before kova truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, kova keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
`kova tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as kova -w)
...
kova automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `kova plugins` → Provider Plugins → Context Engine.
...
`agent.api_max_retries` controls how many times kova retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.

### API Timeouts

kova has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `KOVA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `KOVA_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `KOVA_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `KOVA_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so kova raises this to 30 minutes when it detects a local endpoint. If you explicitly set `KOVA_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default kova disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `KOVA_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
kova turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, kova attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `kova` sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so kova uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](/integrations/providers#xai-grok--responses-api--prompt-caching).
...
Instead of hand-editing YAML, run `kova model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Kova Agent — Auxiliary Models Tutorial"
...
Every model slot in kova — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, kova ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, kova uses that provider's built-in auth and base URL.
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `kova model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `kova model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if kova is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `kova kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that kova tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, kova walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, kova falls back to the main agent model as a final safety net.
...
The shape mirrors what OpenRouter accepts in the chat completions request body. kova forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.kova/.env`):
...
| `"nous"` | Force Nous Portal | `kova auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `kova model` → Codex |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `kova model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `kova model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `kova model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, kova uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
```
Run `kova model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider kova uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `kova config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
field — OpenRouter ignores it for them. kova transparently routes your
...
When `display.file_mutation_verifier` is `true` (default), kova appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `KOVA_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.
...
You can also set this per-session with the `KOVA_LANGUAGE` env var, which overrides the config value.
...
When `display.runtime_footer.enabled: true`, kova appends a small runtime-context footer to the **final** message of each gateway turn. The current footer can show the model, context-window percentage, and current working directory. Off by default; opt in per-gateway if your team wants every reply to include this provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, kova sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, kova falls back automatically in this order: `local` → `groq` → `openai`.
...
When the cap is reached, kova returns a direct limit message for new sessions.
Existing active sessions keep their normal behavior.

The canonical key is top-level `max_concurrent_sessions`. kova also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: kova
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$KOVA_HOME`
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want kova to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. kova still keys DMs by chat/DM ID as usual.
...
Control what kova does when an unknown user sends a direct message:
...
- `pair` is the default for chat-style DM platforms. kova denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.kova/kova-agent && git pull && pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (kova's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `kova tools`:
...
**Exa:** Set `EXA_API_KEY` in `~/.kova/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.kova/browser_recordings/
  # Optional CDP override — when set, kova attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when kova creates a tab
...
Control how kova handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `KOVA_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: kova chat --checkpoints). Default: false (opt-in).
...
kova uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.kova/SOUL.md` or `$KOVA_HOME/SOUL.md` |
| `.kova.md` / `kova.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.kova.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- kova automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`kova`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.kova/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.kova/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
