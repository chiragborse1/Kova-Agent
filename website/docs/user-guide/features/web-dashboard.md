...
The web dashboard is a browser-based UI for managing your Kova Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `kova setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
kova dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
kova dashboard --host 0.0.0.0
...
child with the selected profile's `KOVA_HOME`, so the conversation runs
...
processes (manage them via `kova -p <name> gateway …`), each profile's
...
The Chat tab is part of every `kova dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
KOVA_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `kova config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated kova instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `kova profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `kova skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
`kova mcp catalog` / `kova mcp install` use.
...
Connect kova to any messaging platform from the browser — full parity with
`kova setup gateway`. The page lists every supported channel (Telegram,
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and kova versions, CPU core count + utilization, memory, disk usage of the kova home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The kova version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `kova update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `kova portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `kova curator`.
...
`KOVA_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/kova/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Kova Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `kova dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `kova dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, kova doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `kova dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://kova.example.com/auth/callback` and set `KOVA_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `KOVA_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `KOVA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `KOVA_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `KOVA_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `KOVA_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/kova/   # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `KOVA_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `KOVA_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `KOVA_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-kova.json` — it defines a `kova` realm, a **public PKCE client** (`kova-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "kova-dashboard",
      "name": "Kova Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-kova.json:/opt/keycloak/data/import/realm-kova.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/kova/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/kova`). The admin console is at
...
```bash
export KOVA_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/kova"
export KOVA_DASHBOARD_OIDC_CLIENT_ID="kova-dashboard"
export KOVA_DASHBOARD_PUBLIC_URL="http://localhost:9119"
kova dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
> admin console (Clients → kova-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `KOVA_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/kova"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `KOVA_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `KOVA_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `KOVA_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `KOVA_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `KOVA_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `KOVA_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$KOVA_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```bash
# Quick env-var path.
KOVA_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  kova dashboard --host 0.0.0.0
...
kova desktop can drive a kova backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [kova desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`kova dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
kova dashboard --no-open --host 0.0.0.0 --port 9119
```
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/kova` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `KOVA_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `KOVA_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `KOVA_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `KOVA_DASHBOARD_BASIC_AUTH_USERNAME` / `KOVA_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `KOVA_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `KOVA_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `kova update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `kova dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **kova Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **kova Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
