...
Install kova normally, then add the ACP extra:
...
For Zed registry installs, Zed launches kova through the official ACP Registry entry. That entry uses a `uvx` distribution that runs:

```bash
uvx --from 'kova-agent[acp]==<version>' kova-acp
```
...
This is the standalone command. The Zed registry's terminal-auth flow (`kova acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.
...
3. Search for **Kova Agent**.
4. Install it and start a new kova external-agent thread.

Prerequisites:

- Configure kova provider credentials first with `kova model`, or set them in `~/.kova/.env` / `~/.kova/config.yaml`.
- Install `uv` so the registry launcher can run `uvx --from 'kova-agent[acp]==<version>' kova-acp`.
...
```text
/path/to/kova-agent/acp_registry
```

## Registry manifest

The source copy of kova' official ACP Registry metadata lives at:
...
The upstream registry PR copies those files into the top-level `kova-agent/` directory in `agentclientprotocol/registry`.

The registry entry uses a `uvx` distribution that points directly at the `kova-agent` PyPI release:
...
Provider resolution uses kova' normal runtime resolver, so ACP inherits the currently configured provider and credentials. kova also advertises a terminal auth method (`--setup`) for first-run registry clients; this opens kova' interactive model/provider setup.
...
The underlying `AIAgent` still uses kova' normal persistence/logging paths, but ACP `list/load/resume/fork` are scoped to the currently running ACP server process.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Kova Agent**.
...
or by editing `~/.kova/.env`. Registry clients can also trigger kova' terminal auth flow, which runs the same interactive provider/model setup.

### Zed registry launcher cannot find uv

Install `uv` from the official uv installation docs, then retry the Kova Agent thread from Zed.
