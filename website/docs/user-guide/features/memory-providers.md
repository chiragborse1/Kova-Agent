...
Kova Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
kova memory setup      # interactive picker + configuration
kova memory status     # check what's active
kova memory off        # disable external provider
```
...
When a memory provider is active, kova automatically:
...
**Setup Wizard:**
```bash
kova memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `kova honcho`
If you previously used `kova honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per kova profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.
...
Scans every kova profile, creates host blocks for any profile without one, inherits settings from the default `kova` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `kova memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/Kova-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kova).
...
# Then configure kova
kova memory setup    # select "openviking"
# Or manually:
...
echo "OPENVIKING_API_KEY=..." >> ~/.kova/.env
```
...
`OPENVIKING_AGENT` is kova' peer ID in OpenViking for peer-scoped memories.
...
**Setup (OSS):**
```bash
kova memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
kova memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```
...
**Switching modes:** Re-run `kova memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
| `bank_id` | `kova` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Kova Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/Kova-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
kova memory setup    # select "holographic"
# Or manually:
...
# Then configure kova
kova memory setup    # select "byterover"
# Or manually:
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=kova) |
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kova` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
