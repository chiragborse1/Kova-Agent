...
on macOS/Linux, `install.ps1` on Windows. Use `kova computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `kova tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or kova app). `kova computer-use doctor` will tell you which permission is missing. |
...
kova intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the kova-side `computer_use` action vocabulary — the
...
- **kova spawns its own `cua-driver mcp` child over stdio** — it does
...
- **kova-side skill** — `skills/computer-use/SKILL.md` — teaches the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects kova (planned follow-up), this happens
