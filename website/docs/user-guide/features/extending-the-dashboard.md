...
description: "Build themes and plugins for the kova web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The kova web dashboard (`kova dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.kova/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.kova/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.
...
|-------|---------|------------|--------|
| **kova Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **kova Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except kova Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
#### Calling built-in kova endpoints
...
| `header-left` | Before the kova brand in the top bar. |
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.kova/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.kova/plugins/<name>/dashboard/` | `project` — only when `KOVA_ENABLE_PROJECT_PLUGINS` is set |
...
Check that the file is in `~/.kova/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.kova/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.kova/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `kova dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.kova/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.kova/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
