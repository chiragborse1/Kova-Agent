...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Kova Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.kova/config.yaml`:
...
- **CLI mode** — configured in `~/.kova/config.yaml`, loaded at startup
