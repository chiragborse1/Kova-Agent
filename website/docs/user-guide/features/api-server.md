...
sidebar_position: 14
title: "API Server"
description: "Expose kova-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes kova-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to kova-agent and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
kova itself needs a configured provider and tool backends for the API server to be useful. A [Nous Portal](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `kova setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
# Optional: only if a browser must call kova directly
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus kova' custom `kova.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: kova emits `event: kova.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: kova emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `kova-agent` for the default profile). Required by most frontends for model discovery.
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running kova version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, kova surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while kova asks the active agent to stop at the next safe interruption point.
...
Create a new scheduled job. Body accepts the same shape as `kova cron` — prompt, schedule, skills, provider override, delivery target.
...
External UIs can manage kova sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
## Long-term memory scoping (`X-kova-Session-Key`)

Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-kova-Session-Id` (which rotates on `/new`). Pass `X-kova-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and kova threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.

```http
POST /v1/chat/completions HTTP/1.1
Authorization: Bearer ***
X-kova-Session-Id: transcript-alpha
X-kova-Session-Key: agent:main:webui:dm:user-42
```

Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-kova-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior kova had before.

## System Prompt Handling

When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), kova-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call kova directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.

:::warning Security
The API server gives full access to kova-agent's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `kova-agent` for default profile. |
...
To give multiple users their own isolated kova instance (separate config, memory, skills), use [profiles](/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated kova instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
