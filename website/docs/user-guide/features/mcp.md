...
description: "Connect Kova Agent to external tool servers via MCP — and control exactly which MCP tools kova loads"
---

# MCP (Model Context Protocol)

MCP lets Kova Agent connect to external tool servers so the agent can use tools that live outside kova itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
- Per-server filtering so you can expose only the MCP tools you actually want kova to see
...
4. Ask kova to use the MCP-backed capability.
...
kova will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

kova ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
kova mcp                # interactive picker (default)
kova mcp catalog        # plain-text list, scriptable
kova mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from kova
...
- **OAuth** (third-party provider like Google/GitHub) — kova points you at
  `kova auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, kova probes the MCP server to list every
...
written (if not). Re-run `kova mcp configure <name>` once the server is
reachable to refine.
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/Kova-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed kova
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
kova`) for that entry instead of silently hiding it. Run `kova update`
to install the latest kova when you see that.
...
MCPs are never auto-updated. Re-run `kova mcp install <name>` to refresh
after a kova update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NousResearch/Kova-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints kova connects to directly.
...
- you do not want kova spawning a local subprocess for that integration
...
- **Paste-back (no setup):** on an interactive terminal kova prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `kova mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `kova mcp login` can list the tools and look like it worked, but every real tool call later times out. `kova mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. kova passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
| `enabled` | bool | If `false`, kova skips the server entirely |
...
For well-known MCP servers, `kova mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
You can pick any local name (`kova mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How kova registers MCP tools

kova prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — kova sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, kova also registers utility tools around MCP resources and prompts:
...
- kova only registers resource utilities if the MCP session actually supports resource operations
- kova only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to kova, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, kova skips the server completely and does not even attempt a connection.
...
You can also separately disable kova-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, kova does not create an empty runtime MCP toolset for that server.
...
kova discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify kova when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When kova receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, kova does not blindly pass your full shell environment.
...
Then verify your config and restart kova.
...
Because kova now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, kova may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from kova via the `sampling/createMessage` protocol. This allows an MCP server to ask kova to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through kova
- You want a single MCP server that bridges to all of kova's connected messaging platforms at once
- You already have a running Kova Gateway with connected platforms
...
Add kova to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed kova in a specific location:
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a kova-specific channel browser:
...
The MCP server includes a live event bridge that polls kova's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
kova mcp serve              # Normal mode
kova mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from kova's session store (`~/.kova/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Kova Agent itself.
...
- The embedded `kova mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of kova, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
