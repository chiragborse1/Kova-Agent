...
chat completions — use your kova-managed provider subscription as their
...
| Use case | "Use kova as a chat backend" | "Use my Portal sub from another app" |
...
interface in `KOVA_cli/proxy/adapters/`.
...
If you see `not logged in`, run `kova portal`. If you see
...
`KOVA_cli/proxy/adapters/<provider>.py` and registering it in
