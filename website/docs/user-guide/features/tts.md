...
| CLI | Saved to `~/.kova/audio_cache/` | MP3 |
...
If the file contains `{transcript}` or `{{ transcript }}`, kova replaces that placeholder with the live TTS text. Otherwise, kova appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
tts:
  ...
  gemini:
    voice: Algieba
    persona_prompt_file: ~/.kova/tts/butler-voice.md
```

### Gemini Audio Tags

Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have kova run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
Each provider has a documented per-request input-character cap. kova truncates text before calling the provider so requests never fail with a length error:
...
**Install via `kova tools`** → Voice & TTS → Piper — kova runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
On the first TTS call for a voice that isn't cached locally, kova runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.kova/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](#example-doubao--volcengine-asr) for kova integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. kova substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file kova wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if kova picks a path.      |
| `voice_compatible` | `false` | When `true`, kova converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. kova quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input — treat it the same way you would a shell script on your PATH.
...
| A voice-listing API used by `kova setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.kova/plugins/my-tts/`:
...
Enable it (`kova plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `kova tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `kova tools` / `kova setup`. Without this, the plugin still works but its row in the picker is minimal.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, kova can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `KOVA_LOCAL_STT_COMMAND`.
:::
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.kova/kova-agent && uv pip install -e ".[mistral]"`.
...
**Custom local CLI fallback** — Set `KOVA_LOCAL_STT_COMMAND` if you want kova to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export KOVA_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
kova writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

If your configured provider isn't available, kova automatically falls back:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `KOVA_LOCAL_STT_COMMAND` before cloud providers
...
This complements the legacy `KOVA_LOCAL_STT_COMMAND` escape hatch — that env var still works untouched via the built-in `local_command` path. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.

#### STT placeholders

Your command template can reference these placeholders. kova substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty → kova reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → kova uses that.
...
For `format: json` / `srt` / `vtt`, kova returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as kova with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `KOVA_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `KOVA_LOCAL_STT_COMMAND` env var (preserved for back-compat)  |
...
Drop this in `~/.kova/plugins/my-stt/`:
...
Enable it (`kova plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `kova tools` / `kova setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
