...
**Tool Search** is kova' opt-in progressive-disclosure layer for that
...
:::info Built-in kova tools never defer
The tools that make up kova' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, `send_message`, and the rest of
`_KOVA_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, kova **unwraps the bridge** and
...
- **No JS sandbox.** kova uses the simpler "structured tools" mode
