...
kova can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.kova/auth.json` and refreshed automatically on 401; you only log in once per machine (refresh tokens expire after ~6 months; re-run `kova auth spotify` when they do).

Unlike kova' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `kova auth spotify` walks you through it.
...
- Kova Agent installed and running.
...
### One-shot: `kova tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `kova setup` / `kova setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `kova tools`.

kova drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the toolset is enabled AND authenticated in one pass.
...
If no `KOVA_SPOTIFY_CLIENT_ID` is set, kova walks you through the app registration inline:
...
4. Saves it to `~/.kova/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.kova/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
| Field | Value |
|-------|-------|
| App name | anything (e.g. `kova-agent`) |
| App description | anything (e.g. `personal kova integration`) |
| Website | leave blank |
| Redirect URI | `http://127.0.0.1:43827/spotify/callback` |
| Which API/SDKs? | check **Web API** |

Agree to the terms and click **Save**. On the next page click **Settings** → copy the **Client ID** and paste it into the kova prompt. That's the only value kova needs — PKCE doesn't use a client secret.

### Running over SSH / in a headless environment

If `SSH_CLIENT` or `SSH_TTY` is set, kova skips the automatic browser open during both the wizard and the OAuth step. Copy the dashboard URL and the authorization URL kova prints, open them in a browser on your local machine, and proceed normally — the local HTTP listener still runs on the remote host on port `43827`. Your laptop's browser can't reach the remote loopback without an SSH local-forward:
...
```bash
kova auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across kova restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `kova auth logout spotify`.
...
If Home Assistant manages speakers that already support Spotify Connect (for example Sonos, Echo, Nest, or other Connect-capable speakers), they appear in `spotify_devices list` automatically whenever Spotify can see them. kova does not need a Home Assistant ↔ Spotify bridge for this path — Spotify handles the device routing natively.

Ask kova to transfer playback by the speaker's display name (for example, “transfer Spotify to the kitchen speaker”), or call `spotify_devices list` and pass the exact `device_id` to `spotify_devices transfer` when scripting. If the speaker is missing, open the Spotify app or the speaker's Spotify integration once so Spotify registers it as an active Connect target.
...
Because Spotify tools are regular kova tools, a cron job running in a kova session can trigger playback on any schedule. No new code needed.
...
1. Cron spins up a headless kova session.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `kova tools` for the cron session to see the Spotify tools.
...
Removes tokens from `~/.kova/auth.json`. To also clear the app config, delete `KOVA_SPOTIFY_CLIENT_ID` (and `KOVA_SPOTIFY_REDIRECT_URI` if you set it) from `~/.kova/.env`, or run the wizard again.
...
**`204 No Content` on `get_currently_playing`** — nothing is currently playing on any device. This is Spotify's normal response, not an error; kova surfaces it as an explanatory empty result (`is_playing: false`).

**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what kova is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `KOVA_SPOTIFY_REDIRECT_URI` in `~/.kova/.env` to whatever you registered.

**`429 Too Many Requests`** — Spotify's rate limit. kova returns a friendly error; wait a minute and retry. If this persists, you're probably running a tight loop in a script — Spotify's quota resets roughly every 30 seconds.

**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `kova auth spotify` again.

**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, kova detects it and skips the auto-open. Copy the dashboard URL it prints and open it manually.

## Advanced: custom scopes

By default kova requests the scopes needed for every shipped tool. Override if you want to restrict access:

```bash
kova auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
Or set them permanently in `~/.kova/.env`:

```
KOVA_SPOTIFY_CLIENT_ID=<your_id>
KOVA_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| File | Contents |
|------|----------|
| `~/.kova/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.kova/.env` | `KOVA_SPOTIFY_CLIENT_ID`, optional `KOVA_SPOTIFY_REDIRECT_URI` |
