...
kova kanban owns lifecycle truth — `ready` → `running` → `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-kova external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
- **Block instead of complete**, with `reason` prefixed `review-required: ` so the dashboard / `kova kanban show` surfaces the row as awaiting review.
...
The historical issue for this is [#19931](https://github.com/NousResearch/Kova-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/NousResearch/Kova-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- [`KANBAN_GUIDANCE`](https://github.com/NousResearch/Kova-agent/blob/main/agent/prompt_builder.py) — the worker + orchestrator lifecycle injected into every kanban worker's system prompt.
