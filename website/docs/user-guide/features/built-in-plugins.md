...
description: "Plugins shipped with Kova Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a kova Plugin](/guides/build-a-kova-plugin) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `kova memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `kova plugins list` and the interactive `kova plugins` UI), but none load until you explicitly enable them:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer kova. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
kova plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `kova plugins enable <name>`.
...
| `kova-achievements` | dashboard tab | Steam-style collectible badges generated from your real kova session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `kova memory` and `kova plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KOVA_HOME` or `/tmp/kova-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under KOVA_HOME | always | Never |
...
**State** — everything lives at `$KOVA_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `KOVA_HOME` or `/tmp/kova-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `kova plugins enable disk-cleanup` (or check the box in `kova plugins`).

**Disabling again:** `kova plugins disable disk-cleanup`.
...
**Enabling:** `kova plugins enable security-guidance` (or check the box in `kova plugins`).

**Disabling again:** `kova plugins disable security-guidance`.
...
Traces kova turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of kova' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `kova logs`.
...
```bash
kova tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart kova and the next turn ships a trace.
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "kova turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `KOVA_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the kova session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `kova chat` session lives under one Langfuse session.

**Verify:**

```bash
kova plugins list                 # observability/langfuse should show "enabled"
kova chat -q "hello"              # check the Langfuse UI for a "kova turn" trace
```
...
| `KOVA_LANGFUSE_RELEASE` | — | Release/version tag |
| `KOVA_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `KOVA_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `KOVA_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

kova-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — kova-prefixed wins when both are set.
...
**Disabling:** `kova plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.kova/cache/google_meet/<meeting_id>/`
...
**Disabling:** `kova plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.kova/cache/google_meet/` until you remove them.

### kova-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real kova session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with kova feature changes.
...
- Unlock state is persisted to `$KOVA_HOME/plugins/kova-achievements/state.json`
...
| Secret | Hidden until kova detects the first related signal in your history |

**API** — routes mount under `/api/plugins/kova-achievements/`:
...
**State files** — live under `$KOVA_HOME/plugins/kova-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across kova updates. |
...
**Opting out:** Delete or rename `plugins/kova-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.kova/plugins/kova-achievements/` that ships no dashboard. The plugin's state files under `$KOVA_HOME/plugins/kova-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other kova plugin — see [Build a kova Plugin](/guides/build-a-kova-plugin). The only differences are:
