...
Type `@` followed by a reference to inject content directly into your message. kova expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- kova env: `$KOVA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$KOVA_HOME/skills/.hub/`
