...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, kova automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, kova auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
kova auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
kova auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
kova auth add anthropic --type oauth
...
Run `kova auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `kova auth` | Interactive pool management wizard |
| `kova auth list` | Show all pools and credentials |
| `kova auth list <provider>` | Show a specific provider's pool |
| `kova auth add <provider>` | Add a credential (prompts for type and key) |
| `kova auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `kova auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `kova auth remove <provider> <index>` | Remove credential by 1-based index |
| `kova auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `kova auth` → "Set rotation strategy" or in `config.yaml`:
...
```bash
# After setting up a custom endpoint via kova model:
kova auth list
# Shows:
...
kova auth add Together.ai --api-key sk-together-second-key
```
...
kova automatically discovers credentials from multiple sources and seeds the pool on startup:
...
Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `kova auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. kova can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and kova-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`KOVA_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`KOVA_cli/runtime_provider.py`** — Pool-aware credential resolution
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to kova' credential store, so its token remains persistable.
