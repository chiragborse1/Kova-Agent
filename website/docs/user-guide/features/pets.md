...
kova can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<KOVA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), kova renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `kova pets list [query] [--limit N]` |
| List installed pets | `kova pets list --installed` |
| Install a pet | `kova pets install <slug> [--select] [--force]` |
...
| Preview/animate | `kova pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `kova pets off` |
| Remove an installed pet | `kova pets remove <slug>` |
| Diagnose setup | `kova pets doctor` |

`kova pets show` flags:
...
always-on-top desktop window. Out there it stays visible while kova is
...
Run `kova pets doctor` — it reports:
