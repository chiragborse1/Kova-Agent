...
The Tool Gateway is included with every paid [Nous Portal](https://portal.nousresearch.com) subscription. It routes kova' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let kova default to FLUX 2 Klein. |
...
```bash
kova setup --portal     # Fresh install: Nous OAuth + set Nous as provider + turn on the Tool Gateway in one go
```

```bash
kova model              # Switch your inference provider to Nous Portal — kova then offers to turn on the gateway for all tools
```

```bash
kova tools              # Enable the gateway per-tool — pick "Nous Subscription" for any tool you want
```

`kova setup --portal` and `kova model` are the all-at-once paths: log in once, optionally flip every tool to the gateway. `kova tools` is the à la carte path — turn on just the tools you want, one at a time.

**You don't have to log in first.** With `kova tools`, the Nous-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into Nous Portal. Select one and kova runs the Portal login right there if you aren't already authenticated — no need to run `kova model` beforehand. If your Nous OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.

Check what's active at any time:

```bash
kova portal info        # Portal auth + Tool Gateway routing summary
kova portal tools       # Gateway catalog with current routing per tool
kova status             # Full system status (Tool Gateway is one section)
```

`kova portal info` shows a section like:
...
```bash
kova tools          # Interactive picker for each tool category
```

Select the tool, pick **Nous Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into Nous Portal yet, picking **Nous Subscription** kicks off the Portal login inline — you don't need to authenticate through `kova model` first.
...
The set evolves — `kova tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `kova model` and `kova tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # kova now uses FIRECRAWL_API_KEY from .env
```

`kova tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.kova/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `kova tools`. kova shows a clear error pointing at the portal.
...
Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `kova setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, kova skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
