...
kova can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — kova becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default kova behavior is unchanged unless you flip the flag. kova never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `kova setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your kova session.
- **kova' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into kova for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into kova' message shape so the self-improvement loop sees a normal-looking transcript.
...
These ship with `codex app-server` itself — no kova involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of kova' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. kova also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, kova queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. kova tool callback (MCP server, registered in `~/.codex/config.toml`)

kova registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through kova' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from kova' skill library.
- **`text_to_speech`** — TTS through kova' configured provider.

When the model wants one of these, codex spawns the `KOVA_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as kova' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four kova tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — kova' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — kova' todo store (codex's `update_plan` is the in-runtime equivalent)
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which kova does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `kova chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- The kova tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `KOVA_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `KOVA_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `KOVA_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `kova-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, kova also passes narrow app-server sandbox overrides when `KOVA_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`KOVA_KANBAN_WORKSPACES_ROOT`, `KOVA_KANBAN_WORKSPACE`, legacy `KOVA_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/NousResearch/Kova-agent/issues/27941)).
...
|  | kova default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   kova' own `kova auth login codex` writes to `~/.kova/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, kova auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   kova will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
- Migrates user MCP servers from `~/.kova/config.yaml` to `~/.codex/config.toml`.
...
- **Registers kova' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
You can also set it manually in `~/.kova/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= kova runtime)
```

## Self-improvement loop (memory + skill nudges)

kova' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default kova runtime.
...
One detail: the review fork itself needs to call kova' agent-loop tools (`memory`, `skill_manage`), which require kova' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so kova owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into kova' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, kova shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (kova' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside kova' managed block:

```toml
default_permissions = ":read-only"
```

(kova will preserve your override on re-migration as long as it lives outside the `# managed by kova-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because kova' auxiliary client uses the main provider/model when no per-task override is set.
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.kova/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and kova downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — kova' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

kova wraps everything it manages between two marker comments:
...
- Add your own MCP servers kova doesn't know about
...
By default, kova points the codex subprocess at `~/.codex/` regardless of which kova profile is active. This means `kova -p work` and `kova -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `KOVA_HOME`:

```bash
# Inside the work profile, you might wrap kova:
CODEX_HOME=~/.kova/profiles/work/codex kova chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `kova -p work` will operate on isolated Codex state.
...
kova does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
kova' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| kova (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- kova-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, kova writes a `[plugins."<name>@openai-curated"]` block enabling it in your kova session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable kova' codex runtime, kova activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; kova doesn't touch credentials.

## kova tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, kova registers itself as an MCP server in `~/.codex/config.toml`:
...
When the model calls `web_search` (or another exposed kova tool), codex spawns the `KOVA_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default kova runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Kova Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via kova UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `kova-tools` callback)
...
- kova web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **kova auth and codex auth are separate sessions.** You need both `codex login` AND `kova auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in kova' `_import_codex_cli_tokens` — kova won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/Kova-agent/issues) with the output of `kova logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── kova shell (CLI / TUI / gateway) ───┐
...
For implementation details, see [PR #24182](https://github.com/NousResearch/Kova-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
