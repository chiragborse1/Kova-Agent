...
Kova Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — kova can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — kova still honors it for back-compat, but `kova fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Nous Portal | `nous` | `kova setup --portal` (fresh) or `kova auth add nous` (OAuth) |
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `kova model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `kova model` (Qwen Portal OAuth; optional: `KOVA_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `kova model` (MiniMax portal OAuth) |
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `kova fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
kova uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `kova kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), kova first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, kova now honors user-configured fallback policy before using the built-in discovery chain:
...
Every task above follows the same **provider / model / base_url** pattern. Each task can also declare its own `fallback_chain`; if omitted, `provider: auto` uses the top-level `fallback_providers` chain before kova' built-in auxiliary discovery chain.
...
| `"codex"` | Force Codex OAuth | `kova model` → Codex |
...
`base_url` takes precedence over `provider`. kova uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), kova treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), kova falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, kova logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
kova recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and kova doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
If no provider is available for compression, kova drops middle conversation turns without generating a summary rather than failing the session.
