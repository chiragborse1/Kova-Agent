...
|------------|--------|-------|
| **SuperGrok / X Premium+ OAuth** (preferred) | Browser login at `accounts.x.ai`, refreshed automatically | `kova auth add xai-oauth` — see [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** | Paid xAI API key | Set in `~/.kova/.env` |
...
Auto-enables when xAI credentials (OAuth token or `XAI_API_KEY`) are present. Disable explicitly via `kova tools` → Search → x_search if you don't want this.
...
The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.kova/.env` or run `kova auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
...
1. **Toolset not enabled.** Run `kova tools` and confirm `🐦 X (Twitter) Search` is checked.
2. **No xAI credentials.** The check_fn returns False, so the schema stays hidden. Run `kova auth status` to confirm xai-oauth login state, and check that `XAI_API_KEY` is set (if you're using the API-key path).
