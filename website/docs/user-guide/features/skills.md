...
All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `kova update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `kova skills opt-in`) to re-enable.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling kova to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.kova/plans/` relative to the active workspace/backend working directory.
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). kova fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.kova/.hub/taps.json`:
...
Taps are stored in `~/.kova/.hub/taps.json` (created on demand).
