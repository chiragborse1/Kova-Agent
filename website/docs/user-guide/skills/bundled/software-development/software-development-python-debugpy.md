...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), `debugging-kova-tui-commands` |
...
Easiest: add `breakpoint()` near the suspect line, then run `kova` normally. Control returns to your terminal at the pause point.
...
debugpy.wait_for_client()
```
Start `kova --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
