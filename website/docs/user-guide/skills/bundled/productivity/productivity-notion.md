...
Add those exports to your shell profile (or to `${KOVA_HOME:-~/.kova}/.env`) so every session inherits them.
...
curl -s -X PATCH "https://api.notion.com/v1/blocks/{page_id}/children" \
  ...
  -d '{
    "children": [
      {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "Hello from kova!"}}]}}
    ]
  }'
```
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via kova' MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
