...
| Version | `1.1.1` |
| Author | xdevplatform + openclaw + Kova Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by kova tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official kova Docker layout, `/opt/data` is `KOVA_HOME`, but kova tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for kova-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, kova tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the kova subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- kova adaptation: reformatted for kova skill conventions; safety guardrails preserved verbatim.
