...
If `gsd-sketch` shows up as a sibling skill (installed via `npx get-shit-done-cc --kova`), prefer **`gsd-sketch`** for the full workflow: persistent `.planning/sketches/` with MANIFEST, frontier mode analysis, consistency audits across past sketches, and integration with the rest of GSD. This skill is the lightweight standalone version — one-off sketching without the state machinery.
...
**Verify variants visually — use kova' browser tools.** Don't just write HTML and hope it renders; load each variant and look at it:
