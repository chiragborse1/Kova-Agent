...
description: "Use when a kova kanban worker wants to run Codex CLI as an isolated implementation lane while kova keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a kova kanban worker wants to run Codex CLI as an isolated implementation lane while kova keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`kova-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kova-agent) |
...
- A bounded diff can be evaluated by kova in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- kova can run the relevant tests itself after Codex exits.
...
1. kova owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any kova board CLI as a substitute for the worker.
2. kova owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. kova owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from kova with the repo's canonical wrapper.
4. kova owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. kova owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${KOVA_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional kova edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if kova can do the task directly.
...
kova owns the Kanban lifecycle; do not call kova kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on kova reconciliation.
...
- Explicit statement: kova owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands kova will run afterward.
...
kova must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by kova.
...
- [ ] kova ran the canonical tests itself, using `scripts/run_tests.sh` for Kova Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from kova-run tests.
- [ ] Accepted commits/diffs were applied to the kova-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "kova"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct kova edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from kova.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but kova writes board state.
...
- [ ] kova reviewed `git diff` and safety-sensitive files.
- [ ] kova ran canonical tests independently.
