...
kova drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The kova-side `computer_use` tool exposed
in this skill is a higher-level kova vocabulary; the raw cua-driver
...
|---|---|
| `cua-driver not installed` | Run `kova computer-use install`, or `kova tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `kova computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `kova computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `kova computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
kova intentionally keeps THIS skill focused on the kova-side
...
When `cua-driver skills install` autodetects kova (planned follow-up
