...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for kova. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/googleworkspace/cli) when available for broader coverage, and falls back to Google's Python client libraries otherwise.

**Skill path:** `skills/productivity/google-workspace/`

## Setup

The setup is fully agent-driven — ask kova to set up Google Workspace and it walks you through each step. The flow:
...
3. **Authorize** — kova generates an auth URL, you approve in the browser, paste back the redirect URL
...
| Problem | Fix |
|---------|-----|
| `NOT_AUTHENTICATED` | Run setup (ask kova to set up Google Workspace) |
