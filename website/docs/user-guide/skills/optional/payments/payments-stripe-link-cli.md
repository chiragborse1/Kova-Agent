...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/payments/stripe-link-cli` |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so kova can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — kova cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before kova attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "kova" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with kova' native MCP:

```
kova mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `kova mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
