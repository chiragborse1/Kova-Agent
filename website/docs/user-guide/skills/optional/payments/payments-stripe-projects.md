...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/payments/stripe-projects` |
...
Wraps the [Stripe Projects](https://projects.dev) CLI plugin so kova can provision SaaS services (Neon, Twilio, Vercel, etc.), generate and sync credentials into the user's `.env`, and manage billing across providers from one place.
