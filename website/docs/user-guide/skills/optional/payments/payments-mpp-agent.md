...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so kova can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
