...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. kova-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kova skills install official/email/agentmail` |
...
Add to `~/.kova/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
```
kova --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
