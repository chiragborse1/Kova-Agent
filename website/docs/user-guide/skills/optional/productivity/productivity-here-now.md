...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${KOVA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${KOVA_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `kova`)    |
