...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/finance/3-statement-model` |
...
If the template specifically requires pulling data from SEC filings (10-K, 10-Q), see [references/sec-filings.md](https://github.com/NousResearch/Kova-agent/blob/main/optional-skills/finance/3-statement-model/references/sec-filings.md) for detailed extraction guidance. This reference is only needed when populating templates with public company data from regulatory filings.
...
See [references/formulas.md](https://github.com/NousResearch/Kova-agent/blob/main/optional-skills/finance/3-statement-model/references/formulas.md) for all formula details.
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In kova:

- **If you have any structured financial-data MCP configured** (kova supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
