...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into kova or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to kova. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is kova integration, either:

- configure the server in `~/.kova/config.yaml` using the `native-mcp` skill, or
...
### kova cannot see the deployed server

The server-building part may be correct while the kova config is not. Load the `native-mcp` skill and configure the server in `~/.kova/config.yaml`, then restart kova.
