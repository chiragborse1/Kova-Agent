...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/software-development/rest-graphql-debug` |
...
Drive REST and GraphQL diagnosis through kova tools — `terminal` for `curl`, `execute_code` for Python `requests`, `web_extract` for vendor docs. Isolate the failing layer before guessing at the fix.
...
- Never hardcode tokens in scripts. Read from env (`os.environ["API_TOKEN"]`) or `${KOVA_HOME:-~/.kova}/.env`.
