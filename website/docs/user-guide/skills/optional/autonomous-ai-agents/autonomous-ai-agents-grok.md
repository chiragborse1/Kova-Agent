...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`kova-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kova-agent) |
...
autonomous coding agent CLI, the `grok` command) via the kova terminal. Grok
...
1. Prepare stable input files first with kova tools (`read_file`,
...
2. **Don't conflate kova' xAI auth with the `grok` CLI's auth.** kova'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume kova' xAI auth carries
   over.
