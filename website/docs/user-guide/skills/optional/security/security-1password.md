...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Kova Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during kova terminal calls (desktop app flow only)
...
### Service Account (recommended for kova)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${KOVA_HOME:-~/.kova}/.env` (the skill will prompt for this on first load).
...
## kova Execution Pattern (desktop app flow)

kova terminal commands are non-interactive by default and can lose auth context between calls.
