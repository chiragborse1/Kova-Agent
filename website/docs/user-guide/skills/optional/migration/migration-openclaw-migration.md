---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Kova Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Kova Agent"
---
...
Migrate a user's OpenClaw customization footprint into Kova Agent. Imports kova-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> kova Migration

Use this skill when a user wants to move their OpenClaw setup into Kova Agent with minimal manual cleanup.
...
```bash
kova claw migrate              # Full interactive migration
...
kova claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `kova setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- merge OpenClaw command approval patterns into kova `command_allowlist`
- migrate kova-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to kova working-directory configuration
- copy OpenClaw skills into `~/.kova/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen kova workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.kova/tts/`
- archive non-secret docs that do not have a direct kova destination
...
1. Prefer the installed path under `~/.kova/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of kova-compatible secrets, currently:
...
Kova CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in kova. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a kova workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing kova skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of kova unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty kova targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean kova destination. Unsupported auth blobs must still be reported as skipped.
...
- kova persona state imported
- kova memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.kova/skills/openclaw-imports/`
