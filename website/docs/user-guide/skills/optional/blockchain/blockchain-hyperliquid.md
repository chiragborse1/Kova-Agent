...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Kova Agent |
| License | MIT |
...
The script reads `${KOVA_HOME:-~/.kova}/.env` for two optional defaults:
...
Helper script: `~/.kova/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.kova/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${KOVA_HOME:-~/.kova}/.env`.
...
```bash
python3 ~/.kova/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.kova/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
