...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/devops/pinggy-tunnel` |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. kova' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
