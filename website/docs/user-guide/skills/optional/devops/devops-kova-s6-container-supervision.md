---
title: "Kova S6 Container Supervision"
sidebar_label: "Kova S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the Kova Agent Docker image — adding new services, debugging profile gateways, understanding ..."
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Kova S6 Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the Kova Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
...
# kova s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the kova Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/kova/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the Kova Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-kova-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
│   ├── main-kova/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if KOVA_DASHBOARD=1, runs `kova dashboard`
│
...
└── CMD ("main program")               ← /opt/kova/docker/main-wrapper.sh
    └── routes user args: bare exec | kova subcommand | kova (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/kova/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-kova-setup. |
| `docker/cont-init.d/02-reconcile-profiles` | Calls `KOVA_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to kova via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-kova/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main kova runs as the CMD, not as a supervised service. |
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `KOVA_DASHBOARD` is truthy. |
...
| `KOVA_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `KOVA_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `KOVA_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `Kova Gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main kova to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:

1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `KOVA_ARGS` for a service `run` script to consume.
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/kova/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to kova via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main kova is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to kova via `s6-setuidgid kova` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `KOVA_cli/service_manager.py`. The function is also called by `KOVA_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/KOVA_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.

### Run the docker test harness

```sh
docker build -t kova-agent-harness:latest .
KOVA_TEST_IMAGE=kova-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `kova -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `kova profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `kova-agent-dev`: General kova-agent codebase navigation
- `kova-tool-quirks`: Specific kova-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with kova built-in tools.
