...
Kova Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.kova/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
kova stores session history so it can resume conversations, but it does not
...
kova explicitly injects for that turn.
...
For example, if a user sends an image and asks kova to make a meme from it,
kova may inspect that image once with vision and run an image-processing
...
`kova sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`kova` or `kova chat`) |
...
kova -c "my project"   # → resumes "my project #3"
```
...
kova --resume 20250305_091523_a1b2c3d4
kova -r 20250305_091523_a1b2c3d4

# Resume by title
kova --resume "refactoring auth"

# Or with the chat subcommand
kova chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `kova sessions list`.

### Conversation Recap on Resume

When you resume a session, kova displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a kova session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.kova/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `kova -r "<title>"` from the shell) and pick up where the platform left off.
...
kova automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `kova sessions list` or `kova sessions browse`.
...
```bash
kova sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), kova creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`kova -c "my project"`), it automatically picks the most recent session in the lineage.
...
kova provides a full set of session management commands via `kova sessions`:
...
# Filter by platform
kova sessions list --source telegram

# Show more sessions
kova sessions list --limit 50
```
...
kova sessions export backup.jsonl

# Export sessions from a specific platform
kova sessions export telegram-history.jsonl --source telegram

# Export a single session
kova sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
kova sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
kova sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
kova sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
kova sessions prune --older-than 30

# Only prune sessions from a specific platform
kova sessions prune --source telegram --older-than 60

# Skip confirmation
kova sessions prune --older-than 30 --yes
```
...
When kova cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, kova uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to kova in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.kova/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.kova/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.kova/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.kova/sessions/sessions.json` is the **gateway routing index** — it maps
...
`kova sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.kova/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `kova sessions list`, the cause is
`state.db` not receiving them — run `kova sessions repair` and watch for a
...
`*.jsonl` files in `~/.kova/sessions/`. They are no longer written or
read by kova. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every kova process in the same `KOVA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.kova/config.yaml`:
...
kova sessions delete <session_id>
