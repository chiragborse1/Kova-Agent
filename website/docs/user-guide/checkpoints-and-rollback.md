...
Kova Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
kova chat --checkpoints
```

Or enable globally in `~/.kova/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.kova/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `kova checkpoints` | Show total size, project count, per-project breakdown |
| `kova checkpoints status` | Same as bare `checkpoints` |
| `kova checkpoints list` | Alias for `status` |
| `kova checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `kova checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `kova checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- kova detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.kova/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/kova/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.kova/checkpoints/ at startup
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `kova checkpoints prune` manually.
...
kova responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.kova/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/kova-agent                                        20       2h ago  live
...
Clear with: kova checkpoints clear-legacy
```
...
- **Directory scope** — kova skips overly broad directories (root `/`, home `$HOME`).
...
~/.kova/checkpoints/
  ...
  │   ├── refs/kova/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `kova checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.kova/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.kova/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
kova checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `kova chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `kova checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each kova session in its own worktree/branch, with checkpoints as an extra layer.
