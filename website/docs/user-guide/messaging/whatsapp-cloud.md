...
description: "Set up Kova Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

kova can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Kova Gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a kova limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. kova degrades gracefully if absent.
...
## Exposing kova to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Kova Gateway has to be reachable from Meta's servers.  Three common ways:
...
The `kova whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.
...
When the agent invokes any of these flows, kova uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
kova acknowledges inbound messages immediately:
...
kova TTS produces MP3. Two paths:
...
Message-template support (the workaround for outside-window sends) is not yet implemented in kova. If you need it, please [open an issue](https://github.com/NousResearch/Kova-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  kova's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  kova doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- Wait for template support to land in kova.
...
```bash
kova config set stt.provider openai
kova config set stt.use_gateway true
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that kova will accept as authentic.
...
| | Baileys (`kova whatsapp`) | Cloud API (`kova whatsapp-cloud`) |
|---|---|---|
...
Most users running kova for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
