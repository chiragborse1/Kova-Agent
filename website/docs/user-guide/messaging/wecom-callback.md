...
Connect kova to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
kova supports two WeCom integration modes:
...
> Run `Kova Gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. kova decrypts the message, queues it for the agent
...
(Use `Kova Gateway start` only after `Kova Gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in kova and the token the
...
in `~/.kova/.env` values around `=` will also break signature checks. After
fixing, restart `Kova Gateway run`.
...
Check `Kova Gateway run` logs for the bound host/port. If the adapter bound to
