...
description: Control your smart home with Kova Agent via Home Assistant integration.
...
Kova Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Kova Agent"
5. Copy the token
...
Kova Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.kova/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Kova Agent".
...
The adapter reads credentials from `~/.kova/.env` (auto-merged at startup) or
