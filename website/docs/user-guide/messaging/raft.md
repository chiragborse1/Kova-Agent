...
sidebar_position: 19
title: "Raft"
description: "Connect Kova Agent to Raft as an external agent via wake-channel bridge"
---

# Raft Setup

kova connects to [Raft](https://raft.build) as an external agent through a local wake-channel bridge. The adapter starts a loopback HTTP endpoint that receives content-free wake hints from the bridge, then injects them into the Kova Gateway session pipeline. The agent reads and sends messages through the Raft CLI — the adapter never touches message bodies or delivery cursors.
...
- **The kova adapter** owns: a localhost wake endpoint and injecting a short notice into the agent's context.
...
- **aiohttp** — Python package (included in kova `[all]` extras)

In Raft, open the Agents menu, create an External Agent, and follow the setup card to install the Raft CLI and log in the agent profile. Once the agent is created, Raft shows a kova setup guide with the environment variables and configuration needed to start the gateway.
...
```
Raft Server → Bridge (wake-hints SSE) → POST /wake → kova Adapter → Agent context
...
3. The adapter validates the bridge token, verifies the payload is content-free, and injects a wake notice into the kova session.
