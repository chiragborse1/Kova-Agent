...
Connect kova to **iMessage** through [Photon][photon], a managed
...
your first iMessage from kova — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, kova runs it in a
...
kova photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Kova Agent` project on your account.
...
Runtime credentials are written to `~/.kova/.env`
...
(device token, dashboard project id) lives in `~/.kova/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other kova
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, kova replies with a pairing code. Approve it with:

```bash
kova pairing approve photon <CODE>
```

Use `kova pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.kova/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.kova/.env`):
...
By default kova responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `kova` and
`@Kova Agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and kova will reply.
...
assigned iMessage line kova uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run kova photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `kova photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `kova photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `kova photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** kova sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | kova wake words  | JSON list / comma / newline regex patterns for group mentions |
