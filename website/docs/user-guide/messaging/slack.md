...
public HTTP endpoints, so your kova instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. kova uses the modern
...
The fastest path is to paste a manifest kova generates for you. It
...
### Option A: From a kova-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   kova slack manifest --write
   ```
   This writes `~/.kova/slack-manifest.json` and prints paste-in
   instructions.
...
it will only work in DMs. Without `files:read`, kova can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `kova-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a kova configuration issue.
:::
...
kova uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
Kova Gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
Kova Gateway              # Foreground
Kova Gateway install      # Install as a user service
sudo Kova Gateway install --system   # Linux only: boot-time system service
```
...
Every kova command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
kova command with its description.

Under the hood: kova ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/Kova-agent/blob/main/KOVA_cli/commands.py)
...
When kova adds new commands (e.g. after `kova update`), regenerate
...
3. Paste the new contents of `~/.kova/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/kova <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/kova btw run the tests` — kova routes it the same way as `/btw
run the tests`. Free-form questions also work: `/kova what's the
...
Slack never delivers them to kova.

As a workaround, kova recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
kova treats it identically to the slash form and replies in the same
thread.
...
kova falls back to a text prompt, the prompt instructs you to reply
...
```bash
kova slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how kova behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Kova Agent what time is it?`). In channels, kova replies in a thread attached to that message. |
| **Threads** | If you @mention kova inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.kova/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to kova in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before kova will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where kova will deliver scheduled messages,
...
kova can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, kova also loads tokens from an **OAuth token file** at:

```
~/.kova/slack_tokens.json
```
...
- When a message arrives, kova uses the correct workspace-specific client to respond.
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. kova now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Tokens should be stored in `~/.kova/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your kova config directory
