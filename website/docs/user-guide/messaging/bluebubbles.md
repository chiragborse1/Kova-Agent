...
Connect kova to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between kova and the BlueBubbles server
...
By default, kova responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
When someone messages your iMessage, kova automatically sends them a pairing code. Approve it with:
```bash
...
kova will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → kova
kova → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** kova sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — kova resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `kova logs gateway` for webhook errors (or `kova logs -f` to follow in real-time)
