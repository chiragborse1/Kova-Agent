...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Kova Agent to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect kova to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
QR login connects kova to an **iLink bot identity** (e.g. `a5ace6fd482e@im.bot`), **not** a fully scriptable ordinary personal WeChat account. Consequences:
...
- The `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` settings below only take effect when iLink actually returns group events for your account type. If it doesn't, group messages will never reach kova regardless of policy.

In practice, most deployments only get DMs to the iLink bot working reliably. If group delivery doesn't work after configuration, the limitation is on the iLink side, not in kova. The gateway logs a `WARNING` at startup whenever `WEIXIN_GROUP_POLICY` is set to anything other than `disabled`.
:::
...
- Terminal QR rendering is included when kova is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.kova/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.kova/.env`:
...
- **QR code login** — scan-to-connect setup via `Kova Gateway setup`
...
login connects one iLink bot identity to kova. Other people do not scan the
kova QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and kova will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair kova once with `Kova Gateway setup` and note the connected iLink bot
   account.
...
If only the account that scanned the QR code can talk to kova, verify that the
...
- Tokens are saved per account+peer to `~/.kova/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `Kova Gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `Kova Gateway setup` |
| `Another local Kova Gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `Kova Gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.kova/kova-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
