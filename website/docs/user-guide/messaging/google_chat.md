...
sidebar_position: 12
title: "Google Chat"
description: "Set up Kova Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Kova Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your kova
...
> Run `Kova Gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only kova can read (e.g.,
`~/.kova/google-chat-sa.json`, `chmod 600`).
...
- **App name**: whatever you want users to see ("kova" is reasonable).
...
`ADDED_TO_SPACE` event that kova uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure kova

Add the Google Chat section to `~/.kova/.env`:
...
Install the dependencies the Google Chat adapter needs (no kova extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "kova is thinking…" marker, then
...
Thread support: when a user replies inside a thread, kova detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate kova session.
...
3. Download the JSON. Move it onto the host that runs kova.
4. Register the client with kova (run under the profile you want it scoped to):
...
kova -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's kova home (e.g.
`~/.kova/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.kova/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.kova/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, kova isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `Kova Gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: kova will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.kova/google_chat_user_tokens/<sanitized_email>.json` (filesystem
