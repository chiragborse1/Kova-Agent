...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Kova Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Kova Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; kova opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | kova responds to every message. |
| Group chats | kova responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. kova will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `Kova Gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when kova runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run kova behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, kova starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how kova responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | kova responds to @mentions from any user in any group. |
| `allowlist` | kova only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | kova ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let kova read all group traffic without requiring an @mention:
...
kova auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default kova ignores messages sent by other bots. Enable bot-to-bot messaging when you want kova to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions kova. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `kova update --gateway` needs confirmation, the adapter records the selected answer in kova's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, kova reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.kova/feishu_comment_rules.json` (pairing grants in `~/.kova/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
You can invite the kova Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, kova can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends kova the meeting invitation event.
- kova extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, kova drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so kova can reply to the inviter.
...
|---------|---------|---------|
| Quiet period | `KOVA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `KOVA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `KOVA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `KOVA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.kova/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `KOVA_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `KOVA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `KOVA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `KOVA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `KOVA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `KOVA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `Kova Gateway setup` |
| `Another local Kova Gateway is already using this Feishu app_id` | Only one kova instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | kova can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `kova-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
