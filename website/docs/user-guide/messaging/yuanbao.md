...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Kova Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect kova to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.kova/.env`:
...
All standard kova commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.kova/logs/gateway.log`
...
4. Consider enabling verbose logging: `KOVA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. kova automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
