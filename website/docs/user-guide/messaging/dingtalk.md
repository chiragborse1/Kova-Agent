...
sidebar_position: 10
title: "DingTalk"
description: "Set up Kova Agent as a DingTalk chatbot"
---
...
Before setup, here's the part most people want to know: how kova behaves once it's in your DingTalk workspace.

## How kova Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | kova responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | kova responds when you `@mention` it. Without a mention, kova ignores the message. |
| **Shared groups with multiple users** | By default, kova isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
```bash
cd ~/.kova/kova-agent && uv pip install -e ".[dingtalk]"
```
...
Kova Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
- **QR-code device flow (recommended).** Scan the QR that prints in your terminal with the DingTalk mobile app — your Client ID and Client Secret are returned automatically and written to `~/.kova/.env`. No developer-console trip needed.
...
kova can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
kova automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.kova/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
...
**Fix**: Check that `Kova Gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
