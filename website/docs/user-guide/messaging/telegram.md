...
sidebar_position: 1
title: "Telegram"
description: "Set up Kova Agent as a Telegram bot"
---
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Kova Agent") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_KOVA_bot`)
...
Enable `status_indicator` and kova sets that short description to **Online**
...
`priority_mode` controls how your list combines with kova' built-in priority list:

- `prepend`: put your commands first, then kova defaults
- `append`: keep kova defaults first, then your commands
- `replace`: use only your list for priority ordering

Telegram allows up to 100 BotCommands, but large command payloads can fail. kova defaults to 60 for reliability and clamps configured values to `1..100`; use `/commands` for the full command list.
...
Kova Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, kova connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
You can also set it manually in `~/.kova/.env`:
...
Voice messages you send on Telegram are automatically transcribed by kova's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running kova — no API key required
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.kova/config.yaml`:
...
With STT disabled, the gateway still downloads the voice/audio attachment into kova's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:

```
[The user sent a voice message: /home/<user>/.kova/cache/audio/<hash>.ogg]
```
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by kova with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon — the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and kova auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
### Step 4: Point kova at the local server

Add the URLs under `platforms.telegram.extra` in `~/.kova/config.yaml`:
  ...
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below — only set this if the bot's data
                              # directory is readable by the kova process
```
...
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.kova/logs/gateway.log | tail
```
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. kova must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the kova process can read the path the server returns. Two scenarios:

- **Same machine** — telegram-bot-api and kova run on the same host. Bind-mount the data volume to a directory that kova can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid kova already runs as.
- **Different machines** — the bot server runs on one host (e.g., a NAS, a separate VM) and kova on another. The server's data directory must be shared with the kova machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but kova can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the kova host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
```bash
tail -f ~/.kova/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.kova/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.

## Group Chat Usage

Kova Agent works in Telegram group chats with a few considerations:
...
- In groups with multiple kova bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other kova bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
- Use `telegram.ignored_threads` to keep kova silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, kova keeps the previous open-group behavior and responds to normal group messages it can see

### Multiple kova bots in one group

If you run several kova profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other kova bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.
...
# named profiles
kova -p research gateway start
kova -p research gateway status
kova -p research gateway stop
```

For a small fixed fleet, use a shell loop or script that calls `Kova Gateway <action>` for the default profile and `kova -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
1. **Telegram delivery:** turn off BotFather privacy mode, promote the bot to
   admin, or mention the bot directly. kova cannot respond to group messages
...
3. **kova authorization:** make sure the sender is listed in
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   kova profile uses a unique bot token and keep `exclusive_bot_mentions`
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with kova.
...
1. Open your private chat with the kova bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, kova will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting — the bot cannot enable it programmatically.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.kova/config.yaml`:
...
1. On gateway startup, kova calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
A ChatGPT-style multi-session DM — one bot, many parallel conversations. Unlike the operator-curated `extra.dm_topics` above, this mode is **user-driven**: no config, no pre-declared topic names. The end user flips it on with `/topic`, then taps the Telegram **+** button to create as many topics as they want, each one a fully independent kova session.
...
| Topic names | Chosen by operator | Chosen by user; auto-renamed to match kova session title |
...
When the user first runs `/topic`, kova calls `getMe` to verify both flags. If either is off, kova sends a screenshot of the BotFather Threads Settings page and explains what to toggle — no activation happens until prerequisites are met.
...
4. kova responds inside that topic — the topic is now a standalone session
...
When kova generates a session title for a topic (via the auto-title pipeline, after the first exchange), the Telegram topic itself is renamed to match — e.g. "New Topic" becomes "Database migration plan". The rename is best-effort: failures are logged but don't break the session.
...
When this flag is on, kova still generates an internal session title (used by `kova sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.

### `/new` inside a topic

Resets the current topic's session (new session ID, fresh history) without touching other topics. kova replies with a reminder that for parallel work, creating another topic (via **All Messages**) is usually what you want.
...
This binds the current topic to an existing kova session instead of starting fresh. Useful for continuing a conversation that started before topic mode was enabled. Restrictions:

- The target session must belong to the same Telegram user
- The target session must not already be bound to another topic

kova confirms with the session title and replays the last assistant message for context.

To discover session IDs, send `/topic` (no argument) in the root DM — kova lists the user's unlinked Telegram sessions.
...
Send `/topic off` in the root DM. kova flips the row off, clears the chat's `(thread_id → session_id)` bindings, and the root DM reverts to a normal kova chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.

If you need to clean up by hand (e.g. a bulk reset across many chats), remove the rows directly:

```bash
sqlite3 ~/.kova/state.db \
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.kova/config.yaml`:
...
1. When a message arrives in a mapped group topic, kova looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | kova creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
- **Bot API 9.4 (Feb 2026):** Private Chat Topics — bots can create forum topics in 1-on-1 DM chats via `createForumTopic`. kova uses this for two distinct features: operator-curated [Private Chat Topics](#private-chat-topics-bot-api-94) (config-driven, fixed topic list) and user-driven [Multi-session DM mode](#multi-session-dm-mode-topic) (activated by `/topic`, unlimited user-created topics).
- **Privacy policy:** Telegram now requires bots to have a privacy policy. Set one via BotFather with `/setprivacy_policy`, or Telegram may auto-generate a placeholder. This is particularly important if your bot is public-facing.
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** kova supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.

### Streaming transport (`gateway.streaming.transport`)

When streaming is enabled (`gateway.streaming.enabled: true`), kova picks one of four transports:
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, kova converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
...
When enabled, kova attaches Telegram's `LinkPreviewOptions(is_disabled=True)` to every outgoing message and falls back to the legacy `disable_web_page_preview` parameter on older `python-telegram-bot` versions.
...
When you send `/model` with no arguments in a Telegram chat, kova shows an interactive inline keyboard for switching models:
...
Or add it to `~/.kova/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional kova configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that kova uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `Kova Gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.kova/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.kova/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
...
```bash
KOVA_TELEGRAM_NOTIFICATIONS=all
```
