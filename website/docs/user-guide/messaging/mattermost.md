...
| **Public/private channels** | kova responds when you `@mention` it. Without a mention, kova ignores the message. |
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, kova replies in a thread under your message. Thread context stays isolated from the parent channel. |
| **Shared channels with multiple users** | By default, kova isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |

:::tip
If you want kova to reply as threaded conversations (nested under your original message), set `MATTERMOST_REPLY_MODE=thread`. The default is `off`, which sends flat messages in the channel.
:::
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Profile** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want kova to post as your own user rather than a separate bot user.
:::
...
3. Search for your bot username (e.g., `kova`) and add it.
...
Kova Agent uses your Mattermost User ID to control who can interact with the bot. To find it:
...
The `MATTERMOST_REPLY_MODE` setting controls how kova posts responses:

| Mode | Behavior |
|------|----------|
| `off` (default) | kova posts flat messages in the channel, like a normal user. |
| `thread` | kova replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth. |

Set it in your `~/.kova/.env`:
...
**Fix**: Check that `Kova Gateway` is running. Look at the terminal output for error messages. Common issues: wrong URL, expired token, Mattermost server unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATTERMOST_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATTERMOST_ALLOWED_USERS` in `~/.kova/.env` and restart the gateway. Remember: the User ID is a 26-character alphanumeric string, not your `@username`.
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with Kova Agent.
