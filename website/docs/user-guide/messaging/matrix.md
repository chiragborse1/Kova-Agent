...
sidebar_position: 9
title: "Matrix"
description: "Set up Kova Agent as a Matrix bot"
---

# Matrix Setup

Kova Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Kova Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

kova works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how kova behaves once it's connected.

## How kova Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | kova responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, kova requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | kova supports Matrix threads (MSC3440). If you reply in a thread, kova keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, kova auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | kova accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; kova normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, kova isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
kova sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
kova now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run kova as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

kova needs an access token to authenticate with the homeserver. You have two options:
...
Kova Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. kova always
...
kova supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
In Matrix conversations, kova exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when kova knows who requested the action.

### Media Limits

kova uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. kova rejects arbitrary
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, kova imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If kova bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
1. Stop Synapse and delete the old device from its database:
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40Kova%3Ayour-server/devices/DEVICE_ID'
   ```
...
kova supports the same gateway commands in Matrix that it supports on other
...
kova only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/Kova-agent/issues/12614).
...
If you previously used kova with `MATRIX_ENCRYPTION=true` and are upgrading to
...
a working E2EE setup with a previous version of kova and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote kova API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: kova uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps kova's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the kova logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Kova Gateway isn't running, or it failed to connect.

**Fix**: Check that `Kova Gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.
...
- **Media support**: kova can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
