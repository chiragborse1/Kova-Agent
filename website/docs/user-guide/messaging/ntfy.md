...
ntfy makes a great lightweight push channel for kova: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `Kova Gateway setup` and pick **ntfy** for a guided walk-through.
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a kova dependency.
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The kova adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=kova-myname-2026' >> ~/.kova/.env
   echo 'NTFY_ALLOWED_USERS=kova-myname-2026' >> ~/.kova/.env
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing kova replies:
