...
description: "Chat with kova from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For group chats, hooks, and automation flows, kova supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. kova keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; kova does not hide failures just because the text resembles a silence token.
...
```bash
Kova Gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
Kova Gateway              # Run in foreground
Kova Gateway setup        # Configure messaging platforms interactively
Kova Gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo Kova Gateway install --system   # Linux only: install a boot-time system service
Kova Gateway start        # Start the default service
Kova Gateway stop         # Stop the default service
Kova Gateway status       # Check default service status
Kova Gateway status --system         # Linux only: inspect the system service explicitly
```
...
kova pairing approve telegram XKGH5N7P

# Other pairing commands:
kova pairing list          # View pending + approved users
kova pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, kova appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Off by default. When enabled, kova prepends a human-readable timestamp
...
```bash
Kova Gateway install               # Install as user service
...
```bash
Kova Gateway install          # user service
...
```bash
Kova Gateway install               # Install as launchd agent
Kova Gateway start                 # Start the service
Kova Gateway stop                  # Stop the service
Kova Gateway status                # Check status
...
| API Server | `kova-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
