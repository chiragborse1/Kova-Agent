...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for kova — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for kova — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run kova interactively.
...
kova --tui -r 20260409_000000_aa11bb
kova --tui --resume "my t0p session"
...
```bash
export KOVA_TUI=1
kova          # now uses the TUI
kova chat     # same
```

Or make it the persistent default in `~/.kova/config.yaml`:
...
With `display.interface: tui`, a bare `kova` (and `kova chat`) launches the TUI. Explicit flags always win — run `kova --cli` to drop back to the classic REPL for a single invocation, or `kova --tui` / `KOVA_TUI=1` to force the TUI when the config default is `cli`.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while kova is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `kova doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch kova installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new kova version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point kova at it:

```bash
export KOVA_TUI_DIR=/path/to/prebuilt/ui-tui
kova --tui
```
...
| `/reload` | Re-reads `~/.kova/.env` into the running TUI process so newly added API keys take effect without a restart |
...
Use the live session switcher when you want one terminal to act as a dispatcher for several TUI sessions. It lists only sessions that are currently live in this TUI process; closed sessions remain saved transcripts and can still be reopened with `/resume` or `kova --tui --resume <id-or-title>`.
...
<img alt="kova TUI Session Orchestrator with one live session and a +new row" src="/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="kova TUI Session Orchestrator demo" />
...
1. `KOVA_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export KOVA_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates kova' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `kova --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export KOVA_TUI_RESUME=1          # most-recent TUI session
# or:
export KOVA_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/kova-agent (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`kova --yolo`, `/yolo`, or `KOVA_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.

## Configuration

The TUI respects all standard kova config: `~/.kova/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.kova/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.
...
You may see a `KOVA_TUI_GATEWAY_URL` env var referenced in the codebase or logs. This is an **internal wiring detail of the web dashboard**, not a user-facing remote-attach knob. When you open the dashboard's "Chat" tab (`kova dashboard` → `/chat`), the dashboard's web server spawns an embedded TUI child process and injects `KOVA_TUI_GATEWAY_URL` so that child attaches to the dashboard's own in-process `tui_gateway` over a loopback WebSocket (`/api/ws`). The `/api/ws` endpoint exists only inside the dashboard server (`KOVA_cli/web_server.py`) and is bound to that process's lifetime and auth.

There is no general "point any TUI at any standalone gateway port" mode. In particular, the OpenAI-compatible API server (`Kova Gateway` / the `api_server` platform) does **not** serve `/api/ws` — it's the model-backend surface (`/v1/chat/completions`, `/v1/models`, …) and deliberately does not expose the TUI's JSON-RPC control channel. Setting `KOVA_TUI_GATEWAY_URL` to that port will 404.

If you want multiple surfaces to share one set of sessions, use the shared `~/.kova/state.db` (see [Sessions](sessions.md)) or the web dashboard's embedded chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.

## Reverting to the classic CLI

Launching `kova` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.kova/config.yaml` (persistent) or `KOVA_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `kova --cli` for a one-off.

If the TUI fails to launch (no Node, missing bundle, TTY issue), kova prints a diagnostic and falls back — rather than leaving you stuck.
