...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Kova Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Kova Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `kova setup --portal` — and you're ready to `kova chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
kova also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `kova --tui` — see the [TUI](tui.md) guide.
:::
...
kova chat --model "anthropic/claude-sonnet-4"

# With a specific provider
kova chat --provider nous        # Use Nous Portal
kova chat --provider openrouter  # Force OpenRouter
...
kova --continue             # Resume the most recent CLI session (-c)
kova --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
kova chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
kova -w                         # Interactive mode in worktree
kova -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Kova CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Kova CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `KOVA_YOLO_MODE` is on (either `kova --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`kova -c` or `kova --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend kova to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. kova supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for kova replies |
...
kova loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.kova/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.kova/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain kova..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. kova recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches kova — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while kova is working, kova prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend kova to the background — just like any terminal process. The shell prints a confirmation:

```
Kova Agent has been suspended. Run `fg` to bring Kova Agent back.
```
...
Resume this session with:
  kova --resume 20260225_143052_a1b2c3
...
```bash
kova --continue                          # Resume the most recent CLI session
kova -c                                  # Short form
kova -c "my project"                     # Resume a named session (latest in lineage)
kova --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
kova --resume "refactoring auth"         # Resume by title
kova -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `kova sessions rename <id> <title>` from the command line. Use `kova sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in kova's SQLite state database under `~/.kova/state.db`. The database keeps:
...
kova immediately confirms the task and gives you back the prompt:
