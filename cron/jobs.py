...
Jobs are stored in ~/.kova/cron/jobs.json
Output is saved to ~/.kova/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own KOVA_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same KOVA_HOME — so a job authored in profile
# `coder` lives in `~/.kova/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_kova_home()` (the active profile home), NOT `get_default_kova_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever KOVA_HOME the
...
# `_get_kova_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
KOVA_DIR = get_kova_home().resolve()
CRON_DIR = KOVA_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``kova cron status`` process share it
...
# threshold in `kova cron status` (kova_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``kova`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED kova timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `kova_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                KOVA_tz = _KOVA_now().tzinfo
                dt = dt.replace(tzinfo=KOVA_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _KOVA_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in kova configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured kova timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If kova/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall kova-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _KOVA_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.kova/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _KOVA_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``KOVA_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
...
# execution. Unlike the quick-snapshot store (`kova_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``kova_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _KOVA_now().strftime("%Y-%m-%d_%H-%M-%S")
