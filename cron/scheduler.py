...
Uses a file-based lock (~/.kova/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `kova update` reloads
# the module) fail with ModuleNotFoundError for kova_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from kova_constants import get_kova_home
from kova_cli._subprocess_compat import windows_hide_flags
from kova_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy kova_cli import at cron module load (matches
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``kova tools`` config for the ``cron`` platform.
...
def _get_kova_home() -> Path:
    """Resolve kova home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active KOVA_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("KOVA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid KOVA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within KOVA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other kova-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against KOVA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_kova_home() / "scripts"
    ...
    # escape — scripts MUST reside within KOVA_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["KOVA_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed KOVA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads KOVA_SESSION_PLATFORM
    #     and KOVA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via KOVA_CRON_AUTO_DELIVER_* below.
    ...
    #     send_message gate read KOVA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the KOVA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing KOVA_SESSION_* here does not affect delivery.
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_kova_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_kova_home() / ".env"), override=True, encoding="latin-1")
        ...
        # Model resolution precedence: per-job override > KOVA_MODEL env >
        ...
        model = job.get("model") or os.getenv("KOVA_MODEL") or ""
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"KOVA_MODEL={os.getenv('KOVA_MODEL', '')!r}, "
        ...
        prefill_file = (
            os.getenv("KOVA_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject KOVA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # KOVA_HOME. When a workdir is configured, also inject project
        ...
        # override via KOVA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("KOVA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KOVA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_KOVA_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_KOVA_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _KOVA_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _KOVA_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set KOVA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("KOVA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
