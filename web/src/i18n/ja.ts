...
export const ja: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "kova プラグインを発見、インストール、有効化、更新します (`kova plugins` 相当)。",
    ...
    removeConfirm: "このプラグインを ~/.kova/plugins/ から削除しますか？",
    removeHint: "削除できるのは ~/.kova/plugins 配下のユーザーがインストールしたプラグインのみです。",
  ...
  skills: {
    ...
    categories: "カテゴリ",
    filters: "フィルター",
    noSkills: "スキルが見つかりません。スキルは ~/.kova/skills/ から読み込まれます",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "実際のセッション履歴から獲得できる kova のコレクタブル バッジです。既知の未達成の実績は「Discovered」として表示され、Secret 実績は最初の該当する挙動が検出されるまで非表示のままです。",
      scan_subtitle:
        "kova のセッション履歴をスキャンしています。履歴が大きい場合、初回スキャンには 5～10 秒かかることがあります。",
    },
    actions: {
      rescan: "再スキャン",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "kova をもっと使ってみてください",
      none_yet: "まだありません",
    },
    state: {
      unlocked: "解除済み",
    ...
    guide: {
      ...
      secret_body:
        "シークレットはトリガー条件を隠しています。kova が関連するシグナルを検出すると、カードは「Discovered」になり、要件が表示されます。",
      scan_status_header: "スキャン状況",
      scan_status_body:
        "kova はローカル履歴を一度スキャンし、その後カードが自動的に表示されます。数秒かかってもスタックしているわけではありません。",
