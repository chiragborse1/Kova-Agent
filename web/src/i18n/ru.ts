...
export const ru: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов kova (аналог `kova plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.kova/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.kova/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.kova/skills/",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки kova, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий kova. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте kova чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только kova обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "kova сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
