...
// (e.g. https://mission-control.tilos.com/kova/). The Python backend
// injects ``window.__KOVA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__KOVA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const KOVA_BASE_PATH = readBasePath();
const BASE = KOVA_BASE_PATH;
...
const SESSION_HEADER = "X-kova-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``kova update``, ``Kova Gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__KOVA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__KOVA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("kova.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("kova.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__KOVA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("kova.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__KOVA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the kova dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``KOVA_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__KOVA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-kova-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``KOVA_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__KOVA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  checkKovaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/kova/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** kova-index only: whether the centralized index loaded. */
  available?: boolean;
}
