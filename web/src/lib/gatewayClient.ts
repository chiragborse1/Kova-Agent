...
import { KOVA_BASE_PATH, getWsTicket } from "@/lib/api";
...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    } else if (window.__KOVA_AUTH_REQUIRED__) {
      const { ticket } = await getWsTicket();
      authParamName = "ticket";
      authParamValue = ticket;
    } else {
      authParamName = "token";
      authParamValue = window.__KOVA_SESSION_TOKEN__ ?? "";
      if (!authParamValue) {
        this.setState("error");
        throw new Error(
          "Session token not available — page must be served by the kova dashboard",
        );
      }
    }
    ...
    const ws = new WebSocket(
      `${scheme}//${location.host}${KOVA_BASE_PATH}/api/ws?${authParamName}=${encodeURIComponent(authParamValue)}`,
    );
    this.ws = ws;
