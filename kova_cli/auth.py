...
from kova_cli.config import get_kova_home, get_config_path, read_raw_config
from kova_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "kova-cli"
...
XAI_OAUTH_DOCS_URL = "https://kova-agent.nousresearch.com/docs/guides/xai-grok-oauth"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from kova_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.kova/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `kova model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("KOVA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.kova/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_kova_home() / "auth.json"
    # Seat belt: if pytest is running and KOVA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch KOVA_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set KOVA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom KOVA_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_kova_root()
    except Exception:
        return None
    profile_home = get_kova_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.kova/auth.json`` even when KOVA_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_kova_root()`` for a profile-shaped KOVA_HOME can
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``kova auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``kova auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``kova auth add <provider>`` inside the profile, profile entries
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from kova_cli.config import validate_config_structure
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("KOVA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.kova/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("KOVA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set KOVA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("KOVA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("KOVA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("KOVA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but kova times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``KOVA_LOG_LEVEL=INFO`` (or check
            # ``~/.kova/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>kova auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `kova auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `kova auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `kova auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `kova auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    # Persist so subsequent `kova auth spotify` runs skip the wizard.
    save_env_value("KOVA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("KOVA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved KOVA_SPOTIFY_CLIENT_ID to ~/.kova/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "KOVA_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize kova:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.kova/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from kova auth store (~/.kova/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `kova auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `kova auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `kova auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `kova auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``manual:device_code`` — entries created by ``kova auth add openai-codex``
      ...
      ``kova auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to kova auth store (~/.kova/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `kova auth` to re-authenticate."
            )
            relogin_required = True
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("KOVA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("KOVA_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.kova/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.kova/auth.json even when KOVA_HOME points at a profile path
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``KOVA_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("KOVA_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
#
# File lives at ${KOVA_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<kova-root>/shared/nous_auth.json`` where ``<kova-root>`` is what
# ``get_default_kova_root()`` returns — ``~/.kova`` on Linux/macOS,
# ``%LOCALAPPDATA%\kova`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's KOVA_HOME so named profiles (which
# typically live under ``<kova-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``KOVA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<kova-root>/shared/``, where ``<kova-root>`` is what
    :func:`kova_constants.get_default_kova_root` returns — so
    Linux/macOS classic installs land at ``~/.kova/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\.kova\\shared\\``, and
    Docker / custom ``KOVA_HOME`` deployments at
    ``<KOVA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("KOVA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from kova_constants import get_default_kova_root
    return get_default_kova_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set KOVA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from kova_constants import get_default_kova_root
        real_home_shared = (
            get_default_kova_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set KOVA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No KOVA_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.kova/auth.json) called POST /api/oauth/token with kova's "
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "kova is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("KOVA_PORTAL_BASE_URL")
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``kova auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``kova auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `kova auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("kova is not logged into Nous Portal.",
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("KOVA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("KOVA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``kova doctor`` runs the live
    ...
    try:
        from kova_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                ...
                info["hint"] = (
                    ...
                    "is skipped here. Run `kova doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set KOVA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from kova_cli.config import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    """
    from kova_cli.models import _format_price_per_mtok
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in kova auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("kova will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("KOVA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("kova will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in kova auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(kova creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=kova-agent` lets xAI attribute kova-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in kova — please report at "
            "https://github.com/chiragborse1/Kova-Agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize kova with xAI:")
        print(authorize_url)
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting kova login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `kova model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``kova status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting kova login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `kova model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``KOVA_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `kova model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("kova will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `kova model` or configure an API key to use kova.")
        else:
