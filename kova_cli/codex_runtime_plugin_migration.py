"""Migrate kova' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads kova' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  kova mcp_servers.<n>.command/args/env  → codex stdio transport
  kova mcp_servers.<n>.url/headers       → codex streamable_http transport
  kova mcp_servers.<n>.timeout           → codex tool_timeout_sec
  kova mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  kova-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. kova' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in kova config.")
...
# kova keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # kova' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, KOVA_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one kova MCP server config to the codex inline-table dict
    ...
    if not isinstance(KOVA_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(KOVA_cfg.get("command"))
    has_url = bool(KOVA_cfg.get("url"))
    ...
    for key in KOVA_cfg:
        ...
        elif key not in _KNOWN_KOVA_KEYS:
            skipped.append(f"{key} (unknown kova key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (KOVA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by kova)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert kova' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before kova' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior kova
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="kova-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a KOVA_HOME pointing
...
def _build_kova_tools_mcp_entry() -> dict:
    ...
    so a kova installed under /opt/, /usr/local/, or a venv all work.
    KOVA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # KOVA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_kova_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever KOVA_HOME its launcher sets
    ...
    # — that would override the launcher's KOVA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("KOVA_HOME", tmp_path) would
    ...
    KOVA_HOME = os.environ.get("KOVA_HOME") or ""
    if KOVA_HOME and _looks_like_test_tempdir(KOVA_HOME):
        KOVA_HOME = ""
    if KOVA_HOME:
        env["KOVA_HOME"] = KOVA_HOME
    # PYTHONPATH passes through so a worktree-launched kova finds the
    ...
    env["KOVA_QUIET"] = "1"
    env["KOVA_REDACT_SECRETS"] = env.get("KOVA_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    KOVA_servers = (KOVA_config or {}).get("mcp_servers") or {}
    if not isinstance(KOVA_servers, dict):
        report.errors.append(
            "mcp_servers in kova config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in KOVA_servers.items():
    ...
    # Inject kova' own tool surface as an MCP server so the spawned
