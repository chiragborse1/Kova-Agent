"""
kova Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/kova.app`` or ``~/Applications/kova.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\kova`` (NSIS per-user)
...
``kova uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "kova"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "kova"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/kova.app"),
            home / "Applications" / "kova.app",
        ]
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "kova")
    else:
