...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``kova debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``kova debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, kova version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `kova debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "kova-agent/debug-share"},
    )
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "kova-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``kova dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``kova debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    print(f"To delete now:  kova debug delete <url>")
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``kova debug`` call.
    ...
    # best-effort — any failure is swallowed so ``kova debug`` stays
