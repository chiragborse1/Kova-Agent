...
``$KOVA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-kova-setup
(the stage2 hook) has chowned the volume and seeded $KOVA_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$KOVA_HOME``,
    not under ``profiles/``). The dispatcher in ``kova_cli.gateway``
    ...
    ``$KOVA_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        KOVA_HOME: The container's KOVA_HOME (typically /opt/data).
            Profiles live under ``<KOVA_HOME>/profiles/<name>/``;
            the default profile lives at ``<KOVA_HOME>`` itself.
    ...
    # ``kova gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(KOVA_HOME)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(KOVA_HOME)
    ...
    profiles_root = KOVA_HOME / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `kova profile create` (config.yaml
            # is not — that comes later via `kova setup`). Use it as the
            ...
            # via ``kova -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(KOVA_HOME, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = KOVA_HOME / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("KOVA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the kova args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/kova/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/kova/docker/main-wrapper.sh
    ...
    ``kova`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``kova <subcommand>``, so an
    explicit leading ``kova`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `kova <subcommand>`; peel an explicit kova.
    if args and Path(args[0]).name == "kova":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``kova dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted KOVA_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `kova -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit kova
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the kova user) won't hit EACCES. See
...
def _write_reconcile_log(
    KOVA_HOME: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $KOVA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted KOVA_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        KOVA_HOME=KOVA_HOME, scandir=scandir,
    )
    for a in actions:
        print(
