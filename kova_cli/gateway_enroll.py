"""``kova gateway enroll`` — enroll a self-hosted gateway with a relay connector.
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `kova auth login nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `kova gateway enroll` is not available in a managed/hosted install.\n"
