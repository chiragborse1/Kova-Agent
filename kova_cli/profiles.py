"""
Profile management for multiple isolated kova instances.

Each profile is a fully independent KOVA_HOME directory with its own
...
Usage::
    ...
    kova profile create coder          # fresh profile + bundled skills
    kova profile create coder --clone  # also copy config, .env, SOUL.md, skills
    kova profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    kova -p coder chat                 # or via flag
    kova profile use coder             # set as sticky default
    kova profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See kova_constants.get_subprocess_home().
    "home",
]
...
#
# Rationale per item:
#   kova-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `kova profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `kova
...
# `kova skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_KOVA_HOME().resolve()
...
_RESERVED_NAMES = frozenset({
    "kova", "default", "test", "tmp", "root", "sudo",
})

# kova subcommands that cannot be used as profile names/aliases
_KOVA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the kova root, NOT to the current KOVA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KOVA_HOME points outside
    ``~/.kova``, profiles live under ``KOVA_HOME/profiles/`` so
...
def _get_default_KOVA_HOME() -> Path:
    """Return the default (pre-profile) KOVA_HOME path.

    In standard deployments this is ``~/.kova``.
    In Docker/custom deployments where KOVA_HOME is outside ``~/.kova``
    (e.g. ``/opt/data``), returns KOVA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_KOVA_HOME() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the kova installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KOVA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_KOVA_HOME()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, kova subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _KOVA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a kova subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running kova. If we
    ...
    try:
        ...
        # not be migrated. The next `kova doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``kova -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``kova profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``kova profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``KOVA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing kova config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_KOVA_HOME()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, KOVA_HOME)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own KOVA_HOME.
    ...
    The returned ``KOVA_HOME`` is the path to pass to
    ``set_kova_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_KOVA_HOME())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``kova update`` skips re-seeding this profile's
    ...
    # Keys pages, `kova -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this kova profile.\n"
    ...
    if not soul_path.exists():
        try:
            from kova_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `kova update`'s
    ...
    # Cloned configs can be older than the running kova (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `kova profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `kova -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KOVA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``kova profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KOVA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_KOVA_HOME() / ".env"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``KOVA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from kova_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KOVA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KOVA_HOME")
    try:
        os.environ["KOVA_HOME"] = str(profile_dir)
        from kova_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KOVA_HOME"] = old_home
        elif "KOVA_HOME" in os.environ:
            del os.environ["KOVA_HOME"]
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KOVA_HOME.

    Returns ``"default"`` if KOVA_HOME is not set or points to ``~/.kova``.
    Returns the profile name if KOVA_HOME points into ``~/.kova/profiles/<name>``.
    Returns ``"custom"`` if KOVA_HOME is set to an unrecognized path.
    """
    from kova_constants import get_kova_home
    KOVA_HOME = get_kova_home()
    resolved = KOVA_HOME.resolve()

    default_resolved = _get_default_KOVA_HOME().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kova profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="KOVA_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_KOVA_HOME() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KOVA_HOME path string.

    Called early in the CLI entry point, before any kova modules
    are imported, to set the KOVA_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
