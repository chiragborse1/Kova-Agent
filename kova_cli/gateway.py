...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "kova CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``kova gateway status`` from falsely counting the ``kova`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``kova gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. KOVA_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``kova update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            kova profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to kova profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "kova_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m kova_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose KOVA_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``kova update`` users
    # with no running gateway until they re-invoke ``kova gateway``
    ...
    if sys.platform == "win32":
        try:
            from kova_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / KOVA_HOME) the windowless
        # base interpreter needs to import kova_cli.  Empty on POSIX, where
...
def _sync_KOVA_HOME_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``KOVA_HOME``.

    Under ``sudo``, ``KOVA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_kova_home` falls back to ``/root/.kova`` — the wrong
    profile. The unit file pins ``KOVA_HOME`` for the actual gateway
    ...
    unit_home = env.get("KOVA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("KOVA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["KOVA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}kova gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}kova gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from kova_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: kova gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `kova gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``kova gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from kova_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (kova_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KOVA_HOME-scoped).
...
from kova_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our kova Docker image — that one runs s6-overlay as
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KOVA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (KOVA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KOVA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(KOVA_HOME: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when KOVA_HOME is a named profile.

    For ``~/.kova/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        KOVA_HOME: Optional explicit KOVA_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this KOVA_HOME.

    Default ``~/.kova`` returns ``kova-gateway`` (backward compatible).
    Profile ``~/.kova/profiles/coder`` returns ``kova-gateway-coder``.
    Any other KOVA_HOME appends a short hash for uniqueness.
    """
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "kova_cli.main gateway",
    "kova_cli/main.py gateway",
    "gateway/run.py",
    " kova gateway ",
    "/kova gateway ",
)
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy kova gateway unit files if any are installed.
...
def remove_legacy_KOVA_units(
    ...
    print()
    print("Legacy kova gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo kova gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    kova gateway uninstall")
    print_info("    sudo kova gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so kova can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo kova gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo kova gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo kova gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode kova often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kova`` → ``ai.kova.gateway.plist`` (backward compatible).
    Profile ``~/.kova/profiles/coder`` → ``ai.kova.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.kova/kova-agent  -> /home/alice/.kova/kova-agent
...
def _KOVA_HOME_for_target_user(target_home_dir: str) -> str:
    """Remap the current KOVA_HOME to the equivalent under a target user's home.
    ...
    """
    current_kova = get_kova_home().resolve()
    current_default = (Path.home() / ".kova").resolve()
    ...
    try:
        ...
        # Completely custom path (not under ~/.kova) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m kova_cli.main``, so module
    ...
    dir, or a clone that ``kova update`` later relocates/removes — the path
    ...
    ``KOVA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if KOVA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's KOVA_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m kova_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="KOVA_HOME={KOVA_HOME}"
Restart=always
RestartSec=5
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir KOVA_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved KOVA_HOME
    ...
    exported a throwaway ``KOVA_HOME=/tmp/...`` — writing it to the real
    ...
    with ``KOVA_HOME=/tmp/kova-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="KOVA_HOME=..."`` lines and launchd
    ``<key>KOVA_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'KOVA_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>KOVA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp KOVA_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: KOVA_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported KOVA_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only KOVA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp KOVA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../KOVA_test`` path into
    # ``Environment="KOVA_HOME=..."``. Writing that to the developer's
    ...
    # KOVA_HOME into the unit (manual E2E homes like /tmp/kova-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current kova install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo kova gateway uninstall --system")
    print_info("         kova gateway install")
    print_info("         kova gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}kova gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}kova gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}kova gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `kova gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `kova gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}kova gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}kova gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}kova gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}kova gateway start{scope_flag}"
        )
...
# process invocation (each ``kova gateway start/stop/status`` call).
...
# (the documented `nohup kova gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m kova_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active KOVA_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "kova_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup kova gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup kova gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current kova install"
    )
    return True
...
def launchd_stop():
    ...
    # `kova gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # kova PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the kova PID file).
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: kova gateway stop")
            else:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``kova gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``kova gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``kova gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other KOVA_HOME roots.
    """
    ...
    print("  Use 'kova gateway restart' to replace it,")
    print("  or 'kova gateway stop' first.")
    print("  Or use 'kova gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "$KOVA_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # /restart) rather than through `kova gateway restart` which already
    ...
    # `kova gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where kova delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and kova will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``kova setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.kova/plugins/ still require
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from kova_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'kova pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Skipped — configure later with 'kova gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. kova will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. kova will store the returned account_id/token in ~/.kova/.env."
    )
    print_info(
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `kova pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `kova pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "KovaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``kova_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from kova_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.kova/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.kova/.env: {', '.join(required)}")
    else:
        print_info(
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: kova gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo kova gateway install --system"
                        )
                    print_info("  Or run in foreground:  kova gateway run")
            elif is_wsl():
                ...
                print_info("  Run in foreground: kova gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup kova gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`kova_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from kova_cli.service_manager import (
    ...
    if profile is None:
        ...
        # KOVA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``kova gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``kova gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${KOVA_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: kova gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
            ...
            print(
                "  nohup kova gateway run > ~/.kova/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from kova_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  kova profile create <name>     # creates the s6 service slot")
                print("  kova -p <name> gateway start   # bring it up via s6")
                print("  kova status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: kova gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: kova gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: kova gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  kova profile delete <name>     # tears down the s6 service slot")
                print("  kova -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `kova -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: kova gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_KOVA_GATEWAY") == "1":
            print_error(
                ...
                "Use `kova gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_KOVA_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `kova gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: kova gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  kova gateway install")
                else:
                    print("To install as a service:")
                    print("  kova gateway install")
                    print("  sudo kova gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  kova gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup kova gateway run > ~/.kova/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s kova 'kova gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  kova gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  kova gateway install  # Install as user service")
                    print(
                        "  sudo kova gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy kova gateway unit files from
