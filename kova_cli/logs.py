...
Usage examples::
    ...
    kova logs                    # last 50 lines of agent.log
    kova logs -f                 # follow agent.log in real time
    kova logs errors             # last 50 lines of errors.log
    kova logs gateway -n 100    # last 100 lines of gateway.log
    kova logs gui -f            # follow gui.log (dashboard/pty/ws)
    kova logs desktop -f        # follow desktop.log (Electron app boot/backend)
    kova logs --level WARNING    # only WARNING+ lines
    kova logs --session abc123   # filter by session ID substring
    kova logs --component tools  # only tool-related lines
    kova logs --since 1h         # lines from the last hour
    kova logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kova_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when kova runs — try 'kova chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_kova_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kova_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kova_home()}/logs/")
        return

    print(f"Log files in {display_kova_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kova chat' to generate logs)")
