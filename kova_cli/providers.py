"""
Single source of truth for provider identity in Kova Agent.
...
2. **kova overlays** — transport type, auth patterns, aggregator flags,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kova", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. kova overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + kova overlay
    ...
    overlay = KOVA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + kova extra
    ...
    if overlay is not None:
        # kova-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in KOVA_OVERLAYS
    if provider == "bedrock":
