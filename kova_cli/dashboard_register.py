"""``kova dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.kova/.env``
as ``KOVA_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.kova/auth.json``), refreshing it if needed. Fails fast with a
     "run `kova setup`" hint when the user isn't logged in.
  ...
  3. Writes ``KOVA_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``KOVA_DASHBOARD_PORTAL_URL`` into ``~/.kova/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``KOVA_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from kova_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``kova dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `kova auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from kova_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    KOVA_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    KOVA_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    KOVA_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `kova dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    kova dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from kova_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets KOVA_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `kova dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `kova setup` (or `kova auth login nous`) first, then retry.")
        else:
    ...
    # KOVA_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (KOVA_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("KOVA_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("KOVA_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write KOVA_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  KOVA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      KOVA_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("KOVA_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("KOVA_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://kova.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking KOVA_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as KOVA_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("KOVA_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("KOVA_DASHBOARD_PUBLIC_URL", public_url)
