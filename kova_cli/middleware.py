"""kova middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "kova.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "kova.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before kova sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from kova_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from kova_cli.plugins import has_middleware
