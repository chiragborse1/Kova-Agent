"""Helpers for loading kova .env files consistently across entrypoints."""
...
# load_KOVA_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_KOVA_dotenv().
...
# KOVA_HOME paths we've already pulled external secrets for during this
# process.  ``load_KOVA_dotenv()`` is called at module-import time from
# several hot modules (cli.py, kova_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_KOVA_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which KOVA_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `kova setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``kova_cli.config._sanitize_env_lines`` which
    already knows all valid kova env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from kova_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_KOVA_dotenv(
    *,
    KOVA_HOME: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load kova environment files with user config taking precedence.

    Behavior:
    - `~/.kova/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(KOVA_HOME or os.getenv("KOVA_HOME", Path.home() / ".kova"))
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of KOVA_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of kova reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_KOVA_dotenv()`` runs at import
    time from several hot modules (cli.py, kova_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``kova secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `kova model`
