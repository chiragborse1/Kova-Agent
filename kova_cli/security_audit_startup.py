...
3. Running inside a container with no persistent volume mount over the
   KOVA_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run kova as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("KOVA_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(KOVA_HOME: Optional[Path]) -> Optional[str]:
    ...
    home = KOVA_HOME or Path(
        os.environ.get("KOVA_HOME", os.path.expanduser("~/.kova"))
    )
    try:
...
def run_security_audit(
    *, KOVA_HOME: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(KOVA_HOME)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(KOVA_HOME=KOVA_HOME, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
