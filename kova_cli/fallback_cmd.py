...
Storage: ``fallback_providers`` in ``~/.kova/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `kova model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://kova-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `kova model`, then append the selection to the chain."""
    from kova_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `kova model` — select the provider + model you want as a fallback.")
    print()
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``kova fallback [subcommand]``."""
