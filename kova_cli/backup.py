...
`kova backup` creates a zip archive of the entire ~/.kova/ directory
...
# KOVA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under KOVA_HOME) —
...
# File names that ``kova import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE KOVA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside KOVA_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside KOVA_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"kova-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"kova-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(KOVA_root, followlinks=False):
        ...
        # ``kova-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "kova-agent" and not is_root)
        ]
    ...
    # outside KOVA_HOME, so the walk above never sees it. Ask the active
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a kova backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a kova backup "
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has kova configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under KOVA_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # kova_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: kova profile list  (after installing kova)")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  kova -p {pname} gateway install")

        print("Done. Your kova configuration has been restored.")
...
# Critical state files to include in quick snapshots (relative to KOVA_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $KOVA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside KOVA_HOME and the entry is silently
...
# Relative path of the cron job database inside KOVA_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of KOVA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<KOVA_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `kova claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of KOVA_HOME under ``backups/`` before a
    ``kova claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``kova import <archive>``.  Writes to
    ``<KOVA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
