...
``<root>`` is the **shared kova root** (the parent of any active
...
``kova -p <profile>`` joins the same board as the dispatcher that
...
the profile's kova home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``KOVA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``KOVA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``KOVA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``KOVA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``KOVA_KANBAN_DB``,
``KOVA_KANBAN_WORKSPACES_ROOT``, and ``KOVA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/kova-kanban-v1-spec.pdf`` for the full
...
from kova_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ``KOVA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``KOVA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("KOVA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``KOVA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("KOVA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``KOVA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "KOVA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "KOVA_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``kova-agent``
...
def kanban_home() -> Path:
    """Return the shared kova root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``kova kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``KOVA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``kova kanban boards
    ...
    env = os.environ.get("KOVA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``kova kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``KOVA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("KOVA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``KOVA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("KOVA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``KOVA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("KOVA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``kova kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``KOVA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (kova_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``KOVA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("KOVA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``kova gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources kova treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``KOVA_KANBAN_DB`` env → ``KOVA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `kova kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See kova_state._WAL_INCOMPAT_MARKERS for detection logic.
                from kova_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``kova kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``KOVA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from kova_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``kova --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from kova_cli import projects_db as _pdb
    ...
    # `kova --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``kova kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `kova kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``kova kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``kova kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``kova kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``kova kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``KOVA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold kova' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside kova-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``kova kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the kova checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, kova treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, kova creates/reuses that linked worktree. With
      no ``workspace_path``, kova anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, kova uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``KOVA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a kova
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``kova kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``kova kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real kova profile.
    ...
    if not rows:
        return False
    try:
        from kova_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from kova_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real kova profile.
        # `_default_spawn` invokes ``kova -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a kova
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `kova kanban tail` — without
...
def _module_KOVA_argv() -> list[str]:
    """Return the interpreter-bound kova CLI invocation."""
    # ``kova_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``kova`` package — there is no
    # ``kova`` package to import.
    return [sys.executable, "-m", "kova_cli.main"]


def _absolute_KOVA_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved kova shim."""
...
def _KOVA_path_argv(path: str) -> list[str]:
    """Return argv for a resolved kova executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_KOVA_argv()
    return [_absolute_KOVA_path(path)]


def _resolve_KOVA_argv() -> list[str]:
    """Resolve the ``kova`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$KOVA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("kova")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\kova.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m kova_cli.main`` — fallback for setups where
       kova is launched from a venv and the ``kova`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_KOVA_bin`` for the same reason. Kept
    local (not imported from gateway) because ``kova_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("KOVA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _KOVA_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _KOVA_path_argv(resolved_env_bin)
        return _module_KOVA_argv()

    KOVA_bin = _safe_which_no_cwd("kova") if _IS_WINDOWS else shutil.which("kova")
    if KOVA_bin:
        return _KOVA_path_argv(KOVA_bin)
    return _module_KOVA_argv()
...
def _resolve_worker_cli_toolsets(KOVA_HOME: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``KOVA_KANBAN_TASK`` is set.
    """
    if not KOVA_HOME:
        return None
    try:
        ...
        token = set_kova_home_override(KOVA_HOME)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``kova -p <profile> chat -q ...`` subprocess.
    ...
    ``KOVA_KANBAN_DB`` / ``KOVA_KANBAN_BOARD`` / workspaces_root env
    ...
    env["KOVA_KANBAN_TASK"] = task.id
    env["KOVA_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["KOVA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["KOVA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["KOVA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["KOVA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["KOVA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    env["KOVA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["KOVA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["KOVA_KANBAN_BOARD"] = resolved_board
    # KOVA_PROFILE is the author the kanban_comment tool defaults to.
    # `kova -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["KOVA_PROFILE"] = profile_arg
    ...
    # `kova kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`kova` executable not found on PATH. "
            "Install Kova Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``kova kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled KOVA_PROFILE values like "kova-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default kova root exists

    Reads profile paths directly so this module has no import dependency on
    ``kova_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from kova_constants import get_default_kova_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``kova kanban assignees`` for the terminal.
