...
the existing module-level functions in kova_cli.gateway and
kova_cli.gateway_windows directly. This protocol is a thin facade
...
``kova gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``kova gateway start/stop/restart``).
    """
    ...
    # `kova gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every kova runtime call inside the
    container drops to kova via ``s6-setuidgid``, that silent failure
...
# in ``kova_cli.gateway`` (systemd/launchd) and ``kova_cli.gateway_windows``
...
# active (set via the ``kova -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in kova_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from kova_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from kova_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from kova_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from kova_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in kova_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from kova_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from kova_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from kova_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from kova_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``kova_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's KOVA_HOME blindly;
    derive the shared profile root from the current KOVA_HOME and map the
    ...
    KOVA_HOME = Path(os.environ.get("KOVA_HOME", "/opt/data"))
...
# UID/GID of the in-image ``kova`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``kova`` user — UID 10000 — running every kova runtime
    ...
    with kova ownership before triggering ``s6-svscanctl -a``,
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``kova -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`kova profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources KOVA_HOME (and any extra env) via with-contenv —
             so e.g. ``-e KOVA_HOME=/data/kova`` is honored at run
        ...
        Special case: ``profile == "default"`` emits ``kova gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root KOVA_HOME profile" (the implicit profile that exists at
        the top of $KOVA_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``kova_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$KOVA_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``KOVA_HOME``,
        ...
        allocator in ``kova_cli.profiles._allocate_gateway_port``).
        ...
        # profile's KOVA_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``kova gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-KOVA_HOME PID lock first;
        ...
        # Each profile is scoped to its own KOVA_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${KOVA_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the KOVA_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e KOVA_HOME=/data/kova``
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``kova logs`` reads and what
        ...
        return (
            ...
            f': "${{KOVA_HOME:=/opt/data}}"\n'
            f'log_dir="$KOVA_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            f'chown kova:kova "$KOVA_HOME/logs/gateways" 2>/dev/null || true\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``kova gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``kova -p <profile> gateway start``.
