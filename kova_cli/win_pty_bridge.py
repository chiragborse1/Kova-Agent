"""Windows ConPTY bridge for the `kova dashboard` chat tab.

Drop-in counterpart to ``kova_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``kova_cli.web_server`` consumes: ``spawn``, ``read``,
