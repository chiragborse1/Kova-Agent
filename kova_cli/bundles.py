"""Implementation of the ``kova bundles`` CLI subcommand.

Mirrors the structure of ``kova_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `kova bundles list | grep …` doesn't
...
def register_cli(subparser) -> None:
    """Build the ``kova bundles`` argparse tree.

    Called from ``kova_cli/main.py`` where it owns the top-level
