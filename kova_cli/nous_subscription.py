...
# coverage category (kova_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `kova tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("KOVA_LOCAL_STT_COMMAND"):
        return True
    try:
    ...
    *,
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("KOVA_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("KOVA_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from kova_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`kova tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``kova tools`` when a user selects a Nous-managed Tool Gateway
