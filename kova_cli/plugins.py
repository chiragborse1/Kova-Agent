...
from kova_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``KOVA_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("KOVA_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = os.getenv("KOVA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When KOVA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other kova loggers.
    """
    ...
    logger.debug(
        "KOVA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by kova_cli.kanban_db when a task
    ...
    # separate `kova -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `kova kanban dispatch`),
...
ENTRY_POINTS_GROUP = "KOVA_agent.plugins"

_NS_PARENT = "KOVA_plugins"
...
class PluginManifest:
    ...
    # lookups and by ``kova plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``KOVA_HOME`` via
        :func:`kova_cli.profiles.get_active_profile_name`, so it works in
        ...
        ``KOVA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``kova honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``kova <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`kova_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_slack_action_handler(
        ...
        kova' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``kova model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: kova_cli.main imports plugins indirectly
        from kova_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / KOVA_SAFE_MODE=1): troubleshooting run
        ...
        if env_var_enabled("KOVA_SAFE_MODE"):
            logger.info("KOVA_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to kova_cli/. Two layouts are
        ...
        if _env_enabled("KOVA_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set KOVA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with kova and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform kova ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `kova plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `kova plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``KOVA_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``KOVA_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="kova-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``kova tools`` TUI so plugin-provided toolsets appear
