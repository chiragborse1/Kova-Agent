...
on Japanese locales, etc.).  kova's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``kova_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. kova is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KOVA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KOVA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the kova-managed Git install directories so
    ...
    # already-running shells don't see the change — which means kova
    ...
    # install without kova-managed PortableGit).
    ...
    # stdin is re-configured for completeness; kova's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and kova's
    ``kova config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching kova (User env var in Windows Settings, or
    export in a PowerShell profile) and kova picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known kova-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``kova`` from right after install) retain their old PATH.

    Any subprocess kova spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known kova-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "kova", "git", "cmd"),
        os.path.join(local_appdata, "kova", "git", "bin"),
        os.path.join(local_appdata, "kova", "git", "usr", "bin"),
        # kova venv Scripts directory — host of the kova.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes kova, but harmless to include.
        os.path.join(local_appdata, "kova", "kova-agent", "venv", "Scripts"),
