...
prefix (e.g. ``mission-control.tilos.com/kova/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /kova`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``KOVA_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``kova.example.com`` instead of
    ``https://kova.example.com``) — the single most common cause of
    "I set KOVA_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "kova.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/kova"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# KOVA_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``KOVA_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("KOVA_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("KOVA_DASHBOARD_PUBLIC_URL env var", env_raw)
