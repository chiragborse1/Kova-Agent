...
Three cookies in play:
  ...
  - KOVA_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-KOVA_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "KOVA_session_at"
SESSION_RT_COOKIE = "KOVA_session_rt"
PKCE_COOKIE = "KOVA_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /kova`` we want ``Path=/kova`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (kova #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/kova``)
