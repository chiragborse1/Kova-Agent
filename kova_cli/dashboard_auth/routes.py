...
from kova_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``KOVA_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /kova`` (Mission Control deploys) — we
    ...
    from kova_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``kova_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("KOVA_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from kova_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
