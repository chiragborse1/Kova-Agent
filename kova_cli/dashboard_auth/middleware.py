...
from kova_cli.dashboard_auth import list_session_providers
from kova_cli.dashboard_auth.audit import AuditEvent, audit_log
from kova_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from kova_cli.dashboard_auth.cookies import read_session_cookies
from kova_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /kova``, the
    ``login_url`` is prefixed (``/kova/login?next=...``) so the
    ...
    """
    from kova_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["kova.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``KOVA_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from kova_cli.dashboard_auth.cookies import clear_session_cookies
        from kova_cli.dashboard_auth.prefix import prefix_from_request
