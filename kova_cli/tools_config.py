...
`kova tools` and `kova setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.kova/config.yaml under
...
from kova_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from kova_cli.nous_account import format_nous_portal_entitlement_message
...
from kova_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _KOVA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `kova tools` → Video Generation, which walks
...
# `kova tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from kova_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `kova tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``kova tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from kova_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``kova tools`` checklist actually offers
    for ``platform``.
    ...
    so ``kova tools`` never claims to add or remove a toolset the user was
...
from kova_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "kova routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set KOVA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "KOVA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "KOVA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "KOVA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "KOVA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "KOVA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("KOVA_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the kova telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``kova update`` and
      by ``kova computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `kova update` calls this
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / kova process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kova_home()}/kova-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kova-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/kova-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.kova/config.yaml")
        ...
        # Run the full `kova auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.kova/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from kova_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from kova_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _print_info("    Restart kova for tracing to take effect.")
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `kova auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: kova auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: kova auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``kova tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``kova tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: kova tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "kova-cli" (which include all _KOVA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[kova-cli, spotify]`` after enabling Spotify via ``kova
        ...
        # (e.g. "kova-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``kova tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `kova tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"kova-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("kova-")}
    ...
    # shipped as a bundled plugin but user must opt in via `kova tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `kova tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by kova tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `kova` instead of
    # `kova-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `kova update`/`kova doctor`.
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kova-cli, kova-telegram, etc.)
    ...
    # Opening `kova tools` is the user's opt-in to reconfigure tools, so treat
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from kova_cli.curses_ui import curses_checklist
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `kova tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `kova tools` and `kova setup tools`.
    ...
    print()
    from kova_constants import display_kova_home
    print(color(f"  Tool configuration saved to {display_kova_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'kova' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, kova mcp
