...
billing screens drive. Companion to ``kova_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT kova already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``KOVA_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# kova_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``KOVA_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("KOVA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `kova portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
