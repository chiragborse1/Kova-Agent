...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other kova subprocess resolution: :func:`shutil.which` first,
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from kova_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_kova_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_kova_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_kova_home()}/.env later)[/dim]")
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update kova.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `kova plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid kova plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `kova plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  kova gateway restart[/dim]")
    console.print()
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``kova plugins enable`` /
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``kova plugins list`` CLI filters."""
    filtered = entries
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as kova
    ...
    try:
        from kova_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from kova_cli.curses_ui import flush_stdin
...
def plugins_command(args) -> None:
    """Dispatch kova plugins subcommands."""
