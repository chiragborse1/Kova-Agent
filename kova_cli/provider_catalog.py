...
The provider list shown by ``kova model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`kova_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``kova model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`kova_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``kova model`` shows.
"""
...
# kova_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `kova model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``kova model`` universe.
    ...
    # kova overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = KOVA_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # kova overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
