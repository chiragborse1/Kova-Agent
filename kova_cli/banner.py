...
# (e.g. nix-built kova — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/chiragborse1/Kova-Agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/chiragborse1/kova-agent"
...
def _fetch_pypi_latest(package: str = "kova-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a kova update is available.

    Two paths: if ``KOVA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = KOVA_HOME / ".update_check"
    embedded_rev = os.environ.get("KOVA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # KOVA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `kova update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/kova/update/check`
...
_RELEASE_URL_BASE = "https://github.com/chiragborse1/Kova-Agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical chiragborse1/Kova-Agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Kova Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else KOVA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = KOVA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from kova_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from kova_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built kova; we know an update
    ...
    # Pip-install warning — `pip install kova-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else KOVA_AGENT_LOGO
        console.print(_logo)
        console.print()
