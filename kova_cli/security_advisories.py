"""
Security advisory checker for Kova Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``kova doctor --ack <id>``; the ack is persisted to
...
1. ``kova doctor`` (and ``kova doctor --ack <id>``)
...
run in environments where the rest of kova failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including kova when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.kova/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: kova doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — kova requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'kova doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.kova/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_kova_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``kova doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `kova doctor` on the gateway host for details.")
