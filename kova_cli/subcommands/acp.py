...
def build_acp_parser(subparsers, *, cmd_acp: Callable) -> None:
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Kova Agent as an ACP (Agent Client Protocol) server",
        description="Start Kova Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print kova ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
