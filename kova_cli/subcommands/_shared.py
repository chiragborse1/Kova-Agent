...
These were module-level helpers in ``kova_cli/main.py``. They are pulled
into a neutral module so both ``main.py`` and every
``kova_cli/subcommands/<group>.py`` builder can import them without an
...
def add_accept_hooks_flag(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to KOVA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
