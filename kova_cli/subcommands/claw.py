...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to kova",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to kova",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.kova/ (by default a "
        "single restore-point archive is written to ~/.kova/backups/ "
        "before apply; restorable with 'kova import').",
    )
    claw_migrate.add_argument(
