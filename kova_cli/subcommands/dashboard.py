"""``kova dashboard`` subcommand parser.

Extracted verbatim from ``kova_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Kova Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `kova dashboard` cmdlines and SIGTERM them directly — the same
    # path `kova update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running kova dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running kova dashboard processes and exit",
    )
    # Backward-compat shim: older kova desktop app shells (<= 0.15.x) spawn the
    # backend as `kova dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "kova couldn't start" with no actionable cause. Accept and silently
    ...
    # `kova dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.kova/.env.
    # Nested subparser so bare `kova dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "KOVA_DASHBOARD_OAUTH_CLIENT_ID into ~/.kova/.env, and prints "
            "how to engage the login gate. Requires being logged in (kova setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://kova.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via KOVA_DASHBOARD_PORTAL_URL. Mainly for "
