"""``kova security`` subcommand parser.
...
def build_security_parser(subparsers, *, cmd_security: Callable) -> None:
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the kova "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.kova/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the kova Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
