"""``kova gateway`` and ``kova proxy`` subcommand parsers.
...
from kova_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if KOVA_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy kova.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy kova gateway unit files "
            "(e.g. kova.service) left over from older installs. Profile "
            "units (kova-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.kova/.env. "
            "Requires being logged in (kova setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.kova/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `kova proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
