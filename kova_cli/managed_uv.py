...
kova owns its own uv binary at ``$KOVA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$KOVA_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where kova keeps *its* uv binary.

    ``$KOVA_HOME/bin/uv`` on POSIX, ``$KOVA_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``kova update`` runs
    the call site from the *old*, already-imported ``kova_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``kova update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$KOVA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
