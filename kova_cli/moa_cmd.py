...
from kova_cli.moa_config import DEFAULT_MOA_PRESET_NAME, normalize_moa_config
...
def _pick_slot(current: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if not providers:
        raise RuntimeError("No configured model providers found. Run `kova model` first.")
...
def cmd_moa(args) -> None:
    ...
    if sub == "delete":
        ...
        if not preset_name:
            raise SystemExit("Usage: kova moa delete <name>")
