...
from kova_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _print_setup_summary(config: dict, KOVA_HOME):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'kova setup' to configure"))
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from kova_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'kova setup terminal'"))
    ...
    print(f"   {color('kova setup terminal', Colors.GREEN)} Change terminal backend")
    ...
    print(
        f"   {color('kova config edit', Colors.GREEN)}    Open config in your editor"
    )
...
# between `kova tools` and `kova setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # on demand via `kova auth add`, `kova setup` vision, and
    # `kova setup tts`. This keeps both quick and full setup thin.
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.kova/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'kova setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where kova runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `kova setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `kova setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `kova setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from kova_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _profile_name_from_KOVA_HOME(KOVA_HOME) -> str | None:
    """Return the active profile name when KOVA_HOME is a profile dir."""
...
def _setup_qqbot():
    ...
    from kova_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from kova_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with kova from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'kova setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     kova config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo kova gateway install --system")
                print_info("  Or run in foreground:  kova gateway")
        else:
            from kova_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   kova gateway run          # Run as container main process")
                print_info("")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from kova_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from kova_cli.config
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing kova config):", Colors.YELLOW))
...
def _offer_openclaw_migration(KOVA_HOME: Path) -> bool:
    ...
    print_info("kova can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: kova claw migrate --dry-run"
        )
        return False
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in kova (use kova claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from kova_cli.main import _model_flow_nous
    ...
    print_info("  Run `kova portal info` to inspect routing.")
    print_info("  Run `kova` to start chatting.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``kova tools`` (which
    ...
    try:
        ...
        from kova_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``kova setup agent`` /
    ``kova config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, KOVA_HOME, is_existing: bool):
    ...
    if path == 0:
        ...
        # `kova update` runs don't re-inject them.
        try:
...
def _blank_slate_walkthrough(config: dict, KOVA_HOME):
    ...
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `kova mcp add <name> --url ... | --command ...`.")
    else:
...
def _run_quick_setup(config: dict, KOVA_HOME):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'kova setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect kova to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'kova setup gateway'.")
