...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in kova_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # kova uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while kova openai-codex talks to the same
