"""CLI for the kova Kanban board — ``kova kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/kova-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this KOVA_HOME and its
    ...
    Used by ``kova kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`kova kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across kova profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://kova-agent.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/kova-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # KOVA_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `kova kanban boards switch <slug>` or the "
            "KOVA_KANBAN_BOARD env var). Use `kova kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `kova project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $KOVA_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $KOVA_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `kova gateway start`.",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $KOVA_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $KOVA_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``kova kanban …`` argparse dispatch.
    ...
    # env-var pin for the duration of this call. Using KOVA_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `kova kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # KOVA_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("KOVA_PROFILE_NAME", "KOVA_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (kova kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``kova kanban boards <action>``.
    ...
    DB, so a fresh KOVA_HOME that has never called ``kanban init``
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `kova kanban boards delete <slug>` (alias), the
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `kova -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``kova kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`kova kanban list --status ready` / "
                    f"`kova kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real kova profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from kova_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from kova_cli import kanban_decompose as decomp
