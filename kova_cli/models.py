...
Add, remove, or reorder entries here — both `kova setup` and
`kova` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `kova model`
    flow without maintaining a separate static list.
    """
    from kova_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $KOVA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append kova-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $KOVA_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. kova refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older kova that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older kova that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_kova_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$KOVA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  kova model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `kova model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `kova model` TUI
...
#
# Some vendors expose several kova provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `kova model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``kova model``, the
...
# path. The *interactive* default (GUI onboarding / ``kova model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in kova_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``kova auth add openai-codex`` without
    ``kova model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``kova model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    kova-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from kova_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — kova-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from kova_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from kova_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``kova model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from kova_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from kova_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to kova' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``kova_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether kova should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``kova auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.kova/.env``.
    ...
    """
    try:
        from kova_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from kova_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a kova release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a kova release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from kova_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            from kova_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a kova release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``kova model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $KOVA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from kova_constants import get_kova_home
    return get_kova_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$KOVA_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from kova_cli.copilot_auth import copilot_request_headers
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _KOVA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: kova' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _KOVA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from kova_constants import get_kova_home
    return get_kova_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `kova moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"kova will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so kova cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models kova actually ships.  (#46850)
