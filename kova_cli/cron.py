...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(kova\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*kova)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*kova)"
    r"|(p?kill\s+.*kova.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'kova cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: kova gateway restart")
        ...
        print("    kova gateway install    # Install as a user service")
        print("    sudo kova gateway install --system  # Linux servers: boot-time system service")
        print("    kova gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `kova gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: kova cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
