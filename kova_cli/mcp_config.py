"""
MCP Server Management CLI — ``kova mcp`` subcommand.

Implements ``kova mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.kova/config.yaml under the ``mcp_servers`` key.
"""
...
from kova_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_kova_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from kova_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.kova/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``kova mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return KovaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # kova_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info('  kova mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  kova mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kova_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: kova mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_kova_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `kova mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kova mcp add <name> --url <endpoint>')
        _info('  kova mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``kova mcp login`` and
    ``kova mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `kova mcp remove` + `kova mcp add` to reconfigure auth.")
        return False
...
def cmd_mcp_reauth(args):
    ...
    ``kova mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``kova mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: kova mcp reauth <name>   |   kova mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def mcp_command(args):
    """Main dispatcher for ``kova mcp`` subcommands."""
    ...
    if action == "picker":
        from kova_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from kova_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from kova_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `kova plugin`.
        from kova_cli.mcp_picker import run_picker
        run_picker()
        ...
        _info("kova mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
