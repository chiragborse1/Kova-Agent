...
2. The persistence shape from the June 2026 ``kova-0day`` campaign: a shell
   ...
   whose payload appended an attacker SSH key to ``authorized_keys``; kova
...
3. A hardcoded indicator-of-compromise (IOC) blocklist for that campaign — the
   attacker's ``kova-0day`` SSH public key and source IPs. Any entry whose
...
# A shell payload that touches any of these is the June 2026 kova-0day shape
...
# ── Indicators of compromise: June 2026 kova-0day campaign ──────────────────
...
# multiple compromised public instances (r/KovaAgent, 854.media).
_IOC_SUBSTRINGS = (
    # Attacker SSH public key (the "kova-0day" persistence key).
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a known kova-0day IOC anywhere in command/args/env (hardcoded blocklist);
    ...
    * a shell interpreter whose inline script writes to an OS persistence
      surface (June 2026 kova-0day SSH/PAM/sudoers/cron shape).
    """
    ...
    for ioc in _IOC_SUBSTRINGS:
        if ioc in flat:
            issues.append(
                f"MCP server '{name}' contains a known kova-0day "
    ...
    if _PERSISTENCE_PATTERN.search(script):
        issues.append(
            ...
            f"shell rc) — this is the kova-0day backdoor shape, not a real "
            f"MCP server"
        )
