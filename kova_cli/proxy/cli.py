"""CLI handlers for the ``kova proxy`` subcommand."""
...
from kova_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "kova proxy requires aiohttp. Install one of:\n"
        "  pip install 'kova-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"kova auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting kova proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("kova proxy upstream adapters\n")
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``kova proxy <subcommand>``."""
