...
# xAI's public API is OpenAI-compatible for the endpoints kova commonly
# uses. The Responses endpoint is included because kova' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via kova-managed OAuth credentials."""

    auth_hint = "kova auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`kova auth add xai-oauth --type oauth` first."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `kova auth add xai-oauth --type oauth`."
            )
