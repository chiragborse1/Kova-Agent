#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the kova Skills Hub.

Powers both:
  - `kova skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `kova skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.kova/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.kova/skills/<name>/)[/]"
        )
    try:
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "kova-index".
    ...
    # The identifier is what you pass to `kova skills install`. Browse used
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]kova skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with kova-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_kova_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.kova.blueprint block, it is a runnable automation. Register it as
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="kova skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "kova-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"kova-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying kova-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``kova -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``KOVA_HOME`` at process
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `kova update` keeps)."""
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: kova skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_kova_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via kova Skills Hub.\n\n"
                        f"This skill was scanned by the kova Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `kova skills <subcommand>` — called from kova_cli/main.py."""
