"""MCP picker — interactive `kova mcp picker` (also the default `kova mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``kova mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `kova plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from kova_cli.cli_output import prompt_yes_no
from kova_cli.curses_ui import curses_single_select
from kova_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from kova_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `kova mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: kova mcp install <name>    Picker: kova mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their kova is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer kova — run `kova update` "
...
def show_catalog() -> None:
    """`kova mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`kova mcp picker` (and default `kova mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`kova mcp install <name>` — non-interactive entry-point.
