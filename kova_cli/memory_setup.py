"""kova memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as kova tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if uv_path:
        ...
        if not pip_cmd:
            ...
            print(f"  Then re-run: kova memory setup")
            return
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'kova memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.kova/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.kova/plugins/")
