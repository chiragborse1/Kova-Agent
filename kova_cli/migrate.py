"""CLI handlers for ``kova migrate ...``.

Currently exposes only ``kova migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``kova migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: kova migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `kova migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `kova doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
