"""Slash command definitions and autocomplete for the kova CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of kova config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal kova works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while kova is working", "Configuration",
    ...
    CommandDef("update", "Update Kova Agent to the latest version", "Info"),
    CommandDef("version", "Show Kova Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`kova_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/kova`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. kova ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full kova
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``kova skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kova skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``kova skills list`` and
...
# unrelated command landed. These claim their slots right after /kova,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /kova <command>).
...
# native slot, the alias spelling stays reachable via /kova reset).
...
# ``/kova <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/kova" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /kova credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /kova billing.
...
_SLACK_VIA_KOVA_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/kova <verb>`` subcommand.
    ...
    can still reach them via ``/kova <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/kova`` is always reserved as the first entry so the
    legacy ``/kova <subcommand>`` form keeps working for anything that
    ...
    # Reserve /kova as the catch-all top-level command.
    ...
    # everything except /kova, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://kova-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /kova handler.

    Maps both canonical names and aliases so /kova bg do stuff works
    the same as /kova background do stuff.

    Plugin-registered slash commands are included so ``/kova <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from kova_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
