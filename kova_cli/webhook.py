...
Subscriptions persist to ~/.kova/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _KOVA_HOME() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: kova gateway run
"""
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (kova gateway run).\n")
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from kova webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (kova gateway run)")
