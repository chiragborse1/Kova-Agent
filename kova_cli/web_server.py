...
Usage:
    python -m kova_cli.main web          # Start on http://127.0.0.1:9119
    python -m kova_cli.main web --port 8080
"""
...
from kova_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from kova_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `kova dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["KOVA_WEB_DIST"]) if "KOVA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``kova gateway run`` — but the
    desktop app spawns a ``kova dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same KOVA_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import kova_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire kova_cli.gateway import into a background thread so the event
    # loop is not blocked and KOVA_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (KOVA_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `kova
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("KOVA_DESKTOP") == "1":
...
from kova_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# KOVA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("KOVA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-kova-Session-Token"
...
#
# This list is defined in ``kova_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from kova_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-kova-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``kova-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from kova_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from kova_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates kova (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real kova provider slug — fine.
    ...
    1. Vendor-name → kova-provider mapping: when the provider string is
       not a known kova provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from kova_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from kova_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "KOVA_DASHBOARD_FILES_ROOT"
...
def _default_KOVA_root_is_opt_data() -> bool:
    raw = os.environ.get("KOVA_HOME", "").strip()
    if not raw:
        return False
    try:
        from kova_constants import get_default_kova_root

        root = get_default_kova_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``kova update``.
    ...
    a container — e.g. the kova-webui image sharing the kova source tree),
    the dashboard's ``kova update`` button is the correct update path and
    ...
    """
    if _default_KOVA_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's kova root is actually /opt/data
    # (the container/hosted layout) or when KOVA_DASHBOARD_FILES_ROOT is set.
    if _default_KOVA_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``kova:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.kova-tmp-{os.getpid()}")
    try:
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_kova_home() at call time
    ...
    try:
        ...
        # Windows install the first import of kova_cli.gateway blocks the
        ...
        # and which providers are registered so ``kova status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from kova_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_kova": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the kova home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_kova_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_KOVA_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_KOVA_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_KOVA_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_KOVA_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from kova_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.kova/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_kova_home() / "logs"
...
def _spawn_KOVA_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``kova <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``kova_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "kova_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "KOVA_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["kova", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``kova gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``kova gateway restart`` children race each other on the
    ...
    return _spawn_KOVA_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``kova gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``KOVA_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_kova():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("kova-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "kova-update",
    ...
    try:
        proc = _spawn_KOVA_action(["update"], "kova-update")
    except Exception as exc:
        _log.exception("Failed to spawn kova update")
...
@app.get("/api/kova/update/check")
async def check_KOVA_update(force: bool = False):
    """Report whether a kova update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/kova/update`` actually runs ``kova update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed kova version string
    ...
    try:
        ...
        if force:
            try:
                (get_kova_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.kova/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another kova surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kova supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_kova_home() / provider.name / "config.json"
...
# in kova_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from kova_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `kova model`
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from kova_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.kova/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from kova_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (kova_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from kova_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from kova_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from kova_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from kova_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run kova from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect kova to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use kova from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect kova to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use kova in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use kova through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from kova via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to kova through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect kova to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use kova inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect kova to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use kova through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect kova to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect kova to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose kova as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://kova-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in kova gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in kova gateway setup",
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, kova appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's KOVA_HOME.
...
# the canonical ``kova auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.kova/.anthropic_oauth.json`` — kova-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_KOVA_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_KOVA_oauth_credentials,
            _KOVA_OAUTH_FILE,
        )
    except ImportError:
        read_KOVA_oauth_credentials = None  # type: ignore
        _KOVA_OAUTH_FILE = None  # type: ignore

    KOVA_creds = None
    if read_KOVA_oauth_credentials:
        try:
            KOVA_creds = read_KOVA_oauth_credentials()
        except Exception:
            KOVA_creds = None
    if KOVA_creds and KOVA_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "KOVA_pkce",
            "source_label": f"kova PKCE ({_KOVA_OAUTH_FILE})",
            "token_preview": _truncate_token(KOVA_creds.get("accessToken")),
            "expires_at": KOVA_creds.get("expiresAt"),
            "has_refresh_token": bool(KOVA_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into kova even
    when they also have a separate kova-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    kova never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "kova auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "kova auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "kova auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside kova, so the disconnect
    ...
    user sees exactly what executes, and kova then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside kova — run the disconnect command to remove it."
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"kova auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("KOVA_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the kova-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _KOVA_OAUTH_FILE
                if _KOVA_OAUTH_FILE.exists():
                    _KOVA_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from kova_cli.auth import clear_provider_auth
        ...
        try:
            from kova_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.kova/.anthropic_oauth.json AND credential pool
...
# Guarded so kova web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both kova file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``kova auth add anthropic``.
    """
    from agent.anthropic_adapter import _KOVA_OAUTH_FILE
    payload = {
    ...
    _KOVA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _KOVA_OAUTH_FILE.with_name(
        f"{_KOVA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _KOVA_OAUTH_FILE)
        try:
            _KOVA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("KOVA_PORTAL_BASE_URL")
...
# the tokens exactly like `kova auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("KOVA_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `kova auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from kova_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from kova_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `kova sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `kova sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_kova_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from kova_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_kova_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from kova_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, KOVA_HOME)."""
    from kova_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process KOVA_HOME at import time. The dashboard is a single
    ...
    with _CRON_PROFILE_LOCK:
        ...
        token = set_kova_home_override(str(home))
...
#
# Wraps the same config data layer the CLI uses (kova_cli.mcp_config), so
# servers managed here show up under `kova mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from kova_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from kova_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from kova_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from kova_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # KOVA_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds KOVA_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_KOVA_action(
...
#
# Wraps the same JSON store the CLI uses (kova_cli.webhook); the webhook
...
# spawn the real `kova gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_KOVA_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_KOVA_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
# `kova memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `kova memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_KOVA_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_KOVA_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_KOVA_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `kova import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_KOVA_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from kova_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from kova_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <KOVA_HOME>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_kova_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_KOVA_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``kova`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from kova_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`kova -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the KOVA_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_KOVA_HOME()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "kova setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local KOVA_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local KOVA_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the KOVA_HOME override. Returns the
    number of skills newly disabled.
    """
    from kova_constants import set_kova_home_override, reset_kova_home_override
    from kova_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's KOVA_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``kova profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from KOVA_HOME).
    """
    from kova_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``kova profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the KOVA_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``kova profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from kova_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_kova_home()`` at call
       time — the context-local override from ``set_kova_home_override``
    ...
    retargeted to the *current* ``get_kova_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a KOVA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_kova_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_kova_home()`` at call time
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``kova tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``kova tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``kova tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.kova/.env`` via ``save_env_value`` —
    the same store ``kova tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``kova tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``kova -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same KOVA_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not kova,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``kova computer-use permissions grant`` as a background action.
...
if sys.platform.startswith("win"):
    try:
        from kova_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from kova_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from kova_cli.dashboard_auth.audit import AuditEvent, audit_log
        from kova_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``KOVA_TUI_RESUME`` env var —
    matching what ``kova_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``KOVA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``KOVA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``KOVA_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``KOVA_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_kova_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``kova -p <name>`` performs. The in-process
    ``KOVA_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from kova_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("KOVA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("KOVA_TUI_INLINE", "1")
    env["KOVA_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["KOVA_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["KOVA_TUI_RESUME"] = resume

    if sidecar_url:
        env["KOVA_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile KOVA_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["KOVA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from kova_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="kova-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall kova inside WSL2 to use the dashboard's /chat "
...
# KOVA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`kova_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from kova_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/kova/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /kova`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__KOVA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/kova``)
        ...
        ``__KOVA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__KOVA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KOVA_BASE_PATH__="{prefix}";'
                f"window.__KOVA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__KOVA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__KOVA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KOVA_BASE_PATH__="{prefix}";'
                f"window.__KOVA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/kova`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the kova backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "kova-blue",     "label": "Kova Blue",           "description": "Light mode — vivid Kova-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.kova/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.kova/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_kova_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.kova/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as kova_cli.plugins):
    1. User plugins:    ~/.kova/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.kova/plugins/  (only if KOVA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from kova_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_kova_home() / "plugins", "user"),
    ...
    # ``kova_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("KOVA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".kova" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from kova_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_kova_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from kova_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from kova_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kova_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kova_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.kova/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"KOVA_dashboard_plugin_{plugin['name']}"
...
from kova_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``KOVA_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("KOVA_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    # the kova-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing KOVA_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `kova dashboard register` (Nous Portal) or "
    ...
    # KOVA_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"KOVA_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  kova Web UI → http://{host}:{actual_port}")
