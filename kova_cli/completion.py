"""Shell completion script generation for kova CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_KOVA_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Kova Agent bash completion
# Add to ~/.bashrc:
...
_KOVA_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_KOVA_profiles)" -- "$cur"))
        return
    fi
...
complete -F _KOVA_completion kova
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef kova
# Kova Agent zsh completion
# Add to ~/.zshrc:
...
_kova() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_KOVA_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'kova command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Kova Agent fish completion",
        "# Add to your config:",
        "#   kova completion fish | source",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c kova -f -s p -l profile"
        " -d 'Profile name' -xa '(__KOVA_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__KOVA_profiles)' -d 'Profile name'"
                )
