...
- ``kova_cli.providers``        -- canonical provider identity + overlays
- ``kova_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_KOVA_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous kova 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_KOVA_NON_AGENTIC_RE.search(model_name))


def _check_KOVA_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous kova 3/4 chat model."""
    if is_nous_KOVA_non_agentic(model_name):
        return _KOVA_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``kova config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from kova_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kova model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kova doctor' — config issues detected:"
        ...
        from kova_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from kova_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from kova_cli.config import load_config
            from kova_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from kova_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in kova_cli.runtime_provider.resolve_runtime_provider;
    ...
    kova_warn = _check_KOVA_model_warning(new_model)
    if kova_warn:
        warnings.append(kova_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from kova_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from kova_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from kova_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by kova provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://kova-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a kova release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from kova_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from kova_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check kova-mapped providers ---
    from kova_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from kova_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for kova_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(kova_id)
        if (
            _alias_target
            and _alias_target != kova_id
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(kova_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `kova model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(kova_id)
        if not model_ids:
            model_ids = curated.get(kova_id, [])
            if kova_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(kova_id, model_ids)
        total = len(model_ids)
    ...
    for pid, overlay in KOVA_OVERLAYS.items():
        ...
        # Resolve kova slug — e.g. "github-copilot" → "copilot"
        kova_slug = _mdev_to_kova.get(pid, pid)
        if kova_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(kova_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, kova_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or kova_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(kova_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", kova_slug, exc)
        ...
        # configured.
        if not has_creds and kova_slug == "anthropic":
            try:
                ...
                if (kova_creds and kova_creds.get("accessToken")) or \
        ...
        if kova_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(kova_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(kova_slug)
                model_ids = _ids if _ids else (curated.get(kova_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(kova_slug, []) or curated.get(pid, [])
        elif kova_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `kova model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from kova_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(kova_slug, []) or curated.get(pid, [])
                if kova_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(kova_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": kova_slug,
            "name": get_label(kova_slug),
            "is_current": kova_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "kova",
        ...
        seen_slugs.add(kova_slug.lower())
        _record_builtin_endpoint(kova_slug)
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is kova's canonical write key (matches
            ...
            # kova writes ``models:`` as a dict keyed by model id
            # (see kova_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from kova_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # kova's own writer uses; a hyphen variant is accepted
            ...
            # active model. kova's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, kova_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from kova_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`kova_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from kova_cli.models import fetch_openrouter_models
