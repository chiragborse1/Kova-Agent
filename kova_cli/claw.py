...
from kova_cli.config import get_kova_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a kova gateway is running with connected platforms.
    ...
    print_info("Recommendation: stop the gateway first with 'kova gateway stop'.")
    print()
...
def _cmd_migrate(args):
    ...
    # migrate-kova posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: kova claw migrate --source /path/to/.openclaw")
        return
    ...
    # restorable with `kova import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from kova_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(KOVA_HOME=KOVA_HOME)
            if backup_archive:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  kova claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  kova claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  kova config set OPENROUTER_API_KEY sk-or-v1-...")
