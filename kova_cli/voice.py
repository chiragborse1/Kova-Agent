...
# lookup, and kova-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and kova-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when KOVA_VOICE_DEBUG=1.
