...
Used by ``kova profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``kova_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Kova Agent kanban board.
...
Rules:
  ...
  - Never write "Kova Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_kova_home())
        else:
