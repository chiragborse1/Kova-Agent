"""``kova project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$KOVA_HOME/projects.db`` store
(see :mod:`kova_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Kova Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``kova project …`` argparse dispatch."""
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `kova project add-folder`.",
            file=sys.stderr,
        )
        return 1
