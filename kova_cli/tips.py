...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or kova -c.",
    ...
    "Ctrl+Z suspends kova to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "kova -c resumes your most recent CLI session. kova -c \"project name\" resumes by title.",
    "kova -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "kova -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    ...
    "kova --yolo bypasses all dangerous command approval prompts for the entire session.",
    "kova chat --source telegram tags the session for filtering in kova sessions list.",
    "kova -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "kova doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "kova dump outputs a compact setup summary — great for bug reports.",
    "kova config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "kova config edit opens config.yaml in your default editor.",
    "kova config check scans for missing or stale configuration options.",
    "kova sessions browse opens an interactive session picker with search.",
    "kova sessions stats shows session counts by platform and database size.",
    "kova sessions prune --older-than 30 cleans up old sessions.",
    "kova skills search react --source skills-sh searches the skills.sh public directory.",
    "kova skills check scans installed hub skills for upstream updates.",
    "kova skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "kova skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "kova mcp add github --command npx adds MCP servers from the command line.",
    "kova mcp serve runs kova itself as an MCP server for other agents.",
    "kova auth add lets you add multiple API keys for credential pool rotation.",
    "kova completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "kova logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "kova backup creates a zip backup of your entire kova home directory.",
    ...
    "kova gateway install sets up kova as a system service (systemd/launchd).",
    "kova memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "kova webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: kova tools disables unused tools, kova skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "kova models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as kova -w).",
    ...
    "KOVA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call kova tools programmatically — results stay out of context.",
    ...
    "When exiting, kova prints a resume command with session ID and stats.",
    "kova sessions export backup.jsonl exports all sessions for backup or analysis.",
    "kova -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "kova skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .kova/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: kova cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "kova runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to kova via a local macOS server.",
    ...
    "Context files (.kova.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "kova loads project context from .kova.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — kova tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.kova/checkpoints/ — your project's .git is never touched.",
    ...
    "kova plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "kova acp runs kova as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "KOVA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "kova auth add nous or kova auth add openai-codex sets up OAuth-based providers.",
    ...
    "Cron scripts live in ~/.kova/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make kova your own.",
    ...
    "Before a gateway session reset, kova auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.kova/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope kova state via KOVA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "KOVA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in KOVA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.kova/interrupt_debug.log with timestamps.",
    ...
    "kova claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "KOVA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via kova tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.kova/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.kova/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — kova auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of kova config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while kova is working.',
    ...
    '/reload reloads ~/.kova/.env into the running session — pick up new API keys without restarting.',
    ...
    "KOVA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.kova/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.kova/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'KOVA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "KOVA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'kova dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'kova dashboard embeds the full kova TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.kova/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.kova/dashboard-plugins/ — no npm build required.',
    ...
    'KOVA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'KOVA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'KOVA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'KOVA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.kova/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.kova/skills/ so drop-in skills appear without restarting the session.',
    ...
    "kova chat --source tool tags programmatic chats so they don't clutter kova sessions list.",
    'kova dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'kova sessions rename <ID> "new title" renames any past session; kova sessions delete <ID> removes one.',
    'kova import restores a session export or profile archive produced by sessions export or profile export.',
    'kova fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'kova pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'kova setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'kova status --deep runs the full health sweep across every component; plain kova status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'KOVA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'KOVA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.kova/plugins/ — trust-gated by design.',
    "KOVA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'KOVA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'KOVA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'KOVA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'KOVA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'KOVA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'KOVA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'KOVA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'KOVA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'KOVA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'KOVA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes kova block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from kova sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.kova/dashboard-plugins/.',
]
