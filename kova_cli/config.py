...
Config files are stored in ~/.kova/ for easy access:
- ~/.kova/config.yaml  - All settings (model, toolsets, terminal, etc.)
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``kova config set`` (which rewrites
    ...
    ``config.yaml`` in place: kova never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.kova/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``kova logs``
...
#   the next ``subprocess.run([...])`` kova makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. kova itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``kova update``, the TUI build.
...
#   on every plugin install / ``kova update``.
...
# * ``KOVA_HOME`` / ``kova_PROFILE`` / ``kova_CONFIG`` /
#   ``kova_ENV`` — kova runtime location flags. Writing these into
...
#
# IMPORTANT: ``KOVA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (kova_LANGFUSE_PUBLIC_KEY,
# kova_SPOTIFY_CLIENT_ID, ...). The
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or kova runtime location "
            "(KOVA_HOME, kova_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "kova_TOOL_PROGRESS", "kova_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `kova plugins enable
    # observability/langfuse` or `kova tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "kova_LANGFUSE_ENV",
    "kova_LANGFUSE_RELEASE",
    "kova_LANGFUSE_SAMPLE_RATE",
    "kova_LANGFUSE_MAX_CHARS",
    "kova_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from kova_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("kova_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_kova_home() / ".managed"
...
def is_managed() -> bool:
    """Check if kova is running in package-manager-managed mode.

    Two signals: the kova_MANAGED env var (set by the systemd service),
    or a .managed marker file in KOVA_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``kova_cli/`` — i.e. the git checkout for source
    installs, ``/opt/kova`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$KOVA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how kova was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$KOVA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. kova_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.kova``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$KOVA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.kova:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$KOVA_HOME``, a
    ...
    ``docker`` and ``kova update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/kova-agent`` image bakes a ``docker``
        stamp into ``/opt/kova`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $KOVA_HOME.
    try:
    ...
    #    host install whose shared $KOVA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``kova update``.
    try:
        method = (
            (get_kova_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``kova_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$KOVA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/kova`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* kova lives in a ``uv tool`` layout.

    ``uv tool install kova-agent`` places the install at
    ``.../uv/tools/kova-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``kova-agent`` happens to be uv-tool-installed on the machine while
    the *active* kova is a regular pip/venv install, causing
    ``kova update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/kova-agent/" in norm + "/"
...
# Long-form text for ``kova update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# kova_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side kova, it doesn't update
...
Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    ...
    tags at https://hub.docker.com/r/nousresearch/kova-agent/tags
  • Your config and session history live under ``$KOVA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``kova update`` inside Docker.
...
def format_managed_message(action: str = "modify this kova installation") -> str:
    ...
    raw = os.getenv("kova_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this kova installation is managed by NixOS "
            f"(kova_MANAGED={env_hint}).\n"
            "Edit services.kova-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KOVA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, kova_bin
    or None if container mode is not active, we're already inside the
    container, or kova_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("kova_DEV") == "1":
        return None
    ...
    container_mode_file = get_kova_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "kova-agent")
    exec_user = info.get("exec_user", "kova")
    kova_bin = info.get("kova_bin", "/data/current-package/bin/kova")
...
# Re-export from kova_constants — canonical definition lives there.
from kova_constants import get_kova_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kova_home() / "config.yaml"
...
def _resolve_KOVA_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the kova_UID / kova_GID env vars set by Docker deployments.
...
def _chown_to_KOVA_uid(path) -> None:
    """Chown ``path`` to ``kova_UID:kova_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kova group can
    share state with the gateway service.

    The mode can be overridden via the kova_HOME_MODE environment variable
    (e.g. kova_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KOVA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``kova_UID``/``kova_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("kova_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_KOVA_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When kova runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("kova_CONTAINER") or os.environ.get("kova_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set kova_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KOVA_HOME, upgrading legacy empty templates.
...
def _ensure_KOVA_HOME_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"KOVA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the kova-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps kova
        ...
        # identity slot (SOUL.md). Empty by default. The kova_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, kova adds a coding operating brief
    ...
    "terminal": {
        ...
        #             KOVA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force KOVA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, kova auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), kova sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when kova
        ...
        # bundled kova image, which drops to the `kova` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, kova sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and kova should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``kova chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kova/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: kova sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .kova.md, or .cursorrules before kova applies
    ...
    # single read_file page exceeds these limits, kova truncates the
    ...
    # See tools/kanban_tools.py and kova_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``kova dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``kova kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``kova kanban decompose`` and the kanban
        ...
        # ``kova profile describe <name> --auto`` and the dashboard's
    ...
    "display": {
        ...
        # Which interface bare `kova` (and `kova chat`) launches by default:
        ...
        # REPL and `--tui` (or KOVA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `kova --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kova -c` muscle memory.  Default off so existing
        # users aren't surprised.  KOVA_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/kova`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `kova pets`. Disabled until
        ...
        "pet": {
            ...
            # get_kova_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``kova_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``kova_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``kova_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``kova_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``kova_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/kova``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; kova appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kova/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kova/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, kova feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before kova auto-pauses the goal and
    ...
    # always goes to ~/.kova/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${kova_SKILL_DIR} and ${KOVA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `kova curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `kova curator run --consolidate` overrides
        ...
        # normally restored on every `kova update`, so pruning them only
        ...
        # skipped), snapshot ~/.kova/skills/ into
        # ~/.kova/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `kova curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for kova-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Kova Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KOVA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace kova' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.kova/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or kova_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `kova doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``kova_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow kova to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $KOVA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via kova_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `kova -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `kova` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `kova kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     kova-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core kova tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.kova/logs/.
    ...
    # update model picker lists without shipping a kova-agent release.
    ...
    "gateway": {
        ...
        # the kova_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.kova/.env,
        ...
        # behavior: files must live under the kova cache, under
        ...
        # user. Bridged to kova_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the kova
        # cache (~/.kova/cache/{documents,images,audio,video,screenshots})
        ...
        # to kova_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # kova_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to kova_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.kova/state.db.
    ...
    "sessions": {
        ...
        # default of ``kova sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.kova/sessions/session_{sid}.json`` on every turn
    ...
    # ``kova update`` behaviour.
    "updates": {
        # Run a full ``kova backup``-style zip of KOVA_HOME before every
        # ``kova update``.  Backups land in ``<KOVA_HOME>/backups/`` and
        # can be restored with ``kova import <path>``.  Off by default:
        # zipping a large KOVA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``kova update
        ...
        "backup_keep": 5,
        # What `kova update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<KOVA_HOME>/lsp/bin/`` on first use.
    ...
    # `kova tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.kova/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.kova/.env (or your shell) and never in config.yaml.
            ...
            # ~/.kova/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. kova disables it for our users unless they
        # explicitly opt in here. When false (default), kova sets
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the kova auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use kova, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kova:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. kova_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #kova)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: kova-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'kova-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote kova API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote kova API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote kova API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # kova_TOOL_PROGRESS and kova_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kova.config`` entries, then checks
    ...
    try:
        ...
        # should never break `kova update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older kova versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kova_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kova setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — kova won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kova doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("KOVA_HOME", "~/.kova")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("kova_TOOL_PROGRESS")
            old_mode = get_env_value("kova_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from kova_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from kova_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KOVA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KOVA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$KOVA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_kova_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `kova plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `kova curator status` has no
    ...
    if current_ver < 23:
        try:
            curator_dir = get_kova_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from kova_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    # metadata.kova.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``kova config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _KOVA_HOME / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kova/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kova/config.yaml.
    ...
    (which change ``KOVA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/kova/config.yaml invalidates the
...
#   openai-codex (OAuth — kova auth) — OpenAI Codex
#   nous         (OAuth — kova auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.kova/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset kova writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kova/.env.
    ...
    interactive menu render (`kova tools`, `kova setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `kova tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real kova env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kova/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kova/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kova/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kova/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to kova — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known kova vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kova/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ kova Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from kova_cli.auth import get_anthropic_key
    ...
    # Warn on stale kova_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("kova_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale kova_MAX_ITERATIONS={_env_ghost} "
                f"(run 'kova doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `kova config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `kova config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: kova config set <key> <value>")
            print()
            print("Examples:")
        ...
        if missing_config:
            ...
            print("    Run 'kova config migrate' to add them")
...
# ``_inject_profile_env_vars`` surfaces them in ``kova config`` UI so users
