...
Consent records live under ``~/.kova/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.kova/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.kova/config.yaml.")
