...
Used by ``kova kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``kova_cli/goals.py`` — same aux
...
KOVA_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("KOVA_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Kova Agent board.
...
def _profile_author() -> str:
    """Mirror of ``kova_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("KOVA_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=KOVA_KANBAN_SPECIFY_MAX_TOKENS,
