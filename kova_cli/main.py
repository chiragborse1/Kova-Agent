...
# IMPORTANT: kova_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``kova_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``kova update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``kova_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, kova crashes on import and the user can't run
# ``kova update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
       1. ``setproctitle`` (opt-in dep — installed via ``kova tools`` or
    ...
    try:
        ...
        # Windows: the .exe name is already ``kova.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* kova_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``KOVA_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("KOVA_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``KOVA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("KOVA_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _print_fast_version_info() -> None:
    from kova_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Kova Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    ...
    if os.environ.get("KOVA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from kova_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from kova_cli.subcommands.cron import build_cron_parser
from kova_cli.subcommands.gateway import build_gateway_parser
from kova_cli.subcommands.profile import build_profile_parser
from kova_cli.subcommands.model import build_model_parser
from kova_cli.subcommands.setup import build_setup_parser
from kova_cli.subcommands.postinstall import build_postinstall_parser
from kova_cli.subcommands.whatsapp import build_whatsapp_parser
from kova_cli.subcommands.slack import build_slack_parser
from kova_cli.subcommands.login import build_login_parser
from kova_cli.subcommands.logout import build_logout_parser
from kova_cli.subcommands.auth import build_auth_parser
from kova_cli.subcommands.status import build_status_parser
from kova_cli.subcommands.webhook import build_webhook_parser
from kova_cli.subcommands.hooks import build_hooks_parser
from kova_cli.subcommands.doctor import build_doctor_parser
from kova_cli.subcommands.security import build_security_parser
from kova_cli.subcommands.dump import build_dump_parser
from kova_cli.subcommands.debug import build_debug_parser
from kova_cli.subcommands.backup import build_backup_parser
from kova_cli.subcommands.import_cmd import build_import_cmd_parser
from kova_cli.subcommands.config import build_config_parser
from kova_cli.subcommands.version import build_version_parser
from kova_cli.subcommands.update import build_update_parser
from kova_cli.subcommands.uninstall import build_uninstall_parser
from kova_cli.subcommands.dashboard import build_dashboard_parser
from kova_cli.subcommands.gui import build_gui_parser
from kova_cli.subcommands.logs import build_logs_parser
from kova_cli.subcommands.prompt_size import build_prompt_size_parser
from kova_cli.subcommands.memory import build_memory_parser
from kova_cli.subcommands.acp import build_acp_parser
from kova_cli.subcommands.tools import build_tools_parser
from kova_cli.subcommands.insights import build_insights_parser
from kova_cli.subcommands.skills import build_skills_parser
from kova_cli.subcommands.pairing import build_pairing_parser
from kova_cli.subcommands.plugins import build_plugins_parser
from kova_cli.subcommands.mcp import build_mcp_parser
from kova_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (kova tools, kova setup, kova model) use
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any kova module import.
#
# Many modules cache KOVA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("KOVA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.kova/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KOVA_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `kova mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to kova' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo kova -p <name>` against the invoking user's home.
    ...
    # Mirrors kova_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If KOVA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.kova/profiles/coder or
    # /opt/data/profiles/coder).  If KOVA_HOME points to the kova root
    # instead (e.g. systemd hardcodes KOVA_HOME=/root/.kova), we must
    # still read active_profile — the user may have switched profiles via
    # `kova profile use` and the gateway should honour that choice.
    # See issue #22502.
    KOVA_HOME_env = os.environ.get("KOVA_HOME", "")
    if profile_name is None and KOVA_HOME_env:
        if Path(KOVA_HOME_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the kova root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as KOVA_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``kova gateway run`` to mean
    # "the root KOVA_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("KOVA_S6_SUPERVISED_CHILD"):
        try:
            from kova_constants import get_default_kova_root

            active_path = get_default_kova_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set KOVA_HOME
    if profile_name is not None:
        try:
            from kova_cli.profiles import resolve_profile_env

            KOVA_HOME = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            KOVA_HOME = _resolve_sudo_user_profile_env(profile_name)
            if not KOVA_HOME:
            ...
            # A bug in profiles.py must NEVER prevent kova from starting
            print(
...
# Load .env from ~/.kova/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from kova_cli.config import get_kova_home
from kova_cli.env_loader import load_KOVA_dotenv

load_KOVA_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → KOVA_REDACT_SECRETS env
# var BEFORE kova_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_kova_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "KOVA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["KOVA_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `kova` subcommands
...
try:
    from kova_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from kova_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if kova_constants not importable yet
...
from kova_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to kova_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (kova_cli.main._model_flow_*) keep resolving unchanged.
from kova_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_kova_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("KOVA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("KOVA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from kova_cli.config import get_env_path, get_kova_home, load_config
    from kova_cli.auth import get_auth_status

    # Determine whether kova itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_KOVA_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_kova_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if kova has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants kova to use their tokens.
    if _has_KOVA_config:
        try:
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo kova {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo kova {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @kova/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@kova" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/kova-ink/package.json",
    "packages/kova-ink/index.js",
    "packages/kova-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``KOVA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("KOVA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``KOVA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("KOVA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    KOVA_HOME = os.environ.get("KOVA_HOME") or str(Path.home() / ".kova")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "KOVA_HOME": KOVA_HOME},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(KOVA_HOME) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(KOVA_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if KOVA_cli_dir is None:
        KOVA_cli_dir = Path(__file__).parent
    bundled = KOVA_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``kova update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("KOVA_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this kova checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `kova update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the kova checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (KOVA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("KOVA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from kova_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("KOVA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with KOVA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset KOVA_TUI_DIR (e.g. `unset KOVA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @kova/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @kova/ink resolves through
        # packages/kova-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "kova-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from kova_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from kova_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="kova-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["KOVA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["KOVA_PYTHON_SRC_ROOT"] = os.environ.get(
        "KOVA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("KOVA_PYTHON", sys.executable)
    env.setdefault("KOVA_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["KOVA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["KOVA_MODEL"] = model
        env["KOVA_INFERENCE_MODEL"] = model
    if provider:
        env["KOVA_TUI_PROVIDER"] = provider
        env["KOVA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["KOVA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["KOVA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["KOVA_TUI_SKILLS"] = value
    if query:
        env["KOVA_TUI_QUERY"] = query
    if image:
        env["KOVA_TUI_IMAGE"] = image
    if checkpoints:
        env["KOVA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["KOVA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["KOVA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["KOVA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["KOVA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["KOVA_ACCEPT_HOOKS"] = "1"
    ...
    # KOVA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set KOVA_TUI_RESUME themselves.
    env.pop("KOVA_TUI_RESUME", None)
    if resume_session_id:
        env["KOVA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `kova update` so
    ...
    if code == 42:
        from kova_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``KOVA_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``kova kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``kova kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("KOVA_KANBAN_BOARD"):
        return
    try:
        from kova_cli.kanban_db import get_current_board

        os.environ["KOVA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.kova/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with kova — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. kova still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``KOVA_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("KOVA_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from kova_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from kova_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'kova doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like kova isn't configured yet -- no API keys or providers found."
        )
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'kova setup' at any time to configure.")
        sys.exit(1)
    ...
    # setup (config, rules files, plugins, MCP servers) or from kova itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (KOVA_SAFE_MODE is checked by
    # kova_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["KOVA_SAFE_MODE"] = "1"
        os.environ["KOVA_IGNORE_USER_CONFIG"] = "1"
        os.environ["KOVA_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.kova/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["KOVA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from kova_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from kova_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with kova?")
        print()
    ...
    # has no creds.json.  Every subsequent `kova gateway` then paid a 30s
    ...
    session_dir = get_kova_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  kova gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Kova Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kova gateway")
        ...
        print("⚠ Pairing may not have completed. Run 'kova whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``kova whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``kova_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from kova_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from kova_cli.config import stamp_install_method
    from kova_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ kova post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    from kova_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("KOVA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # PROVIDER_GROUPS in kova_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.kova/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.kova/.env if the active provider is not 'custom'.
    ...
    """
    from kova_cli.config import get_env_value, save_env_value, load_config
...
#
# kova uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`kova_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  kova only falls back to a lightweight backend (OpenRouter,")
...
def _aux_flow_provider_model(
    ...
    from kova_cli.auth import _prompt_model_selection
    from kova_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from kova_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from kova_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use kova URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# the catalog eagerly costs ~55ms on every `kova` invocation — including
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.kova/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from kova_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `kova setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from kova_constants import display_kova_home as _dhh_fn

            print(
                f"    kova will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate kova CLI with a provider."""
    from kova_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from kova_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from kova_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from kova_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from kova_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from kova_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``kova slack <subcommand>``. Currently supports:
    ...
    if sub == "manifest":
        from kova_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from kova_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from kova_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from kova_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from kova_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `kova security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from kova_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from kova_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from kova_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from kova_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up kova home directory to a zip file."""
    if getattr(args, "quick", False):
        from kova_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from kova_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a kova backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    from kova_cli.banner import format_banner_version_label
    ...
    try:
        from kova_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Kova Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from kova_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from kova_cli.uninstall import run_gui_uninstall
    ...
    from kova_cli.uninstall import run_uninstall
...
# Critical files that every ``kova`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``kova update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "kova_cli/main.py",
    "kova_cli/config.py",
    "kova_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``kova`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="kova-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``kova update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``kova_cli/web_dist/`` (per vite.config.ts
    outDir: "../kova_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "kova_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``kova`` launcher present but ``kova_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the kova venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — kova venv python3, no nix-shell overhead
    ...
    # a kova venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``kova update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``kova web``).
    ...
    # build path usable on a stock `py -m kova_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (kova web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``kova update`` can unconditionally call ``kova desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``kova desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $KOVA_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $KOVA_HOME."""
    from kova_constants import get_kova_home
    return get_kova_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``kova update`` that pulled new source but hasn't built yet).
    """
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/kova'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_kova_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``kova.exe`` keeps an exclusive lock on
    ``release/win-unpacked/kova.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\kova.exe: Access is
    ...
    an unrelated "kova" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "kova is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../kova.app/Contents/MacOS/kova  ->  app bundle = .../kova.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``kova desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ kova Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ kova Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from kova_logging import setup_logging as _setup_logging_gui
    ...
    # with_kova_node_path() copies os.environ when called with no arg.
    env = with_kova_node_path()
    if getattr(args, "fake_boot", False):
        env["KOVA_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["KOVA_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "KOVA_root", None):
        env["KOVA_DESKTOP_KOVA_ROOT"] = str(Path(args.KOVA_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["KOVA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds kova.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose kova.exe
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "kova is
    ...
    if source_mode:
        print("→ Launching kova Desktop from source build...")
    ...
    print(f"→ Launching packaged kova Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``kova dashboard`` processes other than ourselves.

    ``kova dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``kova update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the kova Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``kova dashboard`` as
    ...
    environment variable ``KOVA_DESKTOP_CHILD_PID`` on the spawned
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "kova.*dashboard"`) keeps us consistent
            # with `kova_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `kova update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``kova update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``kova update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: kova curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``kova dashboard`` processes.

    Called at the end of ``kova update`` (default ``reason``) and also
    from ``kova dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the kova Desktop Electron app spawns this dashboard as a
    # backend child, it sets KOVA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("KOVA_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.kova-update-staging"
    backup = f"{dst}.kova-update-old"
...
def _update_via_zip(args):
    """Update Kova Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `kova update "
            f"--branch {branch}`, or update against main with `kova update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/chiragborse1/Kova-Agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="kova-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"kova-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for kova-agent itself
        ...
        # GitHub ZIPs extract to kova-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"kova-agent-{branch}")
    ...
    from kova_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: kova skills list-modified  "
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "kova-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if kova behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes kova completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but kova couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but kova couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if kova behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but kova couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but kova couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `kova update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/chiragborse1/Kova-Agent.git",
    "git@github.com:chiragborse1/Kova-Agent.git",
    "https://github.com/chiragborse1/Kova-Agent",
    "git@github.com:chiragborse1/Kova-Agent",
}
OFFICIAL_REPO_URL = "https://github.com/chiragborse1/Kova-Agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_kova_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_kova_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from chiragborse1/Kova-Agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/chiragborse1/Kova-Agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/chiragborse1/Kova-Agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``kova update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from kova_constants import get_default_kova_root

    default_home = get_default_kova_root()
...
# Install-scoped breadcrumb dropped right before ``kova update`` mutates the
...
# survives and the next ``kova`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $KOVA_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``kova update`` left half-done.
    ...
    try:
        ...
        print(
            "⚠ A previous `kova update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from kova_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``kova update`` is still progressing even if pip/uv itself is silent.
    """
...
def _KOVA_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "kova.exe",
        scripts_dir / "kova-gateway.exe",
    ]


def _detect_concurrent_KOVA_instances(
    ...
    The kova Desktop Electron app spawns ``kova.EXE`` as a backend child,
    so during ``kova update`` the user-invoked process and the desktop's
    ...
    launcher (``kova.exe``) is a separate process from the Python
    ...
    for shim in _KOVA_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated kova.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``kova update``
    ...
    #      kova.exe sitting *under* a non-kova parent (e.g. a kova
...
def _format_concurrent_instances_message(
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `kova update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_KOVA_exe(
    ...
    when ``kova update`` runs, ``kova.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``kova.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next kova invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the kova Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       kova Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _KOVA_exe_shims(scripts_dir):
        ...
        print(
            "    Close kova Desktop, exit other `kova` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `kova update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_KOVA_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    shims, and on Windows the live ``kova.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``kova`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_KOVA_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every kova invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``kova update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `kova update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    ``_quarantine_running_KOVA_exe`` for the rationale.
    """
    ...
    # hours later inside ``kova update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``kova update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``kova.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``kova`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `kova update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `kova update --force` after closing other kova processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from kova_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$KOVA_HOME/bin/uv``, but on Termux the official
    ...
    """
    from kova_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `kova update`, so we install root-only first
    ...
    nixos_env = with_kova_node_path(_nixos_build_env())
...
class _UpdateOutputStream:
    """Stream wrapper used during ``kova update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.kova/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``kova update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``kova update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.kova/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``kova update --gateway``) the update is already
    ...
    try:
        ...
        # kova_cli.config.get_kova_home to simulate setup failure.
        from kova_cli.config import get_kova_home as _get_kova_home

        logs_dir = _get_kova_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``kova update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from kova_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``kova update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from kova_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from kova_cli.config import recommended_update_command
        from kova_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``kova update`` without requiring a reinstall.  The
    ...
    ``kova`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/kova.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v kova'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/kova, code at /usr/local/lib/kova-agent).
    fhs_link = Path("/usr/local/bin/kova")
    ...
    path_comment = (
        "# Kova Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of KOVA_HOME before running the update.
    ...
    KOVA_HOME directories.  The ``--backup`` flag on ``kova update``
    ...
    # ``kova_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # KOVA_HOME can add minutes to every update. Users who want the
    ...
    # Render path using display_kova_home so the user sees ~/.kova/...
    try:
        from kova_constants import get_kova_home, display_kova_home

        home = get_kova_home()
        try:
            display_path = f"{display_kova_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    kova.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating kova...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m kova_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: kova gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``kova gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from kova_cli import gateway_windows
        from kova_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``kova update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Kova Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Kova Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `kova update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update kova via pip (for PyPI installs)."""
    from kova_cli import __version__
    from kova_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "kova-agent"]
        ...
        cmd = [pipx, "upgrade", "kova-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "kova-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "kova-agent"]
    ...
    print("✓ Update complete! Restart kova to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Kova Agent...")
    print()

    # On Windows, abort early if another kova.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_KOVA_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $KOVA_HOME, this is cheap
        ...
        try:
            from kova_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in kova_cli/config.py bricked
        # every user who ran ``kova update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``kova
        ...
        # the old bytecode (e.g. get_kova_home added to kova_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``kova`` launch finishes the
        ...
        # build.  ``kova desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``kova desktop`` shouldn't be forced into a full
        # Electron build by ``kova update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from kova_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "kova_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `kova desktop` to retry)")
        ...
        # current — so copy it straight over ~/.kova/cache/model_catalog.json
        ...
        # modules in this process.  Reload kova_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_kova_home() added since the last release.
        try:
            import importlib
        ...
        # seed_profile_skills() uses subprocess with an explicit KOVA_HOME so
        # it is not affected by sync_skills()'s module-level KOVA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's KOVA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from kova_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from kova_cli.backup import restore_cron_jobs_if_emptied
        ...
        # don't have it. Tying the refresh to ``kova update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from kova_cli.tools_config import install_cua_driver
        ...
        # When running as ``kova update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_kova_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... kova-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... kova-gateway*)
            ...
            # Discover all kova-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. kova-gateway.service or kova-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `kova update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: kova gateway status"
                        )
        ...
        # Warn if legacy kova gateway unit files are still installed.
        # When both kova.service (from a pre-rename install) and the
        # current kova-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `kova update` surfaces the issue until the user migrates.
        try:
            from kova_cli.gateway import (
                has_legacy_KOVA_units,
                _find_legacy_KOVA_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_KOVA_units():
                print()
                print("⚠ Legacy kova gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_KOVA_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (kova.service) fight the current")
                print("  kova-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    kova gateway migrate-legacy")
                print()
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``kova -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `kova profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_kova_home()
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kova)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  kova profile alias {name} --name <custom>"
                    )
        ...
        from kova_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from kova_constants import get_kova_home as _hh
        ...
        from kova_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `kova profile info {name}` for full manifest)")
        if alias_name:
        ...
        from kova_cli.profiles import profile_exists
        ...
        from kova_cli.profiles import rename_profile
        ...
        from kova_cli.profiles import export_profile
        ...
        from kova_cli.profiles import import_profile
        ...
        import tempfile
        from kova_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="KOVA_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from kova_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `kova profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"kova -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from kova_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("KOVA_requires"):
            print(f"Requires:     kova {data['KOVA_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from kova_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.KOVA_requires:
        print(f"  Requires: kova {mf.KOVA_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``kova dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``kova dashboard --status``
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``kova dashboard register`` for OAuth.
    ...
    try:
        from kova_cli.web_server import should_require_auth
    ...
    try:
        from kova_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `kova dashboard register`)")
    print("    [3] Cancel")
    print()
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → KOVA_HOME points into
    ...
    if (
        ...
        and os.environ.get("KOVA_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "kova_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # KOVA_HOME.  We must resolve the root explicitly instead of just
        # dropping KOVA_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV KOVA_HOME=/opt/data`), so an unset
        # KOVA_HOME falls back to $HOME/.kova = /opt/data/.kova — an
        ...
        # update-button guard also misfires).  get_default_kova_root()
        # returns the root for both layouts: ~/.kova for a standard install
        ...
        try:
            from kova_constants import get_default_kova_root
            env["KOVA_HOME"] = str(get_default_kova_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop KOVA_HOME) rather than block the reroute.
            env.pop("KOVA_HOME", None)
    ...
    # the same logs directory as every other kova surface.
    try:
        from kova_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "KOVA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["KOVA_WEB_DIST"])
            if "KOVA_WEB_DIST" in os.environ
            else PROJECT_ROOT / "kova_cli" / "web_dist"
        )
    ...
    from kova_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from kova_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from kova_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from kova_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from kova_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter kova log files."""
    from kova_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from kova_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``kova -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``kova_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``kova -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``kova`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("KOVA_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        from kova_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["KOVA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["KOVA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["KOVA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_kova_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_kova_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_kova_home()}/memories/\n")
    else:
        from kova_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Kova Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from kova_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from kova_cli.tools_config import run_post_setup_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from kova_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from kova_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from kova_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from kova_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from kova_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from kova_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for kova CLI."""
    # Cosmetic: make the process show up as 'kova' instead of 'python3.11'
    ...
    # Sweep stale ``kova.exe.old.*`` quarantine files left by previous
    # ``kova update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_KOVA_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``kova update``
    ...
    # ``kova skills install update``) merely defers recovery one launch;
    # under-matching (missing ``kova -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in kova_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from kova_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from kova_cli.fallback_cmd import cmd_fallback
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.kova/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://kova-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from kova_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from kova_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in kova_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in kova_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in kova_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in kova_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `kova whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in kova_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from kova_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in kova_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in kova_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in kova_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in kova_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in kova_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in kova_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from kova_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from kova_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from kova_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in kova_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in kova_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in kova_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in kova_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in kova_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in kova_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.kova/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo kova uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from kova_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in kova_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in kova_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in kova_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in kova_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from kova_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in kova_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``kova --help``, ``kova version``, ``kova logs``,
    ...
    try:
        from kova_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and kova shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from kova_cli.pets import register_cli as _register_pets_cli
    ...
    # =========================================================================
    # memory command  (parser built in kova_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in kova_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `kova computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `kova tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `kova computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the kova app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from kova_cli.tools_config import _cua_driver_cmd
            # Honor KOVA_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from kova_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in kova_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_kova_home() / "sessions"
            ...
            # Launch kova --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in kova_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in kova_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in kova_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in kova_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in kova_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in kova_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in kova_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in kova_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in kova_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in kova_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in kova_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``kova -c Pokemon Agent Dev`` → ``kova -c 'Pokemon Agent Dev'``
    ...
    from kova_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'kova -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (kova hooks list, cron
