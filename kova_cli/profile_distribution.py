"""Profile distributions — shareable, packaged kova profiles via git.

A distribution is a kova profile published as a git repository (or
...
* ``kova profile export/import`` — local backup / restore for a profile
...
Subcommands (all live under ``kova profile``, not a parallel tree):

    kova profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "kova-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            KOVA_requires=str(data.get("KOVA_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.KOVA_requires:
            out["KOVA_requires"] = self.KOVA_requires
        if self.author:
...
def check_KOVA_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires kova {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``kova profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a kova profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a kova distribution."
        )

    # Version check up-front so we fail fast
    check_KOVA_requires(manifest.KOVA_requires, KOVA_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.kova).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="KOVA_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `kova profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`kova profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="KOVA_dist_update_") as tmp:
        plan = plan_install(
