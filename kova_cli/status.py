...
from kova_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``kova config``'s
...
def show_status(args):
    """Show status of all Kova Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Kova Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: kova model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: kova model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from kova_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from kova_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    print(color("  Run 'kova doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'kova setup' to configure", Colors.DIM))
    print()
