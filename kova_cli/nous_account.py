...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"kova could not verify your Nous Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but kova cannot verify "
    ...
    if account_info.paid_service_access is None:
        detail = (
            f"kova could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"kova could not find a Nous Portal account or organisation for this "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `kova model` to refresh kova."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from kova_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from kova_cli.auth import _decode_jwt_claims
    except Exception:
        return None
