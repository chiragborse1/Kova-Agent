...
_DEFAULT_MANAGED_DIR = Path("/etc/kova")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/kova`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``KOVA_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$KOVA_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/kova`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("KOVA_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through kova_cli.config.load_config) to honor managed
    scope. Mirrors kova_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from kova_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
