"""CLI subcommand: ``kova send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"kova send: {file_path} is not a text file. --file reads the "
            ...
            print(f"kova send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"kova send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"kova send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"kova send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"kova send: no targets found for platform '{platform_filter}'. "
...
def _load_KOVA_env() -> None:
    ...
    intentionally reimplement the minimum needed here so ``kova send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if message is None or not message.strip():
        print(
            "kova send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `kova send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `kova send --list telegram`.",
    )
