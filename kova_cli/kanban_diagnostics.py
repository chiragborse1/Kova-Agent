...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``kova -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``kova kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`kova kanban specify`, which uses auxiliary.triage_specifier."
        )
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong KOVA_HOME).
    ...
    works for kova profiles, registered lanes, external workers, and
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full kova runtime config.
