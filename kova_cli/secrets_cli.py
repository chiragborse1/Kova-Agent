"""CLI handlers for ``kova secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``kova_cli.main`` as part of building the top-level
    ``kova secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every kova process."
    )
    console.print(
        "  Status:  [cyan]kova secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]kova secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]kova secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — kova will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`kova secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]kova[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]kova secrets bitwarden setup[/cyan] and pick EU or "
