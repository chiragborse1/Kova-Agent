...
Toggles `model.openai_runtime` between "auto" (= chat_completions, kova'
...
The actual runtime resolution happens in kova_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "kova"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate kova' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into kova for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from kova_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the kova-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "kova-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + kova tool callback are always-on production
            ...
            if "kova-tools" in mig_report.migrated:
                msg_lines.append(
                    "kova tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default kova runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "kova tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default kova runtime.")
