"""
Doctor command for kova CLI.

Diagnoses issues with Kova Agent setup.
"""
...
from kova_cli.config import get_project_root, get_kova_home, get_env_path
...
from kova_cli.models import _KOVA_USER_AGENT
from kova_constants import OPENROUTER_MODELS_URL
...
from kova_constants import is_termux as _is_termux
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("KOVA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from kova_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from kova_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from kova_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != kova_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'kova update', or set "
            "kova_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-kova and dashboard static services are up
    ...
    for static in ("main-kova", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `kova profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from kova_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the KOVA_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("KOVA_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via KOVA_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `kova`.
    os.environ.setdefault("KOVA_INTERACTIVE", "1")

    # Handle `kova doctor --ack <id>` as a fast path. Persist the ack and
    ...
    print(color("│                 🩺 kova Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`kova doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and kova_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'kova setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'kova setup' to configure API keys")
                fixed_count += 1
            else:
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from kova_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from kova_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'kova config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from kova_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            (
                                ...
                                f"or switch providers with 'kova config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from kova_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'kova setup' to migrate config")
                else:
                    issues.append("Run 'kova doctor --fix' or 'kova setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'kova doctor --fix'")
        except Exception:
            pass

        # Detect stale KOVA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives KOVA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from kova_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("KOVA_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"KOVA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `kova setup` run)",
                )
                if should_fix:
                    if remove_env_value("KOVA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale KOVA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove KOVA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the KOVA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale KOVA_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'kova doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses kova' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = KOVA_HOME / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give kova a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Kova Agent Persona\n\n"
                "<!-- Edit this file to customize how kova communicates. -->\n\n"
                "You are kova, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = KOVA_HOME / "memories"
    ...
    state_db_path = KOVA_HOME / "state.db"
    if state_db_path.exists():
        try:
            ...
            from kova_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'kova doctor --fix' "
                        "(or 'kova sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from kova_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'kova doctor --fix' "
                        "(or 'kova sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = KOVA_HOME / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'kova doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                "(kova not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/kova → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/kova points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/kova → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/kova — run 'kova doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/kova exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/kova not found",
                    "(kova command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/kova → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/kova symlink — run 'kova doctor --fix'")
    ...
    try:
        from kova_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `kova doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a kova one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'kova config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``kova doctor``.
    ...
    # never the legitimate source for `kova doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'kova setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: kova skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: kova memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: kova memory setup",
                    "No Honcho API key — run 'kova memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run kova memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: kova memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: kova memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from kova_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "kova -p" in content:
                            _m = _re.search(r"kova -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'kova doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
