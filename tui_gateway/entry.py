...
# from shadowing kova's own top-level modules.  ``kova_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `KOVA_TUI_SIDECAR_URL`, set by the dashboard's
...
# tested; override via ``KOVA_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("KOVA_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``KOVA_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``kova --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``kova_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from kova_cli.mcp_startup import _resolve_discovery_timeout
...
def main():
    ...
    # server stalled the whole shell on "summoning kova…" for the full
    ...
    if _has_mcp_servers:
        def _discover_mcp_background() -> None:
            try:
                from kova_cli.mcp_startup import (
