# OpenAI-Compatible API Server for Kova Agent
...
SSE streaming. By exposing this endpoint, kova-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  kova-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kova/.env`
2. Run `Kova Gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kova-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kova-agent as a model) |
...
Authorization: Bearer kova-api-key-here
...
But kova-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kova-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kova-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kova-agent's core prompt
...
Once implemented, kova-agent works as a drop-in backend for:
