...
```bash
kova              # انٹرایکٹو CLI — بات چیت شروع کریں
kova model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
kova tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
kova config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
Kova Gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
kova setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
kova claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
kova update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
kova doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
ہرمیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `kova` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `kova`                                      | `Kova Gateway setup` اور `Kova Gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`kova setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
kova claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
kova claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
kova claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
kova claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
تمام آپشنز دیکھنے کے لیے `kova claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
...
cd kova-agent
./setup-kova.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/kova کا سیم لنک (symlink) بناتا ہے
./kova              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/chiragborse1/Kova-Agent/issues)
...
- 🔌 [KovaClaw](https://github.com/AaronWong1999/KovaClaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرمیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
