...
Registry and ABC for every inference provider kova knows about.
...
`$KOVA_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `KOVA_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `KOVA_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `KOVA_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `KOVA_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `KOVA_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$KOVA_HOME/plugins/model-providers/` for a private plugin).
