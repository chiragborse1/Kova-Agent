...
KOVA_HOME_DIR="${KOVA_HOME:-$HOME/.kova}"
KOVA_CFG="${KOVA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure kova config has twozero_td MCP entry ──
if [[ ! -f "$KOVA_CFG" ]]; then
    echo -e " ${FAIL} kova config not found at ${KOVA_CFG}"
    manual_steps+=("Create ${KOVA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$KOVA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in kova config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to kova config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${KOVA_CFG} manually"); }
    manual_steps+=("Restart kova session to pick up config change")
fi
