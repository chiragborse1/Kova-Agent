...
All management is via the `kova webhook` CLI command:

### Create a subscription
```bash
kova webhook subscribe <name> \
...
```bash
kova webhook remove <name>
```

### Test a subscription
```bash
kova webhook test <name>
kova webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
kova webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
kova webhook subscribe ci-builds \
  --events "pipeline" \
...
- Subscriptions persist to `~/.kova/webhook_subscriptions.json`

## How It Works

1. `kova webhook subscribe` writes to `~/.kova/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.kova/logs/gateway.log | tail -20`
...
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `kova webhook test <name>` to verify the route works.
