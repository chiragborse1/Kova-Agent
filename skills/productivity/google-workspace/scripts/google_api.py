#!/usr/bin/env python3
"""Google Workspace API CLI for Kova Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing kova-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("KOVA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Kova Agent")
