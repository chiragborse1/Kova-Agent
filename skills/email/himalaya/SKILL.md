...
This skill is separate from the kova Email gateway adapter. The gateway
adapter lets people email the agent and uses kova' built-in IMAP/SMTP
...
To reply non-interactively from kova, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from kova)** — pipe the message via stdin:
