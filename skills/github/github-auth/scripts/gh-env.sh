#!/usr/bin/env bash
# GitHub environment detection helper for Kova Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _KOVA_env="${KOVA_HOME:-$HOME/.kova}/.env"; [ -f "$_KOVA_env" ] && grep -q "^GITHUB_TOKEN=" "$_KOVA_env" 2>/dev/null; then
