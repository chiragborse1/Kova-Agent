...
Chronos lets a hosted Kova Gateway **scale to zero** while idle and still
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nous Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `kova-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> minted under the bootstrap-only client `kova-cli-vps` and seeded into the
...
**dashboard app** (`KOVA_cli/web_server.py`) — the agent's always-reachable
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `KOVA_HOME` — exactly one replica runs each
  fire.
