...
kova observer hooks are the read-only telemetry contract for plugins that
...
They do not replace kova' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. kova catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so kova only builds
