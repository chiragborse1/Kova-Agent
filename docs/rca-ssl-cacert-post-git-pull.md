# RCA: SSL CA cert bundle corruption after `kova update`
...
When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before kova has enough context to produce a useful message.
...
`KOVA_cli doctor` exposes the same check under `SSL / CA Certificates`, so users can diagnose the problem without starting a model session.
...
For a normal corrupted kova venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if kova should use the bundled `certifi` store.

## Environment escape hatch

Set `KOVA_SKIP_SSL_GUARD=1` to bypass the preflight check. This is intended only for sandboxed or managed-trust environments where the Python CA path looks unusual but downstream clients are known to work.
