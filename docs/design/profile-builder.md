...
Supersedes: PR #31781 (prompt_toolkit `kova profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `kova profile wizard` in the terminal.
...
needs, and a profile is just a KOVA_HOME directory — so the dashboard is the
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — KOVA_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with KOVA_HOME override |
...
### Seam #1 — hub-skill install cannot use the KOVA_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = KOVA_HOME / "skills"` at **module
import time**. The context-local `set_KOVA_HOME_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_KOVA_action`
runs `python -m KOVA_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_KOVA_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's KOVA_HOME bound
...
spawned detached (`_spawn_KOVA_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, KOVA_HOME override)
3. write selected MCP servers (synchronous, KOVA_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `kova -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, KOVA_HOME override
...
page afterward. Mirror `_write_profile_model`'s KOVA_HOME-override helper for
...
- Backend E2E with isolated KOVA_HOME: POST a full create body
