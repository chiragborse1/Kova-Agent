# Política de Seguridad de Kova Agent

Este documento describe el modelo de confianza de Kova Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/chiragborse1/Kova-Agent/security/advisories/new)
...
- Detalles del entorno (`kova version`, SHA del commit, SO, versión de Python).
...
Kova Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Kova Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Kova Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Kova Agent
...
Kova Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Kova Agent admite esto de dos maneras:

- **La propia imagen Docker de Kova Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Kova Agent
...
Kova Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Kova Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Kova Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Kova Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Kova Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Kova Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de kova-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Kova Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Kova Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Kova Agent que impidan al
...
- Kova Agent incluye guardias de cadena de suministro para lanzamientos de servidores
