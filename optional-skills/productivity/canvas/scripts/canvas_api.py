#!/usr/bin/env python3
"""Canvas LMS API CLI for Kova Agent.
...
def _check_config():
    ...
    if missing:
        KOVA_env = os.path.join(
            os.environ.get("KOVA_HOME", os.path.expanduser("~/.kova")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {KOVA_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Kova Agent"
    )
