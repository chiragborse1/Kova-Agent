...
$KOVA_OSINT_CACHE/icij/ (default: ~/.cache/kova-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("KOVA_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "kova-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "kova-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $KOVA_OSINT_CACHE/icij or ~/.cache/kova-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
