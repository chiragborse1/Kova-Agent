...
This is an optional third-party workflow, not a kova core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with kova native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer kova native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer kova native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended kova usage patterns
...
- Do not silently choose Parallel when kova native tools are already sufficient.
