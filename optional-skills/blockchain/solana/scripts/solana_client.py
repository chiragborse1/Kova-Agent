#!/usr/bin/env python3
"""
Solana Blockchain CLI Tool for Kova Agent
--------------------------------------------
...
def _http_get_json(url: str, timeout: int = 10, retries: int = 2) -> Any:
    ...
    for attempt in range(retries + 1):
        req = urllib.request.Request(
            url, headers={"Accept": "application/json", "User-Agent": "KovaAgent/1.0"},
        )
        try:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="Solana blockchain query tool for Kova Agent",
    )
