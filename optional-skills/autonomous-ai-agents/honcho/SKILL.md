---
name: honcho
description: Configure and use Honcho memory with kova -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Kova Agent
license: MIT
...
Config file: `$KOVA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kova honcho setup`. Ensure `memory.provider: honcho` is in `~/.kova/config.yaml`.
