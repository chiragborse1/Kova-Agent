...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (kova uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — kova has `image_generate`
...
- License: MIT (inherits from kova-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Kova Agent contributors
