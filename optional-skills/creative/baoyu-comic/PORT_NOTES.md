...
|--------|----------|--------|
| Metadata namespace | `openclaw` | `kova` (with `tags` + `homepage`) |
| Trigger | Slash commands / CLI flags | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of kova infra |
...
| File operations | Generic instructions | kova file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the kova repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
