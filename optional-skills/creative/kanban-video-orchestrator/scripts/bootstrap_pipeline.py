...
../assets/, and writes a setup.sh that creates kova profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `kova kanban create --workspace dir:` initial-task pattern are
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match kova's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per kova profile rules"
                        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'kova profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.kova/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
