...
Maps each role archetype to the kova skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public kova-agent
...
## kova skills relevant to video production

### Visual / rendering skills (`kova-agent/skills/creative/`)
...
### Design / pre-production skills (`kova-agent/skills/creative/`)
...
### Audio / media skills (`kova-agent/skills/creative/` + `skills/media/`)
...
These are **not** kova skills but external CLIs / APIs that profiles invoke.
...
## Built-in kova tools for media review

These are native kova tools — not invoked via terminal but through their own
...
key is present in `${KOVA_HOME:-~/.kova}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every kova profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${KOVA_HOME:-~/.kova}/.env` → environment variable.
...
When a new kova-public video skill ships:
