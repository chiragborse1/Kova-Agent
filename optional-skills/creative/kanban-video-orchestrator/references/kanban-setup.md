...
the actual `setup.sh` that creates the project workspace, configures kova
...
2. **Create profiles** — `kova profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.kova/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `kova kanban create` assigned to the director
...
```bash
kova profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.kova/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.kova/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most kova
...
Each profile gets a `SOUL.md` at `~/.kova/profiles/<name>/SOUL.md` that
...
```bash
kova kanban create "Direct production of <video title>" \
    --assignee director \
...
the kova `.env` (`${KOVA_HOME:-$HOME/.kova}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _KOVA_env="${KOVA_HOME:-$HOME/.kova}/.env"
...
check_key ELEVENLABS_API_KEY kova ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY kova OPENROUTER_API_KEY || exit 1
# ...
```
