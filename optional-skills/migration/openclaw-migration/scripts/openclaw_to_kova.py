#!/usr/bin/env python3
"""OpenClaw -> kova migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Kova Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into kova.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into kova memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into kova memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import kova-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of kova-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into kova command_allowlist.",
    },
    "skills": {
        "label": "User skills",
    ...
    "discord-settings": {
        ...
        "description": "Import Discord bot token and allowlist into kova .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into kova .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into kova .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into kova .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into kova .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into kova config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into kova config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into kova.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into kova memory.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into kova config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'kova cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into kova config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into kova session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into kova custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into kova config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into kova config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into kova config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update kova config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with kova in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``kova`` (prose), but lowercase matches
# like ``openclaw`` → ``kova`` (so filesystem paths like ``~/.openclaw``
# become ``~/.kova`` — the real kova home — not the broken ``~/.kova``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'kova'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'kova'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'kova'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``kova`` but maps ``openclaw`` → ``kova`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.kova/config.yaml`` (the real kova home) instead of the broken
    ``~/.kova/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with kova.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> kova Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-kova/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the kova config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-kova/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new kova session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "kova config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the kova
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No kova-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted kova-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: kova config set",
                        )
                    continue
        ...
        for oc_key, KOVA_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and KOVA_key not in secret_additions:
                secret_additions[KOVA_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, KOVA_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and KOVA_key not in secret_additions:
                        secret_additions[KOVA_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        KOVA_config = load_yaml_file(destination)
        current_model = KOVA_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = KOVA_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                KOVA_config["model"] = {"default": model_str}
            dump_yaml_file(destination, KOVA_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; kova still uses "edge"
        ...
        KOVA_config = load_yaml_file(destination)
        existing_tts = KOVA_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct kova destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected kova-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected kova-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        existing_mcp = KOVA_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in kova config")
                continue

            KOVA_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                KOVA_srv["command"] = srv["command"]
                if srv.get("args"):
                    KOVA_srv["args"] = srv["args"]
                if srv.get("env"):
                    KOVA_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    KOVA_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                KOVA_srv["url"] = srv["url"]
                if srv.get("headers"):
                    KOVA_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    KOVA_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                KOVA_srv["enabled"] = False
            if srv.get("timeout"):
                KOVA_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                KOVA_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                KOVA_srv["tools"] = {}
                if tools_cfg.get("include"):
                    KOVA_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    KOVA_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            ...
            KOVA_cfg["mcp_servers"] = existing_mcp
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'kova cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = KOVA_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> kova reasoning_effort
        ...
        if compaction:
            compression = KOVA_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'Kova Gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("KOVA_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        KOVA_cfg_path = self.target_root / "config.yaml"
        KOVA_cfg = load_yaml_file(KOVA_cfg_path)
        sr = KOVA_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        KOVA_cfg_path = self.target_root / "config.yaml"
        KOVA_cfg = load_yaml_file(KOVA_cfg_path)
        custom_providers = KOVA_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(KOVA_cfg_path)
            KOVA_cfg["custom_providers"] = custom_providers
            dump_yaml_file(KOVA_cfg_path, KOVA_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to kova config
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            KOVA_mode = mode_map.get(mode, "manual")
            KOVA_cfg.setdefault("approvals", {})["mode"] = KOVA_mode
            if self.execute:
                self.maybe_backup(KOVA_cfg_path)
                dump_yaml_file(KOVA_cfg_path, KOVA_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{KOVA_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in kova. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing kova config Not Overwritten)",
                "",
                "These items already existed in your kova config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Kova Agent discovers these",
            "directories, it may read/write to them instead of the kova state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `kova claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `kova claw cleanup` to fix it.",
            "",
            "## kova-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `kova claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `kova setup` to configure any remaining settings",
            "- Run `kova mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `kova cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `kova cron` to recreate scheduled tasks (see archived cron-store)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing kova targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of kova-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> kova Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
