...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("KOVA_HOME", os.path.expanduser("~/.kova")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KOVA_HOME", Path.home() / ".kova")) / "skills" / "red-teaming" / "godmode"
...
PREFILL_PATH = KOVA_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from kova config.yaml.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart kova for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart kova for changes to take effect.")
