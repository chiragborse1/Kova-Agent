...
Usage (via cron with --no-agent):
    ...
    kova cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$KOVA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "kova-Watcher/1.0"})
