...
as ``kind: backend``) or ``~/.kova/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``kova tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$KOVA_HOME/cache/videos/``, creating parents as needed."""
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$KOVA_HOME/cache/videos/``.
