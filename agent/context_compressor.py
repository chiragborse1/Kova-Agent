...
# heuristics. See https://github.com/chiragborse1/Kova-Agent/issues/38389
#
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the kova compressor emits).
    """
    if not messages:
        return messages
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so kova compresses before a
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # user's configured timezone via kova_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _KOVA_now().strftime("%Y-%m-%d")
