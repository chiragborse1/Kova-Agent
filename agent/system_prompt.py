...
upstream prefix cache warm.  See ``kova-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  kova never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want KOVA_HOME persona while keeping
    ...
    # Pointer to the kova-agent skill + docs for user questions about kova itself.
    stable_parts.append(KOVA_AGENT_HELP_GUIDANCE)
    ...
    # KOVA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base kova, any interactive coding surface in a code
    ...
    # Active-profile hint — names the kova profile the agent is running
    # under so it doesn't conflate ~/.kova/skills/ (default profile) with
    # ~/.kova/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.kova/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.kova/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.kova/skills/, ~/.kova/plugins/, "
            f"~/.kova/cron/, ~/.kova/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — kova never rebuilds or reinjects parts of it
