...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the kova
     skill-authoring standards (description <=60 chars, the modern section
     order, kova-tool framing, no invented commands).
...
# (HARDLINE)" and the kova-agent-dev new-skill salvage reference. Embedded in
...
- author: always the literal value `kova`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as kova.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.kova.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through kova tools.
...
kova-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference kova tools by name in backticks: `terminal`, `read_file`,
