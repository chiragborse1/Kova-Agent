"""Lightweight internationalization (i18n) for kova static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by kova itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``KOVA_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``KOVA_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a kova dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`kova_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("KOVA_LANGUAGE")
    if env_lang:
