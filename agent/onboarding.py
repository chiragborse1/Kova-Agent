...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time kova starts and finds ``~/.openclaw/``.

    Points users at ``kova claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kova claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to kova, run "
        "`kova claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `kova claw cleanup` (renames it to ~/.openclaw.pre-migration — "
