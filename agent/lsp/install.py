...
appropriate.  All installs go to a kova-owned bin staging dir,
``<KOVA_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``kova lsp
  status``.
...
# ``<KOVA_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = KOVA_lsp_bin_dir().parent  # <KOVA_HOME>/lsp/
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``kova lsp status`` CLI to give users a quick
