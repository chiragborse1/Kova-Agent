"""Coding-context awareness — base kova, every interactive surface.

When the user runs kova inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), kova shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("kova.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "kova", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _KOVA_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``KOVA_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether kova should operate in its coding posture right now."""
