...
Plugins built on Kova Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing kova
...
:class:`~kova_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors kova' main config
...
handles every provider, fallback chain, and per-task override kova
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`kova_cli.plugins.PluginContext`
