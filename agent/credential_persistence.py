...
``auth.json``.  They intentionally have no dependency on ``kova_cli.auth`` so
...
# Sources kova owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and kova-owned OAuth/device-code state)
