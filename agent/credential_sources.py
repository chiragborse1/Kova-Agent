"""Unified removal contract for every credential source kova reads from.

kova seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.kova/.env
    claude_code   — ~/.claude/.credentials.json
    KOVA_pkce   — ~/.kova/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``kova auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), KOVA_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.kova/.env  → clear it
    ...
    """
    from kova_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to kova.",
            f"  The pool entry is now suppressed — kova will ignore "
            f"{env_var} until you run `kova auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so kova stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `kova auth add anthropic` to re-enable if needed.",
    ])


def _remove_KOVA_pkce(provider: str, removed) -> RemovalResult:
    """~/.kova/.anthropic_oauth.json is ours — delete it outright."""
    from kova_constants import get_kova_home

    result = RemovalResult()
    oauth_file = get_kova_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared kova Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `kova auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `kova auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``kova auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``kova auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `kova model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the kova auth store is not enough — _seed_from_singletons() would
    ...
    via ``kova auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `kova auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `kova auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    kova stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `kova auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="KOVA_pkce",
        remove_fn=_remove_KOVA_pkce,
        description="~/.kova/.anthropic_oauth.json",
    ))
    register(RemovalStep(
