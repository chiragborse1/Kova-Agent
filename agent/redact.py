...
# `export KOVA_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in kova_cli/main.py, gateway/run.py, and
# cli.py) or `KOVA_REDACT_SECRETS=false` in ~/.kova/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("KOVA_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across kova — used by
    ``kova config``, ``kova status``, ``kova dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical kova log line
