...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. kova also honours KOVA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("KOVA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_kova_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Kova Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent kova from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for kova' system prompt, but mapping
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # kova grow conversations past the runtime limit and Ollama
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets kova sends to providers:
