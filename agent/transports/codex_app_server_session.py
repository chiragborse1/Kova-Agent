...
Owns one Codex thread per kova session. Drives `turn/start`, consumes
...
# kova' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_KOVA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse kova/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per kova session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _KOVA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("KOVA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="kova",
            client_title="Kova Agent",
            client_version=_get_KOVA_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but kova' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into kova' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own kova-tools callback we
            # auto-accept — the user already approved kova' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map kova approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    kova returns 'once', 'session', 'always', or 'deny'.
