"""Projects codex app-server events into kova' messages list.

The translator that lets kova' memory/skill review keep working under the
...
kova' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape kova
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way kova' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but kova stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way kova already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — kova' messages store text only.
        text_parts: list[str] = []
