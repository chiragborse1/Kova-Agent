"""Anthropic Messages API adapter for Kova Agent.

Translates between kova's internal OpenAI-style message format and
...
# kova effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    kova historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``kova models``) never mutates ``~/.kova/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by kova)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.kova/auth.json)
    ...
    # 1. kova-managed OAuth/setup token env var
    ...
    # 4. kova credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration kova configs.
...
# ── kova-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.kova/.anthropic_oauth.json (our own file).
...
_KOVA_OAUTH_FILE = get_kova_home() / ".anthropic_oauth.json"
...
def run_KOVA_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run kova-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize kova with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from kova_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_KOVA_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read kova-managed OAuth credentials from ~/.kova/.anthropic_oauth.json."""
    if _KOVA_OAUTH_FILE.exists():
        try:
            data = json.loads(_KOVA_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read kova OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See Kova-Agent#16748.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See kova-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See Kova-Agent#16748 (Kimi) and
    Kova-Agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        #      a) bare kova-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that kova surfaces in its CLI. We
