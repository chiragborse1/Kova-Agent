...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_KOVA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, kova expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``KOVA_HOME`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_kova_home()`` themselves.
        """
        if "KOVA_HOME" not in kwargs:
            from kova_constants import get_kova_home
            kwargs["KOVA_HOME"] = str(get_kova_home())
        for provider in self._providers:
            try:
