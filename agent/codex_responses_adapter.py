...
# output items.  kova injects xAI's native ``web_search`` for the xAI
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on kova threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a kova-side metadata key and not part
