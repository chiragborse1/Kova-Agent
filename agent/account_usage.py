...
from kova_cli.auth import _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from kova_cli.nous_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when KOVA_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so KOVA_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — KOVA_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
