...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.kova/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: kova ↔ models.dev
# ---------------------------------------------------------------------------
...
# Reverse mapping: models.dev → kova (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_kova_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``kova config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a kova provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Kova Agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a kova provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve kova ID → models.dev ID
...
def get_model_info(
    ...
    Accepts kova or models.dev provider ID.  Tries exact match then
