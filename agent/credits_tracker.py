...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``kova_cli.models``
    ...
    if not base_url:
        return False
    try:
        from kova_cli.models import _is_model_free, _pricing_cache
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update kova",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (KOVA_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set KOVA_DEV_CREDITS_FIXTURE to either a
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for KOVA_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag KOVA_DEV_CREDITS
    is also on, so a stray KOVA_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("KOVA_DEV_CREDITS")):
        return None
    raw = os.environ.get("KOVA_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
