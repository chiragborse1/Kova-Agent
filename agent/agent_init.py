...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .kova.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / KOVA_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.kova/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from kova_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind KOVA_DEV_CREDITS.
    ...
    # both live under ~/.kova/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that kova_logging.setup_logging() does not
        ...
        # handler level inside kova_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from kova_cli.config import load_config as _load_br_cfg
    ...
    # worker (kanban_show tool is present iff KOVA_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["KOVA_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.kova/sessions/ alongside gateway sessions
    KOVA_HOME = get_kova_home()
    agent.logs_dir = KOVA_HOME / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.kova/sessions/session_{sid}.json)
    ...
    try:
        from kova_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from kova_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "kova"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace kova' built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Kova Agent.  Choose a model with at least "
