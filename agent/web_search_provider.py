...
``kind: backend``) or ``~/.kova/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``kova tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``kova tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``kova_cli/tools_config.py`` to inject this provider as a
