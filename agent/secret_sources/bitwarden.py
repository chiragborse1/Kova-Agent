...
kova pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.kova/.env``.
...
* The access token is stored in ``~/.kova/.env`` as
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `kova chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.kova/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    `home_path` is what `load_KOVA_dotenv()` already resolved; falling back
    to `$KOVA_HOME` / `~/.kova` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("KOVA_HOME", Path.home() / ".kova"))
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<KOVA_HOME>/cache/bws_cache.json`` (for back-to-back CLI invocations).
