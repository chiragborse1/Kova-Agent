...
#
# When a user invokes a /skill (or /bundle), kova expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_KOVA_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``KOVA_PLATFORM`` env var and
    ``KOVA_SESSION_PLATFORM`` from the gateway session context. Returns
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.kova/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.kova.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_kova_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.kova/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.kova/skills/`` and any ``skills.external_dirs`` so the
