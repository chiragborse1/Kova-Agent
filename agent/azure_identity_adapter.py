...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. kova does
...
class EntraIdentityConfig:
    ...
    Captures the kova-managed Entra knobs we need outside Azure SDK
    ...
    ``kova_cli/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only kova-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.kova/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    kova processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    ``kova auth status`` / wizard preflight.
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``kova_cli/azure_detect.py``).
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `kova doctor` or `az login` to recover.",
                exc,
            )
