...
KOVA_AGENT_HELP_GUIDANCE = (
    ...
    "the documentation at https://kova-agent.nousresearch.com/docs is your "
...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $KOVA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $KOVA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${KOVA_KANBAN_BRANCH:-wt/$KOVA_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$KOVA_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`kova profile list`, or ask the user), "
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `kova computer-use doctor` and share the output. That command "
...
def build_environment_hints() -> str:
    ...
    # kova desktop GUI — any agent running under the desktop app should know
    # it. KOVA_DESKTOP marks the backend powering the chat; KOVA_DESKTOP_TERMINAL
    # marks a kova launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("KOVA_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("KOVA_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the kova desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def build_skills_system_prompt(
    ...
    _platform_hint = (
        os.environ.get("KOVA_PLATFORM")
        or get_session_env("KOVA_SESSION_PLATFORM")
        or ""
    )
...
def _load_KOVA_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not KOVA_md_path:
        return ""
    try:
        content = KOVA_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = KOVA_md_path.name
        try:
            rel = str(KOVA_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        ...
        logger.debug("Could not read %s: %s", KOVA_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    project_context = (
        _load_KOVA_md(cwd_path, context_length)
