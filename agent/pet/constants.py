...
row taxonomy is inferred from the atlas shape so kova can render both legacy
...
# rows (1536x1872), while older kova/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are kova' activity state names. They are not always identical to the
...
# Legacy kova/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical kova activity names -> accepted row-name aliases in descending
