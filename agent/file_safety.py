...
def _KOVA_HOME_path() -> Path:
    """Resolve the active KOVA_HOME (profile-aware) without circular imports."""
    try:
...
def _KOVA_root_path() -> Path:
    """Resolve the kova root dir (always the parent of any profile, never per-profile)."""
    try:
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    KOVA_HOME = _KOVA_HOME_path()
    KOVA_root = _KOVA_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(KOVA_HOME / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(KOVA_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved KOVA_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("KOVA_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    KOVA_dirs = []
    for base in (_KOVA_HOME_path(), _KOVA_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in KOVA_dirs:
                KOVA_dirs.append(real)
        except Exception:
            continue

    for base_real in KOVA_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal kova cache files under ``KOVA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under KOVA_HOME and the global kova
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Resolve BOTH the active KOVA_HOME (profile-aware) AND the global
    # kova root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (KOVA_HOME points at
    ...
    for hd in KOVA_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal kova cache file "
    ...
    for hd in KOVA_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a kova credential store "
    ...
    for hd in KOVA_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the kova mcp token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a kova mcp token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# kova profiles are separate KOVA_HOME dirs under
...
# Profile-scoped directories under KOVA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from KOVA_HOME.
    ...
    """
    try:
        home_real = _KOVA_HOME_path().resolve()
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    kova entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to kova profile {info['target_profile']!r}, but the "
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .kova / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".kova":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative kova state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.kova``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.kova`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any kova resolver to
    succeed, so it works correctly even when called from contexts where
    KOVA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host kova process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real KOVA_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host KOVA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative kova state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host kova "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real KOVA_HOME. Use the "
