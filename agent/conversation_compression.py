        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `kova setup` or set OPENROUTER_API_KEY."
                )
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``kova_state.SessionDB`` class still bound to the
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``kova_logging._session_context`` (set once per turn in
                ...
                try:
                    agent._session_db.create_session(
                        ...
                        source=agent.platform or os.environ.get("KOVA_SESSION_SOURCE", "cli"),
                        model=agent.model,
    ...
    # engines (e.g. kova-lcm) use boundary_reason="compression" to preserve
    # DAG lineage / checkpoint per-session state across the boundary instead of
    # re-initializing fresh. See kova-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="KOVA_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
