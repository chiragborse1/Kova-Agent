...
# Matches ${KOVA_SKILL_DIR} / ${KOVA_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${KOVA_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(KOVA_SKILL_DIR|KOVA_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${KOVA_SKILL_DIR} / ${KOVA_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "KOVA_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "KOVA_SESSION_ID" and session_id:
