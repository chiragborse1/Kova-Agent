...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into kova with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.kova/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of kova
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``kova setup`` for
