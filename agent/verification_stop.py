...
def _session_is_messaging_surface() -> bool:
    ...
    ``KOVA_SESSION_PLATFORM``; the CLI and TUI set ``KOVA_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``KOVA_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("KOVA_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``KOVA_VERIFY_ON_STOP`` env var wins, then an
    ...
    falls back to OFF.
    """
    env = os.environ.get("KOVA_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `kova-verify-` filename prefix, run it "
