...
from kova_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``KOVA_KANBAN_TASK`` / ``KOVA_KANBAN_BOARD`` in the
        ...
        # The kova Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``kova_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``KOVA_PLATFORM`` or
            ``KOVA_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``kova`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    parsing a non-trivial config dominates ``kova`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against KOVA_HOME, not cwd
        if not p.is_absolute():
            p = (KOVA_HOME / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: kova can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(kova, dict):
        kova = {}
    return {
        "fallback_for_toolsets": kova.get("fallback_for_toolsets", []),
        "requires_toolsets": kova.get("requires_toolsets", []),
        "fallback_for_tools": kova.get("fallback_for_tools", []),
        "requires_tools": kova.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes kova metadata, VCS, virtualenv/dependency, cache, and skill
