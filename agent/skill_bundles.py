...
profile-aware directory under ``KOVA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``kova bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KOVA_HOME.

    Honors ``KOVA_BUNDLES_DIR`` for tests; falls back to
    ``<KOVA_HOME>/skill-bundles``.
    """
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `kova bundles` CLI subcommand.
# ---------------------------------------------------------------------------
