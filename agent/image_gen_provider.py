...
as ``kind: backend``) or ``~/.kova/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``kova tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``kova tools`` model picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``kova tools``
...
def _images_cache_dir() -> Path:
    """Return ``$KOVA_HOME/cache/images/``, creating parents as needed."""
    from kova_constants import get_kova_home

    path = get_kova_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$KOVA_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$KOVA_HOME/cache/images/``.
