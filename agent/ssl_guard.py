"""Preventive SSL CA certificate checks for Kova Agent.
...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("KOVA_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via KOVA_SKIP_SSL_GUARD")
        return
