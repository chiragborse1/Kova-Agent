...
Every Kova request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Kova Agent and bucket it by client release.
...
The version is sourced live from ``kova_cli.__version__`` so it auto-aligns
...
``kova_cli.__version__`` is the canonical source of truth.
"""
...
def _kova_version() -> str:
    """Return the current Kova release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``kova_cli`` cannot be imported (should
...
def kova_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=kova-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=kova-client-v{_kova_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=kova-agent", kova_client_tag()]
