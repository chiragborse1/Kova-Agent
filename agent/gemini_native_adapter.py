...
kova keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for kova's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with kova)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). kova typically makes 3-10 API calls per user turn, "
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (kova
...
class GeminiNativeClient:
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "kova-agent (gemini-native)",
        }
