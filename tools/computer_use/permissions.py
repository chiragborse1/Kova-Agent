...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT kova — so no kova entitlement is
...
``kova computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def _child_env() -> Dict[str, str]:
    """cua-driver child env honoring the kova telemetry opt-in policy."""
    try:
