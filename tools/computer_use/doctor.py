"""
`kova computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the kova side — the only
...
def run_doctor(
    ...
    *,
    ...
    Honors `KOVA_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
    ...
    if driver_cmd is None:
        try:
            ...
            driver_cmd = os.environ.get("KOVA_CUA_DRIVER_CMD") or "cua-driver"
