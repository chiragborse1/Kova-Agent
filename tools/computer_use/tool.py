...
`health_report` (surfaced by `kova computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session kova runs in).
_BLOCKED_KEY_COMBOS = {
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of chiragborse1/Kova-Agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$KOVA_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_kova_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `kova computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
