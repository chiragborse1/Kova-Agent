...
etc.) surface as specific blocked checks via `kova computer-use doctor`
...
# fetches the latest release, so a `KOVA_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `KOVA_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("KOVA_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when kova should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of chiragborse1/Kova-Agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or KOVA_CUA_DRIVER_CMD resolves."""
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of chiragborse1/Kova-Agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of chiragborse1/Kova-Agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize kova-managed secrets out of the child env.
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    chiragborse1/Kova-Agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of chiragborse1/Kova-Agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per kova run, with
        ...
        #     concurrent kova runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one kova run = one backend = one session — and pass it as
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps kova `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of chiragborse1/Kova-Agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of chiragborse1/Kova-Agent#47072: prefer the
    ...
    def click(
        ...
        *,
        ...
        # chiragborse1/Kova-Agent#47072). cua-driver-rs gained an explicit
