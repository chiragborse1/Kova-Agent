...
(see ``kova_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<KOVA_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_kova_home() / "pending" / subsystem
