...
it is automatically downloaded from GitHub releases to $KOVA_HOME/bin/tirith.
...
def _get_kova_home() -> str:
    """Return the kova home directory, respecting KOVA_HOME env var."""
    return str(get_kova_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_kova_home(), ".tirith-install-failed")
...
def _KOVA_bin_dir() -> str:
    """Return $KOVA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_kova_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KOVA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_KOVA_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KOVA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $KOVA_HOME/bin/tirith
    ...
    KOVA_bin = os.path.join(_KOVA_bin_dir(), "tirith")
    if os.path.isfile(KOVA_bin) and os.access(KOVA_bin, os.X_OK):
        _resolved_path = KOVA_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return KOVA_bin
