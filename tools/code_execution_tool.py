...
Lets the LLM write a Python script that calls kova tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `kova_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `kova_tools.py` with file-based RPC stubs
...
# available on every platform kova itself runs on.
...
# match a safe prefix, the operational KOVA_ allowlist, or (on Windows) an
# OS-essential name.
#
...
# kova_KANBAN_DB, KOVA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _KOVA_CHILD_ALLOWED below; kova_RPC_SOCKET /
# kova_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational KOVA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set kova_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational KOVA_* vars (_KOVA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret KOVA_* vars dropped by the tightened allowlist (#27303). The
    # broad "KOVA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like kova_KANBAN_DB / kova_BASE_URL), but a sandbox script
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-kova-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-kova-profile soft guard."""',
...
def generate_KOVA_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the kova_tools.py stub module.
...
"""Auto-generated kova tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["kova_RPC_SOCKET"]
...
"""Auto-generated kova tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("kova_RPC_DIR") or os.path.join(tempfile.gettempdir(), "kova_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated kova_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/kova_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_KOVA_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/kova_tools.py", tools_src)
        ...
        env_prefix = (
            f"kova_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("KOVA_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with kova_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="kova_sandbox_")
    ...
    # selector from kova_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"kova_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated kova_tools module.
        ...
        tools_src = generate_KOVA_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "kova_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  kova_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["kova_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the kova-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from kova_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _KOVA_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _KOVA_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # KOVA_TIMEZONE is an internal kova setting and must not leak
        # into child processes.
        _tz_name = os.getenv("KOVA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("KOVA_TIMEZONE", None)

        from kova_constants import apply_subprocess_home_env
...
def _load_config() -> dict:
    ...
    ``kova --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (kova-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``kova tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and kova-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call kova tools programmatically. "
        ...
        f"Available via `from kova_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into kova_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from kova_tools import {import_str}` "
