...
-----------------------------------------------------------------------------
    ...
    ~/.kova/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/kova/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_kova_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/kova"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (kova convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("KOVA_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal kova infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``kova checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `kova checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "kova@local"], store, cfg_wd)
    _run_git(["config", "user.name", "kova Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``KOVA_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at KOVA_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "KOVA_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `kova checkpoints` CLI
# ---------------------------------------------------------------------------
