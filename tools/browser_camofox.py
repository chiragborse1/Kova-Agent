...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.kova/.env``.
...
def _managed_persistence_enabled() -> bool:
    """Return whether kova-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so kova operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether kova should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for kova talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside kova. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the kova profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"KOVA_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
