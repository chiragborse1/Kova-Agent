"""
Lazy dependency installer for opt-in Kova Agent backends.

Many kova features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``kova-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``kova tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``KOVA_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/kova`` read-only), setting
  ``KOVA_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick kova, which is what made it safe to seal
...
_LAZY_TARGET_ENV = "KOVA_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``KOVA_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("KOVA_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``kova update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="kova-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``kova_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``kova update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``kova update``. Never raises; lazy-install failures
