...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("KOVA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("KOVA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("KOVA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set KOVA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind KOVA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds KOVA_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("KOVA_CRON_SESSION"):
        return False
    if env_var_enabled("KOVA_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KOVA_HOME, or by the resolved absolute
# active profile home path such as /home/kova/.kova/config.yaml. The
# resolved-absolute form is folded into the ~/.kova/ patterns at detection
...
# go stale when KOVA_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_KOVA_ENV_PATH = (
    r'(?:~\/\.kova/|'
    r'(?:\$home|\$\{home\})/\.kova/|'
    r'(?:\$KOVA_HOME|\$\{KOVA_HOME\})/)'
    r'\.env\b'
)
# ~/.kova/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _KOVA_ENV_PATH; matches the KOVA_HOME override form as
# well as ~/.kova/.
_KOVA_CONFIG_PATH = (
    r'(?:~\/\.kova/|'
    r'(?:\$home|\$\{home\})/\.kova/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\.kova\s+gateway\s+(stop|restart)\b', "stop/restart Kova Gateway (kills running agents)"),
    (r'\.kova\s+update\b', "kova update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `Kova Gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart kova-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart kova-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(kova|gateway|cli\.py)\b', "kill kova/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill kova` but not
    # `kill -9 $(pgrep -f kova)` because the substitution is opaque
    ...
    # the `Kova Gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.kova.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(kova|ai\.kova)\b', "stop/restart kova launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/kova file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.kova/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a kova-managed security file (~/.kova/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_KOVA_CONFIG_PATH}|{_KOVA_ENV_PATH})', "in-place edit of kova config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_KOVA_CONFIG_PATH}|{_KOVA_ENV_PATH})', "in-place edit of kova config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_KOVA_CONFIG_PATH}|{_KOVA_ENV_PATH})', "in-place edit of kova config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.kova/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / KOVA_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) kova home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\kova), so folding the user home
    # first would eat the prefix the kova-home fold needs.
    command = _rewrite_resolved_KOVA_HOME(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / KOVA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.kova``); the matched
...
def _rewrite_resolved_KOVA_HOME(command: str) -> str:
    """Rewrite the resolved absolute kova home prefix to ``~/.kova/``.

    Resolves the active ``KOVA_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.kova/`` so the static ``_KOVA_CONFIG_PATH`` / ``_KOVA_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/kova/.kova/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$KOVA_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from kova_constants import get_kova_home
        home = get_kova_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.kova")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        ...
        if "KOVA_SPINNER_PAUSE" in os.environ:
            del os.environ["KOVA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("KOVA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set KOVA_INTERACTIVE or KOVA_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("KOVA_EXEC_ASK"):
