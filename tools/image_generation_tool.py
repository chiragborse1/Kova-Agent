...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.kova"
    ...
    # kova cache roots can be translated without side effects. SSH can still
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping kova with a FAL key configured
