...
LOCAL_STT_COMMAND_ENV = "kova_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "kova_LOCAL_STT_LANGUAGE"
...
#
# The single-env-var ``kova_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__KOVA_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"kova-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set kova_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `kova model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("kova_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
