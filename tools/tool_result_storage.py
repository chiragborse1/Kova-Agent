...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/kova-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/kova-results/{tool_use_id}.txt on Termux)
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/kova-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"KOVA_PERSIST_{uuid.uuid4().hex[:8]}"
