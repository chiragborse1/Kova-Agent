...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__KOVA_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__KOVA_FENCE_" in line or "\x1b]" in line
...
class ShellFileOperations(FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.kova-tmp`` file next to the user's data, and the original file
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.kova-tmp``
