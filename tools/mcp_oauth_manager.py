...
`mcp_tool.py`, and `kova_cli/mcp_config.py`. This module is the ONLY place
...
def _make_KOVA_provider_class() -> Optional[type]:
    ...
    class KovaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._KOVA_server_name = server_name
            ...
            self._KOVA_preregistered = preregistered
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._KOVA_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._KOVA_server_name, asm.token_endpoint,
                        )
                        break
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``kova mcp reauth``.
            ...
            back to ``kova mcp reauth``.
            """
            try:
                if self._KOVA_preregistered:
                    return
...
_KOVA_PROVIDER_CLS: Optional[type] = _make_KOVA_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `kova mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``kova mcp remove <name>`` and (indirectly) by
        ``kova mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
