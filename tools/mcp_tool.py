...
Configuration is read from ~/.kova/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.kova/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            KOVA_HOME = os.path.expanduser(
                os.getenv(
                    "KOVA_HOME", os.path.join(os.path.expanduser("~"), ".kova")
                )
            )
            candidates = [
                ...
                # image (which the kova Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → kova MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Kova Gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through kova' existing approval
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'kova-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'kova-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through kova' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until kova is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `kova mcp login {server_name}` (or delete the tokens "
            f"file under ~/.kova/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local KOVA_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local KOVA_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the kova config file.
    ...
    ``os.environ`` (which includes ``~/.kova/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through kova' MEDIA: tag convention and out to messaging
            ...
            # kova' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves kova's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the kova registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``kova tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the kova registry.
