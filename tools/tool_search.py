"""Progressive tool disclosure ("tool search") for Kova Agent.
...
``tool_call`` — and surfaced on demand. Core kova tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._KOVA_CORE_TOOLS`` are *never* deferred.
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_KOVA_CORE_TOOLS``. Core tools are never deferred
