"""Central registry for all kova-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``kova tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``kova tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``kova tools enable foo``)
