...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or kova exit
...
# display_kova_home imported lazily at call site (stale-module safety during kova update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of kova directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "kova-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("KOVA_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("KOVA_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("KOVA_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (KOVA_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (KOVA_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive kova password prompt or the sudo -S password-pipe path.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so kova can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``kova-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by kova processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      kova processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling kova processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, KovaSweEnv, ...)
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for kova-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior kova processes that hit SIGKILL / OOM / a closed terminal
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/kova
        # restart|stop targeting kova-gateway) must never run inside the
        ...
        # never restart. This mirrors the `Kova Gateway restart` guard in
        # kova_cli/gateway.py and the cron-path guard in kova_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_KOVA_GATEWAY") == "1":
            from kova_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `Kova Gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # kova-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in kova-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("KOVA_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("KOVA_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("KOVA_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("KOVA_SESSION_USER_ID", "")
                            _gw_user_name = _gse("KOVA_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("KOVA_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("KOVA_INTERACTIVE"):
                    output += (
