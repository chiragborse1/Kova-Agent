...
# Must match kova_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (kova-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``kova-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. kova-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.kova/logs/subagent-<sid>-<ts>.log``
...
def _load_config() -> dict:
    ...
    to the persistent config (kova_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (kova subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
