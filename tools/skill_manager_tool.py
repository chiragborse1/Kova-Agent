...
~/.kova/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `kova config set skills.guard_agent_created true`.
    """
    try:
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`kova curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`kova curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.kova/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER kova profiles.
    ...
    # Default profile (~/.kova/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.kova/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`kova -p "
            ...
            base += (
                ...
                f"Switch profiles (`kova -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_kova_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `kova curator unpin <name>`. "
