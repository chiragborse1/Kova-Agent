...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``kova_constants.get_subprocess_home()`` so that ``~`` resolves
...
_KOVA_config_resolved: str | None = None
_KOVA_config_resolved_loaded = False


def _get_KOVA_config_resolved() -> str | None:
    """Return the resolved absolute path of the kova config file (cached)."""
    global _KOVA_config_resolved, _KOVA_config_resolved_loaded
    if _KOVA_config_resolved_loaded:
        return _KOVA_config_resolved
    _KOVA_config_resolved_loaded = True
    try:
        from kova_cli.config import get_config_path
        _KOVA_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _KOVA_config_resolved = str(Path(_expand_tilde("~/.kova/config.yaml")).resolve())
        except Exception:
            _KOVA_config_resolved = None
    return _KOVA_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the kova config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    KOVA_config = _get_KOVA_config_resolved()
    if KOVA_config and (resolved == KOVA_config or normalized == KOVA_config):
        return (
            f"Refusing to write to kova config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.kova/config.yaml directly or use 'kova config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side kova mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another kova
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of kova state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.kova/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      kova process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.kova/…`` path.

    Returns ``None`` when the write is in-scope or outside kova scope.
    ...
    # in a session that cd'd into ``~/.kova/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── kova internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under KOVA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == KOVA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-kova-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-kova-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another kova profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another kova profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with KOVA_tools.write_file() for very "
            "large files."
        )
