...
* ``XAI_API_KEY`` is set in ``~/.kova/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``kova auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
from tools.xai_http import kova_xai_user_agent, resolve_xai_http_credentials
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `kova auth add xai-oauth` "
...
def check_x_search_requirements() -> bool:
    ...
    :func:`kova_cli.auth.resolve_xai_oauth_runtime_credentials` which
