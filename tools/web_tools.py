...
When available, kova can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    ...
    through kova' config/.env layer (``kova config set``, ``kova tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.kova/config.yaml."""
    try:
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `kova tools` repaint.
        try:
