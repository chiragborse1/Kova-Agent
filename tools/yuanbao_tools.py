...
提供以下工具函数，供 kova-agent 的 "kova-yuanbao" toolset 使用：
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（KOVA_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("KOVA_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
def _check_yuanbao():
    ...
    try:
        from gateway.session_context import get_session_env
        if get_session_env("KOVA_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
