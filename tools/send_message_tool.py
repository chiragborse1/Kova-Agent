    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.kova/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.kova/config.yaml or environment variables.")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("KOVA_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("KOVA_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("KOVA_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from kova_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
    ...
    if platform == Platform.WHATSAPP and media_files:
        ...
        from kova_cli.plugins import discover_plugins as _dp_wa
        _dp_wa()
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``KOVA_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``KOVA_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``kova send`` CLI command (kova_cli/send_cmd.py)
