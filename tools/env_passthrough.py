...
def _is_KOVA_provider_credential(name: str) -> bool:
    """True if ``name`` is a kova-managed provider credential (API key,
    token, or similar) per ``_KOVA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-kova API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _KOVA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are kova-managed provider credentials (from
    ``_KOVA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a kova-managed
    ...
    Non-kova third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_KOVA_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register kova provider "
                "credential %r (blocked by _KOVA_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # kova-managed provider credentials must not be passed
                ...
                if _is_KOVA_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register kova "
                        "provider credential %r from config.yaml (blocked "
                        "by _KOVA_PROVIDER_ENV_BLOCKLIST). Operator "
