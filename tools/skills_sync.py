...
Copies bundled skills from the repo's skills/ directory into ~/.kova/skills/
...
The manifest lives at ~/.kova/skills/.bundled_manifest.
"""
...
# Marker file written by `kova profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.kova profile).
# When present in KOVA_HOME, sync_skills() is a no-op so neither the
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.kova/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.kova/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.kova/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.kova) that wrote the
    ...
    if (KOVA_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.kova/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``KOVA_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.kova/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``KOVA_HOME``. Require a strict-child relationship so both escape
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `kova skills uninstall`.)"
            ),
            "synced": None,
        }
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``kova skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `kova skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes KOVA_HOME/.no-bundled-skills so the
    ...
    sync. This is the on-disk-state half of ``kova skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = KOVA_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `kova update` (or `kova skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (KOVA_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.kova/skills/ ...")
