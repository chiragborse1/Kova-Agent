...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.kova/config.yaml``. kova
...
Configuration is loaded from ~/.kova/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``kova_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from kova_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.kova/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.kova/config.yaml.
...
# custom voice-cloning scripts, etc.) into kova without any Python code
...
#           output_format: wav
#
# kova writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__KOVA_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when kova actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where kova caches Piper voice models.

    Resolves to ``~/.kova/cache/piper-voices/`` under the active
    KOVA_HOME so voice downloads follow profile boundaries.
    """
    from kova_constants import get_kova_dir
    root = Path(get_kova_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.kova/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'kova-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run kova setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'kova setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'kova tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_kova_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
