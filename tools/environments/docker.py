...
from tools.environments.local import _KOVA_PROVIDER_ENV_BLOCKLIST
...
def _load_KOVA_env_vars() -> dict[str, str]:
    """Load ~/.kova/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active kova profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale kova-tagged containers left behind by prior processes.
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling kova process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=kova-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a kova process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed kova
    process leaves its container behind permanently because there's no
    subsequent kova process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``KOVA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("KOVA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using KOVA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'kova'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``kova-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.kova/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. kova-agent:latest) already use /init as PID 1
        ...
        container_name = f"kova-{uuid.uuid4().hex[:8]}"
        # Labels make kova-created containers identifiable to:
        #   * the orphan reaper (`kova-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`kova-task-id`, `kova-profile`)
        #   * operators running `docker ps --filter label=kova-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active kova profile is captured at
        ...
        # container shared across sessions").  If a prior kova process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic kova secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _KOVA_PROVIDER_ENV_BLOCKLIST)
        KOVA_env = _load_KOVA_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = KOVA_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("kova-task-id", "")
        profile_label = self._labels.get("kova-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"kova-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``kova-*`` but were
        ...
        # container) but can happen if a previous kova process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every kova exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        kova process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next kova startup. That covers the
        ...
        # outlives kova processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"kova-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``kova /quit``
