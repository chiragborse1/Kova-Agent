...
_FORCE_SYNC_ENV = "KOVA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.kova") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.kova to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``KOVA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, KOVA_HOME: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.kova/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="kova-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.kova/skills/a.md`` →
        ``~/.kova/skills/a.md``, a new remote file at
        ``/root/.kova/skills/b.md`` maps to ``~/.kova/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
