...
wrapper, while preserving kova' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_kova_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("kova-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.kova is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.kova"
            )
