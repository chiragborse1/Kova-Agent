...
def register_credential_file(
    ...
    *relative_path* is relative to ``KOVA_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside KOVA_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    KOVA_HOME = _resolve_KOVA_HOME()

    # Reject absolute paths — they bypass the KOVA_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to KOVA_HOME)",
            relative_path,
        )
        return False

    host_path = KOVA_HOME / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape KOVA_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, KOVA_HOME)
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="kova-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching kova_constants.get_kova_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_kova_dir()`` for backward compatibility with old directory layouts.
    """
    from kova_constants import get_kova_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_kova_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.kova``,
    SSH ``<remote_home>/.kova``, etc.) and whether translation is wanted.
