...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("KOVA_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_kova_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $KOVA_HOME/cache/vision/ and cleaned up
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_kova_dir("cache/video", "temp_video_files")
