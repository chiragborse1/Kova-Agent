...
running under the dispatcher (env var ``KOVA_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``kova chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``kova kanban``?
...
Humans continue to use the CLI (``kova kanban …``), the dashboard
(``kova dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``KOVA_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``kova chat`` without the kanban toolset see zero
...
def _worker_run_id(task_id: str) -> Optional[int]:
    ...
    if os.environ.get("KOVA_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("KOVA_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("KOVA_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("KOVA_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``KOVA_KANBAN_TASK`` set
    ...
    ``KOVA_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``KOVA_KANBAN_DB`` → ``KOVA_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting kova.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``KOVA_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``KOVA_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``KOVA_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``KOVA_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("KOVA_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("KOVA_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set KOVA_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # KOVA_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``kova-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("KOVA_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("KOVA_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets KOVA_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("KOVA_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (KOVA_KANBAN_TASK set)
    ...
    # (kanban toolset, no KOVA_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("KOVA_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("KOVA_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``kova dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``KOVA_SESSION_PLATFORM``
      and ``KOVA_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``KOVA_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on KOVA_SESSION_KEY.
            session_key = (
                get_session_env("KOVA_SESSION_KEY", "")
                or os.environ.get("KOVA_SESSION_KEY", "")
            )
            if not session_key:
        ...
        notifier_profile = os.environ.get("KOVA_PROFILE")
