#!/usr/bin/env python3
"""Read the in-app terminal pane in the kova desktop GUI.
...
def read_terminal_tool(
    ...
    if callback is None:
        return tool_error("read_terminal is only available in the kova desktop app.")
...
def check_read_terminal_requirements() -> bool:
    """Desktop GUI only — KOVA_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("KOVA_DESKTOP") or "").strip().lower() in ("1", "true", "yes")


READ_TERMINAL_SCHEMA = {
    "name": "read_terminal",
    "description": (
        "Read what's currently shown in the in-app terminal pane of the kova "
