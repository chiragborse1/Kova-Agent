...
This module provides the glue:
    - ``KovaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Kova Agent"
"""
...
from kova_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__KOVA_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses KOVA_HOME so each profile gets its own OAuth tokens.
    Layout: ``KOVA_HOME/mcp-tokens/``
    """
    try:
        from kova_constants import get_kova_home
        base = Path(get_kova_home())
    except ImportError:
        base = Path(os.environ.get("KOVA_HOME", str(Path.home() / ".kova")))
...
# ---------------------------------------------------------------------------
# KovaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class KovaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        KOVA_HOME/mcp-tokens/<server_name>.json         -- tokens
        KOVA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        KOVA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # kova records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to kova.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://kova-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Kova Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`kova mcp login {server_name}` interactively first to complete "
