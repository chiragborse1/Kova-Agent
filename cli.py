#!/usr/bin/env python3
"""
Kova Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Kova Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills kova-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: kova_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See kova_bootstrap.py for full rationale.
try:
    import kova_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when kova_bootstrap isn't registered in the venv
    # yet — happens during partial ``kova update`` where git-reset landed
...
os.environ["KOVA_QUIET"] = "1"  # Our own modules

import yaml

from kova_cli.fallback_config import get_fallback_chain
from kova_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
...
try:
    from kova_cli.pt_input_extras import (
...
from kova_cli.banner import _format_context_length, format_banner_version_label
...
from kova_cli.browser_connect import (
...
load_KOVA_dotenv(KOVA_HOME=_KOVA_HOME, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.kova/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("KOVA_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.kova/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If KOVA_IGNORE_USER_CONFIG=1 is set (via ``kova chat --ignore-user-config``),
    the user config at ``~/.kova/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({KOVA_HOME}/config.yaml)
    user_config_path = _KOVA_HOME / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("KOVA_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain kova, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me kova - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see kova_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from kova_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and KovaCLI.__init__ checks "default" first.
    ...
    from kova_cli.config import _expand_env_vars
    ...
    # kova_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `kova config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (kova_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && kova` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _KOVA_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_KOVA_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["KOVA_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.kova/logs/.
...
try:
    from kova_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``kova --help`` paths inside the chat command flow).  See
...
from kova_cli.banner import build_welcome_banner
from kova_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from kova_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("KOVA_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("KOVA_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.kova/kova-agent`` clone is updated only by ``kova update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run kova -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"kova-{short_id}"
    branch_name = f"kova/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"kova pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`kova_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        _KOVA_HOME_maint = _get_kova_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_KOVA_HOME_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`kova_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``kova/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("kova-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``kova/kova-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``kova -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("kova/kova-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. KOVA_LIGHT / KOVA_TUI_LIGHT env (true/false) — explicit override
#   2. KOVA_TUI_THEME=light|dark — explicit theme
#   3. KOVA_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy kova assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("KOVA_LIGHT", "KOVA_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("KOVA_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("KOVA_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from kova_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_KOVA_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._KOVA_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in kova output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_KOVA_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_KOVA_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._KOVA_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_KOVA_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._KOVA_bp_start = now
        ...
        _vt100_mod._KOVA_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``KovaCLI._busy_command()``.
        """
        yield self

# ASCII Art - kova-AGENT logo (full width, single line - requires ~95 char terminal)
KOVA_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - kova Caduceus (compact, fits in left panel)
KOVA_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS kova - AI Agent Framework"
        tiny_line = "⚕ NOUS kova"
    else:
        agent_name = _skin.get_branding("agent_name", "Kova Agent") if _skin else "Kova Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("KOVA_FAST_STARTUP_BANNER") == "1":
        from kova_cli import __release_date__ as _release_date
        from kova_cli import __version__ as _version

        version_line = f"Kova Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from kova_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.kova/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.kova/config.yaml on first use)
...
class KovaCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Kova Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: KOVA_MAX_TOKENS)
        _env_mt = os.environ.get("KOVA_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from kova_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("KOVA_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("KOVA_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("KOVA_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `kova chat --ignore-rules` in kova_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("KOVA_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("KOVA_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `kova update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all kova processes for this KOVA_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.kova/checkpoints/.  Opt-in via
        ...
        self._history_file = _KOVA_HOME / ".KOVA_history"
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``kova pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from kova_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'kova'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from kova_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from kova_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from kova_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - KovaCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``kova doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``kova doctor`` so the banner stays
        small.
        """
        try:
            from kova_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   kova needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous kova LLM (not agentic)
        from kova_cli.model_switch import is_nous_KOVA_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_KOVA_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research kova 3 & 4 models are NOT agentic and are not "
                "designed for use with Kova Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.kova/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from kova_cli.clipboard import save_clipboard_image

        img_dir = get_kova_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'kova setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from kova_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from kova_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with kova!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from kova_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from kova_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [kova #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``kova sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `kova sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("KOVA_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.kova/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``kova --resume <id>``
        ...
        try:
            ...
            return
        path = saved_dir / f"KOVA_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: kova --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from kova_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current kova session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from kova_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `kova model` and gateway pickers).
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — kova default (chat_completions)
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_kova_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_kova_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in kova_cli/commands.py instead of touching every dispatch site.
        from kova_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from kova_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from kova_cli.skin_engine import get_active_skin
            ...
            from kova_cli.main import _print_version_info
            ...
            from kova_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `kova plugins
                ...
                # catalog lives behind `kova plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: kova plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_kova_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: kova plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: kova plugins list)")
                    print("  Enable/disable: kova plugins enable/disable <name>")
            except Exception as e:
            ...
            from kova_cli.moa_config import (
                moa_usage,
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from kova_cli.commands import COMMANDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from kova_cli.goals import gather_background_processes as _gather_bg
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``KOVA_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``KOVA_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from kova_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by kova_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors KOVA_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `kova portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `kova portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from kova_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from kova_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `kova portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from kova_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.kova/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_KOVA_HOME / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "KOVA_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "KOVA_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _KOVA_HOME / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard KOVA_HOME, so no -p needed.
            try:
            ...
            print(f"  kova --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  kova -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from kova_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from kova_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from kova_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Kova Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Kova Agent! Type your message or /help for commands."
        ...
        try:
            from kova_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(KOVA_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→kova migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from kova_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in kova_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from kova_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_KOVA_HOME / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting kova binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from kova_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Kova Agent")
        ...
        # Dynamic prompt: shows kova symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("kova needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'kova needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("kova needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = KovaCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_KOVA_osd_patched", False):
                ...
                _pt_renderer._KOVA_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``KOVA_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("KOVA_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the kova process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: kova setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `kova sessions list`
...
def _run_kanban_goal_loop_q(cli: "KovaCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``KOVA_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("KOVA_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from kova_cli import kanban_db as _kb
    from kova_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Kova Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Kova Gateway (messaging platforms)...")
    ...
    # Default to kova-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base kova): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside KovaCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  KOVA_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``kova chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("KOVA_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from kova_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "kova" box, no tool-gen
                        ...
                        if os.environ.get("KOVA_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("KOVA_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from kova_cli.kanban_db import (
                ...
                # Single-query mode (`kova chat -q "…"`): skip the welcome
