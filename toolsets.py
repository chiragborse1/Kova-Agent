...
_KOVA_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on KOVA_DESKTOP
    ...
    # spawned as a kanban worker (KOVA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `kova tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (KOVA_KANBAN_TASK env "
    ...
    # Coding posture (base kova — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in kova_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full kova toolsets (CLI + messaging platforms)
    #
    ...
    # the `kova send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "kova-cli": {
        ...
        "tools": _KOVA_CORE_TOOLS,
        "includes": []
    },

    "kova-cron": {
        # Mirrors kova-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `kova tools` filters
    ...
    "kova-email": {
        "description": "Email bot toolset - interact with kova via email (IMAP/SMTP)",
        "tools": _KOVA_CORE_TOOLS,
        "includes": []
    },
    ...
    "kova-sms": {
        "description": "SMS bot toolset - interact with kova via SMS (Twilio)",
        "tools": _KOVA_CORE_TOOLS,
        "includes": []
    },
    ...
    "kova-webhook": {
        ...
        "tools": _KOVA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``kova-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_KOVA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``kova-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (kova-<name>).
        # Gives them _KOVA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("kova-"):
            platform_name = name[len("kova-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_KOVA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
