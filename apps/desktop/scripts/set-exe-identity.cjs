#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Kova icon + version metadata onto the
// built Kova executable using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Kova executable keeps the stock Electron icon and
...
// so EVERY packed build — first install, `kova desktop`, the installer's
...
// path (which rebuilds via `kova desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Kova executable>
//
...
// Stamp the Kova icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Kova Agent',
      LegalCopyright: 'Copyright (c) 2026 Kova Agent Contributors'
    }
  })

  console.log('[set-exe-identity] done — Kova icon + identity stamped')
}
