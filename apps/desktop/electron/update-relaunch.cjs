 ...
 * Background
 * ----------
 * After `kova update` + `kova desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything KOVA_DESKTOP_* is preserved
// plus KOVA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['KOVA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['KOVA_DESKTOP_']
