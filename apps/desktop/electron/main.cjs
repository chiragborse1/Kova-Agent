...
var require_embed_referer = __commonJS({
  "electron/embed-referer.cjs"(exports2, module2) {
    ...
    var EMBED_SESSION_PARTITION = "persist:kova-embed";
...
var require_bootstrap_platform = __commonJS({
  "electron/bootstrap-platform.cjs"(exports2, module2) {
    ...
    function detectRemoteDisplay2(options = {}) {
      ...
      const override = String(env2.KOVA_DESKTOP_DISABLE_GPU || "").trim().toLowerCase();
      if (GPU_OVERRIDE_ON.has(override)) return "override (KOVA_DESKTOP_DISABLE_GPU)";
...
var require_bootstrap_runner = __commonJS({
  "electron/bootstrap-runner.cjs"(exports2, module2) {
    ...
    function bootstrapCacheDir(kovaHome) {
      return path2.join(kovaHome, "bootstrap-cache");
    }
    function installedAgentInstallScript(kovaHome) {
      if (!kovaHome) return null;
      const candidate = path2.join(kovaHome, "kova-agent", "scripts", installScriptName());
      try {
        fs2.accessSync(candidate, fs2.constants.R_OK);
        return candidate;
      } catch {
    ...
    function cachedScriptPath(kovaHome, commit) {
      return path2.join(bootstrapCacheDir(kovaHome), `install-${commit}.${process.platform === "win32" ? "ps1" : "sh"}`);
    }
    function downloadInstallScript(commit, destPath) {
      const scriptName = installScriptName();
      const url = `https://raw.githubusercontent.com/chiragborse1/Kova-Agent/${commit}/scripts/${scriptName}`;
      return new Promise((resolve, reject) => {
        fs2.mkdirSync(path2.dirname(destPath), { recursive: true });
        const tmpPath = destPath + ".tmp";
        const out = fs2.createWriteStream(tmpPath);
        const req = https2.get(url, (res) => {
    ...
    async function resolveInstallScript({
      ...
      const cached = cachedScriptPath(kovaHome, installStamp.commit);
      try {
    ...
    function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, kovaHome } = {}) {
      return new Promise((resolve, reject) => {
    ...
    function spawnBash(scriptPath, args, { emit, stageName, abortSignal, kovaHome } = {}) {
      return new Promise((resolve, reject) => {
    ...
    function buildPosixPinArgs({ installStamp, activeRoot, kovaHome }) {
    ...
    async function fetchManifest({ scriptPath, installerKind, emit, kovaHome, activeRoot, installStamp }) {
      const isPosix = installerKind === "posix";
      const args = isPosix ? ["--manifest", ...buildPosixPinArgs({ installStamp, activeRoot, kovaHome })] : ["-Manifest", ...buildPinArgs(installStamp)];
...
${result.stdout}`
    ...
    async function runStage({ scriptPath, installerKind, stage, emit, kovaHome, activeRoot, abortSignal, installStamp }) {
      ...
      const args = isPosix ? [
        ...
        "--non-interactive",
        "--json",
        ...buildPosixPinArgs({ installStamp, activeRoot, kovaHome })
    ...
    async function runBootstrap2(opts) {
      ...
      const runLog = openRunLog(logRoot || path2.join(kovaHome, "logs"));
      const emit = (ev) => {
        try {
...
var require_backend_probes = __commonJS({
  "electron/backend-probes.cjs"(exports2, module2) {
    ...
    function canImportKovaCli2(pythonPath) {
      if (!pythonPath) return false;
      try {
        execFileSync2(pythonPath, ["-c", "import kova_cli"], {
          stdio: "ignore",
    ...
    function verifyKovaCli2(kovaCommand, opts = {}) {
      if (!kovaCommand) return false;
      try {
        execFileSync2(kovaCommand, ["--version"], {
          stdio: "ignore",
          timeout: PROBE_TIMEOUT_MS,
    ...
    module2.exports = {
      canImportKovaCli: canImportKovaCli2,
      verifyKovaCli: verifyKovaCli2,
      PROBE_TIMEOUT_MS
    };
  }
});
  ...
  "electron/dashboard-token.cjs"(exports2, module2) {
    ...
    async function fetchPublicText(url, options = {}) {
      ...
      if (protocol2 !== "http:" && protocol2 !== "https:") {
        throw new Error(`Unsupported Kova backend URL protocol: ${protocol2}`);
      }
      const timeoutMs = options.timeoutMs ?? DEFAULT_TOKEN_FETCH_TIMEOUT_MS;
      const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch((error) => {
        if (error.name === "TimeoutError") {
          throw new Error(`Timed out connecting to Kova backend after ${timeoutMs}ms`);
        }
        throw error;
      });
    ...
    function extractInjectedDashboardToken(html) {
      const match = /window\.__KOVA_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ""));
    ...
    async function adoptServedDashboardToken2(baseUrl, spawnToken, { childAlive, label = "Kova backend", ...options }) {
...
var require_backend_ready = __commonJS({
  "electron/backend-ready.cjs"(exports2, module2) {
    "use strict";
    var fs2 = require("node:fs");
    var _READY_RE = /^KOVA_DASHBOARD_READY port=(\d+)/m;
    ...
    function resolvePortAnnounceTimeoutMs(env2 = process.env) {
      const parsed = Number(env2.KOVA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS);
    ...
    function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
      return new Promise((resolve, reject) => {
        ...
        function onExit(code, signal) {
          cleanup();
          reject(new Error(`Kova backend: exited before port announcement (${signal || code})`));
        }
        ...
        const timer = setTimeout(() => {
          cleanup();
          reject(new Error(`Timed out waiting for Kova backend port announcement (${timeoutMs}ms)`));
        }, timeoutMs);
...
var require_vscode_marketplace = __commonJS({
  "electron/vscode-marketplace.cjs"(exports2, module2) {
    ...
    async function fetchMarketplaceThemes2(id) {
      ...
      const vsix = await request(vsixUrl, { headers: { "User-Agent": "Kova-Desktop" } });
...
var require_backend_env = __commonJS({
  "electron/backend-env.cjs"(exports2, module2) {
    ...
    function buildDesktopBackendPath({
      ...
      const kovaNodeBin = kovaHome ? pathModule.join(kovaHome, "node", "bin") : null;
      const venvBin = venvRoot ? pathModule.join(venvRoot, platform === "win32" ? "Scripts" : "bin") : null;
      const saneEntries = platform === "win32" ? [] : POSIX_SANE_PATH_ENTRIES;
      return appendUniquePathEntries([kovaNodeBin, venvBin, currentPath, saneEntries], { delimiter });
    }
    function normalizeKovaHomeRoot2(kovaHome, { pathModule = pathModuleForPlatform(process.platform) } = {}) {
      if (!kovaHome) return kovaHome;
    ...
    module2.exports = {
      ...
      normalizeKovaHomeRoot: normalizeKovaHomeRoot2,
      pathEnvKey
    };
  }
});
  ...
  "electron/hardening.cjs"(exports2, module2) {
    ...
    function encryptDesktopSecret2(value, safeStorageApi) {
      ...
      if (!encryptionAvailable) {
        throw new Error(
          "Secure token storage is unavailable, so Kova Desktop cannot save remote gateway tokens. Set KOVA_DESKTOP_REMOTE_URL and KOVA_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again."
        );
      }
      try {
        return {
        ...
        throw new Error(
          `Failed to encrypt the remote gateway token for secure storage${detail}. Set KOVA_DESKTOP_REMOTE_URL and KOVA_DESKTOP_REMOTE_TOKEN in your environment as a fallback.`
        );
      }
    }
...
var require_update_marker = __commonJS({
  "electron/update-marker.cjs"(exports2, module2) {
    ...
    function markerPath(kovaHome) {
      return path2.join(kovaHome, ".kova-update-in-progress");
    }
    ...
    function readLiveUpdateMarker2(kovaHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
...
var require_update_relaunch = __commonJS({
  "electron/update-relaunch.cjs"(exports2, module2) {
    ...
    var PRESERVED_ENV_KEYS = ["KOVA_HOME", "ELECTRON_DISABLE_SANDBOX"];
    var PRESERVED_ENV_PREFIXES = ["KOVA_DESKTOP_"];
...
var require_git_worktree_ops = __commonJS({
  "electron/git-worktree-ops.cjs"(exports2, module2) {
    ...
    async function addWorktree2(repoPath, options, gitBin) {
      ...
      const branch = sanitizeBranch(opts.branch) || `kova/${slug}`;
...
var require_update_remote = __commonJS({
  "electron/update-remote.cjs"(exports2, module2) {
    "use strict";
    var OFFICIAL_REPO_HTTPS_URL2 = "https://github.com/chiragborse1/Kova-Agent.git";
    var OFFICIAL_REPO_CANONICAL = "github.com/chiragborse1/Kova-Agent";
  ...
  "electron/desktop-uninstall.cjs"(exports2, module2) {
    ...
    function resolveRemovableAppPath2(execPath, platform, env2 = {}) {
      ...
      if (platform === "win32") {
        ...
        if (/[\\/]kova$/i.test(dir2) || /[\\/]kova-desktop$/i.test(dir2)) return dir2;
        return null;
      }
    ...
    function buildPosixCleanupScript2({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, kovaHome }) {
...
var require_connection_config = __commonJS({
  "electron/connection-config.cjs"(exports2, module2) {
    "use strict";
    var AT_COOKIE_VARIANTS = ["__Host-kova_session_at", "__Secure-kova_session_at", "kova_session_at"];
    var RT_COOKIE_VARIANTS = ["__Host-kova_session_rt", "__Secure-kova_session_rt", "kova_session_rt"];
...
var { canImportKovaCli, verifyKovaCli } = require_backend_probes();
...
var { buildDesktopBackendEnv, normalizeKovaHomeRoot } = require_backend_env();
...
var USER_DATA_OVERRIDE = process.env.KOVA_DESKTOP_USER_DATA_DIR || process.env.KOVA_DESKTOP_USER_DATA_DIR;
if (USER_DATA_OVERRIDE) {
  const resolvedUserData = path.resolve(USER_DATA_OVERRIDE);
  fs.mkdirSync(resolvedUserData, { recursive: true });
  app.setPath("userData", resolvedUserData);
}
var DEV_SERVER = process.env.KOVA_DESKTOP_DEV_SERVER || process.env.KOVA_DESKTOP_DEV_SERVER || process.env.KOVA_DESKTOP_DEV_SERVER;
...
var ACTIVE_KOVA_ROOT = path.join(KOVA_HOME, "kova-agent");
var VENV_ROOT = path.join(ACTIVE_KOVA_ROOT, "venv");
var BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_KOVA_ROOT, ".kova-bootstrap-complete");
...
var DEFAULT_UPDATE_BRANCH = "master";
var DESKTOP_LOG_PATH = path.join(KOVA_HOME, "logs", "desktop.log");
...
var BOOT_FAKE_MODE = process.env.KOVA_DESKTOP_BOOT_FAKE || process.env.KOVA_DESKTOP_BOOT_FAKE === "1";
...
var mainWindow = null;
var KovaProcess = null;
var connectionPromise = null;
var backendPool = /* @__PURE__ */ new Map();
var POOL_MAX_BACKENDS = Math.max(1, Number(process.env.KOVA_DESKTOP_POOL_MAX || process.env.KOVA_DESKTOP_POOL_MAX) || 3);
var POOL_IDLE_MS = Math.max(6e4, Number(process.env.KOVA_DESKTOP_POOL_IDLE_MS || process.env.KOVA_DESKTOP_POOL_IDLE_MS) || 10 * 6e4);
...
function applyWindowsNoConsoleSpawnHints(backend) {
  ...
  const usesKovaModule = backend.kind === "python" || Array.isArray(backend.args) && backend.args[0] === "-m" && backend.args[1] === "kova_cli.main";
...
function emitUpdateProgress(payload) {
  ...
  for (const window2 of BrowserWindow.getAllWindows()) {
    window2.webContents.send("Kova:updates:progress", merged);
  }
}
...
function venvKovaShimPath(updateRoot) {
  return IS_WINDOWS ? path.join(updateRoot, "venv", "Scripts", "Kova.exe") : path.join(updateRoot, "venv", "bin", "kova");
}
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== "HEAD") {
          ...
          if (branch2 !== "main") command = `Kova update --branch ${branch2}`;
        }
      } catch {
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot2}`);
      emitUpdateProgress({ stage: "manual", message: command, percent: null });
      return { ok: true, manual: true, command, KovaRoot: updateRoot2 };
    }
    emitUpdateProgress({
      stage: "restart",
      message: "Updating Kova \u2014 this window will close and the updater will open. Don\u2019t reopen Kova yourself; it restarts automatically when the update finishes.",
      percent: 100
    });
...
async function applyUpdatesPosixInApp() {
  ...
  if (desktopChildPids.length) {
    env2.KOVA_DESKTOP_CHILD_PID = desktopChildPids.join(",");
  }
  let branchArgs = [];
  try {
  ...
  const rebuilt = await runRebuildWithRetry((attempt) => {
    ...
    return runStreamedUpdate(Kova, ["desktop", "--build-only"], { cwd: updateRoot, env: env2, stage: "rebuild" });
  });
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: "error",
  ...
  if (!IS_MAC) {
    ...
    if (outcome === "relaunch") {
      ...
      const scriptPath2 = path.join(app.getPath("temp"), `Kova-desktop-update-${Date.now()}.sh`);
      try {
    ...
    if (outcome === "guiSkew") {
      emitUpdateProgress({
        stage: "guiSkew",
        message: "Backend updated, but the desktop app package was not changed. Update or reinstall the Kova desktop app to match.",
        percent: 100
      });
      rememberLog(
    ...
    return {
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: "Backend updated. The rebuilt app can\u2019t relaunch automatically (sandbox helper needs root). Quit and reopen Kova to finish."
    };
  }
  const rebuiltApp = [
...
function resolveWebDist() {
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that is not a real directory: ${fallback}. Static routes will 404. Ensure dist/** is unpacked (asarUnpack) or set KOVA_DESKTOP_WEB_DIST.`
    );
  }
  return fallback;
}
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found \u2014 the desktop app was packaged without a renderer bundle. Tried: ${candidates.join(", ")}. Rebuild with: Kova desktop --force-build`
  );
  return candidates[0];
}
  ...
  if (process.env.KOVA_DESKTOP_IGNORE_EXISTING || process.env.KOVA_DESKTOP_IGNORE_EXISTING !== "1") {
    ...
    if (kovaCommand) {
      ...
      if (verifyKovaCli(kovaCommand, { shell: shellForProbe })) {
        return unwrapWindowsVenvkovaCommand(kovaCommand, dashboardArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === "bootstrap-needed") {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Kova bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ""}: ${bootstrapResult.error || "unknown error"}. Check ${path.join(KOVA_HOME, "logs", "desktop.log")} for the full transcript.`
      );
  ...
  if (!isKovaSourceRoot(ACTIVE_KOVA_ROOT)) {
    throw new Error(
      `Kova install at ${ACTIVE_KOVA_ROOT} is missing or incomplete. Reinstall via the desktop installer or scripts/install.ps1.`
    );
  }
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Git for Windows is required for Kova on Windows (provides Git Bash, which the agent's terminal tool uses). Install it from https://git-scm.com/download/win or run `winget install -e --id Git.Git`, then relaunch Kova."
    );
  }
  const venvPython = getVenvPython(VENV_ROOT);
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (looksHtml || contentType.includes("text/html")) {
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). The endpoint is likely missing on the Kova backend.`
              )
            );
            return;
          }
...
var ZOOM_STORAGE_KEY = "Kova:desktop:zoomLevel";
...
var OAUTH_SESSION_PARTITION = "persist:Kova-remote-oauth";
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === "oauth") {
    if (!await hasLiveOauthSession(baseUrl)) {
      const err = new Error(
        'Remote Kova gateway uses OAuth, but you are not signed in. Open Settings \u2192 Gateway and click "Sign in", or switch back to Local.'
      );
      err.needsOauthLogin = true;
      throw err;
    }
  ...
  if (!token) {
    throw new Error(
      "Remote Kova gateway is selected, but no session token is saved. Open Settings \u2192 Gateway and save a token, or switch back to Local."
    );
  }
  return {
    baseUrl,
...
async function resolveRemoteBackend(profile) {
  ...
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        "KOVA_DESKTOP_REMOTE_URL is set but KOVA_DESKTOP_REMOTE_TOKEN is not. Both must be provided to connect to a remote Kova backend."
      );
    }
...
function terminalShellEnv() {
  ...
  env2.TERM_PROGRAM = "kova";
  env2.TERM_PROGRAM_VERSION = app.getVersion();
  env2.KOVA_DESKTOP_TERMINAL = "1";
  return env2;
}
...
ipcMain.handle("kova:deep-link-ready", () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${KOVA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` + (Object.keys(queued.params).length ? "?" + new URLSearchParams(queued.params).toString() : "")
    );
  }
  return { ok: true };
});
