...
const { canImportKovaCli, verifyKovaCli } = require('./backend-probes.cjs')
...
const { buildDesktopBackendEnv, normalizeKovaHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.KOVA_DESKTOP_USER_DATA_DIR || process.env.KOVA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.KOVA_DESKTOP_DEV_SERVER || process.env.KOVA_DESKTOP_DEV_SERVER || process.env.KOVA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with KOVA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[kova] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[kova] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('kova:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[kova] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[kova] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[kova] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
function resolveKovaHome() {
  if (process.env.KOVA_HOME || process.env.KOVA_HOME) return normalizeKovaHomeRoot(process.env.KOVA_HOME || process.env.KOVA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'kova-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeKovaHomeRoot(fromRegistry)
  }
...
function pathWithkovaManagedNode(...entries) {
  return [...kovaManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
// boots, going straight to `resolveKovaBackend()`. Missing or stale marker
...
//
// Mirror the Python logs (kova_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.KOVA_DESKTOP_BOOT_FAKE || process.env.KOVA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.KOVA_DESKTOP_BOOT_FAKE || process.env.KOVA_DESKTOP_BOOT_FAKE_STEP_MS || process.env.KOVA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'Kova:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.Kova) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.Kova')
}
// Seed the native About panel with the live Kova version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `Kova update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveKovaVersion(),
...
const MEDIA_PROTOCOL = 'Kova-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(Kova-media://…)`.
...
let mainWindow = null
let KovaProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by KovaProcess +
// connectionPromise + startKova(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.KOVA_DESKTOP_POOL_MAX || process.env.KOVA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.KOVA_DESKTOP_POOL_IDLE_MS || process.env.KOVA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning Kova dashboard children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Kova backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-Kova-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('Kova:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the Kova:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:bootstrap:event', ev)
}
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Kova will start automatically when it completes…',
      12
    )
...
function unwrapWindowsVenvkovaCommand(command, dashboardArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^Kova(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Kova no-console Python at ${python}`,
    command: python,
    args: ['-m', 'kova_cli.main', ...dashboardArgs],
    bootstrap: false,
...
function isKovaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'kova_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.KOVA_DESKTOP_PYTHON || process.env.KOVA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Kova deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Kova' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\\kova\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'kova', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'kova', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function getNoConsoleVenvPython(venvRoot) {
  ...
  // and breaks imports (yaml, kova_cli, …) even when PYTHONPATH is patched.
...
function applyWindowsNoConsoleSpawnHints(backend) {
  ...
  const usesKovaModule =
    backend.kind === 'python' ||
    (Array.isArray(backend.args) && backend.args[0] === '-m' && backend.args[1] === 'kova_cli.main')

  if (!usesKovaModule) return backend
...
// install only has PortableGit under %LOCALAPPDATA%\\kova\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'kova', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'kova', 'git', 'bin', 'git.exe'))
  }
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_KOVA_ROOT.
// KOVA_DESKTOP_Kova_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.KOVA_DESKTOP_Kova_ROOT || process.env.KOVA_DESKTOP_Kova || process.env.KOVA_DESKTOP_Kova_ROOT && path.resolve(process.env.KOVA_DESKTOP_Kova_ROOT || process.env.KOVA_DESKTOP_Kova || process.env.KOVA_DESKTOP_Kova_ROOT),
    !IS_PACKAGED && isKovaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isKovaSourceRoot(ACTIVE_KOVA_ROOT) ? ACTIVE_KOVA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_KOVA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('Kova:updates:progress', merged)
  }
}
...
// Path to the venv shim whose lock decides whether `Kova update` can write
// fresh entry points. On Windows this is the file the running backend
// `Kova.exe` holds open; on POSIX it's never mandatory-locked.
function venvKovaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'Kova.exe')
    : path.join(updateRoot, 'venv', 'bin', 'kova')
}
...
// only signals the direct child, so on Windows a backend `Kova.exe` that
// spawned its own grandchildren (a `Kova` REPL, a pty terminal session, the
...
// `KovaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `Kova.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a Kova
// REPL, a pty terminal, the gateway) keep `Kova.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (KovaProcess && Number.isInteger(KovaProcess.pid)) pids.push(KovaProcess.pid)
  ...
  if (KovaProcess && !KovaProcess.killed) {
    try {
      KovaProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvKovaShimPath(updateRoot)
...
// `Kova update`). Instead we spawn the staged kova-setup binary with
// --update and quit, so it can run `Kova update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri kova-setup. Unlike Windows
      ...
      // whole update itself: `Kova update` (backend) + `Kova desktop
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `Kova update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `Kova update` if branch detection fails.
      }
    ...
    // Kova.exe (held by the backend child / its grandchildren) and the update
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvKova = path.join(venvBin, IS_WINDOWS ? 'Kova.exe' : 'kova')
  const updaterArgs = fileExists(venvKova) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the Kova CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `Kova` on PATH.
function resolveKovaCliBinary(updateRoot) {
  const venvKova = path.join(updateRoot, 'venv', 'bin', 'kova')
  if (fileExists(venvKova)) return venvKova
  return findOnPath('kova') || null
}
...
// macOS/Linux in-app update: backend (`Kova update`) + OS-aware GUI rebuild
// (`Kova desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const Kova = resolveKovaCliBinary(updateRoot)
  if (!Kova) {
    emitUpdateProgress({ stage: 'manual', message: 'Kova update', percent: null })
    return { ok: true, manual: true, command: 'Kova update', KovaRoot: updateRoot }
  }

  // Put the kova-managed Node and the venv on PATH so `Kova desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\kova\node, not node\bin.
  const env = {
    KOVA_HOME,
    PATH: pathWithkovaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `Kova update` reaps stale `Kova dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads KOVA_DESKTOP_CHILD_PID and excludes
  ...
  if (KovaProcess && Number.isInteger(KovaProcess.pid)) {
    desktopChildPids.push(KovaProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.KOVA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Kova (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(Kova, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(Kova, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Kova to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `Kova desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Kova…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `Kova-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Kova to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Kova desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Kova to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Kova.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Kova.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Kova to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.Kova-update-new"; then
    rm -rf "$DST.Kova-update-old" 2>/dev/null || true
    mv "$DST" "$DST.Kova-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.Kova-update-new" "$DST"
    rm -rf "$DST.Kova-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Kova to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `Kova
  ...
  return isKovaSourceRoot(ACTIVE_KOVA_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.KOVA_DESKTOP_WEB_DIST || process.env.KOVA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set KOVA_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: Kova desktop --force-build`
  )
  return candidates[0]
}
...
function resolveKovaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Kova.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.KOVA_DESKTOP_CWD || process.env.KOVA_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveKovaCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_KOVA_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return applyWindowsNoConsoleSpawnHints({
    kind: 'python',
    label: `Kova at ${ACTIVE_KOVA_ROOT}`,
    command,
    args: ['-m', 'kova_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      kovaHome: KOVA_HOME,
      pythonPathEntries: [ACTIVE_KOVA_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_KOVA_ROOT,
    bootstrap: true,
    shell: false
  })
}

function resolveKovaBackend(dashboardArgs) {
  // 1. Explicit override -- KOVA_DESKTOP_Kova_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.KOVA_DESKTOP_Kova_ROOT || process.env.KOVA_DESKTOP_Kova || process.env.KOVA_DESKTOP_Kova_ROOT && path.resolve(process.env.KOVA_DESKTOP_Kova_ROOT || process.env.KOVA_DESKTOP_Kova || process.env.KOVA_DESKTOP_Kova_ROOT)
  if (overrideRoot && isKovaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Kova source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `Kova` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isKovaSourceRoot.)
  if (!IS_PACKAGED && isKovaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Kova source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
  ...
  //    (applyUpdates -> git pull) or `Kova update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `Kova` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    KOVA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.KOVA_DESKTOP_IGNORE_EXISTING || process.env.KOVA_DESKTOP_IGNORE_EXISTING !== '1') {
    let kovaCommand = null
    const kovaOverride = process.env.KOVA_DESKTOP_Kova || process.env.KOVA_DESKTOP_Kova

    if (kovaOverride) {
      const resolvedOverride = findOnPath(kovaOverride)
      if (resolvedOverride) {
    ...
    if (kovaCommand) {
      const unwrapped = unwrapWindowsVenvkovaCommand(kovaCommand, dashboardArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `Kova` shim
      ...
      const shellForProbe = isCommandScript(kovaCommand)
      if (verifyKovaCli(kovaCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvkovaCommand(kovaCommand, dashboardArgs) || {
  ...
  // 5. Last-ditch: pip-installed kova_cli module via system Python.
  ...
  if (python) {
    ...
    // having kova_cli installed -- common on dev boxes that have
    ...
    rememberLog(`Ignoring system Python ${python}: kova_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveKovaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Kova Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveKovaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Kova install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Kova recovery was handed off to Kova Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Kova install was cancelled.')
    ...
    return ensureRuntime(resolveKovaBackend(backend.args))
  }
  ...
  if (!isKovaSourceRoot(ACTIVE_KOVA_ROOT)) {
    throw new Error(
      `Kova install at ${ACTIVE_KOVA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Kova' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\\kova\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `Kova` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Kova on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isKovaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Kova venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getNoConsoleVenvPython(VENV_ROOT)
  backend.label = `Kova at ${ACTIVE_KOVA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Kova runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Kova backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Kova backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Kova backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Kova-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('Kova:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveKovaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:preview-file-changed', payload)
}
...
async function waitForKova(baseUrl, token) {
  ...
  throw new Error(`Kova backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Kova...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Kova:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('Kova:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'Kova:desktop:zoomLevel'
...
//
// Hosted Kova gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``kova_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / Kova #37247).
//     Both are set as HttpOnly cookies (``kova_session_at`` ~15 min,
//     ``kova_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:Kova-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Kova backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.KOVA_DESKTOP_REMOTE_URL || process.env.KOVA_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.KOVA_DESKTOP_REMOTE_URL || process.env.KOVA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by KOVA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (Kova #37247), and the
  ...
  if (!token) {
    throw new Error(
      'Remote Kova gateway is selected, but no session token is saved. ' +
...
//   2. env override (KOVA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.KOVA_DESKTOP_REMOTE_URL || process.env.KOVA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.KOVA_DESKTOP_REMOTE_TOKEN || process.env.KOVA_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'KOVA_DESKTOP_REMOTE_URL is set but KOVA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Kova backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.KOVA_DESKTOP_REMOTE_URL || process.env.KOVA_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Kova
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to Kova gateway". Mirror the renderer's connect here so the test
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetKovaConnection() nulls KovaProcess.
  const dying = KovaProcess && !KovaProcess.killed ? KovaProcess : null
  resetKovaConnection()
...
// profile to startKova() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForKova(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
  ...
  const backend = await ensureRuntime(resolveKovaBackend(dashboardArgs))
  const KovaCwd = resolveKovaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Kova backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        KOVA_DESKTOP: '1',
        KOVA_WEB_DIST: webDist,
        ...(readyFile ? { KOVA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Kova backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Kova backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Kova backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Kova backend for profile "${profile}"`,
    rememberLog
  })
...
async function startKova() {
  ...
  connectionPromise = (async () => {
    ...
    if (remote) {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Kova backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Kova runtime', 28)
    const backend = await ensureRuntime(resolveKovaBackend(dashboardArgs))
    const KovaCwd = resolveKovaCwd()
    const webDist = resolveWebDist()
    const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

    await advanceBootProgress('backend.spawn', `Starting Kova backend via ${backend.label}`, 84)
    rememberLog(`Starting Kova backend via ${backend.label}`)
    ...
    KovaProcess.once('error', error => {
      rememberLog(`Kova backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Kova backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    KovaProcess.once('exit', (code, signal) => {
      rememberLog(`Kova backend exited (${signal || code})`)
      KovaProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Kova backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Kova backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentKovaLog()}`
          )
        )
      }
    })
    ...
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null KovaProcess when the child dies.
      childAlive: () => KovaProcess !== null && KovaProcess.exitCode === null && !KovaProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Kova backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Kova is
...
// pushes pet state over IPC (Kova:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via Kova:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Kova app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Kova icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see Kova:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Kova in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('Kova:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startKova().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('kova:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Kova…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('kova:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetKovaConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Kova backend failed liveness probe; dropping stale connection.')
    resetKovaConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('kova:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('kova:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('kova:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('kova:window:openNewSession', async () => {
...
ipcMain.handle('kova:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('kova:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('kova:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('kova:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('kova:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('kova:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('Kova:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('kova:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
...
ipcMain.handle('kova:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startKova() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('kova:bootstrap:cancel', async () => {
...
ipcMain.handle('kova:boot-progress:get', async () => bootProgressState)
ipcMain.handle('kova:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('kova:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('kova:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('kova:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('kova:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('kova:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('kova:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('kova:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('kova:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('kova:profile:set', async (_event, name) => {
...
ipcMain.on('kova:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('kova:requestMicrophoneAccess', async () => {
...
ipcMain.handle('kova:api', async (_event, request) => {
...
ipcMain.handle('kova:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('Kova:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('Kova:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('kova:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('kova:readFileText', async (_event, filePath) => {
...
ipcMain.handle('kova:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('kova:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('kova:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('kova:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('kova:saveClipboardImage', async () => {
...
ipcMain.handle('kova:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('kova:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('kova:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('kova:titlebar-theme', (_event, payload) => {
...
ipcMain.on('kova:native-theme', (_event, mode) => {
...
ipcMain.on('kova:translucency', (_event, payload) => {
...
ipcMain.handle('kova:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('kova:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveKovaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('kova:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveKovaCwd()
}))

ipcMain.handle('kova:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('kova:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('kova:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('kova:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('kova:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
function terminalShellCommand() {
  // KOVA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.KOVA_DESKTOP_SHELL || process.env.KOVA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Kova' TUI into
  ...
  // Let a Kova/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from KOVA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.KOVA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `Kova:terminal:${id}:${suffix}`
}
...
ipcMain.handle('kova:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('kova:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('kova:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('kova:git:branchSwitch', async (_event, repoPath, branch) =>
...
ipcMain.handle('kova:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('kova:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('kova:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('kova:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('kova:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kova:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kova:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kova:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('kova:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('kova:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('kova:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('kova:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('kova:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('kova:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('kova:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Kova.')
  }
...
ipcMain.handle('kova:terminal:write', (_event, id, data) => {
...
ipcMain.handle('kova:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('kova:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('kova:updates:check', async () =>
...
ipcMain.handle('kova:updates:apply', async (_event, payload) =>
...
ipcMain.handle('kova:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('kova:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Kova version (the one `release.py` bumps in
// kova_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Kova version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveKovaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'kova_cli', '__init__.py')
...
// Re-resolve the live Kova version and push it into the native About panel
// just before showing it, so an in-place `Kova update` is reflected without
...
ipcMain.handle('kova:version', async () => ({
  appVersion: resolveKovaVersion(),
...
// the actual removal via `Kova uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (kova_cli/uninstall.py + kova_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_KOVA_ROOT
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'kova_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Kova agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import kova_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_KOVA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `Kova-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `Kova-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('kova:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('kova:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('kova:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('kova:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// Kova:// deep links (e.g. Kova://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const KOVA_PROTOCOL = 'kova'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${KOVA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // Kova://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('Kova:deep-link', payload)
...
ipcMain.handle('kova:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${KOVA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(KOVA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(KOVA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `Kova://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching Kova:// URL is in our own argv.
