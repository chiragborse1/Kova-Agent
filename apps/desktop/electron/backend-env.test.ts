...
test('desktop backend PATH adds Hermes-managed bins and missing POSIX sane entries', () => {
  const result = buildDesktopBackendPath({
    ...
    venvRoot: '/Users/test/.hermes/kova-agent/venv',
  ...
  assert.equal(entries[1], '/Users/test/.hermes/kova-agent/venv/bin')
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  ...
  assert.ok(env.PATH.startsWith('/Users/test/.hermes/node/bin:/Users/test/.hermes/kova-agent/venv/bin:'))
...
test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  const env = buildDesktopBackendEnv({
    ...
    pythonPathEntries: ['C:\\repo\\kova-agent'],
    venvRoot: 'C:\\Users\\test\\AppData\\Local\\hermes\\kova-agent\\venv',
    currentEnv: {
