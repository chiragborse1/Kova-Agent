...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/kova-agent/${commit}/scripts/${scriptName}`
...
async function fetchManifest({ scriptPath, installerKind, emit, kovaHome, activeRoot, installStamp }) {
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, kovaHome })]
