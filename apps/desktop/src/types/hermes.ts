...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when kova doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in KOVA_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `kova model` uses — instead of
...
export type KovaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `kova model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors KOVA_cli/projects_db.Project.to_dict().
...
export interface ToolProvider {
   ...
   *  CLI `kova tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not kova); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/kova/update/check` — the backend's own update state.
