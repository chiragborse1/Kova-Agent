...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.kovaDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.kovaDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.kovaDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.kovaDesktop.openNewSessionWindow(), 'Could not open new session window')
}
