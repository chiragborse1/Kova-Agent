...
function pushNow(): void {
  window.kovaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.kovaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.kovaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.kovaDesktop?.petOverlay?.close()
}
