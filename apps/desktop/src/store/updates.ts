...
const UPDATE_TOAST_SNOOZE_KEY = 'kova:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'kova:backend-skew-toast-snooze-until'
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.kovaDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkKovaUpdate(true))
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `kova update` themselves. Land on a dedicated manual state so the
