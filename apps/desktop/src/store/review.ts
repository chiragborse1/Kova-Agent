...
function reviewCtx(): { cwd: string; review: ReviewBridge } | null {
  const cwd = repoCwd()
  const review = window.kovaDesktop?.git?.review
...
export async function stageReviewFile(path: null | string): Promise<void> {
  await window.kovaDesktop?.git?.review?.stage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function unstageReviewFile(path: null | string): Promise<void> {
  await window.kovaDesktop?.git?.review?.unstage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function revertReviewFile(path: null | string): Promise<void> {
  await window.kovaDesktop?.git?.review?.revert(repoCwd() ?? '', path)
  await afterMutation()
}
