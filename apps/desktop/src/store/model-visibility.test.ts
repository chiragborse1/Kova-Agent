...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    ...
    const visible = effectiveVisibleKeys(stored, [
      provider('nous', ['kova-3-llama-3.1-70b', 'kova-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('nous', 'kova-3-llama-3.1-70b'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    ...
    afterToggle.add(modelVisibilityKey('nous', 'kova-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('nous', 'kova-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nous', 'kova-3-llama-3.1-8b'))).toBe(false)
  })
