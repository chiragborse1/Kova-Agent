...
export const $repoStatus = atom<KovaRepoStatus | null>(null)
...
export const $repoWorktrees = atom<KovaGitWorktree[]>([])
...
async function loadWorktrees(target: string): Promise<void> {
  const list = window.kovaDesktop?.git?.worktreeList
...
export async function refreshRepoStatus(cwd?: null | string): Promise<void> {
  ...
  const probe = window.kovaDesktop?.git?.repoStatus
