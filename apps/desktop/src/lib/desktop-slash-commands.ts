...
/** Why a known kova command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active kova profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Kova Agent version', surface: exec() },
...
function isKnownKovaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * kova' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownKovaSlashCommand(normalized)
}
