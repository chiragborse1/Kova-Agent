...
export const ja = defineLocale({
  ...
  settings: {
    ...
    about: {
      ...
      automaticUpdatesDesc: 'kova はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    gateway: {
      ...
      intro:
        'kova desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している kova バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
  ...
  onboarding: {
    ...
    flowSubtitles: {
      ...
      loopback: 'サインインのためブラウザーを開きます — kova が自動接続します',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'kova 推論ゲートウェイのステータス',
      agents: 'エージェント',
