...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'kova desktop 已就緒',
    desktopBootFailedWithMessage: message => `桌面啟動失敗：${message}`,
    steps: {
      connectingGateway: '正在連線桌面閘道',
      loadingSettings: '正在載入 kova 設定',
      loadingSessions: '正在載入最近工作階段',
      startingDesktopConnection: '正在啟動桌面連線',
      startingkovaDesktop: '正在啟動 kova desktop…'
    },
    errors: {
      backgroundExited: 'kova 背景程序已結束。',
      backgroundExitedDuringStartup: 'kova 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 kova 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    updateKova: '更新 kova',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 kova desktop。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 kova 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 kova 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'kova 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'kova 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '本輪以錯誤結束。'
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 kova 跟隨系統設定。',
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 kova 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 Kova，然後回到此處。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'kova 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'kova 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'kova 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'kova 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'kova 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'kova 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'kova desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 kova 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      envOverrideDesc: '取消設定 KOVA_DESKTOP_REMOTE_URL 和 KOVA_DESKTOP_REMOTE_TOKEN 後才會使用下方儲存的設定。',
      localTitle: '本機閘道',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'kova desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · kova ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。kova 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 kova 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 kova 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 kova 會通知你。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 kova 桌面端' },
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 使用者 ID', placeholder: '@kova:example.org' },
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 kova 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew: '按 cron 表達式排程一個提示詞。kova 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      deleteConfirm: '這會從 kova 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。',
      startWork: '新增工作樹',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 kova...',
    placeholderReconnecting: '正在重新連線至 Kova…',
    ...
    attachUrlDesc: 'kova 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 kova 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 kova 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 kova 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 Kova，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 kova 時會使用新版本。',
    guiSkewTitle: '請更新桌面應用程式',
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 kova 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
  ...
  install: {
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Kova Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 Kova CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 kova 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。kova 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 Kova Agent',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'kova 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 Kova…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 kova 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 kova 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — kova 會自動連線',
      loopback: '開啟瀏覽器登入 — kova 會自動連線',
    ...
    authorizeThere: '請在那裡授權 Kova。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 Kova，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。kova 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'kova 正在重新啟動...',
      askRestart: '請 kova 重新啟動伺服器',
      lookingRestart: taskId => `kova 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'kova 正在背景執行。可在預覽主控台查看進度。',
      ...
      finishedRestarting: message => `kova 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'kova 無法重新啟動伺服器。',
      stillWorking: 'kova 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'kova 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      command: '指令',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.kova/config.yaml）。kova 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'kova 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'kova 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 kova desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 kova desktop 以儲存圖片',
    ...
    handoff: {
      ...
      timedOut: '等待閘道逾時。`Kova Gateway` 是否正在執行？'
    }
  },
