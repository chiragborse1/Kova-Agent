...
const KOVA_PATHS_MIME = 'application/x-kova-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(KOVA_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(KOVA_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (kova --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[kova-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      void window.kovaDesktop?.terminal?.write(id, `${command}\r`)
