...
export const KOVA_PATHS_MIME = 'application/x-kova-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal kova sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  try {
    const internalRaw = transfer.getData(KOVA_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const pickContextPaths = useCallback(
    async (kind: 'file' | 'folder') => {
      const paths = await window.kovaDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(
    async (filePath: string) => {
      ...
      try {
        const previewUrl = await window.kovaDesktop?.readFileDataUrl(filePath)
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.kovaDesktop?.saveImageBuffer(data, blobExtension(blob))
