...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../kova'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.kovaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.kovaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  // kova:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.kovaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshKovaConfig, sttEnabled, voiceMaxRecordingSeconds } = useKovaConfig({
    activeSessionIdRef,
