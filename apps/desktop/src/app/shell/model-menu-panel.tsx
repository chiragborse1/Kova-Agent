...
import { getGlobalModelOptions, getMoaModels } from '@/kova'
...
import type { MoaConfigResponse, ModelOptionProvider, ModelOptionsResponse } from '@/types/kova'
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }: ModelMenuPanelProps) {
  ...
  // session (effort/fast), gated by capability. Unset → kova defaults.
          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // the active model, otherwise its remembered preset (kova
