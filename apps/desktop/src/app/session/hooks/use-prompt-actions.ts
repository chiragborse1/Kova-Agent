...
import { getProfiles, transcribeAudio } from '@/kova'
...
async function readImageForRemoteAttach(filePath: string): Promise<{ contentBase64: string; filename: string } | null> {
  const dataUrl = await window.kovaDesktop?.readFileDataUrl(filePath)
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.kovaDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `Kova Gateway` process performs the actual transfer, so we
