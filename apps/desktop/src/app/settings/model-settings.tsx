...
// kova' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = kova default (medium), shown as Medium.
...
// backend now surfaces the full `kova model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in KOVA_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `kova model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — kova runs the flow for you.`}
          </p>
        )}
