...
export function voiceFieldVisible(key: string, config: KovaConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<KovaConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getKovaConfigRecord(), getKovaConfigDefaults(), getKovaConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
