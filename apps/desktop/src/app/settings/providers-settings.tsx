...
//      in KOVA_cli/provider_catalog.py) — the SAME provider identity
//      `kova model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // kova can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider kova can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // kova never deletes creds another tool owns behind a silent API call.
