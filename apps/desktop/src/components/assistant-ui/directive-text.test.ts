...
import { formatRefValue, kovaDirectiveFormatter } from './directive-text'
...
describe('kovaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = kovaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = kovaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
