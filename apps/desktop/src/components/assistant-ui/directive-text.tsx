...
const KOVA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type KovaRefType = (typeof KOVA_REF_TYPES)[number]
...
const ICON_PATHS: Record<KovaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as KovaRefType] ?? ICON_FALLBACK
...
const KOVA_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(KOVA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as KovaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: KovaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing kova directives (`@file:...`, `@image:...`) as
