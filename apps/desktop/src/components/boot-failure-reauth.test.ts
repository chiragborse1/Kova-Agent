...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'anthropic', displayName: 'Anthropic', supportsPassword: false }])).toEqual({
      isPassword: false,
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'anthropic', displayName: 'Anthropic', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / Anthropic')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Anthropic' })).toBe(
