...
//!
//!   1. wait for the old Kova desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `kova update`
//!      or repair bootstrap can race locked files),
//!   2. run `kova update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hermes_cli/main.py),
//!   3. run `kova desktop --build-only` (the rebuild step update skips),
...
/// install tree before giving up and letting `kova update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // async on Windows. If it still holds the venv shim, `kova update`
    ...
    // --force skips `kova update`'s Windows running-exe guard (which would
    ...
    // time `kova update` runs there is no legitimate kova.exe to protect,
    // and the guard would only produce a false "Kova is still running" stop.
    //
    ...
    // probe only checks the kova.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Kova windows" message.
    ...
    // ---- stage 3: kova desktop --build-only ----------------------------
    // `kova update` deliberately does NOT build apps/desktop (it installs
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend kova.exe (or the desktop Kova.exe
            ...
            // force-kill every Kova.exe except ourselves, then give the OS a
...
/// Force-kill any `kova.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `kova.exe` image tree via
