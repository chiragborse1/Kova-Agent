...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``kova -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("KOVA_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("KOVA_SESSION_SOURCE", "")
...
_loaded_env_paths = load_KOVA_dotenv(KOVA_HOME=_KOVA_HOME, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"KovaAgent/{_KOVA_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[kova-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least kova' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from kova_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``kova chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``KOVA_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``KOVA_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("KOVA_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``KOVA_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("KOVA_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        kova-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See Kova-Agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from kova_cli.models import _should_use_copilot_responses_api
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use kova against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("KOVA_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("kova_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from kova_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``KOVA_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.kova/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects KOVA_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.kova/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``KOVA_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("KOVA_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (KOVA_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (KOVA_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset KOVA_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("KOVA_DEV_CREDITS"))
        ...
        if _dev:
            # KOVA_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `kova logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``kova auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                ...
                from kova_cli.auth import resolve_xai_oauth_runtime_credentials
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("KOVA_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from kova_cli.copilot_auth import resolve_copilot_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from kova_cli.models import copilot_default_headers
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from kova_cli.models import lmstudio_model_reasoning_options
