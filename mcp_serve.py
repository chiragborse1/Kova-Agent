"""
kova mcp Server — expose messaging conversations as MCP tools.
...
class EventBridge:
    ...
    This is the kova equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the kova mcp server with all tools registered."""
    ...
    mcp = FastMCP(
        "kova",
        instructions=(
            "Kova Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the kova mcp server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
