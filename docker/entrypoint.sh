...
# by /etc/cont-init.d/01-kova-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs kova"
...
echo "[kova] WARNING: docker/entrypoint.sh is a deprecated shim under " \
