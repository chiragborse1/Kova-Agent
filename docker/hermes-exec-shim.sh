#!/bin/sh
# shellcheck shell=sh
# /opt/kova/bin/kova — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# returns "Provider authentication failed: kova is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> kova
...
# (/opt/kova/.venv/bin/kova), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set KOVA_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the Kova CLI. Default is to drop.
...
if [ ! -x "$REAL" ]; then
    echo "kova-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user kova`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${KOVA_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the kova user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "kova-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "kova-shim: re-run with --user kova or set KOVA_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the kova user's home before dropping privileges. Without
